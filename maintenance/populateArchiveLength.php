...
/**
 * Maintenance script that populates the ar_len fied
 *
 * @ingroup Maintenance
 */
class PopulateArchiveLength extends LoggedUpdateMaintenance {
	...
	public function doDBUpdates() {
		...
		} else if ( !$db->fieldExists( 'archive', 'ar_sha1', __METHOD__ ) ) {
			$this->output( "ar_sha1 column does not exist\n\n", true );
			return false;
		}
		...
		$missing = 0;
		$fields = array(
			'page', 'id', 'comment', 'user', 'user_text', 'timestamp', 'minor_edit',
			'text_id', 'deleted', 'len', 'sha1', 'content_model', 'content_format'
		);
