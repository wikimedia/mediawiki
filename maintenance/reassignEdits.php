...
class ReassignEdits extends Maintenance {
	...
	private function doReassignEdits( &$from, &$to, $rc = false, $report = false ) {
		...
		$revQueryInfo = ActorMigration::newKey( 'rev_user' )->getWhere( $dbw, $from );
		$res = $dbw->select(
		...
		$arQueryInfo = ActorMigration::newKey( 'ar_user' )->getWhere( $dbw, $from );
		$res = $dbw->select(
		...
		if ( $rc ) {
			...
			$rcQueryInfo = ActorMigration::newKey( 'rc_user' )->getWhere( $dbw, $from );
			$res = $dbw->select(
		...
		if ( !$report ) {
			if ( $total ) {
				...
				if ( $wgActorTableSchemaMigrationStage > MIGRATION_OLD ) {
					$dbw->update(
						'revision_actor_temp',
						[ 'revactor_actor' => $to->getActorId( true ) ],
						[ 'revactor_actor' => $from->getActorId() ],
						__METHOD__
					);
				}
				$this->output( "done.\nReassigning deleted edits..." );
				$dbw->update( 'archive', $this->userSpecification( $to, 'ar_user', 'ar_user_text', 'ar_actor' ),
				...
				if ( $rc ) {
					...
					$dbw->update( 'recentchanges',
						$this->userSpecification( $to, 'rc_user', 'rc_user_text', 'rc_actor' ),
	 ...
	 * @return array
	 */
	private function userSpecification( &$user, $idfield, $utfield, $acfield ) {
		global $wgActorTableSchemaMigrationStage;
		...
		if ( $wgActorTableSchemaMigrationStage > MIGRATION_OLD ) {
			$ret += [ $acfield => $user->getActorId( true ) ];
		}
		return $ret;
	}
