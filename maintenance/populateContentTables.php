...
class PopulateContentTables extends Maintenance {
	...
	public function __construct() {
		...
		$this->addOption( 'table', 'revision or archive table', true, true);
	}
	...
	public function execute() {
		...
		if ( $this->table !== 'revision' && $this->table !== 'archive' ) {
			$this->fatalError( 'Invalid table. Must be either `revision` or `archive`');
		}
	...
	private function getAddress( $row ) {
		if ( $this->table === 'archive' ) {
			return 'ar:' . $row->ar_text_id;
		}

		return 'tt:' . $row->rev_text_id;
	}
