...
class PopulateContentTables extends Maintenance {
	...
	private function populateTable( $table ) {
		...
		$maxId = $this->dbw->selectField( $table, "MAX( $idField )", '', __METHOD__ );
		...
		for ( $startId = 0; $startId <= $maxId; $startId += $batchSize ) {
			...
			$this->writeln( "... $table processed up to revision id $endId" );
		}
			...
			try {
				...
				$this->dbw->insert(
					...
					[
						...
						'content_model' => $this->contentModelStore->acquireId( $this->getContentModel( $row ) ),
						'content_address' => SqlBlobStore::makeAddressFromTextId( $row->text_id ),
					],
					__METHOD__
				);
