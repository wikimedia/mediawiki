...
TEXT
	...
	public function execute() {
		if ( wfReadOnly() ) {
			$this->die( "Wiki is in read-only mode; you'll need to disable it for import to work." );
		}
	...
	private function getNsIndex( $namespace ) {
		...
		$this->die( "Unknown namespace text / index specified: $namespace" );
	}
	...
	function importFromHandle( $handle ) {
		...
		if ( $this->hasOption( 'rootpage' ) ) {
			...
			if ( !$statusRootPage->isGood() ) {
				...
				$this->die( $statusRootPage->getMessage()->text() );
				return false;
			}
		}
