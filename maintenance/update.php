...
class UpdateMediaWiki extends Maintenance {
	...
	function execute() {
		...
		if ( !$wgAllowSchemaUpdates
			...
			$this->die( "Do not run update.php on this wiki. If you're seeing this you should\n"
		...
		if ( substr( $this->getOption( 'schema' ), 0, 2 ) === "--" ) {
			$this->die( "The --schema option requires a file as an argument.\n" );
		} elseif ( $this->hasOption( 'schema' ) ) {
			$file = $this->getOption( 'schema' );
			$this->fileHandle = fopen( $file, "w" );
			if ( $this->fileHandle === false ) {
				$err = error_get_last();
				$this->die( "Problem opening the schema file for writing: $file\n\t{$err['message']}" );
			}
		}
