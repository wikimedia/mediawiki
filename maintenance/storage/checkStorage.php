		...
		for ( $chunkStart = 1; $chunkStart < $maxRevId; $chunkStart += $chunkSize ) {
			...
			if ( count( $externalRevs ) ) {
				...
				foreach ( $res as $row ) {
					...
					list( $proto, ) = $urlParts;
					if ( $proto != 'DB' ) {
						$this->addError( 'restore text', "Error: invalid external protocol \"$proto\"", $row->old_id );
						continue;
					}
			...
			if ( count( $externalNormalBlobs ) ) {
				...
				foreach ( $externalConcatBlobs as $cluster => $xBlobIds ) {
					...
					foreach ( $xBlobIds as $blobId => $oldId ) {
						$this->addError( 'restore text', "Error: missing target $blobId for one-part ES URL", $oldId );
					}
				}
			}
