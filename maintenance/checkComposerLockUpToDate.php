...
class CheckComposerLockUpToDate extends Maintenance {
	...
	public function execute() {
		...
		if ( !file_exists( $lock ) ) {
			...
			$lock = "$IP/vendor/composer.lock";
			if ( !file_exists( $lock ) ) {
				$this->error( 'Could not find composer.lock file. Have you run "composer install"?', 1 );
			}
		}

		$comparer = new ComposerLockComparer( $json, $lock );

		if ( $comparer->isHashUpToDate() ) {
		...
		$found = false;
		$installed = $comparer->getInstalledDependencies();
		foreach ( $comparer->getRequiredDependencies() as $name => $version ) {
