...
class DumpBackup extends BackupMaintenance {
...
TEXT;
		...
		// Actions
		$this->addArg( 'action', 'full, current, logs, stable, revrange' );
		...
		$this->addOption( 'conf', 'Use the specified configuration file (LocalSettings.php)', false, true );
		$this->addOption( 'wiki', 'Only back up the specified wiki', false, true );
	...
	function execute() {
		// Evaluate options specific to DumpBackup
		...
		if ( !$this->hasArg() ) {
			$this->error( 'an action must be specified', 1 );
			return;
		}
