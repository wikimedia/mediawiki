		...
		foreach ( self::$typeMappings[$type] as $logType => $logActions ) {
			...
			foreach ( $res as $row ) {
				...
				if ( $logType === 'delete' ) {
					...
					// FIXME bd808
					/*
					 * 17:54 <AaronSchulz> not sure how to handle the page move
					 *         destination though
					 * 17:55 <AaronSchulz> I guess you can look at move log entries,
					 *         pull out the params and grab the dest
					 * 17:55 <AaronSchulz> doable, a bit code though
					 * 17:55 <AaronSchulz> not too much though
					 *
					 */
				}

				$this->verbose(
					"Purged file {$row->log_title}; {$type} on {$row->log_timestamp}.\n" );
			}
		}
	}
