 ...
 * @ingroup Maintenance
 */
class ScrubDeletedFile extends Maintenance {
	...
	protected function scrubAllVersions( $name ) {
		...
		foreach ( $res as $row ) {
			$this->scubVersion( ArchivedFile::newFromRow( $row ) );
		}
	}

	protected function scubVersion( ArchivedFile $archivedFile ) {
