...
class BatchRowUpdate {
	 ...
	 *
	 * @param callable $output A callback taking a single text parameter to output
	 */
...
class BatchRowWriter {
	...
	/**
	 * @var string $table The table to update
	 */
	protected $table;

	/**
	 * @var string $idField The primary key column of the table to updates
	 */
	protected $idField;
	 ...
	 * @param string       $idField     The primary key column of the table to update
	 */
	public function __construct( DatabaseBase $db, $clusterName, $table, $idField ) {
		$this->db = $db;
	...
	/**
	 * @param [mixed => array] $updates Array mapping row id to the update to run for the row
	 */
...
class BatchRowIterator implements Iterator {
	...
	/**
	 * @var string $table The table to read from
	 */
	protected $table;

	/**
	 * @var string $idField The primary key column of the table being read
	 */
	protected $idField;
	 ...
	 * @param string       $idField   The primary key of the table being read
	 * @param integer      $batchSize The number of rows to fetch per query
	 */
	public function __construct( DatabaseBase $db, $table, $idField, $batchSize ) {
		if ( $batchSize < 1 ) {
	...
	/**
	 * @return array|null The most recently fetched set of rows from the database or null if the iterator is invalid
	 */
	...
	/**
	 * @return integer|null The maximum row id seen so far or null if the iterator is invalid
	 */
	public function key() {
		return $this->maxId;
	}
