...
class CreateAndPromote extends Maintenance {
	...
	public function __construct() {
		...
		foreach( self::$permitRoles as $role ) {
			$this->addOption( $role, "Grant the account {$role} rights" );
		}
	...
	public function execute() {
		...
		if ( $exists && !$password && count( $promotions ) === 0 ) {
			$this->error( "Account exists and nothing to do.", true );
		} else if ( count( $promotions ) !== 0 ) {
			$promoText = "User:{$username} with " . implode( ', ', $promotions ) . "...\n";
			if ( $exists ) {
