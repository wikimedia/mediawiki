...
if ( php_sapi_name() != "cli" ) {
	die( "This is a command line script, it cannot be run in " . php_sapi_name() . " mode\n" );
}
...
class TrimI18n {
	...
	private function load( $path ) {
		...
		if ( $json === false ) {
			throw new MWException( "Can't load JSON data from $path!" );
		}

		$data = json_decode( $json, true );

		if ( !is_array( $data ) ) {
			throw new MWException( "Can't decode JSON data found in $path!" );
		}
	...
	private function prettify( $json ) {
		...
		$tab = $this->tab;
		$callback = function( $matches ) use ( $tab ) {
			return preg_replace( '/    /', $tab, $matches[0] );
		};

		return preg_replace_callback( '/^(    )+/m', $callback, $json );
	}
	...
	private function save( $path, $data ) {
		...
		if ( $json === false ) {
			throw new MWException( "Can't encode data as JSON!" );
		}
		...
		if ( $ok === false ) {
			throw new MWException( "Failed to save JSON data to $path!" );
		}
