...
class GenerateJsonI18n extends Maintenance {
	...
	public function execute() {
		...
		if ( $extension ) {
			if ( $phpfile ) {
				$this->die( "The phpfile is already specified, conflicts with --extension." );
			}
	...
	public function transformI18nFile( $phpfile, $jsondir = null ) {
		...
		if ( !is_dir( $jsondir ) ) {
			...
			if ( !$success ) {
				$this->die( "Could not create directory $jsondir" );
			}
		}
		...
		if ( !isset( $messages ) ) {
			$this->die( "PHP file $phpfile does not define \$messages array" );
		}

		if ( !$messages ) {
			$this->die( "PHP file $phpfile contains an empty \$messages array. " .
				"Maybe it was already converted?" );
		}

		if ( !isset( $messages['en'] ) || !is_array( $messages['en'] ) ) {
			$this->die( "PHP file $phpfile does not set language codes" );
		}
