...
class GenerateJsonI18n extends Maintenance {
	public function __construct() {
		...
		$this->addArg( 'jsondir', 'Directory to write JSON files to. ' .
			'Required unless --update-shim is specified', false );
		$this->addOption( 'langcode', 'Language code; only needed for converting core i18n files',
			false, true );
		$this->addOption( 'update-shim', 'Update an existing backward-compatibility shim ' .
			'generated by an earlier version of this script' );
	}
	...
	public function execute() {
		...
		if ( $this->hasOption( 'update-shim' ) ) {
			$this->updateShim( $phpfile, $jsondir );
			return;
		}

		if ( $jsondir === null ) {
			$this->error( 'Argument [jsondir] is required unless --update-shim is specified.' );
			$this->maybeHelp( true );
		}
	...
	protected function updateShim( $phpfile, $jsondir ) {
		if ( !is_readable( $phpfile ) ) {
			$this->error( "Error reading $phpfile\n", 1 );
		}
