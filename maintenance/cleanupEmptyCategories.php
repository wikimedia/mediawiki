 ...
 * @since 1.28
 */
class CleanupEmptyCategories extends Maintenance {
...
with a description page. It will print out progress indicators every 1000
categories it adds to the table. The script is perfectly safe to run on large,
	...
	public function execute() {
		...
		if ( !$force ) {
			$row = $dbw->selectRow(
				'updatelog',
				'1',
				[ 'ul_key' => 'cleanup empty categories' ],
				__METHOD__
			);
			if ( $row ) {
				$this->output( "Category table already cleaned up. Use php " .
		...
		if ( $dbw->insert(
			...
			$this->output( "Could not insert updatelog record.\n" );
