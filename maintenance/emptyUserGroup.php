...
class EmptyUserGroup extends Maintenance {
	...
	public function execute() {
		...
		$start = $dbw->selectField( 'user_groups', 'MIN(ug_user)',
			array( 'ug_group' => $groupname ), __FUNCTION__ );
		...
		while ( $blockEnd <= $end ) {
			...
			$res = $dbw->select(
				array( 'user_groups' ),
				array( 'ug_user' ),
				array( 'ug_group' => $groupname, "ug_user BETWEEN $blockStart AND $blockEnd" ),
				__METHOD__,
				array( 'IGNORE' )
			);
		...
		$this->output( "\nDone! Removed $count users from user group \"$groupname\".\n" );
	}
}
