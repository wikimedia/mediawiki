...
class GenerateSitemap extends Maintenance {
	...
	public function __construct() {
		...
		$this->addOption( 'skip-redirects', 'Do not include redirecting articles in the sitemap', false, false );
	...
	public function main() {
		...
		foreach ( $this->namespaces as $namespace ) {
			...
			$redirects = 0;  // Number of redirects skipped for that namespace
			foreach ( $res as $row ) {
			  if ($this->skipRedirects && $row->page_is_redirect != 0 ) {
			    $redirects++;
			    continue;
			  }
			...
			if ($this->skipRedirects && $redirects > 0) {
			  $this->output( "  skipped $redirects redirects\n" );
			}
