...
class MigrateActors extends LoggedUpdateMaintenance {
	...
	protected function doDBUpdates() {
		...
		$errors += $this->migrateToTemp(
			'image', 'img_name', [ 'imgactor_timestamp' => 'img_timestamp' ],
			'img_user', 'img_user_text', 'imgactor_name', 'imgactor_actor'
		);
		$errors += $this->migrate(
	...
	protected function migrate( $table, $primaryKey, $userField, $nameField, $actorField ) {
		...
		while ( true ) {
			...
			list( $next, $prompt ) = $this->makePrompt( $dbw, $primaryKey, $lastRow );
			$this->output( "... $prompt\n" );
		}
	...
	protected function migrateToTemp(
		...
		while ( true ) {
			...
			list( $n, $prompt ) = $this->makePrompt( $dbw, [ $primaryKey ], $lastRow );
			$next = [ $n ];
			$this->output( "... $prompt\n" );
		}
	...
	protected function migrateLogSearch() {
		...
		while ( true ) {
			...
			$res = $dbw->select(
				...
				[ 'actor' => [ 'LEFT JOIN', 'ls_value = actor_user' ] ]
			);
			if ( !$res->numRows() ) {
				break;
			}
			...
			foreach ( $res as $row ) {
				...
				if ( !$row->actor_id ) {
					list( , $prompt ) = $this->makePrompt( $dbw, $primaryKey, $row );
					$this->error( "No actor for row with $prompt\n" );
					$countErrors++;
					continue;
				}
				$dbw->update(
		...
		while ( true ) {
			...
			foreach ( $rows as $row ) {
				if ( !$row->actor_id ) {
					list( , $prompt ) = $this->makePrompt( $dbw, $primaryKey, $row );
					$this->error( "Could not make actor for row with $prompt\n" );
					$countErrors++;
					continue;
				}
				$dbw->update(
