...
class CleanupQueryCache extends Maintenance {
	...
	public function execute() {
		...
		if ( $wgMiserMode ) {
			// We're in MiserMode, so QueryPages don't get cached
			// TODO: Just truncate the table?
			$this->output( "\$wgMiserMode is enabled, so QueryPages aren't cached. Nothing to do.\n" );
			return;
                }
		...
		foreach ( $queryCache as $table => $col ) {
			...
			do {
				$query = "DELETE FROM " . $dbw->tableName( $table ) .
					" WHERE $col NOT IN ($qcType) LIMIT 1000";
