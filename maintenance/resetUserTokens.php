...
class ResetUserTokens extends Maintenance {
	public function __construct() {
		...
		$this->addOption( 'batch', 'Query batch size, default 1000', false, true );
	}

	public function execute() {

		$this->nullsOnly = $this->getOption( 'nulls' );
		$this->batchSize = $this->getOption( 'batch', 1000 );
		...
		$counts = $dbr->selectRow( 'user',
			array( 'max' => 'MAX(user_id)' ),
			array(),
			__METHOD__
			);
		$maxid = $counts->max;
	...
	private function updateUser( $userid ) {
		...
		$this->output( 'Resetting user_token for ' . htmlspecialchars( $username ) . ': ' );
		// Change value
		$user->setToken();
		$user->saveSettings();
		$this->output( " OK\n" );
