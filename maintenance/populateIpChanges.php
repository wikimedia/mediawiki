...
TEXT
	...
	public function doDBUpdates() {
		...
		$blockStart = $start;
		$this->output( "blockStart = $blockStart\n" );
		$revCount = 0;
		...
		while ( $blockStart <= $end ) {
			...
			$rows = $dbw->select( 'revision', [ 'rev_id', 'rev_timestamp', 'rev_user_text' ], $cond, __METHOD__ );

			if ( !$rows || $rows->numRows() === 0 ) {
				break;
			}

			$this->output( "...copying revisions starting with rev_id $blockStart\n" );
