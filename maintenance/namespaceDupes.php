...
class NamespaceConflictChecker extends Maintenance {
	...
	public function __construct() {
		...
		$this->addOption( 'add-prefix', "Dupes will be renamed with correct namespace with " .
			"<text> preprended before the article name", false, true );
		$this->addOption( 'source-pseudo-ns', "Move all pages with the given source prefix " .
			"(with an implied colon following it). If --dest-ns is not specified, the colon " .
			"will be replaced with a hyphen.", 
			false, true );
		$this->addOption( 'dest-ns', "In combination with --source-pseudo-ns, specify the namespace " .
			"ID of the destination", false, true );
	...
	public function execute() {
		...
		$options = array(
			...
			'source-pseudo-ns' => $this->getOption( 'source-pseudo-ns', '' ),
			'dest-ns' => intval( $this->getOption( 'dest-ns', 0 ) ) );

		if ( $options['source-pseudo-ns'] !== '' ) {
	...
	private function checkPrefix( $options ) {
		$prefix = $options['source-pseudo-ns'];
		$ns = $options['dest-ns'];
	...
	private function getTargetList( $ns, $name, $options ) {
		$checkNamespaces = $options['move-talk'] ? array( 0, 1 ) : 0;
	...
	private function getDestinationTitle( $ns, $name, $row, $options ) {
		...
		// page_namespace is 0 or 1, so adding it to the destination namespace
		// gives us the relevant subject or talk namespace
		$destNS = $ns + $row->page_namespace;
