...
class MinifyScript extends Maintenance {
	...
	public function execute() {
		if ( !count( $this->mArgs ) ) {
			$this->die( "minify.php: At least one input file must be specified." );
		}

		if ( $this->hasOption( 'outfile' ) ) {
			if ( count( $this->mArgs ) > 1 ) {
				$this->die( '--outfile may only be used with a single input file.' );
			}
	...
	public function getExtension( $fileName ) {
		...
		if ( $dotPos === false ) {
			$this->die( "No file extension, cannot determine type: $fileName" );
		}
