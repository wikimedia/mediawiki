...
class CommandLineInstaller extends Maintenance {
	function __construct() {
		...
		$this->addOption( 'dbpath', 'The path for the SQLite DB (/data)', false, true );
	...
	function execute() {
		global $IP, $wgTitle;
		$siteName = isset( $this->mArgs[0] ) ? $this->mArgs[0] : 'MediaWiki'; // Will not be set if used with --env-checks
		$adminName = isset( $this->mArgs[1] ) ? $this->mArgs[1] : 'WikiSysop';
