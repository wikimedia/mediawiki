...
class WrapOldPasswords extends Maintenance {
	public function __construct() {
		...
		$this->addOption( 'type', 'Password type to wrap passwords in (must inherit LayeredParameterizedPassword)', true, true );
	...
	public function execute() {
		...
		$passObj = Password::newFromType( $layeredType );
		...
		list( $typeClass, $typeConfig ) = $typeInfo[$layeredType];
		...
		do {
			...
			foreach ( $res as $row ) {
				...
				$layeredPassword = Password::newFromType( $layeredType );
