...
class RemoveUnusedAccounts extends Maintenance {
	...
	public function execute() {
		...
		$res = $dbr->select( 'user',
			array( 'user_id', 'user_name', 'user_touched' ), '', __METHOD__ );
		...
		foreach ( $res as $row ) {
			...
			if ( count( array_intersect( $instance->getEffectiveGroups(),
						$excludedGroups ) ) == 0
				&& $instance->isInactiveAccount( $row->user_id, true )
		...
		if ( $count > 0 && $this->hasOption( 'delete' ) ) {
			...
			$dbw->delete( 'user_groups', array( 'ug_user' => $del ),
				__METHOD__ );
			$dbw->delete( 'user_former_groups', array( 'ufg_user' => $del ),
				__METHOD__ );
			$dbw->delete( 'user_properties', array( 'up_user' => $del ),
				__METHOD__ );
