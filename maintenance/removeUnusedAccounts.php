...
class RemoveUnusedAccounts extends Maintenance {
	public function __construct() {
		...
		$this->addOption( 'ignore-groups', 'List of comma-separated groups ' .
						  'to exclude', false, true );
		$this->addOption( 'ignore-touched', 'Skip accounts touched in last N ' .
						  ' days', false, true );
	}
	...
	public function execute() {
		...
		$res = $dbr->select( 'user', [ 'user_id', 'user_name', 'user_touched' ],
							 '', __METHOD__ );
		...
		if ( !ctype_digit( $touched ) ) {
			$this->error( "Please put a valid positive integer on the " .
		...
		foreach ( $res as $row ) {
			...
			if ( count( array_intersect( $instance->getEffectiveGroups(),
										 $excludedGroups ) ) == 0
		...
		if ( $count > 0 && $this->hasOption( 'delete' ) ) {
			...
			self::removeUnusedAccounts( $del, $this );
		} elseif ( $count > 0 ) {
			$this->output( "\nRun the script again with --delete to remove " .
						   "them from the database.\n" );
		}
		$this->output( "\n" );
	}

	public static function removeUnusedAccounts( array $id,
												 Maintenance $that ) {
		$dbw = $that->getDB( DB_MASTER );
		$dbw->delete( 'user', [ 'user_id' => $del ], __METHOD__ );
		$dbw->delete( 'user_groups', [ 'ug_user' => $del ], __METHOD__ );
		$dbw->delete( 'user_former_groups', [ 'ufg_user' => $del ],
					  __METHOD__ );
