	...
	private function isInactiveAccount( $id, $actor, $master = false ) {
		...
		if ( $migrations === null ) {
			...
			foreach ( $checks as $table => $prefix ) {
				$migrations[$table] = ActorMigration::newKey( "{$prefix}_user" );
			}
			$migrations['logging'] = ActorMigration::newKey( 'log_user' );
		}
		...
		foreach ( $checks as $table => $fprefix ) {
			$actorQuery = $migrations[$table]->getWhere( $dbo, $user, $prefix !== 'oi' && $prefix !== 'fa' );
		...
		$actorQuery = $migrations['logging']->getWhere( $dbo, $user, false );
		$count += (int)$dbo->selectField(
