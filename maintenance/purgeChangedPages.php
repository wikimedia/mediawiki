...
class PurgeChangedPages extends Maintenance {
	...
	public function execute() {
		...
		while ( true ) {
			...
			$res = $dbr->select(
				array( 'page', 'revision' ),
				array( 'rev_id', 'rev_timestamp', 'page_namespace', 'page_title' ),
				array(
					"( rev_timestamp = {$encMinTime} AND rev_id > {$lastSeenId} ) OR rev_timestamp > {$encMinTime}",
					"rev_timestamp <= {$encMaxTime}",
				),
				__METHOD__,
				array(
					'ORDER BY' => array( 'rev_timestamp', 'rev_id' ),
			...
			// Track max id and timestamp for next query
			$lastSeenId = $lastRow->rev_id;
			$encMinTime = $dbr->addQuotes( $dbr->timestamp( $lastRow->rev_timestamp ) );
