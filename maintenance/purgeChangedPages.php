...
class PurgeChangedPages extends Maintenance {
	...
	public function execute() {
		...
		// Try to purge the same pages too many times
		$cache = new ProcessCacheLRU( 100000 ); // @TODO: use lighter weight LRUMap class
		...
		while ( true ) {
			...
			if ( !$res->numRows() ) {
				break; // nothing more found so we are done
			}

			// Kludge to not get stuck in loops for batches with the same timestamps
			...
			$this->output( "Doing changes from $minTime to $lastTime.\n" );
			...
			$urls = array();
			foreach ( $rows as $row ) {
				if ( $cache->get( (int)$row->page_id, self::HIT ) === false ) {
					$cache->set( (int)$row->page_id, self::HIT, true );
					$title = Title::makeTitle( $row->page_namespace, $row->page_title );
					$urls[] = $title->getInternalURL();
				} else {
					$this->output( "Already purged page {$row->page_id}\n" );
				}
			}
	 ...
	 *
	 * @param ResultWrapper $res Query result sorted by $column
	 * @param string $column
