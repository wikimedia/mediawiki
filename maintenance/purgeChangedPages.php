...
class PurgeChangedPages extends Maintenance {
	...
	public function execute() {
		...
		// Try not to purge the same pages too many times
		...
		while ( true ) {
			...
			$urls = array();
			foreach ( $rows as $row ) {
				if ( $cache->get( (int)$row->page_id, self::SEEN ) === null ) {
					$cache->set( (int)$row->page_id, self::SEEN, true );
					$title = Title::makeTitle( $row->page_namespace, $row->page_title );
					$urls[] = $title->getInternalURL();

				} else if ( $this->hasOption( 'verbose' ) ) {
					$this->output( "Already purged page with ID {$row->page_id}\n" );
				}
			}
