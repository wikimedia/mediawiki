...
class MigrateFileRepoLayout extends Maintenance {
	...
	public function execute() {
		$layout = $this->getOption( 'newlayout' );
		...
		do {
			...
			foreach ( $res as $row ) {
				...
				foreach ( $file->getHistory() as $ofile ) {
					...
					$spath = $ofile->getPath(); // source paths may be silently translated
					if ( $layout === 'sha1' ) {
						$dpath = "{$origBase}/{$sha1[0]}/{$sha1[1]}/{$sha1[2]}/{$sha1}.{$ext}";
					} else {
						$dpath = $ofile->getPath(); // destination paths are used literally
					}
					$status = $be->prepare( array( 'dir' => dirname( $dpath ) ) );
					if ( !$status->isOK() ) {
						$this->error( print_r( $status->getErrorsArray(), true ) );
					...
					$batch[] = array( 'op' => 'copy', 'src' => $spath, 'dst' => $dpath,
						'overwriteSame' => true, 'img' => $ofile->getArchiveName() );
				}
		...
		do {
			...
			foreach ( $res as $row ) {
				...
				// @note: source paths may be silently translated
				$spath = $be->getContainerStoragePath( "{$repo->getName()}-deleted" ) .
					'/' . $repo->getDeletedHashPath( $sha1Key ) . $sha1Key;
				if ( $layout === 'sha1' ) {
					$dpath = "{$origBase}/{$sha1Key[0]}/{$sha1Key[1]}/{$sha1Key[2]}/{$sha1Key}";
				} else {
					$dpath = $spath; // destination paths are used literally
				}
