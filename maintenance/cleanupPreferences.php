...
class CleanupPreferences extends Maintenance {
	public function __construct() {
		...
		$this->addOption( 'unknown',
			'Drop unknown preferences (not in $wgDefaultPreferences or a gadget or userjs)' );
	 ...
	 *   3) Normalize accepted preference values. This is the biggest part of the work.
	...
	public function execute() {
		...
		// control those names.
		if( $this->hasOption( 'unknown' ) ) {
			$this->deleteByWhere(
				$dbw,
				'Dropping unknown preferences',
				[
					'user_property NOT' . $db->buildLike( 'gadget-', $db->anyString() ),
					'user_property NOT' . $db->buildLike( 'userjs-', $db->anyString() ),
		...
		while ( true ) {
			...
			foreach( $res as $row ) {
				if ( $this->hasOption( 'dry-run' ) ) {
					$this->output( "\t\tDry run, would drop: " . var_export( $row, true ) );
					continue;
				}
				$this->beginTransaction( $dbw );
				$dbw->delete(
					'user_properties',
					[
						'up_user'     => $row->up_user,
