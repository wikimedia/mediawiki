...
class CleanupPreferences extends Maintenance {
	 ...
	 *
	 * @param $db IDatabase A database connection
	 * @param $user int|string
	 * @param $property string|null
	 * @return string
	 */
	private function buildSelectCond( IDatabase $db, $user, $property ) {
		if ( $property === null ) {
	 ...
	 *
	 * @param $db IDatabase A database connection
	 * @param $rows array Rows to build condition from
	 * @return string
	 */
	private function buildDeleteCond( IDatabase $db, array $rows ) {
		$conds = array();
		foreach ( $rows as $row ) {
			$conds[] = $db->makeList(
	 ...
	 *
	 * @param $statsDeletedProperty array
	 */
