	...
	public function execute() {
		...
		foreach ( $class::$lists as $list => $properties ) {
			...
			foreach ( $rows as $row ) {
				...
				$entry = new $class;
				$entry->{$class::getIdColumn()} = $row->id;
				$entry->{$class::getShardColumn()} = $row->shard;
		...
		foreach ( $rows as $row ) {
			$class::loadFromRow( $row )
				...
				->cache()
				// completely rebuild lists & counts
				->updateLists()
