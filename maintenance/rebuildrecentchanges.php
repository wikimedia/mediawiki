...
class RebuildRecentchanges extends Maintenance {
		...
		foreach ( $res as $row ) {
			...
			$dbw->insert(
				...
				[
					...
					+ $rcActorMigration->getInsertValues( $dbw, $user ),
				__METHOD__
			);
	...
	private function rebuildRecentChangesTablePass4() {
		...
		if ( $botgroups ) {
			...
			if ( $botusers ) {
				$actorQuery = ActorMigration::newKey( 'rc_user' )->getWhere( $dbw, $botusers, false );
				$rcids = [];
		...
		if ( !$wgMiserMode && $autopatrolgroups ) {
			...
			if ( $patrolusers ) {
				$actorQuery = ActorMigration::newKey( 'rc_user' )->getWhere( $dbw, $patrolusers, false );
				foreach ( $actorQuery['orconds'] as $cond ) {
					$dbw->update(
						'recentchanges',
						[ 'rc_patrolled' => 1 ],
