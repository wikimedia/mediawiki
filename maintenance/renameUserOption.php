...
class UserOptionRenamer extends Maintenance {
	...
	private function renameOption( $oldOption, $newOption ) {
		...
		while( $startId < $maxUser ) {
			$totalAffectedUsers += $this->updateOptionsBatch( $oldOption, $newOption, $startId, $maxId );
	 ...
	 * @return int
	 */
	private function updateOptionsBatch( $oldOption, $newOption, $startId, $maxId ) {
		$userIds = $this->getUserBatchWithOption( $oldOption, $newOption, $startId, $maxId );
	 ...
	 * @return int[]
	 */
	private function getUserBatchWithOption( $oldOption, $newOption, $startId, $maxId ) {
		$userIds = array();
		...
		while( $row = $result->fetchRow() ) {
			$newResult = $this->dbw->select(
				'user_properties',
				array( 'up_user' ),
				array(
					'up_property' => $newOption,
					'up_user' => $row['up_user']
				),
				__METHOD__
			);

			if ( $newResult->numRows() === 0 ) {
				$userIds[] = $row['up_user'];
			}
		}
