...
class SchemaGenerator {
	...
	private function makeSQLComment( string $scriptName, string $jsonPath ): string {
		...
		$canonicalJsonPath = $this->normalizePath( $jsonPath, $IP, $wgExtensionDirectory );
	 ...
	 *
	 * @param string $jsonPath Path to an existing schema file
	 * @param string $IP Root of MediaWiki core installation
	 * @param string $extensionDirectory Root of dir holding MediaWiki extensions
	 * @return string the relative path
	 */
	public static function normalizePath( string $jsonPath, string $IP, string $extensionDirectory ): string {
		...
		if ( DIRECTORY_SEPARATOR === '\\' ) {
			...
			$extensionDirectory = strtr( $extensionDirectory, '\\', '/' );
		}

		if ( str_starts_with( $jsonPath, $extensionDirectory ) ) {
			// Strip $wgExtensionDirectory which might be configured outside of $IP
			// /path/to/mw-ext-all/Example/bar -> Example/bar
			$canonicalJsonPath = str_replace( "$extensionDirectory/", '', $jsonPath );

			// Strip the extension name
			// Example/bar -> bar
			$canonicalJsonPath = explode( '/', $canonicalJsonPath, 2 )[1];
		} else {
			// For mediawiki/core we strip $IP
			// /path/to/mediawiki/foo/bar                -> foo/bar
