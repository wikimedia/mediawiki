...
class RefreshFileHeaders extends Maintenance {
	...
	public function execute() {
		...
		do {
			...
			if ( strlen( $end ) ) {
				$conds[] = array( "img_name <= {$dbr->addQuotes( $end )}" );
			}
		...
		$this->output( "Done. Updated headers for $count files.\n" );
	}
	...
	protected function updateFileHeaders( File $file, array $headers ) {
		...
		if ( !$status->isGood() ) {
			$this->error( "Encountered error: " . print_r( $status, true ), 1 ); // die
		}
	}
}
