...
$extensions = array_keys( $this->config->extensions );
$this->config->files = array_filter( $_head_files, function ( $file ) use ( $extensions ) {
	$pinfo = pathinfo( $file );
	return in_array(
		strtolower( $pinfo['extension'] ), $extensions );
} );
