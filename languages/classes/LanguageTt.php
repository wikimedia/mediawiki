...
// https://tt.wikipedia.org/wiki/%D0%A4%D0%B0%D0%B9%D0%BB:Tatar_telenen_orfografiase_10.jpg
		...
		*/
		...
		// $text = '('. $text. ')';
		if(
			preg_match( '/суүсем|акъегет|әрдоган/u', $text )
		...
		}elseif(preg_match( '/^көн(чыгыш|батыш)/u', $text )){
			...
			// other russian words
			|| preg_match( '/^к(андидат|урс)/u', $text )
			|| preg_match( '/^нигерия|актив|импер|ефрат/u', $text )
		){
			...
			|| preg_match( '/ьә/u', $text )
			// other arabic words
			|| preg_match( '/куф/u', $text )
		){
	...
	function convertTatarWordWithArabicRootFromCyrillicToLatin( $text ){
		...
		if( $this->currentWordCapitalisationType == '' )
			$this->replaceIntoArray( '/(?<=[qğ])ы(?=й.+[әеүи])/u', '',
				$text, $this->currentWordTargetLetterCase );
		$text = preg_replace( '/(?<=[qğ])ый(?=.+[әеүи])/u', 'i', $text );
		$text = preg_replace( '/(?<=[qğ])ы(?=я.+[әеүи])/u', 'i', $text );
		$text = preg_replace( '/(?<=[qğ])ы(?=.+[әеүи])/u', 'e', $text ); // i
