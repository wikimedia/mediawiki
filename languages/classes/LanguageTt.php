...
class TtConverter extends LanguageConverter {
	...
	function convertTatarWordWithArabicRootFromCyrillicToLatin( $text ){
		...
		// әдәбийат in schools, and it is considered correct by many people and it is more
	...
	function convertTatarWordFromLatinToCyrillic( $text ) {
		...
		*/
		...
		$text = preg_replace( '/(?<=^taş)ya(?=q)/u', 'ъя', $text );
		// ya after vowels
		...
		// yı after vowels
		$text = preg_replace( '/(?<=[aıou])yı/u', 'е', $text );
		// yu -> ю
		// yu as first letter
		...
		$text = preg_replace( '/(?<=^b)yu/u', 'ю', $text );
		...
		// ye after vowels
		$text = preg_replace( '/(?<=[aäö])ye/u', 'е', $text );
		...
		$text = preg_replace( '/(?<=fran)ts/u', 'ц', $text );
			...
			for ( $i = 0; $i < $wordsCount; $i++ ) { // this should be optimised with using i+=2
				...
				if( preg_match( '/^КамАЗ[а-яёәөүҗңһ]*$/u', $words[$i] ) ){
					...
					for( $j = 1; $j < $wordsICount; $j++ ){
						...
						$words[$i][$j] =
							$this->convertTatarWordFromCyrillicToLatin( $words[$i][$j] );
