...
class TtConverter extends LanguageConverter {
	...
	function convertLowercasedTatarWordCyrlToLat( $text ) {
		...
		if ( preg_match( '/^(көн(чыгыш|батыш)|гөлбакча|ч[иы]ная[кг]|башкор[тд]'
			...
			. '|кайбер|һәрчак|унъел|кол(мәт|сәет|мәмәт)|күктау|(акъ|бай)егет|билбау'
			. '|күзал)/u', $text ) ) {
		...
		if (
			...
			|| preg_match(
				...
				. '|а(ук|на|м(аш|бия)|зимов|р(ант|сия)'
				. '|л(мудуг|лия)|биния|й$|рхипел|ити)'
				. '|(о(р(илла|ки|ьк)|а|би|лланд))'
				. '|вин|у(м(анитар|ми)|еве)|бан)/u',
				$text )
			...
			|| preg_match(
				'/^(б(а(гира|лабан|бынин|вар|рнаул|нк|йт|гама)'
					...
					. '|тагил|х(орватия|ими|арьк)|маугли|даут'
			...
			|| preg_match(
				'/^(м(у(зыка|скул)|елвин|а(даг|р(шан|ка)|йка))'
					...
					. '|х(авьер|орват)|уаскаран|марк|эпикур|фолликул'
	...
	function processWordWithRussianStemCyrlToLat( $text ) {
		...
		$parts = preg_split(
			'/(?<!аматур)(?<!о)(?<!^ма)(?<!^хок|бан)(?<!^(физи|окру|нинс))(?=[гк][ые]($|[^нтр]))(?!гель)/u',
			$text, null, PREG_SPLIT_NO_EMPTY );
		if ( count( $parts ) == 2 ) {
			return $this->convertRussianWordWithTatarEndingCyrlToLat( $parts );
		}
		...
		if ( preg_match( '/(?<!ховн|рьск)[еао](?=(е$|ен))/u', $text ) ) {
