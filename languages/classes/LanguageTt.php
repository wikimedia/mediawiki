...
class TtConverter extends LanguageConverter {
		...
		*/
		if( 0 < preg_match_all(
			...
			foreach( $matches[0] as $match ){
				...
				if( $this->areValidSuffixesForQal( $parts[1] ) ){
					return $this->processWordWithTurkicStemCyrlToLat( $text );
				}
			}
		}
		if( preg_match( '/^а[ув]/u', $text ) ){
			// $parts = preg_split( '/(?<^..)/', $text, 2, PREG_SPLIT_NO_EMPTY );
			// if( isset( $parts[1] ) ){
			if( mb_strlen( $text ) <= 2 ){
				return $this->processWordWithTurkicStemCyrlToLat( $text );
			}
			if( $this->areValidSuffixesForAw( $text ) ){
				return $this->processWordWithTurkicStemCyrlToLat( $text );
			}
		}
		// suffixes for verbs
	...
	function areValidSuffixesForQal( $text ) { // text is for suffixes
		if( preg_match( '/^(ы[нл])/u', $text, $matches ) ) {
			$suffix = $matches[0];
			$parts = preg_split( '/(?<=^'.$suffix.')/u', $text, 2, PREG_SPLIT_NO_EMPTY );
			if( isset( $parts[1] ) ) {
				return $this->areValidSuffixesForAlon( $parts[1] );
			}
			return true;
		}
		if( preg_match( '/^ыш/u', $text, $matches ) ) {
			$suffix = $matches[0];
			$parts = preg_split( '/(?<=^'.$suffix.')/u', $text, 2, PREG_SPLIT_NO_EMPTY );
			if( isset( $parts[1] ) ) {
				if( $this->areValidSuffixesForAlos( $parts[1] ) ){
					return true;
				}
				return $this->areValidSuffixesForAwos( $parts[1] );
			}
			return true;
		}
		if( preg_match( '/^дыр/u', $text, $matches ) ) {
			$suffix = $matches[0];
			$parts = preg_split( '/(?<=^'.$suffix.')/u', $text, 2, PREG_SPLIT_NO_EMPTY );
			if( isset( $parts[1] ) ) {
				return $this->areValidSuffixesForAwdar( $parts[1] );
			}
			return true;
		}
		if( preg_match( '/^м[аы]/u', $text, $matches ) ) {
			$suffix = $matches[0];
			$parts = preg_split( '/(?<=^'.$suffix.')/u', $text, 2, PREG_SPLIT_NO_EMPTY );
			if( isset( $parts[1] ) ) {
				return $this->areValidSuffixesForAwma( $parts[1] );
			}
			return true;
		}
		if( preg_match( '/^а/u', $text, $matches ) ) {
			$suffix = $matches[0];
			$parts = preg_split( '/(?<=^.)/u', $text, 2, PREG_SPLIT_NO_EMPTY );
			if( isset( $parts[1] ) ) {
				if( $this->areValidSuffixesForAwgan( $parts[1] ) ) {
					return true;
				}
				if( preg_match( '/^ар/u', $text, $matches ) ) {
					$suffix = $matches[0];
					$parts = preg_split( '/(?<=^..)/u', $text, 2, PREG_SPLIT_NO_EMPTY );
					if( isset( $parts[1] ) ) {
						return $this->areValidSuffixesForAwgan( $parts[1] );
					}
					return true;
				}
				if( preg_match( '/^ак/u', $text, $matches ) ) {
					$suffix = $matches[0];
					$parts = preg_split( '/(?<=^..)/u', $text, 2, PREG_SPLIT_NO_EMPTY );
					if( isset( $parts[1] ) ) {
						return $this->areValidSuffixesForAwos( $parts[1] );
		...
		if( preg_match( '/^(ган|сын|ы([рм]|йм|ң|гыз)|у|ма)/u', $text, $matches ) ) {
			$suffix = $matches[0];
			$parts = preg_split( '/(?<=^'.$suffix.')/u', $text, 2, PREG_SPLIT_NO_EMPTY );
			if( isset( $parts[1] ) ) {
				return $this->areValidSuffixesForAwgan( $parts[1] );
		...
		if( preg_match( '/^(ак|ы(к|п|йк))/u', $text, $matches ) ) {
			$suffix = $matches[0];
			$parts = preg_split( '/(?<=^'.$suffix.')/u', $text, 2, PREG_SPLIT_NO_EMPTY );
			if( isset( $parts[1] ) ) {
				return $this->areValidSuffixesForAwos( $parts[1] );
		...
		if( preg_match( '/^(ды|са)/u', $text, $matches ) ) {
			$suffix = $matches[0];
			$parts = preg_split( '/(?<=^'.$suffix.')/u', $text, 2, PREG_SPLIT_NO_EMPTY );
			if( isset( $parts[1] ) ) {
				return $this->areValidSuffixesForQalsa( $parts[1] );
			}
			return true;
		}
		return false;
	...
	function areValidSuffixesForAw( $text ) { // text is for whole word
		if( preg_match( '/^а(у(дар)|в(ы[нлш]))/u', $text, $matches ) ) {
			$suffix = $matches[2] != '' ? $matches[2] : $matches[3];
			$parts = preg_split( '/(?<=^..'.$suffix.')/u', $text, 2, PREG_SPLIT_NO_EMPTY );
			if( isset( $parts[1] ) ) {
				return $this->areValidSuffixesForAwdar( $parts[1] );
			}
		...
		if( preg_match( '/^аум[аы]/u', $text, $matches ) ) {
			// $suffix = $matches[2] != '' ? $matches[2] : $matches[3];
			$parts = preg_split( '/(?<=^....)/u', $text, 2, PREG_SPLIT_NO_EMPTY );
			if( isset( $parts[1] ) ) {
				return $this->areValidSuffixesForAwma( $parts[1] );
		...
		if(
			preg_match(
			'/^а(у(ды|ган|с(а|ын))|в(а[рк]?|ы([ркпмш]|й[км]|ң|гыз)|у|ма))/u',
			$text, $matches )
		) {
			$suffix = $matches[2] != '' ? $matches[2] : $matches[4];
			$parts = preg_split( '/(?<=^..'.$suffix.')/u', $text, 2, PREG_SPLIT_NO_EMPTY );
			if( isset( $parts[1] ) ) {
				return $this->areValidSuffixesForAwgan( $parts[1] );
			}
	...
	function areValidSuffixesForAwdar( $text ){
		if( preg_match( '/^(т|ы[нлш])/u', $text, $matches ) ) {
		...
		if(
			preg_match(
			'/^(ды|ган|с(а|ын)|а[рк]?|ы([ркпмш]|й[км]|ң|гыз)|у|ма)/u',
			$text, $matches )
		) {
			$parts = preg_split( '/(?<=^'.$matches[0].')/u', $text, 2, PREG_SPLIT_NO_EMPTY );
			if( isset( $parts[1] ) ) {
	...
	function areValidSuffixesForAlon( $text ) {
		if( preg_match( '/^(дыр|ы[нлш])/u', $text, $matches ) ) {
			$parts = preg_split( '/(?<=^'.$matches[0].')/u', $text, 2, PREG_SPLIT_NO_EMPTY );
			if( isset( $parts[1] ) ) {
				return $this->areValidSuffixesForAwdar( $parts[1] );
			}
	...
	function areValidSuffixesForAlos( $text ) {
		if( preg_match( '/^(тыр|ы[нлш])/u', $text, $matches ) ) {
			$parts = preg_split( '/(?<=^'.$matches[0].')/u', $text, 2, PREG_SPLIT_NO_EMPTY );
			if( isset( $parts[1] ) ){
				return $this->areValidSuffixesForAwdar( $parts[1] );
			}
		...
		if(
			preg_match( '/^(ты|кан|с(а|ын)|а[рк]?|ы([ркпмш]|й[км]|ң|гыз)|у|ма)/u',
			$text, $matches )
		) {
			$parts = preg_split( '/(?<=^'.$matches[0].')/u', $text, 2, PREG_SPLIT_NO_EMPTY );
			if( isset( $parts[1] ) ) {
	...
	function areValidSuffixesForAwma( $text ) {
		if(
			preg_match(
	...
	function areValidSuffixesForAwgan( $text ) {
		if(
			preg_match(
			'/^(га|да[нйш]?|дыр|ча|ның?|сыз|лык?|лар|мы|сың|быз|сыз|ы|рак|чык?)/u',
			$text, $matches )
		) {
			$parts = preg_split( '/(?<=^'.$matches[0].')/u', $text, 2, PREG_SPLIT_NO_EMPTY );
			if( isset( $parts[1] ) ) {
		...
		if( preg_match( '/^ла/u', $text, $matches ) ) {
			$parts = preg_split( '/(?<=^'.$matches[0].')/u', $text, 2, PREG_SPLIT_NO_EMPTY );
			if( isset( $parts[1] ) ) {
				return $this->areValidSuffixesForAwdar( $parts[1] );
			}
	...
	function areValidSuffixesForAwos( $text ) {
		if(
			preg_match( '/^(ка|та[нйш]?|тыр|ча|ның?|сыз|лык?|лар|мы|сың|быз|сыз|ы|рак|чык?)/u',
			$text, $matches )
		) {
			$parts = preg_split( '/(?<=^'.$matches[0].')/u', $text, 2, PREG_SPLIT_NO_EMPTY );
			if( isset( $parts[1] ) ) {
	...
	function areValidSuffixesForQalsa( $text ) {
		if( preg_match( '/^(м|к|ң|гыз|лар)/u', $text, $matches ) ) {
			$parts = preg_split( '/(?<=^'.$matches[0].')/u', $text, 2, PREG_SPLIT_NO_EMPTY );
			if( isset( $parts[1] ) ) {
				return $this->areValidSuffixesForQalsam( $parts[1] );
			}
			return true;
		}
	...
	function areValidSuffixesForQalsam( $text ) {
		if( preg_match( '/^мы$/u', $text, $matches ) ) {
			return true;
		}
		return false;
