...
class TtConverter extends LanguageConverter {
	...
	function convertLowercasedTatarWordCyrlToLat( $text, $compoundFirstPart = null ) {
		...
		if ( preg_match( '/^((([кт]өн|һәр)ь|(сул|арт)ъ)я[кг]|биектау|(күп|бер)ьеллы[кг]'
		...
		if ( preg_match( '/.{2,}(намә|ханә|ниса|кяр|улл|[еэ]тдин)/u', $text ) ) {
			$parts = preg_split(
				'/(?=намә|ханә|ниса|кяр|улл|[еэ]тдин)/u', $text, null, PREG_SPLIT_NO_EMPTY
			);
		...
		if ( preg_match( '/аскорма|аккош|суүсем|әрдоган|азкөч'
			...
			. '|юлтимер|аккирәй|айгөл|яшүсмер/u', $text ) ) {
			$parts = preg_split( '/(?<=^..)/u', $text, null, PREG_SPLIT_NO_EMPTY );
			if( $parts[0] == 'яш' ) {
				$parts[0] .= 'ь';
			}
		...
		*/
		...
		if (
			...
			preg_match(
				...
				// к
				. '|кала[кг]|калкын|кае[кнш]|каер|кечкенә[йяею]|коерык|кө[йяею]|комачау'
				. '|ко[йяею]|кызы[кг]|кыя[кг]|кояш|кавын|кыз|капка'
				. '|ки[яею]|коры|коңгырт|карага[йе]|карта|карават|каршы'
				. '|кабат|комар|канат|ку[йяею]|кына|куан|кәүҗияк'
				. '|кыса|кыска|кыр|катнаш|казы|кашык'
				. '|котлуг|ка$|кубрат|кытай|кайсы'
				// |кы[йяею](?!фәт|м?мәт)
				// у
				. '|урта[кг]|урнаш|у[йяею]|утыр|угры|углан|утра[ув]'
				. '|укы|утыз'
				// т
				. '|тука[йяею]|тукран|тая[кг]|тагын|ту[йяею]|тө[йяею]|тапкыр|тырыш|тугры'
				. '|та[пб](?!игат)|тукта|туфрак|туклык|тарка|тутыр|тавыш'
				. '|тар[аы]|туглан|ти(?!тикака)|ти[яею]'
				. '|тарт|тор|тот|та[кг](?!ил)|түбән|туклан|тамга'
				// б
				. '|бое|бае|бул|баш|(бөе|бу|байра)[кг]|быел|бу[йе]|буын|бавыр|буя'
				. '|бәя'
				// '|болга(?!рия)'
				// ба[кг](?!ира)
				// г
				...
				// ч
				. '|чө[йяею]|чекерә[йяею]|чә[йе]|чуен|чыелда|чокыр|чы[кг]|чакыр|чуаш'
				// е
				. '|ера[кг]|елма[йеяю]'
				// о
				. '|о[яею]|оеш|очра|олуг'
				...
				. '|ышан'
				// с
				. '|сугар|са[кг]|си[яею]|суз|сан|соң|сал|саба[кг]|сыек'
				// ю
		...
		if (
			...
			|| preg_match(
				...
				. '|корбан|коръән|кабер|катгый|кагъбә|каһарман|касыйм|кыямәт'
				. '|кыйммәт|кади|каф|кари|кадер|кардәш|каһин|кәеф|кыйсса|кыям'
				// җ
				...
				// н
				. '|намзәт|нәүбәт|нәстәгъ|нәгыйм|ниһаять|нәзарәт|нәкъ'
				// м
				. '|мөрәҗ|мәгар|мәкал|мәгълүм|максат|мәэмүр|маһи'
				. '|маэмай|мөстәкыйль|мәшәкат|мәгънә|мәгъдән|мәшгул|мәүлүд'
				// г
				. '|газиз|гадәләт|гамәл|гыйрак|гарә[пб]|госман|гаят|галим|гасыр|гаяр|гает|гайре'
				. '|гае[пб]|гариф|гар(?!анти|сия|ри|дин|филд|и)'
				. '|гыйффәт|гыйльфан|гайнан|гайн|гыйният|гашыйк'
				...
				. '|гыйбарәт|гафур|гам(?!ет|аш|би)|гыйбад|гафият|гани|газим'
				// и
				. '|иҗтим|идарә|игътибар|икътис|игълан|ильяс|инкыйлаб|иганә'
				// т
				. '|тәэмин|тәвәк|тәшвик|тәрәкк|тәнвин|тәэсир|тасвир|тәэссорат|тәнкыйт|тәкъдим'
				. '|табигат|тәгаен|тәмуг|тәкъвим'
				// ш
				. '|шөгыл|шигыр|шәры[кг]'
				// с
				. '|сәлам|сәясәт|сәнагат|сәгат|сәнгат|сәгъди|сурәт'
				. '|сәяси|суфиян|солтан|ситдыйк|сәркати[пб]'
				// п
				. '|пакь'
				// д
				. '|дөнья|диван|дәүләт|дәвам|дәһли|дәва'
				// ә
				. '|әфган|әнвәр|әхла[кг]|әхмәт|әнкара|әүхәди|әгъза'
				// л
				. '|лихьян|лөгат|лалә'
				// р
				. '|расыйх|рөкъг|рия|равия|риза|ризык|рәфыйк|риваят'
				// я
				...
				// х
				. '|хөсәен|хикәя|халәт|хәрабә|хак(?!ас)|ханә'
				// ф
	...
	function surelyIsATatarVerbWithSuffixes( $text ) {
		...
		// echo $text;
		$roots = [
			'кун?', 'кый?л', 'тул?', 'ка[лтп]', 'уз', 'йар', 'а[лш]', 'укы?', 'арт', 'у',
			'ку[шп]', 'с?а[ув]', 'тай(ан)?', 'йугал', 'ба[рт]', 'тул?', 'кама', 'бакыр',
			'кы[рй]', '(ка|а|су|йы|йа|куры?|ба|йабы)[кг]', '(йокл|кар|тукт)[аы]', 'болга', 'кайт'
		];
	...
	function surelyIsATatarNounWithSuffixes( $text ) {
		...
		$roots = [
			...
			'урман', 'аш', 'йалтыр', 'кар?т', 'йы[лш]', 'арка', 'аркан', 'корт',
			'арт', 'ка[рнз]', 'казан', 'казакъ?', 'болгар',
			'ка[пб]', 'кай', 'баг', 'бакыр', 'чак', 'йар', 'кул',
			'(у|тайа|йа|йу|чу|ва|ча)[кг]'
		];
		foreach( $roots as $root ) {
			if( preg_match( '/^'.$root.'/u', $text, $matches ) ) {
				$parts = preg_split( '/(?<=^'.$matches[0].')/u', $text, 2, PREG_SPLIT_NO_EMPTY );
	...
	function surelyAreValidSuffixesAfterThickVerb( $parts ) {
		...
		if(
			...
			if( isset( $suffixes_parts[1] ) ) {
				...
				if(
					...
					|| preg_match( '/^ак/u', $text, $matches )
					|| preg_match( '/^ачак/u', $text, $matches )
				) {
					$suffix = $matches[0];
		...
		if (
			...
			if( isset( $suffixes_parts[1] ) ) {
				...
				// ялтыравык
				if ( preg_match( '/^вык/u', $text, $matches ) ) {
					$suffixes_parts = preg_split( '/(?<=^...)/u', $text, 2, PREG_SPLIT_NO_EMPTY );
					if( isset( $suffixes_parts[1] ) ) {
						return $this->surelyAreValidSuffixesAfterThickNoun( $suffixes_parts );
					}
	...
	function surelyAreValidSuffixesAfterThickNoun( $parts ) {
		...
		if(
			0 < preg_match_all(
				'/^(ча|ның?|сыз|лы[кг]?|лар|мын?|сың|быз|дыр|рак|чы[кг]?|а[ув])/u',
				$text, $matches
			)
			||
			...
			&& 0 < preg_match_all( '/^(сын?|м|ң|гыз)/u', $text, $matches )
			||
			...
			// after russian words
			preg_match( '/[кпстфхцчшщ][ьъ]?$/u', $parts[0] )
			&& 0 < preg_match_all( '/^(к[аы]|та[нйш]?|тыр)/u', $text, $matches )
			||
			preg_match( '/([бвгдаеёийуыэюя]|[жзлмнр][ьъ]?)$/u', $parts[0] )
			&& 0 < preg_match_all( '/^(г[аы]|да[нйш]?|дыр)/u', $text, $matches )
			||
			preg_match( '/ын$/u', $parts[0] )
			&& 0 < preg_match_all( '/^ың/u', $text, $matches )
			||
			$parts[0] == 'кай'
			&& 0 < preg_match_all( '/^ан/u', $text, $matches )
			||
			preg_match( '/(^кай|ы[мңн])$/u', $parts[0] )
			&& 0 < preg_match_all( '/^а/u', $text, $matches )
		) {
		...
		if(
			...
			if( isset( $suffixes_parts[1] ) ) {
				...
				if( preg_match( '/^ы[кг]/u', $text, $matches ) ) {
					// $suffix = $matches[0];
					$suffixes_parts = preg_split(
						'/(?<=^..)/u', $text, 2, PREG_SPLIT_NO_EMPTY
					);
		...
		// ы is moved up
		if( preg_match( '/^(ла|р)/u', $text, $matches ) ) {
			$suffix = $matches[0];
			$suffixes_parts = preg_split(
				'/(?<=^'.$suffix.')/u', $text, 2, PREG_SPLIT_NO_EMPTY
			);
	...
	function processWordWithRussianStemCyrlToLat( $text ) {
		...
		$parts = preg_split(
			...
			. '(?<!^(шпа|щёт|вол|выш|май|бур|кеп|кон|оме|бан|лан|мар|тон|рам))(?<!нош)'
		...
		$parts = preg_split(
			'/(?<!аматур)(?<!о)(?<!^ма)(?<!^хок|бан|мер)(?<!^(физи|окру|нинс))(?<!^матери|класси)'
			.'(?=[гк][ые]($|[^нтр]))(?!гель)/u',
			$text, null, PREG_SPLIT_NO_EMPTY );
		if ( count( $parts ) == 2 ) {
			return $this->convertRussianWordWithTatarEndingCyrlToLat( $parts );
		...
		$parts = preg_split( '/(?<!^.)(?<!леку|ярос|нико|югос|купа|авос|умми)'
			. '(?<!вац|обк|шка|дуг|коа)(?<!^(ве|га|де|бу))(?<!^архипе|святос)'
			. '(?=л[аә])'
			. '(?!ла(зма|кир|бан))/u',
			$text, null, PREG_SPLIT_NO_EMPTY );
		if ( count( $parts ) == 2 ) {
	...
	function convertSimpleArabicWordFromCyrillicToLatin( $text ) {
		...
		// $text = preg_replace( '/(?<=^гашый)к/u', 'q', $text );
		...
		if(
			...
			$text = preg_replace( '/(?<=ә)е/u', 'йe', $text );
		} else {
			/*
			$text = preg_replace( '/(?<=^иҗтимаğ)ый/u', 'i', $text );
			$text = preg_replace( '/(?<=^ваq)ый(?=ğ)/u', 'i', $text );
			$text = preg_replace( '/(?<=^шиğ)ый/u', 'i', $text );
			$text = preg_replace( '/(?<=^рас)ый(?=х)/u', 'i', $text );
			$text = preg_replace( '/(?<=^ğ)ый(?=льфан)/u', 'i', $text );
			$text = preg_replace( '/(?<=^шаğ)ый(?=р)/u', 'i', $text );
			*/
			if(
				!preg_match( '/^вазыйфа/u', $text )
				&& !preg_match( '/^ğыйраq/u', $text )
				&& !preg_match( '/^qатğый/u', $text )
				&& !preg_match( '/^qыйсса/u', $text )
		...
		//
		/*
		// ы after thin vowel/syllable
		$text = preg_replace( '/^(?<=шәр)ы/u', 'e', $text );
		*/
		//
		// a
		// a after thin vowel/syllable
		/*
		...
		*/
		...
		// i do not change әмм(а), мөл(а)ем, т(а)биг
		/*
		...
		$text = preg_replace( '/(?<=г)а(?=ләм)/u', 'ä', $text );
		*/
		//
		...
		*/
		...
		$text = preg_replace( '/(?<=[qğ])а(?=[^аыоуәэеөүи]+[әеүиь]|е)/u', 'ä', $text );
		// коръән
		$text = preg_replace( '/(?<=[qğ])о(?=[^аыоуәэеөүи]{1,2}[әеүиь])/u', 'ö', $text );
		// шөгыле
		...
		*/
		...
		$text = preg_replace( '/^тәрәqqый/u', 'täräqqi', $text );
		// $text = preg_replace( '/^тәрәккыя/u', 'täräqqiä', $text );
		$text = preg_replace( '/^тәрәqqыя/u', 'täräqqiä', $text );
		...
		$text = preg_replace( '/шәфиğый/u', 'şäfiği', $text );
		// $text = preg_replace( '/кыйммәт/u', 'qimmät', $text );
		// $text = preg_replace( '/гашыйк/u', 'ğaşiq', $text );
		$text = preg_replace( '/ğашыйк/u', 'ğaşiq', $text );
		// ый after qğ, before thin syllables
		$text = preg_replace( '/(?<=[qğ])ый(?=.+[әеүиь])/u', 'i', $text );
		// ый after thin syllable
		// рәфыйк
		$text = preg_replace( '/(?<=ә.)ый/u', 'i', $text );
		// ыя
		// exception to following/next replace
		// кыямәт
		$text = preg_replace( '/(?<=q)ыя(?=мәт)/u', 'ıyä', $text );
		// җәмгыять, тәрәккые
	...
	function convertLowercasedTatarWordLatToCyrl( $text ) {
		if('äxläğendä'==$text){ $test1=true; echo 'OK'; } else{ $test1=false; }
		// exceptional words
		// month names
