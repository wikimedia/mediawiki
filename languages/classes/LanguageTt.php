...
class TtConverter extends LanguageConverter {
	public $toLatin = array(
		...
		'ю' => 'yu', // 'Ю' => 'Yu',
		'я' => 'ya', // 'Я' => 'Ya',
		/*'а' => 'a', 'А' => 'A',//for alternative way with preg_replace_callback
		'ә' => 'ä', 'Ә' => 'Ä',
		'б' => 'b', 'Б' => 'B',
		'в' => 'w', 'В' => 'W',
		'г' => 'g', 'Г' => 'G',
	...
	function convertTatarFromCyrillicToLatin( $text ) {
		...
		// '$1ü', $text );// эволюциягә
		$text = preg_replace( '/([бвгджҗзклмнңпрстфхһцчшщ])ю(?=\w+[аыо])/ui',
		...
		$text = preg_replace( '/гаять/ui', 'ğayät', $text );
		...
		$text = preg_replace( '/юлия(?=[бвгджҗзклмнңпрстфхһцчшщ]+[әе])/ui', 'yuliä', $text );
		$text = preg_replace( '/юлия\b/ui', 'yuliä', $text );
		...
		//$text = preg_replace( '/\bГае/u', 'Ğäye', $text );
		$text = preg_replace( '/гая([тр])(?=е)/ui', 'ğäyä$1', $text );
		$text = preg_replace( '/гая([тр])ь/ui', 'ğäyä$1', $text );
		$text = preg_replace( '/га(?=[дҗтр][ьәе])/ui', 'ğä$1', $text );
		// $text = preg_replace( '/га([])ә/ui', 'ğä$1ä', $text );
		...
		// $text = preg_replace( '/\bВак/u', 'Waq', $text );
		$text = preg_replace( '/\bвасыя/u', 'wasiyä', $text );
		...
		$text = preg_replace( '/(?<!и)Ю([бвгджҗзклмнңпрстфхһцчшщ])ь/u', 'Yü$1', $text );// Юнь-Yün
		$text = preg_replace( '/(?<!и)я([бвгджҗзклмнңпрстфхһцчшщ][әие])/ui', 'yä$1', $text );
		$text = preg_replace( '/(?<!и)я([бвгджҗзклмнңпрстфхһцчшщ])ь/ui', 'yä$1', $text );
		...
		$text = preg_replace( '/([өәү])я/ui', '$1yä', $text );// көя, җәя, гүя
		$text = preg_replace( '/ия/ui', 'iä', $text );// ия, революция
		...
		$text = preg_replace( '/ыять/u', 'iyät', $text );
		$text = preg_replace( '/ать/u', 'ät', $text );
		$text = preg_replace( '/оръән/ui', 'ör\'än', $text );
	...
	/*function convertTatarFromCyrillicToLatin2( $text ) {
		//for alternative way with preg_replace_callback
		// $text = preg_replace( '/онкурс/u', 'onkurs', $text );
		// $text = preg_replace( '/едакц/u', 'edakts', $text );
		// $text = preg_replace( '/втор/u', 'vtor', $text );
		// $text = preg_replace( '/ов/u', 'ov', $text );
		...
		$text = preg_replace_callback(
			'/(?<=[вл])ю(?=\w+[аыо])/ui',
			//(?<=[вл]) - эволюцияга,люгдон
			//(?<=[бвгджҗзклмнңпрстфхһцчшщ])
			function($m){return $m[0]=='ю'?'\'u':'\'U';},
			$text
		);
		// $text = preg_replace( '/([бвгджҗзклмнңпрстфхһцчшщ])ю/ui', '$1ü', $text );// эволюция
		//// $text = preg_replace( '/волю/ui', 'volü', $text );
		...
		// $text = preg_replace( '/\bе([бвгджҗзклмнңпрстфхһцчшщ]+(?=([уаы]|\b)))/u',
		// 'yı$1', $text );
		$text = preg_replace_callback(//еш
			'/(?<=\b)е(?=[бвгджҗзклмнңпрстфхһцчшщ]+([уаы]|\b))/ui',
			function($m){return $m[0]=='е'?'yı':'Yı';},
			$text
		);
		// егу-yığu
		//// $text = preg_replace( '/\bЕ([бвгджҗзклмнңпрстфхһцчшщ]+(?=([уаы]|\b)))/u',
		...
		// $text = preg_replace( '/мв/u', 'mv', $text );
		// $text = preg_replace( '/([аыоу])е/ui', '$1yı', $text );
		$text = preg_replace_callback(//елмаеп
			'/(?<=[аыоу])е/ui',
			function($m){return $m[0]=='е'?'yı':'Yı';},
			$text
		);
		...
		// $text = preg_replace( '/([юу])ы/ui', '$1ı', $text );// укуы-uquı
		// $text = preg_replace( '/(?<!и)ю([бвгджҗзклмнңпрстфхһцчшщ][әие])/ui', 'yü$1', $text );
		// $text = preg_replace( '/(?<!и)ю([бвгджҗзклмнңпрстфхһцчшщ])ь/u',
		// 'yü$1', $text );// юнь-yün
		// $text = preg_replace( '/(?<!и)Ю([бвгджҗзклмнңпрстфхһцчшщ])ь/u',
		// 'Yü$1', $text );// Юнь-Yün
		// $text = preg_replace( '/(?<!и)я([бвгджҗзклмнңпрстфхһцчшщ][әие])/ui', 'yä$1', $text );
		...
		// $text = preg_replace( '/обелен/u', 'obelen', $text );// гобелен-gobelen
		// $text = preg_replace( '/оген/u', 'ogen', $text );// гоген-gogen
		// $text = preg_replace( '/([аәя])[уү](?![бвгджҗзклмнңпрстфхһцчшщ]{2,2})/ui',
		// '$1w', $text );
		$text = preg_replace_callback(
			//'/(?<=[аәя])[уү](?![бвгджҗзклмнңпрстфхһцчшщ]{2,2})/ui',
			'/(?<=[аәя])[уү]/ui',// эшләү
			function($m){return $m[0]=='у'||$m[0]=='ү'?'w':'W';},
			$text
		);
		// $text = preg_replace( '/кама(?=га)/u', 'kama', $text );
		// $text = preg_replace( '/каток/u', 'katok', $text );
	...
	function replace_into_array($pattern,$replacement,$subject,&$target){
	...
	function translate( $text, $toVariant ) {
		if ( $toVariant == 'tt-cyrl' ) {
			...
			$text = preg_replace( '/Q[äÄәÄ]/u', 'Ка', $text );
			$text = preg_replace( '/i[äÄәÄ]/u', 'ия', $text );
			...
			for ( $i=0; $i<$wordsCount; $i++ ){
				...
				// ){
					...
					$this -> replace_into_array( '/(?<=ы)й(?=[бвгджҗзклмнңпрстфхһцчшщ]+[ьә])/ui', ''
						, $words[$i], $sourceLetterCase );// мөстәкыйль-möstäqil,гыйффәт-ğiffät
					$this -> replace_into_array( '/(?<=\b)Е(?=РЭ)/u', 'ul'
						, $words[$i], $sourceLetterCase );// ЕРЭ
					$this -> replace_into_array( '/(?<=\b|[ъь])е/u', 'll'
						, $words[$i], $sourceLetterCase );
					$this -> replace_into_array( '/(?<=\b|[ъь])Е/u', 'uu'
						, $words[$i], $sourceLetterCase );*/
			...
			$text=join( $words );
			/* // alternative way with preg_replace_callback
			$words = preg_split( '/\b/u', $text );
			$wordsCount = count($words);
			for ( $i=0; $i<$wordsCount; $i++ ){
