...
class TtConverter extends LanguageConverter {
		...
		*/
		if (
			...
			preg_match(
				...
				. '|кыйммәт|кади|каф|кари|кадер|кардәш|каһин|кәеф|кыйсса|кыям|камил'
				// җ
	...
	function tryGetRootOfThickWord( $text, &$parts ) {
		...
		for( $i = 2; $i <= 11; $i++ ) {
			...
			if(
				...
				if( $rootType = $this->isValidDivision( $parts ) ) {
					// return $parts;
					return $rootType;
				}
			}
	...
	function __construct(
		...
		$this->tatarVerbs = array_fill_keys(
			[
	...
	function isValidDivision( $parts ){
		if(
			...
			||
			preg_match( '/^(духовной|октябрьской|предприятий|раменской|иҗтимаг)$/u', $parts[0] )
	...
	function processWordWithRussianStemCyrlToLat( $text, $beforeThickSuffixes ) {
