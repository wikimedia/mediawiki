...
class TtConverter extends LanguageConverter {
	...
	function convertLowercasedTatarWordCyrlToLat( $text ) {
		...
		if (
		...
		}elseif ( preg_match( '/^(җигәнтама|йөрәктау|тимербулат|кайсыбер)/u', $text )
		) {
			$parts = preg_split( '/(?<=^.....)/u', $text, null, PREG_SPLIT_NO_EMPTY );
			return $this->convertCompoundTatarWordCyrlToLat( $parts );
			...
			. '|кайбер|һәрчак|унъел|кол(мәт|сәет)|күктау|акъегет)/u', $text ) ) {
		...
		// end of compounds
		}elseif ( preg_match( '/(?<=.)(?<!сосн|град)(?<!самин|тиров)(?<!петропавл)'
			. '(?<!покр)(?<!^новоникола)'
			...
			preg_match(
				...
				// бигате for табигате
				'/әэ'
				. '|^г(а(ярь?|еб|йр|лим|дел|фиятулл)(е|лек?)?|о(мер|реф|шер))'
			...
			|| preg_match(
				'/^(б(агира|о(лгария|тинка))|тагил|х(орватия|ими|арьк)|маугли'
					. '|юбиле|л(и(га|тва)|атвия)|система|республика'
					. '|м(а(гнит|ксим)|олдавия)|органик|извест|нефть'
					. '|и(юн|жау)|дев|самин|эфир|алфавит|васили|савин'
					. '|нижгар|пермь|адмирал)'
				. '/u', $text )
			...
			|| preg_match(
				'/^(м(узыка|елвин|а(даг|ршан|йка))'
					. '|в(епс|ышка|алда)'
					. '|пауза|(за|у)каз'
					. '|с(о(с(тав|нов)|крат)|а(рат|ван))'
					. '|б(ензин|исау|ушу|а(вар|рнаул)|урка)'
					. '|энерг|юлия|фея|яков|порту'
					. '|лив(?!ә)|уганда|елена|тиран'
					. '|о(скар|трад|кт)'
					. '|декрет|июл|наум|талала'
					. '|режисс|фуфай'
					. '|ла(всан|йкра)|пояс|йошкар|ноя|датир|чабаксар?'
					. '|а(ксаков|грар|угуст))'
					. '|танк|юридик|янка|сервис'
	...
	function processWordWithRussianStemCyrlToLat( $text ) {
		...
		$parts = preg_split( '/(?=л[ые][кг])(?!ле(к(тр|ул|с|ом)|г(рам|ац)))/u',
			$text, null, PREG_SPLIT_NO_EMPTY );
		if ( count( $parts ) == 2 ) {
		...
		$parts = preg_split(
			'/((?<!и)(?<!^ёл|де|ян)(?<!^шпа|щёт|вол|выш|май|бур|кеп|кон|оме)(?<!нош)'
			. '(?<!ссыл|музы|трич|стен|тчат|агер|рмал|кров|деле|олив|омег|курт'
			. '|ст[её]р|грей|уфай|осын|убан|ураж|отин|рчат|наго|иров|инов)'
		...
		$parts = preg_split( '/(?<!^(шке|бри|коф))(?=[тд][аә]н?$)/u',
			$text, null, PREG_SPLIT_NO_EMPTY );
		if ( count( $parts ) == 2 ) {
			return $this->convertRussianWordWithTatarEndingCyrlToLat( $parts );
		}
		...
		// активлаштырылган
		$parts = preg_split( '/(?<!^.)(?<!леку|ярос|нико|югос|ватс|купа|авос)'
			. '(?=л[аә])'
			. '(?!ла(зма|кир))/u',
			$text, null, PREG_SPLIT_NO_EMPTY );
		if ( count( $parts ) == 2 ) {
	...
	function convertSimpleTatarWordFromCyrillicToLatin( $text ) {
		if (
			...
			// äw
			$text = preg_replace( '/(?<=[әя])ү/u', 'w', $text );
			// в in ливәшәү
