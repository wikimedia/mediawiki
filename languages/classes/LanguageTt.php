...
class TtConverter extends LanguageConverter {
	...
	function convertLowercasedWordCyrlToLat(
		...
		//
		// compounds
		if ( preg_match( '/стәрлетама|солтангали|гыйлемхан/u', $text ) ) {
			$parts = preg_split( '/(?<=^.{6})/u', $text, null, PREG_SPLIT_NO_EMPTY );
			return $this->convertNotRecursiveCompoundWordCyrlToLat( $parts );
		}
		if (
			preg_match(
				'/^(җигәнтама|йөрәкта|тимер(булат|чыбы)|кайсыбер|миңне.+)/u',
				$text
			)
		) {
			$parts = preg_split( '/(?<=^.{5})/u', $text, null, PREG_SPLIT_NO_EMPTY );
			return $this->convertNotRecursiveCompoundWordCyrlToLat( $parts );
		}
		if ( preg_match( '/^(биектау|карабодай|якшыгол|ярымутра|бөекбрит)/u', $text ) ) {
			$parts = preg_split( '/(?<=^.{4})/u', $text, null, PREG_SPLIT_NO_EMPTY );
			return $this->convertNotRecursiveCompoundWordCyrlToLat( $parts );
		}
		if (
			preg_match(
			'/^('
			. 'көн(чыгыш|батыш|багыш|бакты)'
			. '|гөлбакча|ч[иы]ная[кг]|башкор[тд]'
			. '|коточкыч|бик.{3,}|аяктубы'
			. '|күп(сан|кыр)'
			. '|бер(аз|кат|кай(чан)?|туган|вакыт)'
			. '|кай(бер|чак)'
			. '|һәр(чак|вакыт)'
			. '|кол(мәт|сәет|мәмәт|бил)'
			. '|күктау|байегет'
			. '|(бил|бәк)бау'
			. '|күзал|икепулатлы|нурмөхәммә[тд]'
			. ')/u',
			$text
			)
		) {
		...
		if ( preg_match( '/аскорма|аккош|агыйдел|су(үсем|тудыр)|әрдоган|азкөч|яугир'
		...
		if (
			preg_match(
				'/^('
				. '(([кт]өн|һәр)ь|(сул|арт)ъ)я[кг]|(таш|кул|ел)ъязма'
				. '|акъегет|(күп|бер|мең|ун)[ьъ]еллы[кг]|шәльяу'
				. ')/u',
				$text
			)
		) {
			$parts = preg_split( '/[ъь]/u', $text, null );
			return $this->convertNotRecursiveCompoundWordCyrlToLat( $parts );
		}
		if (
			preg_match( '/^(супер|радио|гидро|вики|гига|авто|мото|теле).+/u', $text, $matches )
		) {
			...
			$parts[1] = $this->convertLowercasedWordCyrlToLat( $parts[1] );
			$text = implode( $parts );
			return $text;
		}
		if ( preg_match( '/^(маһи|г?абде?).{3,}/u', $text, $matches ) ) {
			$parts = preg_split( '/(?<=^'.$matches[1].')/u', $text, null, PREG_SPLIT_NO_EMPTY );
		...
		if ( preg_match( '/.+(?<!^ба)(?<!^ы)(?<!^кон)(?<!сек)су/u', $text ) ) {
			...
			// if( $this->surelyIsATatarNounWithSuffixes( $parts[1] ) ) {
				// return $this->convertCompoundWordCyrlToLat( $parts );
			// }
			return $this->convertNotRecursiveCompoundWordCyrlToLat( $parts );
		}
		...
		if (
			preg_match( '/.{2,}(намә|ханә|ниса|кяр|улл[аи]|тдин|җан)/u', $text, $matches )
		) {
			$parts = preg_split(
		...
		if ( preg_match( '/.+оглу/u', $text ) ) {
			...
			// $text = $this->convertCompoundWordCyrlToLat( $parts );
			// return parent::translate( $text, 'tt-latn' );
			return $this->convertNotRecursiveCompoundWordCyrlToLat( $parts );
		}
		if ( preg_match( '/.+стан(?!(т(ин|а)|ц))/u', $text ) ) {
			...
			$parts[0] = $this->
				convertLowercasedWordCyrlToLat( $parts[0] ); // recursion for башкортстан
			$parts[1] = $this->
				convertSimpleTatarWordFromCyrillicToLatin( $parts[1] );
			// return $parts[0] . parent::translate( $parts[1], 'tt-latn' );
			return $parts[0] . $parts[1];
		}
		if ( preg_match( '/.+(град|стат)/u', $text ) ) {
			$parts = preg_split( '/(?=град|стат)/u', $text, null, PREG_SPLIT_NO_EMPTY );
			$parts[0] = $this->
				convertSimpleRussianWordFromCyrillicToLatin( $parts[0] );
			$parts[1] = $this->
				convertSimpleRussianWordFromCyrillicToLatin( $parts[1] );
			return $parts[0] . $parts[1];
		}
		if ( preg_match( '/.+шин/u', $text ) ) {
			$parts = preg_split( '/(?=шин)/u', $text, null, PREG_SPLIT_NO_EMPTY );
			return $this->convertCompoundWordCyrlToLat( $parts );
		}
		if ( 'түбәнкама' == $text ) {
			return 'tübänkama';
		...
		// end of compounds.
		return $this->convLcNonCompoundWordCyrlToLat( $text, $compoundOtherPart );
	}

	function convertCompoundWordCyrlToLat( $parts ) {
		$nParts[0] = $this->
			convertLowercasedWordCyrlToLat( $parts[0], $parts[1] );
		$nParts[1] = $this->
			convertLowercasedWordCyrlToLat( $parts[1], $parts[0] );
		$text = implode( $nParts );
		return $text;
	}

	function convertNotRecursiveCompoundWordCyrlToLat( $parts ) {
		$nParts[0] = $this->
			convLcNonCompoundWordCyrlToLat( $parts[0], $parts[1] );
		$nParts[1] = $this->
			convLcNonCompoundWordCyrlToLat( $parts[1], $parts[0] );
		$text = implode( $nParts );
		return $text;
	}

	function convLcNonCompoundWordCyrlToLat(
		$text, $compoundOtherPart = null
	) {
		if(
			$text == 'гол'
			&& $compoundOtherPart == 'якшы'
		) {
			$text = 'ğol';
		}
		$text = preg_replace( '/^муравьев/u', 'муравьёв', $text );
		//
		...
		if( $rootType == 1 ) { // thick suffix
			...
			$parts[0] = $this->convLcTtWordWithoutThickSuffixesCyToLa(
				$parts[0], $parts[1], $compoundOtherPart
			);
		...
		} elseif( $rootType == 3 ) { // thick suffix after russian word
			// $parts[0] = $this->convLcTtWordWithoutThickSuffixesCyToLa(
				// $parts[0], $parts[1], $compoundOtherPart
			// );
			$parts[0] = $this->convertSimpleRussianWordFromCyrillicToLatin(
				$parts[0], $parts[1] // , $compoundOtherPart
			);
			$parts[1] = $this->convertSimpleTatarWordFromCyrillicToLatin( $parts[1] );
			if( $this->test ) {
		...
		} elseif( $rootType == 4 ) { // thick suffix after arabic word
		...
		} elseif( $rootType == 5 ) { // thick suffix after tatar word
			$parts[0] = $this->convertSimpleTatarWordFromCyrillicToLatin(
				$parts[0], $parts[1] // , $compoundOtherPart
			);
			$parts[1] = $this->convertSimpleTatarWordFromCyrillicToLatin( $parts[1] );
			if( $this->test ) {
			...
			$parts[0] = $this->convLcTtWordWithoutThickSuffixesCyToLa(
				$parts[0], false, $compoundOtherPart
			);
		}
		if( $this->test ) {
			$parts[0] .= $rootType;
	...
	function convLcTtWordWithoutThickSuffixesCyToLa(
		$text, $beforeThickSuffixes = false, $compoundOtherPart = null
	) {
		//
		if(
			...
			if ( preg_match( '/^җан/u', $compoundOtherPart ) ) {
		...
		if( $text == 'аксак' ) {
			if( preg_match( '/^ов/u', $beforeThickSuffixes ) ) {
				return $this->convertSimpleRussianWordFromCyrillicToLatin( $text );
			} else {
		...
		if (
			...
			preg_match(
				...
				. '|^г('
				. 'а('
				. '(ярь|фиятулл)|(еб|яр|й[рн])е'
				. '|((ли(?!ц)м?|дел|риф)(е|ле|сез)?)|еп(ле|сез)'
				. ')'
				. '|о(мер|реф|шер)|ыйффәт'
				. ')'
				. '|ка(бер|леб|дер)|маэмай|вилаят|риза|ильяс' // |аэтдин
				. '|васыят|(?<!^ст)(?<!^с)ать|фирдәүс|бигате|фиркасе|ядкар|шагыйре|ризыклары'
				. '|^гамь|^гаме(?!т)|газыйм|кяр/u',
				$text )
		) {
			return $this->convertSimpleArabicWordFromCyrillicToLatin(
				$text, $beforeThickSuffixes, $compoundOtherPart
			);
		}
		//
		if (
			...
			|| preg_match( '/ерь|рье|еви|изм|уйк/u', $text )
			// тангаж etc
			...
			// поезд
			// || preg_match( '/(акт|зд|тк)($|ы($|м|ң|н|[бг]ыз))/u', $text )
			|| preg_match( '/(кт|зд|тк)$/u', $text )
			...
			|| preg_match(
				...
				. '|р(м(алка|ик)|амзин|т(ина|уз)|навал|инт|гин)'
				. '|у(чук|ф)|за(нка|да|ки)|л(и(й|гула)|вин)|питал'
				. '|спий|чал|кур|йман|питан|сс|ды[кч]|рась|либр)'
				// do not catch карлар
				...
				. '|у(р(с|асан|тка|кума)|к(мара|уруза|$)|туз|зьм|льт|б($|ан)|пала|инси)'
				// ко
				. '|о('
				. 'м(и(сс|$)|анд(а|ир)|мун|б(инат|айн)|пан)'
				. '|н(с(тан|ул)|ка|курс)'
				. '|д($|ын?|та(н|гы)?|ка(ча)?|сыз|чы|ның|иров|лы)'
				. '|жан|ф(та|и)|с(ынка|та|мик)|р(аб|пус|ея)|лум|ка?|ала|х|лбурн|пи?|шай'
				. ')'
				// к...
				. '|изнер'
				. ')'
				...
				. 'а(ук|на|м(аш|бия)|р(ант|сия|ри|дин|филд|хипел|и)'
				. '|л(мудуг|лия|иц)|биния|й$|ити|ва[йя])'
				// го
				. '|о(р(илла|ки|ьк|$)|а|би|лланд|фман|луб)'
				// г...
				. '|(аз|ол?)($|да(н|гы)?|га(ча)?$|сыз|чы|ның?|ын?|лар)'
				. '|вин|у(м(анитар|ми|ус)|бан|ам)|еве'
				. ')'
				// б
				. '|б('
				// ба
				. 'а(гира|лабан|бынин|вар|рнаул|нк|йт(?!ак)|гама|рак)'
				// бо
				. '|о(л(гария|ив(ия|ар)|дуин)|тинка|гдан|[һг]ем)'
				. '|елоус|ензин|исау|ушу|урка'
				. ')'
				// т
				. '|т(а(гил|миянг|нк|най)|о([кг]|нга)|увалу)'
				. '|талала|тиран|тайга'
				// х
				...
				. '|о(лдавия|замби|рав)|и(чиган|дуэй)'
				. '|у(зыка|скул|рав)|е(лвин|ркель)'
				. ')'
				// и
				. '|и(юн|жау|нка|н(дивид|тер)|кар)'
				. '|июл|извест'
				// н
				. '|н(и(жгар|уэ)|ебраска|аум|ерв|алим|аука)'
				. '|ноя|нефть|нью'
				// п
				. '|п(ермь|ластинка|а(уза|рагвай|рк)|о(дкаст|вар|лк)|угач)'
				. '|порту|пояс|поп'
				// в
				. '|в(епс|ышка|а(лда|ссал|нуату|з(?!ыйф))|у(з|лкан)|акуум)'
				. '|васили|вангар'
				// о
				. '|о(скар|трад|кт)|органик|окру|объект'
				// с
				. '|с(о(с(тав|нов)|к(рат)?|г)|а(рат|ван|па)|ьерра|е(рвер|гмент))'
				. '|самин|субъект|сингапур|система|сервис|савин|сатьяртхи'
				// д
				. '|д(екрет|углас|арья|иалект|атир|ивизи|аут|ев|елавэр|оз)'
				. '|дискусс|дакар'
				// р
				. '|ре(жисс|зерв)'
				. '|республика|рабфа|рамка|рига'
				// а
				...
				. '|заказ'
				// у
				. '|у(а(скаран|йл)|о(ррен|кер|ттс|ллис)|э(льс|йк)|ил(ик|лис))|указ|урсалин|утчанка'
				...
				// ф
				. '|фуфай|физи|фукс|фолликул|фея|фьючерс'
				// я
				. '|яков|янка|яхт|якутия'
				// е
				. '|елена|ермак'
				// ч
				. '|чабакса|чугун|чагин'
			...
			return $this->convertSimpleRussianWordFromCyrillicToLatin(
				$text, $beforeThickSuffixes
			);
		}
		//
		if (
			...
			// other arabic words:
			|| preg_match(
				'/куф[аи]|^г(ае[пт]|ыйлем|омум)|^рия|вакыйга|расыйх|^гыйсъ|ыйк/u',
				$text
			)
	...
	} // convertLowercasedWordCyrlToLat

	// this function is deprecated. it is uncommented to test new function
	...
	function surelyIsATatarNounWithSuffixes( $text ) {
		...
		$roots = [
			...
			'кабат', 'комар', 'канат', 'карта', 'койы', 'каты', 'су', 'куй',
	...
	*/
	...
	function tryGetRootOfThickWord( $text, &$parts ) {
		$len = mb_strlen( $text );
		// trying for nouns from exceptions, with or without suffixes
		if(
			isset( $this->wordsWithFalseThickTatarSuffixesAfterNoun[$text] )
		) {
			$parts[0] = $text;
			$parts[1] = '';
			return $this->wordsWithFalseThickTatarSuffixesAfterNoun[$text];
		}
		if(
			isset( $this->thickTatarVerbs[$text] )
		) {
			$parts[0] = $text;
			$parts[1] = '';
			return 2;
		...
		for( $i = $len - 1; $i >= 2; $i-- ) {
			// going from 1 last letter suffix to longer chains of suffixes
			$parts[0] = mb_substr( $text, 0, $i );
			$parts[1] = mb_substr( $text, $i );
			$surelyAreThinSuffixes = false;
			if(
				$this->surelyAreThinSuffixes( $parts )
			) {
				// әхлаг|ы: should not break, this word is in exceptions,
				// if i break, it goes further and false tatar suffix will be detected;
				// instead, should return.
				// ягъф|әровага: break to optimise, then further try to catch thick suffix.
				$surelyAreThinSuffixes = true;
			}
			if(
				preg_match( '/^[яёюе]/u', $parts[1] )
				&& preg_match( '/([аоуыияёюэеьә])$/u', $parts[0] )
			) {
				$parts[0] .= 'й';
				$parts[1] = preg_replace(
					['/^ев/u', '/^я/u', '/^ё/u', '/^ю/u', '/^е/u'],
					['эв', 'а', 'о', 'у', 'ы'],
					$parts[1]
				);
				$parts[1] = strtr(
				...
				if(
					isset( $this->wordsWithFalseThickTatarSuffixesAfterNoun[$parts[0]] )
					&& $this->surelyAreValidSuffixesAfterThickNoun( $parts )
				) {
					if( $surelyAreThinSuffixes ) {
						$parts[0] = $text;
						$parts[1] = '';
						return 0;
					}
					return $this->wordsWithFalseThickTatarSuffixesAfterNoun[$parts[0]];
				}
				$verb = $parts[0];
				if(
			...
			if(
				...
				// and root part is in exceptions
				// then return.
				if(
					isset( $this->wordsWithFalseThickTatarSuffixesAfterNoun[$parts[0]] )
				) {
				...
				if(
					isset( $this->wordsWithFalseThickTatarSuffixesAfterNoun[$noun2] )
				) {
					if( $surelyAreThinSuffixes ) {
						$parts[0] = $text;
						$parts[1] = '';
					...
					return $this->wordsWithFalseThickTatarSuffixesAfterNoun[$noun2];
				}
			}
			$verb = $parts[0];
			...
			$verb = preg_replace( '/(?<=а)в$/u', 'у', $verb );
			if(
				// if i do not use root verb dictionary,
				// i should catch hypotetical root verbs,
				// and had to add many exceptions, to avoid
				// {wrong detections like валют|а , курсында|гы,
				// which would cause russian words to be detected as tatar verbs,
				// and converted incorrectly},
				// i think adding all tatar root verbs is easier than that.
				isset( $this->thickTatarVerbs[$verb] )
				&& $this->surelyAreValidSuffixesAfterThickVerb( $parts )
			) {
				return 2;
		...
		$parts[0] = mb_substr( $text, 0, 1 );
		$parts[1] = mb_substr( $text, 1 );
		$verb = $parts[0];
		$parts[1] = strtr(
			$parts[1],
			['я'=>'йа', 'е'=>'йы', 'ю'=>'йу']
		...
		for( $i = 2; $i < $len; $i++ ) {
			...
			if(
				preg_match( '/([аоуыияёюэеьә])$/u', $parts[0], $matches )
				&& preg_match( '/^[яёюе]/u', $parts[1] )
			) {
				$parts[0] .= 'й';
				$parts[1] = preg_replace(
			...
			$parts[1] = preg_replace( '/^ев/u', 'эв', $parts[1] );
			$parts[1] = strtr(
				$parts[1],
				['я'=>'йа', 'е'=>'йы', 'ю'=>'йу']
			);
	...
	function surelyAreThinSuffixes( $parts ) {
		// to filter out some thin suffixes, which are easily found,
		// and which can be understood as thick suffix.
		return
			preg_match( '/[әөү]/u', $parts[1] )
			||
			// catching мәдән|ие, to not get it as мәдәнийы,
			// leaving юбиле|ендагы and әхмәтвәл|иевага,
			// catching чә|е
			preg_match( '/и?е([^в]|$)/u', $parts[1] )
			&& preg_match( '/[әөү][бвгджҗзйклмнңпрстфхһцчшщ]{0,2}ь?$/u', $parts[0] )
			||
			// тәрәккы|е, мәдәни|е
			preg_match( '/е([^в]|$)/u', $parts[1] )
			&& preg_match( '/(кы|^.*[әөү].*и)$/u', $parts[0] )
			||
			// әхлагы - ы here i convert as e,
			// so it is not thick suffix
			preg_match( '/^.*[әөү].*г$/u', $parts[0] )
			&& preg_match( '/^ы($|.*[әүеи].*)/u', $parts[1] )
		;
	}
	...
	function __construct(
		...
		$this->thickTatarVerbs = array_fill_keys(
			[
				'кун', 'ку', 'кыйл', 'кыл', 'тул', 'ту', 'кал', 'кат', 'кап', 'уз', 'яр',
				'ал', 'аш', 'укы', 'ук', 'арт', 'у', 'оч', 'елмай',
				'куш', 'сау', 'ау', 'таян', 'тай', 'югал', 'бар', 'бат', 'тул', 'ту', 'кама',
				'сал', 'бул', 'сык', 'бас',
				'кыр', 'кый', 'башкар', 'коткар',
				'как', 'ак', 'сук', 'ек', 'як', 'курык', 'курк', 'бак', 'ябык', 'так', 'йок',
				'йокла', 'кара', 'тукта', 'макта',
				'болга', 'кайт', 'каер', 'тара', 'суз', 'йом',
				'тарка', 'бое', 'буя', 'яз', 'ая', 'айны', 'ое', 'охша', 'оша', 'оял',
				'ят', 'от', 'кой', 'куй', 'тарт', 'тор', 'тот', 'уй', 'тап', 'куп', 'куй',
			],
			1
		);
		$this->russianWordsWithFalseThickTatarSuffixesAfterNoun = array_fill_keys(
			[
				// волга is not вол+га
				'волга', 'шпага', 'лига', 'омега', 'синагога', 'фольга', 'агалега', 'тонга',
				'рига', 'брига', 'тайга', 'штанга',
				// драматургы is not драматур+гы
				'драматург', 'ток', 'физик', 'округ', 'материк', 'классик', 'кубок', 'каталог',
				// духовное is not духовной+ы
				'духовное', 'октябрьское', 'предприятие', 'раменское', 'видение', 'ателье',
				'красное', 'орудие',
				// гуам is not гуа+м
				'гуам',
				// гавая is not гав+ай+а
				'гавая', 'парагвай',
				// горилла is not горил+ла
				'горилла', 'купала', 'коала', 'калигула',
				// валдай is not вал+дай
				'валдай',
				// команда is not коман+да
				'команда', 'канада', 'кофта', 'коста',
				// яков is not як+ов
				'яков',
				// вассалы is not васса+лы
				// if it is detected as васса+лы then i would have to use
				// "васса" instead of "вассал" to detect it as russian word.
				'вассал', 'капитал',
				// щётка is not щёт+ка
				'щётка', 'электричка', 'вышка', 'клетчатка', 'золотоношка', 'оливка', 'куртка',
				'кепка', 'перчатка', 'чукотка', 'обкладка', 'аляска', 'небраска', 'вентреска',
				'продразверстка', 'вёрстка', 'верстка', 'градишка', 'дубровка', 'петрушка',
				// сосновка is not сосн+ов+ка
				'сосновка', 'градовка', 'саминовка', 'датировка', 'кодировка', 'дозировка',
				'петропавловка', 'любовка', 'покровка', 'новониколаевка', 'константиноградовка',
				'тимофеевка', 'миллиметровка',
				// really have tatar homonyms, but that tatar words/wordforms are rarely used:
				'барак', 'канаш', 'канал',
			],
			3
		);
		$this->arabicWordsWithFalseThickTatarSuffixesAfterNoun = array_fill_keys(
			[
				// вакыйга is not вакый+га
				'вакыйга',
				// мөгаен is not мөгай+ы+н
				'мөгаен',
				// имам is not има+м
				'имам',
				// маэмай is not маэм+ай
				'маэмай',
				// игълан is not игъ+ла+н
				'игълан', 'бәла', 'әхлак', 'һәлак', 'мөлаем',
			],
			4
		);
		$this->tatarWordsWithFalseThickTatarSuffixesAfterNoun = array_fill_keys(
			[
				// яссы is not ясс+ы
				'яссы', 'яхшы', 'якшы', 'бие', 'якты', 'ярты',
				// тиен is not тий+ы+н
				'тиен',
			],
			5
		);
		$this->wordsWithFalseThickTatarSuffixesAfterNoun = array_merge(
			$this->russianWordsWithFalseThickTatarSuffixesAfterNoun,
			$this->arabicWordsWithFalseThickTatarSuffixesAfterNoun,
			$this->tatarWordsWithFalseThickTatarSuffixesAfterNoun
		);
		unset( $this->russianWordsWithFalseThickTatarSuffixesAfterNoun );
		unset( $this->arabicWordsWithFalseThickTatarSuffixesAfterNoun );
		unset( $this->tatarWordsWithFalseThickTatarSuffixesAfterNoun );
	}

	function surelyAreValidSuffixesAfterThickVerb( $parts ) {
		// verb in function name means/stands for verb stem
		$awVerbAv = false;
		$awVerbAu = false;
		$aoVerb = false;
		$rVerb = false;
		...
		if(
			...
			&& preg_match( '/^ын[тк]ы/u', $text, $matches )
		) {
			// $suffix = $matches[0];
	...
	function surelyAreValidSuffixesAfterThickNoun( $parts ) {
		// noun in function name means/stands for any word form that is not verb stem
		$text = $parts[1];
		if(
			// лы is moved down
			// мы is moved down
			...
			// i comment this out because i think detecting/separating this
			// here is quite useless for me, and if i did so,
			// i should add exceptions
			// for cases where ий is not true suffix
		) {
			$suffixes_parts = preg_split(
		...
		if(
			...
			// but парк|ы
			!preg_match( '/([аып]|[аоуыиэяёюе]к)$/u', $parts[0] )
			...
			if(
				...
				// further suffixes are not valid.
				// in case except ын, something is wrong.
				if(
					preg_match(
						'/^ы(м|ң|[гб]ыз)/u',
						$text, $matches
				...
				// ын is also ы+н, exit if block, to check further
			}
			// $suffix = $matches[0];
			$suffixes_parts = preg_split(
				'/(?<=^.)/u', $text, 2, PREG_SPLIT_NO_EMPTY
			...
			if( isset( $suffixes_parts[1] ) ) {
				// there are some letters after ы
				if(
					$this->surelyAreValidSuffixesAfterThickNoun( $suffixes_parts )
				) {
					return true;
				}
				// ы was not detected as valid noun to noun suffix, because of further suffixes
				if(
					(
						$parts[0] == 'аш'
						|| $parts[0] == 'кан'
					...
					&& $this->surelyAreValidSuffixesAfterThickVerb( $suffixes_parts )
				) {
					return true;
				}
				if(
				...
				// ы was not detected in no way as part of valid suffix
				return false;
			}
			// there are no letters after ы, it is valid suffix after noun,
			// with specified additional conditions, specified in the above if check
			return true;
		}
		// ы is moved up
		// лы is moved up
	...
	function convertSimpleRussianWordFromCyrillicToLatin( $text, $beforeThickSuffixes = false ) {
		...
		// сентябрь, октябрь
		$text = preg_replace( '/^сентябрь?/u', 'sentäber', $text );
		// this does not work as i expect:
		// $text = preg_replace( '/^октябрь?(?!ск)/u', 'oktäber', $text );
		$text = preg_replace( '/^октябрь(?!ск)/u', 'oktäber', $text );
		$text = preg_replace( '/^октябр(?!ь)/u', 'oktäber', $text );
		// ноябрь
		$text = preg_replace( '/^ноябрь?/u', 'noyäber', $text );
		$text = preg_replace( '/^ноя$/u', 'noyä', $text );
		// декабрь
		$text = preg_replace( '/^декабрь?/u', 'dekaber', $text );
		...
		if( $beforeThickSuffixes ) {
			$text = preg_replace( '/(?<=и)я$/u', 'a', $text );
			$text = preg_replace( '/(?<=е)я$/u', 'ya', $text );
		}
		if( preg_match( '/^(предпри|тамиянг)/u', $text ) ) {
			$text = preg_replace( '/(?<=и)я/u', 'a', $text );
		}
		$text = preg_replace( '/(?<=и)я/u', 'ä', $text );
		$text = preg_replace( '/(?<=е)я(?=.*[әүеи])/u', 'yä', $text );
		// ья
		// девичья
		...
		$text = preg_replace( '/(?<=^сат)ья/u', 'ya', $text );
		...
		$text = preg_replace( '/(?<=^v)е(?=рстка)/u', 'yo', $text );
		$text = preg_replace( '/(?<=^ф)е(?=дор)/u', 'yo', $text );
		$text = preg_replace( '/(?<=^пос)е(?=лок)/u', 'yo', $text );
		// ё
		$text = preg_replace( '/(?<=[щч])ё/u', 'o', $text );
		$text = preg_replace( '/(?<=[аоуеьъ])ё/u', 'yo', $text );
		// ю
		...
		// ь before й
		$text = preg_replace( '/ь(?=й)/u', '', $text );
		// ь before и
		// ильич il'yiç, ананьино
		$text = preg_replace( '/ь(?=и)/u', '\'y', $text );
		// // брь
		// moved this to convertSimpleRussianWordFromCyrillicToLatin
	...
	function convertSimpleArabicWordFromCyrillicToLatin(
		$text, $beforeThickSuffixes = false, $compoundOtherPart = null
	) {
		if( preg_match( '/^эв/u', $beforeThickSuffixes ) ) {
			$text = preg_replace( '/(?<=и)й$/u', '', $text );
		}
		//
		...
		// әхлак, рәфыйк
		$text = preg_replace( '/(?<=[аыу])к(?![ьи])/u', 'q', $text );
		...
		if(
			...
			&& !preg_match( '/^әдәбият/u', $text )
			&& !preg_match( '/^нәшрият/u', $text )
			&& !preg_match( '/^тәğълимат/u', $text )
			&& !preg_match( '/^gөнаһ/u', $text )
			...
			if(
				...
				// у->ü except эзләучеләр which is wrong spelling
				// of эзләүчеләр and is not arabic word
				$text = preg_replace( '/(?<!ә)у/u', 'ü', $text );
			}
			...
			if( !$beforeThickSuffixes ) {
				...
				$text = preg_replace( '/(?<=^тәğълим)а(?=т($|.*[әүие]))/u', 'ä', $text );
			}
		...
		$text = preg_replace( '/(?<=^риза)э/u', '\'e', $text );
		...
		// if(
			// $text == 'риза'
			// && preg_match( '/^тдин/u', $compoundOtherPart )
		// ) {
			// $text .= '\'';
		// }
		//
	...
	function convertSimpleTatarWordFromCyrillicToLatin( $text, $beforeThickSuffixes = false ) {
		...
		if (
			...
			$text = preg_replace( '/(?<=^)ый(?=дел)/u', 'i', $text );
		} else {
			// yä
			...
			// and btw these words has gone to convertSimpleRussianWordFromCyrillicToLatin
	...
	function convertLowercasedTatarWordLatToCyrl( $text ) {
		...
		$text = preg_replace( '/(?<=sat)ya(?=rtxi)/u', 'ья', $text );
		...
		$text = preg_replace( '/(?<=klement)ye(?=v)/u', 'ье', $text );
		// объектив
		...
		// yü -> ью
		$text = preg_replace( '/(?<=äf)yü(?=n)/u', 'ью', $text );
		// yi
		...
		*/
		...
		$text = preg_replace( '/(?<=d)ä(?=nä)/u', 'а', $text );
		...
		$text = preg_replace( '/(?<=fär)ü(?=q)/u', 'у', $text );
		// e
		// äxläğendä -> әхлагындә
		...
		$text = preg_replace( '/(?<=r[äa]f)i(?=q)/u', 'ый', $text );
	...
	function translate( $text, $toVariant, $test = null ) {
		if( $test ) {
			$this->test = $test;
		}
		if ( $toVariant == 'tt-cyrl' ) {
			...
			$text = strtr(
				$text,
				[
					'б.э.к.'=>'b.e.q.', 'б. э. к.'=>'b. e. q.', 'ЮУЙ'=>'YUY',
					'Г. Камал'=>'Ğ. Kamal', 'Кот-д\''=>'Kot-d\'', 'Ғ'=>'Г', 'ғ'=>'г',
				]
			);
			...
			for ( $i = 1; $i < $wordsCount; $i += 2 ) {
				...
				$words[$i] = preg_replace( "/^ЮТК/u", 'YuTQ', $words[$i] );
				if ( 'ткр' == $words[$i] ) {
					$words[$i] = 'tqr';
				} elseif ( preg_match( '/^КамАЗ[а-яёәөүҗңһ]*$/u', $words[$i] ) ) {
					...
					if ( $wordsICount == 1 || $words[$i][0] != '' ) {
						...
						$words[$i][0] =
							$this->convertLowercasedWordCyrlToLat( $words[$i][0] );
					}
					...
					if (
						...
						&& (
							...
							// $this->surelyAreValidSuffixesAfterThickNoun(
								// [ $words[$i-2], $words[$i] ]
							// )
							|| preg_match(
								'/^ma?k$/ui',
	...
	function convCapitalisedWordCyrlLat( $text ) {
		$text = mb_strtolower( mb_substr( $text, 0, 1 ) )
			. mb_substr( $text, 1 );
		$text = $this->convertLowercasedWordCyrlToLat( $text );
