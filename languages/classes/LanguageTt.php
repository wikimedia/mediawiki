...
class TtConverter extends LanguageConverter {
	...
	function convertLowercasedWordCyrlToLat(
		...
		if (
			preg_match(
				'/^(җигәнтама|йөрәкта|тимер(булат|чыбы)|кайсыбер|миңне.+|түбәнкама)/u',
				$text
			)
		) {
			$parts = preg_split( '/(?<=^.....)/u', $text, null, PREG_SPLIT_NO_EMPTY );
		...
		if ( preg_match( '/^(биектау'
			. '|карабодай|якшыгол|ярымутра)/u', $text )
		) {
			$parts = preg_split( '/(?<=^....)/u', $text, null, PREG_SPLIT_NO_EMPTY );
			return $this->convertCompoundTatarWordCyrlToLat( $parts );
		}
		...
		if ( preg_match( '/^(супер|радио|гидро).+/u', $text ) ) {
			$parts = preg_split( '/(?<=^.....)/u', $text, null, PREG_SPLIT_NO_EMPTY );
			$parts[0] =
		...
		if ( preg_match( '/^(вики|гига|авто|мото|теле).+/u', $text ) ) {
			...
			$nParts[0] =
				$this->convertSimpleRussianWordFromCyrillicToLatin( $parts[0] );
			$nParts[1] = $this->convertLowercasedWordCyrlToLat( $parts[1], $parts[0] );
			$text = implode( $nParts );
			return $text;
		}
		if (
		...
		if ( preg_match( '/.+(баш|шин)/u', $text ) ) {
			$parts = preg_split( '/(?=баш|шин)/u', $text, null, PREG_SPLIT_NO_EMPTY );
			return $this->convertCompoundTatarWordCyrlToLat( $parts );
		}
		if (
			...
			// $parts[0] = $this->convertLowercasedWordCyrlToLat( $parts[0], $matches[1] );
			$parts[0] = $this->convLcTtWordWithoutThickSuffixesCyToLa(
				$parts[0], false, $matches[1]
			);
			// $parts[1] = $this->processWordWithArabicStemCyrlToLat( $parts[1] );
			$parts[1] = $this->convLcTtWordWithoutThickSuffixesCyToLa( $parts[1] );
			$text = implode( $parts );
			return $text;
		}
		...
		if ( preg_match( '/.+стан(?!(т(ин|а)|ц))/u', $text ) ) {
			...
			$parts[0] = $this->
				convertLowercasedWordCyrlToLat( $parts[0] ); // recursion
			$parts[1] = $this->
	...
	*/
	...
	function convertCompoundTatarWordCyrlToLat( $parts ) {
		$nParts[0] = $this->
			// convertSimpleTatarWordFromCyrillicToLatin( $parts[0], $parts[1] );
	...
	// function convertCompoundWordCyrlToLat( $parts ) {
		// $parts[0] = $this->
			// convertLowercasedWordCyrlToLat( $parts[0] );
		// $parts[1] = $this->
			// convertLowercasedWordCyrlToLat( $parts[1] );
		// $text = implode( $parts );
		// return $text;
	// }
