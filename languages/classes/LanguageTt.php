...
class TtConverter extends LanguageConverter {
	...
	function convertLowercasedTatarWordCyrlToLat( $text ) {
		...
		*/
		$s1 = 'с(а|ын)|м[аы]'; // са|ма|мы|сын
		$s2 = '[ркнлшп]|й[км]|ң|гыз'; // р|к|н|л|ш|п|йк|йм|ң|гыз
		$s3 = '$|дыр?|ган'; // ды дыр ган
		$s4 = '$|тыр?|кан'; // ты тыр кан
		if (
			// tatar words and other words that work well with tatar converter
			preg_match(
				'/^('
				// verbs
				// кун кыл ту кал уз etc
				. '(кун?|кый?л|ту|кал|уз)'
				. '('.$s3.'|'.$s1.'|а|ы('.$s2.')|у)'
				// ау etc
				. '|а(у('.$s3.'|'.$s1.'|дар)|в(а(?!тар|рия)|ы('.$s2.')|у))'
				// кат курык арт etc
				. '|(кат|куры?к|арт)'
				. '('.$s4.'|'.$s1.'|а|ы('.$s2.')|у)'
				// кап etc
				. '|ка(п('.$s4.'|'.$s1.'|кын)|б(а|ы('.$s2.')|у))'
				// ак ек как etc
				. '|(а|е|ка)(к('.$s4.'|'.$s1.')|г(а|ы('.$s2.')|у))'
				// noun-likes
				. '|(ка[тп]|еш|арт)($|ка|та[нй]?|ча|ла|ның?|сыз|лы|мы|ы)'
				. '|(уку|ел)($|га|да[нй]?|ча|ла|ның?|сыз|лы|мы|ы)'
				. '|[уя](к($|ка|та[нй]?|ча|ла|ның?|сыз|лы|мы)|гы)'
				// к
				. '|кала[кг]|куллан|калкын|кае[кнш]|каер|кечкенә[йяею]|коерык|кө[йяею]'
				. '|ко[йяею]|кы[йяею](?!фәт|ммәт)|кызы[кг]|кыя[кг]|кояш|кавын|кыз|кар(?!ьер)|капка'
				. '|ки[яею]|коры|коңгырт'
				. '|кабат|комар|канат|ка[йя]|ку[йяею]|кына|куан|кан(?!дид|әга)'
				. '|кыса|казан|кыска|кыр'
				. '|корт|котлуг|ка($|ча)'
				// у
				...
				. '|та[пб](?!игат)|тукта|туфрак|туклык'
				. '|тар[аы]|туглан|ти[яею]?|тарт|тор|тот|та[кг](?!ил)|түбән|туклан|тамга'
				// б
				. '|бөе[кг]|бое|бае|бул|баш|бар|ба[кг](?!ира)|бу[кг]|быел|бу[йе]|буын|бавыр'
				// г
				. '|га($|ча)|гүя|гаян|гыйнвар|гырылда|гына'
				// я
				. '|якын|ямь|яш[әиь]|яса|ярдәм|яр|яхшы|яз|яшел|яфрак'
				// а
				. '|ал|аер|ассызы[кг]|ака[йяею]|авыл|аза[кг]|аркан|авыз'
				. '|аек|аяу|азык|акча|аксым'
				// х
				. '|хоку[кг]|хакан|хә[йе]|хайван'
				// в
				. '|ва[кг](?!он|ыйг)|ваем'
				// җ
				. '|җамыя[кг]|җы[йяею]'
				...
				. '|ышан'
				// с
				. '|сугар|са[кг]|си[яею]|суз|сан|соң|сал|сав|саба[кг]'
				// ю
				. '|юнәл|юеш|юри|юнь|ю[кг]'
				// н
				. '|нократ|нокта|начар'
				// п
				. '|пәрәв|почма[кг]'
		...
		if (
			...
			|| preg_match(
				...
				. '|корбан|коръән|кабер|катгый|кыйммәт'
				// җ
				. '|җавап|җәмгыят|җәмгы|җинаят'
				// в
				// васыяви
				. '|вәкил|вазыйф|вәкал|вә$|ватан|вакыйг|вилаят|васыя|вакыт|вәли|вәзгыят'
				// н
				. '|намзәт|нәүбәт|нәстәгъ|нәгыйм|ниһаять'
				// м
				. '|мөрәҗ|мәгар|мәкал|мәгълүм|максат|маэмай|мөстәкыйль|мәшәкат'
				// г
				...
				. '|гадәт|гыйсъян|гатаулл|гали|гайнан|гошер|голәма|гагауз|габдел|гади'
				// и
				. '|иҗтим|идарә|игътибар|икътис|игълан'
				// т
				...
				. '|пакь'
				// д
				. '|дөнья|диван|дәүләт|дәвам'
				// ә
				. '|әфган|әнвәр|әхлак|әхмәт|әнкара'
				// л
				. '|лихьян|лөгат'
				// р
				. '|расыйх|рөкъг|рия|равия|риза'
				// я
				. '|ягъни|яг[ъа]ф[әа]р|яки|яку[пб]'
				// һ
				. '|һава|һәр'
				// һ
