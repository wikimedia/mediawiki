...
class TtConverter extends LanguageConverter {
		...
		*/
		...
		if (
			...
			preg_match(
				...
				. '|тар[аы]|туглан|ти(?!тикака)|ти[яею]|тукы'
				. '|тарт|тор|тот|та[кг](?!ил)|түбән|тамга'
				// б
				. '|бое|бае|(бөе|бу|байра)[кг]|быел|бу[йе]|буын|бавыр|буя|балкы|бакыр'
				. '|бәя|би[еи]'
				// '|болга(?!рия)'
				// ба[кг](?!ира)
				// г
				...
				// я
				. '|якын|ямь|яш[әиь]|яса|ярдәм|яхшы|яз|яшел|яфрак|ярәш|ялгыз'
				// а
				...
				. '|ышан'
				// с
				. '|сугар|са[кг]|си[яею]|суз|соң|саба[кг]|сыек|сө[ейяю]'
				// ю
		...
		if (
			...
			|| preg_match(
				...
				. '|корбан|коръән|кабер|катгый|кагъбә|каһарман|касыйм|кыямәт|кыйтга'
	...
	function surelyIsATatarVerbWithSuffixes( $text ) {
		...
		$roots = [
			...
			'ку[шп]', 'с?а[ув]', 'тай(ан)?', 'йугал', 'ба[рт]', 'тул?', 'кама', 'сал', 'бул',
			'кы[рй]', '(баш|кот)кар',
			'(ка|а|су|йы|йа|куры?|ба|йабы)[кг]',
	...
	function surelyIsATatarNounWithSuffixes( $text ) {
		...
		$roots = [
			...
			'ка[пб]', 'кай', 'багъ?', 'бакыр', 'йар', 'кул', 'са[нл]', 'баш',
			'(та|йа)[ув]',
			'(у|тайа|йа|йу|чу|ва|ча|айы|ту|ны)[кг]'
		];
		foreach( $roots as $root ) {
			if( preg_match( '/^'.$root.'/u', $text, $matches ) ) {
				$parts = preg_split( '/(?<=^'.$matches[0].')/u', $text, 2, PREG_SPLIT_NO_EMPTY );
	...
	function isValidDivision( $parts ){
		if(
			...
			||
			preg_match( '/^(вол|шпа|ли|оме|синаго|фоль|агале|тон|ри|бри)$/u', $parts[0] )
			&& preg_match( '/^га/u', $parts[1] )
			||
			preg_match( '/^(драматур|то|физи|окру|матери|класси|кубо|катало)$/u', $parts[0] )
			&& preg_match( '/^гы/u', $parts[1] )
			||
			preg_match( '/^(духовной|октябрьской|предприятий|раменской)$/u', $parts[0] )
