	...
	function convertLowercasedTatarWordCyrlToLat( $text ) {
		...
		*/
		...
		if (
			...
			preg_match(
				'/^('
				// short roots with 1-2 next morphemes
				// verbs
				// ку кун кыл кыйл ту кал уз
				. '(кун?|кый?л|ту|кал|уз|тул|яр|ал)'
				. '($|'.$drgan.'|'.$sama.'|а|ы('.$rlpgz.')|у)'
				// ау
				. '|а(у($|д(ы|ар)|ган|'.$sama.')|в(а(?!тар|рия)|ы('.$rlpgz.')|у))'
				// кат арт куш аш
				. '|(кат|арт|куш|аш)'
				...
				// ак ек как як
				. '|(а|е|ка|я)(к($|'.$trqan.'|'.$sama.')|г(а(?!ф[аә]р)|ы('.$rlpgz.')|у))'
				// курык
				. '|кур(ык($|ты|кан|'.$sama.')|к(а|ы([ртнлшп]|й[км]|ң|гыз)|у))'
				// кара кана
				. '|ка[рн](а($|ган|гыз|у|'.$sama.'|'.$trnlpn.'(?!чык|из))|ый)'
				// noun-likes
				// кат еш арт аш
				. '|(кат|еш|арт|аш)($|'.$qata.'|'.$calaniszlimi.'|ы|рак)'
				// кап
				. '|ка(п($|'.$qata.'|'.$calaniszlimi.')|б(ы|рак))'
				// ук як
				. '|[уя](к($|'.$qata.'|'.$calaniszlimi.')|г(ы|рак))'
				// уку ел ком кар казан кан каз
				. '|(уку|ел|ком|ка[рнз]|казан)($|'.$gada.'|'.$calaniszlimi.'|ы|рак)'
				// ал
				...
				. '|ки[яею]|коры|коңгырт|карага[йе]|карта|карават|карт|каршы'
		...
		if (
			...
			|| preg_match(
				...
				// м
				. '|мөрәҗ|мәгар|мәкал|мәгълүм|максат|маэмай|мөстәкыйль|мәшәкат|мәгънә|мәгъдән'
				// г
