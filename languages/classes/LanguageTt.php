...
class TtConverter extends LanguageConverter {
	...
	function convertLowercasedWordCyrlToLat(
		...
		if ( preg_match( '/^((([кт]өн|һәр)ь|(сул|арт)ъ)я[кг]|биектау|(күп|бер|мең)[ьъ]еллы[кг]'
			. '|карабодай|якшыгол|(таш|кул)ъязма|шәльяу|ярымутра)/u', $text )
		) {
			$parts = preg_split( '/(?<=^....)/u', $text, null, PREG_SPLIT_NO_EMPTY );
			return $this->convertCompoundTatarWordCyrlToLat( $parts );
		}
		...
		if ( preg_match( '/^(көн(чыгыш|батыш)|гөлбакча|ч[иы]ная[кг]|башкор[тд]'
			. '|коточкыч|бик.{3,}|аяктубы|күп(сан|кыр)'
			. '|бер(аз|кат|кай(чан)?|туган|вакыт)'
			. '|кай(бер|чак)|һәр(чак|вакыт)|кол(мәт|сәет|мәмәт|бил)|күктау|байегет|(бил|бәк)бау'
	...
	function __construct(
		...
		$this->russianWordsWithFalseThickTatarSuffixesAfterNoun = array_fill_keys(
			[
				...
				'продразверстка', 'вёрстка', 'верстка', 'градишка',
