...
class TtConverter extends LanguageConverter {
	...
	function convert_tatar_from_cyrillic_to_latin( $text ) {
	...
	function toupper($text){
		...
		//$text=str_replace(array('I','ı'),array('İ','I'),$text);//i think this would work without mb_internal_encoding('utf-8');
		return $text;
	}
		...
		if ( $toVariant == 'tt-cyrl' ) {
			...
			for($i=0;$i<count($words);$i++){
				if(0===preg_match('/[А-ЯЁӘӨҮҖҢҺа-яёәөүҗңһ]/u',$words[$i])){
					...
					$capitalisation_type='FC';//first capital
				}elseif(preg_match('/^[а-яёәөүҗңһ\']+$/u',$words[$i])){
					$capitalisation_type='AL';//all low
				}elseif(preg_match('/^[А-ЯЁӘӨҮҖҢҺ]$/u',$words[$i])){
					$capitalisation_type='OC';//1 upper/capital and nothing else
				}elseif(preg_match('/^[А-ЯЁӘӨҮҖҢҺ]{2,}$/u',$words[$i])){
					$capitalisation_type='AC';//all upper/capital
				}else{
					$capitalisation_type='';
				}
				$words[$i]=$this->tolower($words[$i]);
				$words[$i]=$this->convert_tatar_from_cyrillic_to_latin($words[$i]);
				if($capitalisation_type=='FC'){
					$words[$i]=$this->toupper(mb_substr($words[$i],0,1)).mb_substr($words[$i],1);
				}elseif($capitalisation_type=='OC'){
					if(mb_strlen($words[$i])==1){
						$words[$i]=$this->toupper($words[$i]);
					}else{
						$words[$i]=$this->toupper(mb_substr($words[$i],0,1)).mb_substr($words[$i],1);
					}
				}elseif($capitalisation_type=='AC'){
					$words[$i]=$this->toupper($words[$i]);
				}
				//$words[$i]=$capitalisation_type.$words[$i];
			}
			$text=join($words);
		}
