...
class TtConverter extends LanguageConverter {
	...
	function convertLowercasedTatarWordLatToCyrl( $text ) {
		...
		*/
		...
		// ...ts by default is ...ц
		$text = preg_replace( '/ts$/u', 'ц', $text );
		// (cons except yl)ts by default is (cons except yl)ц
		$text = preg_replace( '/(?<![aouıiäöüeаяюеауүэыyl])ts/u', 'ц', $text );
		...
		*/
		...
		// pritseplar motsartnı galitsına vetsek spetsekotrans sotsal' matsuuranıñ kotsyubenko
		...
		// vilemovitse bogoroditsı abbreviatsiä
