...
class TtConverter extends LanguageConverter {
	...
	function convertLowercasedTatarWordCyrlToLat( $text ) {
		...
		if(
			...
			. '|викиҗыенты[кг]|карабодай|якшыгол|ташъязма)/u', $text )
		){
			$parts = preg_split( '/(?<=^....)/u', $text, null, PREG_SPLIT_NO_EMPTY);
			return $this->convertCompoundTatarWordCyrlToLat( $parts );
		}elseif( preg_match( '/^(көн(чыгыш|батыш)|гөлбакча|ч[иы]ная[кг]|башкор[тд]|коточкыч'
			. '|бер(аз|кат|кайчан|туган)'
			. '|кайбер|һәрчак|унъел|колмәт|күктау|акъегет)/u', $text ) ){
			$parts = preg_split( '/(?<=^...)/u', $text, null, PREG_SPLIT_NO_EMPTY);
			return $this->convertCompoundTatarWordCyrlToLat( $parts );
		}elseif( preg_match( '/.+су(?<!басу)(?<!^ысу)/u', $text ) ){
			...
			return $this->convertCompoundTatarWordCyrlToLat( $parts );
		}elseif( preg_match( '/илбашы|аскорма|аккош|суүсем|әрдоган'
			. '|юлтимер|аккирәй/u', $text ) ){
			$parts = preg_split( '/(?<=^..)/u', $text, null, PREG_SPLIT_NO_EMPTY);
			return $this->convertCompoundTatarWordCyrlToLat( $parts );
			...
			$text = implode( $parts );
			return $text;
		}elseif( preg_match( '/^(габдел.+|солтангали)/u', $text ) ){
			$parts = preg_split( '/(?<=^......)/u', $text, null, PREG_SPLIT_NO_EMPTY);
			...
			return $this->convertCompoundTatarWordCyrlToLat( $parts );
		}elseif( preg_match( '/.+стан(?!тин)/u', $text ) ){
		...
		// end of compounds
			...
			preg_match(
				...
				// ать for табигать
				'/әэ|^г(а(яр|еб|йр|лим|дел)(е|лек)?|о(мер|реф|шер))'
				. '|ка(бер|леб)|маэмай|вилаят|аэтдин|ильяс'
				. '|васыят|ать/u'
				, $text )
		){
			...
			|| preg_match( '/[бвгджзклмнпрстфхцчшщ]{3,}(?<![рнс]тт)/u', $text )
			...
			|| preg_match( '/^(к(а(н(дидат|ал)|бина|маз?|рмалка|учук|занка)'
				. '|у(р(с|асан)|кмара)'
				. '|о(м(исс|анда)|нстан|'
					. 'д($|ын?|та(н|гы)?|ка(ча)?|сыз|чы|ның))'
				. '))/u', $text )
			// i try ив$ for объектив etc
			|| preg_match( '/актив|^нигерия|импер|^ефрат|тугрик|сигнал'
				. '|^ив|ив$/u', $text )
			|| preg_match(
				'/^г((аз|ол?)($|да(н|гы)?|га(ча)?|сыз|чы|ның?|ын?)|(о(рилла|а|би)))/u',
				$text )
			...
			|| preg_match( '/шпага|^дог($|ка(ча)?|та(н|гы)?|ның?)|юмья|кизнер/u',
				$text ) // дог
			...
			|| preg_match( '/багира|тагил|^хими|маугли|юбиле|лига|система|республика'
				.'|магнит|органик|извест|нефть|калий|июн|комбинат|дев|самин/u', $text )
			...
			|| preg_match( '/музыка|в(епс|ышка|алда)|пауза|(за|у)каз|состав'
				. '|энерг|бензин|юлия|фея|яков|порту|^гана|гвин'
				. '|бисау|лив|мадаг|уганда|елена|сократ|тиран|оскар'
				. '|мелвин|декрет|соснов|июл/u', $text )
		){
	...
	function processWordWithRussianStemCyrlToLat( $text ){
		...
		// июнь, июль
		if( preg_match( '/^ию[нл]/u', $text ) ){
			return $this->convertSimpleTatarWordFromCyrillicToLatin( $text );
		}
		// октябрь
		$parts = preg_split( '/(?<=[ая])(?=бр[ье])/u',
		...
		// ...лык/лек except электр, молекула, телеграмма, александр, алекс, алексеевск
		$parts = preg_split( '/(?=л[ые][кг])(?!ле(к(тр|ул|с)|грам))/u',
			$text, null, PREG_SPLIT_NO_EMPTY);
		if(count($parts)==2){
		...
		// i do not add exceptions to the following words because
		// there are also same words with qa and they cannot be distinguished
		// without semantics or context
		// Александровка Ивановка Aleksandrovka İvanovka
		// Петровка Petrovka
		// Ульяновка Ul\'yanovka
		// Андреевка Павловка Andreevka Pavlovka
		// Михайловка Косяковка Mixaylovka Kosyakovka
		$parts = preg_split(
			'/((?<!и)(?<!^ёл)(?<!^(шпа|щёт|вол|выш))'
			. '(?<!ссыл|музы|трич|стен|тчат|агер|рмал)'
		...
		$parts = preg_split( '/(?<!аматур)(?<!о)(?=[гк][ые]($|[^нтр]))(?!гель)/u',
			$text, null, PREG_SPLIT_NO_EMPTY);
		if(count($parts)==2){
			return $this->convertRussianWordWithTatarEndingCyrlToLat( $parts );
		}
		...
		$parts = preg_split( '/(?<=[бвгджзклмнпрстфхцчшщ])(?=(ы($|н)))/u',
			$text, null, PREG_SPLIT_NO_EMPTY);
		if(count($parts)==2){
			return $this->convertRussianWordWithTatarEndingCyrlToLat( $parts );
		}
		...
		$parts = preg_split( '/(?<!^(шке|бри))(?=[тд][аә]н?$)/u',
			$text, null, PREG_SPLIT_NO_EMPTY);
		if(count($parts)==2){
			return $this->convertRussianWordWithTatarEndingCyrlToLat( $parts );
		}
		// ...лар/etc except семинар, семинария
		$parts = preg_split( '/(?<!семи)(?=[лн][аә]р)/u',
			$text, null, PREG_SPLIT_NO_EMPTY);
		if(count($parts)==2){
			return $this->convertRussianWordWithTatarEndingCyrlToLat( $parts );
		}
		...
		// активлаштырылган
		$parts = preg_split( '/(?<!^.)(?<!леку)(?=л[аә])(?!лазма)(?<!ярос)/u',
			$text, null, PREG_SPLIT_NO_EMPTY);
		if(count($parts)==2){
			return $this->convertRussianWordWithTatarEndingCyrlToLat( $parts );
		}
	...
	function convertTatarWordWithRussianRootFromCyrillicToLatin( $text ){
		...
		// ye
		// премьер, поезд, менделеевск
		// if( $this->currentWordCapitalisationType == '' )
			// $this->lengthenInArray( '/(?<=[ьо])е/u',
				// $text, $this->currentWordTargetLetterCase );
					// change case array
		$text = preg_replace( '/(?<=[ьъое])е/u', 'ye', $text );
		// if( $this->currentWordCapitalisationType == '' )
	...
	function convertTatarWordWithArabicRootFromCyrillicToLatin( $text ){
		// a,ы,ый,ыя after soft vowel/syllable
		...
		$text = preg_replace( '/(?<=^гыйб)а(?=р)/u', 'ä', $text );
		...
		$text = preg_replace( '/(?<=q)а(?=(бил|вия|ди))/u', 'a', $text );
		// а,о,ы,у,ый,ая,ыя after к,г
		...
		// soft endings: сәламебез, but somebodies started to show "alif"ness
