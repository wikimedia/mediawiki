...
class TtConverter extends LanguageConverter {
	...
	function convertLowercasedWordCyrlToLat(
		...
		if ( preg_match( '/^(вики|гига|авто|мото|теле).+/u', $text ) ) {
			...
			$parts[0] =
				$this->convertSimpleRussianWordFromCyrillicToLatin( $parts[0] );
			$parts[1] = $this->convertLowercasedWordCyrlToLat( $parts[1], $parts[0] );
			$text = implode( $parts );
			return $text;
		}
	...
	function convLcTtWordWithoutThickSuffixesCyToLa(
		...
		if( $text == 'кама' && $beforeThickSuffixes ) {
			return $this->convertSimpleRussianWordFromCyrillicToLatin( $text );
		}
		//
		/*
		...
		*/
		...
		if (
			...
			preg_match(
				...
				'/әэ'
				. '|^г(а((ярь|фиятулл)|(еб|яр|й[рн])е|((лим?|дел|риф)(е|ле|сез)?)|еп(ле|сез))'
		...
		if (
			...
			|| preg_match(
				...
				. '|жан|фта|с(ынка|та|мик)|р(аб|пус|ея)|лум|ка|ала|х|лбурн|пи?|шай'
				. ')'
				// к...
				. '|изнер'
				. ')'
				...
				// бо
				. '|о(л(гария|ив(ия|ар)|дуин)|тинка|гдан)'
				...
				. '|июл|извест'
				// н
				. '|н(и(жгар|уэ)|ебраска|аум|ерв|алим)'
				. '|ноя|нефть'
				// п
				. '|п(ермь|ластинка|а(уза|рагвай)|о(дкаст|вар|лк)|угач)'
				. '|порту|пояс|поп'
				// в
		...
		for( $i = $len - 1; $i >= 2; $i-- ) {
			...
			if(
				...
				if(
					...
					// trying suffixes part beginning with е, while
					// root part is ending with vowel.
					// trying to find root part from exceptions array
					// for example, маэмае - маэмай|ы, валдае - валдай|ы
	...
	function __construct(
		...
		$this->russianWordsWithFalseThickTatarSuffixesAfterNoun = array_fill_keys(
			[
				...
				'продразверстка', 'вёрстка', 'верстка',
