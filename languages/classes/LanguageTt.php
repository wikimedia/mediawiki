...
class TtConverter extends LanguageConverter {
	...
	function convertLowercasedTatarWordCyrlToLat(
		...
		if (
			preg_match( '/.{2,}(намә|ханә|ниса|кяр|улл[аи]|[еэ]тдин|җан)/u', $text, $matches )
		) {
			$parts = preg_split(
		...
		if ( preg_match( '/.+град/u', $text ) ) {
			$parts = preg_split( '/(?=град)/u', $text, null, PREG_SPLIT_NO_EMPTY );
			$parts[0] = $this->
	...
	function __construct(
		...
		$this->wordsWithFalseTatarSuffixes = array_fill_keys(
			[
				...
				'шигый', 'мөгаен', 'табигый', 'яхшы', 'гаеп', 'нәгыйм', 'гает', 'газыйм',
				'гуам', 'имам',
				'елмай', 'маэмай', 'мөлаем',
				'горилла', 'игълан', 'бәла', 'әхлак', 'һәлак', 'купала',
				'валдай', 'команда', 'канада', 'кофта', 'казадаев',
				'яков',
				// 'калий', 'камбий',
