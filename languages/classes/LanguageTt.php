...
class TtConverter extends LanguageConverter {
	...
	function convertLowercasedTatarWordCyrlToLat( $text ) {
		...
		if (
			...
			return $this->convertCompoundTatarWordCyrlToLat( $parts );
		}elseif ( preg_match( '/аскорма|аккош|суүсем|әрдоган'
		...
		// end of compounds
			...
			preg_match(
				...
				// бигате for табигате
				'/әэ'
				. '|^г(а((ярь|фиятулл)|(еб|яр|й[рн])е|((лим|дел)(е|ле))|епле)'
					. '|о(мер|реф|шер))'
				. '|ка(бер|леб|дер)|маэмай|вилаят|аэтдин|ильяс'
				. '|васыят|ать|фирдәүс|бигате|фиркасе|ядкар|шагыйре'
				. '|^гамь|^гаме(?!т)/u',
				$text )
		) {
			...
			|| preg_match(
				'/^(б(агира|о(лгария|тинка))|тагил|х(орватия|ими|арьк)|маугли'
					...
					. '|нижгар|пермь|адмирал|дискусс|эласт)'
				. '/u', $text )
			...
			|| preg_match(
				'/^(м(узыка|елвин|а(даг|ршан|йка))'
					. '|в(епс|ышка|алда|уз)'
					. '|п(ауза|одкаст)|(за|у)каз'
					. '|с(о(с(тав|нов)|к(рат)?|г)|а(рат|ван))'
					. '|б(ензин|исау|ушу|а(вар|рнаул|нк)|урка)'
	...
	function processWordWithRussianStemCyrlToLat( $text ) {
		...
		$parts = preg_split(
			...
			. '|ст[её]р|грей|уфай|осын|убан|ураж|отин|рчат|наго|иров|инов|укот)'
