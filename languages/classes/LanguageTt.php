...
class TtConverter extends LanguageConverter {
	...
	function convertLowercasedTatarWordCyrlToLat( $text ) {
		...
		if ( preg_match( '/^([кт]өнья[кг]|биектау|күпьеллы[кг]'
		...
		if (
			...
			|| preg_match(
				'/^(б(а(гира|лабан|бынин|вар|рнаул|нк|йт|гама)'
					...
					. '|тагил|х(орватия|ими|арьк)|маугли|даут|юрист'
					. '|юбиле|л(и(га|тва)|атвия|окк)|система|республика'
					. '|м(а(гнит|ксим|ргарита)|олдавия)|органик|извест|нефть'
					. '|и(юн|жау|нка|ндивид)|дев|самин|эфир|алфавит|васили|савин'
			...
			|| preg_match(
				'/^(м(у(зыка|скул)|е(лвин|ркель)|а(даг|р(шан|ка)|йка))'
					...
					. '|о(скар|трад|кт)'
					. '|д(екрет|углас|арья)|июл|н(аум|ерв)|талала'
					. '|ре(жисс|зерв)|фуфай'
	...
	function processWordWithRussianStemCyrlToLat( $text ) {
		...
		$parts = preg_split( '/(?<!ага)(?=л[ые][кг])(?!ле(к(т(р|у|и)|ул|с|ом)|г(рам|ац)))/u',
			$text, null, PREG_SPLIT_NO_EMPTY );
		if ( count( $parts ) == 2 ) {
	...
	function convertLowercasedTatarWordLatToCyrl( $text ) {
		...
		$text = preg_replace( '/(?<=^ast)ya(?=z)/u', 'ъя', $text );
		...
		// нью, фьючерс
		$text = preg_replace( '/(?<=(n|f))yu/u', 'ью', $text );
		...
		$text = preg_replace( '/(?<=[^aouıiäöüeiаяюеауү])yu/u', 'ю', $text );
		// yä -> я
		// yä as first letter
		...
		$text = preg_replace( '/(?<=^я[şm])(?![äieü])/u', 'ь', $text );
		// yä after vowels
		// oyä for noyäber
		$text = preg_replace( '/(?<=[äöüoi])yä/u', 'я', $text );
		// ye -> е
		// ye as first letter
		...
		$text = preg_replace( '/(?<=^p)e(?=r)(?!r(m|çat|e))/u', 'э', $text );
		// шонгауэр
		...
		// е -> ы : шәрык
		$text = preg_replace( '/(?<=şär)e/u', 'ы', $text );
		// i
		...
		// some rules for ts found using https://sourceforge.net/p/apertium/svn/HEAD/tree/
		// languages/apertium-tat/apertium-tat.tat.lexc and http://www.corpus.tatar/
		...
		$text = preg_replace( '/^ts(?=[jsktnfşeo])/u', 'тс', $text ); // ТСЖ,тсс,ТСК,ТСТ,ТСН,ТСФ
			// ,ТСШ,Тсэ,ТСО
		$text = preg_replace( '/^ts$/u', 'тс', $text ); // тс
		...
		$text = preg_replace( '/^ts(?=u)/u', 'тс', $text ); // тсутсуми, тсуйоси, тсуда, тсутому
			// , тсутсуи, тсушида,
		...
		*/
		...
		// s as ц
		$text = preg_replace( '/(?<=fran)s(?=uz)/u', 'ц', $text );
		...
		$text = preg_replace( '/(?<=^ğаr)(?![eä])/u', 'ь', $text );
	...
	function translate( $text, $toVariant ) {
		if ( $toVariant == 'tt-cyrl' ) {
			...
			for ( $i = 1; $i < $wordsCount; $i += 2 ) {
				if (
					preg_match( '/^M{0,4}(CM|CD|D?C{0,3})(XC|XL|L?X{0,3})(IX|IV|V?I{0,3})$/',
