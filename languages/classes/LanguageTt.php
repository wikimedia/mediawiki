...
class TtConverter extends LanguageConverter {
	...
	function convertLowercasedTatarWordCyrlToLat( $text ) {
		...
		if ( preg_match( '/.+(намә|ханә)/u', $text ) ) {
			$parts = preg_split( '/(?=намә|ханә)/u', $text, null, PREG_SPLIT_NO_EMPTY );
		...
		*/
		...
		if (
			...
			preg_match(
				...
				// к
				. '|кала[кг]|куллан|калкын|кае[кнш]|каер|кечкенә[йяею]|коерык|кө[йяею]|комачау'
		...
		if (
			...
			|| preg_match(
				...
				. '|гыйффәт|гыйльфан|гайн|гыйният|гашыйк'
				. '|гаҗә[пб]|гыйлем|гомер|гомәр|гореф|гомум|гары[кг]|гыйззәт|гаилә|галәм'
				. '|гадәт|гыйсъян|гатаулл|гали|гайнан|гошер|голәма|гагауз|габд|гади|гадел'
	...
	function surelyIsATatarVerbWithSuffixes( $text ) {
		...
		$roots = [
			...
			'кы[рй]', '(ка|а|су|йы|йа|куры?|ба|йабы)[кг]', '(йокл|кар)[аы]', 'болга', 'кайт'
		];
	...
	function processWordWithRussianStemCyrlToLat( $text ) {
		...
		$parts = preg_split( '/(?<!^.)(?<!леку|ярос|нико|югос|купа|авос|умми)'
			. '(?<!вац|обк|шка|дуг|коа)(?<!^(ве|га|де))(?<!^архипе|святос)'
			. '(?=л[аә])'
			. '(?!ла(зма|кир|бан))/u',
			$text, null, PREG_SPLIT_NO_EMPTY );
		if ( count( $parts ) == 2 ) {
	...
	function convertSimpleArabicWordFromCyrillicToLatin( $text ) {
		...
		if(
			...
			|| preg_match( '/^яки/u', $text )
		) {
			$text = preg_replace( '/а(?!лим|сыят)/u', 'ä', $text );
			...
			$text = preg_replace( '/(?<=[әä])е/u', 'йe', $text );
		...
		// ) {
			...
			// $text = preg_replace( '/(?<=ä)е/u', 'йe', $text );
		} else {
			/*
