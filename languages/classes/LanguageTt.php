	...
	*/
	...
	function tryGetRootOfThickWord( $text, &$parts ) {
		...
		for( $i = $len - 1; $i >= 2; $i-- ) {
			...
			// filter out some thin suffixes, which are easiy found,
			...
			if(
				...
				preg_match( '/ие([^в]|$)/u', $parts[1] )
				&& preg_match( '/[әөү][бвгджҗзклмнңпрстфхһцчшщ]{0,2}ь?$/u', $parts[0] )
				||
				...
				// suffix part is thin, i cannot check thin suffixes now
				// $toReturn = true;
				$parts[0] = $text;
				$parts[1] = '';
				return 0;
			...
			/*
			// commenting out, this is not useful for now.
			// i have then modified "if"s,
			// after uncommenting, this should be put somewhere properly.
			// try suffixes part with russian family name suffix эв,
			// assuming it can be only first.
			$parts = $saved_parts;
			if(
				preg_match( '/^ев/u', $parts[1] )
			) {
				if( preg_match( '/([аоуыяёюэеьә])$/u', $parts[0], $matches ) ) {
					$parts[0] .= 'й';
				}
				...
				if( isset( $this->wordsWithFalseThickTatarSuffixesAfterNoun[$parts[0]] ) ) {
					if(
			...
			*/
			if(
				...
				// and trying to catch root part from exceptions array
				...
				// && preg_match( '/^[^әөү]*[аоуыие]$/u', $parts[0] )
				&& preg_match( '/[аоуые]$/u', $parts[0] )
			) {
				$parts[0] .= 'й';
		...
		// trying for words beginnig with thick verb roots
		$saved_text = $text;
		$text = strtr(
			$text,
			['я'=>'йа', 'е'=>'йы', 'ю'=>'йу']
		...
		for( $i = 2; $i < $len; $i++ ) {
			...
			if(
				...
				// && !preg_match( '/[пк]$/u', $parts[0] )
				isset( $this->thickTatarVerbs[$verb] )
				// if i do not use this, i have to do something
				// with валют|а , курсында|гы etc
				// where first part is understood as verb root
		...
		if( isset( $this->thickTatarVerbs[$text] ) ) {
			$parts[0] = $text;
			$parts[1] = '';
			return 2;
		}
		// trying for nouns not listed in exceptions
		// that nouns maybe false, but they are accepted
		// if suffixes are valid
		$text = $saved_text;
		$len = $saved_len;
		for( $i = 2; $i < $len; $i++ ) {
			$parts[0] = mb_substr( $text, 0, $i );
			...
			$saved_parts = $parts;
			if(
				preg_match( '/([аоуыяёюэеьә])$/u', $parts[0], $matches )
				&& preg_match( '/^е/u', $parts[1] )
			) {
				$parts[0] .= 'й';
				if(
	...
	function __construct(
		...
		$this->russianWordsWithFalseThickTatarSuffixesAfterNoun = array_fill_keys(
			[
				...
				'духовное', 'октябрьское', 'предприятие', 'раменское', 'видение',
				'гуам',
				'гавая', 'парагвай',
