...
class TtConverter extends LanguageConverter {
	...
	function convertLowercasedTatarWordCyrlToLat( $text ) {
		...
		}elseif ( preg_match( '/^җигәнтама|йөрәктау/u', $text )
		) {
			$parts = preg_split( '/(?<=^.....)/u', $text, null, PREG_SPLIT_NO_EMPTY );
			return $this->convertCompoundTatarWordCyrlToLat( $parts );
		}elseif ( preg_match( '/^([кт]өнья[кг]|биектау|күпьеллы[кг]'
			. '|карабодай|якшыгол|ташъязма)/u', $text )
		) {
			$parts = preg_split( '/(?<=^....)/u', $text, null, PREG_SPLIT_NO_EMPTY );
			return $this->convertCompoundTatarWordCyrlToLat( $parts );
		}elseif ( preg_match( '/^вики.+/u', $text )
		) {
			$parts = preg_split( '/(?<=^....)/u', $text, null, PREG_SPLIT_NO_EMPTY );
			$parts[0] =
		...
		// end of compounds
		}elseif ( preg_match( '/(?<=.)(?<!сосн|град)(?<!самин)(?<!петропавл)'
			. '(?<!покр)(?<!^новоникола)'
			// russian village names сосновка, саминовка,
			// константиноградовка, покровка, etc
			...
			preg_match(
				...
				// ать for табигать
				// бигате for табигате
				'/әэ|^г(а(ярь?|еб|йр|лим|дел)(е|лек?)?|о(мер|реф|шер))'
				. '|ка(бер|леб)|маэмай|вилаят|аэтдин|ильяс'
				. '|васыят|ать|фирдәүс|бигате|фиркасе|ядкар'
				. '|^гамь|^гаме(?!т)/u',
				$text )
		) {
			...
			// and except югалтса, югалтмаган, югалткан
			|| preg_match(
				'/[бвгджзклмнпрстфхцчшщ]{3,}(?<![рнс]тт)(?<!лт[смк])/u',
				$text )
			...
			// натураль etc except шөгыль, мөстәкыйль, гыйльфан, мәшгуль
			|| preg_match( '/(?<![гк][ыу])(?<![гк]ый)ль/u', $text )
			...
			|| preg_match( '/^(к('
				. 'а(н(дидат|а(л|да)|түк)|бина|маз?'
				. '|р(малка|амзин|тина)|у(чук|ф)|за(нка|да)|лий)'
				. '|арл($|ын|да(н|гы)?|га(ча)?|сыз|ның)'
				. '|у(р(с|асан)|кмара|туз|зьм)'
				. '|о(м(исс|анда|мун|бинат)|нстан|'
			...
			|| preg_match(
				'/б(агира|олгария)|тагил|^(х(ими|орватия))|маугли'
				. '|юбиле|^л(и(га|тва)|атвия)|система|республика'
				. '|м(а(гнит|ксим)|олдавия)|органик|извест|нефть'
				. '|^июн|дев|самин|эфир|алфавит'
				. '/u', $text )
			...
			|| preg_match( '/музыка|в(епс|ышка|алда)|пауза|(за|у)каз|состав'
				. '|энерг|бензин|юлия|фея|яков|порту|сарат'
				. '|бисау|лив(?!ә)|мадаг|^уганда|елена|сократ|тиран|оскар|бушу'
				. '|мелвин|декрет|соснов|июл|все|маршан|наум|талала'
				. '|отрад|режисс|пед|^бавар|бург|^аугуст/u', $text )
				// все is for ТВсе
		) {
	...
	function processWordWithRussianStemCyrlToLat( $text ) {
		...
		$parts = preg_split( '/(?=л[ые][кг])(?!ле(к(тр|ул|с)|г(рам|ац)))/u',
			$text, null, PREG_SPLIT_NO_EMPTY );
		if ( count( $parts ) == 2 ) {
		...
		$parts = preg_split(
			'/((?<!и)(?<!^(ёл|де))(?<!^(шпа|щёт|вол|выш))(?<!нош)'
			. '(?<!ссыл|музы|трич|стен|тчат|агер|рмал|кров|деле)'
		...
		// ...лар/etc except семинар, семинария, коммунар
		$parts = preg_split( '/(?<!семи|омму)(?=[лн][аә]р)/u',
			$text, null, PREG_SPLIT_NO_EMPTY );
		if ( count( $parts ) == 2 ) {
			return $this->convertRussianWordWithTatarEndingCyrlToLat( $parts );
		}
		...
		// активлаштырылган
		$parts = preg_split( '/(?<!^.)(?<!леку|ярос|нико|югос)(?=л[аә])'
			. '(?!лазма)/u',
			$text, null, PREG_SPLIT_NO_EMPTY );
		if ( count( $parts ) == 2 ) {
			return $this->convertRussianWordWithTatarEndingCyrlToLat( $parts );
	...
	function convertTatarWordWithRussianRootFromCyrillicToLatin( $text ) {
		...
		// ау, оу
		// шоу, боулинг, маугли - w but in пауза, наумов it is u
	...
	function convertTatarWordWithArabicRootFromCyrillicToLatin( $text ) {
		...
		$text = preg_replace( '/(?<=ğ)а(?=(лим?|ди|ен|бде))/u',
	...
	function convertLowercasedTatarWordLatToCyrl( $text ) {
		...
		// пэр except перманганат
		$text = preg_replace( '/(?<=^p)e(?=r)(?!rman)/u', 'э', $text );
		// шонгауэр
		...
		$text = preg_replace( '/(?<=häl)ä(?=[qğ])/u', 'а', $text );
