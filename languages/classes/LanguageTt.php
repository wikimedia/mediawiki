...
// https://tt.wikipedia.org/wiki/%D0%A4%D0%B0%D0%B9%D0%BB:Tatar_telenen_orfografiase_10.jpg
		...
		*/
		...
		if(
			...
			// $text = '{'. $text. '}';
		}elseif(preg_match( '/^[кт]өньяк|биектау/u', $text )){
			$parts = preg_split( '/(?<=([кт]өнь|биек))/u', $text, null, PREG_SPLIT_NO_EMPTY);
			$text = $this->convertCompoundTatarWord( $parts );
			return parent::translate( $text, 'tt-latn' );
		}elseif(preg_match( '/^көн(чыгыш|батыш)|гөлбакча|ч[иы]наяк|башкорт/u', $text )){
			// $parts = preg_split( '/(?<=көн)/u', $text, null, PREG_SPLIT_NO_EMPTY);
			$parts = preg_split( '/(?<=^...)/u', $text, null, PREG_SPLIT_NO_EMPTY);
			$text = $this->convertCompoundTatarWord( $parts );
			return parent::translate( $text, 'tt-latn' );
		}elseif(preg_match( '/ара$/u', $text )){
			...
			$parts = preg_split( '/(?<=[ое]в)/u', $text, null, PREG_SPLIT_NO_EMPTY); // i do
			...
			preg_match(
				...
				'/әэ|^г(а(яр|еб|йр)е|о(мер|реф))|ка(бер|леб)|маэмай|вилаят|аэтдин/u'
				, $text )
		){
			...
			// катер, кушкет
			|| preg_match( '/^[^әөүҗңһ]*[аоу][^әөүҗңһ]*[бвгджзклмнпрстфхцчшщ]+е/u', $text )
			...
			// красивый, трамвай
			// версия, веб
			|| preg_match( '/^(в[иоье]|ш[тп]|[пгбкт]р|сс)/u', $text )
			// акт, пакт, тракт, etc
			...
			|| preg_match( '/[аоу]э|их?а|оу|бр[ье]/u', $text )
			// other russian words
			...
			|| preg_match( '/шпага|^дог($|ка(ча)?|та(н|гы)?|ның?)|юмья|кизнер/u', $text ) // дог
			...
			|| preg_match( '/багира|тагил|^хими|маугли|юбиле/u', $text )
			...
			|| preg_match( '/музыка|вепс|пауза|(за|у)каз|вышка/u', $text )
		){
			// process words with russian stem
			// ...лык/лек except электр
			$parts = preg_split( '/(?=л[ые][кг])(?!лектр)/u', $text, null, PREG_SPLIT_NO_EMPTY);
			if(count($parts)==2){
			...
			// ...лар/etc
			$parts = preg_split( '/(?=[лн][аә]р)/u', $text, null, PREG_SPLIT_NO_EMPTY);
			if(count($parts)==2){
				$text = $this->convertRussianWordWithTatarEnding( $parts );
				return parent::translate( $text, 'tt-latn' );
			}
			...
			// other arabic words:
			|| preg_match( '/куф[аи]|^г(ае[пт]|ыйлем|омум)|^рия/u', $text )
			...
			$parts = preg_split( '/(?=л[аә](($|у|м[аә])(ч[ые])?)?)(?!лаем)/u', $text, null, PREG_SPLIT_NO_EMPTY);
			if(count($parts)==2){
