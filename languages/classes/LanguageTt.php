...
class TtConverter extends LanguageConverter {
	...
	function convertLowercasedTatarWordCyrlToLat( $text ) {
		...
		// end of compounds
			...
			|| preg_match( '/^(к('
				. 'а(н(дидат|ал|түк)|бина|маз?|рмалка|учук|за(нка|да)|рл|лий)'
			...
			|| preg_match( '/багира|тагил|^хими|маугли|юбиле|лига|система|республика'
				. '|ма(гнит|ксим)|органик|извест|нефть'
				. '|^июн|комбинат|дев|самин|эфир'
				. '/u', $text )
			...
			|| preg_match( '/музыка|в(епс|ышка|алда)|пауза|(за|у)каз|состав'
				. '|энерг|бензин|юлия|фея|яков|порту|^гана|гвин|сарат'
				. '|бисау|лив(?!ә)|мадаг|уганда|елена|сократ|тиран|оскар|бушу'
				. '|мелвин|декрет|соснов|июл|все|маршан|наум|талала|отрад/u', $text )
				// все is for ТВсе
		) {
	...
	function processWordWithRussianStemCyrlToLat( $text ) {
		...
		// октябрь
		$parts = preg_split( '/(?<=[ая])(?=бр[ье])(?!брьское)/u',
		...
		$parts = preg_split(
			...
			. '(?<!ссыл|музы|трич|стен|тчат|агер|рмал|кров)'
		...
		// активлаштырылган
		$parts = preg_split( '/(?<!^.)(?<!леку|ярос|нико)(?=л[аә])'
			. '(?!лазма)/u',
			$text, null, PREG_SPLIT_NO_EMPTY );
		if ( count( $parts ) == 2 ) {
			return $this->convertRussianWordWithTatarEndingCyrlToLat( $parts );
	...
	function convertLowercasedTatarWordLatToCyrl( $text ) {
		...
		$text = preg_replace( '/(?<=ğib)ä(?=rät)/u', 'а', $text );
