...
class TtConverter extends LanguageConverter {
	...
	function convertLowercasedTatarWordCyrlToLat( $text ) {
		...
		// end of compounds
		}elseif( preg_match( '/(?<=.)(?<!сосн|град)(?<!самин)'
			...
			|| preg_match( '/^(к(а(н(дидат|ал|түк)|бина|маз?|рмалка|учук|занка|рл)'
				. '|у(р(с|асан)|кмара|туз)'
				. '|о(м(исс|анда|мун)|нстан|'
			...
			|| preg_match( '/багира|тагил|^хими|маугли|юбиле|лига|система|республика'
				. '|магнит|органик|извест|нефть|калий|июн|комбинат|дев|самин|эфир'
				. '/u', $text )
			...
			|| preg_match( '/музыка|в(епс|ышка|алда)|пауза|(за|у)каз|состав'
				...
				. '|бисау|лив(?!ә)|мадаг|уганда|елена|сократ|тиран|оскар'
				. '|мелвин|декрет|соснов|июл|все/u', $text )
				// все is for ТВсе
		){
	...
	function processWordWithRussianStemCyrlToLat( $text ){
		...
		// Кузьминовка Александровка
		$parts = preg_split(
			'/((?<!и)(?<!^ёл)(?<!^(шпа|щёт|вол|выш))(?<!нош)'
	...
	function convertTatarWordWithArabicRootFromCyrillicToLatin( $text ){
		...
		$text = preg_replace_callback( '/^һәла([кг])/u',
			function( $m ){return 'hälä'.( $m[1] == 'к' ? 'q' : 'ğ' );},
			$text );
		// if( $this->currentWordCapitalisationType == '' )
