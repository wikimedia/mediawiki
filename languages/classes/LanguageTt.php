...
class TtConverter extends LanguageConverter {
	...
	function convertTatarWordFromLatinToCyrillic( $text ) {
		...
		*/
		...
		// шөгыль
		$text = preg_replace( '/(?<=ğ)e(.)/u', 'ы$1ь', $text );
		// i
		// i -> ы..ь after q in arabic words
		// тәрәккыять
		$text = preg_replace( '/(?<=q)i(яt)/u', 'ы$1ь', $text );
		// q
		// q -> къ before consonants and at end of word in arabic words
		...
		// ğ
		// ğ -> гъ before consonants and at end of word in arabic words
