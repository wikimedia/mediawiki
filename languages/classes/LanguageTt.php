...
class TtConverter extends LanguageConverter {
	...
	function convertTatarWordFromCyrillicToLatin( $text , &$targetLetterCase ) {
		/*
...
// https://tt.wikipedia.org/wiki/%D0%A4%D0%B0%D0%B9%D0%BB:Tatar_telenen_orfografiase_10.jpg
		...
		*/
		...
		// $text = '('. $text. ')';
		if(
			preg_match('/суүсем|акъегет|көньяк|төньяк|көнчыгыш|көнбатыш/u',$text)
			||preg_match('/илбашы|гөлбакча|аскорма/u',$text)
			||preg_match('/бер(аз|рәт|ничә|дәнбер)/u',$text)
			||preg_match('/.+сыман/u',$text)
			||preg_match('/.+баш/u',$text)
		){
			$text = $this->convertSimpleTatarWordFromCyrillicToLatin( $text , $targetLetterCase );
			// $text = '{'. $text. '}';
		}
		elseif(
			// preg_match('/^[бвгджзклмнпрстфхцчшщ]{2,2}/u',$text)
			preg_match('/^(пр|вв)/u',$text)
			||preg_match('/[бвгджзклмнпрстфхцчшщ][яюёэ]/u',$text)
			||preg_match('/.{2,}о/u',$text)
			||preg_match('/ё/u',$text)
			||preg_match('/^[^әөү]*[ао][бвгджзклмнпрстфхцчшщаыоу]+е/u',$text)
			||preg_match('/[бвгджзклмнпрстфхцчшщ]{3,}/u',$text)
			// ||preg_match('/в[бвгджзклмнпрстфхцчшщ]|[бвгджзклмнпрстфхцчшщ]в/u',$text)
			||preg_match('/в[вгдзклмнртхцчш]|[бвгджзклмнпрстфхцчшщ]в/u',$text)
			||preg_match('/ия.+[аыоу]/u',$text)
			||preg_match('/кандидат/u',$text)
		){
			$parts=preg_split('/(?=л[ые][кг])/u',$text,null,PREG_SPLIT_NO_EMPTY);
			if(count($parts)==2){
				$text = $this->convertRussianWordWithTatarEnding( $parts , $targetLetterCase );
			...
			$parts=preg_split('/(?=[гк][аә](ч[аә])?$)/u',$text,null,PREG_SPLIT_NO_EMPTY);
			if(count($parts)==2){
				$text = $this->convertRussianWordWithTatarEnding( $parts , $targetLetterCase );
				return parent::translate( $text, 'tt-latn' );
			}
			$text = $this->convertTatarWordWithRussianRootFromCyrillicToLatin(
				$text , $targetLetterCase );
		}
		elseif(
			preg_match('/.{2,}и/u',$text)
			||preg_match('/[аыо].*[әү]/u',$text)
			||preg_match('/[әөи].*[ау]/u',$text)
			||preg_match('/әэ/u',$text)
			||preg_match('/[ьъ]ә/u',$text)
		){
			$text = $this->convertTatarWordWithArabicRootFromCyrillicToLatin(
				$text , $targetLetterCase );
		}else{
			$text = $this->convertSimpleTatarWordFromCyrillicToLatin(
				$text , $targetLetterCase );
		}
		return parent::translate( $text, 'tt-latn' );
	}

	function convertTatarWordWithRussianRootFromCyrillicToLatin( $text , &$targetLetterCase ){
		$text = preg_replace( '/в/u', 'v', $text );
		if( $this->currentWordCapitalisationType=='' )
			$this->lengthenInArray( '/(?<=ь)е/u', $text, $targetLetterCase );
		$text = preg_replace( '/(?<=ь)е/u', 'ye', $text );
		if( $this->currentWordCapitalisationType=='' )
			$this->lengthenInArray( '/^е/u', $text, $targetLetterCase );
	...
	function convertTatarWordWithArabicRootFromCyrillicToLatin( $text , &$targetLetterCase ){
		// $text = preg_replace( '/га(?=.+[әүи])/u', 'ğä', $text );
		$text = preg_replace( '/к(?=[аыо])/u', 'q', $text );
		$text = preg_replace( '/г(?=[аыо])/u', 'ğ', $text );
		$text = preg_replace( '/(?<=[qğ])а(?=.+[әеүи])/u', 'ä', $text );
		$text = preg_replace( '/(?<=[qğ])ы(?=.+[әеүи])/u', 'i', $text );
		$text = preg_replace( '/(?<=[qğ])о(?=.+[әеүи])/u', 'ö', $text );
		// if(preg_match('/[әөөүи]/u',$text)){
			// $text = preg_replace( '/ия/u', 'iä', $text );
		// }
		if( $this->currentWordCapitalisationType=='' )
			$this->shortenInArray( '/(?<=и)я/u', $text, $targetLetterCase );
		$text = preg_replace( '/ия/u', 'iä', $text );
		$text = '('. $text. ')';
		return $text;
	}

	function convertSimpleTatarWordFromCyrillicToLatin( $text , &$targetLetterCase ){
		if(preg_match('/[аыоу]/u',$text)){
			$text = preg_replace( '/к/u', 'q', $text );
			$text = preg_replace( '/г/u', 'ğ', $text );
			if( $this->currentWordCapitalisationType=='' )
				$this->lengthenInArray( '/(?<=[аыоу])е/u', $text, $targetLetterCase );
			...
			if( $this->currentWordCapitalisationType=='' )
				$this->lengthenInArray( '/(?<=[әөи])е/u', $text, $targetLetterCase );
			$text = preg_replace( '/(?<=[әөи])е/u', 'ye', $text );
			if( $this->currentWordCapitalisationType=='' )
				$this->shortenInArray( '/(?<=и)я(?=.+[әүеи])/u', $text, $targetLetterCase );
			$text = preg_replace( '/ия(?=.+[әүеи])/u', 'iä', $text );
			$text = preg_replace( '/(?<=ә)ү/u', 'w', $text );
		}
		if( $this->currentWordCapitalisationType=='' )
			$this->replaceIntoArray( '/ь/u', '', $text, $targetLetterCase );
		$text = preg_replace( '/ь/u', '', $text );
		$text = '|'. $text. '|';
		return $text;
	}

	function convertRussianWordWithTatarEnding( $parts , &$targetLetterCase ){
		$parts[0] = $this->
			convertTatarWordWithRussianRootFromCyrillicToLatin(
				$parts[0] , $targetLetterCase );
		$parts[1] = $this->
			convertSimpleTatarWordFromCyrillicToLatin(
				$parts[1] , $targetLetterCase );
		$text = implode( $parts );
		return $text;
	}
	...
	function convertTatarWordFromLatinToCyrillic( $text ) {
		...
		// $text = preg_replace( '/Q[äÄәÄ]/u', 'Ка', $text );
			$text = preg_replace( '/ğe([bcçdfghjklmnñpqrsştvwxyz])(?!e)/u', 'гы$1ь', $text ); // шөгыль
	...
	function shortenInArray( $pattern, $subject, &$target ) {
		...
		if ( $patternCount > 0 ) {
			for ( $i = 0; $i < $patternCount; $i++ ){
				$target[$matches[0][$i][1]] = mb_substr( $target[$matches[0][$i][1]], 0, 1 );
			}
		}
		// fwrite($logfileqdb,
			// ':'.var_export($target,true)
		...
		if ( $toVariant == 'tt-cyrl' ) {
			...
			for ( $i = 0; $i < $wordsCount; $i++ ) {
				...
				if ( 0 === preg_match( '/[a-zA-ZäəÄƏöɵÖƟüÜıİşŞçÇñÑğĞ\']/u', $words[$i] ) ) {
					...
					$this->currentWordCapitalisationType = 'OC'; // only 1 letter and it is upper/capital
				// } elseif (
				...
				// ){
					...
					$this->replaceIntoArray(
						'/[a-zäəöɵüışçñğ\'0-9]/u', 'l', $tmp, $targetLetterCase );
					$this->replaceIntoArray(
						'/[A-ZÄƏÖƟÜİŞÇÑĞ]/u', 'u', $tmp, $targetLetterCase
					);
					// $this -> replaceIntoArray(
						// '/[еёяюцщ]/u', 'll', $words[$i], $targetLetterCase);
					// $this -> replaceIntoArray(
						// '/[ЕЁЯЮЦЩ]/u', 'ul', $words[$i], $targetLetterCase);
					// $this -> replaceIntoArray(
						// '/[ьЬъЪ]/u', '', $words[$i], $targetLetterCase);
					ksort( $targetLetterCase );
					$targetLetterCase = implode( $targetLetterCase );
				}
				$words[$i] = $this->toLower( $words[$i] );
				$words[$i] = $this->convertTatarWordFromLatinToCyrillic( $words[$i] );
				// $words[$i] = implode( $targetLetterCase ); // uncomment to see target cases
			...
			for ( $i = 0; $i < $wordsCount; $i++ ) {
				...
				// ){
					...
					// $targetLetterCase = array();
					$this->replaceIntoArray(
						'/[0-9а-еж-хчшыьэәөүҗңһ]/u', 'l', $words[$i], $targetLetterCase );
						// digits also need not to be capitalised
					$this->replaceIntoArray(
						'/[А-ЕЖ-ХЧШЫЬЭӘӨҮҖҢҺ]/u', 'u', $words[$i], $targetLetterCase
					);
					$this->replaceIntoArray(
						'/[ёяюцщ]/u', 'll', $words[$i], $targetLetterCase );
					$this->replaceIntoArray(
						'/[ЁЯЮЦЩ]/u', 'ul', $words[$i], $targetLetterCase );
					$this->replaceIntoArray(
						'/[ъЪ]/u', '', $words[$i], $targetLetterCase );
					/*
					// i comment these out because style of writing all with upper case while
					// it is not an abbreviation is probably not used in wikipedia
					$this -> replaceIntoArray(
						'/(?<=[уУ])е(?=[вВ])/u', 'll', $words[$i], $targetLetterCase);
					$lcCons='бвгджҗзклмнңпрстфхһцчшщ';
					$ucCons=mb_strtoupper('бвгджҗзклмнңпрстфхһцчшщ');
					$cons=$lcCons.$ucCons;
					$this -> replaceIntoArray(
						'/(?<=['. $cons. '])ю(?=\w+[аыоАЫО])/u',
						'-l', $words[$i], $targetLetterCase
					);
					$this -> replaceIntoArray(
						'/(?<=['. $cons. '])Ю(?=\w+[аыоАЫО])/u',
						'-u', $words[$i], $targetLetterCase
					);
					$this -> replaceIntoArray( '/(?<=ы)й(?=[бвгджҗзклмнңпрстфхһцчшщ]+[ьә])/ui', ''
						, $words[$i], $targetLetterCase ); // мөстәкыйль-möstäqil,гыйффәт-ğiffät
					$this -> replaceIntoArray( '/(?<=\b)Е(?=РЭ)/u', 'ul'
						, $words[$i], $targetLetterCase ); // ЕРЭ
					$this -> replaceIntoArray( '/(?<=\b|[ъь])е/u', 'll'
						, $words[$i], $targetLetterCase );
