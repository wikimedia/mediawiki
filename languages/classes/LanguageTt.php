...
class TtConverter extends LanguageConverter {
	...
	function convertLowercasedTatarWordCyrlToLat( $text ) {
		...
		*/
		// words that would be catched as tatar words, but that are rarely used tatar words,
		// i catch them as russian words
		if(
			preg_match( '/^(канаш)/u', $text )
		) {
			return $this->processWordWithRussianStemCyrlToLat( $text );
		}
		if(
		...
		if (
			...
			preg_match(
				...
				// б
				. '|бөе[кг]|бое|бае|бул|баш|ба[кг](?!ира)|бу[кг]|быел|бу[йе]|буын|бавыр|буя'
				. '|бәя|болга(?!рия)'
				// г
				...
				// ю
				. '|юнәл|юеш|юри(?!дик)|юнь|югары|юкә|югал'
				// н
		...
		if (
			...
			|| preg_match(
				...
				. '|корбан|коръән|кабер|катгый|кыйммәт|кади|каф|кари'
				// җ
				...
				// м
				. '|мөрәҗ|мәгар|мәкал|мәгълүм|максат'
				...
				. '|гыйбарәт|гафур|гам(?!ет|аш)|гыйбад|гафият'
				// и
				...
				// ә
				. '|әфган|әнвәр|әхлак|әхмәт|әнкара|әүхәди'
				// л
				. '|лихьян|лөгат'
				// р
	...
	function surelyIsATatarVerbWithSuffixes( $text ) {
		...
		$roots = [
			...
			'ку[шп]', 'с?а[ув]', 'тай(ан)?', 'йугал', 'ба[рт]', 'тул?', 'кама',
			'кыр', '(ка|а|су|йы|йа|куры?)[кг]', '(йокл|кар)[аы]'
		];
	...
	function surelyIsATatarNounWithSuffixes( $text ) {
		$text = preg_replace( ['/я/u', '/е/u', '/ю/u', ], ['йа', 'йы', 'йу'], $text );
		$roots = [
			'ко[тм]', 'матур', 'ун', 'тугыз', 'алты?', 'алма', 'алд?', 'савыт',
	...
	function areValidSuffixesAfterNoun( $parts ) {
		...
		if(
			...
			&& 0 < preg_match_all( '/^(а|ың)/u', $text, $matches )
			||
			preg_match( '/ы[мң]$/u', $parts[0] )
			&& 0 < preg_match_all( '/^(а|ның)/u', $text, $matches )
		) {
			foreach( $matches[0] as $match ){
				$suffixes_parts = preg_split(
