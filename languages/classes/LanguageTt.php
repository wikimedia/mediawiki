...
class TtConverter extends LanguageConverter {
	...
	function surelyIsATatarVerbWithSuffixes( $text ) {
		...
		if( 0 < preg_match_all(
			'/^('
			. 'кун?|кый?л|тул?|ка[лтк]|уз|йар|ал|укы?'
			. '|с?а[ув]|тай(ан)?|йугал|ба[рт]|тул?|сук|кыр'
	...
	function surelyIsATatarNounWithSuffixes( $text ) {
		$text = preg_replace( ['/я/u', '/е/u', '/ю/u', ], ['йа', 'йы', 'йу'], $text );
		if( 0 < preg_match_all(
			'/^(кот|матур|тайак|ун|тугыз|алты?|алма|урман|кита[пб])/u'
			, $text, $matches )
		) {
			foreach( $matches[0] as $match ) {
				$parts = preg_split( '/(?<=^'.$match.')/u', $text, 2, PREG_SPLIT_NO_EMPTY );
	...
	function areValidSuffixesAfterQal( $parts ) {
		...
		if(
			...
			&& (
				...
				preg_match( '/^([мкпңш]|г(а[нк]|ы(ры|[зч]?)))/u', $text, $matches )
				||
			...
			||
			// в is checked below
			( !( $awVerbAu || $pVerbp || $qVerbq ) || $parts[0] == 'ук' )
			&& preg_match( '/^у/u', $text, $matches )
			||
			( $stcsVerb || $pVerbp || $qVerbq )
			&& preg_match( '/^к(ы(ры|ч?)|а[нк])/u', $text, $matches )
			||
			// $aoVerb is checked above
			( $zylmnuVerb || $awVerbAu || $rVerb )
			&& preg_match( '/^г(ы(ры|ч?)|а[нк])/u', $text, $matches )
		) {
			$suffix = $matches[0];
			$suffixes_parts = preg_split( '/(?<=^'.$suffix.')/u', $text, 2, PREG_SPLIT_NO_EMPTY );
			if( isset( $suffixes_parts[1] ) ) {
	...
	function areValidSuffixesAfterAwgan( $parts ) {
		...
		if(
			0 < preg_match_all(
				'/^(ча|ның?|сыз|лык?|лар|мын?|сың|быз|дыр|рак|чык?|ау)/u',
				$text, $matches
			)
			||
			...
			||
			// китабы, китабым, китабың, китабыгыз, китабыбыз, китаплары
