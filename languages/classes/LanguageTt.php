...
class TtConverter extends LanguageConverter {
	...
	function convertLowercasedTatarWordCyrlToLat( $text ) {
		...
		*/
		if( 0 < preg_match_all(
			'/^(кун?|кый?л|тул?|кал|уз|яр|а[лув])/u'
			, $text, $matches )
		){
	...
	function areValidSuffixesAfterQal( $parts ) {
		...
		$text = $parts[1];
		if(
			preg_match( '/^ы[нл]/u', $text )
			&& ( $zylmnuVerb || $awVerbAv || $pVerbb || $qVerbg || $rVerb )
			||
			preg_match( '/^[нл]/u', $text ) && $aoVerb
		) {
			// $suffix = $matches[0];
			$parts = preg_split( '/(?<=^..)/u', $text, 2, PREG_SPLIT_NO_EMPTY );
			if( isset( $parts[1] ) ) {
				// return $this->areValidSuffixesAfterAlon( $parts[1] );
				return $this->areValidSuffixesAfterQal( $parts );
			}
			return true;
		}
		if(
			...
			$parts = preg_split( '/(?<=^..)/u', $text, 2, PREG_SPLIT_NO_EMPTY );
			if( isset( $parts[1] ) ) {
				// if( $this->areValidSuffixesAfterAlosVerb( $parts[1] ) ){
				if( $this->areValidSuffixesAfterQal( $parts ) ){
					return true;
				}
				return $this->areValidSuffixesAfterAwgan( $parts );
			}
			return true;
		}
		if(
			...
			if( isset( $parts[1] ) ) {
				// return $this->areValidSuffixesAfterAwdar( $parts[1] );
				return $this->areValidSuffixesAfterQal( $parts );
			}
			return true;
		}
		if(
			...
			$parts = preg_split( '/(?<=^.)/u', $text, 2, PREG_SPLIT_NO_EMPTY );
			if( isset( $parts[1] ) ) {
				// return $this->areValidSuffixesAfterAlosVerb( $parts[1] );
				return $this->areValidSuffixesAfterQal( $parts[1] );
			}
			return true;
		}
		if( preg_match( '/^м[аы]/u', $text ) && !( $awVerbAv || $pVerbb || $qVerbg )  ) {
			...
			$parts = preg_split( '/(?<=^..)/u', $text, 2, PREG_SPLIT_NO_EMPTY );
			if( isset( $parts[1] ) ) {
				return $this->areValidSuffixesAfterAwmaVerb( $parts[1] );
			}
			return true;
		}
		if(
			...
			if( isset( $parts[1] ) ) {
				...
				if(
					...
					&& preg_match( '/[ая].$/u', $parts[0] )
					// to do: ятар but not калар
				) {
					// $suffix = $matches[0];
					$parts = preg_split( '/(?<=^..)/u', $text, 2, PREG_SPLIT_NO_EMPTY );
					if( isset( $parts[1] ) ) {
		...
		if( preg_match( '/^(ды|са)/u', $text ) && !$awVerbAv ) {
			...
			$parts = preg_split( '/(?<=^..)/u', $text, 2, PREG_SPLIT_NO_EMPTY );
			if( isset( $parts[1] ) ) {
				return $this->areValidSuffixesAfterQalsa( $parts[1] );
			}
			return true;
		}
		return false;
	...
	function areValidSuffixesAfterQalsa( $text ) {
		...
		if( preg_match( '/^м/u', $text, $matches ) ) {
			$parts = preg_split( '/(?<=^'.$matches[0].')/u', $text, 2, PREG_SPLIT_NO_EMPTY );
			if( isset( $parts[1] ) ) {
				return $this->areValidSuffixesAfterQalsam( $parts[1] );
			}
			return true;
		}
