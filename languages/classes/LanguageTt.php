...
class TtConverter extends LanguageConverter {
	...
	function convertLowercasedTatarWordCyrlToLat( $text ) {
		...
		if ( preg_match( '/^(көн(чыгыш|батыш)|гөлбакча|ч[иы]ная[кг]|башкор[тд]'
			...
			. '|кайбер|һәрчак|унъел|кол(мәт|сәет)|күктау|(акъ|бай)егет|билбау'
			. '|күзал)/u', $text ) ) {
		...
		if (
			...
			|| preg_match(
				'/^(б(а(гира|лабан|бынин|вар|рнаул|нк|йт)'
					. '|о(л(гария|ив(ия|ар))|тинка)'
					. '|елоус|ензин|исау|ушу|урка)'
					. '|тагил|х(орватия|ими|арьк)|маугли'
	...
	function processWordWithRussianStemCyrlToLat( $text ) {
		...
		$parts = preg_split(
			'/(?<!аматур)(?<!о)(?<!^ма)(?<!^хок)(?<!^(физи|окру|нинс))(?=[гк][ые]($|[^нтр]))(?!гель)/u',
			$text, null, PREG_SPLIT_NO_EMPTY );
		if ( count( $parts ) == 2 ) {
			return $this->convertRussianWordWithTatarEndingCyrlToLat( $parts );
		}
	...
	function convertTatarWordWithRussianRootFromCyrillicToLatin( $text ) {
		...
		// белоусово it is u
		$text = preg_replace( '/(?<=[оа])(?<!па|на)(?<!бело)у/u', 'w', $text );
		// ь before ю
		// компьютер
