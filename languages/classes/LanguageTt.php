...
class TtConverter extends LanguageConverter {
		...
		*/
		...
		if (
			...
			preg_match(
				...
				// б
				. '|бөе[кг]|бое|бае|бул|баш|бу[кг]|быел|бу[йе]|буын|бавыр|буя'
				. '|бәя|болга(?!рия)'
				// ба[кг](?!ира)
				// г
				...
				// а
				. '|аер|ассызы[кг]|ака[йяею]|авыл|аза[кг]|авыз'
				...
				// ч
				. '|чө[йяею]|чекерә[йяею]|чә[йе]|чуен|чыелда|чокыр|чы[кг]'
				// е
				. '|ера[кг]|елма[йеяю]'
				// о
				. '|о[яею]|оеш|очра'
		...
		if (
			...
			|| preg_match(
				...
				. '|корбан|коръән|кабер|катгый|кагъбә'
				. '|кыйммәт|кади|каф|кари|кадер|кардәш|каһин'
				// җ
				. '|җавап|җәмгыят|җәмгы|җинаят'
				// в
				// васыяви
				. '|вәкил|ваз(ый|и)ф|вәкал|вә$|ватан|вакыйг'
				. '|вилаят|васыя|вакыт|вәли|вәзгыят|в[аә]гыйз'
				// н
				. '|намзәт|нәүбәт|нәстәгъ|нәгыйм|ниһаять|нәзарәт|нәкъ'
				// м
				. '|мөрәҗ|мәгар|мәкал|мәгълүм|максат|мәэмүр'
				...
				. '|гае[пб]|гар(?!анти|сия|ри|дин|филд)|гыйффәт|гыйльфан|гайн|гыйният'
				. '|гаҗә[пб]|гыйлем|гомер|гомәр|гореф|гомум|гары[кг]'
				...
				// и
				. '|иҗтим|идарә|игътибар|икътис|игълан|ильяс|инкыйлаб'
				// т
				...
				// х
				. '|хөсәен|хикәя|халәт|хәрабә'
				// ф
				. '|фагыйләт|фигыл|фирка|фидакар'
				// ш
	...
	function surelyIsATatarVerbWithSuffixes( $text ) {
		...
		// echo $text;
		$roots = [
			'кун?', 'кый?л', 'тул?', 'ка[лтп]', 'уз', 'йар', 'а[лш]', 'укы?', 'арт',
			'ку[шп]', 'с?а[ув]', 'тай(ан)?', 'йугал', 'ба[рт]', 'тул?', 'кама', 'бакыр',
			'кыр', '(ка|а|су|йы|йа|куры?|ба)[кг]', '(йокл|кар)[аы]'
		];
		foreach( $roots as $root ) {
			if( preg_match( '/^'.$root.'/u', $text, $matches ) ) {
				...
				if( !isset( $parts[1] ) ) {
					...
					// echo $parts[1];
					if( $this->areValidSuffixesAfterVerb( $parts ) ) {
						return true;
					}
				}
			}
		...
		if( 0 < preg_match_all(
			'/^('
			. 'кун?|кый?л|тул?|ка[лткп]|уз|йар|а[лшк]|укы?|арт|ку[шп]'
			. '|с?а[ув]|тай(ан)?|йугал|ба[рт]|тул?|кыр'
			. '|(су|йы|йа|куры?)к'
			. '|(аш|йокл|кар|ялтыр)[аы]'
			. ')/u'
			, $text, $matches )
			...
			// (and there is indeed such suffix, seems rare variant of ган),
	...
	function surelyIsATatarNounWithSuffixes( $text ) {
		...
		$roots = [
			...
			'урман', 'аш', 'йалтыр', 'кар?т', 'йы[лш]', 'арка', 'аркан',
			'арт', 'ка[рнз]', 'казан', 'казакъ?',
			'ка[пб]', 'кай', 'баг', 'бакыр',
			'(у|тайа|йа|йу|чу|ва)[кг]'
		];
		foreach( $roots as $root ) {
			if( preg_match( '/^'.$root.'/u', $text, $matches ) ) {
				$parts = preg_split( '/(?<=^'.$matches[0].')/u', $text, 2, PREG_SPLIT_NO_EMPTY );
				if( !isset( $parts[1] ) ) {
					...
					// echo $parts[1];
					if( $this->areValidSuffixesAfterNoun( $parts ) ) {
						return true;
					}
				}
			}
		...
		if( 0 < preg_match_all(
			'/^('
			. 'ко[тм]|матур|ун|тугыз|алты?|алма|алд?'
			. '|урман|аш|йалтыр|кар?т|йы[шл]|арт|ка[рнз]|казан'
			. '|ка[пб]'
			. '|(у|тайа|йа|йу)[кг]'
			. ')/u'
			, $text, $matches )
	...
	function areValidSuffixesAfterVerb( $parts ) {
		$awVerbAv = false;
		$awVerbAu = false;
		$aoVerb = false;
		$rVerb = false;
		...
		if(
			...
			if( isset( $suffixes_parts[1] ) ) {
				...
				return $this->areValidSuffixesAfterVerb( $suffixes_parts );
			}
			return true;
		}
		if(
			...
			if( isset( $suffixes_parts[1] ) ) {
				...
				if( $this->areValidSuffixesAfterVerb( $suffixes_parts ) ){
					return true;
				}
				return $this->areValidSuffixesAfterNoun( $suffixes_parts );
			}
			return true;
		}
		if(
		...
		if(
			...
			$suffixes_parts = preg_split( '/(?<=^.)/u', $text, 2, PREG_SPLIT_NO_EMPTY );
			if( isset( $suffixes_parts[1] ) ) {
				if( $this->areValidSuffixesAfterNoun( $suffixes_parts ) ) {
					return true;
				}
				if(
					preg_match( '/^ар/u', $text, $matches )
	...
	function areValidSuffixesAfterNoun( $parts ) {
		$text = $parts[1];
		if(
			0 < preg_match_all(
