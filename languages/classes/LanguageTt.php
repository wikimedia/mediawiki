	...
	function convertLowercasedTatarWordCyrlToLat( $text ) {
		...
		if (
			...
			|| preg_match(
				...
				. '|а(ук|на|м(аш|бия)|зимов|р(ант|сия|ри|дин|филд)'
				// го
				. '|(о(р(илла|ки|ьк)|а|би|лланд|фман))'
				// г...
				. '|л(мудуг|лия)|биния|й$|рхипел|ити|вай)'
				. '|вин|у(м(анитар|ми)|бан)|еве'
				. ')'
				// б
				. '|б('
				// ба
				...
				. '|ю(рист|биле)'
				// л
				. '|л(и(га|тва)|окк|а(всан|йкра|нка|твия))'
				// м
				. '|м(а(гнит|ксим|ргарита)|олдавия|ичиган)'
				// и
				. '|и(юн|жау|нка|н(дивид|тер))'
				// н
				. '|н(ижгар|ебраска|аум|ерв)'
				// п
				. '|п(ермь|ластинка|а(уза|рагвай)|о(дкаст|вар|лк))'
				// м
				. '|м(у(зыка|скул)|е(лвин|ркель)|а(даг|р(шан|ка)|йка|к))'
				// в
				...
				// д
				. '|д(екрет|углас|арья|иалект)'
				// р
				. '|ре(жисс|зерв)'
				// а
				. '|а(ксаков|грар|угуст|рхи|ньелли|ква|ляска)'
				// з
				. '|з(игзаг|акари)'
				// у
				. '|у(аскаран|о(ррен|кер))'
				// etc
				...
				. '|талала'
				. '|фуфай'
				. '|пояс|йошкар|ноя|датир|чабакса'
				. '|юридик|янка|сервис'
				. '|маугли|даут'
				. '|система|республика'
				. '|органик|извест|нефть'
				. '|дев|самин|эфир|алфавит|васили|савин'
				. '|адмирал|дискусс|эласт|сингапур|физи|агалега'
				. '|окру|дивизи|субъект|объект|жук|юхн|рабфа|фукс'
				. '|марк|эпикур|фолликул|йорк'
				. ')'
				. '/u', $text )
	...
	function processWordWithRussianStemCyrlToLat( $text ) {
		...
		$parts = preg_split(
			...
			. '|фоль|клад|иров|анио|калу|аляс)'
				...
				// ин - сапа инка
			. '(?<!казан|любов|агале)'
			. '(?<!соснов|шешмин|градов|амазон|титика|небрас)'
			. '(?<!саминов|поварён|дозиров|пластин)'
			. '(?<!кондоркан)'
			. '(?<!петропавлов)'
	...
	function convertLowercasedTatarWordLatToCyrl( $text ) {
		...
		*/
		...
		$text = preg_replace( '/(?<!(jabl|atel|vest|neft))\''
			.'(?=n[eı]ñ?|t[aä]n?|d[aä]n?|ğa|gä|qa|kä|l(ıq?|ek?)|[eı](n|$))/u', '-', $text );
