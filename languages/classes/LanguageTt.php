...
class TtConverter extends LanguageConverter {
	...
	function convertLowercasedTatarWordCyrlToLat( $text, $compoundFirstPart = null ) {
		...
		if ( preg_match( '/аскорма|аккош|су(үсем|тудыр)|әрдоган|азкөч'
		...
		*/
		...
		if (
			...
			preg_match(
				...
				. '|та[пб](?!игат)|тукта|туфрак|тарка|тутыр|тавыш'
				. '|тар[аы]|туглан|ти(?!тикака)|ти[яею]'
				. '|тарт|тор|тот|та[кг](?!ил)|түбән|тамга'
				// б
		...
		if (
			...
			|| preg_match(
				...
				// м
				. '|мөрәҗ|мәгар|мәкал|мәгълүм|максат|мәэмүр|маһи|мәдән'
				...
				. '|табигат|тәгаен|тәмуг|тәкъвим|тәмам'
				// ш
				. '|шөгыл|шигыр|шәры[кг]'
				// с
				...
				. '|лихьян|лөгат|лалә'
				// р
				. '|расыйх|рөкъг|рия|равия|риза|ризык|рәфыйк|риваят|робагый'
				// я
				. '|ягъни|яг[ъа]ф[әа]р|яки|яку[пб]|ядкар'
				// һ
				. '|һава|һәр|һәла[кг]'
				// б
				. '|бәян|бәләгат|бәгыр|балигъ|бәлигъ|бәлагат|бәһа|бәла|бакый'
				// х
				. '|хөсәен|хикәя|халәт|хәрабә|хак(?!ас)|ханә|хәрәкә'
				// ф
	...
	function surelyIsATatarNounWithSuffixes( $text ) {
		...
		$roots = [
			...
			'арт', 'ка[рнз]', 'казан', 'казакъ?', 'болгар', 'җай', 'тук',
			'ка[пб]', 'кай', 'баг', 'бакыр', 'чак', 'йар', 'кул',
			'(у|тайа|йа|йу|чу|ва|ча|айы)[кг]'
		];
		foreach( $roots as $root ) {
			if( preg_match( '/^'.$root.'/u', $text, $matches ) ) {
				$parts = preg_split( '/(?<=^'.$matches[0].')/u', $text, 2, PREG_SPLIT_NO_EMPTY );
	...
	function tryGetRoot( $text ) {
		for( $i = 2; $i <= 11; $i++ ) {
			$parts[0] = mb_substr( $text, 0, $i );
			$parts[1] = mb_substr( $text, $i );
			if( $this->isValidDivision( $parts ) ) {
				return $parts;
			}
			if(
	...
	function __construct(
		...
		$this->russianWordsEndingWithKa = array_fill_keys(
			[
				'щёт', 'электрич', 'выш', 'клетчат', 'золотонош', 'олив', 'курт', 'кеп', 'перчат',
				'чукот', 'обклад', 'аляс', 'небрас', 'вентрес',
			],
			1
		);
	}
	...
	function isValidDivision( $parts ){
		if(
			...
			||
			preg_match( '/^(вол|шпа|ли|оме|синаго|фоль|агале|тон)$/u', $parts[0] )
			&& preg_match( '/^га/u', $parts[1] )
			||
			preg_match( '/^(драматур|то|физи|окру|матери|класси)$/u', $parts[0] )
			&& preg_match( '/^гы/u', $parts[1] )
			||
			preg_match( '/^(духовной|октябрьской|предприятий)$/u', $parts[0] )
			&& preg_match( '/^ы/u', $parts[1] )
			||
			preg_match( '/^(ба)$/u', $parts[0] )
			&& preg_match( '/^рак/u', $parts[1] )
			||
			preg_match( '/^(гуа)$/u', $parts[0] )
		...
		// $unmodifiedSuffixes = $parts[1];
		$parts[1] = preg_replace( ['/я/u', '/е/u', '/ю/u'], ['йа', 'йы', 'йу'], $parts[1] );
		if(
			!preg_match( '/[әөүеэи]/u', $parts[1] )
			&& $this->surelyAreValidSuffixesAfterThickNoun( $parts )
		) {
			// $parts[1] = $unmodifiedSuffixes;
			return true;
		}
		return false;
	}
	...
	function surelyAreValidSuffixesAfterThickNoun( $parts ) {
		...
		if(
			0 < preg_match_all(
				'/^(ча|ның|сыз|лы[кг]|лар|мын|сың|быз|дыр|рак|чы[кг])/u',
				$text, $matches
			)
			||
			// лы is moved down
			...
			// after russian words
			// also о for го etc
			preg_match( '/[бвгдкпстфхцчшщ][ьъ]?$/u', $parts[0] )
			&& 0 < preg_match_all( '/^(к[аы]|та[нйш]?|тыр)/u', $text, $matches )
			||
			preg_match( '/([оаеёийуыэюя]|[жзлмнр][ьъ]?)$/u', $parts[0] )
		...
		if(
			...
			if( isset( $suffixes_parts[1] ) ) {
				...
				if(
					preg_match( '/^ы[кг]/u', $text, $matches )
					&& $parts[0] == 'кан'
				) {
					// $suffix = $matches[0];
					$suffixes_parts = preg_split(
	...
	function processWordWithRussianStemCyrlToLat( $text ) {
		...
		$parts = preg_split( '/(?<!(ага|диа))'
			...
			.'(?=лы[кг])(?!ле(к(т(р|у|и)|ул|с|ом)|г(рам|ац)))/u',
			$text, null, PREG_SPLIT_NO_EMPTY );
		if ( count( $parts ) == 2 ) {
			return $this->convertRussianWordWithTatarEndingCyrlToLat( $parts );
		}
		...
		$parts = preg_split( '/(?=ны$)/u', $text, null, PREG_SPLIT_NO_EMPTY );
		if ( count( $parts ) == 2 ) {
			return $this->convertRussianWordWithTatarEndingCyrlToLat( $parts );
		}
		// ...сы/сын/etc
		...
		$parts = preg_split( '/(?<!семи|омму)(?=[лн]ар)(?!наренко)/u',
			$text, null, PREG_SPLIT_NO_EMPTY );
		if ( count( $parts ) == 2 ) {
			return $this->convertRussianWordWithTatarEndingCyrlToLat( $parts );
		}
		...
		$parts = preg_split( '/(?=чык)/u', $text, null, PREG_SPLIT_NO_EMPTY );
		if ( count( $parts ) == 2 ) {
			return $this->convertRussianWordWithTatarEndingCyrlToLat( $parts );
		}
	...
	function convertSimpleArabicWordFromCyrillicToLatin( $text ) {
		...
		if(
			...
			if(
				...
				&& !preg_match( '/^баqый/u', $text )
				&& !preg_match( '/^робаğый/u', $text )
			) {
				$text = preg_replace( '/ый/u', 'i', $text );
			}
