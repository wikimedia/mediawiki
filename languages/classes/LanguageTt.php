...
class TtConverter extends LanguageConverter {
	...
	function convertLowercasedTatarWordCyrlToLat( $text, $compoundOtherPart = null ) {
		...
		if ( preg_match( '/^(маһи|габд).{3,}/u', $text ) ) {
			$parts = preg_split( '/(?<=^....)/u', $text, null, PREG_SPLIT_NO_EMPTY );
			$parts[0] = $this->convertSimpleArabicWordFromCyrillicToLatin( $parts[0] );
			$parts[1] = $this->processWordWithArabicStemCyrlToLat( $parts[1] );
			$text = implode( $parts );
	...
	function surelyAreValidSuffixesAfterThickNoun( $parts ) {
		...
		if(
			...
			&& 0 < preg_match( '/^(сын?|м|ң|гыз)/u', $text, $matches )
			||
			...
			&& 0 < preg_match( '/^(к[аы]|та[нйш]?|тыр)/u', $text, $matches )
			||
			preg_match( '/([оаеёийуыэюя]|[жзлмнр][ьъ]?)$/u', $parts[0] )
			&& 0 < preg_match( '/^(г[аы]|да[нйш]?|дыр)/u', $text, $matches )
			||
			preg_match( '/ын$/u', $parts[0] )
			&& 0 < preg_match( '/^ың/u', $text, $matches )
			||
			$parts[0] == 'кай'
			&& 0 < preg_match( '/^ан/u', $text, $matches )
			||
			preg_match( '/(^кай|ы[мңн])$/u', $parts[0] )
			&& 0 < preg_match( '/^а/u', $text, $matches )
		) {
			$suffixes_parts = preg_split(
