...
class TtConverter extends LanguageConverter {
	...
	function convertTatarFromLatinToCyrillic( $text ) {
		...
		$text = preg_replace( '/(?<=a)k(?!e)/u', 'кь', $text );// пакьстан, пакеты
		$text = preg_replace( '/nyaq/u', 'ньяк', $text );
		$text = preg_replace( '/material/u', 'материал', $text );
		// $text = preg_replace( '/xalıq/u', 'халык', $text );
		$text = preg_replace( '/natural/u', 'натураль', $text );//натураль
		// $text = preg_replace( '/(<?=r)al/u', 'аль', $text );//натураль
		// $text = preg_replace( '/(<?=[ir])al/u', 'аль', $text );//натураль
		$text = preg_replace( '/aw\b/u', 'ау', $text );
		$text = preg_replace( '/aw(?=[çmc])/u', 'ау', $text );
		$text = preg_replace( '/äw(?=([bcçdfghjklmnñpqrsştvwxyz]|\b))/u', 'әү', $text );// яшәүче, тәвәккәл
		...
		$text = preg_replace( '/[äeiöü][bcçdfghjklmnñpqrsştvwxyz]y[äәa]/u', '$1ья', $text );// дөнья
		$text = preg_replace( '/[aıou][bcçdfghjklmnñpqrsştvwxyz]y[äәa]/u', '$1ъя', $text );// ташъязма
		...
		$text = preg_replace( '/(?<=[äeiöü])q/u', 'къ', $text );// икътисадый, wäqalät-вәкаләт, тәрәккыять
		// $text = preg_replace( '/(?<=[äeiöü])q(?!([bcçdfghjklmnñpqrsştvwxyz]+(a|iyä)))/u', 'къ', $text );// икътисадый, wäqalät-вәкаләт, тәрәккыять
		...
		if ( $toVariant == 'tt-cyrl' ) {
			...
			for ( $i = 0; $i < $wordsCount; $i++ ) {
				...
				if ( 0 === preg_match( '/[a-zA-ZäəÄƏöɵÖƟüÜıİşŞçÇñÑğĞ\']/u', $words[$i] ) ) {
					...
					// }elseif (
					// Commented out because style of writing all with upper case while
