		...
		*/
		...
		if (
			...
			|| preg_match(
				...
				. '|у(р(с|асан|тка)|к(мара|уруза|$)|туз|зьм|льт|б($|ан)|пала|инси)'
				// ко
				. '|о('
				. 'м(и(сс|$)|анд(а|ир)|мун|б(инат|айн)|пан)'
				. '|н(с(тан|ул)|ка)'
				. '|д($|ын?|та(н|гы)?|ка(ча)?|сыз|чы|ның|иров|лы)'
				. '|жан|фта|с(ынка|та|мик)|р(аб|пус|ея)|лум|ка|ала|х|лбурн|пи?'
				. ')'
				// к...
				. '|изнер'
				. ')'
				...
				. '|заказ'
				// у
				. '|у(а(скаран|йл)|о(ррен|кер|ттс|ллис)|э(льс|йк)|ил(ик|лис))|указ'
	...
	function tryGetRootOfThickWord( $text, &$parts ) {
		...
		for( $i = $len; $i >= 2; $i-- ) {
			...
			if(
				...
				if( preg_match( '/([аоуыияёюэеь])$/u', $parts[0], $matches ) ) {
					if( $matches[1] != 'и' ) {
						$parts[0] .= 'й';
					}
				}
	...
	function __construct(
		...
		$this->russianWordsWithFalseThickTatarSuffixesAfterNoun = array_fill_keys(
			[
				...
				'духовное', 'октябрьское', 'предприятие', 'раменское',
				'гуам',
				'гавая', 'парагвай',
