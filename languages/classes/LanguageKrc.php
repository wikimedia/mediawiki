...
class KrcConverter extends LanguageConverter {
	...
	private static function replaceU( $left, $text, $right ) {
		...
		if ( preg_match( $patternOfLC, $left ) ) {
			return $left . self::replaceU(
				self::capU( mb_substr( $text, 0, 1 ), "u" ),
				mb_substr( $text, 1 ),
				$right
				);
		} elseif ( preg_match( $patternOfLV, $left ) ) {
			return $left . self::replaceU(
				self::capU( mb_substr( $text, 0, 1 ), "w" ),
				mb_substr( $text, 1 ),
				$right
				);
			...
			return self::replaceU(
				$left,
				mb_substr( $text, 0, -1 ),
				self::capU( mb_substr( $text, -1 ), "u" )
				) . $right;
		} elseif ( preg_match( $patternOfRV, $right ) ) {
			return self::replaceU(
				$left,
				mb_substr( $text, 0, -1 ),
				self::capU( mb_substr( $text, -1 ), "w" )
				) . $right;
		} else {
			return $left . self::replaceU(
				self::capU( mb_substr( $text, 0, 1 ), "u" ),
	...
	private static function replaceV( $left, $v ) {
	...
	public function translate( $text, $toVariant='cyrl' ) {
		...
		while ( preg_match( '/([^у]?)(у+)([^у]?)/miu', $text, $matches ) ) {
			$text = preg_replace_callback( '/([^у]?)(у+)([^у]?)/miu', function ( $match ) use ( $converter ) {
				return $converter::replaceU( $match[1], $match[2], $match[3] );
			}, $text, 1 );
		}
		while ( preg_match( '/(.?)([яюеё])/emiu', $text, $matches ) ) {
			$text = preg_replace_callback( '/(.?)([яюеё])/miu', function ( $match ) use ( $converter ) {
				return $converter::replaceV( $match[1], $match[2] );
			}, $text, 1 );
		}
