...
class KrcConverter extends LanguageConverter {
	...
	public function translate( $text, $toVariant='cyrl' ) {
		...
		while ( preg_match( '/([^у]?)(у+)([^у]?)/miu', $text, $matches ) ) {
			$text = preg_replace_callback( '/([^у]?)(у+)([^у]?)/miu', function ( $match ) use ( $ref ) {
				return $ref::replaceU( $match[1], $match[2], $match[3] );
			}, $text, 1 );
		}
		while ( preg_match( '/(.?)([яюеё])/emiu', $text, $matches ) ) {
			$text = preg_replace_callback( '/(.?)([яюеё])/miu', function ( $match ) use ( $ref ) {
				return $ref::replaceV( $match[1], $match[2] );
			}, $text, 1 );
		}
