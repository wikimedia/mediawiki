...
class KrcConverter extends LanguageConverter {
	...
	public static function replaceU( $left, $text, $right ) {
		...
		if ( preg_match( $patternOfLC, $left ) ) {
			return $left . self::replaceU(
				self::capU( mb_substr( $text, 0, 1, "UTF8" ), "u" ),
				mb_substr( $text, 1, null, "UTF8" ),
				$right
				);
		} elseif ( preg_match( $patternOfLV, $left ) ) {
			return $left . self::replaceU(
				self::capU( mb_substr( $text, 0, 1, "UTF8" ), "w" ),
				mb_substr( $text, 1, null, "UTF8" ),
				$right
				);
		} elseif ( preg_match( $patternOfRC, $right ) ) {
			return self::replaceU(
				$left,
				mb_substr( $text, 0, -1, "UTF8" ),
				self::capU( mb_substr( $text, -1, null, "UTF8" ), "u" )
				) . $right;
		} elseif ( preg_match( $patternOfRV, $right ) ) {
			return self::replaceU(
				$left,
				mb_substr( $text, 0, -1 ),
				self::capU( mb_substr( $text, -1, null, "UTF8" ), "w" )
				) . $right;
		} else {
			return $left . self::replaceU(
				self::capU( mb_substr( $text, 0, 1, "UTF8" ), "u" ),
	...
	public function translate( $text, $toVariant='cyrl' ) {
		...
		while ( preg_match( '/у/msiu', $tocheck ) ) {
			preg_match( '/([^у]*)(у)(.*)/msiu', $tocheck, $matches );
			$partOne = mb_substr( $matches[1], 0, -1, "UTF8" );
			$partTwo = mb_substr( $matches[1], -1, null, "UTF8" );
			$partFour = mb_substr( $matches[3], 0, 1, "UTF8" );
			$partFive = mb_substr( $matches[3], 1, null, "UTF8" );
			$donePart = $partOne . self::replaceU( $partTwo, $matches[2], $partFour );
			$donePartOne = mb_substr( $donePart, 0, -1, "UTF8");
			$donePartTwo = mb_substr( $donePart, -1, null, "UTF8" );
			$done = $done . $donePartOne;
			$tocheck = $donePartTwo . $partFive;
		}
		$tocheck = $done;
		$done = "";
		while ( preg_match( '/[яюеё]/smiu', $tocheck ) ) {
			...
			$partOne = mb_substr( $matches[1], 0, -1, "UTF8" );
			$partTwo = mb_substr( $matches[1], -1, null, "UTF8" );
			$done = $done . $partOne . self::replaceV( $partTwo, $matches[2] );
			$tocheck = $matches[3];
		}
		$text = preg_replace( '/[ьъ]/umi', '', $done );
		return $text;
	}
}
/**
