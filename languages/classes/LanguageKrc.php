...
class KrcConverter extends LanguageConverter {
	...
	private function replaceU( $left, $text, $right ) {
		...
		if ( preg_match( $patternOfLC, $left ) ) {
			return $left . $this->replaceU(
				$this->capU( mb_substr( $text, 0, 1 ), "u" ),
				mb_substr( $text, 1 ),
				$right
				);
		} elseif ( preg_match( $patternOfLV, $left ) ) {
			return $left . $this->replaceU(
				$this->capU( mb_substr( $text, 0, 1 ), "w" ),
				mb_substr( $text, 1 ),
				$right
				);
		} elseif ( preg_match( $patternOfRC, $right ) ) {
			return $this->replaceU(
				$left,
				mb_substr( $text, 0, -1 ),
				$this->capU( mb_substr( $text, -1 ), "u" )
				) . $right;
		} elseif ( preg_match( $patternOfRV, $right ) ) {
			return $this->replaceU(
				$left,
				mb_substr( $text, 0, -1 ),
				$this->capU( mb_substr( $text, -1 ), "w" )
				) . $right;
		} else {
			return $left . $this->replaceU(
				$this->capU( mb_substr( $text, 0, 1 ), "u" ),
