...
class UzConverter extends LanguageConverter {
	 ...
	 * This and other lists of words in here can be found at https://github.com/6ahodir/uzbek-words
	 * @var array
	 */
	...
	/**
	 * 'e' in these words need to be converted to 'э' and not to 'е'.
	 * @var array
	 */
	public $eWords = array(
		'beletaj' => 'бельэтаж',
	...
	/**
	 * In the following words the 'ь' needs to be added when transliterated from latin
	 * @var array
	 */
	...
	function translateToLatn ( $text ) {
		...
		// after a vowel
		$text = preg_replace_callback(
			'/(' . implode( '|', $this->cyrillicVowels ) . ')(' .
				implode( '|', array_keys( $afterVowelRules ) ) . ')/ui',
	...
	function translateToCyrl ( $text ) {
		...
		$compoundsSecond = array(
			'yo' => 'ё', 'Yo' => 'Ё', 'YO' => 'Ё',
			// 'ts' => 'ц', 'Ts' => 'Ц', 'TS' => 'Ц',  # No need for this rule, see $tsWords
			'yu' => 'ю', 'Yu' => 'Ю', 'YU' => 'Ю',
			'ya' => 'я', 'Ya' => 'Я', 'YA' => 'Я',
			'ye' => 'е', 'Ye' => 'Е', 'YE' => 'Е',
		...
		// these are used to convert words from tsWords, shWords, eWords, yoWords, yuWords, yaWords,
		// 	yeWords, softSignWords
		...
		// FIXME: this should not be word boundaries only, but it should work with
		//   words with suffixes too
		// replace soft sign words
		$text = preg_replace_callback(
			'/\b('. implode( '|', array_keys( $this->softSignWords ) ) .')\b/ui',
			function( $matches ) use ( $that, $getExceptionWords ) {
				return $getExceptionWords( $matches[1], $that->softSignWords);
			},
			$text
		);
		...
		$text = preg_replace_callback(
			...
			function( $matches ) use ( $that, $getExceptionWords ) {
				return $getExceptionWords( $matches[1], $that->tsWords);
			},
			$text
		);
		...
		$text = preg_replace_callback(
			...
			function( $matches ) use ( $that, $getExceptionWords ) {
				return $getExceptionWords( $matches[1], $that->shWords);
			},
			$text
		);
		...
		$text = preg_replace_callback(
			...
			function( $matches ) use ( $that, $getExceptionWords ) {
				return $getExceptionWords( $matches[1], $that->eWords);
			},
			$text
		);
		...
		$text = preg_replace_callback(
			...
			function( $matches ) use ( $that, $getExceptionWords ) {
				return $getExceptionWords( $matches[1], $that->yoWords);
			},
			$text
		);
		...
		$text = preg_replace_callback(
			...
			function( $matches ) use ( $that, $getExceptionWords ) {
				return $getExceptionWords( $matches[1], $that->yuWords);
			},
			$text
		);
		...
		$text = preg_replace_callback(
			...
			function( $matches ) use ( $that, $getExceptionWords ) {
				return $getExceptionWords( $matches[1], $that->yaWords);
			},
			$text
		);
		...
		$text = preg_replace_callback(
			...
			function( $matches ) use ( $that, $getExceptionWords ) {
				return $getExceptionWords( $matches[1], $that->yeWords);
			},
			$text
		);
