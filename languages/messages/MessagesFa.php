...
پایگاه داده این خطا را باز گرداند:
<div dir="ltr">$3: $4</div>',
'noconnect'            => 'شرمنده! ویکی مشکلات فنی دارد، و نمی‌تواند با کارگزار پایگاه داده تماس بگیرد.',
...
'cascadeprotected'     => 'این صفحه در مقابل ویرایش محافظت شده‌است برای اینکه در صفحه‌های محافظت‌شدهٔ زیر که گزینهٔ «آبشاری» آنها تیک خورده‌است، قرار گرفته‌است:',
...
یک نفر (احتمالاً خود شما) با نشانی آی‌پی $1 درخواست کرده‌است که ما کلمهٔ عبور جدیدی برای  حساب کاربری‌ {{SITENAME}}ی شما در {{SERVERNAME}} بفرستیم.
...
'blockedtext'               => "نام کاربری یا نشانی IP شما توسط $1 بسته شده است. دلیل داده‌شده این است:<br />''$2''<p>شما می‌توانید با $1 یا یکی از [[{{ns:project}}:مدیران|مدیران]] تماس بگیرید و در این باره صحبت کنید.
...
نشانی IP شما $3 است. لطفاً این نشانی را در کلیهٔ پرس‌وجوهایتان ذکر کنید.
...
این قطع دسترسی در این زمان خاتمه می‌یابد: $6
...
'whitelistedittext'         => 'برای ویرایش مقاله‌ها باید به سیستم [[Special:Userlogin|وارد]] شوید.',
...
'yourdiff'                  => 'تفاوت‌ها',
'copyrightwarning'          => 'لطفاً توجه داشته باشید که فرض می‌شود کلیهٔ مشارکت‌های شما با {{SITENAME}} تحت «اجازه‌نامهٔ مستندات آزاد گنو» منتشر می‌شوند (برای جزئیات بیشتر به $1 مراجعه کنید). اگر نمی‌خواهید نوشته‌هایتان بی‌رحمانه ویرایش شده و به دلخواه توزیع شود، اینجا نفرستیدشان.<br />
...
'cascadeprotectedwarning'   => "'''هشدار:''' این صفحه به علت قرارگرفتن در صفحه‌های آبشاری-محافظت‌شدهٔ زیر قفل شده‌است تا فقط مدیران بتوانند ویرایشش کنند.",
...
'revdelete-selected'          => "نسخه(ها)ی انتخاب شده از '''$1:'''",
'logdelete-selected'          => "موارد انتخاب شده از سیاهه‌های مرتبط با '''$1:'''",
...
'logdelete-logaction'         => '$1 مورد به وضعیت $2 تغییر یافتند.',
...
و $3 متن مقاله است.',
'noexactmatch'          => 'صفحه‌ای با دقیقاً این عنوان وجود ندارد، تلاش برای جستجوی کل متن.',
...
'upload-proto-error-text' => 'بارگذاری از دوردست به نشانی‌هایی که با <code>//:http‎</code> یا <code>//:ftp</code> آغاز شوند نیاز دارد.',
...
'clearwatchlist'       => 'پاک‌کردن فهرست تعقیبات',
'watchlistcleartext'   => 'آیا مطمئن هستید که می‌خواهید آنها را حذف کنید؟',
'watchlistclearbutton' => 'پاک‌کردن فهرست تعقیبات',
'watchlistcleardone'   => 'فهرست تعقیبات شما پاک شد. $1 مورد از این فهرست حذف شد.',
...
در جعبهٔ صفحاتی که می‌خواهید از فهرست تعقیباتتان حذف شود تیک بزنید و روی دکمهٔ «برداشتن موارد» در پایین
صفحه کلیک کنید.',
...
'protect-cascadeon'           => 'این صفحه در حال حاضر محافظت‌ شده‌است، چون که در صفحه‌های زیر که گزینهٔ محافظت آبشاری آنها فعال شده‌است آمده. می‌توانید سطح محافظت این صفحه را تغییر دهید، ولی این کار تأثیری بر محافظت آبشاری نخواهد داشت.',
...
'undelete-revision'        => 'حذف نسخهٔ $1 از $2:',
...
'whatlinkshere-prev'   => '۵۰تای قبلی',
'whatlinkshere-next'   => '۵۰تای بعدی',
...
'file-info-size'       => '‏(‎$1 × $2 پیکسل، اندازهٔ پرونده: $3، نوع MIME‏: $4)',
...
# DISPLAYTITLE
'displaytitle' => '(پیوند با نام [[$1]] به این صفحه)',
...
'autoredircomment' => 'تغییر مسیر به [[$1]]', # This should be changed to the new naming convention, but existed beforehand
