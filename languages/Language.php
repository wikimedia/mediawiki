...
class Language {
	 ...
	 */
	private static function dateFormatNumber( &$dateTimeObj, $ts, $zone, $code ) {
		if ( !$dateTimeObj ) {
	...
	function sprintfDate( $format, $ts, DateTimeZone $zone = null, &$ttl = null ) {
		...
		for ( $p = 0; $p < $formatLength; $p++ ) {
			...
			switch ( $code ) {
				...
				case 'z':
					$usedDay = true;
					$num = Language::dateFormatNumber( $dateTimeObj, $ts, $zone, $code );
					break;
				case 'W':
					$usedWeek = true;
					$num = Language::dateFormatNumber( $dateTimeObj, $ts, $zone, $code );
		...
		if ( $usedSecond ) {
			...
			// XXX: Someone who understands the non-Gregorian calendars should write proper logic for them
			...
			if ( $usedWeek ) {
				$possibleTtls[] = ( 7 - Language::dateFormatNumber( $dateTimeObj, $ts, $zone, 'N' ) ) * 86400 + $timeRemainingInDay;
			} elseif ( $usedISOYear ) {
				...
				$currentISOWeek = Language::dateFormatNumber( $dateTimeObj, $ts, $zone, 'W' );
				$weeksRemaining = $lastWeekOfISOYear - $currentISOWeek;
				$timeRemainingInWeek = ( 7 - Language::dateFormatNumber( $dateTimeObj, $ts, $zone, 'N' ) ) * 86400 + $timeRemainingInDay;
				$possibleTtls[] = $weeksRemaining * 604800 + $timeRemainingInWeek;
			}

			if ( $usedMonth ) {
				$possibleTtls[] = ( Language::dateFormatNumber( $dateTimeObj, $ts, $zone, 't' ) - substr( $ts, 6, 2 ) ) * 86400 + $timeRemainingInDay;
			} elseif ( $usedYear ) {
				$possibleTtls[] = ( Language::dateFormatNumber( $dateTimeObj, $ts, $zone, 'L' ) + 364 - Language::dateFormatNumber( $dateTimeObj, $ts, $zone, 'z' ) ) * 86400
					+ $timeRemainingInDay;
			} elseif ( $usedIsLeapYear ) {
				$year = substr( $ts, 0, 4 );
				$timeRemainingInYear = ( Language::dateFormatNumber( $dateTimeObj, $ts, $zone, 'L' ) + 364 - Language::dateFormatNumber( $dateTimeObj, $ts, $zone, 'z' ) ) * 86400
					+ $timeRemainingInDay;
				$mod = $year % 4;
