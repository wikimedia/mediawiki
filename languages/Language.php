...
class Language {
	...
	public function formatNum( $number, $noSeparators = false ) {
		...
		if ( !$noSeparators ) {
			if ( class_exists( NumberFormatter::class ) ) {
				...
				$minimumGroupingDigits = (int)$this->minimumGroupingDigits();
				if ( $minimumGroupingDigits > 1 && strlen( (string)abs( (int)$number ) )
					< $minimumGroupingDigits + $fmt->getAttribute( NumberFormatter::GROUPING_SIZE )
				) {
					// Even if numbers does not need commafy, do decimal separator tranformation.
					// For example 1234.56 becoms 1234,56 in pl with $minimumGroupingDigits = 2
					return strtr( $number, $separatorTransformTable ?: [] );
				}

				// NumberFormatter supports separator transformation, but it does not know all
				...
				if ( strpos( $number, '.0' ) === strlen( $number ) - 2 ) {
					// Setting to retain fraction part for values like 5.0, 9898.0 etc.
					$fmt->setAttribute( NumberFormatter::MIN_FRACTION_DIGITS, 1 );
				};

				$number = $fmt->format( $number );
			} else {
				// Fallback implementation. Follows fixed thousands separator pattern
				$trans = $separatorTransformTable ?: [ ',' => ',', '.' => '.' ];
				$number = number_format( $number, 3, $trans[ '.' ], $trans[ ',' ] );
			}
		...
		$number = strtr( $number, [ "\xE2\x80\x8E" => '', "\xE2\x80\x8F" => '' ] );
	 ...
	 *
	 * @since 1.19
	 * @deprecated in 1.30 use formatNum directly
	 * @param mixed $number
	 * @return string
	 */
	function commafy( $number ) {
		return self::formatNum( $number, false );
	}
