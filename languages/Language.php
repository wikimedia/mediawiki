...
class Language {
	 ...
	 */
	public function prettyTimestamp( $ts, $relativeTo = false, $user = false ) {
		if ( $relativeTo === false ) {
		...
		$adjustedNow = $this->userAdjust( wfTimestamp( TS_MW, $now ), false, $user );
		...
		$adjustedThen = $this->userAdjust( wfTimestamp( TS_MW, $ts ), false, $user );
		...
		if ( $timeAgo < 0 ) {
			...
			intval( $adjustedNow / self::$durationIntervals['days'] ) ===
			intval( $adjustedThen / self::$durationIntervals['days'] )
		) {
			// Today at XX:XX
			...
			intval( $adjustedNow / self::$durationIntervals['days'] ) ===
			( intval( $adjustedThen / self::$durationIntervals['days'] ) + 1 )
		) {
			// Yesterday at XX:XX
			$time = $this->time( $adjustedThen );
			return wfMessage( 'yesterday-at' )->inLanguage( $this )->params( $time )->text();
		} elseif ( $timeAgo < ( self::$durationIntervals['days'] * 5 ) ) {
			// Xday at XX:XX
			$day = date( 'w', $adjustedThen ) + 1;
			$dayName = $this->getWeekdayName( $day );
			$time = $this->time( $adjustedThen );
			return wfMessage( 'dayofweek-at' )->inLanguage( $this )->params( $dayName, $time )->text();
