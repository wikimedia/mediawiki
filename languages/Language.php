	...
	function sprintfDate( $format, $ts, $zone = null ) {
		...
		for ( $p = 0; $p < strlen( $format ); $p++ ) {
			...
			switch ( $code ) {
				...
				case 'P':
				case 'T':
					// If we want to make these unconditional (assuming UTC on
					// no $zone), just add these cases to the cases for 'cr' above
					if ( $zone ) {
						if ( !$dateTimeObj ) {
				...
				case 'Z':
					// If we want to make these unconditional (assuming UTC on
					// no $zone), just add these cases to the cases for 'wNzWtLoU' above
					if ( $zone ) {
						if ( !$dateTimeObj ) {
							$dateTimeObj = DateTime::createFromFormat( 'YmdHis', $ts, $zone );
						}
