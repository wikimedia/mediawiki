...
class Language {
	 ...
	 * @param DateInterval|stdClass $diff Pre-calculate diff between the two
	 * @param User $user User preferences to use
	 * @return string Human timestamp
	 * @since 1.21
	 */
	public function getHumanTimestamp( MWTimestamp $ts, MWTimestamp $relativeTo, $diff, User $user ) {
		if ( $diff->invert ) {
			// Future timestamps not supported.
			throw new TimestampException( __METHOD__ . ' : Future timestamps not supported.' );
		} elseif ( $ts->timestamp->format( 'Y' ) !== $relativeTo->timestamp->format( 'Y' ) ) {
