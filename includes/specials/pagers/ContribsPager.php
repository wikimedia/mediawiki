...
class ContribsPager extends ReverseChronologicalPager {
	 ...
	 * @return bool      True if current target is an IP range
	 */
	public static function isValidIPRange( $ip ) {
		...
		if (
			( $bits === false ) ||
			( IP::isIPv4() && $bits < $wgRangeContributionsCIDRLimit['IPv4'] ) ||
			( IP::isIPv6() && $bits < $wgRangeContributionsCIDRLimit['IPv6'] )
		) {
			return false;
		}
	...
	function formatRow( $row ) {
		...
		if ( $validRevision ) {
			...
			if ( $this->contribs == 'newbie' || self::isValidIPRange( $this->target ) ) {
