...
class NewFilesPager extends RangeChronologicalPager {
	...
	function getQueryInfo() {
		...
		if ( $user !== '' ) {
			$conds[] = ActorMigration::newKey( 'img_user' )
				->getWhere( wfGetDB( DB_REPLICA ), User::newFromName( $user, false ) )['conds'];
		}
		...
		if ( $opts->getValue( 'hidepatrolled' ) ) {
			...
			if ( if ( $wgActorTableSchemaMigrationStage === MIGRATION_NEW ) {
				$jcond = 'rc_actor = ' . $imgQuery['fields']['img_actor'];
			} else {
				$rcQuery = ActorMigration::newKey( 'rc_user' )->getJoin();
