...
class SpecialRandomInCategory extends SpecialPage {
	...
	public function getRandomTitle() {
		...
		if ( !$row ) {
			$row = $this->selectRandomPageFromDB( 0, $offset, true );
		}
		...
		if ( !$row ) {
			$row = $this->selectRandomPageFromDB( 0, 0, true );
		}
	...
	protected function getQueryInfo( $rand, $offset, $up ) {
		...
		if ( $minClTime ) {
			...
			$qi['conds'][] = 'cl_timestamp ' . $op . ' ' . $dbr->addQuotes( $minClTime );
		}
		return $qi;
	}
	...
	protected function getTimestampOffset( $rand ) {
		...
		if ( $dbr->getType() === 'mysql' ) {
			...
			return $dbr->timestamp( intval( $ts ) );
		}
	}
