...
class SpecialMergeHistory extends SpecialPage {
	...
	public function execute( $par ) {
		...
		if (
			$opts->getValue( 'merge' ) && $targetObj && $destObj &&
			$this->getUser()->matchEditToken( $opts->getValue( 'wpEditToken' ) )
		) {
			$this->merge();
	...
	private function showHistory() {
		...
		$out = $this->getOutput();
		$header = Html::element( 'h2', [ 'id' => 'mw-mergehistory'],
			$this->msg( 'mergehistory-list' )->text() ) . "\n";
