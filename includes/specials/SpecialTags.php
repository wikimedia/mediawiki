...
class SpecialTags extends SpecialPage {
	...
	function showTagList() {
		...
		foreach ( array_keys( $this->tagStats ) as $tag ) {
			$html .= $this->doTagRow( $tag, $showActions, $showEditLinks );
		}
	...
	function doTagRow( $tag, $showActions, $showEditLinks ) {
		...
		if ( $changeTag->isRegistered && $changeTag->extensionSource != '' ) {
			...
			$msgKey = 'tags-description-extension-' . $changeTag->extensionSource;
			$extMsg = $this->msg( $msgKey );
			if ( $extMsg->exists() ) {
				if ( $msg->exists() ) {
					$desc .= Xml::element( 'br' );
				}
				$desc .= $extMsg->params( $changeTag->extensionMsgParams )->parse();
			}
		}
		if ( $showEditLinks ) {
			$desc .= ' ';
		...
		$newRow .= Xml::tags( 'td', array( 'data-sort-value' => $changeTag->hitcount ), $hitcountLink );
