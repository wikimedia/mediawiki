...
class SpecialTags extends SpecialPage {
	...
	function doTagRow( $tag, $hitcount, $showActions ) {
		$user = $this->getUser();
		$newRow = '';
		$newRow .= Xml::tags( 'td', null, Xml::element( 'kbd', null, $tag ) );
	...
	protected function showDeleteTagForm( $tag ) {
		...
		$canDeleteResult = ChangeTags::canDeleteTag( $tag );
	...
	protected function showActivateDeactivateForm( $tag, $activate ) {
		...
		if ( !$activate && !$tagUsage[$tag] ) {
			$preText .= $this->msg( 'tags-deactivate-question-delete', $tag )->parseAsBlock();
		}
	...
	public function processTagForm( array $data, HTMLForm $form ) {
		...
		$status = call_user_func( array( 'ChangeTags', "{$form->tagAction}TagWithChecks" ),
			$data['HiddenTag'], $data['Reason'], $context->getUser() );
