...
class SpecialTags extends SpecialPage {
	...
	function doTagRow( $tag, $hitcount, $showManageActions, $showDeleteActions,
		...
		// Build change tag object (tag exists, no need to check existence)
		$changeTag = new ChangeTag( $tag, $this->changeTagsContext, false );
		...
		// delete
		if ( $showDeleteActions && $changeTag->canDelete()->isOK() ) {
	...
	public function processCreateTagForm( array $data, HTMLForm $form ) {
		...
		$status = ChangeTags::createTagWithChecks( $tag, $data['Reason'],
			$context->getUser(), $ignoreWarnings, $this->changeTagsContext );
	...
	public function processTagForm( array $data, HTMLForm $form ) {
		...
		$status = call_user_func( [ 'ChangeTags', "{$form->tagAction}TagWithChecks" ],
			$tag, $data['Reason'], $context->getUser(), true, $this->changeTagsContext );
