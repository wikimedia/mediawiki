...
class SpecialTags extends SpecialPage {
	...
	/**
	 * @var  object $changeTagContext provides context in the form
	 * of tags defined on the wiki, listed by source
	 */
	protected $changeTagContext = null;
	...
	function showTagList() {
		...
		// Retrieving tag usage stats
		$tagStats = ChangeTags::tagUsageStatistics();

		// Retrieving context (defined tags by source) for use in #doTagRow()
		$this->changeTagContext = ChangeTag::getFullContext();

		// Show header only if there exists at least one tag
		if ( !$tagStats && !$this->changeTagContext->registeredTags &&
			!$this->changeTagContext->storedTags ) {
			return;
		}
		...
		$html .= $this->appendNeverAppliedDefinedTags( $tagStats );
	...
	function doTagRow( $tag, $hitcount, $showActions, $showEditLinks ) {
		...
		// providing context by reference
		$contextRef =& $this->changeTagContext;
		$changeTag->setContext( $contextRef );

		// try to retrieve extension name when relevant
		$extName = $changeTag->getExtName();
		...
		if ( $extName ) {
			...
			if ( $extMsg->exists() ) {
				...
				$extPar = $changeTag->getExtMsgPar();
				if ( $extPar ) {
					$desc .= $extMsg->params( $extPar )->parse();
				} else {
					$desc .= $extMsg->parse();
				}
			}
	...
	function appendNeverAppliedDefinedTags( $tagStats ) {
