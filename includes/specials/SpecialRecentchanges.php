...
class SpecialRecentChanges extends ChangesListSpecialPage {
	...
	public function parseParameters( $par, FormOptions $opts ) {
		...
		foreach ( $bits as $bit ) {
			...
			if ( preg_match( '/^days=(\d+)$/', $bit, $m ) ) {
				$rcMaxAgeOptions = $this->getConfig()->get( 'RCMaxAgeOptions' );
				$opts['maxage'] =
					self::getClosestValidChangesAge( $rcMaxAgeOptions, (int)$m[1] * 86400 );
			}
	...
	protected function addModules() {
		...
		$out->addModuleStyles( 'mediawiki.special.recentchanges.styles' );
	...
	private function makeDurationOptions() {
		$language = $this->getLanguage();
		$config = $this->getConfig();
		$changesAgeDurations = $config->get( 'RCMaxAgeOptions' );

		$options = [];
		foreach ( $changesAgeDurations as $duration ) {
			$options[] = [
