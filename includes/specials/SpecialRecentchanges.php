...
class SpecialRecentChanges extends IncludableSpecialPage {
	 ...
	 */
	 public static function makeLegend( $self ) {
		...
		$legend = Xml::openElement( 'dl', array( 'id' => 'mw-changeslist-legend' ) ) . "\n";
		...
		foreach ( $legendItems as $label => $letter ) { # generate items of the legend
			$legend .= Xml::element( 'dt', array( 'class' => $label ), $self->msg( $letter . 'letter' )->text() ) . "\n";
			if ( $letter === 'newpage' ) {
				$legend .= Xml::openElement( 'dd' );
				$legend .= $self->msg( "recentchanges-label-$label" )->text();
				$legend .= Xml::tags( 'i', array(), ' ' . $self->msg( 'recentchanges-legend-newpage' )->parse() );
				$legend .= Xml::closeElement( 'dd' ) . "\n";
			} else {
				$legend .= Xml::element( 'dd', array(), $self->msg( "recentchanges-label-$label" )->text() ) . "\n";
			}
		}
		# (+-123)
		$legend .= Xml::tags( 'dt', array( 'style' => 'font-style: italic;', 'class' => 'mw-plusminus-pos' ), '(&#177; 123)' ) . "\n";
		$legend .= Xml::element( 'dd', array( 'style' => 'margin-left: 4em;' ), $self->msg( "recentchanges-label-plusminus" )->text() ) . "\n";
