...
/**
 * Let users change their password or recover their password using a
...
class SpecialChangePassword extends FormSpecialPage {
	 ...
	 *
	 * @param string $par Sub-page parameter
	 */
	function setParameter( $par ) {
		...
		if (
			...
			$user->isAnon() ||
			!$user->checkTemporaryPassword( $this->token )
		) {
			return;
		}
	...
	function getFormFields() {
		...
		if ( $extraFields ) {
			wfDeprecated( __METHOD__ . ' : ChangePasswordForm hook', 1.22 );
		...
		wfRunHooks( 'ChangePasswordFields', array( $this, &$a ) );
