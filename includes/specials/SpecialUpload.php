...
class SpecialUpload extends SpecialPage {
	...
	protected function getUploadForm( $message = '', $sessionKey = '', $hideIgnoreWarning = false ) {
		...
		if(
			...
			$form->addPreText( wfMessage( 'session_fail_preview', 'parseinline' )->parse() );
		}
	...
	protected function showRecoverableUploadError( $message ) {
		...
		$message = '<h2>' . wfMessage( 'uploaderror' )->escaped() . "</h2>\n" .
			'<div class="error">' . $message . "</div>\n";

		$form = $this->getUploadForm( $message, $sessionKey );
		$form->setSubmitText( wfMessage( 'upload-tryagain' )->escaped() );
	...
	protected function showUploadWarning( $warnings ) {
		...
		foreach( $warnings as $warning => $args ) {
			if( $warning == 'exists' ) {
				...
				$msg = "\t<li>" . wfMessage( 'file-deleted-duplicate', array(
	...
	protected function processUpload() {
		...
		if( $permErrors !== true ) {
			...
			$this->showRecoverableUploadError( wfMessage( $code, $permErrors[0] )->parse() );
			return;
		}
	...
	protected function processVerificationError( $details ) {
		...
		switch( $details['status'] ) {
			...
			case UploadBase::MIN_LENGTH_PARTNAME:
				$this->showRecoverableUploadError( wfMessage( 'minlength1' )->escaped() );
				break;
			case UploadBase::ILLEGAL_FILENAME:
				$this->showRecoverableUploadError( wfMessage( 'illegalfilename',
					$details['filtered'] )->parse() );
				break;
			case UploadBase::FILENAME_TOO_LONG:
				$this->showRecoverableUploadError( wfMessage( 'filename-toolong' )->escaped() );
				break;
			case UploadBase::FILETYPE_MISSING:
				$this->showRecoverableUploadError( wfMessage( 'filetype-missing' )->parse() );
				break;
			case UploadBase::WINDOWS_NONASCII_FILENAME:
				$this->showRecoverableUploadError( wfMessage( 'windows-nonascii-filename' )->parse() );
				break;

			/** Statuses that require reuploading **/
			case UploadBase::EMPTY_FILE:
				$this->showUploadError( wfMessage( 'emptyfile' )->escaped() );
				break;
			case UploadBase::FILE_TOO_LARGE:
				$this->showUploadError( wfMessage( 'largefileserver' )->escaped() );
				break;
			...
			case UploadBase::VERIFICATION_ERROR:
				...
				$this->showUploadError( wfMessage( $code, $details['details'] )->parse() );
				break;
			case UploadBase::HOOK_ABORTED:
				...
				$this->showUploadError( wfMessage( $error, $args )->parse() );
				break;
			default:
	...
	protected function getSourceSection() {
		...
		$descriptor['UploadFile'] = array(
			...
			'help' => wfMsgExt( 'upload-maxfilesize',
				...
				) . ' ' . wfMessage( 'upload_source_file' )->escaped(),
