...
class SpecialEmailUser extends UnlistedSpecialPage {
	...
	public function sendEmailForm() {
		...
		$form->addPreText( $this->msg( 'emailpagetext', $this->mTarget )->parse() )
			// a little hack, otherwise HTMLForm will think, that the user (when submitting the
			// user selection form) submitted this one and sends an emoty e-mail. The submit call-
			// back of this form will check this field, so it must not be set by the first form!
	...
	public static function submit( array $data, IContextSource $context ) {
		...
		// This is a hidden field, set to 1, which is sent by the UserEmailForm only, not
		// by the UserSelectionForm. This is used to make sure, that we send an e-mail only,
