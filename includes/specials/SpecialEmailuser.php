...
class SpecialEmailUser extends UnlistedSpecialPage {
	 ...
	 * @param User|null $user User sending the email
	 * @return User|string User object on success or a string on error
	 */
	public static function getTarget( $target, User $user = null ) {
		if ( $user === null ) {
			wfWarn( "The sending user is now required as the second argument." );
		}
		...
		$error = self::validateTarget( $nu, $user );
	 ...
	 */
	public static function validateTarget( $target, User $user = null ) {
		if ( $user === null ) {
			wfWarn( "The sending user is now required as the second argument." );
		}
