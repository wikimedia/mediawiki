...
class SpecialUserlogin extends FormSpecialPage {
	...
	function getFormFields() {
		...
		$a['Name'] = array(
			...
			'size' => 20,
			'required' => true,
			'filter-callback' => function( $val ) {
				return (string)User::getCanonicalName( $val, false );
			},
			'validation-callback' => function( $val ) use ( $that ) {
				return User::isUsableName( $val ) ?: $that->msg( 'noname' );
			},
		);
