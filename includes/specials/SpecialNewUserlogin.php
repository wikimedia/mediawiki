...
class SpecialUserlogin extends FormSpecialPage {
	...
	function headerText() {
		...
		if( $this->getUser()->isAllowed( 'createaccount' ) ) {
			...
			$html = wfMessage( 'nologinlink' )->text();
	...
	function alterForm( HTMLForm $form ) {
		...
		if( is_array( $wgPasswordAttemptThrottle ) ) {
			$throttle = new Throttle(
				...
				Throttle::PERUSER | Throttle::PERIP
			);
	...
	function getFormFields() {
		...
		$a['Name'] = array(
			...
			'default' => $user->isLoggedIn() ? $user->getName() : $request->getSessionData( 'mediawikiUserName' ),
			'size' => 20,
			'required' => true,
			'filter-callback' => function( $value, $fields ) { return User::getCanonicalName( $value, 'usable' ); },
			'validation-callback' => array( 'User', 'isUsableName' )
		);
	...
	function onSubmit( array $data ) {
		...
		global $wgBlockDisablesLogin, $wgUser, $wgDisableCookieCheck;
		...
		$extuser = ExternalUser::newFromName( $data['Name'] );
		...
		if( $user->isAnon() ) {
			...
			if( $this->getUser()->isBlockedFromCreateAccount() ) {
				...
				if( $wgAutocreatePolicy == 'never' ) {
					$status->fatal( 'nosuchuser' );
				} elseif( !$extuser->authenticate( $data['Password'] ) ) {
					$status->fatal( 'wrongpassword' );
				}
			} else {
				if( !$wgAuth->autoCreate() || !$wgAuth->userExists( $user->getName() ) ) {
					$status->fatal( 'nosuchuser' );
			...
			$error = '';
			if( !$status->isOK() ) {
				// Autocreation failed. User does not exist.
				return $status;
			} elseif( !wfRunHooks( 'AbortAutoAccount', array( $user, &$error ) ) ) {
				// Hook aborted. User does not exist.
				$status->fatal( $error );
				return $status;
			} else {
				...
				if( $extuser ) {
					...
					$email = $extuser->getPref( 'emailaddress' );
					!$email or $user->setEmail( $email );
				}
				$user->saveSettings();
			...
			is_object( $extuser ) &&
			$extuser->authenticate( $data['Password'] )
		) {
			// External user authentication succeeded, so link the local
			// user to it.
			$extuser->linkToLocal( $user->getID() );
		}

		// Allow hooks to abort login.
		$abort = true;
		$msg = null;
		if( !wfRunHooks( 'AbortLogin', array( $user, $data['Password'], &$abort, &$message ) ) ) {
		...
		wfRunHooks( 'LoginAuthenticateAudit', array( $user, $data['Password'], $retval ) );
		return $status;
	}
