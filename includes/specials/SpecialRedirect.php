...
class SpecialRedirect extends FormSpecialPage {
	...
	protected $mType;
	// subtype of the redirect (by-id, by-name)
	protected $mSubType;
	...
	//    and Special:Redirect/revision/by-id/xxx
	...
	protected function getFormFields() {
		...
		$ns = array(
			// subpage       => message
			"user/by-id"     => $mp . "-user-by-id",
			"revision/by-id" => $mp . "-revision-by-id",
			"file/by-name"   => $mp . "-file-by-name"
		);
		$a = array();
		$a['namespace'] = array(
			'type' => 'select',
		...
		if ( !empty( $this->mType ) && !empty( $this->mSubType ) ) {
			$a['namespace']['default'] = $this->mType . '/' . $this->mSubType;
		}
