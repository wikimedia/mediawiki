...
class UserrightsPage extends SpecialPage {
	...
	public function execute( $par ) {
		...
		if ( $request->wasPosted() ) {
			...
			if ( $request->getCheck( 'saveusergroups' ) ) {
				if ( $user->matchEditToken( $request->getVal( 'wpEditToken' ), $this->mTarget ) ) {
					...
					if ( $targetUser->getTouched() > $request->getVal( 'formLoadTimestamp' ) ) {
	...
	protected function showEditUserGroupsForm( $user, $groups ) {
		...
		$this->getOutput()->addHTML(
			...
			Html::hidden( 'formLoadTimestamp', User::newTouchedTimestamp() ) . // Conflict detection
