...
class SpecialEditWatchlist extends UnlistedSpecialPage {
	...
	/**
	* get a list of non talk namespaces
	* @reutrn a list of non talk namespaces
	*/
	private static function getNontalkNamespaces(){
		$nonTalkNamespaces =array();
		foreach ( MWNamespace::getValidNamespaces() as $ns ) {
			if ( !MWNamespace::isTalk( $ns ) ) {
				array_push($nonTalkNamespaces, $ns);
			}
		}
		return $nonTalkNamespaces;
	}
	...
	*/
	private function selectWatchListInfo( $limit = false, $offset = false ) {
		if ( $limit !== false ) {
		...
		$options = array( 'ORDER BY' => array( 'wl_namespace', 'wl_title' ) );
		$options['LIMIT'] = $limit;
		if( $offset ) {
			$options['OFFSET'] = $offset;
		}
		$dbr = wfGetDB( DB_MASTER );
		//query only non talk namespaces. any better way to do it?
		$nonTalkNamespaces =array();
		foreach ( MWNamespace::getValidNamespaces() as $ns ) {
			if ( !MWNamespace::isTalk( $ns ) ) {
				array_push($nonTalkNamespaces, $ns);
	...
	protected function getNormalForm(){
		...
		//when no limit is specified, fallback to use wllimit option
		if( !$this->limit ){
			$this->limit = intval( $this->getUser()->getOption( 'wllimit' ) );
		}

		$watchedItems = $this->selectWatchListInfo($this->limit, $this->offset );
