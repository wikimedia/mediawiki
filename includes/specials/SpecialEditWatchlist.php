...
class SpecialEditWatchlist extends UnlistedSpecialPage {
	...
	public function execute( $par ) {
		...
		if( $par == 'clear' || $par == 'raw' || $par == 'edit'
			...
			$mode = '';
			$this->groupname = WatchlistGroup::getGroupFromName( $par, $this->getUser()->getId() );
		}
	...
	private function buildRemoveLine( $title ) {
		...
		$wgroup = WatchlistGroup::getGroupFromUserTitle( $title, $this->getContext()->getUser()->getId() );
		$wgrouplink = $wgroup[0];
	...
	protected function getRawForm() {
		...
		$fields = array();
		$groups = array( 0 => array( $this->msg( 'watchlistedit-raw-ungrouped' )->parse(), 0 ) );
		$groups = $groups + WatchlistGroup::getGroups( $this->getUser()->getId() );
		foreach( $groups as $gid => $g ) {
			if( !empty( $titles[$gid] ) ) {
				$fields['group' . $gid . '_space'] = array(
					'type' => 'info',
					'default' => '&nbsp;',
					'raw' => true,
				);
				$fields['group' . $gid . '_label'] = array(
					'type' => 'info',
					'default' => '<strong>' . $g[0] . '</strong>',
					'raw' => true,
				);
