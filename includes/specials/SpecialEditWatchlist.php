	...
	*/
	...
	private function getWatchlistInfo( $watchedItems ) {
		...
		if ( $this->dir === 'prev' ) {
			...
			//in prev dir go over the result in reverse order (ignore the first additonal row, that tells whether prev enabled)
			for ( $i =( $numRows === $this->limit )? ( $numRows-1 ) : ( $numRows-2 ); $i >= 0; $i-- ) {
				$watchedItems->seek( $i );
	...
	private function buildNavigationLinks( $watchlistInfo, $rowNum ) {
		...
		if ( $rowNum > 0 ) {
			...
			$titlesFirstNs = array_keys( $watchlistInfo[$firstNs] );
			$titlesLastNs = array_keys( $watchlistInfo[$lastNS] );
			$firstTitle = reset( $titlesFirstNs );
			$lastTitle = end( $titlesLastNs );

			$hasPrev = ( $rowNum <= $this->limit && $this->dir === 'prev' ) || ( $this->offset === null );
			$hasNext = $rowNum <= $this->limit && $this->dir !== 'prev';

			$prevLink = $hasPrev ? $this->msg( 'prevn' )->numParams( $this->limit ) : Linker::linkKnown(
				$this->getTitle(),
				$this->msg( 'prevn' )->numParams( $this->limit ),
			...
			$nextLink = $hasNext? $this->msg( 'nextn' )->numParams( $this->limit ) : Linker::linkKnown(
				$this->getTitle(),
				$this->msg( 'nextn' )->numParams( $this->limit ),
