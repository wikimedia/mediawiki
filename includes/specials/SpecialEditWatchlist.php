...
class SpecialEditWatchlist extends UnlistedSpecialPage {
	...
	*/
	private function selectWatchListInfo() {
		...
		$options = array( 'LIMIT' => intval( $this->limit ) + 1 );
		$dbr = wfGetDB( DB_MASTER );

		$dir = $this->getRequest()->getVal( 'dir' );
		$whereCond = array(
			'wl_user' => $this->getUser()->getId(),
			'wl_namespace' => MWNamespace::getSubjectNamespaces()//query only non talk namespaces.
		);
		if ( !is_null( $this->offset ) ) {
			$offsetNS = intval( $this->offsetNS );
			if ( $dir === 'prev' ) {
				...
				$titleCond = 'wl_title '.( $dir=='next'? '>' : '>=' ) . $dbr->addQuotes( $this->offset );
				$whereCond[] = "(wl_namespace > $offsetNS OR (wl_namespace = $offsetNS AND $titleCond))";
	...
	private function getWatchlistInfo( $watchedItems ) {
		...
		$lb = new LinkBatch();
		$count = 0;
		if ( $this->getRequest()->getVal( 'dir' ) === 'prev' ) {
...
*/			
	...
	private function buildNavigationLinks( $watchlistInfo, $rowNum ) {
		...
		if (  $rowNum>0 ) {
			...
			$firstNs = current( $displayedNS );
			$lastNS = end( $displayedNS );
			$firstTitle = current( array_keys( $watchlistInfo[$firstNs] ) );
			$lastTitle = end( array_keys( $watchlistInfo[$lastNS] ) );
			$dir = $this->getRequest()->getVal( 'dir' );
			$hasPrev = ( $rowNum <= $this->limit && $dir === 'prev' ) || is_null( $this->offset );
			$hasNext = $rowNum <= $this->limit && $dir === 'next';
			$prevLink = $hasPrev ? $this->msg( 'prevn' )->rawParams( $this->limit ) : Linker::linkKnown(
				...
				array( 'offset' => $firstTitle, 'offset-ns' => $firstNs, 'dir'=> 'prev', 'limit' => $this->limit )
			);
			$nextLink = $hasNext? $this->msg( 'nextn' )->rawParams( $this->limit ) : Linker::linkKnown(
				...
				array( 'offset' => $lastTitle, 'offset-ns' => $lastNS, 'dir'=> 'next', 'limit' => $this->limit )
			);
