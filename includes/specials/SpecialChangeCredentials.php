...
class SpecialChangeCredentials extends AuthManagerSpecialPage {
	...
	protected function loadAuth( $subPage, $authAction = null, $reset = false  ) {
		...
		$this->authRequests = array_filter( $this->authRequests, function ( $req ) use ( $subPage ) {
			$ok = !in_array( get_class( $req ), $this->getRequestBlacklist(), true );
			if ( $subPage ) {
				$ok = $ok && $req->getUniqueId() === $subPage;
			}
			return $ok;
		} );
		if ( $subPage && count( $this->authRequests ) > 1 ) {
	...
	protected function getRequestBlacklist() {
		global $wgChangeCredentialsBlacklist;
		return $wgChangeCredentialsBlacklist;
	}
}
