...
class SpecialChangeCredentials extends AuthManagerSpecialPage {
	...
	// FIXME UNLINK and REMOVE is handled with the same action internally; AuthenticationRequests
	// supporting UNLINK tend to have buttons because Special:UnlinkAccounts shows all of them at
	// the same time; but the change/remove form is only for a single request so an extra submit
	// button would only confuse the user, so we get rid of it. This is not very clean and might
	// break requests which try to do something very tricky with buttons...
	protected function getAuthFormDescriptor( $requests, $action ) {
		$formDesriptor = parent::getAuthFormDescriptor( $requests, $action );
		foreach ( $formDesriptor as $key => $field ) {
			if (
				isset( $field['type'] ) && $field['type'] === 'submit'
				|| isset( $field['class'] ) && $field['class'] === 'HTMLSubmitField'
			) {
				unset( $formDesriptor[$key] );
			}
		}
		return $formDesriptor;
	}
