...
class LoginForm extends SpecialPage {
	...
	public function authenticateUserData() {
		global $wgUser, $wgAuth, $wgBlockDisablesLogin, $wgExternalAuthType, $wgAutocreatePolicy, $wgContLang;
		...
		$valid = true;
		$u = User::newFromName( $this->mUsername, 'usable' );
		if( $u === false ) {
			// Because there's no way to guarantee you'll get a user object
			// from User::newFromName.
			$valid = false;
			$u = new User;
			$u->mName = $wgAuth->getCanonicalName( $wgContLang->ucfirst( $name ) );
			$u->setItemLoaded( 'name' );
			// Prevent calls to User::load()
			$u->mFrom = 'invalid';
		}
