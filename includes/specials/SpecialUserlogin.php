...
class SpecialUserLogin extends AuthManagerSpecialPage {
	...
	protected function isSignup() {
		return in_array( $this->authAction,
			[ AuthManager::ACTION_CREATE, AuthManager::ACTION_CREATE_CONTINUE ], true );
	}
				...
				if ( $this->isSignup() ) {
					if ( $response->loginRequest && $authManager->canAuthenticateNow() ) {
						$response2 = $authManager->beginAuthentication(
							[ $response->loginRequest ], $returnToUrl );
		...
		if ( !$this->isSignup() ) {
			...
			if ( $this->showCreateAccountLink() ) {
				...
				if ( $this->getUser()->isLoggedIn() ) {
					...
					$createOrLoginHtml = Html::rawElement( 'div',
						[ 'id' => 'mw-createaccount-cta', 'class' => 'mw-ui-vform-field mw-form-related-link-container' ],
		...
		if ( $this->isSignup() ) {
			$fieldDefinitions = [
				...
				'retype' => [
					...
					'size' => 20,
					'required' => true, // FIXME only if the password is required
