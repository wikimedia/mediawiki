...
class SpecialSearch extends SpecialPage {
	...
	protected function getDidYouMeanHtml( SearchResultSet $textMatches ) {
		...
		$suggest = $this->getLinkRenderer()->makeKnownLink(
			$this->getPageTitle(),
	...
	protected function getDidYouMeanRewrittenHtml( $term, SearchResultSet $textMatches ) {
		...
		$rewritten = $this->getLinkRenderer()->makeKnownLink(
			$this->getPageTitle(),
		...
		$original = $this->getLinkRenderer()->makeKnownLink(
			$this->getPageTitle(),
			htmlspecialchars( $term ),
		...
		if ( !is_null( $redirectTitle ) ) {
			...
			$redirect = "<span class='searchalttitle'>" .
				$this->msg( 'search-redirect' )->rawParams(
					$this->getLinkRenderer()->makeKnownLink( $redirectTitle, $redirectText ) )->text() .
				"</span>";
		}
		...
		if ( !is_null( $sectionTitle ) ) {
			...
			$section = "<span class='searchalttitle'>" .
				$this->msg( 'search-section' )->rawParams(
					$this->getLinkRenderer()->makeKnownLink( $sectionTitle, $sectionText ) )->text() .
				"</span>";
		}
	...
	protected function showInterwikiHit( $result, $lastInterwiki, $query ) {
		...
		if ( is_null( $lastInterwiki ) || $lastInterwiki != $title->getInterwiki() ) {
			...
			$searchLink = $this->getLinkRenderer()->makeKnownLink(
				$searchTitle,
