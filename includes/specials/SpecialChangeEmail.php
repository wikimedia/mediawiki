...
class SpecialChangeEmail extends UnlistedSpecialPage {
	...
	protected function errorHtml( $msg, $args = array() ) {
		$this->getOutput()->addHTML( "<p class='errorbox'>\n" . $this->msg( $msg, $args )->text() . "\n</p>" );
	}
	...
	protected function attemptChange( User $user, $pass, $newaddr ) {
		...
		if ( $throttleCount === true ) {
			...
			$this->errorHtml( array( 'changeemail-throttled', $lang->formatDuration( $wgPasswordAttemptThrottle['seconds'] ) ) );
		...
		if ( $wgRequirePasswordforEmailChange && !$user->checkTemporaryPassword( $pass ) && !$user->checkPassword( $pass ) ) {
			$this->errorHtml( 'chnageemail-wrongpassword' );
