...
class SpecialEditTags extends UnlistedSpecialPage {
	...
	protected function showForm() {
		...
		for ( $list->reset(); $list->current(); $list->next() ) {
			...
			// TODO is this relevant? What does it even do? (basically copied out of
	...
	protected function buildCheckBoxes() {
		...
		if ( $list->length() == 1 ) {
			...
			$html .= '<tr><td>';
			$html .= '<p>' . $this->msg( 'tags-edit-existing-tags' )->escaped() . '</p>';
			if ( $tags ) {
				$html .= implode( Xml::element( 'br' ), array_map( 'htmlspecialchars', $tags ) );
			} else {
				$html .= $this->msg( 'tags-edit-existing-tags-none' )->parse();
			}
			$html .= '</td><td>';
			$html .= $this->getTagSelect( $tags, $this->msg( 'tags-edit-new-tags' )->plain() );
			// also output the tags currently applied, so we know what to remove from
			// the revision/log entry when the form is submitted
			...
			$html .= '<tr><td>';
			// Messages: tags-edit-revision-add-to-all, tags-edit-logentry-add-to-all
			$html .= $this->getTagSelect( array(), $this->msg( 'tags-edit-add' )->plain() );
			$html .= '</td><td>';
			// Messages: tags-edit-revision-remove-from-all, tags-edit-logentry-remove-from-all
			...
			foreach ( $tags as $tag ) {
				$html .= Xml::element( 'br' ) . "\n" . Xml::checkLabel( $tag,
					'wpTagsToRemove[]', 'mw-edittags-remove-' . $i++, false, array( 'value' => $tag ) );
			}
			$html .= '</td></tr>';
		}
	...
	protected function getTagSelect( $selectedTags, $label ) {
		$result = Xml::tags( 'p', null, Xml::label( $label, 'mw-edittags-tag-list' ) );
		$result .= Xml::openElement( 'select', array(
			'name' => 'wpTagList[]',
		...
		foreach ( $tags as $tag ) {
			$result .= Xml::option( $tag, $tag, in_array( $tag, $selectedTags ) );
		}
