		...
		if ( $rev->isDeleted( Revision::DELETED_TEXT ) ) {
			...
			if (
				...
				$out->addWikiMsg(
					...
					wfEscapeUrl( $this->getRequest()->getVal( 'target' ) )
				);
			} elseif (
				$rev->isDeleted( Revision::DELETED_RESTRICTED ) &&
				$rev->userCan( Revision::DELETED_TEXT, $user )
			) {
				// No point trying to hide it - they know they can view most deleted revisions
				// but not this one, therefore it is suppressed
				$out->addWikiMsg( 'rev-suppressed-text-permission' );
				if ( $user->isAllowed( 'suppressionlog' ) ) {
					$out->addWikiMsg(
						'rev-suppressed-text-permission-log',
						wfEscapeUrl( $this->getRequest()->getVal( 'target' ) )
					);
				}
				$out->addHTML( "\n</div>\n<br />" );
