...
class PageArchive {
	...
	public static function listPagesByPrefix( $prefix, $titles = array() ) {
		...
		if ( $titles ) {
			...
			$conds = [
				'ar_namespace' => $ns,
				$dbr->makeList( [ 'ar_title' => $condTitles ], LIST_OR ) . " OR ar_title " . $dbr->buildLike( $prefix, $dbr->anyString() )
			];
// 			$condTitles = array_map( function (Title $t) use($dbr) {
// 					return $dbr->makeList( [ 'ar_namespace' => $t->getNamespace(),
// 											 'ar_title' => $t->getDBkey() ],
// 										   LIST_AND);
// 				}, $titles);
// 			$condTitles[] =  $dbr->makeList( [ 'ar_namespace' => $ns,
...
class SpecialUndelete extends SpecialPage {
	...
	function showSearchForm() {
		...
		if ( $this->mSearchPrefix ) {
			...
			$results = $se->searchArchiveTitle( $this->mSearchPrefix );
			if ( !is_array( $results ) ) {
				$results = [];
			}
