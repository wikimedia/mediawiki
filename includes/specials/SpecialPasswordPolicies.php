...
class SpecialPasswordPolicies extends SpecialPage {
	...
	public function execute( $par ) {
		...
		foreach ( $allGroups as $group ) {
			...
			$groupnameLocalized = !$msg->isBlank() ? $msg->text() : $group;

			$msg = $this->msg( 'grouppage-' . $group )->inContentLanguage();
			$grouppageLocalized = !$msg->isBlank() ?
				$msg->text() :
				MWNamespace::getCanonicalName( NS_PROJECT ) . ':' . $group;
			$grouppageLocalizedTitle = Title::newFromText( $grouppageLocalized );

			if ( !$grouppageLocalizedTitle ) {
