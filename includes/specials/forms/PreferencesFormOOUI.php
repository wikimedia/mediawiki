...
class PreferencesFormOOUI extends OOUIHTMLForm {
	...
	public function getBody() {
		...
		$useMobileLayout =
			$config->get( MainConfigNames::SpecialPreferencesUseMobileLayout );
	...
	private function createMobilePreferencesForm() {
		...
		$iconNames = [
			...
			'openstack' => 'key',
			'uploads' => 'upload',
			'centralnotice-banners' => 'feedback',
			'misc' => '',
		];
		...
		foreach ( $this->mFieldTree as $key => $val ) {
			...
			if ( $iconExists ) {
				...
				wfDebug( __METHOD__ . " icon does not exist for section: '$key'" );
				$iconName = "";
			}
			...
			$prefPanel->appendContent( ( new OOUI\Tag( 'p' ) )
				->appendContent( $this->msg( "prefs-description-" . $key )->text() )
