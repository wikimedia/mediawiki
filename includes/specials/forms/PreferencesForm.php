...
class PreferencesForm extends OOUIHTMLForm {
	...
	function getBody() {
		...
		foreach ( $this->getPreferenceSections() as $i => $key ) {
			$fakeTabs[] =
				Html::rawElement(
					...
					[
						'class' =>
							'oo-ui-widget oo-ui-widget-enabled oo-ui-optionWidget oo-ui-tabOptionWidget oo-ui-labelElement' .
		...
		$fakeTabsHtml = Html::rawElement(
			...
			Html::rawElement(
				'div',
				[ 'class' => 'oo-ui-widget oo-ui-widget-enabled oo-ui-selectWidget oo-ui-selectWidget-depressed oo-ui-tabSelectWidget' ],
				implode( $fakeTabs )
			)
		);
