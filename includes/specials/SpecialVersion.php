...
class SpecialVersion extends SpecialPage {
	...
	static function softwareInformation() {
		...
		// be loaded here, so feel free to use $this->msg() in the 'name'. Raw HTML or
	...
	public static function getVersion( $flags = '' ) {
		...
		if ( !$svnInfo && !$gitInfo ) {
			...
			$shortSha1 = $this->msg( 'parentheses' )->params( $shortSha1 )->escaped();
	...
	public static function getExtensionTypes() {
		if ( self::$extensionTypes === false ) {
			self::$extensionTypes = array(
				'specialpage' => $this->msg( 'version-specialpages' )->text(),
				'parserhook' => $this->msg( 'version-parserhooks' )->text(),
				'variable' => $this->msg( 'version-variables' )->text(),
				'media' => $this->msg( 'version-mediahandlers' )->text(),
				'antispam' => $this->msg( 'version-antispam' )->text(),
				'skin' => $this->msg( 'version-skins' )->text(),
				'api' => $this->msg( 'version-api' )->text(),
				'other' => $this->msg( 'version-other' )->text(),
			);
	...
	public static function arrayToString( $list ) {
		...
		if( is_object( $list ) ) {
			$class = $this->msg( 'parentheses' )->params( get_class( $list ) )->escaped();
			return $class;
			...
			return $this->msg( 'parentheses' )->params( "$class, {$list[1]}" )->escaped();
		}
	}
