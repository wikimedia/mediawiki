...
class SpecialVersion extends SpecialPage {
		...
		switch ( strtolower( $parts[0] ) ) {
			case 'credits':
				...
				if ( $extName === 'MediaWiki' ) {
					$wikiText = file_get_contents( $config->get( 'IP' ) . '/CREDITS' );
			...
			case 'license':
				...
				if ( $extName === 'MediaWiki' ) {
					$wikiText = file_get_contents( $config->get( 'IP' ) . '/COPYING' );
				} elseif ( ( $extNode !== null ) && isset( $extNode['path'] ) ) {
	...
	public static function getVersionLinked() {
		...
		$config = ConfigFactory::getDefaultInstance()->makeConfig( 'main' );
		$gitVersion = self::getVersionLinkedGit( $config->get( 'IP' ) );
		if ( $gitVersion ) {
			$v = $gitVersion;
		} else {
			$svnVersion = self::getVersionLinkedSvn( $config );
			if ( $svnVersion ) {
				$v = $svnVersion;
			} else {
				$v = $config->get( 'Version' ); // fallback
			}
		}
	 ...
	 */
	private static function getVersionLinkedSvn( $config = null ) {
		if ( !$config ) {
			$config = ConfigFactory::getDefaultInstance()->makeConfig( 'main' );
		}
		$info = self::getSvnInfo( $config->get( 'IP' ) );
		...
		return self::getwgVersionLinked( $config->get( 'Version' ) ) . " $version";
	}
	 ...
	 */
	private static function getVersionLinkedGit( $IP ) {
		global $wgLang;
