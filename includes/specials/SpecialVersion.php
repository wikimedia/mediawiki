	...
	public function execute( $par ) {
		...
		switch ( strtolower( $parts[0] ) ) {
			case 'credits':
				...
				$out->setPageTitle( new Message( 'version-credits-title', array( $extName ) ) );
				...
				if ( $extName === 'MediaWiki' ) {
					...
					if ( $file ) {
						...
						if ( !isset( $extNode['license-name'] ) ) {
							...
							// now wikitext friendly.
							$wikiText = "<pre>$wikiText</pre>";
						}
					}
				}

				$out->setPageTitle( new Message( 'version-license-title', array( $extName ) ) );
				$out->addWikiText( $wikiText );
				break;
		...
		*/
		...
		if ( $vcsVersion ) {
			if ( $vcsLink ) {
				...
				$vcsVerString = Xml::openElement( 'span', array( 'class' => 'mw-version-ext-vcs-version') ) .
					"({$vcsVersion})" .
			...
			if ( $vcsDate ) {
				$vcsTimeString = '<br />' .
					Xml::openElement( 'span', array( 'class' => 'mw-version-ext-vcs-timestamp') ) .
