...
class SpecialGroupPermissions extends SpecialPage {
	...
	private function doSubmit( $subpage ) {
		...
		if ( !isset( $params['wpGroup'] ) && $subpage == '' ) {
			throw new MWException( "" );
		} elseif ( !isset( $params['wpGroup'] ) ) {
			$group = $subpage;
		} else {
		...
		// Do the actual adding to the database.
		$db = wfGetDB( DB_MASTER );

		// If this is an existing group, update. Otherwise insert.
		if ( $db->selectField(
			'groups',
			'group_permissions',
			array( 'group_internal_name' => $group ),
			__METHOD__
		) ) { // Row already exists for this group.
			$db->update(
				'groups',
				array( 'group_permissions' => json_encode( $newRights ) ),
				array( 'group_internal_name' => $group ),
				__METHOD__
			);
		} else {
			$db->insert(
				'groups',
				array(
					'group_internal_name' => $group,
					'group_permissions' => json_encode( $newRights )
				),
				__METHOD__
			);
		}
	...
	protected function showLogFragment( $group, $output ) {
		...
		LogEventsList::showLogExtract( $output, 'modifygroup', $title->getPrefixedText() );
	}
}
