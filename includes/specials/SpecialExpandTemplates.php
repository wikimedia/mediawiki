...
class SpecialExpandTemplates extends SpecialPage {
	...
	private function showHtmlPreview( Title $title, ParserOutput $pout, OutputPage $out ) {
		...
		$out->addModules( $pout->getModules() );
		$out->addModuleScripts( $pout->getModuleScripts() );
		$out->addModuleStyles( $pout->getModuleStyles() );
		$out->addModuleMessages( $pout->getModuleMessages() );
	}
