...
class SpecialRevisionDelete extends UnlistedSpecialPage {
	...
	public function execute( $par ) {
		...
		if ( $request->getVal( 'action' ) == 'historysubmit' || $request->getVal( 'action' ) == 'revisiondelete' ) {
			...
			if ( $this->targetObj->isSpecialPage() && $this->targetObj->getText() == 'Log' ) {
				$result = wfGetDB( DB_MASTER )->select( 'logging',
					'log_type',
					array( 'log_id IN (' . implode( ',', $this->ids ) . ')' ),
					__METHOD__
				);

				$logTypes = array();
				foreach ( $result as $row ) {
					if ( in_array( $row->log_type, $logTypes ) ) {
						continue;
					} else {
				...
				if ( count ( $logTypes ) == 1 ) { // If there's only one type, the target can be set to include it.
