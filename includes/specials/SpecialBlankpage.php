...
class SpecialBlankpage extends UnlistedSpecialPage {
	...
	public function execute( $par ) {
		...
		$exceptions = array(
			...
			"plwikimedia" => "Wikimedia Polska",
			"sewikimedia" => "Wikimedia Sverige",
		...
		$this->getOutput()->addWikiText( FormatJson::encode( $outEn, true, FormatJson::ALL_OK ) );
		$this->getOutput()->addWikiText( '= qqq.json =' );
		$this->getOutput()->addHTML( '<pre>'. FormatJson::encode( $outQqq, true, FormatJson::ALL_OK ).'</pre>' );

		$this->getOutput()->addWikiText( '= Table =' );
		$tableWikitext = "{|\n! scope=\"col\" | dbname\n! scope=\"col\" | Name\n! scope=\"col\" | URL\n";
		foreach( $table as $dbname => $wikiInfo ) {
			$tableWikitext .= "|-\n! scope=\"row\" | " . "$dbname\n";
			$tableWikitext .= "| " . $wikiInfo['name'] . " || " . $wikiInfo['url'] . "\n\n";
		}
		$tableWikitext .= "|}";
		$this->getOutput()->addHTML( Html::element( 'pre', array(), $tableWikitext ) );
