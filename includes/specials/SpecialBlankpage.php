...
class SpecialBlankpage extends UnlistedSpecialPage {
	...
	private function createQQQ( $projName, $projURL ) {
		return "The localized project name for '[$projURL $projName]'";
	}
	...
	public function execute( $par ) {
		...
		for ( $i = 0; $i < count( $info ); $i++ ) {
			...
			for ( $j = 0; $j < count( $sites ); $j++ ) {
				...
				// if ( !isset( $sites[ $j ]['closed'] ) ) {
					...
					// Output the line
					$out_en .= "\"project-humanreadable-name-" . $dbname . "\": \"" .
						$lang . " " . $sitename . "\";\n\n";

					$out_qqq .= $this->createQQQ( $lang . " " . $sitename, $url ) . ";\n\n";
					$counter++;
				// }
			}
		}
		...
		for ( $i = 0; $i < count( $specials ); $i++ ) {
			...
			// if ( !isset( $specials[ $j ]['closed'] ) ) {
				...
				$out_en .= "\"project-humanreadable-name-" . $dbname . "\": \"" .
					$sitename . "\";\n\n";
				$out_qqq .= $this->createQQQ( $sitename, $url ) . ";\n\n";
				$counter++;
			// }
		}
		...
		$this->getOutput()->addWikiText( "<hr>\n" );
		$this->getOutput()->addWikiText( '= en.json =' );
		$this->getOutput()->addWikiText( $out_en );
		$this->getOutput()->addWikiText( '= qqq.json =' );
		$this->getOutput()->addWikiText( $out_qqq );
	}
