...
class SpecialTrackingCategories extends SpecialPage {
	...
	function execute( $par ) {
		...
		foreach( $wgTrackingCategories as $catMsg ) {
			...
			if ( $descMsg->isBlank() ) {
				$descMsg = $this->msg( 'trackingcat-nodesc' );
			}

			$this->getOutput()->addHTML(
				Html::openElement( 'tr' ) .
				Html::openElement( 'td', array( 'class' => 'mw-trackingcatname' ) ) .
					$this->getLanguage()->commaList( array_unique( $allMsgs ) ) .
				Html::closeElement( 'td' ) .
				Html::openElement( 'td', array( 'class' => 'mw-trackingcatmsg' ) ) .
					$catMsgTitleText .
				Html::closeElement( 'td' ) .
				Html::openElement( 'td', array( 'class' => 'mw-trackingcatdesc' ) ) .
					$descMsg->parse() .
				Html::closeElement( 'td' ) .
