 ...
 * of options instead of select fields. For activating the latter behavior, the option 'usecheckboxes' can be set to
...
class HTMLMultiSelectField extends HTMLFormField {
	...
	/**
	 * whether the form is using checkboxes or a multi-select field for representation.
	 * @var bool
	 */
	...
	function __construct( $params ) {
		...
		if( !is_bool( $this->useCheckboxes ) ) {
			// keep using checkboxes if the total number of options is below or equal to the number
			// the 'usecheckboxes' is set to.
	 ...
	 * 'multiple' attribute set is used to represent the multi select. To activate the latter mode, the option
	...
	function getInputHTML( $value ) {
		...
		if( !$this->isUsingCheckboxes() ) {
			// height of the element, not higher than 15 for now, could be configurable as well
