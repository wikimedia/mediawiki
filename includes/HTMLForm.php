 ...
 * of options instead of select fields. For activating the later behavior, the option 'usecheckboxes' can be set to false
 * or to a number stating how many options have to be available till switching behavior.
 *
...
class HTMLMultiSelectField extends HTMLFormField {
	...
	function isUsingCheckboxes() {
		if( isset( $useCheckboxes ) ) {
			return $useCheckboxes; // use changed result so we don't have to count all options again
		}

		static $useCheckboxes;
		$useCheckboxes = isset( $this->mParams['usecheckboxes'] )
			? $this->mParams['usecheckboxes']
			: true; // fallback, use checkboxes

		if( is_bool( $useCheckboxes ) ) {
			return $useCheckboxes;
		}
		// keep using checkboxes until we have more options than the number in 'usecheckboxes' is set to
		$useCheckboxes = count( $this->mParams['options'] ) <= $useCheckboxes;
		return $useCheckboxes;
	}
	...
	function getInputHTML( $value ) {
		...
		if( ! $this->isUsingCheckboxes() ) {
			...
			$size = count( $this->mParams['options'] ) <= 15
				? count( $this->mParams['options'] )
				: 15;

			$html = XML::openElement( 'select', array( 'multiple' => 'multiple', 'size' => $size, 'name' => $this->mName . '[]' ) )
				. $html
