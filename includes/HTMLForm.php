 ...
 *
 * Uses either:
 *  - multiple html input elements (default)
 *  - a select list with 'multiple' attribute and a set of options instead of
 *  select field.
 *
 * To activate the latter behavior, the option 'usecheckboxes' can be set to
 * false or to a number stating how many options have to be available till
 * switching behavior.
 *
...
class HTMLMultiSelectField extends HTMLFormField {
	...
	/**
	 * Whether the form is using checkboxes or a multi-select field for
	 * representation.
	 * @var bool
	 */
	private $useCheckboxes;

	/**
	 * @see HTMLFormField::__construct
	 */
	function __construct( $params ) {
		parent::__construct( $params );
		...
		if( !is_bool( $this->useCheckboxes ) ) {
			// keep using checkboxes if the total number of options is below or
			// equal to the number the 'usecheckboxes' is set to.
			$this->useCheckboxes = count( $this->mParams['options'] ) <= $this->useCheckboxes;
		}
	}

	/**
	 * Whether to uses checkboxes when displaying multi select.
	 *
	 * If this returns false, a select element with 'multiple' attribute set is
	 * used to represent the multi select.
	 *
	 * To activate the latter mode, the option 'usecheckboxes' in the options
	 * given to the constructor must be set to true or to a number lower than
	 * the actual number of available options.
	 *
	 * @since 1.20
	 *
	 * @return bool
	...
	function getInputHTML( $value ) {
		...
		if( !$this->isUsingCheckboxes() ) {
			// height of the element, not higher than 15 for now, could be
			// configurable as well.
			$size = min( count( $this->mParams['options'] ), 15 );

			$html = Html::rawElement( 'select',
				array(
					'multiple' => 'multiple',
					'size'     => $size,
					'name'     => $this->mName . '[]'
		...
		foreach ( $options as $label => $info ) {
			if ( is_array( $info ) ) {
				...
				if( $this->isUsingCheckboxes() ) {
					...
					$checkbox .= '&#160;';
					$checkbox .=Html::rawElement( 'label',
