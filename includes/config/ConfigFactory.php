...
class ConfigFactory {
	 ...
	 * @throws ConfigException if an invalid callback is provided
	 */
	public function register( $name, $callback ) {
		if ( !is_callable( $callback ) ) {
			throw new ConfigException( 'Invalid callback provided' );
		}
	 ...
	 * object is returned
	 * @param string $name
	 * @throws ConfigException if a builder isn't registered,
	 * or if the builder returns a non-Config object
	 * @return Config
	 */
	public function makeConfig( $name ) {
		if ( !isset( $this->configs[$name] ) ) {
			...
			$conf = call_user_func( $this->builders[$name] );
			if ( $conf instanceof Config ) {
				$this->configs[$name] = $conf;
			} else {
				throw new ConfigException( "The builder for $name returned a non-Config object." );
			}
		}
