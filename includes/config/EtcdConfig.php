...
class EtcdConfig implements Config, LoggerAwareInterface {
		...
		$loop = new WaitConditionLoop(
			function () use ( $key, $now, &$data, &$error ) {
				...
				if ( $this->srvCache->lock( $key, 0, $this->baseCacheTTL ) ) {
					try {
						list( $config, $error, $retry, $lastSeenIndex ) = $this->fetchAllFromEtcd();
						if ( is_array( $config ) ) {
	 ...
	 * @return int lastModifiedIndex The last modified index of any key in the directory
	 */
	protected function parseDirectory( $dirName, $dirNode, &$config ) {
		...
		foreach ( $dirNode['nodes'] as $node ) {
			...
			if ( !empty( $node['dir'] ) ) {
				$lastModifiedIndex = max($this->parseDirectory( $fullName, $node, $config ), $lastModifiedIndex);
