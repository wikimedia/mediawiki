...
class EtcdConfig implements Config, LoggerAwareInterface {
	...
	public function __construct( array $params ) {
		$params += [ 'encoding' => 'JSON', 'cacheTTL' => 10, 'skewTTL' => 1, 'timeout' => 10 ];
	...
	private function load() {
		...
		$loop = new WaitConditionLoop(
			function () use ( $key, $now, &$data ) {
				...
				if ( $this->srvCache->lock( $key, 0, $this->baseCacheTTL ) ) {
					try {
						$config = $this->fetchConfigFromEtcd();
						...
						$this->logger->error(
							"Failed to refresh configuration data. Got " .
		...
		if ( $loop->invoke() !== WaitConditionLoop::CONDITION_REACHED ) {
			...
			throw new ConfigException( "Failed to load configuration from etcd." );
		}
	 ...
	 */
	public function fetchConfigFromEtcd() {
		...
		foreach ( $values as $key => $value ) {
			...
			// Extract the core value out of the etcd object (which contains other metadata)
			$this->assertMapHasKeys( $map, [ 'val' ], $name );
			$config[$name] = $map['val'];
		}
	 ...
	 * @throws ConfigException
	 */
	private function assertMapHasKeys( $value, array $keys, $name ) {
		if ( !is_array( $value ) ) {
			throw new ConfigException( "Value for '$name' is not an array." );
		}

		foreach ( $keys as $key ) {
			if ( !array_key_exists( $key, $value ) ) {
				throw new ConfigException( "Expected key '$key' inside '$name' value." );
			}
		}
	}
}
