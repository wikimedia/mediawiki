...
class FileContentsHasher {
	 ...
	 */
	protected function getFileContentsHashInternal( $filePath, $algo = 'md4' ) {
	 ...
	 */
	public function getFileContentsHash( $filePaths, $algo = 'md4' ) {
		if ( !is_array( $filePaths ) ) {
			$filePaths = (array) $filePaths;
		}

		if ( count( $filePaths ) === 1 ) {
			return $this->getFileContentsHashInternal( $filePaths[0], $algo );
		}

		sort( $filePaths );
		$that = $this;
		$hashes = array_map( function ( $filePath ) use ( $algo, $that ) {
			return $that->getFileContentsHashInternal( $filePath, $algo ) ?: '';
		}, $filePaths );
