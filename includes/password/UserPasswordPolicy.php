...
class UserPasswordPolicy {
	 ...
	 * their own password validity checks. Checking functions are called with the
	 * policy value for this user, the username, and the password to check.
	 * @param array $mappings between policy statements and functions to check compliance
	 */
	public static function addPolicyCheckFunctions( array $mappings ) {
		if ( !is_array( $mappings ) ) {
			throw new InvalidArgumentException(
				'Trying to add non-array password validity mapping'
			);
		} elseif ( array_intersect_key( $mappings, self::$policyCheckFunctions ) ) {
			throw new InvalidArgumentException(
				'Cannot override built-in password policy checks'
			);
		}
	...
	/**
	 * Get the effective policy from a list of policies, based on a list of groups.
	 * @param array $policies list of policies to consider
	 * @param array $groups the groups from which we calculate the effective policy
	 * @param array $defaul the default policy to start from
	 * @return effective policy from the list of $policies, based on $groups
	 */
	public static function getPoliciesForGroups( array $policies, array $userGroups,
		array $defaultPolicy
	) {
		$effectivePolicy = $defaultPolicy;
		foreach ( array_keys( $policies ) as $group ) {
