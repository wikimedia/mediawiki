...
class UserPasswordPolicy {
	 ...
	 * @var array
	 */
	private $policyCheckFunctions = array(
	...
	public function __construct( array $policies ) {
		if ( count( $policies ) < 1 || !isset( $policies['default'] ) ) {
			throw new MWException( 'Must include a \'default\' password policy' );
		}
	 ...
	 */
	public function addPolicyCheckFunction( $mappings ) {
		if ( !is_array( $mappings ) ) {
			throw new Exception( 'Trying to add non-array password validity mapping' );
		}
		$this->policyCheckFunctions += $mappings;
	}
	...
	public function checkUserPassword( User $user, $password ) {
		...
		foreach ( $effectivePolicy as $policy=>$value ) {
			if ( !isset( $this->policyCheckFunctions[$policy] ) ) {
				throw new Exception( 'Invalid password policy config' );
			}
			$status->merge(
				call_user_func(
