 ...
 class LayeredParameterizedPassword extends ParameterizedPassword {
	...
	function getDefaultParams() {
		foreach ( $this->config['types'] as $type ) {
			...
			$params[$type] = implode( $passObj->getDelimiter(), $passObj->getDefaultParams() );
		}
	...
	function crypt( $password ) {
		...
		foreach ( $this->config['types'] as $type ) {
			...
			$args = '';
			if ( $this->params[$type] !== '' ) {
				$params = $this->params[$type] . $passObj->getDelimiter();
			}
			...
			$this->params[$type] = implode( $passObj->getDelimiter(), $passObj->params );
