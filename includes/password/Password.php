...
abstract class Password implements Serializable {
	 ...
	 */
	final public static function register( $type, $class, array $config ) {
		$config['type'] = $type;
		self::$types[$type] = array( $class, $config );
	}
	...
	final public static function initFromGlobals() {
		...
		foreach ( $wgPasswordConfig as $type => $config ) {
			$class = $config['class'];
			self::register( $type, $class, $config );
		}
	...
	final public static function newFromCiphertext( $hash ) {
		if ( $hash === null || $hash === '' ) {
			return new InvalidPassword( array(), null );
		} elseif ( $hash[0] !== ':' ) {
		...
		list( $class, $config ) = self::$types[$type];
		return new $class( $config, $hash );
	}
	...
	final public static function newFromPlaintext( $password, Password $existing = null ) {
		if ( $existing === null ) {
			list( $class, $config ) = self::$types[self::$default];
