 ...
 * and is added back in in the toString() function.
 *
 * When inheriting this class, there are a couple of expectations that are expected
 * to be fulfilled:
 ...
 *    for being passed to the constructor of that class, and then stores that hash (and whatever other data)
 ...
 *  * Password::needsUpdate(), which can be useful if a specific password hash has different logic for
 *    when the hash needs to be updated.
 *  * Password::toString(), which can be useful if the hash was changed in the constructor and needs to
 *    be re-assembled before being returned as a string. This function is expected to add the type back
 *    on to the hash, so make sure to do that if you override the function.
 *  * Password::equalsCiphertext() - This function compares two Password objects to see if they are equal.
 *    The default is to just do a timing-safe string comparison on the $this->hash values.
 *  * Password::equalsPlaintext() - This function compares a plaintext password directly to the hash object.
 *    The default implementation of this is to just call Password::newFromPlaintext() and then pass the return
 *    value back into Password::equalsCiphertext(). The only reason this function should be overridden is if
 *    the class is made to encapsulate a third party library, and that library does not technically allow you
 *    to create password hashes based on an existing hash (for an example, see PhpApiPassword).
 *
 * After creating a new password hash type, it can be registered using the static Password::register()
 * method. The default type is set using the Password::setDefault() type. Types must be registered before
 * they can be set as the default.
 *
 * @since 1.22
 */
abstract class Password implements Serializable {
	 ...
	 * @param string $class Fully qualified name of the class to register
	 */
	final public static function register( $type, $class ) {
	...
	final public static function setDefault( $type ) {
		if ( !isset( self::$types[$type] ) ) {
			throw new MWException( "Invalid password type $type." );
		}
		self::$default = $type;
	}
	...
	final public static function newFromCiphertext( $hash ) {
		if ( $hash === null || $hash === '' ) {
			return new InvalidPassword( null );
		} elseif ( $hash[0] !== ':' ) {
		...
		return new self::$types[$type]( $hash );
	}
	 ...
	 *
	 * @param string $hash Plaintext password
	 ...
	 *
	 * @param string $hash The raw hash with its type (the initial :<TYPE>:) stripped
	 */
	final private function __construct( $hash = null ) {
	...
	/**
	 * Get the type of this hash based on the runtime class
	 *
	 * @return string
	 */
	final public function getType() {
		return array_search( get_class( $this ), self::$types );
	}
	...
	public function needsUpdate() {
		$currentType = array_search( $this->getType(), self::$types );

		return $currentType !== self::$default;
	}

	/**
	 * Compare the given password to this
	 *
	 * @param Password|string $other Password to compare
	 * @return bool True if passwords match, false otherwise
	 */
	final public function equals( $other ) {
		if ( $other instanceof self ) {
			return $this->equalsCiphertext( $other );
		} else {
			return $this->equalsPlaintext( $other );
		}
	}
	 ...
	 * custom comparison, but it is not recommended unles necessary.
	 *
	 * @param Password $other The other password
	 * @return bool True if equal, false otherwise
	 */
	protected function equalsCiphertext( Password $other ) {
	...
	/**
	 * Compare a plaintext password to this object
	 *
	 * By default, just make a new Password object from the plaintext and pass it over
	 * to Password::equalsCiphertext. This can be overridden if the hash type is wrapping
	 * a library that can only check equality based on plaintext (e.g., PhpApiPassword).
	 *
	 * @param string $other Plaintext password
	 * @return bool True if equal, false otherwise
	 */
	protected function equalsPlaintext( $other ) {
		return $this->equalsCiphertext( self::newFromPlaintext( $other, $this ) );
	}
