...
class Title {
	...
	public function getPageLanguageSettings() {
		...
		if ( !$this->mPageLanguage || $this->mPageLanguage[1] !== $wgLanguageCode ) {
			...
			// "usedb" should be checked for in the frontend;
			// maybe it makes sense to check here as well?
			if( $this->mDbPageLanguage ) {
				$settings['pagelanguage'] = $this->mDbPageLanguage;
			} else {
				$dbr = wfGetDB( DB_SLAVE );
				$row = $dbr->selectRow(
					'page',
				...
				if( isset( $row->page_lang ) ) {
					$settings['pagelanguage'] = $row->page_lang;
				}
			}
