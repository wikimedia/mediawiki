...
class Title {
	...
	public function getParentCategoryListForEditNotices() {
		...
		$timestamp = $list[-1];
		if ( $list && $this->getTouched() < $timestamp &&
			$msg->getTouched() < $timestamp ) {
			unset( $list[-1] );
			return $list;
		}
	...
	public static function makeCategoryListForEditNotices() {
		...
		foreach ( $rows as $rowtext ) {
			// getting text before first //
			$row = explode( "//", $rowtext, 2 );
			$string = $row[0];
			$name = trim( $string );
			// if anything, add it to the list with category prefix
			if ( $name ) {
				$entry = $wgContLang->getNsText( NS_CATEGORY ) . ':';
				$entry .= str_replace( " ", "_", $name );
				$list[] =  $entry;
