...
class Title implements LinkTarget {
	...
	public function isUserJsConfigPage() {
		return (
			NS_USER == $this->mNamespace
			&& $this->isSubpage()
			$this->hasContentModel( CONTENT_MODEL_JS )
		);
	}
	...
	private function checkUserConfigPermissions( $action, $user, $errors, $rigor, $short ) {
		...
		if ( $action != 'patrol' ) {
			if ( preg_match( '/^' . preg_quote( $user->getName(), '/' ) . '\//', $this->mTextform ) ) {
				if ( $this->isUserCssConfigPage() && !$user->isAllowedAny( 'editmyusercss', 'editusercss' ) ) {
					$errors[] = [ 'mycustomcssprotected', $action ];
				} elseif ( $this->isUserJsonConfigPage() && !$user->isAllowedAny( 'editmyuserjson', 'edituserjson' ) ) {
					$errors[] = [ 'mycustomjsonprotected', $action ];
				} elseif ( $this->isUserJsConfigPage() && !$user->isAllowedAny( 'editmyuserjs', 'edituserjs' ) ) {
					$errors[] = [ 'mycustomjsprotected', $action ];
				}
			} else {
				if ( $this->isUserCssConfigPage() && !$user->isAllowed( 'editusercss' ) ) {
					$errors[] = [ 'customcssprotected', $action ];
				} elseif ( $this->isUserJsonConfigPage() && !$user->isAllowedAny( 'edituserjson' ) ) {
					$errors[] = [ 'customjsonprotected', $action ];
				} elseif ( $this->isUserJsConfigPage() && !$user->isAllowed( 'edituserjs' ) ) {
