...
class Title {
	...
	public static function newFromRequest( WebRequest $request ) {
		...
		if ( is_string( $request->getVal( 'search' ) ) ) {
			...
			$title = self::newFromURL( $pagename );
			if ( $title ) {
				// Alias NS_MEDIA page URLs to NS_FILE...we only use NS_MEDIA
				// in wikitext links to tell Parser to make a direct file link
				if ( $title->getNamespace() === NS_MEDIA ) {
					$title = self::makeTitle( NS_FILE, $title->getDBkey() );
				}
				// Check variant links so that interwiki links don't have to worry
				// about the possible different language variants
				global $wgContLang;
				if ( $wgContLang->hasVariants() && $title->getArticleID() === 0 ) {
					$wgContLang->findVariantLink( $pagename, $title );
				}
			}
		}
		...
		if ( !$title || !$title->isSpecialPage() ) {
			...
			$oldid = $request->getInt( 'oldid' ) ?: $request->getInt( 'diff' );
			// Allow oldid to override a changed or missing title
			if ( $oldid ) {
				$rev = Revision::newFromId( $oldid );
				$title = $rev ? $rev->getTitle() : $title;
			}
		}
		...
		if ( !$title
			...
			&& $request->getVal( 'action', 'view' ) !== 'delete'
		) {
