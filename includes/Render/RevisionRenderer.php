...
class RevisionRenderer {
	...
	public function renderRevisionSlots(
		...
		// TODO: MCR: Split into single-slot and multi-slot renderer!
		// For multiple slots, either use a separate implementation for multiple slots,
		// or some kind of hook system.
		// TODO: We may want to combine the meta-data from all slots, regardless of if and how
		// the HTML is combined, see T190063.

		$title = Title::newFromLinkTarget( $title );
		$slot= $slots->getSlot( $this->slot );

		return $this->getSlotRendering(
			$title,
			$slot,
			$options,
			$revisionId,
	 ...
	 * @param SlotRenderingProvider $slotRenderingProvider
	 * @param int|null $revisionId
	 * @param bool $generateHtml
	 *
	 * @return Rendering
	 */
	protected function getSlotRendering(
		Title $title,
		SlotRecord $slot,
		ParserOptions $options,
		SlotRenderingProvider $slotRenderingProvider = null,
		...
		if ( $slotRenderingProvider ) {
			return $slotRenderingProvider->getRendering( $slot->getRole(), $generateHtml );
		}
