	...
	public function checkArray( array $extDependencies, array $credits ) {
		...
		foreach ( $extDependencies as $extension => $dependencies ) {
			foreach ( $dependencies as $dependency => $constraint ) {
				...
				// check, if the extension has it's version mentioned in extension.json and set as incompatible if not
				if ( !isset( $credits[$dependencyName]['version'] ) ) {
					// if the extension doesn't need a special version of the dependency, just log an info, that the version is missing
					if ( $constraint === '*' ) {
