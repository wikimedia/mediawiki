...
class ExtensionProcessor implements Processor {
	 ...
	 * @param array $info
	 */
	protected function extractAttributes( array $info ) {
		if ( isset( $info['attributes'] ) ) {
			foreach ( $info['attributes'] as $extName => $value ) {
				if ( isset( $this->extAttributes[$extName] ) ) {
					$this->extAttributes[$extName] = array_merge_recursive(
						$this->extAttributes[$extName], $value
					);
				} else {
					$this->extAttributes[$extName] = $value;
				}
	...
	public function getExtractedInfo() {
		...
		// Merge extAttributes into attributes depending on what is loaded
