...
class ActorMigration {
	...
	public static function newMigration() {
		...
		if ( !self::$singleton || self::$singleton->stage !== $wgActorTableSchemaMigrationStage ) {
			self::$singleton = new ActorMigration( $wgActorTableSchemaMigrationStage );
		}
	...
	public function getWhere( IDatabase $db, $key, $users, $useId = true ) {
		...
		foreach ( $users as $user ) {
			...
			$actorId = $this->stage === MIGRATION_OLD ? null : $user->getActorId();
			if ( $actorId ) {
				$actors[] = $actorId;
			}
		}
