...
class ActorMigration {
	 ...
	 *
	 * @param User $user User to set in the update
	 * @return array to merge into `$values` to `IDatabase->update()` or `$a` to `IDatabase->insert()`
	 */
	public function getInsertValues( User $user ) {
		if ( isset( self::$tempTables[$this->key] ) ) {
		...
		if ( $this->stage >= MIGRATION_WRITE_BOTH ) {
			$ret[$this->actor] = $user->getActorId( true );
		}
		return $ret;
	}
	 ...
	 *  - callback to call with the database handle and the primary key for the main table insert.
	 */
	public function getInsertValuesWithTempTable( User $user ) {
		...
		if ( $this->stage >= MIGRATION_WRITE_BOTH ) {
			$id = $user->getActorId( true );
			if ( isset( self::$tempTables[$this->key] ) ) {
				$func = __METHOD__;
				$callback = function ( $dbw, $pk ) use ( $id, $func ) {
					$t = self::$tempTables[$this->key];
					$dbw->upsert(
						$t['table'],
						[ $t['pk'] => $pk, $t['field'] => $id ],
		...
		return [ $ret, $callback ?: function ( IDatabase $dbw, $pk ) {
		} ];
	}
	 ...
	 * @param User|User[] $users Users to test for
	 * @return array With three keys:
	 *   - tables: (string[]) to include in the `$table` to `IDatabase->select()`
	 ...
	 */
	public function getWhere( IDatabase $db, $users ) {
		$tables = [];
		$conds = [];
		$orconds = null;
		$joins = [];
		...
		if ( $actors ) {
			$conds[] = $db->makeList( [ $joinField => $actors ], IDatabase::LIST_AND );
		}
