...
class RotateFileTransform extends FileTransform {
	...
	public function loadFromRow( stdClass $row ) {
		...
		$this->rotation = intval( $row->ft_rotate_deg );
	}
	...
	protected function insertFields() {
		return array_merge( parent::insertFields(), array(
			'ft_rotate_deg' => $this->rotation
		) );
	}
	...
	protected function transform() {
		...
		$comment = wfMessage(
			'rotate-comment'
		)->numParams( $this->rotation )->inContentLanguage()->text();
