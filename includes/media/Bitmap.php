	...
	protected function generateBucketIfNeeded( $image, $params, $flags ) {
		...
		if ( $image->repo && $bucket && $bucket != $params[ 'physicalWidth' ] ) {
			...
			if ( !$image->repo->fileExists( $bucketPath ) ) {
				...
				$bucketParams['physicalHeight'] = round( ( $bucket / $bucketParams['physicalWidth'] ) * $bucketParams['physicalHeight'] );
				$bucketParams['physicalWidth'] = $bucket;

				$image->generateAndSaveThumb( $this->getBucketThumbName( $image, $bucket ), $bucketParams, $flags );
			} else {
