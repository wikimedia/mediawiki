...
class BitmapHandler extends ImageHandler {
	...
	function doTransform( $image, $dstPath, $dstUrl, $params, $flags = 0 ) {
		...
		if ( $flags & self::TRANSFORM_LATER ) {
			...
			if ( isset( $scalerParams['quality'] ) ) {
				$params['quality'] = $scalerParams['quality'];
			}
	...
	protected function transformImageMagickExt( $image, $params ) {
		...
		try {
			...
			if ( $params['mimeType'] == 'image/jpeg' ) {
				...
				$im->setCompressionQuality( isset( $params['quality'] ) ? $params['quality'] : 80 );
			} elseif ( $params['mimeType'] == 'image/png' ) {
				$im->setCompressionQuality( isset( $params['quality'] ) ? $params['quality'] : 95 );
			} elseif ( $params['mimeType'] == 'image/gif' ) {
				if ( $this->getImageArea( $image ) > $wgMaxAnimatedGifArea ) {
					// Extract initial frame only; we're so big it'll
					// be a total drag. :P
	...
	protected function transformGd( $image, $params ) {
		...
		$typemap = array(
			...
			'image/png' => array( 'imagecreatefrompng', 'bits', true, 'imagepng' ),
