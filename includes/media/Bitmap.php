	...
	protected function generateBucketIfNeeded( $image, $params, $flags ) {
		...
		if ( $image->repo && $bucket && $bucket != $params[ 'physicalWidth' ] ) {
			...
			if ( !$image->repo->fileExists( $bucketPath ) ) {
				...
				$tmpFile = $image->makeTransformTmpFile( $bucketPath );

				$thumb = $image->generateAndSaveThumb( $tmpFile, $bucketName, $params, $flags );
	...
	protected function getSourcePath( $image, $params ) {
		...
		if ( $image->repo && $bucket ) {
			...
			if ( isset( $this->tmpThumbCache[ $bucket ] ) ) {
				wfDebug( __METHOD__ . ": Tmp cache for {$bucket} bucket is set, checking if file exists\n" );
				$tmpPath = $this->tmpThumbCache[ $bucket ];

				if ( file_exists( $tmpPath ) ) {
					wfDebug( __METHOD__ . ": Reusing {$bucket} bucket's temporary file\n" );
					return $tmpPath;
				} else {
					wfDebug( __METHOD__ . ": {$bucket} bucket's temporary file is gone: {$tmpPath}\n" );
				}
			}
