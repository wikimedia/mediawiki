...
class BitmapHandler extends TransformationalImageHandler {
	...
	function parseParamString( $str ) {
		$m = false;
		$res = false;
		if ( preg_match( '/^interlaced-(.*)$/', $str, $m ) ) {
			$res = parent::parseParamString( $m[1] );
			if ( !$res || count( $res ) !== count( $m[1] ) ) {
				return false;
			}
			$res['interlace'] = true;
		} else {
	...
	function normaliseParams( $image, &$params ) {
		...
		// If interlacing was requested, check the limits
		if ( $interlace && isset( $wgMaxInterlacingAreas[$mimeType] )
