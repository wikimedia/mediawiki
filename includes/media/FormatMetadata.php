			...
			foreach ( $vals as &$val ) {
				...
				case 'YResolution':
					switch ( $resolutionunit ) {
						case 2:
							$val = $this->exifMsg( 'XYResolution', 'i', self::formatNum( $val ) );
							break;
						case 3:
							$val = $this->exifMsg( 'XYResolution', 'c', self::formatNum( $val ) );
							break;
						default:
							/* If not recognized, display as is. */
							break;
				...
				case 'SubjectDistance':
					$val = $this->exifMsg( $tag, '', self::formatNum( $val ) );
					break;
...
//						'reserved' => ($val & bindec( '10000000' )) >> 7,
				...
				case 'GPSAltitude':
					if ( $val < 0 ) {
						$val = $this->exifMsg( 'GPSAltitude', 'below-sealevel', self::formatNum( -$val, 3 ) );
					} else {
						$val = $this->exifMsg( 'GPSAltitude', 'above-sealevel', self::formatNum( $val, 3 ) );
					}
					break;
				...
				case 'GPSDOP':
					...
					if ( $val <= 2 ) {
						$val = $this->exifMsg( $tag, 'excellent', self::formatNum( $val ) );
					} elseif ( $val <= 5 ) {
						$val = $this->exifMsg( $tag, 'good', self::formatNum( $val ) );
					} elseif ( $val <= 10 ) {
						$val = $this->exifMsg( $tag, 'moderate', self::formatNum( $val ) );
					} elseif ( $val <= 20 ) {
						$val = $this->exifMsg( $tag, 'fair', self::formatNum( $val ) );
					} else {
						$val = $this->exifMsg( $tag, 'poor', self::formatNum( $val ) );
					}
					break;
				...
				case 'ImageWidth':
				case 'ImageLength':
					$val = self::formatNum( $val ) . ' ' . $this->msg( 'unit-pixel' )->text();
					break;
	 ...
	 */
	public function flattenArray( $vals, $type = 'ul', $noHtml = false, $useContentLang = false ) {
		if ( !is_array( $vals ) ) {
		...
		if ( count( $vals ) === 1 && $type !== 'lang' ) {
			...
			case 'lang':
				...
				if ( !$useContentLang ) {
					...
					$priorityLanguages = $wgContLang->getFallbackLanguages();
				}
	 ...
	 */
	private function langItem( $value, $lang, $default = false, $noHtml = false, $content = false ) {
