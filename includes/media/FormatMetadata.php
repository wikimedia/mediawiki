...
//						'reserved' => ($val & bindec( '10000000' )) >> 7,
	 ...
	 *
	 * @note In order to be easily cached, this method does all parsing
	 *  and date formats, as if user is an anon.
	 *
	...
	public static function getExtendedMeta( File $file, $lang = false, $single = true, $context = false ) {
		...
		if ( $cachedValue ) {
			if ( wfRunHooks( 'ValidateExtendedMetadataCache', array( $cachedValue['timestamp'], $file ) ) ) {
				return $cachedValue['data'];
			}
		}

		$valueToCache = array( 'timestamp' => wfTimestampNow() );

		list( $cacheExpiry, $valueToCache['data'] ) = $format->fetchExtendedMetadata( $file );

		$wgMemc->set( $cacheKey, $valueToCache, $cacheExpiry );

		return $format->formatExtendedMetadata( $file, $valueToCache['data'] );
	}

	/**
	 * Format extended metadata returned from fetchExtendedMetadata.
	 *
	 * Will be called from getExtendedMeta, so use that.
	 *
	 * @note This method cannot be called while $wgParser is parsing something.
	 *
	 * @param File $file File for which we're formatting metadata
	 * @param array $metadata Metadata from fetchExtendedMetadata
	 * @return array Formatted version
	 */
	public function formatExtendedMetadata( File $file, array $metadata ) {
		global $wgParser;
		...
		foreach ( $normalValues as $val ) {
			if ( isset( $metadata[$val] ) ) {
				$metadata[$val]['value'] = htmlspecialchars( $metadata[$val]['value'] );
			}
		}

		$commonKeys = array_keys( $file->getCommonMetaArray() );
		$commonFormatted = $this->makeFormattedData( array_intersect_key( $metadata, $commonKeys ) );
		$metadata = array_merge( $metadata, $commonFormatted );
		...
		foreach ( $metadata as $key => $value ) {
			if ( !isset( $value['translatedName'] ) ) {
				...
				$metadata[$key]['translatedName'] = $translated;
			}
		}
	 ...
	 *
	 * getExtendedMetadata is probably helpful - returns usefully-formatted
	 * version.
	 *
	 * formatExtendedMetadata will do it for you, too, if you want to mess
	 * with the context somehow before you run it.
	 *
	 * @param File $file File to use
	...
	public function fetchExtendedMetadata( File $file, $language = false, $single = true ) {
		...
		if ( $file instanceof ForeignAPIFile ) {
			$maxCacheTime = 60 * 60 * 12;
			$combinedMeta = $file->getExtendedMetadata();
			if ( $combinedMeta ) {
				return array( $maxCacheTime, $combinedMeta );
			} else {
				return array();
			}
		}
