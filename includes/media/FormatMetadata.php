...
//						'reserved' => ($val & bindec( '10000000' )) >> 7,
	...
	public static function getExtendedMeta( File $file, $lang = false, $single = true, $context = false ) {
		...
		// Hacky part. We want this to not depend on the user (for easy cachability).
		// We also want to use full urls, not relative urls, since the html may be
		// used outside of our site.
		// To accomplish that, we mess with a bunch of globals in an ugly way.
		$anonUser = new User;
		$derivContext->setUser( $anonUser );

		// Because not all of core is converted to RequestContext
		$oldUser = $wgUser;
		$oldUserContext = RequestContext::getMain()->getUser();
		$wgUser = $anonUser;
		...
		$wgRequest = new DerivativeRequest(
			$oldRequest,
			array( 'action' => 'render' ) + $oldRequest->getValues(),
			$oldRequest->wasPosted()
		);

		list( $cacheExpiry, $valueToCache['data'] ) = $format->makeExtendedMeta( $file );

		$wgRequest = $oldRequest;
		$wgUser = $oldUser;
		RequestContext::getMain()->setUser( $oldUserContext );

		$wgMemc->set( $cacheKey, $valueToCache, $cacheExpiry );

		return $valueToCache['data'];
	}
	...
	public function makeExtendedMeta( File $file ) {
		...
		$uploadDate = $this->getLanguage()->timeanddate( $file->getTimestamp(), false, false );
		$combinedMeta = array(
			// This is modification time, which is close to "upload" time.
			'DateTime' => array(
				'value' => htmlspecialchars( $uploadDate ),
