...
//						'reserved' => ($val & bindec( '10000000' )) >> 7,
	 ...
	 */
	public function flattenArray( $vals, $type = 'ul', $noHtml = false ) {
		if ( isset( $vals['_type'] ) ) {
			$type = $vals['_type'];
			unset( $vals['_type'] );
		}
	...
	private function langItem( $value, $lang, $default = false, $noHtml = false ) {
		...
		if ( $lang === false ) {
			if ( $noHtml ) {
				return $this->msg( 'metadata-langitem-default',
					$wrappedValue )->text() . "\n\n";
			} /* else */
			return '<li class="mw-metadata-lang-default">'
				. $this->msg( 'metadata-langitem-default',
					$wrappedValue )->text()
				. "</li>\n";
		}
		...
		if ( $noHtml ) {
			return '*' . $this->msg( 'metadata-langitem',
		...
		$item .= '" lang="' . $lang . '">';
		$item .= $this->msg( 'metadata-langitem',
			$wrappedValue, $langName, $lang )->text();
		$item .= "</li>\n";
		return $item;
	}
