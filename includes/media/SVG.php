...
class SvgHandler extends ImageHandler {
	...
	*/
	...
	function verifyUpload( $fileName ) {
		...
		$cfg = $svgp->getConfig(); //$cfg could be cached, and purify called as a static function
...
class SVGPurifier {
	...
	protected $animation_elements, $descriptive_elements, $shape_elements, $structural_elements, $gradient_elements, $common_elements, $font_face_elements, $glyph_elements, $filter_elements, $filter_function_elements, $filter_lightsource_elements, $text_elements;

	//Groups of white-listed attributes
	protected $root_node_attributes, $conditional_processing_attributes, $core_attributes, $xlink_attributes, $presentation_attributes, $general_attributes, $dimensioning_attributes, $circle_attributes, $path_attributes, $line_attributes, $polyline_polygon_attributes, $rect_attributes, $ellipse_attributes, $animation_attribute_target_attributes, $animation_timing_attributes, $animation_value_attributes, $animation_addition_attributes, $animate_motion_attributes, $linear_gradient_attributes, $radial_gradient_attributes, $filter_primitive_attributes, $filter_attributes, $filter_lightsource_attributes, $glyph_attributes, $clip_path_attributes, $color_profile_attributes, $cursor_attributes, $font_attributes, $font_face_format_attributes, $font_face_name_attributes, $marker_attributes, $mask_attributes, $pattern_attributes, $style_attributes, $text_attributes, $view_attributes, $textpath_attributes, $kerning_attributes, $use_tag_attributes;
	...
	public function getConfig() {
		...
		$config->set( 'HTML.AllowedElements', array_merge(
			...
			$this->filter_function_elements, $this->filter_lightsource_elements, $this->text_elements
		));
	...
	function addDefinitionsCommonElements( &$def ){
		...
		$def->addElement(
		  	'image', 
			'Block', 
			'Optional: ' . join( array_merge( $this->animation_elements, $this->descriptive_elements ), ' | ' ),
			'Common', 
			array_merge(
				$this->conditional_processing_attributes,
				$this->core_attributes, 
		...
		$def->addElement(
			...
			'Block', 
			'Optional: ' . join( array_merge(
				$this->animation_elements, $this->descriptive_elements,
				$this->shape_elements, $this->structural_elements, 
				$this->gradient_elements, $this->common_elements
			), ' | ' ),
			'Common', 
			array_merge(
				$this->core_attributes, 
		...
		$def->addElement(
			...
			'Optional: ' . join( array_merge(
				$this->animation_elements, $this->descriptive_elements,
				$this->shape_elements, $this->common_elements
			), ' | ' ),
			'Common', 
			array_merge(
				$this->conditional_processing_attributes,
	...
	function addDefinitionsGlyphElements( &$def ){
		...
		$def->addElement(
			...
			'Optional: ' . join(
				array_merge(
					$this->animation_elements, $this->descriptive_elements, 
					$this->glyph_elements, $this->shape_elements, $this->common_elements,
					$this->structural_elements, $this->gradient_elements 
				), ' | '
			),
			'Common', 
			array_merge(
		...
		$def->addElement(
			...
			'Common', 
			array_merge(
				$this->core_attributes, $this->presentation_attributes, $this->xlink_attributes, 
				$this->glyph_attributes
			)
		);
		$def->addElement(
	...
	function addDefinitionsShapeElements( &$def ){
		...
		$def->addElement(
			...
			'Block', 
			'Optional: ' . join(
				array_merge( $this->animation_elements, $this->descriptive_elements ), ' | '
			),
			'Common', 
			array_merge(
				$this->core_attributes, $this->general_attributes, 
				$this->presentation_attributes, $this->circle_attributes
			)
		);
		$def->addElement(
		  	'ellipse', 
			...
			array_merge(
				$this->core_attributes, $this->general_attributes, 
				$this->presentation_attributes, $this->ellipse_attributes
			)
		);
		$def->addElement(
		  	'line', 
			...
			array_merge(
				$this->core_attributes, $this->general_attributes, 
				$this->presentation_attributes, $this->line_attributes
			)
		);
		$def->addElement(
		  	'mpath', 
			...
			'Common', 
			array_merge(
				$this->core_attributes, $this->general_attributes, $this->xlink_attributes
			)
		);
		$def->addElement(
		  	'textpath', 
			...
			'Optional: ' . join(
				array_merge(
					$this->animation_elements, $this->descriptive_elements, 
					$this->glyph_elements, $this->text_elements, array('a') 
				), ' | '
			),
			'Common', 
			array_merge(
				$this->conditional_processing_attributes, 
				$this->core_attributes, $this->general_attributes, 
				$this->presentation_attributes, $this->xlink_attributes,
		...
		$def->addElement(
			...
			array_merge(
				$this->core_attributes, $this->general_attributes, 
				$this->presentation_attributes, $this->path_attributes
			)
		);
		$def->addElement(
		  	'polygon', 
			...
			array_merge(
				$this->core_attributes, $this->general_attributes,
				$this->presentation_attributes, $this->polyline_polygon_attributes	
			)
		);
		$def->addElement(
		  	'polyline', 
		...
		$def->addElement(
			...
			array_merge(
				$this->core_attributes, $this->general_attributes,
				$this->rect_attributes, $this->dimensioning_attributes, 
				$this->presentation_attributes
			)
		);
	}
	...
	function addDefinitionsStructuralElements( &$def ){
		...
		$def->addElement(
			...
			'Common', 
			array_merge(
				$this->conditional_processing_attributes, $this->core_attributes, 
				$this->presentation_attributes, $this->general_attributes, $this->circle_attributes
			)
		);
		...
		$def->addElement(
			...
			'Block', 
			'Optional: ' . join(
				array_merge( $this->animation_elements, $this->descriptive_elements, $this->shape_elements ), ' | '
			),
			'Common', 
			array_merge(
				$this->conditional_processing_attributes, $this->core_attributes, 
	...
	function addDefinitionsGradientElements( &$def ){
		...
		$def->addElement(
		  	'lineargradient', 
			'Block', 
			'Optional: ' . join( array_merge( $this->descriptive_elements, $this->animation_elements ), ' | ' ),
			'Common', 
			array_merge(
				$this->core_attributes, 
				$this->general_attributes, 
	...
	function addDefinitionsFilterElements( &$def ){
		...
		$def->addElement(
			...
			'Block', 
			'Optional: ' . join( array_merge(
				$this->descriptive_elements, $this->filter_elements, $this->animation_elements
			), ' | ' ),
			'Common', 
			array_merge(
				$this->core_attributes, $this->presentation_attributes,
				$this->xlink_attributes, $this->filter_primitive_attributes
			)
		);
		$def->addElement(
		  	'feblend', 
			...
			'Common', 
			array_merge(
				$this->core_attributes, $this->presentation_attributes, $this->filter_attributes,
				$this->general_attributes, $this->filter_primitive_attributes
			)
		);
		$def->addElement(
		  	'fecolormatrix', 
		...
		$def->addElement(
		  	'fediffuselighting', 
			'Block', 
			'Optional: ' . join( array_merge( $this->descriptive_elements, $this->filter_lightsource_elements ), ' | ' ),
			'Common', 
			array_merge(
				$this->core_attributes, $this->presentation_attributes, $this->filter_attributes,
				$this->general_attributes, $this->filter_primitive_attributes
		...
		$def->addElement(
			...
			array_merge(
				$this->core_attributes, $this->presentation_attributes, $this->filter_attributes,
				$this->general_attributes, $this->filter_primitive_attributes, $this->xlink_attributes
			)
		);
		$def->addElement(
		  	'fefunca', 
		...
		$def->addElement(
			...
			'Common', 
			array_merge(
				$this->core_attributes, $this->filter_primitive_attributes, $this->filter_lightsource_attributes
			)
		);
		$def->addElement(
		  	'fepointlight', 
