...
class WebPHandler extends BitmapHandler {
	...
	function getMetadata( $image, $filename ) {
		try {
			$header = file_get_contents( $filename, false, null, 0, self::MINIMUM_FILESIZE );
			if ( $header === false ) {
				return self::BROKEN_FILE;
			}
			$parsedWebPData = self::decodeHeader( $header );
			if ( $parsedWebPData === false ) {
				// Not a parseable WebP file
				return self::BROKEN_FILE;
			}
		} catch ( Exception $e ) {
			// Broken file?
	 ...
	 * @return bool|array Header data array with entries 'compression', 'width' and 'height', where
