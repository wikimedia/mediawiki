...
# ------------------------------------------------------------------------------
	...
	public function tableName( $name, $format = 'quoted', $alias = false ) {
		...
		if ( $requestedTable !== false ) {
			$requestedTableName = "{$prefix}{$table}";
		} else {
			$requestedTableName = false;
		}
		if ( $format == 'quoted' && !$this->isQuotedIdentifier( $tableName ) ) {
			$tableName = $this->addIdentifierQuotes( $tableName );
			if ( $requestedTableName !== false ) {
				$requestedTableName = $this->addIdentifierQuotes( $requestedTableName );
			}
		}
		...
		if ( $alias && $requestedTableName !== false ) {
			return $tableName . ' ' . $requestedTableName;
		} else {
			return $tableName;
		}
	}
