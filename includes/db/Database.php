...
abstract class DatabaseBase implements IDatabase {
	...
	public function selectSQLText( $table, $vars, $conds = '', $fname = __METHOD__,
		...
		if ( is_array( $table ) ) {
			$from = ' FROM ' .
				$this->tableNamesWithUseIndexOrIgnoreIndexOrJOIN( $table, $useIndexes, $ignoreIndexes, $join_conds );
		} elseif ( $table != '' ) {
			if ( $table[0] == ' ' ) {
				$from = ' FROM ' . $table;
			} else {
				$from = ' FROM ' .
					$this->tableNamesWithUseIndexOrIgnoreIndexOrJOIN( [ $table ], $useIndexes, $ignoreIndexes, [] );
			}
		} else {
			$from = '';
		}
	 ...
	 * @return string
	 */
	protected function tableNamesWithUseIndexOrIgnoreIndexOrJOIN(
		...
		foreach ( $tables as $alias => $table ) {
			...
			if ( isset( $join_conds[$alias] ) ) {
				...
				if ( isset( $ignore_index[$alias] ) ) { // has IGNORE INDEX?
					$use = $this->ignoreIndexClause( implode( ',', (array)$ignore_index[$alias] ) );
					if ( $ignore != '' ) {
						$tableClause .= ' ' . $ignore;
					}
				}
