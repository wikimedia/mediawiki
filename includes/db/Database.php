...
abstract class DatabaseBase implements IDatabase {
	...
	private function updateTrxWriteQueryTime( $sql, $runtime ) {
		if ( $runtime < .500 ) {
			...
			// Usually insert(), upsert(), though could be insertSelect(). The first two are
			// fast unless bulky in size or blocked on locks. Likewise for the INSERT..SELECT,
			// unless the SELECT scans rows heavily only to find a few rows.
			$isSane = ( $this->affectedRows() > 50 );
		} elseif ( substr( $sql, 0, 8 ) === 'REPLACE ' ) {
			// Usually replace(). This is a DELETE by unique indexes plus an INSERT.
