...
class LoadMonitorMySQL implements LoadMonitor {
	...
	public function getLagTimes( $serverIndexes, $wiki ) {
		...
		$key = wfForeignMemcKey( $this->parent->getServerName( 0 ), '', 'lag_times' );
		...
		if ( $value && ['timestamp'] > ( microtime( true ) - $ttl ) ) {
	...
	public function clearCaches() {
		...
		$memcKey = wfForeignMemcKey( $masterName, '', 'lag_times' );

		$this->srvCache->delete( $memcKey );
		$this->mainCache->delete( $memcKey );
	}
}
