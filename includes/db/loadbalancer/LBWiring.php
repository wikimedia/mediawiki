...
return [
	...
	'LBFactoryCloaking' => function( ServiceContainer $container, MediaWikiServices $services ) {
		$config = $services->getMainConfig();

		$factory = new CloakingLBFactory(
			LBFactorySimple::buildServerSpecsFromConfig( $config ),
			[],
			new HashBagOStuff(),
			new ChronologyProtector( new HashBagOStuff(), [] ),
			new TransactionProfiler(),
			new NullLogger(),
			new StatsdDataFactory()
		);
