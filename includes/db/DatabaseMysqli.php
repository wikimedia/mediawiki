...
class DatabaseMysqli extends DatabaseMysqlBase {
	 ...
	 * @var int|null
	 */
	protected $savedAffectedRows = null;
	...
	protected function doQuery( $sql ) {
		global $wgLogDBWarnings;

		$conn = $this->getBindingHandle();

		// Clear any saved number of affected rows
		$this->savedAffectedRows = null;
		...
		if ( $wgLogDBWarnings && $ret !== false ) {
			// Log any warnings generated by the successful query. (We don't
			// do this for unbuffered queries, for which we would first have
			// to wait until the result set has been freed.)
			if ( ( $bufferResults || $ret === true ) && $conn->warning_count ) {
				$this->logMysqlWarnings( $sql );
			}
		}
	...
	/**
	 * Log any MySQL warnings that we are configured to log.
	 *
	 * @param string $msg Text to log along with the warnings
	 */
	protected function logMysqlWarnings( $msg ) {
		global $wgLoggableMysqlWarnings;

		// Ask for the list of warnings, but first, save the number of affected
		// rows. Returning the number of warnings rather than the number of rows
		// inserted or updated could break calling code.
		$conn = $this->getBindingHandle();
		$this->savedAffectedRows = $conn->affected_rows;
		$ret = $conn->query( 'SHOW WARNINGS' );

		$warningsToLog = array();
		while ( $ret && ( $row = $ret->fetch_row() ) ) {
			list( $level, $code, $message ) = $row;

			// Find the most specific configuration entry that applies
			// to this warning...
			if ( isset( $wgLoggableMysqlWarnings["$level $code"] ) ) {
				$isLoggable = $wgLoggableMysqlWarnings["$level $code"];
			} elseif ( isset( $wgLoggableMysqlWarnings[$code] ) ) {
				$isLoggable = $wgLoggableMysqlWarnings[$code];
			} elseif ( isset( $wgLoggableMysqlWarnings[$level] ) ) {
				$isLoggable = $wgLoggableMysqlWarnings[$level];
			} elseif ( isset( $wgLoggableMysqlWarnings['*'] ) ) {
				$isLoggable = $wgLoggableMysqlWarnings['*'];
			} else {
				$isLoggable = false;
			}

			// ... and if it says to log the warning, do so.
			if ( $isLoggable ) {
				$warningsToLog[] = "$level (Code $code): $message";
			}
		}

		if ( $warningsToLog ) {
			wfDebugLog( 'DBWarnings', "$msg\n" . implode( "\n", $warningsToLog ) );
		}
	}
	...
	function affectedRows() {
		...
		if ( $this->savedAffectedRows !== null ) {
			return $this->savedAffectedRows;
		}
