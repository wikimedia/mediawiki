...
class DatabaseMysqli extends DatabaseMysql {
	...
	function prepare( $sql, $func = 'MysqliDatabase::prepare' ) {
		$stmt = $this->mConn->stmt_init();
		$stmt->prepare( $sql );
		return $stmt;
	}
	...
	function execute( $prepared, $args = null ) {
		...
		if ( $args ) {
			$this->fillPrepared( $prepared, $args );
		}

		$prepared->execute();
		return $this->resultObject( $prepared->get_result() );
	}
	...
	function fillPrepared( $preparedQuery, $args ) {
		...
		call_user_func_array( array( $prepared, 'bind_param' ), $args );
	}
