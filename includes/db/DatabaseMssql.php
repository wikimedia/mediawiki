...
class DatabaseMssql extends DatabaseBase {
	...
	public function cascadingDeletes() {
		return false; // for now until we get all the FKs in the schema
	}
	...
	public function open( $server, $user, $password, $dbName ) {
		...
		global $wgDBport, $wgDBNTAuthentication;
		...
		// ugly, ugly!
		// @deprecated Replaced by $wgDBNTAuthentication, but this workaround kept
		// for backwards compatibility
		$ntAuthUserTest = strtolower( $user );
		$ntAuthPassTest = strtolower( $password );

		// Decide which auth scenerio to use
		if ( $wgDBNTAuthentication || ( $ntAuthPassTest == 'ntauth' && $ntAuthUserTest == 'ntauth' ) ) {
	...
	public function fetchObject( $res ) {
		...
		if ( $this->mSeekTo !== null) {
			$result = sqlsrv_fetch_object( $res, 'stdClass', array(), SQLSRV_SCROLL_ABSOLUTE, $this->mSeekTo );
			$this->mSeekTo = null;
		} else {
	...
	public function fetchRow( $res ) {
		...
		if ( $this->mSeekTo !== null ) {
			$result = sqlsrv_fetch_array( $res, SQLSRV_FETCH_BOTH, SQLSRV_SCROLL_ABSOLUTE, $this->mSeekTo );
			$this->mSeekTo = null;
		} else {
		...
		if ( isset( $options['EXPLAIN'] ) ) {
			try {
				...
				if ( isset( $options['FOR COUNT'] ) ) {
					...
					$ret = $this->select( $table, 'COUNT(*) AS EstimateRows', $conds, $fname, $options, $join_conds );
				} else {
	...
	public function limitResult( $sql, $limit, $offset = false ) {
		if ( $offset === false || $offset == 0 ) {
			...
			$sql = "SELECT {$select[1]} FROM (SELECT ROW_NUMBER() OVER({$overOrder}) AS rowNumber, * FROM ({$sql}) {$sub1}) {$sub2} WHERE rowNumber BETWEEN {$first} AND {$last}{$postOrder}";
	...
	public function addQuotes( $s ) {
		if ( $s instanceOf MssqlBlob ) {
			...
			return ( new MssqlBlob( $s->fetch() ) )->fetch();
		} else {
			if ( is_bool( $s ) ) {
				$s = $s ? 1 : 0;
			}
	 ...
	 * @since 1.23
	 */
	public function buildGroupConcatField( $delim, $table, $field, $conds = '', $join_conds = array() ) {
	...
	private function getBinaryColumns( $table ) {
		...
		return isset( $this->mBinaryColumnCache[$tableRaw] ) ? $this->mBinaryColumnCache[$tableRaw] : array();
	}
	...
	private function getBitColumns( $table ) {
		...
		return isset( $this->mBitColumnCache[$tableRaw] ) ? $this->mBitColumnCache[$tableRaw] : array();
	}
	...
	// called in the installer and updater, probably doesn't need to be called anywhere else in the codebase
