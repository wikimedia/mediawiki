...
class WatchedItem {
	...
	public static function batchAddWatch( array $items ) {
		...
		foreach ( $items as $item ) {
			...
			$rowGroupKey = $item->getExpiry() === null ? 'null': $item->getExpiry();
			$rowGroups[$rowGroupKey][] = array(
		...
		$dbw = wfGetDB( DB_MASTER );
		foreach ( $rowGroups as $rowGroupKey => $rows ) {
			foreach ( array_chunk( $rows, 100 ) as $toInsert ) {
				$dbw->upsert(
					...
					array(
						'wl_notificationtimestamp' => null,
						'wl_expirytimestamp' => $rowGroupKey === 'null' ? null: $rowGroupKey,
					),
					__METHOD__
				);
			}
