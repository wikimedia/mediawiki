...
class WatchedItem {
	...
	/** @var null|string MW_TS format or infinity */
	private $expiry;
	...
	public static function fromUserTitle(
		...
		if ( $expiry === WatchedItem::CHECK_USER_RIGHTS || $expiry === WatchedItem::IGNORE_USER_RIGHTS ) {
			// TODO log deprecated use of method signature
			$checkRights = $expiry;
			$expiry = null;
		}
	...
	/**
	 * @return null|string MW_TS format or infinity
	 */
	...
	private function load() {
		...
		if ( $row === false ) {
			...
			$this->expiry = $dbr->decodeExpiry( $row->wl_expirytimestamp );
		}
	}
	...
	public function isWatched() {
		...
		return $this->watched && !$this->isExpired();
	}

	/**
	 * @return bool has this item expired?
	 */
	private function isExpired() {
		if ( $this->expiry == '' || $this->expiry == 'infinity' ) {
			return false;
		}
		return wfTimestamp( TS_MW, $this->expiry ) < wfTimestamp( TS_MW );
	}
	...
	public static function batchAddWatch( array $items ) {
		...
		foreach ( $items as $item ) {
			...
			$rows[] = array(
				...
				'wl_expirytimestamp' => $dbw->encodeExpiry( $item->getExpiry() ),
			);
		...
		foreach ( array_chunk( $rows, 100 ) as $toInsert ) {
			if ( $dbw->getType() === 'mysql' ) {
				// MySQL supports VALUES() in ON DUPLICATE KEY UPDATE
				$dbw->upsert(
					'watchlist',
					$toInsert,
				...
				// For all other types of db we fall back to individual queries...
				// Try to update any existing rows, then add any that did not exist
				foreach ( $toInsert as $insertRow ) {
					$dbw->update(
						'watchlist',
						array(
							'wl_expirytimestamp' => $insertRow['wl_expirytimestamp'],
						),
						array(
							'wl_user' => $insertRow['wl_user'],
							'wl_namespace' => $insertRow['wl_namespace'],
							'wl_title' => $insertRow['wl_title'],
						),
						__METHOD__
					);
				}
		...
		// Flush old entries from the `watchlist` table
		if ( mt_rand( 0, 9 ) == 0 ) {
			JobQueueGroup::singleton()->lazyPush( WatchlistUpdateJob::newPurgeJob() );
		}
	...
	private static function doDuplicateEntries( $ot, $nt ) {
		...
		foreach ( $res as $s ) {
			$values[] = array(
				...
				'wl_expirytimestamp' => $dbw->encodeExpiry( $s->wl_expirytimestamp ),
			);
		}
