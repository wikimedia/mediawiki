...
class WatchedItem {
	 ...
	 *
	 * @param Title $oldTitle Page title to duplicate entries from, if present
	 * @param Title $newTitle Page title to add watches on
	 *
	...
	public static function duplicateEntries( $oldTitle, $newTitle ) {
		static $store;
		if ( !$store ) {
			$store = new WatchedItemStore( wfGetLB() );
		}
		$store->duplicateEntries( $oldTitle, $newTitle );
	}
