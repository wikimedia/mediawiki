...
class InfoAction extends FormlessAction {
	...
	public function onView() {
		...
		$table = '';
		foreach ( $pageInfo as $infoRow )
		{
			// One element in an array is a header
			if ( count( $infoRow ) == 1 )
			{
				// End previous table
				if ( !empty( $table ) )
				...
				$header = ( is_a( $infoRow[0], 'Message' ) ) ? $infoRow[0]->escaped() : $infoRow[0];
				$content .= $this->makeHeader( $header );
			}
			// Two elements in an array is a table row
			else
			{
				$name = ( is_a( $infoRow[0], 'Message' ) ) ? $infoRow[0]->escaped() : $infoRow[0];
				$value = ( is_a( $infoRow[1], 'Message' ) ) ? $infoRow[1]->escaped() : $infoRow[1];
	...
	protected function pageInfo( $title )
	{
		...
		// Basic information
		$pageInfo = array();
		$pageInfo['header-basic'] = array( $this->msg( 'pageinfo-header-basic' ) );
		...
		$pageInfo['display-title'] = array( $this->msg( 'pageinfo-display-title' ), $displayTitle );
		...
		$pageInfo['default-sort'] = array( $this->msg( 'pageinfo-default-sort' ), $sortKey );

		// Page length (in bytes)
		$pageInfo['length'] = array(
			$this->msg( 'pageinfo-length' ), $lang->formatNum( $title->getLength() )
		);

		// Page ID (number not localised, as it's a database ID.)
		$pageInfo['article-id'] = array( $this->msg( 'pageinfo-article-id' ), $id );
		...
		// Page protection
		$pageInfo['header-restrictions'] = array( $this->msg( 'pageinfo-header-restrictions' ) );
		...
		foreach ( $title->getRestrictionTypes() as $restrictionType ) {
			...
			$pageInfo["restriction-$restrictionType"] = array(
				$this->msg( "restriction-$restrictionType" ), $message
			);
		}

		// Edit history
		$pageInfo['header-edits'] = array( $this->msg( 'pageinfo-header-edits' ) );

		$firstRev = $this->page->getOldestRevision();

		// Page creator
		$pageInfo['firstuser'] = array(
			$this->msg( 'pageinfo-firstuser' ),
			Linker::userLink(
				$firstRev->getUser( Revision::FOR_THIS_USER, $user ),
				$firstRev->getUserText( Revision::FOR_THIS_USER, $user )
			)
		);

		// Date of page creation
		$pageInfo['firsttime'] = array(
		...
		// Latest editor
		$pageInfo['lastuser'] = array(
			$this->msg( 'pageinfo-lastuser' ),
			Linker::userLink(
				$this->page->getUser( Revision::FOR_THIS_USER, $user ),
				$this->page->getUserText( Revision::FOR_THIS_USER, $user )
			)
		);

		// Date of latest edit
		$pageInfo['lasttime'] = array(
		...
		if ( count( $listItems ) > 0
			...
			// Page properties
			$pageInfo['header-properties'] = array( $this->msg( 'pageinfo-header-properties' ) );

			// Magic words
			if ( count( $listItems ) > 0 ) {
				$pageInfo['magic-words'] = array(
			...
			if ( count( $hiddenCategories ) > 0 ) {
				$pageInfo['hidden-categories'] = array(
