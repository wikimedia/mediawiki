...
class InfoAction extends FormlessAction {
	...
	protected function pageInfo() {
		...
		if ( MWNamespace::hasSubpages( $title->getNamespace() ) ) {
			...
			$pageInfo['header-basic'][] = [
				$linkRenderer->makeLink(
					$prefixIndex, $this->msg( 'pageinfo-subpages-name' )->escaped()
				),
		...
		if ( $title->isCascadeProtected() ) {
			...
			foreach ( $sources as $sourceTitle ) {
				$cascadingFrom .= Html::rawElement(
					'li', [], $linkRenderer->makeLink( $sourceTitle ) );
			}
	...
	protected function getContributors() {
		...
		foreach ( $contributors as $user ) {
			...
			if ( $user->getId() == 0 ) {
				$anon_ips[] = $linkRenderer->makeLink( $page, htmlspecialchars( $user->getName() ) );
			} elseif ( !in_array( 'realname', $hiddenPrefs ) && $user->getRealName() ) {
				$real_names[] = $linkRenderer->makeLink( $page, htmlspecialchars( $user->getRealName() ) );
			} else {
				$user_names[] = $linkRenderer->makeLink( $page, htmlspecialchars( $user->getName() ) );
			}
		}
