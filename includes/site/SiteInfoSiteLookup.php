...
class SiteInfoSiteLookup implements SiteLookup {
	...
	public function getSite( $globalId ) {
		$unaliased = $this->siteInfoLookup->resolveLocalId( SiteInfoLookup::ALIAS_ID, $globalId );
	...
	private function newSite( $globalId ) {
		...
		$site->setForward( $info[SiteInfoLookup::SITE_IS_LOCAL] );
		...
		$articlePath = $this->composeUrl( $baseUrl, $info[SiteInfoLookup::SITE_LINK_PATH] );
		$scriptPath = $this->composeUrl( $baseUrl, $info[SiteInfoLookup::SITE_SCRIPT_PATH], '$1' );
		...
		if ( isset( $idMap[SiteInfoLookup::NAVIGATION_ID] ) ) {
			foreach ( $idMap[SiteInfoLookup::NAVIGATION_ID] as $id ) {
