...
class SiteExporter {
	...
	public static function newfromFileName( $file, $mode = 'w' ) {
		$handle = fopen( $file, $mode );

		if ( !$handle )  {
			throw new MWException( "Failed to open $file for writing!" );
		}

		return new SiteExporter( $handle );
	}
	...
	private function exportSite( Site $site ) {
		fwrite( $this->sink, "\t" . XML::openElement( 'site', array( 'type' => $site->getType() ) ) . "\n" );
