...
class Uri {
	 ...
	 */
	protected static $validComponents = array( 'scheme', 'host', 'port', 'user', 'pass', 'path', 'query', 'fragment' );
	 ...
	 */
	protected static $unescapes = array( ';' => '%3B', '@' => '%40', '$' => '%24', '!' => '%21', '*' => '%2A', '(' => '%28', ')' => '%29', ',' => '%2C', '/' => '%2F' );
	 ...
	 * Similar to urlencode(), but does not escape as much.
	 *
	...
	public function extendQuery( $parameters ) {
		...
		if( $query instanceof UriQuery ) {
			...
			if( is_array( $parameters ) ) {
				$parameters = new UriFormQuery( $parameters );
			}
			$query .= "&$parameters";
		}
	...
	public function getUserInfo() {
		$user = self::encode( $this->getComponent( 'user' ) );
		$pass = self::encode( $this->getComponent( 'pass' ) );
	...
	public function getHostPort() {
		$host = self::encode( $this->getComponent( 'host' ) );
		$port = self::encode( $this->getComponent( 'port' ) );
	...
	public function toString() {
		...
		if( $authority && !in_array( substr( $path, 0, 1 ), array( false, '?', '#', '/' ) ) ) {
			// Path is a relative path. Cannot have authority.
			$url = $path;
		} else {
			$url .= $path;
		}
...
/**
 * The URI query format used in HTML forms (www-x-urlencoded).
 * Ex: a=b&d=c
 *
 * @since 1.20
 * @author Tyler Romeo
 */
class UriFormQuery extends UriQuery {
	private $elements;
...
/**
 * Raw query format. It is stored internally as a string,
 * and any adding/getting/extending is done using basic
 * string operations.
 *
 * @since 1.20
 * @author Tyler Romeo
 */
class UriRawQuery extends UriQuery {
	private $query;
