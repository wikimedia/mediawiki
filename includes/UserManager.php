...
class UserManager {
	...
	/**
	 * There are two users involved here: $user is the user who is being
	 * managed, while $context->getUser() holds the user doing the managing.
	 *
	 ...
	 *
	 * $context->getUser() is the user performing the reset, which may not be
	 * the same as the user whose password is going to be reset.
	 *
	 * @param IContextSource $context
	 * @return Status
	 */
	 ...
	 * $wgPasswordResetRoutes, and is used to determine the user whose password
	 * is to be reset. $context->getUser() is the user doing the resetting.
	 *
	 * Return values are:
	...
	public static function resetPassword( IContextSource $context, array $data, $capture = false ) {
		...
		$check = self::canResetPassword( $context );
		if ( !$check->isGood() ) {
			$status->merge( $check );
			return $status;
		}
