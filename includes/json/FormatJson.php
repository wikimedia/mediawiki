...
class FormatJson {
	...
	private static function encode54( $value, $pretty, $escaping ) {
		...
		if ( $pretty !== false ) {
			...
			$json = self::replaceIndentString( $json, $pretty );
		}
	...
	/**
	 * Replace PHP's four-space indent with a given indent string.
	 *
	 * @param string $json
	 * @param string $indentString
	 * @return string
	 */
	private static function replaceIndentString( $json, $indentString ) {
		if ( $indentString === '    ' ) {
			return $json;
		}
		...
		return str_replace( "\t", $indentString, $json );
	}
