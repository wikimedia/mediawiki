...
class MWExceptionHandler {
	...
	public static function jsonSerializeException( Exception $e, $pretty = false, $escaping = 0 ) {
		...
		if ( $wgLogExceptionBacktrace ) {
			...
			// Elements in the 'args' array may not be serializable. Replace
			// each element with a string naming its class (if the element is
			// an object) or type (if primitive).
			foreach( $backtrace as &$frame ) {
				if ( isset( $frame['args'] ) ) {
					foreach( $frame['args'] as &$arg ) {
						$arg = is_object( $arg ) ? get_class( $arg ) : gettype( $arg );
					}
				}
			}
			$exception_data['backtrace'] = $backtrace;
		}
