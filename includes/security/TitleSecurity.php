...
class TitleSecurity {
	...
	public function userCan( $action, $user = null, $doExpensiveQueries = true ) {
		...
		return !count( self::getUserPermissionsErrorsInternals(
	...
	public function getUserPermissionsErrors( $action, $user, $doExpensiveQueries = true,
		...
		$errors = self::getUserPermissionsErrorsInternal( $action, $user, $doExpensiveQueries );
	...
	protected function checkQuickPermissions( $action, $user, $errors,
		...
		if ( !wfRunHooks( 'TitleQuickPermissions',
			array( $this->title, $user, $action, &$errors, $doExpensiveQueries, $short ) )
		) {
			return $errors;
		}
		...
		if ( $action == 'create' ) {
			...
			$errors[] = self::missingPermissionError( $action, $short );
		}
	 ...
	 * @return array List of errors
	 */
	protected function resultToError( $errors, $result ) {
	 ...
	 * @return array List of errors
	 */
	protected function checkPermissionHooks( $action, $user, $errors, $doExpensiveQueries, $short ) {
	...
	protected function checkSpecialsAndNSPermissions( $action, $user, $errors,
		...
		if ( self::isNamespaceProtected( $user ) ) {
	 ...
	 * @return array List of errors
	 */
	protected function checkCSSandJSPermissions( $action, $user, $errors, $doExpensiveQueries, $short ) {
		# Protect css/js subpages of user pages
		# XXX: this might be better using restrictions
		# XXX: right 'editusercssjs' is deprecated, for backward compatibility only
		if ( $action != 'patrol' && !$user->isAllowed( 'editusercssjs' ) ) {
			if ( preg_match( '/^' . preg_quote( $user->getName(), '/' ) . '\//', $this->title->getText() ) ) {
				if ( $this->title->isCssSubpage() && !$user->isAllowedAny( 'editmyusercss', 'editusercss' ) ) {
					$errors[] = array( 'mycustomcssprotected', $action );
				} elseif ( $this->title->isJsSubpage() && !$user->isAllowedAny( 'editmyuserjs', 'edituserjs' ) ) {
	 ...
	 * @return array List of errors
	 */
	protected function checkPageRestrictions( $action, $user, $errors, $doExpensiveQueries, $short ) {
	...
	protected function checkCascadingSourcesRestrictions( $action, $user, $errors,
		...
		if ( $doExpensiveQueries && !$this->title->isCssJsSubpage() ) {
			...
			# protection anyway.
			list( $cascadingSources, $restrictions ) = self::getCascadeProtectionSources();
	...
	protected function checkActionPermissions( $action, $user, $errors,
		...
		if ( $action == 'protect' ) {
			if ( count( self::getUserPermissionsErrorsInternal( 'edit',
			...
			$title_protection = self::getTitleProtection();
			if ( $title_protection ) {
			...
			$tempErrors = self::checkPageRestrictions( 'edit',
				$user, array(), $doExpensiveQueries, true );
			if ( !$tempErrors ) {
				$tempErrors = self::checkCascadingSourcesRestrictions( 'edit',
			...
			if ( $doExpensiveQueries && $wgDeleteRevisionsLimit
				&& !self::userCan( 'bigdelete', $user ) && $this->title->isBigDeletion()
			) {
	 ...
	 * @return array List of errors
	 */
	protected function checkUserBlock( $action, $user, $errors, $doExpensiveQueries, $short ) {
		...
		if ( ( $action == 'edit' || $action == 'create' ) && !$user->isBlockedFrom( $this->title ) ) {
	 ...
	 * @return array List of errors
	 */
	protected function checkReadPermissions( $action, $user, $errors, $doExpensiveQueries, $short ) {
		...
		if ( !$whitelisted ) {
			...
			wfRunHooks( 'TitleReadWhitelist', array( $this->title, $user, &$whitelisted ) );
			if ( !$whitelisted ) {
				$errors[] = self::missingPermissionError( $action, $short );
			}
		}
	...
	public function getRestrictionTypes() {
		...
		wfRunHooks( 'TitleGetRestrictionTypes', array( $title, &$types ) );
	 ...
	 */
	protected function deleteTitleProtection() {
