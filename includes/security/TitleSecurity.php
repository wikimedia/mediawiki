...
class TitleSecurity {
	...
	protected function checkPermissionHooks( $action, $user, $errors, $doExpensiveQueries, $short ) {
		...
		if ( !Hooks::run( 'getUserPermissionsErrors', array( &$this->title, &$user, $action, &$result ) ) ) {
		...
		if (
			...
			&& !Hooks::run( 'getUserPermissionsErrorsExpensive', array( &$this->title, &$user, $action, &$result ) )
		) {
			$errors = self::resultToError( $errors, $result );
		}
