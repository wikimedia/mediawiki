...
class TitleSecurity {
	 ...
	 * @return bool
	 */
	public function quickUserCan( $action, $user ) {
	 ...
	 * @return bool
	 */
	public function userCan( $action, $user, $doExpensiveQueries = true ) {
	 ...
	 * @return array List of errors
	 */
	static protected function resultToError( $errors, $result ) {
	...
	protected function checkPermissionHooks( $action, $user, $errors, $doExpensiveQueries, $short ) {
		...
		$result = '';
		if ( !wfRunHooks( 'userCan', array( &$this->title, &$user, $action, &$result ) ) ) {
			return $result ? array() : array( array( 'badaccess-group0' ) );
		}
		// Check getUserPermissionsErrors hook
		if ( !wfRunHooks( 'getUserPermissionsErrors', array( &$this->title, &$user, $action, &$result ) ) ) {
		...
		if (
			...
			&& !wfRunHooks( 'getUserPermissionsErrorsExpensive', array( &$this->title, &$user, $action, &$result ) )
		) {
			$errors = self::resultToError( $errors, $result );
		}
			...
			if ( $this->title->exists() ) {
				...
				if ( $title_protection ) {
					...
					if ( !$expiry || $expiry > $now ) {
						...
						$this->mRestrictions['create'] = explode( ',', trim( $title_protection['create_perm'] ) );
