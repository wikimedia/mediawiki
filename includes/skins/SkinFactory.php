...
class SkinFactory {
	...
	/**
	 * Main entry point to get a Skin instance for usage
	 *
	 * @param string $name Skin name you want
	 * @param string $fallback Fallback skin if it isn't available
	 * @return Skin
	 */
	public function getSkin( $name, $fallback ) {
		try {
			return $this->makeSkin( $name );
		} catch ( SkinException $e ) {
			return $this->makeSkin( $fallback );
		}
	}
	...
	public function getSkinNames() {
		return $this->displayNames + $this->getLegacySkinNames();
	}
	 ...
	 * @return Skin
	 */
	private function makeSkin( $name ) {
