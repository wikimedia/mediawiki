...
class MultiHttpClient {
	...
	public function __construct( array $options ) {
		...
		# connect to SSL Websites. It is the first 3 letters of the PHP_OS variable.
		if ( in_array( ( strtolower( substr( PHP_OS, 0, 3 ) ) ), array( "win", "dar" ) ) ) {
			if ( isset( $options['caBundlePath'] ) ) {
				$this->caBundlePath = $options['caBundlePath'];
			} else {
				$this->caBundlePath = dirname( __FILE__ ) . "/share/cacert.pem";
			}
			if ( !file_exists( $this->caBundlePath ) ) {
				throw new Exception( "Could not use CA bundle: " . $this->caBundlePath );
			}
		}
	...
	protected function getCurlHandle( array &$req ) {
		...
		if ( isset( $req['stream'] ) ) {
			curl_setopt( $ch, CURLOPT_FILE, $req['stream'] );
		} else {
