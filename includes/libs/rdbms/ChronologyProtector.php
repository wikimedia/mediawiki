...
class ChronologyProtector implements LoggerAwareInterface {
	...
	protected $clientId;
	/** @var float|integer Expected minimum index of the last write to the position store */
	 ...
	 * @param int $posIndex Write counter index
	 * @since 1.27
	 */
	public function __construct( BagOStuff $store, array $client, $posIndex = 0 ) {
		$this->store = $store;
		...
		if ( $this->wait ) {
			...
			if ( $this->waitForPosIndex > 0 ) {
				...
				if ( $result == $loop::CONDITION_REACHED ) {
					$msg = "expected and found pos time {$this->waitForPosIndex} ({$waitedMs}ms)";
					$this->logger->debug( $msg );
				} else {
					$msg = "expected but missed pos time {$this->waitForPosIndex} ({$waitedMs}ms)";
