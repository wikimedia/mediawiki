...
class LoadBalancer implements ILoadBalancer {
	...
	const ROUND_CALLBACKS = 'callbacks';
	...
	public function commitMasterChanges( $fname = __METHOD__ ) {
		...
		if ( $restore ) {
			...
			$this->forEachOpenMasterConnection( function ( Database $conn ) {
				...
				// After the COMMIT for the finalized writes, callbacks can now run
				$conn->setTrxEndCallbackSuppression( false );
			} );
		}
		$this->trxRoundStage = self::ROUND_CALLBACKS;
	}

	public function runMasterPostTrxCallbacks( $type ) {
		$this->assertTransactionRoundStage( self::ROUND_CALLBACKS );
		...
		do {
			...
			$this->forEachOpenMasterConnection( function ( Database $conn ) use ( &$e ) {
				...
				// Commit any writes without adding dangling transactions due to callbacks
				$conn->setTrxEndCallbackSuppression( true );
				try {
	...
	public function rollbackMasterChanges( $fname = __METHOD__ ) {
		...
		if ( $restore ) {
			...
			$this->forEachOpenMasterConnection( function ( Database $conn ) {
				$this->undoTransactionRoundFlags( $conn );
				// After the COMMIT for the main writes, callbacks can now run
				$conn->setTrxEndCallbackSuppression( false );
			} );
		}
		$this->trxRoundStage = self::ROUND_CALLBACKS;
	...
	private function assertTransactionRoundStage( $stage ) {
		if ( $this->trxRoundStage !== $stage ) {
			$actualStage = $this->trxRoundStage;

			throw new DBTransactionError(
				null,
				"Transaction round stage must be '$stage' (not '$actualStage')"
			);
		}
	}
