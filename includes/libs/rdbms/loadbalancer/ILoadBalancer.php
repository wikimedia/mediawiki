 ...
 *
 * A "cluster" is considered to be one master dataase and zero or more replica databases.
 * Typically, the replica DB replicate from the master asynchronously. The first node in the
 ...
 * when all the DBs are non-replicating clones of each other holding read-only data. This, the
 * role of "master" is some cases might merely be nominal.
 *
 ...
 * hold accross separate queries in the DB transaction since the data appears as a consistent
 * point-in-time snapshot.
 *
 * The typical caller will use LoadBalancer::getConnection( DB_* ) to yeild a live database
 ...
 *      lagged by this much or more. If all DBs are this lagged, then the load balancer consider
 ...
 *     the load balancer ignore whatever it things the lag of the logical replica is (which would
 *     probably just randomly bounce around).
 *
 ...
 * the (logical) replica server entry. The proxy could make this "replica" DB to any number of
 * physical replica DBs.
 *
 * @since 1.28
 * @ingroup Database
 */
