...
/**
 * Abstract base class for implementations of IDatabase that proxy all (or most)
...
abstract class DBConnectionProxy implements IDatabase {
	...
	/**
	 * @var callable|null An instantiator callback returning an IDatabase object
	 */
	private $connectionInstantiator;

	/**
	 * @param IDatabase|callable $conn Database handle, or a callback returning a database handle.
	 */
	public function __construct( $conn ) {
		if ( $conn instanceof IDatabase ) {
			$this->connection = $conn; // live handle
		} elseif ( is_callable( $conn ) ) {
			$this->connectionInstantiator = $conn; // instantiator callback
		} else {
			throw new InvalidArgumentException(
				'$conn must be an IDatabase instance of a callable instantiator'
	...
	function __call( $name, array $arguments ) {
		if ( $this->connection === null ) {
			$connection = call_user_func( $this->connectionInstantiator );

			Assert::postcondition(
				$connection instanceof IDatabase,
				'Instantiator is expected to return an IDatabase instance'
			);
	...
	public function __toString() {
		return get_class( $this ) . '( '. $this->__call( __FUNCTION__, func_get_args() ) . ' )';
	}
