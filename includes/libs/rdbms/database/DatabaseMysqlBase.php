...
abstract class DatabaseMysqlBase extends Database {
	...
	public function getReplicaPos() {
		...
		if ( $this->useGTIDs() ) {
			...
			// MariaDB uses gtid_slave_pos while MySQL uses MariaDB uses gtid_executed
	...
	public function getMasterPos() {
		...
		if ( $this->useGTIDs() ) {
			// Try to use GTIDs, fallbacking to binlog positions if not possible
			$data = $this->getServerGTIDs( __METHOD__ );
			// MariaDB uses gtid_binlog_pos while MySQL uses MariaDB uses gtid_executed
		...
		if ( $data && strlen( $data['File'] ) ) {
			return new MySQLMasterPos( "{$data['File']}/{$data['Position']}", $now );
		}
	...
	protected function getServerGTIDs( $fname = __METHOD__ ) {
		...
		$res = $this->query( "SHOW GLOBAL VARIABLES LIKE '%gtid%'", $fname );
		foreach ( $res as $row ) {
