...
interface IMaintainableDatabase extends IDatabase {
	 ...
	 * method. If locks scoped to the session are not supported, then caller must decide whether:
	 *   - a) To start a new transaction and acquire table locks for the scope of that transaction,
	 *        doing all row updates within that transaction
	 ...
	 * locks are acquired, as that breaks in case when a transaction is needed. The startAtomic()
