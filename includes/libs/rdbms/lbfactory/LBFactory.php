...
abstract class LBFactory implements ILBFactory {
	...
	/**
	 * @param string $value Possibly "<index>" or "<index>@<timestamp>"
	 * @param int $nowUnix Current UNIX timestamp
	 * @param int $ttl The TTL of $value
	 * @return int|null Write index or null if $value is empty or expired
	 * @since 1.32
	 */
	public static function getCPIndexFromCookieValue( $value, $nowUnix, $ttl ) {
		if ( !preg_match( '/^(\d+)(?:@(\d+))?$/', $value, $m, PREG_UNMATCHED_AS_NULL ) ) {
			return null;
		}

		$index = (int)$m[1];

		if ( isset( $m[2] ) && ( $nowUnix - $m[2] ) > $ttl ) {
			return null; // expired
		}
