...
class MemoizedCallable {
	 ...
	 * @param callable $callable Function or method to memoize. Should be idempotent /
	...
	public function invoke() {
		...
		foreach ( $args as $arg ) {
			if ( $arg !== null && !is_scalar( $arg ) ) {
				throw new InvalidArgumentException( 'MemoizedCallable::invoke() '
					'called with non-scalar argument' );
			}
		}
