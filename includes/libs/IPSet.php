...
class IPSet {
	...
	public function __construct( array $cfg ) {
		...
		if ( $resetNesting ) {
			ini_set( 'xdebug.max_nesting_level', $oldNesting );
		}
	}
	...
	private static function recOptimize( &$node ) {
		if ( is_array( $node[0] ) && self::recOptimize( $node[0] ) ) {
			$node[0] = 1;
		}
		if ( is_array( $node[1] ) && self::recOptimize( $node[1] ) ) {
			$node[1] = 1;
		}
	...
	private static function recCompress( &$node, $curBit, $maxCompStart ) {
		if ( !( $curBit & 7 ) ) { # byte boundary, check for depth-8 single path(s)
			...
			if ( $success ) {
				...
				$curBit += 8;
				self::recCompress( $node['next'], $curBit, $maxCompStart );
				return;
			}
		}
