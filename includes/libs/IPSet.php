...
#  a limit of 100...
if(ini_get('xdebug.max_nesting_level') < 130) {
...
class IPSet {
	...
	private function add_cidr( $cidr ) {
		...
		if(strpos($cidr, '/') === false) {
			...
			if(!preg_match('/^[0-9]+$/', $mask)) {
				trigger_error( "IPSet: Invalid netmask '$mask' from '$cidr', ignored", E_USER_WARNING );
				return;
			}
		}
		...
		$raw = inet_pton($net);
		if($raw === false) {
			trigger_error( "IPSet: inet_pton() failed to parse '$net', ignored", E_USER_WARNING );
			return;
		}
	...
	public function match( $ip ) {
		$raw = inet_pton($ip);
		if($raw === false) {
			trigger_error( "IPSet: Invalid match input address '$ip'", E_USER_WARNING );
			return false;
		}
		...
		if(!($cur_bitdepth & 7)) { # byte boundary, check for depth-8 single path(s)
			...
			while(1) {
				...
				for($i = 0; $i < 8; $i++) {
					...
					elseif($cnode[1] == 0 && ($cnode[0] == 1 || is_array($cnode[0]))) { # 0-bit
						$cnode =& $cnode[0];
					}
