...
class Uri
{
	/** @var null|string The full string URI */
	protected $uri = null;

	/** @var string[]|null|bool Parsed components of the URI, null if not loaded, false for invalid */
	protected $components = null;

	/** @var string[] Valid URI components. */
	...
	/** @var string[] Aliases for component names. */
	protected static $componentAliases = array(
		'protocol' => 'scheme',
		'password' => 'pass'
	);

	/** @var string[] Characters to unescape (urlencode does too much). */
	...
	/** @var int[] Mapping of scheme to the default port */
	 ...
	 * 2) Handles protocols that don't use :// (e.g., mailto: and news: ,
     *    as well as protocol-relative URLs) correctly
	 *
	...
	public function setComponents( array $components, $blank = true, $override = false ) {
		...
		// Remove the port if it is the default value
		if ( isset( $this->components['scheme'] ) && isset ( $this->components['port'] )
			&& self::$defaultPorts[$this->components['scheme']] === $this->components['port']
		) {
	...
	public function extendQuery( $parameters ) {
		...
		if ( !$query ) {
			$query = $parameters;
		} elseif ( $query instanceof UriQuery ) {
	...
	public function getUserInfo() {
		...
		$pass = urlencode( $this->getPassword() );
	...
	public function getHostPort() {
		$host = urlencode( $this->getHost() );
		$port = urlencode( $this->getPort() );

		// Omit port if it is the default, as per RFC 3986 3.2.3
		return $port && $port !== self::$defaultPorts[$this->getScheme()]
			? "$host:$port"
			: $host;
	}
