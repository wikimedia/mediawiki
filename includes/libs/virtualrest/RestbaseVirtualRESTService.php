...
class RestbaseVirtualRESTService extends VirtualRESTService {
	...
	public function onParsoid3Request( array $req, Closure $idGeneratorFunc ) {
		...
		if ( $action === 'page' && $req['method'] === 'GET' ) {
			$title = isset( $parts[4] ) ? $parts[4] : null;
			$revision = isset( $parts[5] ) ? $parts[5] : null;
			$req['url'] .= 'page/' . $format . '/' . $title;
			if ( $revision !== null ) {
				$req['url'] .= '/' . $revision;
			}
		} elseif ( $action === 'transform' && $req['method'] === 'POST' ) {
			$req['url'] .= 'transform/' . $parts[3] . '/to/' . $parts[5];
			$title = isset( $parts[6] ) ? $parts[6] : null;
			$revision = isset( $parts[7] ) ? $parts[7] : null;
			if ( $title !== null ) {
				$req['url'] .= '/' . $title;
				if ( $revision !== null ) {
					$req['url'] .= '/' . $revision;
