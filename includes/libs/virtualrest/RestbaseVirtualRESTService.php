...
class RestbaseVirtualRESTService extends VirtualRESTService {
	...
	public function __construct( array $params ) {
		...
		// Ensure the correct url format (trailing slash).
	...
	/**
	 * Remaps Parsoid v1/v2 requests to Restbase paths
	 */
	public function onParsoidRequests( array $reqs, Closure $idGeneratorFunc ) {
		...
		foreach ( $reqs as $key => $req ) {
			...
			if ( $parts[0] === 'v2' ) {
				$result[$key] = onParsoid2Request( $req, $idGeneratorFunc );
			} elseif ( $parts[1] === 'v1' ) {
				$result[$key] = onParsoid1Request( $req, $idGeneratorFunc );
			} else {
	...
	/**
	 * Remap a Parsoid v2 request to a Restbase path
	 * Example requests:
