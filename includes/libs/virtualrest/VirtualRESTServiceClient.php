	...
	public function runMulti( array $reqs ) {
		...
		do {
			...
			foreach ( $replaceReqsByService as $prefix => $servReqs ) {
				...
				foreach ( $service->replaceOutgoing( $servReqs, $idFunc ) as $index => $req ) {
					// Services use unique IDs for replacement requests
					if ( isset( $servReqs[$index] ) || isset( $reqs[$index] ) ) {
			...
			foreach ( $replaceReqsByService as $prefix => $servReqs ) {
				...
				foreach ( $service->replaceIncoming( $servReqs, $idFunc ) as $index => $req ) {
					// Services use unique IDs for replacement requests
					if ( isset( $servReqs[$index] ) || isset( $reqs[$index] ) ) {
						// A current or original request which was not modified
					} else {
		...
		foreach ( $reqs as $origIndex => &$req ) {
			...
			// Don't rely on MultiHttpClient for this as the service might set "response"
			$req['headers'] = array();
