...
class SwiftVirtualRESTService extends VirtualRESTService {
	...
	public function replaceOutgoing( array $reqs, Closure $idGeneratorFunc ) {
		$result = array();
		$firstReq = reset( $reqs );
	...
	public function replaceIncoming( array $reqs, Closure $idGeneratorFunc ) {
		$firstReq = reset( $reqs );
		if ( $firstReq && count( $reqs ) == 1 && isset( $firstReq['isAuth'] ) ) {
			$result = array();
			// This was an authentication request for work requests...
			if ( $this->applyAuthResponse( $firstReq ) ) {
				...
				$result = $this->replaceOutgoing( $firstReq['chain'], $idGeneratorFunc );
			} else {
