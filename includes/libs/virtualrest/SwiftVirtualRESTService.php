...
class SwiftVirtualRESTService extends VirtualRESTService {
	...
	public function digestRequests( array $reqs, Closure $idGeneratorFunc ) {
		$result = array();
		$firstReq = reset( $reqs );
	...
	public function digestResponses( array $reqs, Closure $idGeneratorFunc ) {
		$firstReq = reset( $reqs );
		if ( $firstReq && count( $reqs ) == 1 && isset( $firstReq['isAuth'] ) ) {
			$result = array();
			// This was an authentication request for work requests...
			if ( $this->applyAuthResponse( $firstReq ) ) {
				...
				$result = $this->digestRequests( $firstReq['chain'], $idGeneratorFunc );
			} else {
