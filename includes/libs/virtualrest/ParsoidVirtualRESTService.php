...
class ParsoidVirtualRESTService extends VirtualRESTService {
	...
	/**
	 * Remap a Parsoid v1 request to a Parsoid v2 request.
	 *
	...
	public function onResponses( array $reqs, Closure $idGeneratorFunc ) {
		...
		foreach ( $reqs as $key => $req ) {
			if ( isset( $req['FIXUPv1'] ) && $req['response']['code'] === 200 ) {
				...
				$body = json_decode( $req['response']['body'], true );
				if ( $body->isGood() ) {
					$resp = $body->getValue();
					$req['response']['headers']['content-type'] = $resp['wikitext']['headers']['content-type'];
					$req['response']['body'] = $resp['wikitext']['body'];
				} else {
