...
class WaitConditionLoop {
	...
	public function invoke() {
		...
		do {
			...
			$callbackUsesInterrupts = ( $real > 0.100 && $cpu <= $real * .03 );
			if ( $this->popAndRunBusyCallback() && !$callbackUsesInterrupts ) {
	 ...
	 *
	 * @return bool Whether a callback was run
	 */
	private function popAndRunBusyCallback() {
		if ( $this->busyCallbacks ) {
			/** @var callable $workCallback */
			$workCallback = array_pop( $this->busyCallbacks ); // consume
			$workCallback();
