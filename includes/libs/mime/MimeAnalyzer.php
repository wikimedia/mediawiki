...
class MimeAnalyzer implements LoggerAwareInterface {
	 ...
	 *
	 * (Note that appending $wgMimeTypeFile to the end of self::$wellKnownTypes
	 * sucks because you can't redefine canonical types. This could be fixed by
	 * appending self::$wellKnownTypes behind $wgMimeTypeFile, but who knows
...
EOT;
	 ...
	 *   - detectorCommand : Path to program that takes a file argument and prints the MIME type
	 ...
	 *   - dataCallback: callback to improve the guessed MIME type using the file data [optional]
	...
	public function __construct( array $params ) {
		$this->detectorCommand = $params['detectorCommand'];
		...
		$this->initCallback = isset( $params['initCallback'] )
			? $params['initCallback']
			: null;
		$this->dataCallback = isset( $params['dataCallback'] )
			? $params['dataCallback']
			: null;
	...
	private function detectMimeType( $file, $ext = true ) {
		...
		$mimeDetectorCommand = $this->detectorCommand;
		$m = null;
		if ( $mimeDetectorCommand ) {
			// @TODO: librarize these
