...
class CSSJanus {
	...
	private static function buildPatterns() {
		...
		$patterns['four_notation_quantity'] = "/{$patterns['possibly_negative_quantity']}(\s+){$patterns['possibly_negative_quantity']}(\s+){$patterns['possibly_negative_quantity']}(\s+){$patterns['possibly_negative_quantity']}(\s*[;}])/i";
	...
	private static function fixFourPartNotation( $css ) {
		$css = preg_replace( self::$patterns['four_notation_quantity'], '$1$2$7$4$5$6$3$8', $css );
	...
	private static function fixShadows( $css ) {
		// PHP sucks, you can't 'use ( $this )'
		$self = $this;
		
		$css = preg_replace_callback( self::$patterns['box_shadow'], function ( $matches ) use ( $self ) {
			return $matches[0] . $self::flipSign( $matches[1] );
		}, $css );

		$css = preg_replace_callback( self::$patterns['text_shadow1'], function ( $matches ) use ( $self ) {
			return $matches[0] . $matches[1] . $matches[2] . $self::flipSign( $matches[3] );
		}, $css );

		$css = preg_replace_callback( self::$patterns['text_shadow2'], function ( $matches ) use ( $self ) {
			return $matches[0] . $self::flipSign( $matches[1] );
		}, $css );
	...
	/**
	 * Flips the sign of a CSS value, possibly with an unit. (We can't just negate
