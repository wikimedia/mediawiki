...
class VirtualRESTServiceClient {
	...
	public function runMulti( array $reqs ) {
		...
		$unfinished = array_flip( array_keys( $reqs ) ); // (index => 1)
		...
		do {
			...
			foreach ( $replaceReqsByService as $prefix => $serviceReqs ) {
				$service = $this->instances[$prefix];
				foreach ( $service->replaceOutgoing( $serviceReqs ) as $index => $req ) {
					// Services use unique IDs for replacement requests
					if ( isset( $serviceReqs[$index] ) || isset( $reqs[$index] ) ) {
						// An original request which was not modified
						$key = $index;
					} else {
						// Replacement requests with pre-set responses should not execute
						$key = "{$prefix}#{$index}";
						$newReplaceReqsByService[$prefix][$key] = $req;
					}
					if ( isset( $req['response'] ) ) {
						// Replacement requests with pre-set responses should not execute
						unset( $executeReqs[$key] );
						$doneReqs[$key] = $req;
					} else {
			...
			foreach ( $this->http->runMulti( $executeReqs ) as $key => $ranReq ) {
				$doneReqs[$key] = $ranReq;
			...
			foreach ( $replaceReqsByService as $prefix => $serviceReqs ) {
				...
				$serviceReqs = array_intersect_key( $doneReqs, $serviceReqs );
				foreach ( $service->replaceIncoming( $serviceReqs ) as $index => $req ) {
					// Services use unique IDs for replacement requests
					if ( isset( $serviceReqs[$index] ) || isset( $reqs[$index] ) ) {
						// An original request which was not modified
						$key = $index;
		...
		foreach ( $reqs as $index => &$req ) {
			...
			$req['response'] = $doneReqs[$index]['response'];
			$req['headers'] = array();
			if ( isset( $doneReqs[$index]['headers'] ) ) {
				foreach ( $doneReqs[$index]['headers'] as $name => $value ) {
					$req['headers'][strtolower( $name )] = $value;
				}
			}
			unset( $req['chain'] ); // not needed by caller
		}
		unset( $req ); // don't assign over this by accident
