...
class WANObjectCacheRepear implements LoggerAwareInterface {
	 ...
	 *          - The starting unique ID used for breaking timestamp collisions
	 ...
	 *        The events should be in asending order, by (timestamp,id).
	...
	final public function invoke( $n = 100 ) {
		...
		if ( !$status ) {
			$status = [ 'pos' => time() - $this->initialStartWindow, 'id' => 1 ];
		}

		$events = call_user_func_array(
			$this->logChunkCallback,
			[ $status['pos'], $status['id'], time() - WANObjectCache::HOLDOFF_TTL - 1, $n ]
		);
		...
		foreach ( $keyEvents as $key => $keyEvent ) {
			if ( !$this->cache->reap( $key, $keyEvent['pos'] + WANObjectCache::HOLDOFF_TTL ) ) {
				break;
			}
			++$purgeCount;
		}
		...
		if ( $lastEvent ) {
			$ok = $this->store->merge(
				$posKey,
				function ( $bag, $key, $curValue ) use ( $lastEvent ) {
					if ( !$curValue || $lastEvent['pos'] > $curValue['pos'] ) {
						// Use new position
					} elseif ( $lastEvent['id'] >= $curValue['id'] ) {
						// Use new position
					} else {
