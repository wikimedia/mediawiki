...
class WANObjectCache implements IExpiringStore, LoggerAwareInterface {
	 ...
	 *      Default: WANObjectCache::STALE_TTL_NONE
	 *   - isCacheAsideEnabled: Whether this method should perform its normal cache-aside logic.
	 ...
	 * @note Options added in 1.31: staleTTL, graceTTL, isCacheAsideEnabled
	 * @note Callable type hints are not used to avoid class-autoloading
	 */
	final public function getWithSetCallback( $key, $ttl, $callback, array $opts = [] ) {
		$pcTTL = isset( $opts['pcTTL'] ) ? $opts['pcTTL'] : self::TTL_UNCACHEABLE;

		if ( isset( $opts['isCacheAsideEnabled'] ) && !$opts['isCacheAsideEnabled'] ) {
			$setOpts = [];
