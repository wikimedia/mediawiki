...
class WANObjectCache implements IExpiringStore, LoggerAwareInterface {
	...
	/** Default time to ignore popularity refreshes after key modification */
	const AGE_NEW = 60;
	/** The "popularity" refresh window */
	const HOT_TTR = 3600;
	/** Hits/second for a refresh to be expected within the "popularity" window */
	const HIT_RATE_HIGH = 100;
	 ...
	 *      Default: null.
	 *   - hotTTR: Expected time-till-refresh for keys that average ~100 hits/second.
	 *      This should be greater than "ageNew". Key with higher hit rates will regenerate
	...
	/**
	 * Check if a is due for randomized regeneration due to its popularity
	 *
	...
	protected function worthRefreshPopular( $asOf, $ageNew, $timeTillRefresh ) {
		...
		// P(refresh) * (# hits in $refreshWindowSec) = 1
