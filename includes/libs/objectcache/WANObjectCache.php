...
class WANObjectCache implements IExpiringStore, LoggerAwareInterface {
	...
	final public function getMulti(
		...
		foreach ( $valueKeys as $vKey ) {
			...
			if ( $value !== false ) {
				...
				if ( $checkKeyTimeForAll ) {
					$safeTimestamp = $checkKeyTimeForAll + self::HOLDOFF_TTL;
					if ( $safeTimestamp >= $wrappedValues[$vKey][self::FLD_TIME] ) {
						$curTTL = min( $curTTL, $checkKeyTimeForAll - $now );
					}
				}
				if ( isset( $checkKeyTimeByKey[$key] ) ) {
					$safeTimestamp = $checkKeyTimeByKey[$key] + self::HOLDOFF_TTL;
					if ( $safeTimestamp >= $wrappedValues[$vKey][self::FLD_TIME] ) {
						$curTTL = min( $curTTL, $checkKeyTimeByKey[$key] - $now );
					}
				}
			}
	...
	private function processCheckKeys( Array $checks, Array $wrappedValues, $now ) {
		...
		foreach ( $checks as $check ) {
			...
			$lowest = $lowest ? min( $lowest, $timestamp ) : $timestamp;
		}
		return $lowest;
	}
