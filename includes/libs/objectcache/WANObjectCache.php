...
class WANObjectCache implements IExpiringStore, LoggerAwareInterface {
	...
	final public function delete( $key, $ttl = self::HOLDOFF_TTL ) {
		...
		if ( $ttl <= 0 ) {
			...
			$ok = $this->cache->set( $key,
				$this->makePurgeValue( microtime( true ), 0 ),
				$ttl
			);
			// Publish the purge to all datacenters
			$ok = $this->relayPurge( $key, $ttl, 0 ) && $ok;
		}
	 ...
	 * @param holdoff $ttl How long to ignore sets for this value
	 * @return bool Success
	 */
	protected function relayPurge( $key, $ttl, $holdoff = 0 ) {
	 ...
	 */
	protected static function makePurgeValue( $timestamp, $ttl ) {
		return self::PURGE_VAL_PREFIX . (float)$timestamp . ':' . (int)$ttl;
	}
}
