...
abstract class BagOStuff implements IExpiringStore, LoggerAwareInterface {
	 ...
	 *   - dupGetCallback: callable that takes a callback and runs it after
	...
	public function __construct( array $params = [] ) {
		...
		if ( isset( $params['dupGetCallback'] ) &&
			is_callable( $params['dupGetCallback'] )
		) {
			$this->duplicateKeyLookups = [];
			$this->dupGetCallback = $params['dupGetCallback'];
		}
	}
	...
	private function trackDuplicateKeys( $key ) {
		...
		if ( $handlerRegistered === false ) {
			// Submit a callback that will look through the keys that
			// have been processed by get() and emit a log message for any
			// keys that were requested more than once.
			call_user_func( $this->dupGetCallback, function () {
