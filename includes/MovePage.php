...
class MovePage {
	...
	private function moveToInternal( User $user, &$nt, $reason = '', $createRedirect = true ) {
		...
		# Optional tags for moves
		if ( $oldNamespace != $newNamespace && ChangeTags::isActivatedOptionalTag( 'core', 'move', 'crossnamespace' ) ) {
			$tag = wfMessage( $wgOptionalTags['core']['move']['crossnamespace'] )->inContentLanguage->plain;
			if ( !$user->isAllowed('autopatrol') ) {
				# if user is not autopatrolled, tag both recent change for the null revision and move log entry
				ChangeTags::addTags( $tag, $nullRevision->getRecentChange->mAttribs['rc_id'], null, $logid	);
			} else {
				#if user is autopatrolled, only tag move log entry
				ChangeTags::addTags( $tag, null, null, $logid );
			}
		} elseif ( $oldNamespace == NS_USER && $newNamespace == NS_USER && $this->oldTitle->getDBkey() == $user->getTitleKey() &&
			strpos( $this->newTitle->getDBkey(), '/' ) == false && ChangeTags::isActivatedOptionalTag( 'core', 'move', 'rename' ) ) {
			$tag = wfMessage( $wgOptionalTags['core']['move']['rename'] )->inContentLanguage->plain;
			if ( !$user->isAllowed('autopatrol') ) {
				# if user is not autopatrolled, tag both recent change for the null revision and move log entry
				ChangeTags::addTags( $tag, $nullRevision->getRecentChange->mAttribs['rc_id'], null, $logid	);
			} else {
