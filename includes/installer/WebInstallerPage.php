...
class WebInstallerComplete extends WebInstallerPage {
	...
	public function execute() {
		...
		// Try to write LocalSettings.php directly.
		$ls = InstallerOverrides::getLocalSettingsGenerator( $this->parent );
		$rightsProfile = $this->parent->rightsProfiles[$this->parent->getVar( '_RightsProfile' )];
		foreach ( $rightsProfile as $group => $rightsArr ) {
			$ls->setGroupRights( $group, $rightsArr );
		}

		wfSuppressWarnings();
		$localSettingsWritten = $ls->writeFile( "$IP/LocalSettings.php" );
		wfRestoreWarnings();

		if ( $localSettingsWritten === false ) {
			// We were unable to write the file, so tell the user to do it.
		...
		$this->startForm();
		$this->parent->disableLinkPopups();
		$wikilink = $this->getVar( 'wgServer' ) . $this->getVar( 'wgScriptPath' )
				. '/index' . $this->getVar( 'wgScriptExtension' );
		if ( $localSettingsWritten === false ) {
			$this->addHTML(
				$this->parent->getInfoBox(
			...
			$this->addHTML(
				$this->parent->getInfoBox(
					wfMessage( 'config-install-done-copied',
						$wikilink
					)->plain(), 'tick-32.png'
				)
			);
