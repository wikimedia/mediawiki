...
class MssqlInstaller extends DatabaseInstaller {
	...
	protected $globalNames = array(
		...
		'wgDBmwschema',
		'wgDBprefix',
		'wgDBNTAuthentication',
	);

	protected $internalDefaults = array(
		'_InstallUser' => 'sa',
		'_InstallNTAuthentication' => 'sqlauth',
		'_WebNTAuthentication' => 'sqlauth',
	);
	...
	public function getConnectForm() {
		if ( $this->getVar( '_InstallNTAuthentication' ) == 'ntauth' ) {
		...
		return $this->getTextBox(
			...
			$this->getRadioSet( array(
				'var' => '_InstallNTAuthentication',
				'label' => 'config-mssql-auth',
				'itemLabelPrefix' => 'config-mssql-',
				'values' => array( 'sqlauth', 'ntauth' ),
				'itemAttribs' => array(
					'sqlauth' => array(
						'class' => 'showHideRadio',
						'rel' => 'dbCredentialBox',
	...
	public function submitConnectForm() {
		...
		// User box
		$this->setVarsFromRequest( array( '_InstallUser', '_InstallPassword', '_InstallNTAuthentication' ) );
	...
	public function openConnection() {
		...
		if ( $this->getVar( '_InstallNTAuthentication' ) == 'ntauth' ) {
			// Use NT authentication for this connection (TODO: find a better way of doing this)
			$user = 'ntauth';
			$password = 'ntauth';
		}
	...
	public function getSettingsForm() {
		...
		$displayStyle = $this->getVar( '_WebNTAuthentication' ) == 'ntauth' ? 'display: none' : '';
	...
	public function submitSettingsForm() {
		$this->setVarsFromRequest(
			array( 'wgDBuser', 'wgDBpassword', '_SameAccount', '_CreateDBAccount', '_WebNTAuthentication' )
		);

		if ( $this->getVar( '_SameAccount' ) ) {
			$this->setVar( '_WebNTAuthentication', $this->getVar( '_InstallNTAuthentication' ) );
			$this->setVar( 'wgDBuser', $this->getVar( '_InstallUser' ) );
			$this->setVar( 'wgDBpassword', $this->getVar( '_InstallPassword' ) );
		}

		if ( $this->getVar( '_WebNTAuthentication' ) == 'ntauth' ) {
			$this->setVar( 'wgDBuser', '' );
			$this->setVar( 'wgDBpassword', '' );
			$this->setVar( 'wgDBNTAuthentication', true );
		} else {
			$this->setVar( 'wgDBNTAuthentication', false );
		}

		if ( $this->getVar( '_CreateDBAccount' ) && $this->getVar( '_WebNTAuthentication' ) == 'sqlauth' && strval( $this->getVar( 'wgDBpassword' ) ) == '' ) {
	...
	public function setupUser() {
		...
		if ( $dbUser == $this->getVar( '_InstallUser' )
				|| ( $this->getVar( '_InstallNTAuthentication' ) == 'ntauth'
					&& $this->getVar( '_WebNTAuthentication' ) == 'ntauth' ) ) {
			return Status::newGood();
		}
		$status = $this->getConnection();
		if ( !$status->isOK() ) {
		...
		if ( $tryToCreate ) {
			...
			if ( !$this->loginExists( $dbUser ) ) {
				try {
					...
					$logintype = $this->getVar( '_WebNTAuthentication' ) == 'ntauth' ? 'FROM WINDOWS' : "WITH PASSWORD = $escPass";
	...
	public function getLocalSettings() {
		...
		$ntauth = $this->getVar( 'wgDBNTAuthentication' ) ? 'true' : 'false';

		return "# MSSQL specific settings
\$wgDBNTAuthentication = {$ntauth};
\$wgDBmwschema = \"{$schema}\";
\$wgDBprefix = \"{$prefix}\";";
	}
}
