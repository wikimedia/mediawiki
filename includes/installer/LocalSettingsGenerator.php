...
class LocalSettingsGenerator {
	...
	public function __construct( Installer $installer ) {
		...
		$nonNullItems = array( 'wgServer' );

		foreach ( $confItems as $c ) {
			$val = $installer->getVar( $c );

			if ( in_array( $c, $nonNullItems ) && $val == null ) {
				continue;
			}
...
";
		if ( array_key_exists( 'wgServer', $this->values ) ) {
			$returnPHP .= "## The protocol and server name to use in fully-qualified URLs
\$wgServer = \"{$this->values['wgServer']}\";
";
		}
		$returnPHP .= "## The relative URL path to the skins directory
