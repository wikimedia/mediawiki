...
class WatchedItemStore {
	...
	public function loadWatchedItem( User $user, Title $title ) {
		...
		// Only cache pages that could possibly be watched
		if ( $watchingIsPossible ) {
			$this->cache( $item );
		}
		return $item;
	...
	public function resetNotificationTimestamp( User $user, Title $title, $force = '', $oldid = 0 ) {
		...
		if ( $force != 'force' ) {
			...
			if ( !$item->isWatched() || $item->getNotificationTimestamp() === null ) {
				return false;
			}
		}
		...
		if ( !$oldid ) {
			...
			if ( $item->getNotificationTimestamp() === null ) {
