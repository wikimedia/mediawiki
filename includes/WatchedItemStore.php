...
class WatchedItemStore implements StatsdAwareInterface {
	...
	/**
	 * Deletes ALL watched items for the given user when under MAX_INTERACTIVE_ITEM_DELETE entries exist.
	 *
	 * @since 1.30
	 *
	 * @param User $user
	...
	public function clearUserWatchedItems( User $user ) {
		...
		$dbw = $this->loadBalancer->getConnectionRef( DB_MASTER, [ 'watchlist' ] );
		$dbw->delete(
			'watchlist',
			[ 'wl_user' => $user->getId() ],
			__METHOD__
