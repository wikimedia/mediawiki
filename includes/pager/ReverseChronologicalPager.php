...
abstract class ReverseChronologicalPager extends IndexPager {
	 ...
	 * @param int $year Year up to which we want revisions. If not set, current year is used.
	 * @param int $month Month up to which we want revisions. If not set, end of year is used.
	 * @param int $day Day up to which we want revisions. If not set, end of month is used.
	 * @return string Timestamp
	 */
	function getDateCond( $year = 0, $month = 0, $day = 0 ) {
		$year = intval( $year );
		$month = intval( $month );
		$day = intval( $day );
