...
abstract class ReverseChronologicalPager extends IndexPager {
	 ...
	 * year and month in the direction specified. true means forward and false otherwise
	 *
	 * @param int $year
	 * @param int $month
	 ...
	 * @return int|null
	 */
	public function getDateTimeCond( $year, $month, $direction = true ) {
		$year = intval( $year );
		$month = intval( $month );
		...
		// "WHERE rev_timestamp > result" depending upon "direction" parameter
		// Examples: year = 2006 equals < 20070101 (+000000) with direction = true
		// year=2005, month=1    equals < 20050201 with direction = true
		// year=2005, month=12   equals < 20051130 with direction = false
		if ( !$this->mYear && !$this->mMonth && $direction ) {
			if ( $direction ) {
				return null;
			} else {
				$ymd = (int)sprintf( "%04d%02d01", 1970, 1 );
		...
		$ymd = $direction ? (int)sprintf( "%04d%02d01", $year, $month ):
	...
	/**
	 * Sets the timestamp for the offset given the year and month
	 *
	 * @param int $year
	 * @param int $month
	 */
	public function getDateCond( $year, $month ) {
		$offset = $this->getDateTimeCond( $year, $month, true );
		if ( $offset !== null ) {
			$this->mOffset = $offset;
		}
	}
