...
class MWTimestamp {
	...
	public function getHumanTimestamp( MWTimestamp $relativeTo = null, User $user = null, Language $lang = null ) {
		...
		$ts = '';
		if ( wfRunHooks( 'GetHumanTimestamp', array( &$ts, $this, $relativeTo, $diff, $user ) ) ) {
			$ts = $lang->getHumanTimestamp( $this, $relativeTo, $diff, $user );
		}
	 ...
	 */
	public function getRelativeTimestamp( MWTimestamp $relativeTo = null, Language $lang = null, array $chosenIntervals = array() ) {
		if ( $relativeTo === null ) {
			$relativeTo = new self;
		}
		if ( $lang === null ) {
		...
		$ts = '';
		$diff = $this->diff( $relativeTo );
		if ( !wfRunHooks( 'GetRelativeTimestamp', array( &$ts, $this, $relativeTo, $diff ) ) ) {
			return $ts;
		} else {
			$seconds = ( ( ( $diff->a * 24 + $diff->h ) * 60 + $diff->i ) * 60 + $diff->s );
			return wfMesage( 'ago', $lang->formatDuration( $seconds, $chosenIntervals );
		}
	}
	 ...
	 *
	 * @since 1.21
	 * @param MWTimestamp Relative current time
	 * @return stdClass|DateInterval Difference
	 */
