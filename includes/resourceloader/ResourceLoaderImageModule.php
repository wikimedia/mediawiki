		...
		if ( !isset( $this->imageObjects ) ) {
			...
			foreach ( $this->images as $type => $list ) {
				foreach ( $list as $key => $value ) {
					...
					$variantConfig = $this->variants[$type];
					$allowedVariants = array_merge(
						isset( $options['variants'] ) ? $options['variants'] : array(),
						$this->getGlobalVariants( $type )
					);

					$image = new ResourceLoaderImage( $name, $this->getName(), $path, $variantConfig, $allowedVariants );
	...
	public function getStyles( ResourceLoaderContext $context ) {
		...
		foreach ( $this->getImages() as $name => $image ) {
			...
			$declarations = $this->getCssDeclarations(
				...
				$image->getUrl( $script, $context->getVersion(), null, 'rasterized' )
			);
			...
			foreach ( $image->getVariants() as $variant ) {
				$declarations = $this->getCssDeclarations(
					...
					$image->getUrl( $script, $context->getVersion(), $variant, 'rasterized' )
				);
				$declarations = implode( "\n\t", $declarations );
