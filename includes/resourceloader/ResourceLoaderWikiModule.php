 ...
 *   - getPosition()
 *   - getPages()
 * - Basic methods that stricly involve the foreign database
 *   - getDB()
 *   - isKnownEmpty()
 *   - getTitleInfo()
 */
class ResourceLoaderWikiModule extends ResourceLoaderModule {
	 ...
	 * @param ResourceLoaderContext $context
	 * @return array Keyed by revision id. Value is an array with 'rev_len' and 'rev_sha1' keys
	 */
		...
		if ( !isset( $this->titleInfo[$key] ) ) {
			...
			if ( !$batch->isEmpty() ) {
				$res = $dbr->select( array( 'page', 'revision' ),
					array( 'page_id', 'rev_len', 'rev_sha1' ),
					$batch->constructSet( 'page', $dbr ),
					__METHOD__,
					array(),
					array( 'page' => array( 'INNER JOIN', array( 'page_latest=rev_id' ) ) )
				);
				foreach ( $res as $row ) {
					$this->titleInfo[$key][$row->page_id] = array(
						'rev_len' => $row->rev_len,
