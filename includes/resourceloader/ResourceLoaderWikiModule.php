...
class ResourceLoaderWikiModule extends ResourceLoaderModule {
	...
	public function isKnownEmpty( ResourceLoaderContext $context ) {
		$pages = $this->getTitleInfo( $context );
	 ...
	 * @param ResourceLoaderContext $context
	 * @return array Keyed by page id. Value is an array with 'rev_id' and 'rev_len' keys
	 */
	protected function getTitleInfo( ResourceLoaderContext $context ) {
		...
		if ( !isset( $this->titleInfo[$hash] ) ) {
			...
			if ( !$batch->isEmpty() ) {
				$res = $dbr->select( 'page',
					array( 'page_id', 'page_latest', 'page_len' ),
					$batch->constructSet( 'page', $dbr ),
					__METHOD__
				);
				foreach ( $res as $row ) {
					$this->titleInfo[$hash][$row->page_id] = array(
						'rev_id' => $row->page_latest,
						'rev_len' => $row->page_len,
					);
				}
			}
