...
class ResourceLoaderClientHtml {
	...
	public function getHeadHtml() {
		...
		// RLQ 3): Embedded modules and only=scripts
		foreach ( $data['embed']['scripts'] as $name ) {
			$chunks[] = $this->makeModuleEmbed( $name, ResourceLoaderModule::TYPE_SCRIPTS );
		}
		foreach ( $data['embed']['general']['top'] as $name ) {
			$chunks[] = $this->makeModuleEmbed( $name, ResourceLoaderModule::TYPE_COMBINED );
		}
		...
		foreach ( $data['embed']['styles'] as $name ) {
			$chunks[] = $this->makeModuleEmbed( $name, ResourceLoaderModule::TYPE_STYLES );
		}
	...
	private function getData() {
		...
		foreach ( $this->moduleScripts as $name ) {
			$module = $rl->getModule( $name );
			$context = $this->makeContext( $module );
	 ...
	 */
	private function makeModuleEmbed( $name, $type ) {
		$module = $this->resourceLoader->getModule( $name );
		$modules = [ $name => $module ];
		$context = $this->makeContext( $module, $type );
		return ResourceLoader::makeInlineScript(
			$this->resourceLoader->makeModuleResponse( $context, $modules )
		);
	}
	 ...
	 */
	private function makeContext( ResourceLoaderModule $module, $type ) {
		$group = $module->getGroup();
