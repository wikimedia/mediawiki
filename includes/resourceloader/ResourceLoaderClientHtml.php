...
class ResourceLoaderClientHtml {
	...
	public function getHeadHtml() {
		...
		// Async scripts. The startup module loads non-blocking.
		// Once it arrives, it will execute the inline RLQ scripts scheduled above.
		$chunks[] = $this->makeResourceLoaderLink( 'startup', ResourceLoaderModule::TYPE_SCRIPTS );
	...
	private function makeResourceLoaderLink( $modules, $only ) {
		$modules = (array)$modules;
		return self::makeResourceLoaderLink( $this->resourceLoader, $modules, $only );
	}
	 ...
	 * @return string|WrappedStringList HTML
	 */
	public static function makeResourceLoaderLink( ResourceLoader $rl, array $modules, $only, array $extraQuery = [] ) {
		$chunks = [];
