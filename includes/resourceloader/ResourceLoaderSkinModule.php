	...
	public function getStyles( ResourceLoaderContext $context ) {
		...
		if ( $logoHD ) {
			if ( isset( $logoHD['1.5x']) ) {
				$styles['(-webkit-min-device-pixel-ratio: 1.5), (min--moz-device-pixel-ratio: 1.5), (min-resolution: 1.5dppx), (min-resolution: 144dpi)'][] = '.mw-wiki-logo { background-image: ' .
				CSSMin::buildUrlValue( $logoHD['1.5x'] ) .
				'; background-size: 135px auto; }';
			}
			if ( isset( $logoHD['2x']) ) {
				$styles['(-webkit-min-device-pixel-ratio: 2), (min--moz-device-pixel-ratio: 2), (min-resolution: 2dppx), (min-resolution: 192dpi)'][] = '.mw-wiki-logo { background-image: ' .
