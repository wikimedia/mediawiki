...
class ResourceLoader {
	 ...
	 * @param Array $modules List of module names or module objects
	 * @return string Hash
	 */
	public function getCombinedVersion( ResourceLoaderContext $context, Array $modules ) {
		...
		foreach ( $modules as $module ) {
			if ( !( $module instanceof ResourceLoader ) ) {
				if ( !is_string( $module ) ) {
					throw new Exception( '$module must be a string or object' );
				}
				$module = $this->getModule( $module );
			}
			$hashes .= $module->getVersionHash( $context );
		}
		// Allow a single hash to pass through. Saves computation and makes output more predictable.
		if ( count( $modules ) === 1 ) {
				return $hashes;
		}
		return self::makeHash( $hashes );
	}
