...
class ResourceLoader {
	...
	public function respond( ResourceLoaderContext $context ) {
		...
		// Preload information in a batch to optimise calls to ResourceLoader::getVersionHash below
		try {
		...
		// FIXME: To send Last-Modified and support If-Modified-Since, we need to detect
		...
		$versionHash = sha1( $hashes );

		// Try the client-side cache first
		if ( $this->tryRespondNotModified( $context, $versionHash ) ) {
		...
		$this->sendResponseHeaders( $context, $versionHash, (bool)$this->errors );
	...
	/**
	 * Send Content-Type and cache control headers to the client.
	 * @param ResourceLoaderContext $context
	 * @param string $versionHash Combined module version hash
	 * @param bool $errors Whether there are errors in the response
	 * @return void
	 */
	protected function sendResponseHeaders( ResourceLoaderContext $context, $versionHash, $errors ) {
	 ...
	 *
	 * @param ResourceLoaderContext $context
	 * @param string $versionHash The combined module version hash (SHA-1)
	 * @return bool True if HTTP 304 was sent and output handled
	 */
	protected function tryRespondNotModified( ResourceLoaderContext $context, $versionHash ) {
		$clientKey = $context->getRequest()->getHeader( 'If-None-Match' );
		// Never send 304s in debug mode
		if ( $clientKey !== false && !$context->getDebug() ) {
			if ( $clientKey === $versionHash ) {
				...
				$this->sendResponseHeaders( $context, $versionHash, false );
				return true;
			}
		}
		return false;
	...
	protected function tryRespondFromFileCache(
		...
		if ( $good ) {
			// FIXME: Broken, needs to be converted to use version hashes (T94810)
