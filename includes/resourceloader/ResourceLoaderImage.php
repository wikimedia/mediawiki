...
class ResourceLoaderImage {
	...
	/**
	 * Get the path to image file for given language.
	 * @param Language $language Language object
	 * @return string
	 */
	public function getPath( Language $lang ) {
		$desc = $this->descriptor;
		if ( is_string( $desc ) ) {
			return $this->basePath . '/' . $desc;
		} elseif ( isset( $desc['lang'][ $lang->getCode() ] ) ) {
			return $this->basePath . '/' . $desc['lang'][ $lang->getCode() ];
		} elseif ( isset( $desc[ $lang->getDir() ] ) ) {
			return $this->basePath . '/' . $desc[ $lang->getDir() ];
		} else {
	 ...
	 * @return string
	 */
	public function getUrl( $script, $version, $variant, $format, Language $lang ) {
		$query = array(
	 ...
	 * @param Language $language Language object to get the URI for
	 * @return string
	 */
	public function getDataUri( $variant, $format, Language $lang ) {
		$type = $this->getMimeType( $format );
		$contents = $this->getImageData( $variant, $format, $lang );
	 ...
	 * @param Language $language Language object to get the data for
	 * @return string|false Possibly binary image data, or false on failure
	 */
	public function getImageData( $variant, $format, Language $lang ) {
		if ( $this->getExtension() !== 'svg' ) {
			return file_get_contents( $this->getPath( $lang ) );
		}

		if ( $variant && isset( $this->variants[$variant] ) ) {
			$data = $this->variantize( $this->variants[$variant], $lang );
		} else {
			$data = file_get_contents( $this->getPath( $lang ) );
		}
	 ...
	 */
	protected function variantize( $variantConf, Language $lang ) {
		$dom = new DomDocument;
		$dom->load( $this->getPath( $lang ) );
