...
class ResourceLoaderImage {
	 ...
	 * @endcode
	 */
	public function __construct( ResourceLoader $resourceLoader, $module, $path, $type, array $options = array() ) {
	...
	protected function variantize( $variant ) {
		// TODO This will not work with most images, and we can't use the
		// <style>* { fill: whatever }</style> trick because rsvg doesn't understand it
		$svg = new DomDocument;
		$svg->load( $this->path );
		$svg->getElementsByTagName( 'g' )->item( 0 )->setAttribute( 'fill', $variant['color'] );
		return $svg->saveXml();
	}
