...
class ResourceLoaderImage {
	 ...
	 * @endcode
	 */
	public function __construct( $module, $path, $type, $variantConfiguration, array $options = array() ) {
		$this->module = $module;
		$this->path = $path;
		$this->type = $type;
	...
	public function getUrl( $script, $variant, $format ) {
		$query = array(
			'variant' => $variant,
			'format' => $format,
	...
	protected function variantize( $variantConf ) {
		...
		$svg = new DomDocument;
		$svg->load( $this->path );
		$svg->getElementsByTagName( 'g' )->item( 0 )->setAttribute( 'fill', $variantConf['color'] );
		return $svg->saveXml();
	}
