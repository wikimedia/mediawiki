...
*/ );
	 ...
	 * @return array
	 */
	public static function getList( $name, $shard = null, $offset = 0, $sort = 'id', $order = 'ASC' ) {
		global $wgMemc;
		...
		$key = wfMemcKey( get_called_class(), 'getList', $name, $shard, $offset, $sort, $order );
		...
		if ( $list === false ) {
			...
			$list = static::getListFromDB( $name, $shard, $min, $batchSize, $sort, $order );
	...
	protected function updateLists( DataModel $old = null ) {
		...
		foreach ( static::$lists as $list => $properties ) {
			...
			if ( $existsNew ) {
				...
				foreach( $newLists[$list] as $sort => $sortValue) {
		...
		foreach ( array( $shard, null ) as $shard ) {
			...
			$callback = function( BagOStuff $cache, $key, $existingValue ) use ( $name, $shard, $difference, $class ) {
				...
				if ( $existingValue === false ) {
					return static::getCount( $name, $shard );
				}
	 ...
	 * @return array
	 */
	protected static function getListFromDB( $name, $shard, $offset, $limit, $sort, $order ) {
	...
	protected function generateId() {
		...
		$callback = function( BagOStuff $cache, $key, $existingValue ) use ( &$newId, $class ) {
			if ( $existingValue === false ) {
				$existingValue = static::getIdFromDB();
			}
