...
class MediaWikiServices extends ServiceContainer {
	...
	/**
	 * @var bool true during the execution of resetLegacyServices().
	 */
	...
	public static function resetGlobalInstance( Config $bootstrapConfig = null, $quick = '' ) {
		...
		if ( defined( 'MW_SERVICE_BOOTSTRAP_COMPLETE' )
			...
			throw new MWException( __METHOD__ . ' must not be called after service bootstrapping.' );
		}
	...
	private function salvage( self $other ) {
		foreach ( $this->getServiceNames() as $name ) {
			$oldService = $this->peekService( $name );
	...
	private static function resetLegacyServices() {
		...
		$guard = new ScopedCallback( function() {
