...
class FileDeleteForm {
	...
	private function showForm() {
		...
		$options[] = [
			'data' => 'other',
			'label' => $wgOut->msg( 'filedelete-reason-otherlist' )->inContentLanguage()->text(),
		];
		$list = $wgOut->msg( 'filedelete-reason-dropdown' )->inContentLanguage()->text();
		foreach ( explode( "\n", $list ) as $option ) {
			$value = trim( $option );
			if ( $value == '' ) {
				continue;
		...
		if ( $wgUser->isAllowed( 'editinterface' ) ) {
			$link = Linker::linkKnown(
				$wgOut->msg( 'deletereason-dropdown' )->inContentLanguage()->getTitle(),
