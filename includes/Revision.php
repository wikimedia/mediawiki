...
class Revision implements IDBAccessObject {
	...
	public static function newFromRow( $row ) {
		if ( is_array( $row ) ) {
			$rec = self::getRevisionStore()->newRevisionFromArray( $row );
		} else {
	...
	public static function userJoinCond() {
		return self::getExtraQueryInfo( ['user'] )['joins']['user'];
	}
	...
	public static function pageJoinCond() {
		return self::getExtraQueryInfo( ['page'] )['joins']['page'];
	}
	...
	public static function selectFields() {
		return self::getQueryInfo()['fields'];
	}
	...
	public static function selectArchiveFields() {
		return self::getArchiveQueryInfo()['fields'];
	}
	...
	public static function selectTextFields() {
		return self::getExtraQueryInfo( ['text'] )['fields'];
	}
	...
	public static function selectPageFields() {
		return self::getExtraQueryInfo( ['page'] )['fields'];
	}
	...
	public static function selectUserFields() {
		return self::getExtraQueryInfo( ['user'] )['fields'];
	}
	...
	/**
	 * Like getQueryInfo(), but excluding query info for the revision table.
	 *
	 * @param array $options
	 *
	 * @return array
	 */
	private static function getExtraQueryInfo( $options = [] ) {
		return self::getRevisionStore()->getQueryInfo( $options, false );
	}
	...
	function __construct( $row, $queryFlags = 0, Title $title = null ) {
		if ( $row instanceof RevisionRecord ) {
			...
			$this->mRecord = self::getRevisionStore()->newRevisionFromArray( $row, $queryFlags, $title );
	...
	public function setUserIdAndName( $id, $name ) {
		if ( $this->mRecord instanceof MutableRevisionRecord ) {
			$this->mRecord->setUserIdAndName( intval( $id ), $name );
		} else {
			throw new ImmutableRevisionException( __METHOD__ . ' is not supported on this instance' );
		}
	}
	 ...
	 *
	 * @deprecated since 1.31; use RevisionRecord::hasSameContent() to check if two
	 * revisions have the same content.
	 *
	...
	public function getTextId() {
		...
		return $slot->hasAddress() ? $slot->getAddress() : null;
	}
	...
	public function insertOn( $dbw ) {
		...
		// A RevisionStoreRecord comes from the database, so it cannot be inserted into the database.
		if ( $this->mRecord instanceof RevisionStoreRecord ) {
			throw new ImmutableRevisionException( 'This revision cannot be saved since it already exists' );
		}

		// Note that getId() will typically return null here, but not always,
		// e.g. not when restoring a revision.

		if ( $this->mRecord->getUserId() === null ) {
			$name = $this->mRecord->getUserText();
			if ( $name === null ) {
				$this->mRecord->setUser( $wgUser );
			} else {
				$this->mRecord->setUserIdAndName( 0, $name );
			}
		}
		...
		Hooks::run( 'RevisionInserted', [ $this ] );
