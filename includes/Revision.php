...
/**
 * @deprecated since 1.30, use RevisionRecord, RevisionStore, and ContentStore instead.
 */
class Revision implements IDBAccessObject {
	/** @var int|null Needed on top of $mRecord, bnecause it may be updated! */
	protected $mId;
	 ...
	 * @access private
	 */
	function __construct( $row, $queryFlags = 0 ) {
		$this->mQueryFlags = 0;

		if ( $row instanceof RevisionRecord ) {
			$this->mRecord = $row;
		} elseif ( is_array( $row ) ) {
			$this->mRecord = self::getRevisionStore()->newRevisionFromArray_1_29( $row );
		} else {
			$this->mRecord = self::getRevisionStore()->newRevisionFromRow_1_29( $row );
		}
	...
	public function setUserIdAndName( $id, $name ) {
		...
		// FIXME: RevisionRecord is immutable, we'll need to create a new instance!
		// $this->mUser = (int)$id;
		// $this->mUserText = $name;
	 ...
	 *
	 * @warn Since 1.30, this returns the main slot's content address as a string.
	...
	public function getTitle() {
		// XXX: this really is not very pretty. Maybe move getTitle into RevisionRecord afterall.
	...
	public function setTitle( $title ) {
		$pageId = $this->getPage();
		if ( $pageId !== null && $pageId > 0 && $pageId !== $title->getArticleID( ) ) {
			throw new InvalidArgumentException( 'Title mismatches the revision\'s page ID' );
		}
	...
	public function isUnpatrolled() {
		// XXX: this really is not very pretty. Maybe move isUnpatrolled into RevisionRecord afterall.
	...
	public function getRecentChange( $flags = 0 ) {
		// XXX: this really is not very pretty. Maybe move getRecentChange into RevisionRecord afterall.
	...
	public function getSerializedData() {
		// FIXME: there is no caching here. Could use setTransientData().
	...
	public function getPrevious() {
		return self::getRevisionStore()->getPreviousRevision( $this->mRecord );
	}
	...
	public function getNext() {
		return self::getRevisionStore()->getNextRevision( $this->mRecord );
	}
	...
	public static function getRevisionText( $row, $prefix = 'old_', $wiki = false ) {
		...
		return self::getContentStore()->expandBlob( $text, $flags, $row->old_id );
	}
	...
	public static function decompressRevisionText( $text, $flags ) {
		return self::getContentStore()->decompressRevisionData( $text, $flags );
	}
	...
	public function insertOn( $dbw ) {
		...
		$this->mId = self::getRevisionStore()->insertRevisionOn( $this->mRecord, $dbw, $this->mQueryFlags );
	...
	public static function newNullRevision( $dbw, $pageId, $summary, $minor, $user = null ) {
		...
		return self::getRevisionStore()->newNullRevision( $dbw, $pageId, $summary, $minor, $user );
	}
	...
	public function userCan( $field, User $user = null ) {
		...
		return $this->mRecord->userCan( $field, $user ); // FIXME: not in the interface...
	}
	...
	static function countByPageId( $db, $id ) {
		return self::getRevisionStore()->countByPageId( $db, $id );
	}
	...
	static function countByTitle( $db, $title ) {
		return self::getRevisionStore()->countByTitle( $db, $title );
	}
	...
	public static function newKnownCurrent( IDatabase $db, $pageId, $revId ) {
		return self::getRevisionStore()->newKnownCurrent( $db, $pageId, $revId );
	}
}
