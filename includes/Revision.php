...
class Revision implements IDBAccessObject {
	...
	function __construct( $row, $queryFlags = 0 ) {
		...
		if ( $row instanceof RevisionRecord ) {
			...
			$this->mRecord = self::getRevisionStore()->newMutableRevision( $row );
		} else {
			$this->mRecord = self::getRevisionStore()->newRevisionFromRow( $row );
		}
	}
	 ...
	 *
	 * @note: since 1.30, this is now a noop.
	 *
	 * @param Title $title
	 */
	...
	public function insertOn( $dbw ) {
		...
		$this->mQueryFlags |= self::READ_LATEST; // FIXME: is this really needed??
