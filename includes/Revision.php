 ...
 * @todo document
 */
class Revision implements DBAO {
	protected $mId;
	protected $mPage;
	protected $mUserText;
	 ...
	 *
	 * $flags include:
	 *      DBAO::LATEST_READ  : Select the data from the master
	 *      DBAO::LOCKING_READ : Select & lock the data from the master
	 *      DBAO::AVOID_MASTER : Avoid master queries; data may be stale
	 *
	 * @param $id Integer
	...
	public static function newFromTitle( $title, $id = 0, $flags = 0 ) {
		...
		if ( $id ) {
			...
			$conds['rev_id'] = $id;
		} elseif ( !( $flags & DBAO::AVOID_MASTER ) && wfGetLB()->getServerCount() > 1 ) {
	...
	private static function newFromConds( $conditions, $flags = 0 ) {
		$db = wfGetDB( ( $flags & DBAO::LATEST_READ ) ? DB_MASTER : DB_SLAVE );
		$rev = self::loadFromConds( $db, $conditions, $flags );
		if ( is_null( $rev ) && wfGetLB()->getServerCount() > 1 ) {
			if ( !( $flags & DBAO::LATEST_READ ) && !( $flags & DBAO::AVOID_MASTER ) ) {
				$dbw = wfGetDB( DB_MASTER );
