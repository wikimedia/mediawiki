...
class Revision implements IDBAccessObject {
	...
	function __construct( $row ) {
		if ( is_object( $row ) ) {
			...
			if ( isset( $row['slots'] ) ) {
				...
				$this->mSlotBlobInfo = $row['slots'];
			} elseif ( isset( $row['content'] ) ) {
				// B/C only!
				$this->mSlotContent['main'] = $row['content'];
				...
				$content = $handler->unserializeContent( $row['text'], $format );
				$this->mSlotContent['main'] = $content;
			} else {
			...
			if ( !$this->mSize ) {
				$slots = $this->getSlots(); // XXX: possibly doing work / loading from DB!
	...
	public function getSize() {
		// TODO: calc from slots when needed
		return $this->mSize;
	}
	...
	protected function getSlots() {
		if ( $this->mSlotBlobInfo === null ) {
			$store = $this->getRevisionContentLookup();
			$this->mSlotBlobInfo = $store->getRevisionSlots( $this->getId() );
		}

		// FIXME: create BlobInfo for entries in $this->mSlotContent!

		return $this->mSlotBlobInfo;
	}
	...
	protected function getMainSlotInfo() {
		...
		if ( !isset( $slots['main'] ) ) {
			throw new MWException( 'Main slot not know!' );
		}
	...
	public function insertOn( $dbw ) {
		...
		$row = [
			...
			'rev_sha1'       => $this->mSha1 === null
				? self::calculateRevisionHash( $slots )
				: $this->mSha1,
		];
