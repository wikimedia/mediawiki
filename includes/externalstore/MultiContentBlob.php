...
abstract class MultiContentBlob {
	...
	private static $knownTypes = [
		...
		'diff' => DiffMultiContentBlob::class,
	];
	...
	/**
	 * @param string &$blob Blob to read from the start of. The used bytes will
	 *  be removed.
	 * @return int
	 */
	private static function consumeInt( &$blob ) {
		static $max = ( PHP_INT_MAX >> 7 ) - ( ( PHP_INT_MAX & 0x7f ) === 0x7f ? 0 : 1 );
	 ...
	 * @return string
	 */
	private static function consumeStr( &$blob ) {
		$len = self::consumeInt( $blob );
		if ( $len === 0 ) {
			return '';
		}
		if ( strlen( $blob ) < $len ) {
		...
		$ret = substr( $blob, 0, $len );
		$blob = substr( $blob, $len );
		return $ret;
	}
	...
	final public static function decode( $blob ) {
		...
		// Get type
		$blob = substr( $blob, 6 ); // Remove header
		$type = self::consumeStr( $blob );
		if ( !isset( self::$knownTypes[$type] ) ) {
			throw new \OutOfBoundsException( 'Unknown blob type "' . $type . '"' );
		}

		// Get flags
		$flags = self::consumeInt( $blob );
		...
		// Extract metadata
		$metadataStr = self::consumeStr( $blob );
		if ( $metadataStr === '' ) {
			$metadata = null;
		} else {
		...
		// Parse data
		$count = self::consumeInt( $blob );
		$data = [];
		for ( $i = 0; $i < $count; $i++ ) {
			$k = $isAssoc ? self::consumeStr( $blob ) : $i;
			$data[$k] = self::consumeStr( $blob );
		}
