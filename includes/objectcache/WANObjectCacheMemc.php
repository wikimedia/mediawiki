...
class WANObjectCacheMemc extends WANObjectCache {
	...
	public function __construct( array $params ) {
		...
		if ( ! $this->cache instanceof MemcachedBagOStuff ) {
			throw new InvalidArgumentException( __CLASS__ . ' requires a MemcachedBagOStuff.' );
		}
	...
	public function doRelayPurge( $key, $ttl = self::HOLDOFF_TTL ) {
		return $this->relayer->notify(
			...
			array(
				...
				'key' => $key,
				'val' => 'PURGED:$UNIXTIME$',
				'flg' => 0, // data is not serialized nor gzipped (for memcached driver)
				'ttl' => max( $ttl, 1 ),
