...
class ObjectCacheManager {
	 ...
	 * @var array
	 */
	private $defaultMemcachedParams = [];
	...
	public function __construct(
		...
		// default handling for CACHE_ACCEL alias
		if ( !isset( $this->objectCaches[CACHE_ACCEL] ) ) {
			$this->objectCaches[CACHE_ACCEL] = [ 'factory' => [ $this, 'getLocalServerInstance' ] ];
		}

		// default handling for CACHE_ANYTHING alias
		if ( !isset( $this->objectCaches[CACHE_ANYTHING] ) ) {
			$this->objectCaches[CACHE_ANYTHING] = [ 'factory' => [ $this, 'newAnything' ] ];
		}
	}

	/**
	 * Parameters defaults to be applied to instances of MemcachedBagOfStuff.
	 *
	 * @param array $memcachedParams
	 */
	public function setDefaultMemcachedParams( array $memcachedParams ) {
		$this->defaultMemcachedParams = $memcachedParams;
	}
	...
	private function newFromId( $id ) {
		if ( !isset( $this->objectCaches[$id] ) ) {
			throw new MWException( "Invalid object cache type \"$id\" requested. " .
				"Configured cache types: " . join( ', ', array_keys( $this->objectCaches ) ) );
		}

		$cache = $this->newFromParams( $this->objectCaches[$id] );
		Assert::postcondition(
			$cache instanceof BagOStuff,
			"Config for cache $id did not produe a BagOStuff instance!"
		);
	 ...
	 *  - factory: Callback called with ( $params, $this ) that returns BagOStuff.
	 *  - class: BagOStuff subclass constructed with $params.
	 *  - loggroup: Alias to set 'logger' key with LoggerFactory group.
	 *  - .. Other parameters passed to factory or class.
	 * @return BagOStuff
	...
	private function newFromParams( $params ) {
		...
		if ( isset( $params['factory'] ) ) {
			return call_user_func( $params['factory'], $params, $this );
			...
			if ( is_subclass_of( $class, 'MemcachedBagOStuff' ) ) {
				$params = array_merge( $this->defaultMemcachedParams, $params );
			}
	...
	private function newWANCacheFromId( $id ) {
		if ( !isset( $this->wanObjectCaches[$id] ) ) {
			throw new MWException( "Invalid WAN object cache type \"$id\" requested. " .
				"Configured WAN cache types: " . join( ', ', array_keys( $this->wanObjectCaches ) ) );
		}
		...
		Assert::postcondition(
			$cache instanceof WANObjectCache,
			"Config for WAN cache $id did not produe a WANObjectCache instance!"
		);
	...
	public function getMainWANInstance() {
		$id = $this->mainWANCache === false ? CACHE_NONE : $this->mainWANCache;
		return $this->getWANInstance( $id );
	}
