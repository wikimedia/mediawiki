...
class RedisBagOStuff extends BagOStuff {
	protected $servers, $connectTimeout, $persistent, $password, $automaticFailover;
	protected $conns = array(), $deadServers = array();
	...
	public function delete( $key, $time = 0 ) {
		wfProfileIn( __METHOD__ );
		list( $server, $conn ) = $this->getConnection( $key );
		$this->debug( "delete $key on $server" );
		if ( !$conn ) {
			wfProfileOut( __METHOD__ );
			return false;
		}
		try {
			$result = (bool)$conn->delete( $key );
		} catch ( RedisException $e ) {
			$result = false;
			$this->handleException( $server, $e );
		}
	...
	public function getMulti( array $keys ) {
		...
		$this->logRequest( 'getMulti', $key, $server, $result );
		wfProfileOut( __METHOD__ );
		return $result;
	}
