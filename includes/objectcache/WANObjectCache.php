...
abstract class WANObjectCache {
	 ...
	 *   - b) float (positive); if the key exists and has a TTL
	 *   - c) float (negative); if the key is still tombstoned by delete()
	 *   - d) null; if the key does not exist and is not tombstoned by delete()
	 ...
	 * the timestamp of $key, then $ctl will be set to how long ago that $cKey
	 * was invalidated. Callers can use $ctl to know when the value is stale.
	 *
		...
		foreach ( $cKeys as $cKey ) {
			...
			if ( isset( $wVals[$cKey] )
				...
				if ( (float)$m[1] > $valTimestamp ) {
					$ctl = min( $ctl, $now - (float)$m[1] ); // stale
				}
			} else {
	 ...
	 *   - lockTSE : if there is no key available or it expired less (by $cKeys)
