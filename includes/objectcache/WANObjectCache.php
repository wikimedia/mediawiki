...
abstract class WANObjectCache {
	...
	final public function delete( $key, $ttl = self::HOLDOFF_TTL ) {
		...
		return $this->purge( $key, $ttl ) && $ok;
	}

	/**
	 * Do the actual bus-purge for a key for delete()
	 *
	 * @return bool Success
	 */
	abstract protected function purge( $key, $ttl );
	...
	final public function purgeDependencies( $key ) {
		return $this->delete( "WOC:c:$key", 365 * 86400 );
	}
	...
	final public function getWithCallback(
		...
		if ( $lock === 'lock' ) {
			...
			if ( $value !== false ) {
				...
				$value = $this->getAndWaitRaw( "WOC:s:$key", $timeout );
		...
		if ( $regen ) {
			...
			if ( $value !== false ) {
				...
				if ( $lock === 'lock' ) {
					$this->cache->set( "WOC:s:$key", $value, $timeout + 2 );
				}
