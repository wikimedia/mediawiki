...
abstract class WANObjectCache {
	...
	final public function getMulti(
		...
		foreach ( $checkKeys as $checkKey ) {
			...
			if ( !is_float( $timestamp ) ) {
				...
				$this->cache->add( $checkKey, self::PURGE_VAL_PREFIX . $now, self::CHECK_KEY_TTL );
				$timestamp = $now;
			}
	...
	final public function touchCheckKey( $key ) {
		...
		// Update the local cluster immediately
		$ok = $this->cache->set( $key, self::PURGE_VAL_PREFIX . microtime( true ), self::CHECK_KEY_TTL );
		// Publish the purge to all clusters
	 ...
	 * @return bool Success
	 */
	final protected function relayPurge( $key, $ttl ) {
		$ok = $this->doRelayPurge( $key, $ttl );
		if ( !$ok ) {
	...
	/**
	 * @see WANObjectCache::relayPurge()
	 * @param string $key
	 * @param integer $ttl
	 * @return bool
	 */
	abstract protected function doRelayPurge( $key, $ttl );
