...
class MultiWriteBagOStuff extends BagOStuff {
	...
	/** @var bool Async callback is registered */
	protected $callbackSet = false;
	/** @var array[] */
	 ...
	 *                  BagOStuff objects can also be used as keys.
	...
	protected function doWrite( $method /*, ... */ ) {
		...
		foreach ( $this->caches as $i => $cache ) {
			if ( $i == 0 || !$this->asyncReplication ) {
				...
				$this->queueWrite( $cache, $method, $args );
			}
		}
	 ...
	 * @param array $args
	 */
	protected function queueWrite( BagOStuff $cache, $method, array $args ) {
		$this->asyncWrites[] = array( $cache, $method, $args );
		if ( !$this->callbackSet ) {
			$this->callbackSet = true;

			$that = $this;
			DeferredUpdates::addCallableUpdate( function() use ( $that ) {
				$that->flushAsyncWritesInternal();
			} );
		}
	}
	 ...
	 *
	 * Do not call this method outside of MultiWriteBagOStuff
	 */
	public function flushAsyncWritesInternal() {
		$this->callbackSet = false;

		foreach ( $this->asyncWrites as $write ) {
			list( $cache, $method, $args ) = $write;
			if ( !call_user_func_array( array( $cache, $method ), $args ) ) {
				$this->logger->warning( "Async $method op failed" );
			}
		}
