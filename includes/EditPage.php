...
class EditPage {
	...
	protected function getSummaryPreview( $isSubjectPreview, $summary = "" ) {
		...
		if ( $isSubjectPreview ) {
			$summary = $this->context->msg( 'newsectionsummary' )->rawParams( $wgParser->stripSectionName( $summary ) )
				->inContentLanguage()->text();
		}
	...
	public static function getCopyrightWarning( $title, $format = 'plain' ) {
		...
		if ( $wgRightsText ) {
			$copywarnMsg = [ 'copyrightwarning',
				'[[' . $this->context->msg( 'copyrightpage' )->inContentLanguage()->text() . ']]',
				$wgRightsText ];
		} else {
		...
		return "<div id=\"editpage-copywarn\">\n" .
			call_user_func_array( '$this->context->msg', $copywarnMsg )->$format() . "\n</div>";
	}
		...
		if ( $wgRawHtml && !$this->mTokenOk ) {
			...
			if ( $this->textbox1 !== '' ) {
				...
				$parsedNote = $wgOut->parse( "<div class='previewnote'>" .
					$this->context->msg( 'session_fail_preview_html' )->text() . "</div>", true, /* interface */true );
			}
	...
	static function getEditToolbar( $title = null ) {
		...
		$toolarray = [
			[
				...
				'open'   => '\'\'\'',
				'close'  => '\'\'\'',
				'sample' => $this->context->msg( 'bold_sample' )->text(),
				'tip'    => $this->context->msg( 'bold_tip' )->text(),
			],
			[
				'id'     => 'mw-editbutton-italic',
				'open'   => '\'\'',
				'close'  => '\'\'',
				'sample' => $this->context->msg( 'italic_sample' )->text(),
				'tip'    => $this->context->msg( 'italic_tip' )->text(),
			],
			[
				'id'     => 'mw-editbutton-link',
				'open'   => '[[',
				'close'  => ']]',
				'sample' => $this->context->msg( 'link_sample' )->text(),
				'tip'    => $this->context->msg( 'link_tip' )->text(),
			],
			[
				'id'     => 'mw-editbutton-extlink',
				'open'   => '[',
				'close'  => ']',
				'sample' => $this->context->msg( 'extlink_sample' )->text(),
				'tip'    => $this->context->msg( 'extlink_tip' )->text(),
			],
			[
				'id'     => 'mw-editbutton-headline',
				'open'   => "\n== ",
				'close'  => " ==\n",
				'sample' => $this->context->msg( 'headline_sample' )->text(),
				'tip'    => $this->context->msg( 'headline_tip' )->text(),
			],
			$imagesAvailable ? [
				'id'     => 'mw-editbutton-image',
				'open'   => '[[' . $wgContLang->getNsText( NS_FILE ) . ':',
				'close'  => ']]',
				'sample' => $this->context->msg( 'image_sample' )->text(),
				'tip'    => $this->context->msg( 'image_tip' )->text(),
			] : false,
			$imagesAvailable ? [
				'id'     => 'mw-editbutton-media',
				'open'   => '[[' . $wgContLang->getNsText( NS_MEDIA ) . ':',
				'close'  => ']]',
				'sample' => $this->context->msg( 'media_sample' )->text(),
				'tip'    => $this->context->msg( 'media_tip' )->text(),
			] : false,
			[
				'id'     => 'mw-editbutton-nowiki',
				'open'   => "<nowiki>",
				'close'  => "</nowiki>",
				'sample' => $this->context->msg( 'nowiki_sample' )->text(),
				'tip'    => $this->context->msg( 'nowiki_tip' )->text(),
			],
			$showSignature ? [
				'id'     => 'mw-editbutton-signature',
				'open'   => $this->context->msg( 'sig-text', '~~~~' )->inContentLanguage()->text(),
				'close'  => '',
				'sample' => '',
				'tip'    => $this->context->msg( 'sig_tip' )->text(),
			] : false,
			[
				'id'     => 'mw-editbutton-hr',
				'open'   => "\n----\n",
				'close'  => '',
				'sample' => '',
				'tip'    => $this->context->msg( 'hr_tip' )->text(),
			]
		];
