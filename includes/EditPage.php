...
class EditPage {
	...
	function internalAttemptSave( &$result, $bot = false ) {
		...
		// Hack for T87645
		$sanityNs = Title::newFromText( 'User:Foo' )->getNamespace();
		if ( $sanityNs !== 2 ) {
			wfDebugLog( 'T87645', "Parsing of canonical namespace failed (claims 'User:Foo' is {$sanityNs})" );
		...
		if ( $this->mTitle->getNamespace() === 0 ) {
			...
			if ( $colonPos !== false ) {
				...
				if ( in_array( $pseudoNs, array(
					...
					wfDebugLog( 'T87645', 'Attempting to save title in invalid pseudo-namespace' );
