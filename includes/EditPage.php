...
class EditPage {
	...
	function importFormData( &$request ) {
		...
		} elseif ( $request->getBool( 'safemode' ) ) {
			# "safemode" is a parameter used by old versions of MediaWiki when armoring
			# non-ASCII characters against broken browsers. It is no longer supported.
			wfDebug( __METHOD__ . ": Refusing to process data from old form.\n" );
			$this->oldForm = true;
		} else {
			$wasPosted = true;
		}
	...
	protected function showHeader() {
		...
		if ( $this->isConflict ) {
			...
			if ( $this->oldForm ) {
				$wgOut->addWikiMsg( 'edit_form_old' );
			}
