...
class AvroFormatter implements FormatterInterface {
	...
	protected function getSchema( $channel ) {
		...
		} elseif ( isset( $this->schemas[$channel]['protocol'] ) 
				&& $this->schemas[$channel]['protocol']
		) {
			$protocol = AvroProtocol::parse( $this->schemas[$channel]['schema'] );
			$schema = $protocol->schemata->schema( $channel );
			if ( $schema === null ) {
				trigger_error( "Named schema not included in protocol: " .
