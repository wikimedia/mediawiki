...
class AvroFormatter implements FormatterInterface {
	/**
	 * @var mixed[] Map from schema name to schema definition
	 */
	protected $schemas;
	...
	public function format( array $record ) {
		...
		if ( $schema === null ) {
			trigger_error( "The schema for channel '$channel' is not available" );
			return null;
		}
		try {
			$this->writer->write_data( $schema, $record['context'], $this->encoder );
		} catch ( AvroIOTypeException $e ) {
			trigger_error( "Avro failed to serialize record for {$record['channel']}" );
			return null;
		}
