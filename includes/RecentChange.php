...
class RecentChange {
	 ...
	 *
	 * @param int $type Change type to represent
	 * @return string
	 */
	...
	public function notifyLiveFeeds() {
		...
		foreach ( $wgRCLiveFeeds as $feedID => $feed ) {
			$engine = new $feed['engine']();
			$line = $engine->getLine( $feed, $this, $this->mExtras['actionCommentIRC'] or null );
			...
			$engine->send( $line, $feed, $prefix );
		}
	}

	/**
	 * @deprecated since 1.22, moved to UDPRCFeedEngine
	 */
	public static function cleanupForIRC( $text ) {
		return UDPRCFeedEngine::cleanupForIRC( $text );
	}
