...
class RecentChange {
	...
	public static function typeToString( $type ) {
		$map = array(
			...
			RC_MOVE_OVER_REDIRECT => 'move over redirect'
		);
	...
	public function notifyLiveFeeds() {
		...
		foreach ( $wgRCLiveFeeds as $feed ) {
			...
			if (
				...
				file_put_contents( "/var/www/MediaWiki/Git/core/debug.txt", "continue!?\n", FILE_APPEND );
				continue;
			}

			$formatter = new $feed['formatter']();
			$line = $formatter->getLine( $feed, $this, $actionComment );
			file_put_contents( "/var/www/MediaWiki/Git/core/debug.txt", "$line\n", FILE_APPEND );
