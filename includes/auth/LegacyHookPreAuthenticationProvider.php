...
class LegacyHookPreAuthenticationProvider extends AbstractAuthenticationProvider implements PreAuthenticationProvider {
	...
	public function testForAuthentication( AuthManager $manager, array $reqs ) {
		...
		if ( !Hooks::run( 'LoginUserMigrated', array( $user, &$msg ) ) ) {
			return $this->makeFailResponse( $user, LoginForm::USER_MIGRATED, $msg, 'LoginUserMigrated' );
		}
	 ...
	 *   into such.
	 * @param AuthenticationRequest[] $reqs Keys are class names
	 * @param bool $autocreation True if this is an auto-creation attempt
	 * @return Status
	 */
	public function testForAccountCreation( AuthManager $manager, $user, array $reqs, $autocreation ) {
		$this->request = $manager->getRequest();
	...
	public function testForAutoCreation( AuthManager $manager, $user ) {
		...
		if ( !Hooks::run( 'AbortAutoAccount', array( $user, &$abortError ) ) ) {
			...
			return $this->makeFailResponse( $user, LoginForm::ABORTED, $msg, 'AbortAutoAccount' );
		}
	 ...
	 */
	protected function makeFailResponse( $user, $constant, $msg, $hook ) {
		...
		switch ( $constant ) {
			...
			case LoginForm::CREATE_BLOCKED:
				...
				/// @todo: Does using RequestContext::getMain()->getUser() actually make sense here?
