...
class LegacyHookPreAuthenticationProvider extends AbstractAuthenticationProvider implements PreAuthenticationProvider {
	...
	public function testForAccountCreation( $user, $creator, array $reqs ) {
		...
		if ( !Hooks::run( 'AbortNewAccount', array( $user, &$abortError, &$abortStatus ) ) ) {
			...
			if ( $abortStatus === null ) {
				...
				return StatusValue::newFatal( 'createaccount-hook-aborted', $abortError );
			} else {
	...
	public function testForAutoCreation( $user ) {
		...
		if ( !Hooks::run( 'AbortAutoAccount', array( $user, &$abortError ) ) ) {
			...
			return $this->makeFailResponse( $user, $user, LoginForm::ABORTED, $abortError, 'AbortAutoAccount' );
		}
	...
	protected function makeFailResponse( $user, $creator, $constant, $msg, $hook ) {
		switch ( $constant ) {
			...
			case LoginForm::USER_BLOCKED:
				return StatusValue::newFatal( $msg ?: 'login-userblocked', wfEscapeWikiText( $user->getName() ) );

			case LoginForm::ABORTED:
				return StatusValue::newFatal( $msg ?: 'login-abort-generic', wfEscapeWikiText( $user->getName() ) );
