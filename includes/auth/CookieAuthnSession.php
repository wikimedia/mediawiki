...
class CookieAuthnSession extends AuthnSession {
	...
	public function suggestLoginUsername() {
		...
		return $request->getCookie( 'UserName', $prefix );
	}
	...
	/**
	 * Return the data to store in cookies and $_SESSION
	 * @return array Array( array $cookies, array $session )
	 */
	protected function sessionDataToExport() {
		...
		if ( $this->userInfo['UserID'] === 0 ) {
			...
			if ( $this->params['callUserSetCookiesHook'] ) {
	...
	protected function storeSessionData( $key, $logout = false ) {
		...
		list( $cookies, $session ) = $this->sessionDataToExport();
