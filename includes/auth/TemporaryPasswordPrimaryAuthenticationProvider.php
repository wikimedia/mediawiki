...
class TemporaryPasswordPrimaryAuthenticationProvider extends AbstractPasswordPrimaryAuthenticationProvider {
	...
	public function beginAuthentication( AuthManager $manager, array $reqs ) {
		...
		$row = $dbw->selectRow(
			'user',
			array( 'user_id', 'user_newpassword', 'user_newpass_time' ),
		...
		if ( $req instanceof TemporaryPasswordAuthenticationRequest && $req->username !== null ) {
			...
			$dbw->update(
				...
				array(
					...
					'user_newpass_time' => $db->timestamp(),
				),
				array( 'user_name' => $req->username ),
				__METHOD__
			);
	 ...
	 * @return Status
	 */
	public function testForAccountCreation( AuthManager $manager, $user, $creator, array $reqs ) {
	 ...
	 * @return AuthenticationResponse
	 */
	public function beginAccountCreation( AuthManager $manager, $user, array $reqs ) {
		if ( isset( $reqs['TemporaryPasswordAuthenticationRequest'] ) ) {
			$req = $reqs['TemporaryPasswordAuthenticationRequest'];
	...
	public function finishAccountCreation(
		AuthManager $manager, $user, AuthenticationResponse $res
	) {
