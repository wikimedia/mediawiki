...
class AuthFrontend {
	 ...
	 * FIXME should this be in AuthenticationRequest instead?
	...
	public static function getUsernameFromRequests( $requests ) {
		...
		foreach ( $requests as $req ) {
			if ( array_key_exists( 'username', $req->getFieldInfo() ) ) {
				if ( $username === null ) {
					$username = $req->username;
					$otherClass = get_class( $req );
				} elseif ( $username !== $req->username ) {
					$requestClass = get_class( $req );
					throw new \LogicException( "conflicting username fields: '{$req->username}' from "
	 ...
	 * @throws \Exception
	 */
	public static function mergeFieldInfo( array $requests ) {
		$merged = array();
		foreach ( $requests as $req ) {
