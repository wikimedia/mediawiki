	...
	protected function continueLinkAttempt( $user, $key, array $reqs ) {
		...
		foreach ( $req->confirmedLinkIDs as $id ) {
			if ( isset( $maybeLink[$id] ) ) {
				$status = $this->manager->allowsAuthenticationDataChange( $maybeLink[$id] );
				$statuses[] = array( $maybeLink[$id], $status );
				if ( $status->isGood() ) {
		...
		return AuthenticationResponse::newUI(
			array( new ButtonAuthenticationRequest( 'linkOk', wfMessage( 'ok' ) ) ),
