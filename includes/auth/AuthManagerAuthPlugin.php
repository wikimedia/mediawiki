...
class AuthManagerAuthPlugin extends \AuthPlugin {
	...
	public function authenticate( $username, $password ) {
		...
		$reqs = AuthManager::singleton()->getAuthenticationRequests(
			...
			\Title::makeTitle( NS_SPECIAL, 'Badtitle/AuthManagerAuthPlugin can\'t redirect' )->getFullUrl()
		);
	...
	public function addUser( $user, $password, $email = '', $realname = '' ) {
		...
		$res = AuthManager::singleton()->beginAccountCreation(
			$user->getName(), $wgUser, $reqs, 'null:'
		);
		switch ( $res->status ) {
			case AuthenticationResponse::PASS:
				return true;
