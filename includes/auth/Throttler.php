...
class Throttler implements LoggerAwareInterface {
	...
	/**
	 * Return the default instance, reading the config from app settings.
	 * @return Throttler
	 */
	public static function getInstance() {
		if ( !self::$instance ) {
			$config = \ConfigFactory::getDefaultInstance()->makeConfig( 'main' );
			$throttleConditions = $config->get( 'PasswordAttemptThrottle' );
			self::$instance = new static( [
				'type' => 'password',
				'conditions' => $throttleConditions,
				'cache' => \ObjectCache::getLocalClusterInstance(),
			] );
			self::$instance->setLogger( LoggerFactory::getInstance( 'throttler' ) );
		}
		return self::$instance;
	}

	/**
	 * Like the constructor, but sensible defaults are used.
	 * @param array $conditions
	 * @param array $params
	 * @return Throttler
	 */
	public static function newFromConditions( array $conditions, array $params = [] ) {
		...
		$instance->setLogger( LoggerFactory::getInstance( 'throttler' ) );
		return $instance;
	}

	/**
	 * @param array $params Parameters (all required):
	 *   - type: throttle type, used as a namespace for counters
	 *   - conditions: an array of arrays describing throttling conditions, see documentation of
	 *     $wgPasswordAttemptThrottle for format.
	...
	protected function logRejection( array $context ) {
		...
		$logMsg = 'Login attempt rejected because logins to {username} from IP {ip} have been '
			. 'throttled for {expiry} seconds due to {count} failed attempts';
