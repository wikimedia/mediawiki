...
final class AuthManager implements LoggerAwareInterface {
	...
	public function getAuthenticationRequests( $action ) {
		...
		return $this->getAuthenticationRequestsInternal( $providerAction, $options, $providers );
	}
	...
	public function getAuthenticationRequestsInternal(
		$providerAction, array $options, array $providers
	) {
		...
		if ( $providerAction === self::ACTION_CREATE ) {
			$reqs[] = new UserDataAuthenticationRequest;
		}

		// Fill in reqs data
		$reqAction = isset( self::$actionContinueMap[$providerAction] )
			? self::$actionContinueMap[$providerAction]
			: $providerAction;
		foreach ( $reqs as $req ) {
