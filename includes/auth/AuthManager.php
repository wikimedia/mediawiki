...
final class AuthManager implements LoggerAwareInterface {
	...
	private static $actionContinueMap = array(
	...
	public function beginAuthentication( array $reqs, $returnToUrl ) {
		...
		$req = AuthenticationRequest::getRequestByClass(
			$reqs, 'MediaWiki\\Auth\\CreatedAccountAuthenticationRequest'
		);
		if ( $req ) {
		...
		$req = AuthenticationRequest::getRequestByClass(
			$reqs, 'MediaWiki\\Auth\\CreateFromLoginAuthenticationRequest'
		);
		if ( $req ) {
	 ...
	 * @return AuthenticationResponse
	 */
	public function beginAccountCreation( $username, User $creator, array $reqs, $returnToUrl ) {
		$session = $this->request->getSession();
		...
		if ( $req ) {
			...
			if ( !$reqs ||
				count( $reqs ) === 1 && reset( $reqs ) instanceof UserDataAuthenticationRequest
			) {
				// No reqs needed, so we can just continue.
				$req->createRequest->returnToUrl = $returnToUrl;
				$reqs = array( $req->createRequest );
			} else {
				// Needs some reqs, so request them
				$reqs[] = new CreateFromLoginAuthenticationRequest( $req->createRequest, array() );
	...
	public function continueAccountCreation( array $reqs ) {
		...
		try {
			...
			// to extract the createRequest (if any) and call the
			// pre-authentication providers since we didn't do it before.
			$req = AuthenticationRequest::getRequestByClass(
				$reqs, 'MediaWiki\\Auth\\CreateFromLoginAuthenticationRequest'
			);
			if ( $req ) {
				...
				$creator = $state['creatorid']
					? \User::newFromId( $state['creatorid'] )
					: \User::newFromName( $state['creatorname'] );
				$providers = $this->getPreAuthenticationProviders() +
					$this->getPrimaryAuthenticationProviders() +
					$this->getSecondaryAuthenticationProviders();
				foreach ( $providers as $id => $provider ) {
	...
	public function getAuthenticationRequests( $action ) {
		...
		switch ( $action ) {
			...
			case self::ACTION_LOGIN_CONTINUE:
				...
				return is_array( $state ) ? $state['continueRequests'] : array();
			...
			case self::ACTION_LINK_CONTINUE:
				$state = $this->request->getSessionData( 'AuthManager::accountLinkState' );
				return is_array( $state ) ? $state['continueRequests'] : array();
		...
		return $this->getAuthenticationRequestsInternal( $action, $providerAction, $options, $providers );
	}

	/**
	 * Internal request lookup for self::getAuthenticationRequests
	 *
	 * @param string $action Action from self::getAuthenticationRequests
	 ...
	 * @return AuthenticationRequest[]
	 */
	public function getAuthenticationRequestsInternal(
		$action, $providerAction, array $options, array $providers
	) {
		...
		// AuthManager has its own req for account creation
		if ( $action === self::ACTION_CREATE ) {
			$reqs[] = new UserDataAuthenticationRequest;
		}

		// Fill in reqs data
		$reqAction = isset( self::$actionContinueMap[$action] )
			? self::$actionContinueMap[$action]
			: $action;
		foreach ( $reqs as $req ) {
			$req->action = $reqAction;
		...
		if ( $providerAction === self::ACTION_CHANGE || $providerAction === self::ACTION_REMOVE ) {
			$that = $this;
			$reqs = array_filter( $reqs, function ( $req ) use ( $that ) {
				return $that->allowsAuthenticationDataChange( $req, false )->isGood();
			} );
		}
