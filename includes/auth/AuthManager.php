...
final class AuthManager implements LoggerAwareInterface {
	...
	/** @var AuthenticationSession|null */
	private $session;

	/** @var LoggerInterface */
	private $logger;

	/** @var AuthenticationSessionProvider[] */
	...
	public function getSession() {
		if ( !$this->session ) {
			...
			foreach ( $this->getSessionProviders() as $provider ) {
				$possibleSession = $provider->provideAuthenticationSession( $this->request );
				if ( !$possibleSession ) {
					continue;
				}
				...
				if ( $possibleSession->canSetSessionUserInfo() ) {
					...
					if ( !$this->checkUserInfo( $id, $name, $token ) ) {
						...
						$possibleSession = $provider->provideAuthenticationSession( $this->request, true );
						if ( !$possibleSession ) {
							continue;
						}
					}
			...
			if ( $session === null ) {
				$this->logger->warning( 'No AuthenticationSession was provided! Using a null session.' );
	...
	public function continueAuthentication( array $reqs ) {
		try {
			...
			// Step 4: Authentication complete! Mark the AuthenticationSession as
			// authenticated and clean up.
	 ...
	 *
	 * Either via a PrimaryAuthenticationProvider or via an immutable
	...
	public function userCanAuthenticate( $username ) {
		...
		foreach ( $this->getSessionProviders() as $provider ) {
			if ( $provider->immutableSessionCouldExistForUser( $username ) ) {
				return true;
			}
		}
	...
	public function revokeAccessForUser( $username ) {
		...
		foreach ( $this->getSessionProviders() as $provider ) {
			$provider->preventImmutableSessionsForUser( $username );
		}

		$this->invalidateAuthenticationToken( $username );
	}
	...
	private function getSessionStore() {
		...
		if ( !$config->get( 'SessionsInObjectCache' ) ) {
