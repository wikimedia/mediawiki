...
final class AuthManager implements LoggerAwareInterface {
		...
		try {
			...
			if ( $state['primary'] === null ) {
				...
				foreach ( $this->getPrimaryAuthenticationProviders() as $id => $provider ) {
					$res = $provider->beginAuthentication( $reqs );
					switch ( $res->status ) {
				...
				$res = $provider->continueAuthentication( $reqs );
				switch ( $res->status ) {
					case AuthenticationResponse::PASS;
						$state['primaryResponse'] = $res;
						break;
			...
			if ( $user->getId() === 0 ) {
				...
				$user = User::newFromName( $res->username, 'usable' );
				if ( $user->getId() === 0 ) {
					throw new UnexpectedValueException( 'Autocreate succeeded, but ID is still 0' );
				}
			}
		...
		try {
			...
			if ( $state['primary'] === null ) {
				...
				foreach ( $this->getPrimaryAuthenticationProviders() as $id => $provider ) {
					...
					$res = $provider->beginAccountCreation( $user, $reqs );
					switch ( $res->status ) {
						case AuthenticationResponse::PASS;
							$state['primary'] = $id;
							$state['primaryResponse'] = $res;
			...
			if ( $state['primaryResponse'] === null ) {
				$res = $provider->continueAccountCreation( $user, $reqs );
				switch ( $res->status ) {
					case AuthenticationResponse::PASS;
						$state['primaryResponse'] = $res;
						break;
			...
			if ( $state['userid'] === 0 ) {
				...
				// Inform the provider
				$provider->finishAccountCreation( $user, $reqs, $state['primaryResponse'] );
			}
	...
	public function autoCreateAccount( $username, User $creator ) {
		...
		$providers = $this->getPreAuthenticationProviders() +
			...
			$this->getSecondaryAuthenticationProviders();
		foreach ( $providers as $provider ) {
			$status = $provider->testForAutoCreation( $user, $creator );
			if ( !$status->isGood() ) {
				return $status;
			}
		}
