...
final class AuthManager implements LoggerAwareInterface {
	...
	public function beginAuthentication( array $reqs ) {
		...
		if ( !$session->canSetSessionUserInfo() ) {
			...
			unset( $_SESSION['AuthManager::authnState'] );
		...
		self::persistSession();
		$_SESSION['AuthManager::authnState'] = array(
			'reqs' => $reqs,
			'primary' => null,
	...
	public function continueAuthentication( array $reqs ) {
		...
		if ( !isset( $_SESSION['AuthManager::authnState'] ) ) {
			return AuthenticationResponse::newFail(
				wfMessage( 'authmanager-authn-not-in-progress' )
			);
		}
		$state = &$_SESSION['AuthManager::authnState'];
		...
		if ( $state['primary'] === null ) {
			...
			$provider = $this->getAuthenticationProvider( $providers[$id] );
		...
		if ( $res->username === null ) {
			...
			if ( $provider->accountCreationType() === PrimaryAuthenticationProvider::TYPE_LINK ) {
				$_SESSION['AuthManager::maybeLink'][] = $res->createReq;
		...
		if ( $user->getId() === 0 ) {
			...
			if ( !Hooks::run( 'AbortAutoAccount', array( $user, &$abortError ) ) ) {
				...
				wfDebug( __METHOD__ . ": a hook blocked creation: $abortError\n" );
				unset( $_SESSION['AuthManager::authnState'] );
				return AuthenticationResponse::newFail(
					wfMessage(
		...
		foreach ( $this->getSecondaryAuthenticationProviders() as $id => $provider ) {
			if ( !isset( $state['secondary'][$id] ) ) {
				...
				$res = $provider->continueSecondaryAuthentication(
					$this, $user, array_merge( $beginReqs, $reqs )
				);
			} else {
				continue;
			}
		...
		if ( $session->getAuthenticationRequestType() ) {
			$req = self::findRequestByType(
				$state['reqs'], $session->getAuthenticationRequestType()
			);
		} else {
			$req = null;
		}
		$session->resetSessionKey();
		$session->setSessionUserInfo( $id, $name, $token, $req );
	...
	public function allowChangingAuthenticationType( $type ) {
		...
		// * Pass $type to each one, return false if false
		// * Return true
	}
	...
	public function canChangeAuthenticationData( AuthenticationRequest $req  ) {
		...
		// * Instantiate list of SecondaryAuthenticationProviders
		// * Pass $req to each one, return $status if not good
		// * Return good
	}
	...
	public function getAuthenticationRequestTypes( $which ) {
		...
		// change: Get the lists of all PrimaryAuthenticationProviders, then
		//   filter out any for which a provider returns false for
		//   allowChangingAuthenticationType()
	...
	/**
	 * Find a request of a specific type in an array
	 * @param AuthenticationRequest[] $reqs
	 * @param string $type
	 * @return AuthenticationRequest|null
	 */
	public static function findRequestByType( array $reqs, $type ) {
		foreach ( $reqs as $req ) {
			if ( is_a( $req, $type ) ) {
				return $req;
			}
	...
	/**
	 * Find all request of a specific type in an array
	 * @param AuthenticationRequest[] $reqs
	 * @param string $type
	 * @return AuthenticationRequest[]
	 */
	public static function findRequestsByType( array $reqs, $type ) {
		return array_filter( $reqs, function ( $req ) use ( $type ) {
			return is_a( $req, $type );
		} );
	}
