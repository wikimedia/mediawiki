...
final class AuthManager implements LoggerAwareInterface {
	...
	public function resetSessionId() {
		...
		$oldSessionId = $session->getSessionKey();
	...
	public function beginAuthentication( array $reqs ) {
		...
		foreach ( $this->getPreAuthenticationProviders() as $provider ) {
			$status = $provider->testForAuthentication( $this, $reqs );
			if ( !$status->isGood() ) {
	...
	public function continueAuthentication( array $reqs ) {
		try {
			...
			if ( $user->getId() === 0 ) {
				...
				$status = $this->autoCreateAccount( $res->username );
				if ( !$status->isGood() ) {
					$this->request->setSessionData( 'AuthManager::authnState', null );
					return AuthenticationResponse::newFail(
						Status::wrap( $status )->newMessage( 'authmanager-authn-autocreate-failed' )
					);
				}
			}
	 ...
	 * @return AuthenticationResponse
	 */
	public function beginAccountCreation( $username, array $reqs ) {
		...
		foreach ( $providers as $provider ) {
			$status = $provider->testForAccountCreation( $this, $user, $reqs );
			if ( !$status->isGood() ) {
				return AuthenticationResponse::newFail(
					Status::wrap( $status )->getMessage()
				);
		...
		try {
			...
			if ( $state['primary'] === null ) {
				...
				foreach ( $this->getPrimaryAuthenticationProviders() as $id => $provider ) {
					...
					$res = $provider->beginAccountCreation( $this, $user, $reqs );
					switch ( $res->status ) {
						case AuthenticationResponse::PASS;
							$state['primary'] = $id;
							$state['primaryResponse'] = $res;
						...
						default:
							throw new DomainException(
								get_class( $provider ) . "::beginAccountCreation() returned $res->status"
							);
					}
				}
				if ( $state['primary'] === null ) {
			...
			if ( $state['primaryResponse'] === null ) {
				$res = $provider->continueAccountCreation( $this, $user, $reqs );
				switch ( $res->status ) {
					case AuthenticationResponse::PASS;
						$state['primaryResponse'] = $res;
						break;
					...
					default:
						throw new DomainException(
							get_class( $provider ) . "::continueAccountCreation() returned $res->status"
						);
				}
			}
			...
			if ( $state['userid'] === 0 ) {
				...
				// Inform the provider
				$provider->finishAccountCreation( $this, $user, $reqs, $state['primaryResponse'] );
			}
			...
			foreach ( $this->getSecondaryAuthenticationProviders() as $id => $provider ) {
				if ( !isset( $state['secondary'][$id] ) ) {
					...
					$res = $provider->beginSecondaryAccountCreation( $this, $user, $beginReqs );
				} elseif ( !$state['secondary'][$id] ) {
					$func = 'continueSecondaryAccountCreation';
					$res = $provider->continueSecondaryAccountCreation( $this, $user, $reqs );
				} else {
					continue;
				}
				switch ( $res->status ) {
			...
			$this->createdAccountAuthenticationRequest[] = $req;
			return $ret;
		} catch ( Exception $ex ) {
			$this->request->setSessionData( 'AuthManager::accountCreationState', null );
			throw $ex;
	 ...
	 * @return StatusValue
	 */
	public function autoCreateAccount( $username ) {
		if ( !$this->userExists( $username ) ) {
		...
		$providers = $this->getPrimaryAuthenticationProviders() +
			$this->getSecondaryAuthenticationProviders();
		foreach ( $providers as $provider ) {
			$provider->autoCreatedAccount( $this, $user );
		}
