...
final class AuthManager implements LoggerAwareInterface {
	...
	public function beginAccountCreation( $username, User $creator, array $reqs ) {
		...
		foreach ( $providers as $provider ) {
			$status = $provider->testForAccountCreation( $this, $user, $creator, $reqs );
			if ( !$status->isGood() ) {
				return AuthenticationResponse::newFail(
					Status::wrap( $status )->getMessage()
				);
	 ...
	 * @return StatusValue
	 */
	public function autoCreateAccount( $username, User $creator ) {
		if ( !$this->userExists( $username ) ) {
		...
		$providers = $this->getPreAuthenticationProviders() +
			...
			$this->getSecondaryAuthenticationProviders();
		foreach ( $providers as $provider ) {
			$status = $provider->testForAutoCreation( $this, $user, $creator );
			if ( !$status->isGood() ) {
				return $status;
			}
		}
