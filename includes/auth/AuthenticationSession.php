 ...
 *
 * An AuthenticationSession represents the fact that the current HTTP request
 * is part of a session. There are two broad types of AuthenticationSessions,
 * based on whether they return true or false from self::canSetSessionUserInfo():
 * * When true (mutable), the AuthenticationSession identifies multiple
 *   requests as part of a session generically, with no tie to a particular
 *   user. In this case AuthManager can use the PrimaryAuthenticationProviders
 *   to determine the authenticated user and then tell the
 *   AuthenticationSession about that user for use in future requests in the
 *   session.
 * * When false (immutable), the AuthenticationSession identifies multiple
 *   requests as part of a session by identifying and authenticating the
 *   request itself as belonging to a particular user. In this case AuthManager
 *   cannot change the associated user, and so will refuse attempts to
 *   reauthenticate.
 *
 * An example of the first type is CookieAuthenticationSession, which stores
 * the user information in cookies. An example of the second type could be
 * an OAuthAuthenticationSession, where the request is known to be part of a
 * session by the fact that it has valid OAuth headers that also identify the
 * user making the request.
 *
 * The AuthenticationSession is also responsible for managing the data behind
 * $_SESSION. This base class handles most of the details, so a subclass will
 * generally just have to manage the key returned by self::getSessionKey(). An
 * immutable AuthenticationSession might just base the key on the user; a
 * mutable AuthenticationSession might use $_SESSION to store the user info.
 *
 * @ingroup Auth
 * @since 1.26
 */
abstract class AuthenticationSession implements LoggerAwareInterface {
	const MAX_PRIORITY = 100;

	/** @var string|null Session key, if any */
	protected $key;

	/** @var int Session priority (relative to other potential sessions) */
	protected $priority;

	/** @var BagOStuff|null Session data storage */
	protected $store = null;

	/** @var int Session data lifetime, in seconds */
	protected $lifetime = 86400;

	/** @var string|null Hash of current session data, used to avoid unnecessary writes */
	 ...
	 * @param int $priority Priority, see self::getSessionPriority()
	 */
	...
	/**
	 * Returns the unique key for this session.
	 *
	 * Different sessions from the same provider must not share a key.
	 *
	 * If null, the current request isn't part of any known session. If
	 * canResetSessionKey() is false, the key must not be null.
	 *
	 * @return string|null
	 */
	 ...
	 *
	 * When a request might be part of multiple sessions, it will be considered
	 * part of the session with the highest priority value. If multiple
	 * AuthenticationSessions tie for highest priority, an error will be
	 * raised.
	 *
	 * @return int 0 to self::MAX_PRIORITY
	 */
	 ...
	 *
	 * This may actually change the key, or may be a no-op.
	 *
	 ...
	 *
	 * For example, CookieAuthenticationSession will set a new cookie here.
	 *
