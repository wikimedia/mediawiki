...
abstract class AuthnSession implements LoggerAwareInterface {
	...
	public function __construct( BagOStuff $store, LoggerInterface $logger, $key, $priority ) {
		...
		if ( $key === null && !$this->canResetSessionKey() ) {
			throw new RuntimeException( 'Cannot set null key with ' . get_class( $this ) );
		}
	}
	...
	public function setSessionUserInfo( $id, $name, $token, $req ) {
		if ( $this->canSetSessionUserName() ) {
			throw new BadMethodCallException(
				__METHOD__ . ' must be implemented if ' . get_class( $this ) . '::canSetSessionUserName() returns true'
			);
		} else {
			throw new LogicException(
				__METHOD__ . ' was called when ' . get_class( $this ) . '::canSetSessionUserName() returns false'
			);
		}
	}
