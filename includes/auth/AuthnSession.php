...
abstract class AuthnSession implements LoggerAwareInterface {
	...
	/**
	 * If a user should be auto-created, return the AuthenticationRequest to
	 * pass to account creation here.
	 *
	 * Only used if $this->canSetSessionUserInfo() is true,
	 * getSessionUserInfo() returns $id=0 and $name!=null, and the name doesn't
	 * exist locally.
	 *
	 * @return CreateAccountFromTokenAuthenticationRequest|null
	 */
	public function getAutoCreateAuthenticationRequest() {
		return null;
	}
	...
	protected function setNewSessionKey( $newkey ) {
		if ( $this->canResetSessionKey() ) {
			throw new BadMethodCallException(
				__METHOD__ . ' must be implemented if ' . __CLASS__ . '::canResetSessionKey() returns true'
			);
		} else {
			throw new LogicException(
				__METHOD__ . ' was called when ' . __CLASS__ . '::canResetSessionKey() returns false'
			);
		}
	}
	 ...
	 * array( 0, $name, null ) and have
	 * $this->getAutoCreateAuthenticationRequest() return something
	 * appropriate.
	 *
	...
	public function setSessionUserInfo( $id, $name, $token, $req ) {
		if ( $this->canSetSessionUserName() ) {
			throw new BadMethodCallException(
				__METHOD__ . ' must be implemented if ' . __CLASS__ . '::canSetSessionUserName() returns true'
			);
		} else {
			throw new LogicException(
				__METHOD__ . ' was called when ' . __CLASS__ . '::canSetSessionUserName() returns false'
			);
		}
	}
