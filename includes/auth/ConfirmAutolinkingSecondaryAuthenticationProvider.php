...
/**
 * Requires autolinked accounts to be confirmed by the user before they are passed to
 * finishAccountLink(). This prevents "autolink fixation" attacks where the attacker does a partial
 * login (successful authentication but the local account does not exist and cannot be autocreated,
 * so the remote account is stored for autolinking) then leaves the machine with the login form
 * around for the victim to log in (after which the attacker has a linked account with which they
 * can authenticate as the victim).
 *
 * To prevent this, the user needs to confirm all to-be-linked accounts on a checkbox form. The
 * labels are constructed from the data returned by the describe() method of the autolink request.
 *
 * To avoid confusing behavior, this provider should be later in the configuration list than any
 * provider that can abort the authentication process, so that it is only invoked for
	...
	extends AbstractSecondaryAuthenticationProvider
{
	public function getAuthenticationRequests( $action ) {
		switch ( $action ) {
			...
			case AuthManager::ACTION_CREATE_CONTINUE:
				// FIXME this is ugly. Couldn't AuthManager just calculate _CONTINUE calls from saved state?
	...
	public function beginSecondaryAuthentication( $user, array $reqs ) {
		...
		$reqs = array_map( function ( $item ) { return $item['req']; }, $maybeLink );
	...
	public function continueSecondaryAuthentication( $user, array $reqs ) {
		...
		foreach ( $req->getConfirmedAccounts() as $reqId ) {
			if ( isset( $maybeLink[$reqId] ) ) {
				$reallyLink[$reqId] = $maybeLink[$reqId];
			}
		}
		$session->set( 'AuthManager::maybeLink', $reallyLink );
