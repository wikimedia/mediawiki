...
class LocalPrimaryAuthenticationProvider extends AbstractPasswordPrimaryAuthenticationProvider {
	...
	public function beginAuthentication( AuthManager $manager, array $reqs ) {
		...
		return AuthenticationRequest::newPass( $req->username, $req );
	}
	...
	public function canChangeAuthenticationData( AuthenticationRequest $req ) {
		if ( $req instanceof PasswordAuthenticationRequest &&
			...
			if ( $row ) {
				/** @todo: Find the checks for $wgMinPasswordLength and anything else, and copy them here */
	...
	public function beginAccountCreation( AuthManager $manager, array $reqs ) {
		if ( isset( $reqs['PasswordAuthenticationRequest'] ) ) {
			$req = $reqs['PasswordAuthenticationRequest'];
			if ( $this->canChangeAuthenticationData( $req ) ) {
				$this->changeAuthenticationData( $req );
