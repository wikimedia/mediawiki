...
class ResetPasswordSecondaryAuthenticationProvider extends AbstractSecondaryAuthenticationProvider {
	...
	protected function tryReset( array $reqs ) {
		...
		} elseif ( !$data->msg instanceof \MessageSpecifier ) {
		...
		if ( !$data->hard ) {
			$req = AuthenticationRequest::getRequestByClass(
				$reqs, 'MediaWiki\\Auth\\SkipResetPasswordAuthenticationRequest'
			);
			if ( $req ) {
		...
		if ( !$data->hard ) {
			$needReqs[] = new SkipResetPasswordAuthenticationRequest;
		}
