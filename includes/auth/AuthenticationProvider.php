...
interface AuthenticationProvider extends LoggerAwareInterface {
	 ...
	 *    to remove credentials. Called on primary and secondary providers.
	 *
	 * AuthenticationRequests returned for ACTION_REMOVE <b>must not</b> cause
	 * the implementing class's
	 * PrimaryAuthenticationProvider::providerAllowsAuthenticationDataChange()
	 * or SecondaryAuthenticationProvider::providerAllowsAuthenticationDataChange()
	 * to return failure.
	 *
	 * @see AuthManager::getAuthenticationRequests()
	 * @param string $action
	 * @param array $options Options are:
	 *  - username: User name related to the action, or null/unset if anon. This does
	 *    not need to be copied into the returned requests, you only need to
	 *    pay attention to it if the set of requests differs based on the user.
	 *  - returnToUrl: Redirect return-to URL that is going to be used, or
	 *    unset if not relevant. This does not need to be copied into the
	 *    returned requests.
