...
LUA;
	 ...
	 * @return mixed
	 * @todo: move to RedisConnectionPool?
	 */
	protected function redisEval( RedisConnRef $conn, $script, array $params, $numKeys ) {
		...
		if ( $conn->getLastError() && $conn->script( 'exists', $sha1 ) === array( 0 ) ) {
			...
			wfDebugLog( 'redis', __METHOD__ . ": Used eval() for Lua script $sha1." );
		}

		if ( $conn->getLastError() ) { // script bug?
			wfDebugLog( 'redis', __METHOD__ . ": Lua script error: " . $conn->getLastError() );
		}
