...
class SwiftFileBackend extends FileBackendStore {
	...
	protected function doCreateInternal( array $params ) {
		...
		$handler = function( array $request, Status $status ) use ( $be, $method, $params ) {
			list( $rcode, $rhdrs, $rbody, $rerr ) = $request['response'];
			if ( $rcode === 201 ) {
				// good
			} elseif ( $rcode === 412 ) {
	...
	protected function doExecuteOpHandlesInternal( array $fileOpHandles ) {
		...
		for ( $stage = 0; $stage < count( $httpReqsByStage ); ++$stage ) {
			...
			foreach ( $httpReqs as $index => $httpReq ) {
				...
				$fileOpHandles[$index]->callback( $httpReq, $statuses[$index] );
				...
				if ( !$statuses[$index]->isOK() ) {
					$stages = count( $fileOpHandle[$index]->httpOp );
	...
	private function objectListing(
		...
		if ( $after !== null ) {
			$query .= '&marker=' . rawurlencode( $after );
		}
		if ( $prefix !== null ) {
			$query .= '&prefix=' . rawurlencode( $prefix );
		}
		if ( $delim !== null ) {
			$query .= '&delimiter=' . rawurlencode( $delim );
		}

		list( $rcode, $rhdrs, $rbody, $rerr ) = $this->http->run( array(
			'method'  => 'GET',
			'url'     => $this->storageUrl( $auth, $fullCont ) . "?$query",
			'headers' => $this->authTokenHeaders( $auth )
		) );
