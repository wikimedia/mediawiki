...
# for splitting into an independent library, except that is currently
# is highly optimized for MediaWiki use.  It only implements the portions
...
# an true DOM in memory, instead serializing elements to an output string
...
class Balancer {
	 ...
	 * @param bool $allowAllAttributes Don't sanitize attributes, used
	 *   for test cases only.  Defaults to false.
	 */
	public function __construct( $htmlelements, $allowAllAttributes = false ) {
		...
		if ( count( $bad ) > 0 ) {
			...
			throw new ParameterAssertionException(
				'$htmlelements',
				"Balance attempted with unsupported elements: {$badstr}"
			);
		}
	}
	...
	private function advance() {
		...
		$goodtag = $t && isset( $this->allowedHtmlElements[$t] ) &&
			Sanitizer::validateTag( $attribs, $t );
		if ( $goodtag ) {
