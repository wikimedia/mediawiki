...
class InlineDifferenceEngine extends DifferenceEngine {
	public function showDiffStyle() {
		$this->getOutput()->addModuleStyles( 'mediawiki.action.history.diff.inline' );
	}

	protected function getInlineHeader() {
		$prev = $this->prevLink;
		$next = $this->nextLink;
		if ( $prev || $next ) {
			$history = Html::openElement( 'ul', array( 'class' => 'hlist revision-history-links' ) );
			if ( $prev ) {
				$history .= Html::rawElement( 'li', array(), $prev );
			}
			$history .= Html::rawElement( 'li', array(), $this->newRevisionHeader );
			if ( $next ) {
				$history .= Html::rawElement( 'li', array(), $next );
			}
	...
	public function getDiff( $otitle, $ntitle, $notice = '' ) {
		...
		return $this->getInlineHeader() . '<div id="mw-mf-minidiff">' .
			$diff .
			'</div>';
	}
	/**
