...
class DifferenceEngine extends ContextSource {
	...
	public function showDiffPage( $diffOnly = false ) {
		...
		Hooks::run( 'DifferenceEngineShowDiffPage', array( $out ) );
		...
		if ( $this->mOldRev === false ) {
			...
			Hooks::run( 'DifferenceEngineOldHeaderNoOldRev', array( &$oldHeader ) );
		} else {
			...
			// Allow extensions to change the $oldHeader variable
			Hooks::run( 'DifferenceEngineOldHeader', array( $this, &$oldHeader, $prevlink, $oldminor, $diffOnly, $ldel, $this->unhide ) );
		...
		Hooks::run( 'DifferenceEngineNewHeader', array( $this, &$newHeader, $formattedRevisionTools, $nextlink, $rollback, $newminor, $diffOnly, $rdel, $this->unhide ) );
	...
	protected function markPatrolledLink() {
		...
		if ( $this->mMarkPatrolledLink === null ) {
			...
			if (
				...
				Hooks::run( 'DifferenceEngineMarkPatrolledRCID', array( &$rcid, $this, $change, $user ) );
				...
				if ( $rcid ) {
					...
					Hooks::run( 'DifferenceEngineMarkPatrolledLink', array( $this, &$this->mMarkPatrolledLink, $rcid, $token ) );
				} else {
		...
		if ( Hooks::run( 'ArticleContentOnDiff', array( $this, $out ) ) ) {
			...
			if ( $this->mNewPage->isCssJsSubpage() || $this->mNewPage->isCssOrJsPage() ) {
				...
				if ( $parserOutput ) {
					...
					if ( !Hooks::run( 'DifferenceEngineRenderRevisionAddParserOutput', array( $this, $out, $parserOutput, $wikiPage ) ) ) {
	...
	public function showDiff( $otitle, $ntitle, $notice = '' ) {
		...
		Hooks::run( 'DifferenceEngineShowDiff', array( $this ) );
	...
	public function getDiffBody() {
		...
		if ( $this->mOldRev === false || ( $this->mOldRev && $this->mNewRev
			...
			if ( !Hooks::run( 'DifferenceEngineShowEmptyOldContent', array( $this ) ) ) {
				return '';
			}
		}
		// Cacheable?
