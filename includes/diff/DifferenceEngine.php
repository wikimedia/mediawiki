...
class DifferenceEngine extends ContextSource {
	...
	var $revisionsLoaded = false; // Have the revisions been loaded
	var $textLoaded = 0; // How many text blobs have been loaded, 0, 1 or 2?
	var $cacheHit = false; // Was the diff fetched from cache?
	...
	function showDiffPage( $diffOnly = false ) {
		...
		if ( $this->oldPage ) { # mOldPage might not be set, see below.
		...
		if ( $this->oldRev === false ) {
			...
			if ( $this->newPage->equals( $this->oldPage ) ) {
				...
				$out->setPageTitle( $this->msg( 'difference-title-multipage', $this->oldPage->getPrefixedText(),
					$this->newPage->getPrefixedText() ) );
			...
			if ( $samePage && $this->newPage->quickUserCan( 'edit', $user ) ) {
				...
				if ( !$this->oldRev->isDeleted( Revision::DELETED_TEXT ) && !$this->newRev->isDeleted( Revision::DELETED_TEXT ) ) {
		...
		$newRevisionHeader = $this->getRevisionHeader( $this->newRev, 'complete' ) . ' ' . implode( ' ', $formattedRevisionTools );
	...
	public function getDiffBody() {
		...
		if ( !$this->loadRevisionData() ) {
			wfProfileOut( __METHOD__ );
			return false;
		} elseif ( $this->oldRev && !$this->oldRev->userCan( Revision::DELETED_TEXT, $this->getUser() ) ) {
			wfProfileOut( __METHOD__ );
			return false;
		} elseif ( $this->newRev && !$this->newRev->userCan( Revision::DELETED_TEXT, $this->getUser() ) ) {
			wfProfileOut( __METHOD__ );
			return false;
		}
		// Short-circuit
		...
		if ( $this->oldId && $this->newId ) {
			$key = wfMemcKey( 'diff', 'version', self::CACHE_VERSION,
	...
	function loadRevisionData() {
		...
		$this->newRev = $this->newId
			? Revision::newFromId( $this->newId )
