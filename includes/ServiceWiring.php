...
use MediaWiki\Cdn\NullCdnController;
...
return [
	...
	'CdnController' => function( MediaWikiServices $services ) {
		...
		$useSquid = $services->getMainConfig()->get( 'UseSquid' );

		if ( $useSquid ) {
			$squidServers = $services->getMainConfig()->get( 'SquidServers' );
			$squidMaxAge = $services->getMainConfig()->get( 'SquidMaxAge' );

			if ( !empty( $squidServers ) ) {
				// TODO: inject server list and other settings!
				return new ActiveCdnController( $squidMaxAge );
			} else {
				return new CacheHeaderCdnController( $squidMaxAge );
			}
		}

		return new NullCdnController();
	},
