...
/**
 * Stores a user password. Mostly a wrapper object for the Hash
...
class Password {
	...
	public function update( $plaintext ) {
		...
		if( Hash::$default != get_class( $this->hash ) ) {
...
abstract class Hash {
	...
	public static function init() {
		...
		self::register( 'Hash_MWUnsalted' );
		self::register( 'Hash_MWSalted' );
		self::register( 'Hash_PBKDF2' );

		// Default to PBKDF2 unless $wgPasswordHash is set.
		self::register( 'Hash_' . $wgPasswordHash );
		self::$default = 'Hash_' . $wgPasswordHash;
	...
	public static function register( $class ) {
		if( in_array( $class, self::$types ) ) {
			...
			return false;
		} elseif( !is_subclass_of( $class, 'Hash' ) ) {
			return false;
		} else {
			self::$types[] = $class;
		}
 ...
 */
class Hash_MWOldCrypt extends Hash {
	/**
 ...
 */
class Hash_MWSalted extends Hash {
	/**
 ...
 */
class Hash_PBKDF2 extends Hash {
	/**
