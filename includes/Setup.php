...
// Backwards-compatibility reading of old Squid-named CDN settings as of MediaWiki 1.32
if ( !isset( $wgUseCdn ) && isset( $wgUseSquid ) ) {
	$wgUseCdn = $wgUseSquid;
	// wfDeprecated( '$wgUseSquid set instead of $wgUseCdn', '1.32' );
}
if ( !isset( $wgCdnServers ) && isset( $wgSquidServers ) ) {
	$wgCdnServers = $wgSquidServers;
	// wfDeprecated( '$wgSquidServers set instead of $wgCdnServers', '1.32' );
}
if ( !isset( $wgCdnServersNoPurge ) && isset( $wgSquidServersNoPurge ) ) {
	$wgCdnServersNoPurge = $wgSquidServersNoPurge;
	// wfDeprecated( '$wgSquidServersNoPurge set instead of $wgCdnServersNoPurge', '1.32' );
}
if ( !isset( $wgCdnMaxAge ) && isset( $wgSquidMaxage ) ) {
	$wgCdnMaxAge = $wgSquidMaxage;
	// wfDeprecated( '$wgSquidMaxage set instead of $wgCdnMaxAge', '1.32' );
}
if ( !isset( $wgCdnPurgeUseHostHeader ) && isset( $wgSquidPurgeUseHostHeader ) ) {
	$wgCdnPurgeUseHostHeader = $wgSquidPurgeUseHostHeader;
	// wfDeprecated( '$wgSquidPurgeUseHostHeader set instead of $wgCdnPurgeUseHostHeader', '1.32' );
}
