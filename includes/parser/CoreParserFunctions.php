...
class CoreParserFunctions {
	 ...
	 * of the handler. The handler must support getStandardMetaArray() for this
	...
	public static function filemetadata( $parser, $property = '' ) {
		...
		if ( !isset( $metadataCache[ $title->getDBKey() ] ) ) {
			...
			$metadataCache[ $title->getDBKey() ] = $file->getStandardMetaArray();
		}
		...
		if ( !is_array( $metadata ) ) {
			$error = '<span class="error">' .
				wfMessage( 'filemetadata-unsupported', $title->getFullText() )->inContentLanguage()->text() .
				'</span>';
		...
		if ( !isset( $metadata[$splitProperty[0]] ) ) {
			return '<span class="error">' .
				wfMessage( 'filemetadata-noprop', $title->getFullText(), $splitProperty[0] )->inContentLanguage()->plain() .
				'</span>';
		}
		...
		if ( !isset( $splitProperty[1] ) ) {
			...
			if ( !is_array( $metadata ) || !isset( $metadata[$splitProperty[1]] ) ) {
				return '<span class="error">' .
					wfMessage( 'filemetadata-nosubprop', $title->getFullText(), $property, $splitProperty[0] )
