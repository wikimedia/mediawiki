...
class CoreParserFunctions {
	...
	static function displaytitle( $parser, $text = '' ) {
		...
		$htmlTagsCallback = !$wgRestrictDisplayTitle ? null : function ( $params ) {
			if ( isset( $params['style'] ) ) {
				// this is called later anyway, but we need it right now for the regexes below to be safe
				// calling it twice doesn't hurt
				$params['style'] = Sanitizer::checkCss( $params['style'] );
				
				if ( preg_match( '/(display|user-select|visibility)\s*/i', $params['style'] ) ) {
					$params['style'] = '/* attempt to bypass $wgRestrictDisplayTitle */';
				}
			}
		}
