...
class CoreParserFunctions {
	...
	public static function filemetadata( $parser, $property = '' ) {
		...
		if ( !$raw ) {
			// Non-raw mode returns the metadata in formatted in user
			// language, just like how it shows up on the image page.
			// Given that commons would probably be the main user of
			// this, I think it makes sense to return in user language.
		...
		if ( !isset( $splitProperty[1] ) ) {
			...
			if ( $raw ) {
				$metadata = FormatMetadata::flattenArray( $metadata );
			} else {
				$metadata = FormatMetadata::flattenArrayContentLang( $metadata );
			}
		} else {
