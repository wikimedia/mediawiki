...
class Preprocessor_Hash extends Preprocessor {
	...
	public function preprocessToObj( $text, $flags = 0 ) {
		...
		if ( $tree !== false ) {
			return new PPNode_Hash_Tree( json_decode( $tree ), 0 );
		}
		...
		foreach ( $stack->rootAccum as &$node ) {
			if ( is_array( $node ) && $node[0] === 'possible-h' ) {
				$node[0] = 'h';
			}
		}
		...
		// Cache
		$this->cacheSetTree( $text, $flags,
			json_encode( $rootStore, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE ) );
