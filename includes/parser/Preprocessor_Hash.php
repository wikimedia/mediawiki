		...
		while ( true ) {
			...
			if ( $found == 'angle' ) {
				...
				if ( $count >= $rule['min'] ) {
					...
					# Add literal brace(s)
					self::addLiteral( $accum, $reserve . str_repeat( $curChar, $count ) );
				}
				$i += $count;
			} elseif ( $found == 'close' ) {
				$piece = $stack->top;
				...
				if ( $matchingCount < $piece->count ) {
					...
					if ( $piece->count >= $min ) {
						$stack->push( $piece );
						$accum =& $stack->getAccum();
					} elseif ( $piece->count == 1 && $piece->open == '{' && $piece->reserve == '-' ) {
						$piece->reserve = '';
						$piece->open = '-{';
						$piece->count = 2;
						...
						self::addLiteral( $accum, $piece->reserve . $s );
					}
				} elseif ( strlen( $piece->reserve ) > 0 ) {
					self::addLiteral( $accum, $piece->reserve );
				}
...
class PPDStackElement_Hash extends PPDStackElement {
	...
	public function breakSyntax( $openingCount = false ) {
		if ( $this->open == "\n" ) {
			$accum = array_merge( [ $this->reserve ], $this->parts[0]->out );
		} else {
