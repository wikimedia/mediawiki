...
class Parser {
	 ...
	 * @return mixed|string
	 */
	public function preprocess( $text, Title $title = null, ParserOptions $options, $revid = null, $frame = false, $expandHtml = false ) {
		wfProfileIn( __METHOD__ );
			...
			if ( !$title->isExternal() ) {
				if ( $title->isSpecialPage()
					...
					if ( $ret ) {
						...
						$wikitextFrame = $this->getPreprocessor()->newCustomFrame( $wikitextBits );
						$wikitext = $wikitextFrame->implode( '' );
	...
	public function getTemplateDom( $title, $wikitext = null ) {
		...
		if ( $parserOutput ) {
			...
			$this->getOutput()->addOutputPageMetadata( $parserOutput );
		}
