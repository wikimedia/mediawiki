...
class Parser {
	...
	const IPv4_ADDR = '(?:(?:\\d{1,3}\\.){3}\\d{1,3})';
	# Regexp to match IPv6 addresses (RFC2373, Appendix B)
	const IPv6_PART = '(?i:[0-9A-F]{1,4}(?:[:][0-9A-F]{1,4}){0,7})'; # "hexseq"
	const IPv6_ADDR = '(?:' .
		# addresses ending with IPv6_PART
		'(?:' . self::IPv6_PART . '(?:::' . self::IPv6_PART . ')?|::' . self::IPv6_PART . ')(?::' . self::IPv4_ADDR . ')?|' .
		# addresses ending with ::
		self::IPv6_PART . '?::' . self::IPv4_ADDR . '?)';
	# Regexp to match IPv4 or IPv6 address
	const EXT_LINK_IP_ADDR = '(?:' . self::IPv4_ADDR . '|\\[' . self::IPv6_ADDR . '\\])';
	# IP address or at least one character of hostname
	const EXT_LINK_ADDR = '(?:' . self::EXT_LINK_IP_ADDR . '|' . self::EXT_LINK_URL_CLASS . ')';
	const EXT_IMAGE_REGEX = '/^(http:\/\/|https:\/\/)(' . self::EXT_LINK_ADDR .
