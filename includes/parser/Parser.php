		...
		if ( !$found && $title ) {
			...
			if ( !$title->isExternal() ) {
				if ( $title->isSpecialPage()
					...
					$templateParams = new PPTransclusionParameters( $frame, $args );
	...
	public static function statelessFetchTemplate( Title $title, $parser = false ) {
		...
		for ( $i = 0; $i < 2 && is_object( $title ); $i++ ) {
			...
			if ( $rev ) {
				...
				if ( $content ) {
					return new ContentTransclusion( $content, $title, $deps );
				}
				...
				$text = $message->plain();
				return new WikitextTransclusion( $text, $title, $deps );
			} else {
				break;
			}
