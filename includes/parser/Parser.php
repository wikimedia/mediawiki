...
class Parser {
	...
	const OT_PREPROCESS_TRANSCLUDE_HTML = 5; # like OT_PREPROCESS, but also expand non-wikitext transclusions to HTML.
	 ...
	 * @param bool $expandHtml Expand non-wikitext transclusions to HTML.
	 *
	 * @return mixed|string
	 */
	function preprocess( $text, Title $title = null, ParserOptions $options, $revid = null, $frame = false, $expandHtml = false ) {
		wfProfileIn( __METHOD__ );

		if ( $expandHtml ) {
			// Escape any tags of the form <mw-xyz>, to avoid confusing such tags entered by the
			// user from such tags generated by HTML transclusion. This is allows the caller to
			// trust any HTML enclosed in such tags to have been generated by MediaWiki.
			$text = preg_replace( '/<mw-/i', '&lt;mw-', $text );
			$ot = self::OT_PREPROCESS_TRANSCLUDE_HTML;
		} else {
			$ot = self::OT_PREPROCESS;
		}

		$magicScopeVariable = $this->lock();
		$this->startParse( $title, $options, $ot, true );
		if ( $revid !== null ) {
			$this->mRevisionId = $revid;
		}
		wfRunHooks( 'ParserBeforeStrip', array( &$this, &$text, &$this->mStripState ) );
	...
	function setOutputType( $ot ) {
		...
		$this->ot = array(
			...
			'pre' => $ot == self::OT_PREPROCESS || $ot == self::OT_PREPROCESS_TRANSCLUDE_HTML,
			'plain' => $ot == self::OT_PLAIN,
			'pre+html' => $ot == self::OT_PREPROCESS_TRANSCLUDE_HTML,
		);
	}
		...
		if ( !$found && $title ) {
			...
			if ( !$title->isExternal() ) {
				if ( $title->isSpecialPage()
					...
					if ( $ret ) {
						$wikitext = $frame->virtualBracketedImplode( '{{', '|', '}}', $titleWithSpaces, $args );
	 ...
	 * @return array
	 */
	private function getTemplateDom( $title, $wikitext = null ) {
		$cacheTitle = $title;
	...
	/**
	 * Fetch the unparsed text of a template. In case the template was included via HTML,
	 ...
	 */
	private function fetchTemplateAndTitle( Title $title, $wikitext ) {
		...
		if ( $parserOutput ) {
			...
			$parserOutput = $templateInfo['ParserOutput'];
	...
	/**
	 * Determines whether the template dependencies include a reference to the page
	 ...
	 * @return bool
	 */
	private function hasSelfTransclusion( $dependencies ) {
	 ...
	 * @return string|bool
	 */
	function fetchTemplate( Title $title ) {
		$wikitext = '{{' . $title->getFullText() . '}}';
	 ...
	 *   'deps' => $deps )
	 */
	static function statelessFetchTemplate( Title $title, $parser = false ) {
		$text = $skip = false;
		$finalTitle = $title;
		$deps = array();
		...
		for ( $i = 0; $i < 2 && is_object( $title ); $i++ ) {
			...
			if ( $rev ) {
				...
				if ( $templateParserOutput === false || $templateParserOutput === null ) {
					$templateParserOutput = false;
				}

				if ( $text === false && $templateParserOutput === false ) {
					break;
				}
