...
class Parser {
	...
	public function parse( $text, Title $title, ParserOptions $options, $linestart = true, $clearState = true, $revid = null ) {
		...
		if ( $this->mOptions->getEnableLimitReport() ) {
			...
			wfRunHooks( 'ParserPrepareLimitReport', array( $this, $this->mOutput ) );

			$limitReport = "NewPP limit report\n";
			foreach ( $this->mOutput->getLimitReportData() as $key => $data ) {
				list( $value, $opts ) = $data;
				if ( in_array( 'noNewPP', $opts ) ) {
					continue;
				}

				$label = wfMessage( $key )->inLanguage( 'en' )->useDatabase( false )->text();
				$limitReport .= "$label:";

				$msg = wfMessage( array( "$key-value-text", "$key-value" ) )
				...
				if ( in_array( 'multiline', $opts ) ) {
					...
					$limitReport .= " {$msg->params( $value )->text()}\n";
				}
			}
