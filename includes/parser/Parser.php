...
class Parser {
	...
	public function guessLegacySectionNameFromWikiText( $text ) {
		...
		if ( $wgFragmentMode[0] === 'legacy' ) {
			// ForHtml() and ForLink() are the same for legacy encoding
			$id = Sanitizer::escapeIdForHtml( $text );
		} elseif ( isset( $wgFragmentMode[1] ) && $wgFragmentMode[1] === 'legacy' ) {
			$id = Sanitizer::escapeIdForHtml( $text, Sanitizer::ID_FALLBACK );
		} else {
