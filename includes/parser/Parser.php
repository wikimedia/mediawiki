...
class Parser {
	...
	function getVariableValue( $index, $frame = false ) {
		...
		switch ( $index ) {
			case 'currentmonth':
				$value = $pageLang->formatNum( wfDateTime( $ts )->format( 'm' ) );
				break;
			case 'currentmonth1':
				$value = $pageLang->formatNum( wfDateTime( $ts )->format( 'n' ) );
				break;
			case 'currentmonthname':
				$value = $pageLang->getMonthName( wfDateTime( $ts )->format( 'n' ) );
				break;
			case 'currentmonthnamegen':
				$value = $pageLang->getMonthNameGen( wfDateTime( $ts )->format( 'n' ) );
				break;
			case 'currentmonthabbrev':
				$value = $pageLang->getMonthAbbreviation( wfDateTime( $ts )->format( 'n' ) );
				break;
			case 'currentday':
				$value = $pageLang->formatNum( wfDateTime( $ts )->format( 'j' ) );
				break;
			case 'currentday2':
				$value = $pageLang->formatNum( wfDateTime( $ts )->format( 'd' ) );
				break;
			case 'localmonth':
				$value = $pageLang->formatNum( wfLocalDateTime( $ts )->format( 'm' ) );
				break;
			case 'localmonth1':
				$value = $pageLang->formatNum( wfLocalDateTime( $ts )->format( 'n' ) );
				break;
			case 'localmonthname':
				$value = $pageLang->getMonthName( wfLocalDateTime( $ts )->format( 'n' ) );
				break;
			case 'localmonthnamegen':
				$value = $pageLang->getMonthNameGen( wfLocalDateTime( $ts )->format( 'n' ) );
				break;
			case 'localmonthabbrev':
				$value = $pageLang->getMonthAbbreviation( wfLocalDateTime( $ts )->format( 'n' ) );
				break;
			case 'localday':
				$value = $pageLang->formatNum( wfLocalDateTime( $ts )->format( 'j' ) );
				break;
			case 'localday2':
				$value = $pageLang->formatNum( wfLocalDateTime( $ts )->format( 'd' ) );
				break;
			case 'pagename':
			...
			case 'subjectspacee':
				...
				break;
			case 'currentdayname':
				$value = $pageLang->getWeekdayName( wfDateTime( $ts )->format( 'w' ) + 1 );
				break;
			case 'currentyear':
				$value = $pageLang->formatNum( wfDateTime( $ts )->format( 'Y' ), true );
				break;
			case 'currenttime':
				$value = $pageLang->time( wfTimestamp( TS_MW, $ts ), false, false );
				break;
			case 'currenthour':
				$value = $pageLang->formatNum( wfDateTime( $ts )->format( 'H' ), true );
				break;
			case 'currentweek':
				# @bug 4594 PHP5 has it zero padded, PHP4 does not, cast to
				# int to remove the padding
				$value = $pageLang->formatNum( (int)wfDateTime( $ts )->format( 'W' ) );
				break;
			case 'currentdow':
				$value = $pageLang->formatNum( wfDateTime( $ts )->format( 'w' ) );
				break;
			case 'localdayname':
				$value = $pageLang->getWeekdayName( wfLocalDateTime( $ts )->format( 'w' ) + 1 );
				break;
			case 'localyear':
				$value = $pageLang->formatNum( wfLocalDateTime( $ts )->format( 'Y' ), true );
				break;
			case 'localtime':
				$value = $pageLang->time( wfLocalDateTime( $ts )->format( 'YmdHis' ), false, false );
				break;
			case 'localhour':
				$value = $pageLang->formatNum( wfLocalDateTime( $ts )->format( 'H' ), true );
				break;
			case 'localweek':
				# @bug 4594 PHP5 has it zero padded, PHP4 does not, cast to
				# int to remove the padding
				$value = $pageLang->formatNum( (int)wfLocalDateTime( $ts )->format( 'W' ) );
				break;
			case 'localdow':
				$value = $pageLang->formatNum( wfLocalDateTime( $ts )->format( 'w' ) );
				break;
			...
			case 'currenttimestamp':
				...
				break;
			case 'localtimestamp':
				$value = wfLocalDateTime( $ts )->format( 'YmdHis' );
				break;
			case 'currentversion':
	...
	function pstPass2( $text, $user ) {
		...
		$tzMsg = $localDateTime->format( 'T' );  # might vary on DST changeover!
