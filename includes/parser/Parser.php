...
class Parser {
	 ...
	 */
	private function setExternalImageRegex() {
		global $wgAllowExternalSVG;
		# List of allowed image extensions
		$imageExtensions = array ('gif', 'png', 'jpg', 'jpeg');
		if ( $wgAllowExternalSVG == true ) {
			$imageExtensions[] = 'svg';
		}
		$ext_image_regex = '/^(http:\/\/|https:\/\/)([^][<>"\\x00-\\x20\\x7F\p{Zs}]+)
			\\/([A-Za-z0-9_.,~%\\-+&;#*?!=()@\\x80-\\xFF]+)\\.((?i)' .
			implode('|', $imageExtensions) . ')$/Sxu';
	}
