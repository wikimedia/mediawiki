...
class CoreTagHooks {
	...
	public static function register( $parser ) {
		...
		$parser->setHook( 'mw-transclude', array( __CLASS__, 'transclude' ) );
		if ( $wgRawHtml ) {
	...
	/**
	 * Core parser tag hook function for 'mw-transclude', used for transclusion tags
	 * generated by the OT_PREPROCESS_TRANSCLUDE_HTML mode.
	 *
	 * <mw-transclude data-wikitext="{{Foo}}">....</mw-transclude> is just a fany way of
	 * writing {{Foo}}. The text content of the tag would be HTML, but should be ignored,
	 * as it is potentially user-supplied and unsafe.
	 *
	 * @param string $content Will be ignored
	 * @param array $attributes $attributes['data-wikitext'] will be parsed using recursiveTagparse
	 * @param Parser $parser
	 *
	 * @throws MWException
	 * @return array
	 */
	static function transclude( $content, $attributes, $parser ) {
		if ( !isset( $attributes['data-wikitext'] ) ) {
			return '';
		}

		$wikitext = $attributes['data-wikitext'];
		$text = $parser->recursiveTagParse( $wikitext );
