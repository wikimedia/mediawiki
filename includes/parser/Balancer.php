...
class Balancer {
	...
	public static function balance( $text ) {
		...
		while ( $bitsIterator->valid() ) {
			...
			if ( $t === 'mw:balance-block' ) {
				...
				if ( $slash ) {
					...
					for ( $len = count( $stack ); $j < $len; $j++ ) {
						if ( isset( Balancer::$blockBlacklist[$stack[$j]] ) )
							break;
					}
				}
				while ( $j < count( $stack ) ) {
			...
			if ( $slash ) {
				$len = count( $stack );
				if ( $len > 0 && $stack[$len - 1] === $t ) {
					// Our stack is conservative; it might have extra
					// tags on it if the original input was not well-nested.
					array_pop( $stack );
				}
			} elseif ( !isset( Balancer::$emptyElements[ $t ] ) ) {
				array_push( $stack, $t );
			}
			if ( $suppress ) {
				$result .= $rest;
