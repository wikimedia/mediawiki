...
class ParserCache {
	...
	/** Do not use expired or outdated data */
	const DO_NOT_USE = 0;

	/** Use expired data */
	const USE_EXPIRED = 1;

	/** Use expired data and data from different revisions */
	 ...
	 *  compatibility, boolean false is treated as DO_NOT_USE and
	...
	public function getKey( $article, $popts, $useOutdated = self::USE_ANYTHING ) {
		...
		if ( is_bool( $useOutdated ) ) {
			$useOutdated = $useOutdated ? self::USE_ANYTHING : self::DO_NOT_USE;
		}
	...
	public function get( $article, $popts, $useOutdated = false ) {
		...
		$parserOutputKey = $this->getKey( $article, $popts,
			$useOutdated ? self::USE_OUTDATED : self::DO_NOT_USE
		);
