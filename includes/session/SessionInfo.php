...
class SessionInfo {
	...
	public function __construct( $priority, array $data ) {
		...
		if ( $data['provider'] !== null && !$data['provider']->canChangeUser() &&
			( $data['userInfo'] === null || !$data['userInfo']->isVerified() )
		) {
			throw new \InvalidArgumentException(
				'Provider ' . $data['provider'] . ' cannot set user info, ' .
					'but no verified userInfo was provided'
			);
		}
	...
	final public function loadFromStore(
		...
		if ( $blob !== false ) {
			...
			if ( !$this->userInfo ) {
				...
				try {
					if ( $metadata['userId'] ) {
						$this->userInfo = UserInfo::newFromId( $metadata['userId'] );
					} elseif ( $metadata['userName'] !== null ) { // Shouldn't happen, but just in case
						$this->userInfo = UserInfo::newFromName( $metadata['userName'] );
					} else {
						$this->userInfo = UserInfo::newAnonymous();
					}
				...
				if ( $metadata['userId'] ) {
					if ( $metadata['userId'] !== $this->userInfo->getId() ) {
						$logger->warning( "Session $this: User ID mismatch, " .
							$metadata['userId'] . ' !== ' . $this->userInfo->getId() );
						return false;
					}

					// If the user was renamed, probably best to fail here.
					if ( $metadata['userName'] !== null &&
						$this->userInfo->getName() !== $metadata['userName']
					) {
						$logger->warning( "Session $this: User ID matched but name didn't (rename?), " .
							$metadata['userName'] . ' !== ' . $this->userInfo->getName() );
						return false;
					}

				} elseif ( $metadata['userName'] !== null ) { // Shouldn't happen, but just in case
					if ( $metadata['userName'] !== $this->userInfo->getName() ) {
						$logger->warning( "Session $this: User name mismatch, " .
							$metadata['userName'] . ' !== ' . $this->userInfo->getName() );
						return false;
					}
				} elseif ( !$this->userInfo->isAnon() ) {
			...
			if ( $metadata['userToken'] !== null &&
				$this->userInfo->getToken() !== $metadata['userToken']
			) {
				$logger->warning( "Session $this: User token mismatch" );
				return false;
			}
			$this->userInfo = $this->userInfo->verified();
			...
			if ( !$this->userInfo->isVerified() ) {
				$logger->warning(
					"Session $this: Unverified user provided and no metadata to auth it"
				);
				return false;
			}
