...
class SessionInfo {
	 ...
	 *
	 * The value here indicates whether a non-anonymous user was passed to the
	 * SessionInfo constructor, as it's assumed that the provider can only know
	 * a user to pass to the constructor if the "remember" state was previously
	 * set when the session was last persisted.
	 *
	 * @return bool
	 */
	...
	final public function loadFromStore(
		...
		if ( ( $data === false ) !== ( $metadata === false ) ) {
			$logger->warning( "Session $this: Has data or metadata but not both" );
			return false;
		}
