 ...
 * @since 1.27
 */
class CookieSessionProvider extends MWSessionProvider {
	...
	public function provideSessionInfo( WebRequest $request ) {
		...
		if ( !MWSessionManager::validateSessionId( $info['id'] ) ) {
			unset( $info['id'] );
		}
		...
		if ( $userId !== null ) {
			try {
				$user = MWSessionUserInfo::newFromId( $userId );
		...
		return new MWSessionInfo( $this->params['priority'], $info );
	}
