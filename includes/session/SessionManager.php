...
final class SessionManager implements SessionManagerInterface {
	...
	private function getSessionFromInfo( SessionInfo $info, WebRequest $request ) {
		...
		if ( !isset( $this->allSessionBackends[$id] ) ) {
			...
			// In case the session is newly created, save it. If it's not new
			// this'll be a no-op since the session won't be dirty.
			$backend->save();
		} else {
	...
	public function generateSessionId() {
		do {
			...
			$key = wfMemcKey( 'MWSession', 'metadata', $id );
		} while ( isset( $this->allSessionIds[$id] ) || $this->store->get( $key ) !== false );
		return $id;
	}
