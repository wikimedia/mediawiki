 ...
 * If doing so, different session cookie names should be used for different
 * providers to avoid collisions.
 *
 * @ingroup Session
 * @since 1.27
 */
abstract class SessionProvider implements SessionProviderInterface, LoggerAwareInterface {
	 ...
	 *
	 * @protected For use by \\MediaWiki\\Session\\SessionInfo only
	 ...
	 * requests will cause self::provideSessionInfo() to provide a SessionInfo
	 * with that ID. This can be as simple as not passing any 'user' into
	 ...
	 * unauthenticated UserInfo to pass to SessionInfo::__construct().
	 *
	 ...
	 *
	 * If the provider is capable of returning a SessionInfo with an
	 * authenticated UserInfo for the named user in some manner other than by
	 * validating against $user->getToken(), steps must be taken to prevent
	 * that from occurring in the future. This might add the username to a
	 * blacklist, or it might just delete whatever authentication credentials
	 * would allow such a session in the first place (e.g. remove all OAuth
	 * grants or delete record of the SSL client certificate).
	 *
