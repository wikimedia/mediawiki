...
abstract class SessionProvider implements SessionProviderInterface, LoggerAwareInterface {
	...
	/**
	 * Validate a loaded SessionInfo
	 *
	 * This is similar in purpose to the 'SessionCheckInfo' hook. On failure,
	 * the provider is expected to write an appropriate message to its logger.
	 *
	 * @protected For use by \\MediaWiki\\Session\\SessionManager only
	 * @param SessionInfo $info
	 * @param WebRequest $request
	 * @param array|null &$metadata Provider metadata
	 * @return bool Return false to reject the SessionInfo after all.
	 */
	public function checkSessionInfo( SessionInfo $info, WebRequest $request, &$metadata ) {
		return true;
	}
