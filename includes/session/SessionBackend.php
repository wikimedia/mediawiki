...
final class SessionBackend {
	...
	public function save( $closing = false ) {
		if ( $this->provider->getManager()->isUserSessionPrevented( $this->user->getName() ) ) {
			$this->logger->debug(
				'SessionBackend "{session}" not saving, user {user} was passed to SessionManager::preventSessionsForUser',
				array(
					'session' => $this->id,
					'user' => $this->user,
			) );
		...
		$this->logger->debug(
			'SessionBackend "{session}" save: dataDirty={dataDirty} metaDirty={metaDirty} forcePersist={forcePersist}',
			array(
				'session' => $this->id,
