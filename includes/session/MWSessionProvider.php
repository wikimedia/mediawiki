...
/**
 * An MWSessionProvider provides MWSessionInfo and support for MWSession
 *
 * An MWSessionProvider is responsible for taking a WebRequest and determining
 * the authenticated session that it's a part of. It does this by returning an
 * MWSessionInfo object with basic information about the session it thinks is
 * associated with the request, namely the session ID and possibly the
 * authenticated user the session belongs to.
 *
 * The MWSessionProvider also provides for updating the WebResponse with
 ...
 * @since 1.27
 */
abstract class MWSessionProvider implements LoggerAwareInterface {
	 ...
	 */
	public function setManager( MWSessionManager $manager ) {
	 ...
	 * MWSessionInfo object identifying the session.
	 *
	 * If multiple MWSessionProviders provide sessions, the one with highest
	 * priority wins. In case of a tie, an exception is thrown.
	 * MWSessionProviders are encouraged to make priorities user-configurable
	 ...
	 * null to the $user parameter of MWSessionInfo::__construct()).
	 *
	 * @return bool
	 */
	 ...
	 * MWSessionInfo::__construct().
	 *
	 * @param MWSessionBackend $session Session to persist
	 * @param WebRequest $request Request into which to persist the session
	 */
	abstract public function persistSession( MWSessionBackend $session, WebRequest $request );
