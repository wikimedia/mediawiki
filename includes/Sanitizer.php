...
class Sanitizer {
	...
	public static function filterForXSS(
		...
		$scriptStartTag = '<\W*script(?:>|[^a-zA-Z][^>]*>)';
		$scriptEndTag = '</\W*script(?:>|[^a-zA-Z][^>]*>)';

		$m = array();
		$result = preg_match( "#$scriptStartTag.{0,70}|$scriptEndTag#s", $text, $m );
		if ( $result ) {
			$context = array(
				'method' => __METHOD__,
			...
			$log->error( "XSS detected on $titleText", $context );

			if ( !$reportOnly ) {
				$text = preg_replace( "#$scriptEndTag#", '</div>', $text );
				...
				$text = preg_replace( "#$scriptStartTag#", $replaceDiv, $text );
			}
		}
		return $text;
	}
