...
class Sanitizer {
	...
	static function validateAttributes( $attribs, $attribWhitelist, $roleWhitelist ) {
		...
		# For <img> with empty alt text, strip all roles other than presentation from
		# the role whitelist. <img alt=""> may only have the presentation role to conform
		# to HTML5 spec.
		if ( isset( $attribs['alt'] )
			&& isset( $attribWhitelist['alt'] )
			&& $attribs['alt'] === ''
		) {
			$roleWhitelist = array_intersect( $roleWhitelist, array( 'presentation' ) );
		}
