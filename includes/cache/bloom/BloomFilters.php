...
class BloomFilterTitleHasLogs {
	public static function merge(
		...
		$scopedLock = ( mt_rand( 1, pow( 4, max( 0, 5 - $age ) ) ) == 1 )
			? $bcache->getScopedLock( $virtualKey )
			: false;

		if ( $scopedLock ) {
			$updates = self::populate( $bcache, $domain, $virtualKey, $memberPrefix, $status );
			$age = isset( $updates['asOfTime'] ) ? ( time() - $updates['asOfTime'] ) : $age;
		}

		return ( $status['ready'] && $age < 30 );
	}
