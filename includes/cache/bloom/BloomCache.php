...
abstract class BloomCache {
	 ...
	 * A class must exist with the name BloomCheck<type> and a static public
	 * checkWithMerge() method. The later takes the following arguments:
	 * (BloomCache $bcache, $domain, $type, $member).
	 *
	 * @param string $domain
	 * @param string $type
	 * @param string $member
	...
	final public function checkWithMerge( $domain, $type, $member ) {
		...
		if ( method_exists( "BloomCheck{$type}", 'checkWithMerge' ) ) {
			try {
				return call_user_func_array(
					array( "BloomCheck{$type}", 'checkWithMerge' ),
					array( $this, $domain, $type, $member )
				);
			} catch ( MWException $e ) {
