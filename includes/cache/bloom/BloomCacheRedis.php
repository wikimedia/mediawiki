...
class BloomCacheRedis extends BloomCache {
	...
	protected function doInit( $key, $size, $precision ) {
		...
		// 250000000 items at p=.001 take up 428.49MB and fit into one value.
		// Do not hit the 512MB redis value limit by reducing the demands.
		$size = min( $size, 250000000 * $this->segments );
		$precision = max( round( $precision, 3 ), .001 );

		static $script =
<<<LUA
		local kMetadata, kData = unpack(KEYS)
		local aEntries, aPrec = unpack(ARGV)
...
LUA;
	...
	protected function doAdd( $key, array $members ) {
		...
		// Based on https://github.com/ErikDubbelboer/redis-lua-scaling-bloom-filter
		// but simplified to use a single filter instead of up to 32 filters
		static $script =
<<<LUA
		local kMetadata, kData = unpack(KEYS)
...
LUA;
	...
	protected function doGetStatus( $virtualKey ) {
		...
		if ( is_array( $res ) ) {
			$res['ready'] = isset( $res['ready'] ) ? (bool)$res['ready'] : false;
