...
class MessageCache {
	 ...
	 */
	protected function loadFromDBWithLock( $code, array &$where, $mode = null, WANObjectCache $wanCache ) {
	 ...
	 * @param WANObjectCache $wanCache
	 */
	public function replace( $title, $text, WANObjectCache $wanCache ) {
		...
		DeferredUpdates::addCallableUpdate(
			function () use ( $title, $msg, $code ) {
				...
				$scopedLock = $this->getReentrantScopedLock( $wanCache->makeKey( 'messages', $code ) );
				if ( !$scopedLock ) {
				...
				$this->wanCache->touchCheckKey( $wanCache->makeKey( 'messages', $code ) );
				...
				foreach ( $codes as $code ) {
					$this->wanCache->delete( $wanCache->makeKey( 'sidebar', $code ) );
				}
	...
	public function updateMessageOverride( Title $title, Content $content = null ) {
		...
		$this->replace( $title->getDBkey(), $msgText, $this->wanCache );
