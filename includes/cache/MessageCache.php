...
class MessageCache {
	...
	function load( $code = false ) {
		...
		# (2) memcache
		# Fails if nothing is in the cache, or if the data is expired
		if ( !$success ) {
		...
		if ( !$success ) {
			...
			if ( !$acquired && $staleCache ) {
				...
				$this->mCache[$code] = $cache;
				$success = $this->saveToCaches( $cache, 'all', $code );
	 ...
	 * @param $style string: Either "local-only" to save to local caches only
