	...
	public function getBlobs( array $modules, $lang ) {
		...
		foreach ( $modules as $name => $module ) {
			...
			if ( !isset( $result[$key] ) || $curTTLs[$key] === null || $curTTLs[$key] < 0 ) {
				$this->logger->info( 'Blob expired {cacheKey}', array( 'cacheKey' => $key ) );
				$blobs[$name] = $this->recacheMessageBlob( $key, $module, $lang );
			} else {
				$blobKeys = array_keys( FormatJson::decode( $result[$key], /* assoc = */ true ) );
				...
				if ( $blobKeys !== $moduleMsgs ) {
					...
					$this->logger->info( 'Blob updated {cacheKey}', array( 'cacheKey' => $key ) );
					$blobs[$name] = $this->recacheMessageBlob( $key, $module, $lang );
				} else {
					// Use unexpired cache
