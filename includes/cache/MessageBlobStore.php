...
class MessageBlobStore implements LoggerAwareInterface {
	...
	public function updateMessage( $key ) {
		...
		foreach ( $moduleNames as $name ) {
			// This touch uses a holdoff to account for database slave lag (behind MessageCache)
			$this->wanCache->touchCheckKey( $this->wanCache->makeKey( __CLASS__, $name ) );
		}
	}
	...
	public function clear() {
		$cache = $this->wanCache;
		// Modules no longer existing will naturally expire from cache.
		$moduleNames = $this->getResourceLoader()->getModuleNames();
		foreach ( $moduleNames as $name ) {
			// Disables holdoff because clear() invalidates a lot and it's not needed since
			// LocalisationCache is stored outside the database and doesn't have any lag.
			$cache->touchCheckKey( $cache->makeKey( __CLASS__, $name ), $cache::HOLDOFF_NONE );
		}
	}
