...
class RequestContext implements IContextSource, MutableContext {
	...
	public function setRequest( WebRequest $r ) {
		$this->request = $r;
		$this->syncGlobalVariable( 'wgRequest', $this->request ); //FIXME TESTME
	}
	...
	private function isMain() {
		return $this === self::getMain();
	}

	/**
	 * Sets the given global variable to $value, if this RequestContext is the main instance.
	 * This method exists for backwards compatibility: old code may rely on $wgTitle to
	 * be set and in sync with the main RequestContext instance.
	 *
	 * @param $varName
	 * @param $value
	 */
	private function syncGlobalVariable( $varName, $value ) {
		if ( $this->isMain() ) {
	...
	public function setTitle( Title $title ) {
		...
		$this->syncGlobalVariable( 'wgTitle', $title ); //FIXME TESTME
	...
	public function getTitle() {
		if ( !$this->title ) {
			...
			Assert::postcondition( $this->title !== null, '$this->title must not be null' );
			$this->syncGlobalVariable( 'wgTitle', $this->title ); //FIXME TESTME
		}
	...
	private function parseTitle() {
		...
		if ( $request->getCheck( 'search' ) ) {
			...
			if ( count( $contLang->getVariants() ) > 1
	 ...
	 *
	 * @since 1.25
	 * @deprecated since 1.27 always treturns true
	 * @return bool
	 */
	...
	public function setOutput( OutputPage $o ) {
		$this->output = $o;
		$this->syncGlobalVariable( 'wgOutput', $this->output ); //FIXME TESTME
	}
	...
	public function setUser( User $u ) {
		$this->user = $u;
		$this->syncGlobalVariable( 'wgUser', $this->user ); //FIXME TESTME
	}
	...
	/**
	 * Get the Language object representing the site's default content language.
	 *
	 * @return Language
	 * @throws Exception
	 * @since 1.27
	...
	/**
	 * Get the Language object representing the user interface language.
	 * This may trigger initialization of user or request objects.
	 *
	 * The user interface language is usually taken from the user preferences,
	 * but it may be overridden with the uselang parameter in the request.
	 * For anonymous users and upon failure, the site's content language is assumed.
	 *
	 * @return Language
	 * @throws Exception
	 * @since 1.19
	...
	public static function getMain() {
		return MediaWikiServices::getInstance()->getRequestContext();
	}
	...
	public static function resetMain() {
		MediaWikiServices::getInstance()->resetServiceForTesting( 'RequestContext' );
	}
