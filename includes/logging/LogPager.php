...
class LogPager extends ReverseChronologicalPager {
	...
	private function limitNamespace( $namespace ) {
		global $wgNamespaceFilteredLogs;
		if ( $this->typeCGI !== '' ) {
			// If config tells us it doesn't make sense to show the namespace filter, don't.
			if ( isset( $wgNamespaceFilteredLogs[$this->typeCGI] ) &&
				$wgNamespaceFilteredLogs[$this->typeCGI] === false
			) {
				$this->mLogEventsList->disableNamespaceInput();
				return;
			}
			// If user wants a specific action and config tells us it doesn't make sense to
			// show the namespace filter for this specific action, don't.
			if ( $this->action !== '' && is_array( $wgNamespaceFilteredLogs[$this->typeCGI] ) &&
				$wgNamespaceFilteredLogs[$this->typeCGI][$this->action] === false
			) {
				$this->mLogEventsList->disableNamespaceInput();
				return;
			}
