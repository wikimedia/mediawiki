...
class BlockLogFormatter extends LogFormatter {
	protected function getMessageParameters() {
		...
		// change title link to a link without prefix and a user tool bar
		...
		if ( substr( $title->getText(), 0, 1 ) == '#' ) {
			...
			$params[2] = Message::rawParam( Linker::userLink( $id, $title->getText() )
		...
		if ( $subtype === 'block' || $subtype === 'reblock' ) {
			...
			$durationTooltip = '&lrm;' . htmlspecialchars( $params[3] );
			$params[3] = Message::rawParam( "<span class='blockExpiry' title='$durationTooltip'>" .
				$this->context->getLanguage()->translateBlockExpiry( $params[3] ) . '</span>' );
			$params[4] = isset( $params[4] ) ?
				self::formatBlockFlags( $params[4], $this->context->getLanguage() ) : '';
		}
