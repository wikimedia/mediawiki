		...
		switch( $entry->getType() ) {
			...
			case 'rights':
				if ( count( $parameters['4::oldgroups'] ) ) {
					...
					$oldgroups = wfMsgExt( 'rightsnone', $msgOpts );
				}
				if ( count( $parameters['5::newgroups'] ) ) {
					$newgroups = implode( ', ', $parameters['5::newgroups'] );
				} else {
					$newgroups = wfMsgExt( 'rightsnone', $msgOpts );
				}
				switch( $entry->getSubtype() ) {
					case 'rights':
						$text = wfMsgExt( 'rightslogentry', $msgOpts, $target, $oldgroups, $newgroups );
						break;
					case 'autopromote':
						$text = wfMsgExt( 'rightslogentry-autopromote', $msgOpts, $target, $oldgroups, $newgroups );
						break;
				}
				break;
