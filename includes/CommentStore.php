...
class CommentStore {
	...
	/**
	 * @param IDatabase|null $db Database handle
	 */
	...
	/**
	 * Return a CommentStore with no database
	 * @return CommentStore
	 */
	public static function newNull() {
		return new CommentStore( null );
	}

	/**
	 * Return a CommentStore associated with a default replica
	 * @return CommentStore
	 */
	public static function newReplica() {
		return new CommentStore( wfGetDB( DB_REPLICA ) );
	}
	...
	public function getComment( $key, $row, $fallback = false ) {
		if ( !$this->db ) {
			throw new BadMethodCallException(
				'Cannot call ' . __METHOD__ . ' when no database is set'
			);
		}
