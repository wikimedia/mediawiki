...
class PasswordReset {
	...
	public function isAllowed( User $user, $displayPassword = false ) {
		...
		if ( $statuses ) {
			...
			} elseif ( !$authManager->allowsAuthenticationDataChange( new TemporaryPasswordAuthenticationRequest() ) ) {
				// Maybe the external auth plugin won't allow local password changes
				$status = StatusValue::newFatal( 'resetpass_forbidden' );
	 ...
	 * @throws ThrottledError When the user is throttled
