...
abstract class CentralIdLookup implements IDBAccessObject {
	...
	protected function checkAudience( $audience ) {
		...
		if ( $audience === self::FOR_PUBLIC ) {
			return new User;
		}
		if ( $audience === self::RAW ) {
			return null;
		}
	...
	/**
	 * Given central user IDs, return the user names
	...
	/**
	 * Given user names, return the central IDs
	...
	/**
	 * Given a central user ID, return the user name
	 * @param int $id Central user ID
	 * @param int|User $audience One of the audience constants, or a specific user
	 * @param int $flags IDBAccessObject read flags
	 * @return string|null Central user name, or empty string $audience lacks
	 *  the rights needed to see it, or null if $id doesn't correspond to a
	 *  user
	 */
	public function nameFromCentralId(
		$id, $audience = self::FOR_PUBLIC, $flags = self::READ_NORMAL
	) {
	...
	/**
	 * Given a user name, return the central ID
	...
	public function centralIdFromName(
		$name, $audience = self::FOR_PUBLIC, $flags = self::READ_NORMAL
	) {
	 ...
	 *  user, $audience lacks the rights needed to see the user, or the user
	...
	/**
	 * Given a User, return the central ID
	...
	public function centralIdFromLocalUser(
		User $user, $audience = self::FOR_PUBLIC, $flags = self::READ_NORMAL
	) {
