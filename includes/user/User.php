...
class User implements IDBAccessObject {
	...
	public function setEmailWithConfirmation( $str ) {
		...
		if ( $str !== '' && $wgEmailAuthentication ) {
			...
			if ( $type == 'changed' ) {
				// Send the user an email notifying the user of the change in registered email address
				global $wgPasswordSender;
				$sender = new MailAddress( $wgPasswordSender,
					wfMessage( 'emailsender' )->inContentLanguage()->text() );

				$to = new MailAddress( $oldaddr, $this->getName(), $this->getRealName() );
				$notificationResult = UserMailer::send( $to, $sender, wfMessage('notificationemail_subject')->text(),
