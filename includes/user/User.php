		...
		if ( isset( $this->mOptions['email-blacklist'] ) ) {
			if ( $this->mOptions['email-blacklist'] ) {
				...
				if ( is_array( $value ) ) {
					...
					$names = explode( "\n", $value );
					$nameToId = array_fill_keys( array_keys( $nameToId ), false );
					$ids = array_filter( $lookup->lookupUserNames( $nameToId, $this ), function ( $id ) {
						return $id !== false;
					} );
				}
				$this->mOptions['email-blacklist'] = array_values( array_unique(  $ids ) );
