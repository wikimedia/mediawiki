...
class UserGroupMembership {
	...
	public function delete( IDatabase $dbw = null ) {
		...
		if ( !$wgDisableUserGroupExpiry && !$this->id ) {
			throw new UnexpectedValueException(
				'UserGroupMembership::delete() requires that the id member be filled' );
		}
	 ...
	 *   inserted row on success (or true on success if $wgDisableUserGroupExpiry
	 *   is true)
	 */
		...
		if ( !$wgDisableUserGroupExpiry && !$affected ) {
			...
			if ( $existingRow ) {
				...
				if ( $allowUpdate ?
					...
					( $existingRow->ug_expiry && $existingRow->ug_expiry < wfTimestampNow() )
				) {
	...
	public static function purgeExpired( IDatabase $dbw = null ) {
		...
		DeferredUpdates::addUpdate( new AtomicSectionUpdate(
			...
			function ( IDatabase $dbw, $fname ) {
				...
				// divide into array of IDs and array of users/groups
				$ids = [];
				$usersAndGroups = [];
				foreach ( $res as $row ) {
					$ids[] = $row->ug_id;
					$usersAndGroups[] = [ 'ufg_user' => $this->userId, 'ufg_group' => $this->group ];
				}

				// delete 'em all
				$dbw->delete( 'user_groups', [ 'ug_id' => $ids ], __METHOD__ );
	...
	public static function getLink( $ugm, IContextSource $context, $format,
		...
		if ( $expiry ) {
			...
			$paramsFunc = $format === 'html' ? 'rawParams' : 'params';
			$msg = $context->msg( 'group-membership-link-with-expiry' )->$paramsFunc( $groupLink )
				->params( $expiry, $expiryD, $expiryT )->text();
		} else {
			return $groupLink;
		}
	}
