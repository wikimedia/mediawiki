...
/**
 * The Age-From-Edit-Condition for autopromote
 */
class AutopromoteUserAgeFromEditCondition extends AutopromoteConditionBase {
	...
	public function getDescriptionMsgKey( IContextSource $context, User $user ) {
		return parent::getDescriptionMsgKey( $context, $user )
			. ( $user->getFirstEditTimestamp() === false ? '-no-edit' : '' );
	}
	...
	public function getDescriptionParamter( IContextSource $context, User $user ) {
		...
		return [
			...
			MWTimestamp::getInstance( $user->getFirstEditTimestamp() )
