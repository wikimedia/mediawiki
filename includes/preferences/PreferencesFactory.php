...
/**
 * The PreferencesFactory is a MediaWiki service that provides the definitions of preferences for
 * a given user. These definitions are in the form of HTMLForm descriptors.
 *
 * PreferencesForm (a subclass of HTMLForm) is used to generate the
...
class PreferencesFactory {
	...
	/** @var array */
	protected $defaultPreferences = null;

	/** @var array */
	protected $saveFilters = [
		'timecorrection' => [ PreferencesFactory::class, 'filterTimezoneInput' ],
		'rclimit' => [ PreferencesFactory::class, 'filterIntval' ],
		'wllimit' => [ PreferencesFactory::class, 'filterIntval' ],
		'searchlimit' => [ PreferencesFactory::class, 'filterIntval' ],
	];

	// Stuff that shouldn't be saved as a preference.
	protected $saveBlacklist = [
		'realname',
		'emailaddress',
	];
	 ...
	 * @return array|null
	 */
	public function getPreferences( $user, IContextSource $context ) {
		...
		$this->profilePreferences( $user, $context, $defaultPreferences );
		$this->skinPreferences( $user, $context, $defaultPreferences );
		$this->datetimePreferences( $user, $context, $defaultPreferences );
		$this->filesPreferences( $user, $context, $defaultPreferences );
		$this->renderingPreferences( $user, $context, $defaultPreferences );
		$this->editingPreferences( $user, $context, $defaultPreferences );
		$this->rcPreferences( $user, $context, $defaultPreferences );
		$this->watchlistPreferences( $user, $context, $defaultPreferences );
		$this->searchPreferences( $user, $context, $defaultPreferences );
		$this->miscPreferences( $user, $context, $defaultPreferences );

		Hooks::run( 'GetPreferences', [ $user, &$defaultPreferences ] );

		$this->loadPreferenceValues( $user, $context, $defaultPreferences );
	 ...
	 * @return array|null
	 */
	public function loadPreferenceValues( $user, $context, &$defaultPreferences ) {
		...
		foreach ( $defaultPreferences as $name => &$info ) {
			...
			if ( $disable && !in_array( $name, $this->saveBlacklist ) ) {
	 ...
	 * @return void
	 */
	public function profilePreferences( $user, IContextSource $context, &$defaultPreferences ) {
	...
	public function getFormObject(
		...
		$formDescriptor = $this->getPreferences( $user, $context );
		if ( count( $remove ) ) {
	 ...
	 */
	public function tryFormSubmit( $formData, $form ) {
		...
		foreach ( array_keys( $formData ) as $name ) {
			if ( isset( $this->saveFilters[$name] ) ) {
				$formData[$name] =
					call_user_func( $this->saveFilters[$name], $formData[$name], $formData );
			}
		}
		...
		if ( $user->isAllowed( 'editmyoptions' ) ) {
			...
			foreach ( $this->saveBlacklist as $b ) {
				unset( $formData[$b] );
			}
	 ...
	 * @return Status
	 */
	public function tryUISubmit( $formData, $form ) {
		$res = $this->tryFormSubmit( $formData, $form );
