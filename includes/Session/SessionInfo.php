 ...
 * - a SessionProvider will return a SessionInfo based on information in the request
 *   (and possibly some checks that do not involve the session store, such as DB lookups)
 * - SessionManager will look up the session ID in the session store, verify the contents
 *   of the SessionInfo, and pass an updated SessionInfo to SessionManager::getSessionFromInfo()
 *
 * @newable
 * @since 1.27
 * @ingroup Session
...
class SessionInfo implements Stringable {
	 ...
	 *  - userInfo: (UserInfo|null) User known from the request. Null means the
	 *    request contained no user information (e.g. session ID only) and we'll need
	 *    to rely on the metadata in the session store. An unverified UserInfo means
	 *    the request contained a username or similar easily forgeable data, and should
	 *    only result in a valid session if the user information in the session store
	 *    matches. A verified UserInfo means the request contained some unforgeable
	 *    proof of user identity, and should be accepted even if there is no metadata in
	 *    the session store. When the UserInfo is verified and the session store metadata
	 *    is present but does not match, behavior is governed by the 'forceUse' flag.
	 *  - persisted: (bool) Whether this session was persisted
	 *  - remembered: (bool) Whether the verified user used the "remember me" option
	 *    during login, which should result in longer login lifetimes.
	 *    Only meaningful when the UserInfo is verified (as the longer lifetime means
	 *    we can't rely on the session store for verification).
	 *    Defaults to true.
	 ...
	 * - $this->getProvider()->persistsSessionId() is false (the client cannot be
	 *   manipulated into using an arbitrary session ID).
