...
function execute() {
	# Ugly but don't want to pollute SpecialUserLogin for one temporary
	# override.
	global $wgRequest;

	if ( $wgRequest->getBool( 'agoraForm' ) ) { // new appearance
?>
...
<?php } ?>
	...
	<input type="hidden" name="agoraForm" value="1" />
<?php if( $this->haveData( 'uselang' ) ) { ?><input type="hidden" name="uselang" value="<?php $this->text( 'uselang' ); ?>" /><?php } ?>
<?php if( $this->haveData( 'token' ) ) { ?><input type="hidden" name="wpLoginToken" value="<?php $this->text( 'token' ); ?>" /><?php } ?>
</form>
<?php if( $this->haveData( 'createOrLoginHref' ) ) { ?>
<div id="createaccount-cta">
    <h3 id="userloginlink"><?php $this->msg( 'donthaveaccount' ) ?><a href="<?php $this->text( 'createOrLoginHref' ) ?>" id="joinwikipedia" class="mw-btn green"><?php $this->msg( 'joinwikipedia' ) ?></a></h3>
</div>
</div>
<?php } ?>
