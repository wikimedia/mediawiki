	...
	function execute() {
		...
		<div>
			<label for='wpName2'>
				...
				<span
					class="mw-ui-flush-right"><?php echo $this->getMsg( 'createacct-helpusername' )->parse(); ?></span>
			</label>
			<?php
		...
		<?php if ( $this->haveData( 'uselang' ) ) { ?><input type="hidden" name="uselang"
															 value="<?php $this->text( 'uselang' ); ?>" /><?php } ?>
		<?php if ( $this->haveData( 'token' ) ) { ?><input type="hidden" name="wpCreateaccountToken"
														   value="<?php $this->text( 'token' ); ?>" /><?php } ?>
		</form>
