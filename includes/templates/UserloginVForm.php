	...
	if( $this->haveData( 'extrafields' ) ) {
		...
		<div>
			<?php if( $this->data['canremember'] ) {
				...
				echo Xml::openElement( 'label', array(
					'for' => 'wpRemember',
					'class' => 'mw-ui-checkbox-label'
				) );
				echo Xml::check( 'wpRemember', $this->data['remember'], array(
					'id' => 'wpRemember',
					'tabindex' => 4
				) );
				echo $this->getMsg( 'userlogin-remembermypassword' )->numParams( $expirationDays )->text();
				echo Xml::closeElement( 'label' );
			} ?>
		</div>
<?php if( $this->data['cansecurelogin'] ) { ?>
		<div>
			...
			echo Xml::check( 'wpStickHTTPS', $this->data['stickHTTPS'], array(
				'id' => 'wpStickHTTPS',
				'tabindex' => 5
			) );
			echo $this->getMsg( 'securelogin-stick-https' )->text();
			echo Xml::closeElement( 'label' );
			?>
		</div>
<?php } ?>
