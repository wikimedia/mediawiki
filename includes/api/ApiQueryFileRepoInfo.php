...
class ApiQueryFileRepoInfo extends ApiQueryBase {
	...
	public function execute() {
		// Helper - necessary for local and foreign repos
		function filterResults( $filter, $info ) {
			$results = array();
		...
		$repoGroup->forEachForeignRepo( function ( $repo ) use ( &$params, &$repos ) {
			$repos[] = filterResults( $params['prop'], $repo->getInfo() );
		} );

		$repos[] = filterResults( $params['prop'], $repoGroup->localRepo->getInfo() );
	...
	public function getProps() {
		...
		return array_merge( $props, array_keys( $repoGroup->localRepo->getInfo() ) );
	}
