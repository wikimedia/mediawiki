...
class ApiHelp extends ApiBase {
	...
	public function execute() {
		...
		foreach ( $modules as $m ) {
			...
			for ( $i = 0; $i < count( $subNames ); $i++ ) {
				$subs = $module->getSubModules();
				if ( is_null( $subs ) ) {
					$module = null;
				} else {
					$module = $subs->instantiateModule( $subNames[ $i ] );
				}
				if ( is_null( $module ) ) {
					$name = count( $subNames ) === 2 && $i === 1 && $subNames[0] === 'query' && in_array( $subNames[1], $queryModules )
						? $subNames[ 1 ]
						: implode( '+', array_slice( $subNames, 0, $i + 1 ) );
	...
	public function getExamples() {
		return array(
			...
			'api.php?action=help&modules=query+categorymembers' => 'Query categorymembers modules help page',
			'api.php?action=help&modules=login|query+info' => 'Login and query_info modules help page',
		);
	}
