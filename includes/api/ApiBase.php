...
abstract class ApiBase extends ContextSource {
	 ...
	 * @return Status
	 */
	public static function errorArrayToStatus( array $errors ) {
		$status = Status::newGood();
		foreach ( $errors as $error ) {
			if ( is_array( $error ) && $error[0] === 'blockedtext' ) {
				$status->fatal( ApiMessage::create(
					'apierror-blocked',
					'blocked',
					[ 'blockinfo' => ApiQueryUserInfo::getBlockInfo( $user->getBlock() ) ]
				) );
			} elseif ( is_array( $error ) && $error[0] === 'autoblockedtext' ) {
				$status->fatal( ApiMessage::create(
	...
	public function checkTitleUserPermissions( Title $title, $actions, $user = null ) {
		...
		if ( $errors ) {
			$this->dieStatus( self::errorArrayToStatus( $errors ) );
		}
	}
	...
	public function getErrorFromStatus( $status, &$extraData = null ) {
		...
		$msg = Message::newFromSpecifier( $errors[0] );
	...
	private static $messageMap = [
		...
		'autoblockedtext' => 'autoblockedtext',
