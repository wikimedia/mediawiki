...
class ApiQueryAllLinks extends ApiQueryGeneratorBase {
	...
	public function getParamDescription() {
		...
		return array(
			...
			'unique' => "Only show unique {$link}s. Cannot be used with {$p}prop=ids. As generator yields target pages instead of source pages.",
			'prop' => array(
	...
	public function getExamples() {
		...
		$link = $this->descriptionLink;
		return array(
			"api.php?action=query&list=all{$this->descriptionLink}s&{$p}from=B&{$p}prop=ids|title" 
					=> "List $link titles with page ids they are from, including missing links. Start at B",
			"api.php?action=query&list=all{$this->descriptionLink}s&{$p}unique=&{$p}from=B"
					=> "List unque $link titles",
			"api.php?action=query&generator=all{$this->descriptionLink}s&g{$p}unique=&g{$p}from=B"
					=> "As 'unique' generator, gets all $link target pages",
			"api.php?action=query&generator=all{$this->descriptionLink}s&g{$p}from=B"
					=> "As generator without unque, gets pages containing the {$link}s",
		);
	}
