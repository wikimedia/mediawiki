...
class ApiStashEdit extends ApiBase {
	...
	public function execute() {
		...
		if ( $user->pingLimiter( 'stashedit' ) ) {
			...
			$stats->increment( "editstash.cache_stores.ratelimited" );
	...
	public static function parseAndStash( WikiPage $page, Content $content, User $user, $summary ) {
		...
		//
		// MySQL 5.7.5+ allows lock names only up to 64 characters, so use sha1() to shorten it.
		$dbw = wfGetDB( DB_MASTER );
		if ( !$dbw->lock( sha1( $key ), __METHOD__, 0 ) ) {
			// De-duplicate requests on the same key
			return self::ERROR_BUSY;
		}
		/** @noinspection PhpUnusedLocalVariableInspection */
		$unlocker = new ScopedCallback( function () use ( $dbw, $key ) {
			$dbw->unlock( sha1( $key ), __METHOD__ );
		} );
		...
		if ( $editInfo && $editInfo->output ) {
			...
			if ( $stashInfo ) {
				...
				if ( $ok ) {
					...
					// Our test cache can't fail, so we can't test this
	...
	public static function checkCache( Title $title, Content $content, User $user ) {
		...
		if ( !$editInfo ) {
			...
			$logger->info( "Cache for key '$key' ('$title') has vary_revision." ); // @codeCoverageIgnore
		} elseif ( $editInfo->output->getFlag( 'vary-revision-id' ) ) {
			// Similar to the above if we didn't guess the ID correctly.
			$logger->info( "Cache for key '$key' ('$title') has vary_revision_id." ); // @codeCoverageIgnore
		}
