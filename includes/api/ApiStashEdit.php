...
class ApiStashEdit extends ApiBase {
	...
	public static function parseAndStash( WikiPage $page, Content $content, User $user, $summary ) {
		...
		if ( $stashedInfo && wfTimestamp( TS_UNIX, $stashedInfo->timestamp ) >= $cutoffTime ) {
			...
			$derivedDataUpdater = $page->getDerivedDataUpdater( $user, null, $slots );
			$derivedDataUpdater->prepareRevisionCreation( $user, $slots, [], false );
			$output = $derivedDataUpdater->getCanonicalParserOutput();

			list( $stashedInfo, , ) = self::buildStashValue(
				$derivedDataUpdater->getRawContent( 'main' ), // post-PST
				$output,
				$output->getCacheTime(),
				$user
			);
