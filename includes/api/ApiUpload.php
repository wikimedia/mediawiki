...
class ApiUpload extends ApiBase {
	...
	private function performStash( $failureMode, &$data = null ) {
		try {
			...
			list( $msg, $code ) = $this->handleStashException( $e );
		...
		if ( $status->isGood() ) {
			...
			if ( $failureMode === 'critical' ) {
				$this->dieStatus( $status );
			} elseif ( $failureMode === 'optional' ) {
				list( $code, $msg ) = $this->getErrorFromStatus( $status );
				$data['stashfailed'] = $msg;
				return null;
			} else {
				throw new MWException( "Invalid failure mode '$failureMode' given" );
			}
		}
	}
	...
	protected function handleStashException( $e ) {
		...
		switch ( $exceptionType ) {
			...
			case 'UploadStashNotLoggedInException':
				return [ 'Not logged in: ' . $e->getMessage(), 'stashnotloggedin' ];
			case 'UploadStashWrongOwnerException':
				return [ 'Wrong owner: ' . $e->getMessage(), 'stashwrongowner' ];
			case 'UploadStashNoSuchKeyException':
				return [ 'No such filekey: ' . $e->getMessage(), 'stashnosuchfilekey' ];
			default:
				return [ $exceptionType . ': ' . $e->getMessage(), 'stasherror' ];
		}
	}
