...
class ApiUpload extends ApiBase {
	...
	public function execute() {
		...
		$this->mParams['async'] =
			( $this->mParams['async'] && $this->getConfig()->get( 'EnableAsyncUploads' ) );
	...
	protected function handleStashException( $e ) {
		...
		switch ( $exceptionType ) {
			...
			case 'UploadStashZeroLengthFileException':
				$this->dieUsage(
					'File is of zero length, and could not be stored in the stash: ' . $e->getMessage(),
					'stashzerolength'
				);
				break;
			case 'UploadStashNotLoggedInException':
				$this->dieUsage(
					'Not logged in: ' . $e->getMessage(),
					'stashnotloggedin'
				);
				break;
			case 'UploadStashWrongOwnerException':
				$this->dieUsage(
					'Wrong owner: ' . $e->getMessage(),
					'stashwrongowner'
				);
				break;
			case 'UploadStashNoSuchKeyException':
				$this->dieUsage(
					'No such filekey: ' . $e->getMessage(),
					'stashnosuchfilekey'
				);
				break;
			default:
