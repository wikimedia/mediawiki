...
class ApiOpenSearch extends ApiBase {
	...
	protected function search( $search, $limit, $namespaces, $resolveRedir, &$results ) {
		...
		$suggestions = $searchEngine->searchSuggestions( $search );

		$titles = array();
		foreach ( $suggestions as $s ) {
			// opensearch supports only suggestions with a Title
			if ( $s->getSuggestedTitle() !== null ) {
				$titles[] = $s->getSuggestedTitle();
			}
		}
		if ( !$titles ) {
			return;
