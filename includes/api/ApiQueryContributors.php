...
class ApiQueryContributors extends ApiQueryBase {
	...
	public function execute() {
		...
		if ( $params['continue'] !== null ) {
			...
			$this->dieContinueUsageIf( $cont[2] !== (string)$cont_user );
		}
		...
		// First, count anons (but only for pages where we haven't already)
		if ( $cont_anonpage === '-' ) {
			$anonpages = [];
		...
		if ( $anonpages ) {
			...
			if ( count( $anonpages ) === 1 ) {
				// See below
				$this->addWhere( 'rev_page = ' . (int)$anonpages[0] );
			} else {
			...
			foreach ( $counts as $page => $count ) {
				...
				if ( !$fit ) {
					$this->setContinueEnumParameter( 'continue', "$page|$cont_page|$cont_user" );
					return;
				}
			}
		...
		foreach ( $res as $row ) {
			if ( ++$count > $params['limit'] ) {
				...
				$this->setContinueEnumParameter( 'continue', "$cont_anonpage|$row->page|$row->user" );
				return;
			}
