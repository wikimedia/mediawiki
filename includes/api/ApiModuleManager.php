...
class ApiModuleManager extends ContextSource {
	 ...
	 * @param bool $allowMultiver if false, makes sure that only one version of the given module is created
	 * @return mixed the new module instance, or null if failed
	 */
	public function getModule( $moduleName, $group = null, $ignoreCache = false, $allowMultiver = false ) {
		...
		if ( !$ignoreCache && isset( $this->mInstances[$moduleName] ) ) {
			...
			if ( !$ignoreCache ) {
				...
				if ( !$allowMultiver && array_key_exists( $mod, $this->mInstanceVersions ) ) {
