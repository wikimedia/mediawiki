...
class ApiQueryTags extends ApiQueryBase {
	...
	public function execute() {
		...
		foreach ( $tags as $tagName => $hitcount ) {
			...
			if ( $fld_defined ) {
				$tag['defined'] = $changeTagsContext->isDefined( $tagName );
			}

			if ( $fld_source ) {
				$tag['source'] = [];

				if ( $changeTagsContext->isSoftwareDefined( $tagName ) ) {
					// TODO: Can we change this to 'software'?
					$tag['source'][] = 'extension';
				}
				if ( $changeTagsContext->isUserDefined( $tagName ) ) {
					$tag['source'][] = 'manual';
				}
			}

			if ( $fld_active ) {
				$tag['active'] = $changeTagsContext->isActive( $tagName );
			}
