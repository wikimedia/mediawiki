...
class ApiQueryAllImages extends ApiQueryGeneratorBase {
	...
	private function run( $resultPageSet = null ) {
		...
		$dir = ( in_array( $params['dir'], array( 'descending', 'older' ) ) ? 'descending' : 'ascending' );
		...
		if ( $params['sort'] == 'name' ) {
			...
			foreach ( $disallowed as $pname ) {
				if ( isset( $params[$pname] ) ) {
					$this->dieUsage( "Parameter '{$preffix}{$pname}' can only be used with {$preffix}sort=timestamp", 'badparams' );
				}
			}
			if ( $params['filterbots'] != 'all' ) {
					$this->dieUsage( "Parameter '{$preffix}filterbots' can only be used with {$preffix}sort=timestamp", 'badparams' );
			}
			...
			// Check mutually exclusive params
			$disallowed = array( 'continue', 'from', 'to', 'prefix', 'sha1', 'sha1base36' );
			foreach ( $disallowed as $pname ) {
				if ( isset( $params[$pname] ) ) {
					$this->dieUsage( "Parameter '{$preffix}{$pname}' can only be used with {$preffix}sort=name", 'badparams' );
				}
			}

			// Pagination
			$rangedir = ( $dir == 'ascending' ? 'newer' : 'older' );
			$this->addTimestampWhereRange( 'img_timestamp', $rangedir, $params['start'], $params['end'] );
	...
	public function getParamDescription() {
		...
		return array(
			...
			'continue' => "When more results are available, use this to continue. Can only be used with {$p}sort=name",
			...
			'sha1' => "SHA1 hash of image. Overrides {$p}sha1base36. Can only be used with {$p}sort=timestamp",
			'sha1base36' => "SHA1 hash of image in base 36 (used in MediaWiki). Can only be used with {$p}sort=timestamp",
	...
	public function getPossibleErrors() {
		...
		return array_merge( parent::getPossibleErrors(), array(
			...
			array( 'code' => 'badparams', 'info' => "Parameter'{$p}continue' can only be used with {$p}sort=name" ),
			...
			array( 'code' => 'badparams', 'info' => "Parameter'{$p}sha1' can only be used with {$p}sort=name" ),
			array( 'code' => 'badparams', 'info' => "Parameter'{$p}sha1base36' can only be used with {$p}sort=name" ),
