 ...
 * @ingroup API
 */
class ApiResult extends ContextSource {
	...
	public function __construct(
		...
		if ( $context instanceof ContextSource ) {
	...
	public static function setValue( array &$arr, $name, $value, $flags = 0 ) {
		...
		if ( !$exists || ( $flags & ApiResult::OVERRIDE ) ) {
			...
			if ( !$conflicts ) {
				...
				throw new MWException( "Conflicting keys ($keys) when attempting to merge element $name" );
			}
		} else {
			throw new MWException( "Attempting to add element $name=$value, existing value is {$arr[$name]}" );
		}
	}
	...
	private static function validateValue( $value ) {
		...
		if ( is_array( $value ) ) {
			...
			throw new MWException( "Cannot add non-finite floats to ApiResult" );
		} elseif ( $value !== null && !is_scalar( $value ) ) {
			$type = gettype( $value );
			if ( is_resource( $value ) ) {
				$type .= '(' . get_resource_type( $value ) . ')';
			}
			throw new MWException( "Cannot add $type to ApiResult" );
		}
	...
	public function removeValue( $path, $name, $flags = 0 ) {
		...
		if ( $name === null ) {
			if ( !$path ) {
				throw new MWException( 'Cannot remove the data root' );
			}
			$name = array_pop( $path );
		}
	...
	public static function setContentValue( array &$arr, $name, $value, $flags = 0 ) {
		if ( $name === null ) {
			throw new MWException( 'Content value must be named' );
		}
		...
		foreach ( $status->errors as $error ) {
			...
			if ( is_array( $error ) && isset( $error['message'] ) ) {
				...
				if ( $error['message'] instanceof Message ) {
					...
					$msg = $this->msg( $error['message'], $error['params'] );
				}
	...
	private function addWarningOrError( $tag, $moduleName, $msg, $code, $data ) {
		...
		if ( $this->errorContext === null ) {
			$this->errorContext = new DerivativeContext( $this->getContext() );
	...
	public static function setIndexedTagName( array &$arr, $tag ) {
		if ( !is_string( $tag ) ) {
			throw new MWException( 'Bad tag name' );
		}
	 ...
	 *
	 * Types are:
	 *  - default: Guess between 'assoc' and 'array'
	 *  - array: Data is a plain array; keys should be ignored in output
	 *  - assoc: Data is an associative array
	 *  - kvp: Data is an associative array with arbitrary keys
	 *
	 * @since 1.25
	 * @param array &$arr
	 * @param string $type See above
	 */
	public static function setArrayType( array &$arr, $type ) {
		if ( !in_array( $type, array( 'default', 'array', 'assoc', 'kvp' ), true ) ) {
	 ...
	 * @since 1.25
	 * @param array|string|null $path See ApiResult::addValue()
	 * @param string $type See self::setArrayType()
	 */
	public function defineArrayType( $path, $tag ) {
		$arr = &$this->path( $path );
		self::setArrayType( $arr, $tag );
	}
	 ...
	 * @param string $type See self::setArrayType()
	 */
	public static function setArrayTypeRecursive( array &$arr, $type ) {
		if ( !in_array( $type, array( 'default', 'array', 'assoc', 'kvp' ), true ) ) {
			throw new MWException( 'Bad type' );
		}
		$arr[self::META_TYPE] = $type;
		foreach ( $arr as $k => &$v ) {
			if ( !self::isMetadataKey( $k ) && is_array( $v ) ) {
				self::setArrayTypeRecursive( $v, $type );
			}
		}
	}
	 ...
	 * @param string $type See self::setArrayType()
	 */
	public function defineArrayTypeRecursive( $path, $tag ) {
		$arr = &$this->path( $path );
		self::setArrayTypeRecursive( $arr, $tag );
	}
	...
	public static function removeMetadata( $data ) {
		if ( is_array( $data ) ) {
			$preserveKeys = isset( $data[self::META_PRESERVE_KEYS] )
				...
				: array();
			foreach ( $data as $k => $v ) {
				if ( self::isMetadataKey( $k ) && !in_array( $k, $preserveKeys ) ) {
					unset( $data[$k] );
	...
	public static function transformForBC( array $data ) {
		$boolKeys = isset( $data[self::META_BC_BOOLS] ) ? $data[self::META_BC_BOOLS] : array();
		foreach ( $data as $k => $v ) {
			if ( $v === true && !in_array( $k, $boolKeys ) ) {
				$data[$k] = '';
			} elseif ( $v === false && !in_array( $k, $boolKeys ) ) {
				unset( $data[$k] );
			} elseif ( is_array( $v ) ) {
		...
		if ( isset( $data[self::META_CONTENT] ) && $data[self::META_CONTENT] !== '*' ) {
			$k = $data[self::META_CONTENT];
			$data['*'] = $data[$k];
			unset( $data[$k] );
	 ...
	 *  - BCarray: If $options['BC'], like 'assoc'. Otherwise like 'array'.
	...
	public static function transformForTypes( $dataIn, $options = array() ) {
		...
		if ( $type === 'BCkvp' ) {
			if ( !isset( $metadata[self::META_KVP_KEY_NAME] ) ) {
				throw new MWException( 'Type "BCkvp" used without setting ' .
			...
			if ( !isset( $ret[$k] ) ) {
				switch ( $create ) {
					...
					default:
						...
						throw new MWException( "Path $fail does not exist" );
				}
			}
			if ( !is_array( $ret[$k] ) ) {
				$fail = join( '.', array_slice( $path, 0, $i + 1 ) );
				throw new MWException( "Path $fail is not an array" );
			}
			$ret = &$ret[$k];
		}
		return $ret;
	...
	public static function setContent( &$arr, $value, $subElemName = null ) {
		wfDeprecated( __METHOD__, '1.25' );
		if ( is_array( $value ) ) {
			throw new MWException( __METHOD__ . ': Bad parameter' );
		}
