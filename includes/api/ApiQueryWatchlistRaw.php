...
class ApiQueryWatchlistRaw extends ApiQueryGeneratorBase {
	...
	private function run( $resultPageSet = null ) {
		...
		if ( isset( $show['changed'] ) ) {
			$options['filter'] = WatchedItemStore::FILTER_ONLY_CHANGED;
		}
		if ( isset( $show['!changed'] ) ) {
			$options['filter'] = WatchedItemStore::FILTER_ONLY_NOT_CHANGED;
		}
		...
		$titles = [];
		$count = 0;
		$items = WatchedItemStore::getDefaultInstance()->getWatchedItemsForUser( $user, $options );
		foreach ( $items as $item ) {
			...
			if ( is_null( $resultPageSet ) ) {
				...
				if ( !$fit ) {
					$this->setContinueEnumParameter( 'continue', $ns . '|666' . $dbKey );
					break;
				}
			} else {
				$titles[] = $t;
