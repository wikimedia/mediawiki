...
class ApiFormatXml extends ApiFormatBase {
	...
	public function execute() {
		...
		$data = self::preTransformForTypes( $data );
	...
	private static function preTransformForTypes( $data ) {
		$metadata = array();
		$data = ApiResult::removeMetadataNonRecursive( $data, $metadata );
		...
		foreach ( $data as $k => $v ) {
			if ( is_array( $v ) ) {
				$data[$k] = self::preTransformForTypes( $v );
			}
		}
