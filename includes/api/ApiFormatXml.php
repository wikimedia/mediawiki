	...
	public static function recXmlPrint( $name, $value, $indent, $attributes = array() ) {
		...
		if ( is_array( $value ) ) {
			...
			foreach ( $value as $k => $v ) {
				if ( ApiResult::isMetadataKey( $k ) && !in_array( $k, $preserveKeys ) ) {
					continue;
				}

				$oldv = $v;
				if ( is_bool( $v ) && !in_array( $k, $bcBools ) ) {
					$v = $v ? 'true' : 'false';
				}

				if ( $k === $contentKey && ( is_scalar( $v ) || $v === null ) ) {
					$content = $v;
				} elseif ( is_int( $k ) ) {
					$indexedSubelements[$k] = $v;
				} elseif ( is_array( $v ) || is_object( $v ) || in_array( $k, $subelementKeys, true ) ) {
