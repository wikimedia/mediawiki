...
class ApiCSPReport extends ApiBase {
	...
	public function execute() {
		...
		// FIXME: Is it ok to log user-agent? Does it have to be treated specially
		// since its semi-personally identifying?
	...
	private function logReport( $flags, $logLine, $context ) {
		...
		} elseif ( in_array( 'bad-browser', $flags ) ) {
			// These reports are from browsers we intentionally
			// do not support. So not that bad a thing, but still
			// worth keeping track of, just to make sure that
			// we don't have more people using these browsers
			// than expected.
			$this->log->info( $logLine, $context );
		} else {
			// Normal report.
