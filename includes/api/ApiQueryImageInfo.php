...
class ApiQueryImageInfo extends ApiQueryBase {
	...
	protected function mergeThumbParams( $image, $thumbParams, $otherParams ) {
		...
		$h = $image->getHandler();
		if ( !$h ) {
			$this->addWarning( [ 'apiwarn-nothumb-noimagehandler', $image->getName() ] );
		...
		if ( !$paramList ) {
			...
			// handlers.
			$this->addWarning( [ 'apiwarn-badurlparam', $p, $image->getName() ] );
			$this->checkParameterNormalise( $image, $thumbParams );
			return $thumbParams;
		}
	...
	protected function checkParameterNormalise( $image, $finalParams ) {
		...
		if ( !$h->normaliseParams( $image, $finalParams ) ) {
			$this->dieWithError( [ 'apierror-urlparamnormal', $image->getName() ] );
		}
	}
