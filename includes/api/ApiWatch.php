...
class ApiWatch extends ApiBase {
	...
	public function execute() {
		...
		if ( !isset( $params['title'] ) ) {
			...
			foreach ( $res as $k => $v ) {
				//user may watch/unwatch missing titles
				if ( isset( $v['missing'] ) && isset( $v['title'] ) ) {
					$missingTitle = Title::newFromText( $v['title'] );
					$res[$k]= array_merge( $v, $this->watchTitle( $missingTitle, $user, $params ) );
				}
			}
			...
			if ( count( $extraParams ) > 0 ) {
				...
				$this->dieUsage( "The parameter {$p}title can not be used with ".  implode( ", {$p}", $extraParams ), 'invalidparammix' );
			}
