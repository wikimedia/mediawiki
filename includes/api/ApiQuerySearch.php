...
class ApiQuerySearch extends ApiQueryGeneratorBase {
	...
	private function run( $resultPageSet = null ) {
		...
		if ( isset( $prop['interwiki'] ) && $matches->hasInterwikiResults() ) {
			...
			while ( $result ) {
				$title = $result->getTitle();
				$vals = array(
					'prefix' => $result->getInterwikiPrefix(),
					'namespace' => $title->getNamespace(),
					'title' => $title->getText(),
					'url' => $title->getFullUrl(),
				);

				$apiResult->addValue( array( 'query',  'interwiki' . $this->getModuleName(),  ),
					null, $vals );
				$result = $matches->next();
			}
		}
	...
	public function getParamDescription() {
		$descriptions = array(
			...
			'prop' => array(
				...
				' interwiki        - Include interwiki results for this search if any are available',
			),
