...
class ApiRevisiondelete extends ApiBase {
	...
	public function execute() {
		...
		for ( $list->reset(); $list->current(); $list->next() ) {
			...
			$data['items'][$item->getId()] += $list->current()->getApiData( $this->getResult() );
		}
	...
	private function formatStatusMessages( $messages ) {
		...
		foreach ( $messages as $m ) {
			...
			$msg = wfMessage( $m['message'] )->useDatabase( false )->inLanguage( 'en' );
			if ( isset( $m['params'] ) ) {
	...
	public function getPossibleErrors() {
		return array_merge( parent::getPossibleErrors(),
			array(
				...
				'badparams' => "Mutually exclusive values for 'hide' and 'show'",
				'badparams' => "At least one value is required for 'hide' or 'show'",
				'badparams' => "At least one value is required for 'ids'",
			)
		);
	}
