	...
	public function execute() {
		...
		if ( !$params['parentrevid'] ) {
			if ( !is_null( $params['starttimestamp'] ) && $params['starttimestamp'] != '' ) {
				...
				$timestamp = wfTimestampNow(); // Fake wpStartime
			}
		...
		if ( is_null( $parentrevid ) ) {
			$this->dieUsageMsg( array( 'nosuchrevid', $params['parentrevid'] ) );
		}
		...
		switch ( $status->value ) {
			...
			case EditPage::AS_SUCCESS_UPDATE:
				...
				if ( $newRevId == $parentRev->getId() ) {
					$r['nochange'] = '';
				} else {
					$r['oldrevid'] = intval( $parentRev->getId() );
