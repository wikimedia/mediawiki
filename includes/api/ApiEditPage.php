...
class ApiEditPage extends ApiBase {
	public function execute() {
		...
		if ( $wgUseNewEditorBackend ) {
			$edit = new EditFormDataWrapper( $titleObj );
			$edit->setTextbox1( $params['text'] );
			$edit->setContentModelAndFormat( $contentModel, $contentFormat );

			if ( !is_null( $params['section'] ) ) {
				$section = $params['section'];
				if ( !ContentHandler::getForTitle( $titleObj )->supportsSections() ) {
					$this->dieWithError( 'sectioneditnotsupported-title',
			...
			if ( $params['basetimestamp'] !== null && (bool)$this->getMain()->getVal( 'basetimestamp' ) ) {
				$edit->setEditTime( $params['basetimestamp'] );
			} else {
				$edit->setEditTime( $pageObj->getTimestamp() );
			}
			...
			if ( count( $params['tags'] ) ) {
				$changeTags = array_filter( array_map( 'trim', $changeTags ) );
			} else {
				$changeTags = [];
			}
