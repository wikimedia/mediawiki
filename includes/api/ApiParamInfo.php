...
class ApiParamInfo extends ApiBase {
	...
	private function addModulesInfo( $params, $type, &$res, $result ) {
		...
		$mgr = $isQuery
				? $this->queryObj->getSubModules()
				: $this->getMain()->getSubModules();
		$res[$type] = array();
	...
	public function getAllowedParams() {
		$modules = $this->getMain()->getSubModules()->getNames('action');
		sort( $modules );
		$querymodules = $this->queryObj->getSubModules()->getNames();
		sort( $querymodules );
		$formatmodules = $this->getMain()->getSubModules()->getNames('format');
		sort( $formatmodules );
		return array(
			'modules' => array(
