...
class ApiParamInfo extends ApiBase {
	...
	private function addModulesInfo( $params, $type, &$res, $result ) {
		...
		if ( $isQuery ) {
			$mgr = $this->queryObj->getModuleMgr();
		} else {
			$mgr = $this->getMain()->getModuleMgr();
		}
		$res[$type] = array();
	...
	public function getAllowedParams() {
		$modules = $this->getMain()->getModuleMgr()->getNames( 'action' );
		sort( $modules );
		$querymodules = $this->queryObj->getModuleMgr()->getNames();
		sort( $querymodules );
		$formatmodules = $this->getMain()->getModuleMgr()->getNames( 'format' );
		sort( $formatmodules );
		return array(
			'modules' => array(
