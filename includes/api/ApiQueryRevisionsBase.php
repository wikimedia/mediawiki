		...
		if ( ( $this->fld_content || $this->fld_structuredcontent ) && $content ) {
			...
			if ( $this->fld_structuredcontent ) {
				...
				if ( $text === false ) {
					...
					// Set the proper metadata so content is represented properly regardless of
					// output format. Also need to force it to be an array...
					if ( is_array( $text ) || $text instanceof stdClass ) {
						$text = ApiResult::addMetadataToResultVars( (array)$text, is_object( $text ) );
					}
				}
			}
