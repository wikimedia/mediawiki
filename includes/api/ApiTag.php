...
class ApiTag extends ApiBase {
	...
	public function execute() {
		...
		$toAdd = is_null( $params['add'] ) ? array() : explode( ',', $params['add'] );
		$toRemove = is_null( $params['remove'] ) ? array() : explode( ',', $params['remove'] );
		$status = ChangeTags::updateTagsWithChecks( $toAdd, $toRemove, null,
			$params['revid'], $params['logid'], null, $params['reason'],
			$this->getUser() );
		if ( !$status->isOK() ) {
