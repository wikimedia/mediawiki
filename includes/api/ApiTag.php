...
class ApiTag extends ApiBase {
	...
	public function execute() {
		...
		$status = ChangeTags::updateTagsWithChecks( $params['add'],
			$params['remove'], null, $params['revid'], $params['logid'], null,
			$params['reason'], $this->getUser() );
		if ( !$status->isOK() ) {
		...
		if ( $status->value === true ) {
			...
			$ret['tag-event-logid'] = $status->value->logId;
			if ( $status->value->addedTags ) {
				$ret['added'] = $status->value->addedTags;
				// not using the obvious name ("tag") here, because it is already used
				// for the outer element (beneath <api>), and some people like to use
				// getElementsByTagName for evil instead of good
				$result->setIndexedTagName( $ret['added'], 'added-tag' );
			}
			if ( $status->value->removedTags ) {
				$ret['removed'] = $status->value->removedTags;
				$result->setIndexedTagName( $ret['removed'], 'removed-tag' );
			}
		}
	...
	protected function getExamplesMessages() {
		return array(
			...
			'action=tag&logid=123&remove=spam&token=123ABC'
