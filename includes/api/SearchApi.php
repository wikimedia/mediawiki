...
trait SearchApi {
	...
	private function buildProfileApiParam() {
		$config = $this->getSearchProfileParams();
		$searchEngine = MediaWikiServices::getInstance()->newSearchEngine();
		$profiles = $searchEngine->getProfiles( $config['profile-type'] );
		if ( !$profiles ) {
			return [];
		}
		...
		return [
			$config['profile-name'] => [
				...
				ApiBase::PARAM_HELP_MSG => $config['profile-help'],
	...
	public function buildSearchEngine( array $params = null ) {
		if ( $params != null ) {
			...
			// Initialize requested search profile.
			$profileParams = $this->getSearchProfileParams();
			if ( isset( $params[$profileParams['profile-name']] ) ) {
				$searchEngine->setFeatureData(
					$profileParams['profile-type'],
					$params[$profileParams['profile-name']]
				);
			}
		} else {
			$searchEngine = MediaWikiServices::getInstance()->newSearchEngine();
