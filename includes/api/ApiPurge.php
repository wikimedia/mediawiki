			...
			if ( $forceLinkUpdate || $forceRecursiveLinkUpdate ) {
				if ( !$user->pingLimiter( 'linkpurge' ) ) {
					...
					if ( $revision ) {
						...
						$updates = $pupdater->getSecondaryDataUpdates( $forceRecursiveLinkUpdate );
						...
						if ( $enableParserCache ) {
							$output = $pupdater->getCanonicalParserOutput();
							$pcache = MediaWikiServices::getInstance()->getParserCache();
							$pcache->save( $output, $page, $pupdater->getCanonicalParserOptions() );
						}
					}
				} else {
					$this->addWarning( 'apierror-ratelimited' );
