 ...
 *   - a) The next waitForReplication() call if no writes are pending on any DB
 *   - b) A commit happens on wfGetDB( DB_MASTER ) if no writes are pending on any DB
 *   - c) EnqueueableDataUpdate tasks enqueue when a commit happens on wfGetDB( DB_MASTER )
 *   - d) At the completion of execute() for Maintenance scripts
 *
...
class DeferredUpdates {
	...
	public static function execute( array &$queue, $mode, $type ) {
		...
		while ( $updates ) {
			...
			// In case of DB job queue or dangling snapshot transactions from reads
	...
	public static function tryOpportunisticExecute( $mode = 'run' ) {
		...
		self::enqueueUpdates( self::$preSendUpdates );
		self::enqueueUpdates( self::$postSendUpdates );
