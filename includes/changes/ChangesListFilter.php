...
abstract class ChangesListFilter {
	...
	public function __construct( array $filterDefinition ) {
		if ( isset( $filterDefinition['group'] ) ) {
			...
			throw new MWException( 'You must use \'group\' to specify the ChangesListFilterGroup this filter belongs to' );
		}
	 ...
	 *
	 * This means that anything in the results for the current filter is also in the
	 * results for the other one.
	 *
	 * @param ChangesListFilter The filter the current instance is a superset of
	 * @param string $contextDescription i18n key for subset message.  It is used
	 *  when both filters are selected, in the context of the other one.
	 */
	public function setAsSupersetOf( ChangesListFilter $other, $contextDescription ) {
	 ...
	 */
	public abstract function displaysOnUnstructuredUi( ChangesListSpecialPage $specialPage );
	...
	public function getJsData() {
		...
		foreach ( $this->subsetFilters as $subsetFilterInfo ) {
			$output['subset'][] = $subsetFilterInfo;
			$output['messageKeys'][] = $subsetFilterInfo['contextDescription'];
		}
