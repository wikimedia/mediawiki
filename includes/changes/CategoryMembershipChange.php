...
class CategoryMembershipChange {
	...
	/**
	 * @var WikiPage instance of the categorized page
	 */
	private $page;
	 ...
	 * false if something went wrong getting a user object
	 */
	private $user;
	...
	private function createRecentChangesEntry( $categoryName, $type ) {
		...
		if ( $this->correspondingRC ) {
			/**
			 * T109700 - Default bot flag to true when there is no revision associated with the change
			 * This means all changes caused by parser functions & Lua are marked as bot
			 * Also in the case no revision could be found due to slave lag
			 */
			$bot = $this->correspondingRC->getAttribute( 'rc_bot' ) ?: 1;
	 ...
	 * 'autochange_username' message is invalid.
	 *
	 * @return User|bool
	 */
	private function getUser() {
		...
		$username = wfMessage( 'autochange_username' )->inContentLanguage()->text();
