...
class CategoryMembershipChange {
	...
	public function __construct( Title $pageTitle, Revision $revision ) {
		...
		if ( !$this->correspondingRC ) {
			throw new MWException( __CLASS__ . ' could not be instantiated; revision does not refer to a RecentChange' );
		}
		if ( !$this->user instanceof User ) {
			throw new MWException( __CLASS__ . ' could not be instantiated; revision does not refer to a user' );
		}
	}
