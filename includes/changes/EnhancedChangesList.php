...
class EnhancedChangesList extends ChangesList {
	...
	protected function getLineData( array $block, RCCacheEntry $rcObj, array $queryParams = [] ) {
		...
		if ( $type == RC_LOG ) {
			...
			$link = $this->htmlPageLinkRenderer->makeKnownLink(
				$rcObj->getTitle(),
				$rcObj->timestamp,
				[],
				$params
	...
	protected function getLogText( $block, $queryParams, $allLogs, $isnew, $namehidden ) {
		...
		if ( !$allLogs ) {
			if ( !ChangesList::userCan( $rcObj, Revision::DELETED_TEXT, $this->getUser() ) ||
				...
				$links['total-changes'] = $this->htmlPageLinkRenderer->makeKnownLink(
					$block0->getTitle(),
					$nchanges[$n],
					[],
					$queryParams + [
				...
				if ( $sinceLast > 0 && $sinceLast < $n ) {
					$links['total-changes-since-last'] = $this->htmlPageLinkRenderer->makeKnownLink(
							$block0->getTitle(),
							$sinceLastVisitMsg[$sinceLast],
							[],
							$queryParams + [
		...
		if ( $allLogs || $rcObj->mAttribs['rc_type'] == RC_CATEGORIZE ) {
			...
			$links['history'] = $this->htmlPageLinkRenderer->makeKnownLink(
					$block0->getTitle(),
	...
	public function getDiffHistLinks( RCCacheEntry $rc, array $query ) {
		...
		$retVal = ' ' . $this->msg( 'parentheses' )
				->rawParams( $rc->difflink . $this->message['pipe-separator']
					. $this->htmlPageLinkRenderer->makeKnownLink(
						$pageTitle,
						$this->message['hist'],
						[],
						$query
