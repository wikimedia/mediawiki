...
class RecentChange {
	...
	public function save( $noudp = false ) {
		...
		$user = User::newFromRow( (object)[
			'user_id' => isset( $row['rc_user'] ) ? $row['rc_user'] : null,
			'user_name' => isset( $row['rc_user_text'] ) ? $row['rc_user_text'] : null,
			'actor_id' => isset( $row['rc_actor'] ) ? $row['rc_actor'] : null,
		] );
	...
	public function loadFromRow( $row ) {
		...
		$user = User::newFromRow( (object)[
			'user_id' => isset( $this->mAttribs['rc_user'] ) ? $this->mAttribs['rc_user'] : null,
			'user_name' => isset( $this->mAttribs['rc_user_text'] ) ? $this->mAttribs['rc_user_text'] : null,
			'actor_id' => isset( $this->mAttribs['rc_actor'] ) ? $this->mAttribs['rc_actor'] : null
		] );
		$this->mAttribs['rc_user'] = $user->getId();
		$this->mAttribs['rc_user_text'] = $user->getName();
		$this->mAttribs['rc_actor'] =
			$wgActorTableSchemaMigrationStage > MIGRATION_OLD ? $user->getActorId() : 0;
	}
	...
	public function getAttribute( $name ) {
		...
		if ( $name === 'rc_user' || $name === 'rc_user_text' || $name === 'rc_actor' ) {
			$user = User::newFromRow( (object)[
				'user_id' => isset( $this->mAttribs['rc_user'] ) ? $this->mAttribs['rc_user'] : null,
				'user_name' => isset( $this->mAttribs['rc_user_text'] )
			...
			if ( $name === 'rc_actor' ) {
				return $wgActorTableSchemaMigrationStage > MIGRATION_OLD ? $user->getActorId() : 0;
			}
		}
