	...
	private function buildHelpfulInformationMessage() {
		...
		if ( $installedSkins ) {
			...
			foreach ( $installedSkins as $skin ) {
				...
				if ( $isEnabled ) {
					$skinsInstalledText[] = wfMessage( 'default-skin-not-found-row-enabled' )
						->params( $normalizedKey, $skin )->plain();
				} else {
					$skinsInstalledText[] = wfMessage( 'default-skin-not-found-row-disabled' )
						->params( $normalizedKey, $skin )->plain();
			...
			return wfMessage( 'default-skin-not-found-no-skins' )->params(
				$wgDefaultSkin
			)->parseAsBlock();
		}
	}
