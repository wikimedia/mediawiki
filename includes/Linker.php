...
class Linker {
	...
	public static function link(
		...
		$htmlPageLinkRenderer = new HtmlPageLinkRenderer(
		...
		if ( in_array( 'forcearticlepath', $options ) ) {
		...
		if ( isset( $options['stubThreshold'] ) ) {
			$stubThreshold = $options['stubThreshold'];
		} else {
			global $wgUser;
			$stubThreshold = $wgUser->getStubThreshold();
		}

		$htmlPageLinkRenderer->setStubThreshold( $stubThreshold );

		if ( in_array( 'known', $options ) ) {
			return $htmlPageLinkRenderer->makeKnownLink( $target, $html, $query );
		} elseif ( in_array( 'broken', $options ) ) {
			return $htmlPageLinkRenderer->makeBrokenLink( $target, $html, $query );
		} else {
			return $htmlPageLinkRenderer->makeLink( $target, $html, $query );
		}
	}
