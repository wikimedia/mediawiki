...
class Linker {
	 ...
	 * other users.
	 *
	 * If $verify is set this function will return the link only in case the
	 ...
	 * @return string
	 */
	public static function generateRollback( $rev, IContextSource $context = null, $verify = false ) {
		if ( $context === null ) {
			$context = RequestContext::getMain();
		}
		$editCount = false;
		if ( $verify ) {
			$editCount = self::rollbackData( $rev, true );
			if ( $editCount === false ) {
				return '';
			}
		}
		...
		return '<span class="mw-rollback-link">'
			. $context->msg( 'brackets' )->rawParams(
				self::buildRollbackLink( $rev, $context, $editCount ) )->plain()
			. '</span>';
	}
	 ...
	 * can be reverted (is the page latest and the user isn't the only contributor)
	 * Returns null if $wgShowRollbackEditCount is disabled!
	 *
	 * @param $rev Revision object
	 * @param $verify Bool Try to verfiy that this revision can really be rolled back
	 * @return integer|bool|null
	 */
	protected static function rollbackData( $rev, $verify ) {
		...
		$where = array( 'rev_page' => $rev->getTitle()->getArticleID() );
		$joinConds = array();

		$dbr = wfGetDB( DB_SLAVE );
		if ( $verify ) {
			// Adding additional data to the SQL query to verify the rollback link
			$tables[] = 'page';
			// Use equal or bigger, as we read from the slave
			// (using the rev_id like this is a bit hacky, as higher doesn't always mean newer)
			$where[] = 'page_latest >= ' . $rev->getId();
			$joinConds = array( 'page' => array( 'INNER JOIN','page_id=rev_page') );
		}
		...
		if ( $verify && !$res ) {
			// Query returned nothing, probably the rev isn't page latest (or newer)
			return false;
		}
	...
	public static function buildRollbackLink( $rev, IContextSource $context = null, $editCount = false ) {
		...
		if( !$disableRollbackEditCount && is_int( $wgShowRollbackEditCount ) && $wgShowRollbackEditCount > 0 ) {
			if ( !is_numeric( $editCount ) ) {
				$editCount = self::rollbackData( $rev, false );
			}
