...
class Linker {
		...
		$handlerParams = array(), $time = false, $query = "" )
	{
		...
		if ( !$exists ) {
			...
			$s .= htmlspecialchars( wfMessage( 'thumbnail_error', '' )->text() );
			$zoomIcon = '';
		} else {
			$params = array(
				'alt' => $fp['alt'],
	...
	public static function formatRevisionSize( $size ) {
		if ( $size == 0 ) {
			$stxt = wfMessage( 'historyempty' )->text();
		} else {
			global $wgLang;
			$stxt = wfMessage( 'nbytes', $wgLang->formatNum( $size ) )->text();
	...
	public static function revDeleteLink( $query = array(), $restricted = false, $delete = true ) {
		...
		$html = $delete ? wfMessage( 'rev-delundel' ) : wfMessage( 'rev-showdeleted' );
		$html = $html->escaped();
