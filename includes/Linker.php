...
class Linker {
	...
	public static function link(
		...
		if ( $options ) {
			// Custom options, create new HtmlPageLinkRenderer
			if ( !isset( $options['stubThreshold'] ) ) {
				global $wgUser;
				$options['stubThreshold'] = $wgUser->getStubThreshold();
			}

			$htmlPageLinkRenderer = HtmlPageLinkRenderer::newFromLegacyOptions(
				$services->getTitleFormatter(),
				$options
			);
		} else {
			$htmlPageLinkRenderer = $services->getHtmlPageLinkRenderer();
		}
		...
		if ( in_array( 'known', $options, true ) ) {
			return $htmlPageLinkRenderer->makeKnownLink( $target, $text, $customAttribs, $query );
		} elseif ( in_array( 'broken', $options, true ) ) {
			return $htmlPageLinkRenderer->makeBrokenLink( $target, $text, $customAttribs, $query );
		} else {
			return $htmlPageLinkRenderer->makeLink( $target, $text, $customAttribs, $query );
		}
	}
