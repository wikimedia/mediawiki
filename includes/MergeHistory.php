...
class MergeHistory {
	...
	public function __construct( Title $source, Title $dest, $timestamp = false ) {
		...
		try {
			if ( $timestamp ) {
				// If we have a requested timestamp, use the latest revision up to that point as the insertion point
	...
	public function isValidMerge() {
		...
		if ( $this->timestampLimit && $this->getRevisionCount() > self::REVISION_LIMIT ) {
			$status->fatal( 'mergehistory-fail-toobig', self::REVISION_LIMIT );
		}
