...
class MergeHistory {
	...
	public function __construct( Title $source, Title $dest, $timestamp = false ) {
		...
		// If the pages are null then we can't fetch timestamps
		// (null pages should be caught by $this->isValidMerge)
		if ( !is_null( $this->source ) && !is_null( $this->dest ) ) {
			...
			try {
				if ( $timestamp ) {
					...
					$timeInsert = $mwtimestamp;
					$this->timestampLimit = $mwtimestamp;
				} else {
	...
	public function checkPermissions( User $user, $reason ) {
		...
		if ( !$user->isAllowed( 'mergehistory' ) ) {
			...
			$status->fatal( 'mergehistory-failure-permission' );
		}
	...
	public function isValidMerge() {
		...
		if ( is_null( $this->source ) || is_null( $this->dest ) ) {
			$status->fatal( 'mergehistory-failure-page-null' );
			return $status;
		}

		// If either article ID is 0, then revisions cannot be reliably selected
		if ( $this->source->getArticleID() === 0 || $this->dest->getArticleID() === 0 ) {
			$status->fatal( 'mergehistory-failure-bad-article-id' );
			return $status;
		}

			// Make sure page aren't the same
		if ( $this->source->getArticleID() == $this->dest->getArticleID() ) {
			$status->fatal( 'mergehistory-failure-pages-same' );
		}

		// Pages must exist with revisions
		if ( !$this->source->isKnown() ) {
			$status->fatal( 'mergehistory-failure-source-nonexistant' );
		}
		if ( !$this->dest->isKnown() ) {
			$status->fatal( 'mergehistory-failure-dest-nonexistant' );
		}

		// Make sure the timestamp is valid
		if ( !$this->timestampLimit ) {
			$status->fatal( 'mergehistory-failure-bad-timestamp' );
		}

		// $this->timestampLimit must be older than $this->maxTimestamp
		if ( $this->timestampLimit > $this->maxTimestamp ) {
			$status->fatal( 'mergehistory-failure-timestamps-overlap' );
		}

		// Check that there are not too many revisions to move
		if ( $this->getRevisionCount() > self::REVISION_LIMIT ) {
			$status->fatal( 'mergehistory-failure-too-many-revisions' );
		}
	...
	public function merge( User $user, $reason = '' ) {
		...
		if ( $this->revisionsMerged < 1 ) {
			$status->fatal( 'mergehistory-failure-no-change' );
			return $status;
		}
		...
		if ( !$haveRevisions ) {
			...
			if ( $redirectContent ) {
				...
				$status->warning( 'mergehistory-warning-redirect-create-fail' );
			}
		} else {
