...
class MessagePosterRegistry {
	...
	/**
	 * Creates the MessagePosterRegistry and registers the specified implementations
	 *
	 * @param array $contentModelMessagePosters Mapping of content model string to
	 *   class name of implementation
	 */
	public function __construct( $contentModelMessagePosters ) {
		foreach ( $contentModelMessagePosters as $contentModel => $className ) {
			$this->register( $contentModel, $className );
		}
	}
	 ...
	 */
	protected function register( $contentModel, $className ) {
	 ...
	 */
	protected function unregister( $contentModel ) {
	...
	protected function createForContentModel( $contentModel, $title ) {
		if ( !isset( $this->contentModelToClass[$contentModel] ) ) {
			...
			$message = '"' . $prefixedDbText . '" has the content model "' .
				$contentModel . '", which is not registered';
