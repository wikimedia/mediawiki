...
class WikitextMessagePoster implements IMessagePoster {
	 ...
	 *
	 * Service used to format a LinkTarget
	 */
	protected $titleFormatter;

	/**
	 * @param TitleFormatter $titleFormatter Service for formatting link targets
	 */
	...
	public function postTopic( LinkTarget $linkTarget, User $user, $subject, $body, $flags = 0 ) {
		...
		$params = [
			'action' => 'edit',
			'title' => $this->titleFormatter->getPrefixedText( $linkTarget ),
			'section' => 'new',
			'summary' => $subject,
			'text' => $body,
