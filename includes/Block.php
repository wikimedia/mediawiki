...
class Block {
	 ...
	 *
	 * The cookie's expiry time will be set based on the Block's expiry time:
	 * 1. If less than $wgCookieExpiration, the Block expiry will be used.
	 * 2. If between $wgCookieExpiration and 'infinity', $wgCookieExpiration will be used
	 *    (defaults to 180 days).
	 * 3. If set to 'infinity', then $wgAutoblockExpiry will be used
	 *    (defaults to 1 day).
	 *
	...
	public function setCookie( WebResponse $response, $setEmpty = false ) {
		...
		if ( $this->getExpiry() === 'infinity' ) {
			...
			$expiry = wfTimestamp( TS_UNIX, self::getAutoblockExpiry( wfTimestampNow() ) );
