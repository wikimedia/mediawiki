...
class Block {
	 ...
	 *
	 * @param String $xff Comma separated list of IP's, probably retrieved from
	 *	   the X-Forwarded-For header of the request
	 * @param Bool $isAnon Exclude anonymous-only blocks if false
	 * @param String|Bool $ip IP to exclude from check, or false to check every IP
	 ...
	 * @since 1.21
	 */
	public static function newFromXFF( $xff, $isAnon, $ip = false, $fromSlave = true ) {
		global $wgApplyIpBlocksToXff;
		if ( !$wgApplyIpBlocksToXff || !$xff ) {
			return null;
		}
