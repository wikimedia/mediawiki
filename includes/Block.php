...
class Block {
	...
	protected function initFromRow( $row ) {
		...
		if ( isset( $row->ipb_by_actor ) && $row->ipb_by_actor ) { // actor, local or remote
			$this->setBlocker( User::newFromActorId( $row->ipb_by_actor ) );
		} elseif ( $row->ipb_by ) { // local user
	...
	protected static function defaultRetroactiveAutoblock( Block $block, array &$blockIds ) {
		...
		$dbr = wfGetDB( DB_REPLICA );
		$rcQuery = ActorMigration::newKey( 'rc_user' )->getWhere( $dbr, $target );
