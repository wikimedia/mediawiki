...
class Block {
	 ...
	 *
	 * @since 1.25 accepts $options array instead of individual parameters; order
	...
	function __construct( $options = array() ) {
		...
		if ( func_num_args() > 1 || !is_array( $options ) ) {
			...
			wfDeprecated( __METHOD__ . ' with multiple arguments', '1.25' );
		}
			...
			|| $options['expiry'] == wfGetDB( DB_SLAVE )->getInfinity() )
		{
			...
			$this->mExpiry = wfTimestamp( TS_MW, $options['expiry'] );
		}
