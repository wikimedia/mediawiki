...
class OutputPage extends ContextSource {
	...
	/**
	 * Depending on the Skin, this may get lazy-initialised in either headElement()
	 * or getBottomScripts(). See SkinTemplate::prepareQuickTemplate().
	 *
	 ...
	 */
	private function getRlClient() {
		if ( !$this->rlClient ) {
			// Ideally we'd construct ResourceLoaderClientHtml early on and forward addModules(),
			// but we can't because disallowUserJs() and others can affect filters retroactively.
			...
			$rlClient->setExemptStates( [
				// Manually handled by buildExemptModules() and getScriptsForBottomQueue()
				'site.styles' => 'ready',
				'noscript' => 'ready',
				'user.cssprefs' => 'ready',
				'user' => 'loading'
	...
	public function headElement( Skin $sk, $includeStyle = true ) {
		...
		$pieces = array_merge( $pieces, $this->getHeadLinksArray() );
		$pieces = array_merge( $pieces, $this->mHeadItems );
	 ...
	 */
	protected function makeResourceLoaderLink( $modules, $only, array $extraQuery = [] ) {
		...
		if ( ResourceLoader::inDebugMode() && count( $modules ) > 1 ) {
			...
			foreach ( $modules as $name ) {
				$chunks[] = $this->makeResourceLoaderLink( $name, $only, $extraQuery );
			}
			return WrappedString::join( "\n", $chunks );
		}

		return ResourceLoaderClientHtml::makeResourceLoaderLink(
	...
	/**
	 * JS stuff to put at the 'bottom', which goes at the bottom of the `<body>`. These are
	 * modules marked with position 'bottom', legacy scripts ($this->mScripts), and user JS.
	 *
	 * @return string|WrappedStringList HTML
	 */
	protected function getScriptsForBottomQueue() {
		$rlClient = $this->getRlClient();
		$chunks = [];

		$chunks[] = $rlClient->getBodyHtml();
		...
		return self::getHtmlFromLoaderLinks( $chunks );
	}
	...
	protected function buildExemptModules() {
		...
		if ( $this->getConfig()->get( 'AllowUserCss' ) && $this->getTitle()->isCssSubpage()
			...
			$chunk = $this->makeResourceLoaderLink(
				'user',
				ResourceLoaderModule::TYPE_STYLES,
				[ 'excludepage' => $this->getTitle()->getPrefixedDBkey() ]
			);
		...
		// That way, user and site styles will stay at the end of the cascade.
		$chunks[] = Html::element(
			'meta',
			[ 'name' => 'ResourceLoaderDynamicStyles', 'content' => '' ]
		);

		foreach ( $groups as $grpModules ) {
			$chunks[] = $this->makeResourceLoaderLink( $grpModules,
				ResourceLoaderModule::TYPE_STYLES
			);
		}
