...
class OutputPage extends ContextSource {
	...
	public function output() {
		...
		if ( $this->mArticleBodyOnly ) {
			...
			$this->addModules( 'mediawiki.page.startup' ); // add required startup module
			$sk = $this->getSkin();
			$modules = $sk->getDefaultModules();
			foreach( $modules as $group ) {
				$this->addModules( $group ); // add skin specific modules
			}
			MWDebug::addModules( $this );
			if ( $wgUseAjax ) {
				wfRunHooks( 'AjaxAddScript', array( &$this ) ); // FIXME: deprecate? - not clear why this is useful
			}
