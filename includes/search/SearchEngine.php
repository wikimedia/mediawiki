...
class SearchEngine {
	...
	protected function completionSearchBackend( $search ) {
		$results = array();
		$srchres = new SearchSuggestionSet();
		...
		if ( !Hooks::run(
			...
			// Hook returns array of strings
			foreach ( $results as $n => $text ) {
				$srchres->append( SearchSuggestion::fromText( $n, $text ) );
			}
		} else {
			// Hook did not work, use PrefixSearch as default
			$prefSearch = new TitlePrefixSearch();
			$results = $prefSearch->defaultSearchBackend($search, $this->limit,
					$this->namespaces, $this->offset );
			foreach ( $results as $n => $title ) {
				$srchres->append( SearchSuggestion::fromTitle( $n, $title ) );
			}
		}
		return $srchres;
	}
	...
	public function extractTitles( SearchSuggestionSet $completionResults ) {
		return $completionResults->map( function( SearchSuggestion $sugg ) { return $sugg->getSuggestedTitle(); } );
	}
	 ...
	 * @return SearchSuggestionSet
	 */
	public function processCompletionResults( $search, SearchSuggestionSet $suggestions ) {
