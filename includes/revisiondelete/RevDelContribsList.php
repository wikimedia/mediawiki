	...
	function __construct( IContextSource $context, Title $title, array $ids ) {
		...
		foreach ( $ids as $revId ) {
			...
			if ( $this->targetUser === null ) {
				// target user is user who made the first rev
				$this->isIP = !$rev->getUser( Revision::RAW );
				$this->targetUser = $rev->getUserText( Revision::RAW );
				if ( !$this->targetUser ) {
					throw new MWException( 'No user could be retrieved from the contribution page!' );
				}
			} elseif ( $this->targetUser !== $rev->getUserText( Revision::RAW ) ) {
				// skip if from a distinct user
				continue;
			}
