...
/**
 * Implementation of DomainEventSink and DomainEventSource based on
 * HookContainer and DeferredUpdates.
 *
 * Since this implementation of DomainEventSink is based on HookContainer,
...
class DomainEventDispatcher implements DomainEventSink, DomainEventSource {
	 ...
	 */
	public function send( DomainEvent $event, IConnectionProvider $dbProvider ): void {
	...
	private function registerTrigger( string $eventName ) {
		$this->hookContainer->register(
			...
			function ( DomainEvent $event, IConnectionProvider $dbProvider ) use ( $eventName ) {
				$this->dispatch( $event, $dbProvider );
			}
		);
	}
	 ...
	 */
	private function dispatch( DomainEvent $event, IConnectionProvider $dbProvider ) {
