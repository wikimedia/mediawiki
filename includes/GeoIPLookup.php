...
class GeoIPLookup {
	...
	public static function isEnabled() {
		...
		return $wgGeoIPDataDirectory !== false
			&& class_exists( Reader::class )
			&& file_exists( "{$wgGeoIPDataDirectory}/GeoLite2-City.mmdb" )
			&& file_exists( "{$wgGeoIPDataDirectory}/GeoLite2-ASN.mmdb" );
	}
	...
	public function getCityInfo( $ip ) {
		if ( !file_exists( "{$this->dataDirectory}/GeoLite2-City.mmdb" ) ) {
			wfDebug( '' );
			return false;
		}
		if ( !$this->cityDb ) {
	...
	public function getASNInfo( $ip ) {
		if ( !file_exists( "{$this->dataDirectory}/GeoLite2-ASN.mmdb" ) ) {
			return false;
		}
		if ( !$this->asnDb ) {
