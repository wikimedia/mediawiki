...
class MediaWiki {
	...
	protected function triggerJobs() {
		...
		$query = array( 'action' => 'chron',
			'tasks' => 'jobs', 'maxjobs' => $n, 'sigexpiry' => time() + 5 );
		$query['signature'] = ApiChron::getQuerySignature( $query );
		...
		if ( !$sock ) {
			wfDebug( "Failed to start chron API (socket error $errno): $errstr\n" );
			return;
		}
		...
		// Send a chron API request to be performed in the background.
		// Give up if this takes to long to send (which should be rare).
		stream_set_timeout( $sock, 1 );
		$bytes = fwrite( $sock, $req );
		if ( $bytes !== strlen( $req ) ) {
			wfDebug( "Failed to start chron API (socket write error)\n" );
		} else {
			// Do not wait for the response (the script should handle client aborts).
			// Make sure that we don't close before that script reaches ignore_user_abort().
			$status = fgets( $sock );
			if ( !preg_match( '#^HTTP/\d\.\d 204 #', $status ) ) {
				wfDebug( "Failed to start chron API: received '$status'\n" );
			}
		}
		fclose( $sock );
	}
