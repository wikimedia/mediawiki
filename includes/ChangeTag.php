...
class ChangeTag {
	...
	/**
	 * Get full context for a ChangeTag object
	 * @return object
	 * @since 1.26
	 */
	 ...
	 *
	 * @param array $tagStats Array mapping tags to their hitcount
	 * @since 1.26
	 */
	public function setHitcount( $tagStats ) {
		$this->hitcount = $value;
	}
	 ...
	 * as a static object
	 *
	 * @param object $changeTagContext
	 * @since 1.26
	 */
	public function setContext( $changeTagContext ) {
		$this->context = $changeTagContext;
	...
	public function getExtMsgPar() {
		...
		if ( $changeTag->extensionDefined && isset( $this->params['descParams'] ) ) {
	...
	/**
	 * Returns whether the tag exists, in the sense of being attached to edits or defined
	 * somewhere
	 *
	 * @return bool
	 * @since 1.26
	...
	public function exists() {
		if ( !isset( $this->context ) || !isset( $this->hitcount ) ) {
			throw new Exception( "Cannot determine existence if context and hitcount are
				not both set." );
		}
		return ( $this->hitcount > 0 ) || $this->extensionDefined || $this->userDefined;
	}
	...
	public function canDeactivate( User $user = null ) {
		...
		if ( !$changeTag->isStored || ( $changeTag->userDefined && $changeTag->hitcount == 0 ) ) {
	...
	/**
	 * Increment hitcount cache of the given tag
	 *
	 * @param string $tag
	 * @since 1.26
	 */
	public static function incrementHitcount( $tag ) {
		global $wgMemc;
