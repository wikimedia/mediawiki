	...
	public function get( $type ) {
		...
		if ( isset( $wgJobTypeConf[$type] ) ) {
			return JobQueue::factory( array(
				'class' => $wgJobTypeConf[$type]['class'],
				'wiki'  => $this->wiki,
				'store' => $wgJobTypeConf[$type]['store']
			) );
		} else {
			return JobQueue::factory( array(
				'class' => $wgJobTypeConf['default']['class'],
				'wiki'  => $this->wiki,
				'store' => $wgJobTypeConf['default']['store'],
				'type'  => $type
			) );
		}
	}
...
abstract class JobQueue {
	...
	public function waitForBackup() {
		$this->store->waitForSlaves( $this->getType() );
	}
}
