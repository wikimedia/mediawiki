...
class JobQueueDB extends JobQueue {
	...
	protected $reduceContention = false; // bool; really try to avoid lock contention

	/**
	 * $params includes:
	 *     reduceContention : Take measures to avoid lock contention.
	 *                        This is useful if there are many job runners.
	 * @param $params array
	 */
	protected function __construct( array $params ) {
		parent::__construct( $params );
		$this->reduceContention = !empty( $params['reduceContention'] );
	}
	...
	protected function claim( $uuid, $rand, $gte ) {
		...
		$dir  = $gte ? 'ASC' : 'DESC';
		$ineq = $gte ? '>=' : '<=';
		if ( $this->reduceContention && $this->order === 'random' ) {
			// When "reduceContention" is disabled, the UPDATE + ORDER BY below can cause
			// deadlocks, mostly when there are not that many jobs. Removing the ORDER BY
			// solves this but is not replication safe. With "reduceContention", we do
