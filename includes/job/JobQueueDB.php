...
class JobQueueDB extends JobQueue {
	...
	protected function claimRandom( $uuid, $rand, $gte ) {
		...
		$dbw = $this->getMasterDB();
		// Check cache to see if the queue has less that OFFSET items
		$tinyQueue = $wgMemc->get( $this->getSmallnessCacheKey() );

		$row = false; // the row acquired
		$invertedDirection = 0; // whether one job_random direction was already scanned
		...
		do {
			if ( $tinyQueue ) { // queue has < MAX_OFFSET rows
				...
				if ( !$row ) {
					...
					$wgMemc->set( $this->getSmallnessCacheKey(), 1, 30 );
	 ...
	 * @return string
	 */
	private function getSmallnessCacheKey() {
		list( $db, $prefix ) = wfSplitWikiID( $this->wiki );
		return wfForeignMemcKey( $db, $prefix, 'jobqueue', $this->type, 'issmall' );
	}
