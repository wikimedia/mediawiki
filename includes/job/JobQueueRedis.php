 ...
 *
 * There are five main redis key names used to track jobs:
 *   - l-unclaimed      : A list of job IDs used for push/pop
 *   - h-deduplicated   : A hash of (SHA1 => job ID) used for de-duplication
 *   - h-claimed        : A hash of (job ID => UNIX timestamp) used for job retries
 *   - h-attempts       : A hash of (job ID => attempt count) used for job retries
...
class JobQueueRedis extends JobQueue {
	...
	protected function doBatchPush( array $jobs, $flags ) {
		...
		// Convert the maps into serialized blobs
		...
		try {
			...
			if ( count( $dedupUids ) ) {
				...
				foreach ( $duplicateMap as $uid => $isDuplicate ) {
					if ( $isDuplicate ) {
						...
						$newHashes[$dedupUids[$uid]] = $uid; // blob will be inserted
					}
				}
			}
			...
			if ( count( $blobs ) ) {
				...
				if ( count( $newHashes ) ) {
					$conn->hMSet( $this->getQueueKey( 'h-deduplicated' ), $newHashes );
				}
	...
	protected function popAndDeleteBlob( RedisConnRef $conn ) {
		return $conn->eval(
			...
			// Get the job data
			"local item = redis.call('hGet',KEYS[5],id)\n" .
			// Remove the job data
			"redis.call('hDel',KEYS[5],id)\n" .
			// Allow new duplicates of this job
			"redis.call('hDel',KEYS[2],id)\n" .
			// Remove it from any "claimed"/"attempts" hash for sanity
			...
			array(
				...
				$this->getQueueKey( 'h-deduplicated' ), # KEYS[2]
				$this->getQueueKey( 'h-claimed' ), # KEYS[3]
				$this->getQueueKey( 'h-attempts' ), # KEYS[4]
	...
	protected function popAndAcquireBlob( RedisConnRef $conn ) {
		return $conn->eval(
			...
			// Allow new duplicates of this job
			"redis.call('hDel',KEYS[2],id)\n" .
			// Place the item in the "claimed" hash and return it
	...
	protected function doAck( Job $job ) {
		if ( $this->claimTTL > 0 ) {
			...
			try {
				...
				$res = $conn->eval(
					// Remove the job from any "claimed"/"attempts" hash
	...
	public function getJobFromUidInternal( $uid, RedisConnRef $conn ) {
		try {
			$key = $this->getQueueKey( 'h-data' );
			$item = unserialize( $conn->hGet( $key, $uid ) );
			if ( !is_array( $item ) ) { // wtf?
				$conn->hDel( $key, $uid );
	...
	public function recycleAndDeleteStaleJobs() {
		...
		try {
			...
			list( $released, $pruned ) = $conn->eval(
				# Get IDs of all claimed jobs
				"local released = 0, pruned = 0\n" .
				"local idsClaimTimes = redis.call('hGetAll',KEYS[1])\n" .
				"for id,ts in ipairs(idsClaimTimes) do\n" .
				"	local attempts = redis.call('hGet',KEYS[2],id)\n" .
				"	if attempts >= ARGV[3] and ts <= ARGV[2] then\n" .
						// Stale and out of retries: remove any traces of the job
				"		redis.call('hDel',KEYS[1],id)\n" .
				"		redis.call('hDel',KEYS[2],id)\n" .
				"		pruned = pruned + 1\n" .
				"	elseif attempts < ARGV[3] and ts <= ARGV[1] then\n" .
						// Claim expired and retries left: push job back into the queue
				...
				array(
					$this->getQueueKey( 'h-claimed' ), # KEYS[1]
					$this->getQueueKey( 'h-attempts' ), # KEYS[2]
					$this->getQueueKey( 'h-deduplicated' ), # KEYS[3]
					$this->getQueueKey( 'l-unclaimed' ), # KEYS[4]
					$this->getQueueKey( 'h-data' ), # KEYS[5]
