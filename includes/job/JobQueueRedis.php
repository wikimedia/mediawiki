...
class JobQueueRedis extends JobQueue {
	...
	protected function doPop() {
		...
		try {
			do {
				// Periodically delete and/or recycle jobs that were claimed for too long
				if ( mt_rand( 0, 99 ) == 0 ) {
				...
				if ( $item === false || ( $this->claimTTL && !$ok ) ) {
					wfDebug( "Could not find or delete job $uid; probably got a duplicate." );
		...
		// Delete and/or recycle jobs that were claimed for too long
		if ( !$job ) { // useful for small queues
			$this->doInternalMaintenance();
		}
	...
	protected function doInternalMaintenance() {
		return ( $this->claimTTL > 0 ) ? $this->recycleStaleJobs() : $this->cleanupClaimedJobs();
	}
	...
	protected function recycleStaleJobs() {
		...
		try {
			...
			list( $claimedUids, $claimTimes ) = $conn->exec();
			...
			if ( count( $itemsAdd ) ) { // copy jobs to unclaimed queue
				...
				$conn->mset( $this->prefixKeysWithQueueKey( 'data', $itemsAdd ) );
				call_user_func_array(
					array( $conn, 'lPush' ),
			...
			if ( in_array( false, $res, true ) ) {
				...
				wfIncrStats( 'job-recycle', count( $itemsAdd ) );
			}
	...
	protected function cleanupClaimedJobs() {
		...
		try {
			...
			if ( count( $uids ) ) {
				...
				$conn->multi( Redis::MULTI ); // begin (atomic trx)
				$conn->delete( array_keys(
					$this->prefixKeysWithQueueKey( 'data', array_flip( $uids ) ) ) );
