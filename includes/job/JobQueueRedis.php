...
class JobQueueRedis extends JobQueue {
	...
	const REDIST_PERIOD = 86400; // integer; seconds between redistribution checks
		...
		try {
			...
			do {
				if ( $this->claimTTL > 0 ) {
					...
					if ( $item ) {
						// Set the claim timestamp/attempts metadata. If this step fails,
	...
	protected function doAck( Job $job ) {
		if ( $this->claimTTL > 0 ) {
			...
			try {
				...
				$conn->lRem( $this->getQueueKey( 'claimed' ), $item, -1 );
				// Delete the claim timestamp/attempts metadata
	...
	protected function doRedistribute() {
		...
		try {
			...
			foreach ( $this->servers as $server ) { // for each server
				...
				foreach ( array( 'claimed', 'unclaimed', 'claim-times' ) as $type ) {
