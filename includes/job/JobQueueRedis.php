...
class JobQueueRedis extends JobQueue {
	...
	protected function doPop() {
		...
		if ( mt_rand( 0, 99 ) == 0 ) {
			$this->releaseReadyDelayedJobs(); // push ready delayed jobs into the queue
	...
	public function releaseReadyDelayedJobs() {
		...
		try {
			...
			$uids = $conn->zRangeByScore(
				$this->getQueueKey( 'z-delayed' ), 0, time(), array( 'limit' => 500 )
			);
			$count = count( $uids );
			if ( $count ) {
				...
				$conn->zRem( $this->getQueueKey( 'z-delayed' ), $uids );
