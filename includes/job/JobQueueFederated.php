...
class JobQueueFederated extends JobQueue {
	...
	protected function tryJobInsertions( array $jobs, array &$partitionsTry, $flags ) {
		...
		$jobsLeft = array();
		$partitions = array_keys( $partitionsTry );
		...
		// This weighs all queue partitions equally regardless of the given weights.
		$uJobsByPartition = array(); // (partition name => job list)
		foreach ( $jobs as $key => $job ) {
			if ( $job->ignoreDuplicates() ) {
				$sha1 = sha1( serialize( $job->getDeduplicationInfo() ) );
				ArrayUtils::consistentHashSort( $partitions, $sha1 );
				$uJobsByPartition[$partitions[0]][] = $job;
				unset( $jobs[$key] );
			}
		}
	...
	/**
	 * @return bool The queue is actually split up accross multiple queue partitions
	 */
