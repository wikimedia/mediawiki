...
class JobQueueFederated extends JobQueue {
	...
	/** @var Array List of partition names) */
	...
	protected function __construct( array $params ) {
		...
		if ( $this->order === 'fifo' ) {
			throw new MWException( 'Federated queues do not support FIFO ordering.' );
		}
		...
		if ( isset( $params['partitionsPopOnly'] ) ) {
			$this->partitionsPopOnly = $params['partitionsPopOnly'];
		}
		$this->baseConfig = $params;
		unset( $this->baseConfig['class'] );
		foreach ( array(
			'sectionsByWiki', 'partitionsBySection', 'configByPartition', 'partitionsPopOnly'
		) as $o ) {
	...
	protected function doBatchPush( array $jobs, $flags ) {
		...
		$partitionsTry = array_diff_key(
			...
			array_flip( $this->partitionsPopOnly )
		); // (partition => weight)
