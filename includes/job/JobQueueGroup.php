...
class JobQueueGroup {
	...
	public function push( $jobs ) {
		...
		if ( $this->cache->has( 'queues-ready', 'list', 15 ) ) {
			$this->cache->set( 'queues-ready', 'list', array_unique( array_merge(
				$this->cache->get( 'queues-ready', 'list' ), array_keys( $jobsByType )
			) ) );
		}
	...
	public function pop( $queueType = self::TYPE_DEFAULT, $flags = 0 ) {
		...
		foreach ( $types as $type ) { // for each queue...
			...
			if ( $job ) { // found
				return $job;
			} elseif ( $flags & self::USE_CACHE ) { // not found
				$this->cache->set( 'queues-ready', 'list', array_diff(
					$this->cache->get( 'queues-ready', 'list' ), array( $type )
				) );
			}
		}
