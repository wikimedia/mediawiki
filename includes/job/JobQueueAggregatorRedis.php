...
class JobQueueAggregatorRedis extends JobQueueAggregator {
	...
	protected function doNotifyQueueEmpty( $wiki, $type ) {
		...
		try {
			$conn->hDel( $this->getReadyQueueCacheKey(), "{$type}>>{$wiki}" );
			return true;
	...
	protected function doNotifyQueueNonEmpty( $wiki, $type ) {
		...
		try {
			$conn->hSet( $this->getReadyQueueCacheKey(), "{$type}>>{$wiki}", time() );
			return true;
		} catch ( RedisException $e ) {
			$this->handleException( $conn, $e );
			return false;
	...
	protected function doGetAllReadyWikiQueues() {
		...
		try {
			...
			$conn->exists( $this->getReadyQueueCacheKey() );
			$conn->hGetAll( $this->getReadyQueueCacheKey() );
			list( $exists, $map ) = $conn->exec();

			if ( $exists ) { // cache hit
				$pendingDBs = array(); // (type => list of wikis)
				foreach ( $map as $key => $time ) {
					list( $type, $wiki ) = explode( '>>', $key, 2 );
				...
				$conn->hMSet( $this->getReadyQueueCacheKey(), $map );
			}
