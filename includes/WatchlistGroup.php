...
class WatchlistGroup {
	...
	public static function getGroupFromUserTitle( $title, $user ) {
		...
		$res = $dbr->selectRow( 'watchlist', array( 'wl_group' ),
			 array( 'wl_user' => $user, 'wl_namespace' => $title->getNamespace(), 'wl_title' => $title->getDBkey() ), __METHOD__
		);
	 ...
	 * @return bool
	 */
	public static function renameGroup( $group, $name, $user ) {
		$dbw = wfGetDB( DB_MASTER );
	 ...
	 * @return bool
	 */
	public static function changePerm( $group, $perm, $user ) {
		$dbw = wfGetDB( DB_MASTER );
