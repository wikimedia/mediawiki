	...
	public function showPatrolFooter() {
		...
		if ( !$rcid ) {
			if ( $wgUseRCPatrol ) {
				...
				global $wgRCMaxAge;
				if ( wfTimestamp( TS_UNIX, $this->mRevision->getTimestamp() ) < time() - 21600 - $wgRCMaxAge ) {
					// The revision is more than 6 hours older than the Max RC age
					// no need to torture the DB any further (6h because the RC isn't cleaned out regularly)
					wfProfileOut( __METHOD__ );
					return;
				}
		...
		if (
			...
			// Don't cache in case can patrol as this could change
			if( $wgUseRCPatrol ) {
