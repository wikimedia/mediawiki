...
class Article extends Page {
	...
	public function getOldIDFromRequest() {
		...
		$oldid = $this->getContext()->getRequest()->getIntOrNull( 'oldid' );
		...
		if ( $this->getContext()->getRequest()->getVal( 'direction' ) == 'next' ) {
		...
		} elseif ( $this->getContext()->getRequest()->getVal( 'direction' ) == 'prev' ) {
	...
	public function view() {
		...
		$permErrors = $this->getTitle()->getUserPermissionsErrors( 'read', $this->getContext()->getUser() );
		if ( count( $permErrors ) ) {
		...
		if ( $this->mRedirectUrl ) {
			$this->getContext()->getOutput()->redirect( $this->mRedirectUrl );
		...
		$this->getContext()->getOutput()->setPageTitle( $this->getTitle()->getPrefixedText() );

		$this->getContext()->getOutput()->setArticleFlag( true );
		...
		if ( $this->getContext()->getOutput()->isPrintable() ) {
		...
		if ( !$wgDebugToolbar && $oldid === 0 && $this->mPage->checkTouched() ) {
			if ( $wgUseETag ) {
				$this->getContext()->getOutput()->setETag( $parserCache->getETag( $this, $parserOptions ) );
			}

			# Is it client cached?
			if ( $this->getContext()->getOutput()->checkLastModified( $this->mPage->getTouched() ) ) {
		...
		if ( $this->getContext()->getUser()->getStubThreshold() ) {
		...
		while ( !$outputDone && ++$pass ) {
			switch( $pass ) {
				...
				case 2:
					...
					if ( $useParserCache ) {
						...
						if ( $this->mParserOutput !== false ) {
							...
							$this->getContext()->getOutput()->addParserOutput( $this->mParserOutput );
							# Ensure that UI elements requiring revision ID have
							# the correct version information.
							$this->getContext()->getOutput()->setRevisionId( $this->mPage->getLatest() );
							# Preload timestamp to avoid a DB hit
							$cachedTimestamp = $this->mParserOutput->getTimestamp();
							if ( $cachedTimestamp !== null ) {
								$this->getContext()->getOutput()->setRevisionTimestamp( $cachedTimestamp );
				...
				case 3:
					...
					# Ensure that UI elements requiring revision ID have
					# the correct version information.
					$this->getContext()->getOutput()->setRevisionId( $this->getRevIdFetched() );
					# Preload timestamp to avoid a DB hit
					$this->getContext()->getOutput()->setRevisionTimestamp( $this->getTimestamp() );
					...
					if ( $this->getTitle()->isCssOrJsPage() || $this->getTitle()->isCssJsSubpage() ) {
						...
						$outputDone = true;
					} elseif( !wfRunHooks( 'ArticleViewCustom', array( $this->mContent, $this->getTitle(), $this->getContext()->getOutput() ) ) ) {
						...
						if ( $rt ) {
							...
							$this->getContext()->getOutput()->addHTML( $this->viewRedirect( $rt ) );
							# Parse just to get categories, displaytitle, etc.
							$this->mParserOutput = $wgParser->parse( $text, $this->getTitle(), $parserOptions );
							$this->getContext()->getOutput()->addParserOutputNoText( $this->mParserOutput );
							$outputDone = true;
						}
					}
					break;
				case 4:
					...
					if ( !$poolArticleView->execute() ) {
						...
						if ( $error ) {
							$this->getContext()->getOutput()->clearHTML(); // for release() errors
							$this->getContext()->getOutput()->enableClientCache( false );
							$this->getContext()->getOutput()->setRobotPolicy( 'noindex,nofollow' );

							$errortext = $error->getWikiText( false, 'view-pool-error' );
							$this->getContext()->getOutput()->addWikiText( '<div class="errorbox">' . $errortext . '</div>' );
						}
					...
					if ( $poolArticleView->getIsDirty() ) {
						$this->getContext()->getOutput()->setSquidMaxage( 0 );
						$this->getContext()->getOutput()->addHTML( "<!-- parser cache is expired, sending anyway due to pool overload-->\n" );
					}
		...
		if ( $this->getTitle()->isMainPage() ) {
			...
			if ( !$msg->isDisabled() ) {
				$this->getContext()->getOutput()->setHTMLTitle( $msg->title( $this->getTitle() )->text() );
			}
		}

		# Check for any __NOINDEX__ tags on the page using $pOutput
		$policy = $this->getRobotPolicy( 'view', $pOutput );
		$this->getContext()->getOutput()->setIndexPolicy( $policy['index'] );
		$this->getContext()->getOutput()->setFollowPolicy( $policy['follow'] );
	...
	public function showDiffPage() {
		...
		$rcid = $this->getContext()->getRequest()->getVal( 'rcid' );
		$diffOnly = $this->getContext()->getRequest()->getBool( 'diffonly', $this->getContext()->getUser()->getOption( 'diffonly' ) );
	...
	protected function showCssOrJsPage() {
		...
		$this->getContext()->getOutput()->wrapWikiMsg( "<div id='mw-clearyourcache' lang='$lang' dir='$dir' class='mw-content-$dir'>\n$1\n</div>",
			'clearyourcache' );

		// Give hooks a chance to customise the output
		if ( wfRunHooks( 'ShowRawCssJs', array( $this->mContent, $this->getTitle(), $this->getContext()->getOutput() ) ) ) {
			// Wrap the whole lot in a <pre> and don't parse
			$m = array();
			preg_match( '!\.(css|js)$!u', $this->getTitle()->getText(), $m );
			$this->getContext()->getOutput()->addHTML( "<pre class=\"mw-code mw-{$m[1]}\" dir=\"ltr\">\n" );
			$this->getContext()->getOutput()->addHTML( htmlspecialchars( $this->mContent ) );
	...
	public function showRedirectedFromHeader() {
		...
		if ( isset( $this->mRedirectedFrom ) ) {
			...
			if ( wfRunHooks( 'ArticleViewRedirect', array( &$this ) ) ) {
				...
				$this->getContext()->getOutput()->addSubtitle( wfMessage( 'redirectedfrom' )->rawParams( $redir ) );

				// Set the fragment if one was specified in the redirect
				if ( strval( $this->getTitle()->getFragment() ) != '' ) {
					$fragment = Xml::escapeJsString( $this->getTitle()->getFragmentForURL() );
					$this->getContext()->getOutput()->addInlineScript( "redirectToFragment(\"$fragment\");" );
				}

				// Add a <link rel="canonical"> tag
				$this->getContext()->getOutput()->addLink( array( 'rel' => 'canonical',
					'href' => $this->getTitle()->getLocalURL() )
				);

				// Tell the output object that the user arrived at this article through a redirect
				$this->getContext()->getOutput()->setRedirectedFrom( $this->mRedirectedFrom );
	...
	public function showPatrolFooter() {
		...
		$token = $this->getContext()->getRequest()->getUser()->getEditToken( $rcid );
		$this->getContext()->getOutput()->preventClickjacking();
	...
	public function showMissingArticle() {
		...
		if ( $this->getTitle()->getNamespace() == NS_USER || $this->getTitle()->getNamespace() == NS_USER_TALK ) {
			...
			if ( !($user && $user->isLoggedIn()) && !$ip ) { # User does not exist
				$this->getContext()->getOutput()->wrapWikiMsg( "<div class=\"mw-userpage-userdoesnotexist error\">\n\$1\n</div>",
		...
		LogEventsList::showLogExtract( $this->getContext()->getOutput(), array( 'delete', 'move' ), $this->getTitle()->getPrefixedText(), '',
			array(  'lim' => 10,
		...
		$this->getContext()->getOutput()->addWikiText( $text );
	}
	...
	public function showDeletedRevisionHeader() {
		...
		if ( !$this->mRevision->userCan( Revision::DELETED_TEXT ) ) {
			$this->getContext()->getOutput()->wrapWikiMsg( "<div class='mw-warning plainlinks'>\n$1\n</div>\n",
	...
	public function setOldSubtitle( $oldid = 0 ) {
		...
		$td = $this->getContext()->getLanguage()->timeanddate( $timestamp, true );
		$tddate = $this->getContext()->getLanguage()->date( $timestamp, true );
		$tdtime = $this->getContext()->getLanguage()->time( $timestamp, true );
		...
		$this->getContext()->getOutput()->addSubtitle( "<div id=\"mw-{$infomsg}\">" . wfMessage( $infomsg,
		...
		$this->getContext()->getOutput()->addSubtitle( "<div id=\"mw-revision-nav\">" . $cdel .
	...
	public function delete() {
		...
		if ( $latest === false ) {
			$this->getContext()->getOutput()->setPageTitle( wfMessage( 'cannotdelete-title', $title->getPrefixedText() ) );
			$this->getContext()->getOutput()->wrapWikiMsg( "<div class=\"error mw-error-cannotdelete\">\n$1\n</div>",
					array( 'cannotdelete', wfEscapeWikiText( $title->getPrefixedText() ) )
				);
			$this->getContext()->getOutput()->addHTML( Xml::element( 'h2', null, LogPage::logName( 'delete' ) ) );
			LogEventsList::showLogExtract(
				$this->getContext()->getOutput(),
				'delete',
		...
		$deleteReasonList = $this->getContext()->getRequest()->getText( 'wpDeleteReasonList', 'other' );
		$deleteReason = $this->getContext()->getRequest()->getText( 'wpReason' );
		...
		if ( $this->getContext()->getRequest()->wasPosted() && $user->matchEditToken( $this->getContext()->getRequest()->getVal( 'wpEditToken' ),
			array( 'delete', $this->getTitle()->getPrefixedText() ) ) )
		{
			# Flag to hide all contents of the archived revisions
			$suppress = $this->getContext()->getRequest()->getVal( 'wpSuppress' ) && $user->isAllowed( 'suppressrevision' );

			$this->doDelete( $reason, $suppress );

			if ( $this->getContext()->getRequest()->getCheck( 'wpWatch' ) && $user->isLoggedIn() ) {
				$this->doWatch();
	...
	public function confirmDelete( $reason ) {
		...
		$this->getContext()->getOutput()->setPageTitle( wfMessage( 'delete-confirm', $this->getTitle()->getPrefixedText() ) );
		$this->getContext()->getOutput()->addBacklinkSubtitle( $this->getTitle() );
		$this->getContext()->getOutput()->setRobotPolicy( 'noindex,nofollow' );
		$this->getContext()->getOutput()->addWikiMsg( 'confirmdeletetext' );

		wfRunHooks( 'ArticleConfirmDelete', array( $this, $this->getContext()->getOutput(), &$reason ) );
		...
		$this->getContext()->getOutput()->addHTML( $form );
		$this->getContext()->getOutput()->addHTML( Xml::element( 'h2', null, LogPage::logName( 'delete' ) ) );
		LogEventsList::showLogExtract( $this->getContext()->getOutput(), 'delete',
			$this->getTitle()->getPrefixedText()
		);
	}
	...
	public function doDelete( $reason, $suppress = false ) {
		...
		if ( $this->mPage->doDeleteArticle( $reason, $suppress, 0, true, $error ) ) {
			...
			$this->getContext()->getOutput()->setPageTitle( wfMessage( 'actioncomplete' ) );
			$this->getContext()->getOutput()->setRobotPolicy( 'noindex,nofollow' );

			$loglink = '[[Special:Log/delete|' . wfMsgNoTrans( 'deletionlog' ) . ']]';

			$this->getContext()->getOutput()->addWikiMsg( 'deletedtext', wfEscapeWikiText( $deleted ), $loglink );
			$this->getContext()->getOutput()->returnToMain( false );
		} else {
			$this->getContext()->getOutput()->setPageTitle( wfMessage( 'cannotdelete-title', $this->getTitle()->getPrefixedText() ) );
			if ( $error == '' ) {
				$this->getContext()->getOutput()->wrapWikiMsg( "<div class=\"error mw-error-cannotdelete\">\n$1\n</div>",
				...
				$this->getContext()->getOutput()->addHTML( $error );
			}
		}
	}
