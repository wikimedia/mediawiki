...
class RootSpecHandler extends SimpleHandler {
	...
	private function getPathsSpec(): array {
		...
		foreach ( $this->getRouter()->getAllRoutes() as $route ) {
			$path = $route['path'];
			$op = strtolower( $route['method'] ?? 'get' );
			$specs[ $path ][ $op ] = $this->getRouteSpec( $route );
		}

		return $specs;
	}

	private function getRouteSpec( array $handlerObjectSpec ): array {
		$handler = $this->getRouter()->instantiateHandlerObject( $handlerObjectSpec );

		$operationSpec = $handler->getOpenApiSpec();
