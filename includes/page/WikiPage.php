		...
		if ( $content ) {
			...
			if ( $wgRCWatchCategoryMembership
				...
				if ( $options['created'] ) {
					...
					$priorRev = $options['oldrevision'] ?: $revision->getPrevious();
				}
				// Note: doEditContent() calls RecentChange::notifyNew(), and this update relies
				// on DeferredUpdates being FIFO so it can see the recent change entry (also done
				// via deferred updates) and carry over any bot/deletion/IP flags, ect.
				DeferredUpdates::addUpdate( new CategoryMembershipUpdate(
					$this,
					$revision,
					$editInfo->output,
					$priorRev
