...
class WikiPage implements Page, IDBAccessObject {
	...
	/**
	 * @var PageMetaDataUpdater|null
	 */
	private $metaDataUpdater = null;
	...
	protected function clearCacheFields() {
		...
		$this->mLatest = false;
		// T59026: do not clear $this->updater since PageUpdater::prepareTextForEdit() already
		// checks the requested rev ID and content against the cached one for equality. For most
	...
	public function isCountable( $editInfo = false ) {
		...
		// FIXME: move to PageMetaDataUpdater
	 ...
	 */
	private function newMetaDataUpdater( User $user ) {
		...
		$metaDataUpdater->setRcWatchCategoryMembership( $wgRCWatchCategoryMembership );
		$metaDataUpdater->setArticleCountMethod( $wgArticleCountMethod );

		return $metaDataUpdater;
	}

	/**
	 * Returns this page's current PageMetaDataUpdater, if any.
	 * WikiPage has a current PageMetaDataUpdater while executing doEditContent()
	 * or doEditUpdates(). Hook handlers triggered while these methods are being executed my
	 * use this method to gain access to the current MetaDataUpdater.
	 *
	 * @return PageMetaDataUpdater|null
	 */
	public function getCurrentMetaDataUpdater() {
	 ...
	 * @return PageUpdater
	 */
	public function getPageUpdater( User $user ) {
		global $wgAjaxEditStash, $wgUseAutomaticEditSummaries, $wgUseNPPatrol, $wgUseRCPatrol;

		$pageUpdater = new PageUpdater(
			$this, // NOTE: eventually, PageUpdater should not know about WikiPage
			$this->newMetaDataUpdater( $user ),
		...
		$pageUpdater->setUseNPPatrol( $wgUseNPPatrol );
		$pageUpdater->setUseRCPatrol( $wgUseRCPatrol );
	 ...
	 *
	 * @deprecated since 1.31, use PageUpdater::commitEdit instead.
	 *
	...
	public function doEditContent(
		...
		// We want to re-use the updater, so we don't have to re-calculate the PreparedEdit info.
		// But we can only do that during an edit, if the updater hasn't been committed yet.
		// The idea is that hook handlers triggered during edit should all be using the updater of
		// the edit in progress when calling methods on WikiPage.
		if ( !$pageUpdater || $pageUpdater->wasCommitted() ) { !!!!
			// FIXME
		}
		...
		// consume the updater, don't re-use it!
		$pageUpdater = null;
	 ...
	 *
	 * @deprecated since 1.31, use getMetaDataUpdater instead.
	 *
	 * @param Content $content
	 * @param Revision|RevisionRecord|int|null $revision Revision object. For backwards compatibility, a
	...
	public function prepareContentForEdit(
		...
		$udpater = $this->getMetaDataUpdater( $user, $revision, $slots );

		$udpater->prepareEdit( $slots, [], $useCache ); // FIXME: if not already!

		if ( $revision ) {
			$udpater->prepareUpdate( $revision );  // FIXME: if not already!
		}

		return $udpater->getPreparedEdit();
	}
	 ...
	 * @param RevisionSlots|null $forContent
	 *
	 * @return PageMetaDataUpdater|null
	 */
	public function getMetaDataUpdater(
		...
		if ( $this->metaDataUpdater && $forContent
			&& !$this->metaDataUpdater->canUseForContent( $user, $forContent )
		) {
			$this->metaDataUpdater = null;
		}

		if ( $this->metaDataUpdater && !$forRevision
			&& !$this->metaDataUpdater->canUseForRevision( $user, $forRevision )
		) {
			$this->metaDataUpdater = null;
		}

		if ( !$this->metaDataUpdater ) {
			$this->metaDataUpdater = $this->newMetaDataUpdater( $user );
		}
	 ...
	 *
	 * @deprecated since 1.31, use PageUpdater::doEditUpdates instead.
	 *
	 * @param Revision $revision
	...
	public function doEditUpdates( Revision $revision, User $user, array $options = [] ) {
		...
		$updater = $this->getMetaDataUpdater( $user, $revision );

		$updater->prepareUpdate( $revision, $options ); // FIXME: if not redundat!
