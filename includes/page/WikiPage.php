...
class WikiPage implements Page, IDBAccessObject {
	...
	public function isCountable( $editInfo = false ) {
		...
		// XXX: Keep in sync with PageMetaDataUpdater::isCountable.
	...
	public function getMetaDataUpdater(
		...
		// TODO: Use an LRU cache instead of trying to re-use a single instance. However,
		// we'd need two LRU lists, one per $forContent and one per $forRevision.
		...
		if ( $this->metaDataUpdater
			&& !$this->metaDataUpdater->canUseFor( $user, $forRevision, $forContent, $forStopSlots )
		) {
			$this->metaDataUpdater = null;
		}
	...
	public function doEditContent(
		...
		// prepareContentForEdit will generally return the PageMetaDataUpdater from
		// that is used by the PageUpdater. Hoever, there is no guarantee for this.
	...
	public function prepareContentForEdit(
		...
		if ( !$revision ) {
			$udpater->prepareEdit( $slots, [], $useCache );
		}

		if ( $revision ) {
			$udpater->prepareUpdate( $revision );
		}
	...
	public static function onArticleEdit(
		...
		if ( $slotsChanged === null || in_array( 'main',  $slotsChanged ) ) {
			...
			// Only for the main slot, because only the main slot defines redirects
			DeferredUpdates::addUpdate(
