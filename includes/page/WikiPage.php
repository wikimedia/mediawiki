...
class WikiPage implements Page, IDBAccessObject {
	...
	public function isCountable( $editInfo = false ) {
		...
		// NOTE: Keep in sync with PageMetaDataUpdater::isCountable.
	 ...
	 * This method attempts to re-use the same PageMetaDataUpdater instance for subsequent calls.
	 * The parameters passed to this method are used to ensure that the PageMetaDataUpdater
	 ...
	 * PageMetaDataUpdater::canUseFor(), and creating a new instance of the parameters do not
	 * match the existign one.
	 *
	 * If neither $forRevision nor $forContent is given, a new PageMetaDataUpdater is always
	 * created, replacing any PageMetaDataUpdater currently cached.
	 *
	 ...
	 * @param string[] $forStopSlots Any slots removed by the edit.
	 *
	...
	public function getMetaDataUpdater(
		...
		// XXX: canUseFor currently checks $forContent against the pre-PST content.
		// Double-check that this is actually what we want.
		...
		if ( !$this->metaDataUpdater ) {
			$this->metaDataUpdater = $this->newMetaDataUpdater();
		}
	 ...
	 *
	 * @deprecated since 1.31, use PageUpdater::doEdit instead.
	 *
	...
	public function doEditContent(
		...
		// NOTE: while doEditContent() executes, callbacks to getMetaDataUpdater and
		// prepareContentForEdit will generally use the PageMetaDataUpdater that is also
	...
	public function makeParserOptions( $context ) {
		$renderer = $this->getRevisionRenderer();

		if ( $context === 'canonical' ) { // canonical settings
			return $renderer->makeCanonicalParserOptions( $this->getTitle() );
		} elseif ( $context instanceof IContextSource ) {
			return $renderer->makeUserParserOptions(
				$this->getTitle(),
				$context->getUser(),
				$context->getLanguage()
			);
			...
			throw new InvalidArgumentException( "Bad context for parser options: $context" );
		}
	}
	...
	public function prepareContentForEdit(
		...
		if ( !$updater->isUpdatePrepared() ) {
			$updater->prepareEdit( $user, $slots, [], $useCache );
	...
	public static function onArticleCreate( Title $title ) {
		// TODO: move this into PageMetaDataUpdater, make non-static.
