...
class PageArchive {
	...
	private function getRevisionStore() {
		// TODO: PageArchive should be converted into an ArchiveStore service!
	 ...
	 *
	 * @deprecated since 1.31, use getArchivedRevision() instead.
	 *
	 * @param string $timestamp
	 * @return Revision|null
	 */
	public function getRevision( $timestamp ) {
		$rec = $this->getRevisionByConditions( [ 'ar_timestamp' => $timestamp ] );
	...
	public function getPreviousRevision( $timestamp ) {
		...
		return $rec ? new Revision( $rec ) : $rec;
	}

	/**
	 * Get the text from an archive row containing ar_text_id.
	 *
	 * @deprecated since 1.31, appears unused. In the MCR schema, ar_text_id no longer exists.
	...
	public function getTextFromRow( $row ) {
		// TODO: use SqlBlobStore::makeAddressFromTextId from I31b3ee5e40185
		$address = 'tt:' . $row->ar_text_id;
	 ...
	 *
	 * @deprecated since 1.31, appears unused. For compatibility with the MCR schema,
	 * calling code should switch to getArchiveRevision().
	 *
	 * @todo remove in 1.32
	 *
	 ...
	 * This says nothing about whether the page currently exists in the page tale or not.
	 *
	 * @return bool
	 */
