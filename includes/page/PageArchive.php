...
class PageArchive {
	...
	public function undelete( $timestamps, $comment = '', $fileVersions = [],
		...
		if ( $restoreText ) {
			$this->revisionStatus = $this->undeleteRevisions( $timestamps, $unsuppress, $comment, $reset );
	 ...
	 */
	private function undeleteRevisions( $timestamps, $unsuppress = false, $comment = '', $reset = false ) {
		if ( wfReadOnly() ) {
