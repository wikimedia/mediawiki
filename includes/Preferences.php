...
class Preferences {
	 ...
	 * @return array|null
	 */
	static function loadPreferenceValues( $user, IContextSource $context, &$defaultPreferences ) {
	...
	static function skinPreferences( $user, IContextSource $context, &$defaultPreferences ) {
		...
		if ( $config->get( 'AllowUserCss' ) || $config->get( 'AllowUserJs' ) ) {
			$linkTools = array();
			$userName = $user->getName();
	...
	static function rcPreferences( $user, IContextSource $context, &$defaultPreferences ) {
		...
		$defaultPreferences['rcdays'] = array(
			...
			'min' => 1,
			'max' => ceil( $config->get( 'RCMaxAge' ) / ( 3600 * 24 ) ),
			'help' => $context->msg( 'recentchangesdays-max' )->numParams(
				ceil( $config->get( 'RCMaxAge' ) / ( 3600 * 24 ) ) )->text()
		);
	...
	static function generateSkinOptions( $user, IContextSource $context ) {
		...
		foreach ( $validSkinNames as $skinkey => $sn ) {
			...
			if ( $skinkey == $config->get( 'DefaultSkin' ) ) {
	...
	static function getTimezoneOptions( IContextSource $context ) {
		...
		// Check that $localTZoffset is the same as the local time zone offset
