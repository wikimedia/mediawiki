...
class ChangeTags {
	...
	public static function canActivateTag( $tag, User $user = null ) {
		...
		if ( !$changeTag->exists ) {
			return Status::newFatal( 'tags-manage-not-found', $changeTag->name );
		}

		// defined tags cannot be activated
		if ( $changeTag->extensionDefined || $changeTag->userDefined ) {
			return Status::newFatal( 'tags-activate-not-allowed', $changeTag->name );
		}
	...
	public static function canDeactivateTag( $tag, User $user = null ) {
		...
		// only user defined tags applied at least once can be deactivated
		if ( !$changeTag->userDefined || ( $changeTag->hitcount == 0 ) ) {
			return Status::newFatal( 'tags-deactivate-not-allowed', $changeTag->name );
		}
		return Status::newGood();
	}
	 ...
	 *
	 * Includes a call to ChangeTags::canDeleteTag(), so your code doesn't need to
	 * do that.
	 *
	 * @param string $tag
	 * @param string $reason
	...
	public static function canDeleteTag( $tag, User $user = null ) {
		...
		if ( $changeTag->hitcount > self::MAX_DELETE_USES ) {
			return Status::newFatal( 'tags-delete-too-many-uses', $changeTag->name, self::MAX_DELETE_USES );
		}

		if ( $changeTag->extensionDefined && ( !isset( $changeTag->params['canDelete'] ) ||
			!$changeTag->params['canDelete'] ) ) {
