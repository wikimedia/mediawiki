...
class ChangeTags {
	...
	public static function deleteTagEverywhere( $tag ) {
		...
		$dbw = wfGetDB( DB_MASTER );
		$result = $dbw->delete( 'valid_tag', array( 'vt_tag' => $tag ), __METHOD__ );
		if ( $result !== true ) {
		...
		$result = $dbw->delete( 'change_tag', array( 'ct_tag' => $tag ), __METHOD__ );
		if ( $result !== true ) {
			$dbw->rollback( __METHOD__ );
			return false;
		}
	...
	/**
	 * Checks whether the gievn tag is defined (listed in the 'valid_tag' table).
	 *
	 * Tries memcached first.
	 *
	 * @param string $tag
	...
	public static function isTagDefined( $tag ) {
		...
		// Some DB stuff
		$dbr = wfGetDB( DB_SLAVE );
		$res = $dbr->select( 'valid_tag', 'vt_tag', array( 'vt_tag' => $tag ), __METHOD__ );
		if ( $res ) {
			return true;
		}
	...
	/**
	 * Invalidates the short-term cache of defined tags used by isTagDefined.
	 */
