...
class ChangeTags {
	 ...
	 * @param User $user Who to give credit for the action
	 * @return Status If successful, the value of this Status object can either be
	 * true, if no actions were performed, or an object (stdClass) with the
	 * following fields:
	 *  - logId: the ID of the added log entry
		...
		/*switch ( $logAction ) {
			case 'add':
				$logParams = array(
					'4::revid' => $rev_id,
					'5::logid' => $log_id,
					'6:list:tags' => $tagsAdded,
					'7:number:tagCount' => count( $tagsAdded ),
				);
				break;
			case 'remove':
				$logParams = array(
					'4::revid' => $rev_id,
					'5::logid' => $log_id,
					'6:list:tags' => $tagsRemoved,
					'7:number:tagCount' => count( $tagsRemoved ),
				);
				break;
			case 'update':*/
				$logParams = array(
