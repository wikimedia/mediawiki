...
class ChangeTags {
	...
	public static function getChangeTagObject( $tag, $tagStats, $storedTags,
		...
		// an inactive tag can be activated provided it is not extension or core defined
		$ctObj->canActivate = !$ctObj->isActive && !$ctObj->extensionDefined &&
			!$ctObj->coreDefined;
		# no permission check, make it separately
		# no existence check, make it first if not from SpecialTags

		// an active tag can be deactivated provided it is stored in valid_tag
	 ...
	 * You need to pass the tag names as keys and an array of params as values.
	 * All params are optional and include:
	 * (bool) 'active' whether the tag is in active use (false by default)
	 * (bool) 'problem' whether the tag is meant to indicate a problem (false by default)
	 * (string) 'source' name of the extension or more specific project issuing the tag
	 * (array) 'msgParams' array of params for the tag description specific to the source
	 * (bool) 'canDelete' whether the tag can be deleted by admins (false by default)
	 *
	 * Tries memcached first.
	 *
