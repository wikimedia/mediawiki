...
class ChangeTags {
	 ...
	 *
	 * @deprecated since 1.25 Use ChangeTags::updateTags instead
	 */
	public static function addTags( $tags, $rc_id = null, $rev_id = null,
		$log_id = null, $params = null
	) {
		wfDeprecated( __METHOD__, '1.25' );
		return self::updateTags( $tags, null, $rc_id, $rev_id, $log_id, $params );
	}

	/**
	 * Add and remove tags to/from a change given its rc_id, rev_id and/or log_id,
	 * without verifying that the tags exist or are valid.
	 *
	...
	public static function updateTags( $tagsToAdd, $tagsToRemove, $rc_id = null,
		...
		if ( !is_array( $tagsToAdd ) ) {
			$tagsToAdd = array( $tagsToAdd );
		}
		$tagsToAdd = array_filter( $tagsToAdd ); // Make sure we're submitting all tags...
		...
		if ( !self::updateTagSummaryRow( $tagsToAdd, $tagsToRemove, $rc_id, $rev_id, $log_id ) ) {
			// nothing to do
		...
		if ( count( $tagsToRemove ) ) {
			...
			$dbw->delete( 'change_tag', $tagsRows, __METHOD__ );
		}

		self::purgeTagUsageCache();
		return array( $tagsToAdd, $tagsToRemove );
	}
	...
	protected static function updateTagSummaryRow( &$tagsToAdd, &$tagsToRemove,
		...
		// remove tags
		$tagsToRemove = array_values( array_intersect( $tagsToRemove, $prevTags ) );
	...
	public static function canAddTagsAccompanyingChange( array $tags,
		...
		$allowedTags = self::getExplicitlyDefinedTags();
		if ( array_diff( $tags, $allowedTags ) ) {
			return Status::newFatal( 'tags-apply-not-allowed', $tag );
		}
	...
	public static function addTagsAccompanyingChangeWithChecks( array $tags,
		...
		// do it!
		self::updateTags( $tagsToAdd, array(), $rc_id, $rev_id, $log_id, $params );
	...
	public static function canUpdateTags( array $tagsToAdd, array $tagsToRemove,
		...
		if ( $diff ) {
			return Status::newFatal( 'tags-update-add-not-allowed', $diff[0] );
		}
		...
		if ( $diff ) {
			...
			if ( $intersect ) {
				return Status::newFatal( 'tags-update-remove-not-allowed', $intersect[0] );
			}
		}
	 ...
	 * entry as its value (or Boolean true if no action was taken)
	 * @since 1.25
	 */
	public static function updateTagsWithChecks( $tagsToAdd, $tagsToRemove,
		...
		$numTagsToAdd = count( $tagsToAdd );
		$numTagsToRemove = count( $tagsToRemove );
		if ( !( $numTagsToAdd + $numTagsToRemove ) ) {
			// no-op, don't bother
			return Status::newGood( true );
		}
		...
		// do it!
		$updateResult = self::updateTags( $tagsToAdd, $tagsToRemove, $rc_id, $rev_id,
			$log_id, $params );
		...
		// log it
		$logAction = ( $numTagsToRemove ? ( $numTagsToAdd ? 'add-remove' : 'remove' ) : 'add' );
		$logEntry = new ManualLogEntry( 'tag', $logAction );
		$logEntry->setPerformer( $user );
		$logEntry->setTarget( Title::newMainPage() );  // @todo fix this
		$logEntry->setComment( $reason );

		$logEntry->setParameters( array( '4::tag' => $tag ) );
		$logEntry->setRelations( array( 'Tag' => $tag ) );

		$dbw = wfGetDB( DB_MASTER );
		$logId = $logEntry->insert( $dbw );
		$logEntry->publish( $logId );  // @todo Does this really need to go in RC? Seems a bit trivial?

		$statusValue = (object)array();
		$statusValue->logId = $logId;
		$statusValue->addedTags = $updateResult[0];
		$statusValue->removedTags = $updateResult[1];
		return Status::newGood( $statusValue );
	}
