...
class WatchedItemQuery {
	 ...
	 *        'allRevisions' => bool return multiple revisions of the same page if true, only the most
	 *        recent if false (default)
	 *        'rcTypes' => int[] which types of RecentChanges to include (defaults to all types),
	 *        allowed values: RC_EDIT, RC_NEW, RC_LOG, RC_EXTERNAL, RC_CATEGORIZE
	 ...
	 *        'start' => string (format accepted by wfTimestamp) requires 'dir' option, timestamp to
	 *        start enumerating from
	 *        'end' => string (format accepted by wfTimestamp) requires 'dir' option, timestamp to
	 *        end enumerating
	 *        'startFrom' => [ string $rcTimestamp, int $rcId ] requires 'dir' option, return items
	 *        starting from the RecentChange specified by this, $rcTimestamp should be in the format
	 *        accepted by wfTimestamp
	 *        'watchlistOwner' => User user whose watchlist items should be listed if different than
	 *        the one specified with $user param, requires 'watchlistOwnerToken' option
	 ...
	 *        'usedInGenerator' => bool include only RecentChange id field required by the generator
	 *        ('rc_cur_id' or 'rc_this_oldid') if true, or all id fields ('rc_cur_id', 'rc_this_oldid',
	...
	private function getConds( User $user, array $options ) {
		...
		if ( isset( $options['start'] ) ) {
			...
			$conds[] = 'rc_timestamp ' . $after . ' ' . $this->db->addQuotes( $options['start'] );
		}
		if ( isset( $options['end'] ) ) {
			$before = $options['dir'] === self::DIR_OLDER ? '>=' : '<=';
			$conds[] = 'rc_timestamp ' . $before . ' ' . $this->db->addQuotes( $options['end'] );
		}
		...
		if ( array_key_exists( 'onlyByUser', $options ) || array_key_exists( 'notByUser', $options ) ) {
			$conds = array_merge( $conds, $this->getUserRelatedConds( $user, $options ) );
		}

		$deletedPageLogCond = $this->getExtraDeletedPageLogEntryRelatedCond( $user );
		...
		if ( array_key_exists( 'startFrom', $options ) ) {
			...
			$rcTimestamp = $this->db->addQuotes( $this->db->timestamp( $rcTimestamp ) );
			$rcId = (int)$rcId;
			$conds[] = $this->db->makeList(
				[
	...
	private function getUserRelatedConds( User $user, array $options ) {
		...
		if ( array_key_exists( 'onlyByUser', $options ) ) {
			...
			$conds[] = 'rc_user_text != ' . $this->db->addQuotes( $options['notByUser'] );
		}
		...
		if ( $bitmask ) {
			$conds[] = $this->db->bitAnd( 'rc_deleted', $bitmask ) . " != $bitmask";
		}
