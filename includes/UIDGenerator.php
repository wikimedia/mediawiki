...
class UIDGenerator {
	...
	public static function newTimestampedUID88( $base = 10 ) {
		...
		return wfBaseConvert(
			$gen->getTimestampedID88( $gen->getTimestampAndDelay( 'lockFile88' ) ),
			2,
			$base
		);
	}
	...
	protected function getTimestampedID88( array $info ) {
		...
		$id_bin .= str_pad( decbin( $counter % 1024 ), 10, '0', STR_PAD_LEFT );
	...
	public static function newTimestampedUID128( $base = 10 ) {
		...
		// Generate a UID that is higher than the last one from this server
		return wfBaseConvert(
			$gen->getTimestampedID128( $gen->getTimestampAndDelay( 'lockFile128' ) ),
			2,
			$base
		);
	}
	...
	protected function getTimestampedID128( array $info ) {
		...
		# Add a 14 bit clock sequence resulting in 80 bits total
	 ...
	 *
	 * The counter is a 20 bit integer and the clock sequence a 14 bit integer.
	 *
	...
	protected function getTimestampAndDelay( $lockFile ) {
		...
		if ( count( $data ) == 5 ) { // last UID info already initialized
			$clkSeq = (int)$data[0] % 16384;
			$prevTime = array( (int)$data[1], (int)$data[2] );
			$offset = (int)$data[4] % 1048576; // random counter offset
			...
			if ( !$time ) { // too long to delay?
				...
				$counter = (int)$data[3] % 1048576;
				if ( ++$counter > 1048575 ) { // sanity
					flock( $handle, LOCK_UN ); // abort
					throw new MWException( "Counter overflow for timestamp value." );
				}
			}
		} else { // last UID info not initialized
			$clkSeq = mt_rand( 0, 16383 ); // 2^14 = 16384
			$counter = 0;
			$offset = mt_rand( 0, 1048575 ); // 2^20 = 1048576
			$time = self::millitime();
		}
		...
		// If microtime() is synchronized and a clock change was detected,
		// then the clock was set back. Just bump the clock sequence number.
		if ( $clockChanged ) {
			$clkSeq = ( $clkSeq + 1 ) % 16384; // 2^14 = 16384
		...
		return array( $time, ( $counter + $offset ) % 1048575, $clkSeq );
	}
	...
	protected function timeWaitUntil( array $time ) {
		do {
			$ct = self::millitime();
			if ( $ct[0] > $time[0] || ( $ct[0] === $time[0] && $ct[1] >= $time[1] ) ) {
				return $ct; // current timestamp is higher than $time
			}
		} while ( ( 1e3*$time[0] + $time[1] - 1e3*$ct[0] - $ct[1] ) <= 10 );
	...
	protected static function millitime() {
		...
		return array( (int)$sec, (int)( $msec * 1e3 ) );
	}
