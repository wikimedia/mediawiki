...
class UIDGenerator {
	 ...
	 *
	 * Based on http://php.net/manual/en/function.uniqid.php#94959 by Andrew Moore
	 *
	 * @param $format string Use "pretty" to include hyphens and "raw" otherwise
	 * @return string 32 hex characters with standard hyphens if specified
	 * @throws MWException
	 */
	public static function newUUIDv4( $format = 'raw' ) {
		if ( $format === 'raw' ) {
			$sformat = '%04x%04x%04x%04x%04x%04x%04x%04x';
		} elseif ( $format === 'pretty' ) {
			$sformat = '%04x%04x-%04x-%04x-%04x-%04x%04x%04x';
		} else {
