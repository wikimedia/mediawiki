...
class UIDGenerator {
	...
	protected function __construct() {
		...
		} elseif ( is_executable( '/usr/bin/hostid' ) ) { // Linux/BSD/Solaris
			# See http://linux.die.net/man/1/hostid
			$hostId = trim( `/usr/bin/hostid` );
		} elseif ( is_executable( '/sbin/ifconfig' ) ) { // OS X
			# See http://linux.die.net/man/8/ifconfig
			$m = array();
			preg_match( '/\s([0-9a-f]{2}(:[0-9a-f]{2}){5})\s/', `/sbin/ifconfig -a`, $m );
			$hostId = count( $m ) ? str_replace( ':', '', $m[1] ) : '';
		} else {
			$hostId = ''; // fall back to host name
		}
		wfRestoreWarnings();
	 ...
	 * @throws MWException
	 */
	public static function newTimestampedUID88() {
		$generator = self::singleton();
		// Generate a UID that is higher than the last one from this server
		return $generator->getTimestampedID88(
			$generator->getTimestampAndDelay( 'lockFile88', 'microtime' ) );
	}
	 ...
	 * @return string
	 */
	protected function getTimestampedID88( array $time ) {
		# Take the 56 MSBs of "microseconds since epoch" (rolls over in 4354)
		$id_bin = $this->microsecondsSinceEpochBinary( $time );
	 ...
	 * @throws MWException
	 */
	public static function newTimestampedUID128() {
		$generator = self::singleton();
		// Generate a UID that is higher than the last one from this server
		return $generator->getTimestampedID128(
			$generator->getTimestampAndDelay( 'lockFile128', 'microtime' ) );
	}
	 ...
	 * @return string
	 */
	protected function getTimestampedID128( array $time ) {
		$rand = mt_rand( 0, 255 );
	 ...
	 *
	 * @param $format string Use "normal" to include hyphens and "raw" otherwise
	 * @return string 32 hex characters with standard hyphens if specified
	 * @throws MWException
	 */
	public static function newUUIDv4( $format = 'normal' ) {
		if ( $format === 'raw' ) {
			$glue = '';
		} elseif ( $format === 'normal' ) {
			$glue = '-';
		} else {
			throw new MWException( "Invalid format '$format'." );
		}
		$hex = wfRandomString( 31 );
	...
	/**
	 * Acquire a lock file, call a timestamp function, wait until that function
	 * returns a higher value, then release the lock and return the original value.
	 * This is useful for making UIDs sequential on a per-node bases.
	 *
	 * @param string $lockFile Name of a local lock file
	 * @param string $timeFunc Name of a UIDGenerator timestamp function
	 * @return Array
	 * @throws MWException
	 */
	protected function getTimestampAndDelay( $lockFile, $timeFunc ) {
		$timeFunc = array( __CLASS__, $timeFunc );
		...
		$time = call_user_func( $timeFunc ); // timestamp to specified precision
		// Delay so the next process on this server will generate higher UIDs
		do {
			$cTime = call_user_func( $timeFunc );
			if ( self::lessThanTime( $time, $cTime ) ) {
				break; // next caller will get a higher timestamp
			} elseif ( self::lessThanTime( $cTime, $time ) ) { // sanity
				sleep( 1 ); // only handle 1 second of skew
				// Release the UID lock file
				flock( $handle, LOCK_UN );
				fclose( $handle );
				throw new MWException( "System clock was set back." );
			}
		} while ( true );
		// Release the UID lock file
		flock( $handle, LOCK_UN );
	...
	/**
	 * @param $time array Result of UIDGenerator::microtime()
	 * @return string 56 MSBs of "microseconds since epoch" in binary (rolls over in 4354)
	 */
	protected function microsecondsSinceEpochBinary( array $time ) {
		list( $sec, $usec ) = $time;
		...
		if ( PHP_INT_SIZE === 8 ) { // 64 bit integers
			...
			$ts = gmp_add( gmp_mul( "$sec", '1e6' ), "$usec" );
			$id_bin = wfBaseConvert( gmp_mod( $ts, gmp_pow( '2', '56' ) ), 10, 2, 56 );
	 ...
	 *
	 * @param $t1 array Result of UIDGenerator::microtime()
	 * @param $t2 array Result of UIDGenerator::microtime()
	 */
	protected static function lessThanTime( array $t1, array $t2 ) {
