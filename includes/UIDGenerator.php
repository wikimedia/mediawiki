...
class UIDGenerator {
	 ...
	 * @throws MWException
	 */
	public static function newTimestampedUID64() {
		$generator = self::singleton();
		// Generate a UID that is higher than the last one from this server
		return $generator->getTimestampedID64(
			$generator->getTimestampAndDelay( 'lockFile64', 'millitime' ) );
	}
	 ...
	 * @return string
	 */
	protected function getTimestampedID64( array $time ) {
		list( $sec, $msec ) = $time;
	...
	protected function getTimestampAndDelay( $lockFile, $timeFunc ) {
		...
		do {
			...
			if ( self::lessThanTime( $time, $cTime ) ) {
				...
				fclose( $handle );
				throw MWException( "System clock was set back." );
			}
		} while ( true );
		// Release the UID lock file
		flock( $handle, LOCK_UN );
