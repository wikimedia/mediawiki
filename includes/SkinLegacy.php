...
class LegacyTemplate extends BaseTemplate {
	...
	function doBeforeContent() {
		...
		if ( $this->getSkin()->qbSetting() == 0 ) {
			$s .= "<td class='top' align='left' style='vertical-align: top;' rowspan='{$rows}'>\n" .
				$this->getSkin()->logoText( $wgLang->alignStart() ) . '</td>';
		}

		$l = $wgLang->alignStart();
		$s .= "<td {$borderhack} align='$l' style='vertical-align: top;'>\n";

		$s .= $this->topLinks();
		$s .= '<p class="subtitle">' . $this->pageTitleLinks() . "</p>\n";

		$r = $wgLang->alignEnd();
		$s .= "</td>\n<td {$borderhack} style='vertical-align: top;' align='$r' nowrap='nowrap'>";
		$s .= $this->nameAndLogin();
