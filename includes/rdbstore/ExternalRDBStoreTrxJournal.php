...
class ExternalRDBStoreTrxJournal {
	...
	public function onPrePrepare( $trxId, array $servers ) {
		...
		$data = serialize( array( 'servers' => $servers ) );
		...
		try {
			...
			if ( $dirHandle ) {
				...
				while ( $file = readdir( $dirHandle ) ) {
					$m = array(); // matches
					$regex = '/^(\d{14}-[0-9a-f]{32})\.(commit|prepare)\.tlog$/';
					...
					if ( $step === 'commit' || !isset( $logs[$trxId] ) ) { // commit precedence
			...
			// Get each cluster master DB hosts/IP for the RDB store...
			$masterSrvsInfo = array(); // (host/IP => server info array)
			...
			foreach ( $logs as $trxId => $trxInfo ) {
				if ( $this->finishTransaction( $trxId, $trxInfo, $masterSrvsInfo ) ) {
	 ...
	 * @return boolean
	 */
	protected function finishTransaction( $trxId, array $trxInfo, array $masterSrvsInfo ) {
		$ok = true;
