...
class RDBStoreGroup {
	...
	protected function initFromGlobals() {
		...
		// Sanity check the information for denormalized tables...
		foreach ( $this->denormalizedTables as $table => $tinfo ) {
			if ( !is_string( $tinfo['rowIdColumn'] ) ) {
				throw new MWException( "Table '$table' has no registered ID column." );
			} elseif ( !is_string( $tinfo['canonicalShardKey'] ) ) {
				throw new MWException( "Table '$table' has no registered shard column." );
			} elseif ( !is_array( $tinfo['secondaryShardKeys'] ) ) {
				throw new MWException( "Table '$table' missing registered shard columns." );
			}
		}
	}
