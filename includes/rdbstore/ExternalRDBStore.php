...
class ExternalRDBStore extends RDBStore {
	...
	public function commit() {
		...
		if ( $this->trxDepth > 0 && --$this->trxDepth <= 0 ) {
	...
	public function rollback() {
		...
		if ( $this->trxDepth > 0 && --$this->trxDepth <= 0 ) {
			foreach ( $this->trxJournals as $tj ) { // each journal used
				if ( $tj->getPhase() === ExternalRDBStoreTrxJournal::PHASE_PROPOSING ) {
					$tj->onPreRollback(); // forget affected rows
				}
			}
			foreach ( $this->clusters as $cluster ) { // for existing connections
				$this->getClusterLB( $cluster )->forEachOpenConnection( $funcBegin );
			}
			foreach ( $this->trxJournals as $tj ) { // each journal used
				if ( $tj->getPhase() === ExternalRDBStoreTrxJournal::PHASE_CANCELLING ) {
					$tj->onPostRollback(); // forget affected rows
				}
			}
		}
		return true;
	...
	protected function doGetPartition( $table, $column, $value, $wiki ) {
		// Map this row to a consistent table shard, which MUST only depend on $value
...
class ExternalRDBStoreTrxJournal {
	...
	protected function doCorrectRowSyncAnomalies( array $rowRefs ) {
		foreach ( $rowRefs as $rowRef ) {
			...
			// Get the row on the canonical shard and lock it
			...
			foreach ( $rowRef[self::IDX_REFS] as $dupRowRef ) {
				...
				// Get the row on the denormalized shard and lock it
				$dupRow = (array)$dtpartition->selectRow( DB_MASTER, '*',
					array( $dupRowRef[self::IDX_UID_COL] => $dupRowRef[self::IDX_UID_VAL] ),
					__METHOD__,
					array( 'FOR UPDATE' ) // lock
				);
				// Check if the rows are identical, and sync if needed...
				if ( $row !== $dupRow ) {
	...
	protected function getDenormalizedTableInfo( $table ) {
		global $wgRDBStoreDenormalizedTables;

		if ( !isset( $wgRDBStoreDenormalizedTables[$table] ) ) {
			return null; // table is not denormalized
		}

		$tinfo = $wgRDBStoreDenormalizedTables[$table];
