...
class ExternalRDBStore extends RDBStore {
	...
	protected function beginOutermost() {
		if ( $this->enable2PC ) {
			$this->xaTrxId = wfTimestamp( TS_MW ) . '-' . wfRandomString( 32 );
			wfDebug( "Starting XA transaction with ID {$this->xaTrxId}.\n" );
		} else {
	...
	protected function finishOutermost() {
		...
		if ( count( $xaTrxConns ) == 1 ) { // single DB transaction (1PC)
			...
			foreach ( $connsPrepared as $conn ) {
				$this->connCommitXAInternal( $conn ); // also unregisters
			}
			$this->trxJournal->onPostCommit();
			wfDebug( "Committed distributed XA transaction with ID {$this->xaTrxId}.\n" );
		} else {
			wfDebug( "Committed transaction.\n" );
		}
		$this->xaTrxId = null; // done
	}
	...
	public function connStartXAInternal( DatabaseBase $conn ) {
		if ( !isset( $conn->xa_trx_id ) ) { // not in XA transaction already
			...
			if ( $conn->getType() === 'mysql' ) {
				$conn->query( "XA START {$conn->addQuotes($id)}", __METHOD__ );
			} elseif ( $conn->getType() === 'postgres' ) {
				$conn->query( "BEGIN", __METHOD__ );
			} else {
				throw new DBUnexpectedError( "DB does not support 2PC." );
			}
			$conn->xa_trx_id = $id;
	 ...
	 * @throws DBUnexpectedError
	 */
	public function connEndXAInternal( DatabaseBase $conn ) {
		$id = $this->xaTrxId;
		if ( $conn->getType() === 'mysql' ) {
			$conn->query( "XA END {$conn->addQuotes($id)}", __METHOD__ );
		} elseif ( $conn->getType() === 'postgres' ) {
			// nothing to do
		} else {
			throw new DBUnexpectedError( "DB does not support 2PC." );
	...
	public function connPrepareXAInternal( DatabaseBase $conn ) {
		$id = $this->xaTrxId;
		if ( $conn->getType() === 'mysql' ) {
			$conn->query( "XA PREPARE {$conn->addQuotes($id)}", __METHOD__ );
		} elseif ( $conn->getType() === 'postgres' ) {
			$conn->query( "PREPARE TRANSACTION {$conn->addQuotes($id)}", __METHOD__ );
		} else {
			throw new DBUnexpectedError( "DB does not support 2PC." );
		}
	}
	...
	public function connCommitXAInternal( DatabaseBase $conn, $mode = '2PC', $id = null ) {
		...
		if ( $id === $this->xaTrxId ) { // from begin()/finish()?
			$this->unregisterConnTrxInternal( $conn );
		}
	}
	...
	public function connExistsXAInternal( DatabaseBase $conn, $id ) {
		if ( $conn->getType() === 'mysql' ) {
			...
			return $conn->query(
				"SELECT * FROM pg_prepared_xacts WHERE transaction = {$conn->addQuotes($id)};",
				__METHOD__
			)->numRows() > 0;
		} else {
			throw new DBUnexpectedError( "DB does not support 2PC." );
