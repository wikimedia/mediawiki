...
class ExternalRDBStore extends RDBStore {
	...
	public function begin() {
		if ( ++$this->trxDepth > 1 ) {
			wfDebug( "Deferred BEGIN; no nested transactions allowed." );
		}
		return true;
	}
	...
	public function commit() {
		...
		if ( $this->trxDepth <= 0 ) {
			throw new DBUnexpectedError( "Could not COMMIT; store is not in a transaction." );
		} elseif ( $this->trxDepth > 1 ) {
			wfDebug( "Deferred COMMIT; no nested transactions allowed." );
		}
	...
	public function rollback() {
		...
		if ( $this->trxDepth <= 0 ) {
			throw new DBUnexpectedError( "Could not ROLLBACK; store is not in a transaction." );
		} elseif ( $this->trxDepth > 1 ) {
			throw new DBUnexpectedError( "Could not ROLLBACK; no nested transactions allowed." );
		}
		...
		if ( $this->trxDepth > 0 && --$this->trxDepth <= 0 ) {
			...
			foreach ( $this->clusters as $cluster ) { // for existing connections
				$this->getClusterLB( $cluster )->forEachOpenConnection( $funcRollback );
			}
			foreach ( $this->trxJournals as $tj ) { // each journal used
