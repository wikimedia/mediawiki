...
class AuthManagerSpecialPage extends SpecialPage {
	...
	protected function beforeExecute( $subPage ) {
		...
		if ( $subPage === 'return' ) {
			...
			$authManager->setAuthenticationSessionData( $sessionKey, $sessionParams );

			$this->getOutput()->redirect( $url );
			return false;
		}

		$sessionParams = $authManager->getAuthenticationSessionData( $sessionKey );
		if ( $sessionParams ) {
			$authManager->removeAuthenticationSessionData( $sessionKey );
			$this->isReturn = true;
	...
	protected function loadAuth( $subPage, $authAction = null ) {
		...
		$returnToUrl = $this->getPageTitle( 'return' )->getFullURL( $this->getPreservedParams(),
			false, PROTO_HTTPS );
		$this->authRequests =
