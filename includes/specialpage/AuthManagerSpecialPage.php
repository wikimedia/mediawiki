...
abstract class AuthManagerSpecialPage extends SpecialPage {
		...
		if ( $securityLevel ) {
			...
			if ( $securityStatus === AuthManager::SEC_REAUTH ) {
				...
				$query = [
					...
					'returntoquery' => wfArrayToCgi( [ 'authUniqueId' => $uniqueId ]
						+ array_diff_key( $request->getQueryValues(), [ 'title' => true ] ) ),
					'force' => $securityLevel,
				];
	...
	protected function getAuthFormDescriptor( $requests, $action ) {
		...
		$formDescriptor = $this->fieldInfoToFormDescriptor( $fieldInfo, $action );
	 ...
	 */
	protected function fieldInfoToFormDescriptor( array $fieldInfo, $action ) {
		$formDescriptor = [];
		foreach ( $fieldInfo as $fieldName => $singleFieldInfo ) {
			$formDescriptor[$fieldName] = self::mapSingleFieldInfo( $singleFieldInfo, $fieldName );
		}

		$this->onAuthChangeFormFields( $fieldInfo, $formDescriptor, $action );
	 ...
	 * @return array
	 */
	private static function mapSingleFieldInfo( $singleFieldInfo, $fieldName ) {
	 ...
	 * @return array
	 */
	private static function sortFormDescriptorFields( array &$formDescriptor ) {
		$i = 0;
	 ...
	 * @throws \LogicException
	 */
	private static function mapFieldInfoTypeToFormDescriptorType( $type ) {
		$map = [
			'string' => 'text',
			'password' => 'password',
			'select' => 'select',
