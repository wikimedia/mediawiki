...
abstract class AuthManagerSpecialPage extends SpecialPage {
		...
		switch ( $action ) {
			...
			case AuthManager::ACTION_UNLINK:
				...
				if ( $action === AuthManager::ACTION_CHANGE ) {
					$this->runLegacyChangeHook( $req, $status );
				}
				if ( !$status->isOK() ) {
	...
	protected function trySubmit() {
		...
		if ( $this->authAction === AuthManager::ACTION_CHANGE && $status && !$status->isOK() ) {
			$this->runLegacyChangeHook( reset( $this->authRequests ), $status );
		}
	 ...
	 * @param Status $status
	 */
	protected function runLegacyChangeHook( $req, $status ) {
		Hooks::run( 'ChangeAuthenticationDataAudit', [ $req, $status ], '1.27' );
	}
}
