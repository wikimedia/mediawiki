...
abstract class ChangesListSpecialPage extends SpecialPage {
			...
			[
				...
				'filters' => [
					[
						...
						'default' => false,
						'queryCallable' => function ( $specialClassName, $ctx, $dbr, &$tables, &$fields, &$conds, &$query_options, &$join_conds ) {
							$conds[] = 'rc_user = 0';
						}
			...
			[
				...
				'filters' => [
					[
						...
						'queryCallable' => function ( $dbr, $pageClass, $ctx, &$tables, &$fields, &$conds, &$query_options, &$join_conds ) {
	...
	protected function registerFiltersFromDefinitions( array $definition ) {
		...
		foreach ( $definition as $groupDefinition ) {
			...
			$priority--;
			$this->registerFilterGroup( new ChangesListFilterGroup( $groupDefinition ) );
		}
	}
	...
	public function parseParameters( $par, FormOptions $opts ) {
		...
		} else if ( $filterGroup->getType() === ChangesListFilterGroup::SEND_UNSELECTED_IF_ANY ) {
			foreach ( $filterGroup->getFilters() as $filter ) {
		...
		foreach ( $bits as $bit ) {
			if ( isset( $hideParameterNameSet[$bit] ) ) {
				...
				if ( isset( $stringParameterNameSet[$m[0]] ) ) {
					$opts[$m[0]] = $m[1];
				}
			}
		}
	...
	protected function buildQuery( &$tables, &$fields, &$conds, &$query_options,
		...
		foreach( $this->filterGroups as $filterGroup ) {
			...
			// or per-filter, like 'hideminor'.
			if ( $filterGroup->isPerGroupRequestParameter() ) {
				if ( $opts[$filterGroup->getName()] !== ChangesListFilterGroup::NONE ) {
	 ...
	 */
	public function filterOnUserExperienceLevel( $specialPageClassName, $context, $dbr, &$tables, &$fields, &$conds, &$query_options, &$join_conds, $selectedExpLevels ) {
