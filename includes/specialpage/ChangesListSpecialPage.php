...
abstract class ChangesListSpecialPage extends SpecialPage {
	...
	protected function redirectForDefaultSavedQuery() {
		...
		foreach ( $excludedParams as $excluded ) {
			$i = array_search( $excluded, array_keys( $knownParams ) );
			if ( $i !== false ) {
				// Remove from array
				array_splice( $knownParams, $i, 1 );
			}
		}
		...
		if (
			...
			$savedQueryDefaultID = $savedQueries && isset( $savedQueries[ 'default' ] ) ? $savedQueries[ 'default' ] : '';
			$savedQueryVersion = $savedQueries && isset( $savedQueries[ 'version' ] ) ? $savedQueries[ 'version' ] : '';
