...
abstract class ChangesListSpecialPage extends SpecialPage {
	...
	/**
	 * Whether the validation process for the options is redirecting us to a fixed-up URL.
	 * If this is set to true, we know that a redirect is coming and won't attempt to generate
	 * any output or perform any queries.
	 *
	 * @var boolean
	 */
	protected $validationRedirect = false;

	/**
	 * Whether the validation process found that the options were valid.
	 * If this is set to false, we will not perform a query and immediately generate the
	 * "no results" output, along with an HTTP 400 status code.
	 *
	 * @var boolean
	 */
	protected $optionsValid;
	 ...
	 * @return bool
	 */
	protected function areFiltersInConflict( FormOptions $opts ) {
	...
	public function execute( $subpage ) {
		...
		if ( !$this->optionsValid ) {
			// Don't output feed links for invalid options, and don't attempt to perform the query
		...
		// HACK: Format the result, then check if any rows were formatted
		// We have to do this because categorization rows for hidden categories
		// can get dropped, so we can end up formatting 0 rows even if getNumRows() > 0
		$body = $this->pager->getBody();
		if ( $this->pager->getNumFormattedRows() === 0 ) {
			if ( !$this->including() ) {
				$this->outputNoResults();
	...
	public function setup( $parameters ) {
		...
		$this->optionsValid = $this->validateOptions( $opts );
