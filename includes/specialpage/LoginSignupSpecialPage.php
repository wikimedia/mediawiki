...
abstract class LoginSignupSpecialPage extends AuthManagerSpecialPage {
	...
	protected function getAuthForm( array $requests, $action, $msg = '', $msgType = 'error' ) {
		...
		$formDescriptor = static::fieldInfoToFormDescriptor( $fieldInfo, $this->authAction );
...
class LoginForm extends SpecialPage {
	...
	public function __construct( $request = null ) {
		...
		if ( $wgDisableAuthManager ) {
			$this->realLoginForm = new LoginForm_PreAuthManager( $request );
		} else {
			$this->realLoginForm = new LoginForm_AuthManager( $request );
		}
	}
	...
	public static function __callStatic( $name, $args ) {
		global $wgDisableAuthManager;
		return call_user_func_array( [ $wgDisableAuthManager ? 'LoginForm_PreAuthManager'
			: 'LoginForm_AuthManager', $name ], $args );
	}

	// all public SpecialPage methods need to be proxied explicitly

	public function getName() {
		return call_user_func_array( array( $this->realLoginForm, __FUNCTION__ ), func_get_args() );
	}
 ...
 */
class LoginForm_AuthManager extends SpecialPage {
	const SUCCESS = 0;
	const NO_NAME = 1;
	const ILLEGAL = 2;
	const WRONG_PLUGIN_PASS = 3;
