...
class HTMLTitleTextField extends HTMLTextField {
	...
	public function validate( $value, $alldata ) {
		...
		if ( $this->mParams['namespace'] !== false && !$title->inNamespace( $this->mParams['namespace'] ) ) {
			return $this->msg( 'htmlform-title-badnamespace' )->escaped();
		}

		if ( $this->mParams['creatable'] && !$title->canExist() ) {
			return $this->msg( 'htmlform-title-not-creatable' )->escaped();
		}

		if ( $this->mParams['exists'] && !$title->exists() ) {
			return $this->msg( 'htmlform-title-not-exists', $title->getPrefixedText() )->parse();
		}
	...
	protected function getInputWidget( $params ) {
		if ( isset( $params['readOnly'] ) ) {
			// Read-only, no point in adding extra JS
			return parent::getInputWidget( $params );
		}
