...
/**
 * A radio matrix
 * Operates similarly to HTMLCheckMatrix, but uses radio buttons instead of checkboxes
 *
 * Options:
 *   - columns
 ...
 * @since 1.25
 */
class HTMLRadioMatrix extends HTMLFormField implements HTMLNestedFilterable {
	static private $requiredParams = array(
		// Required by underlying HTMLFormField
		'fieldname',
		// Required by HTMLRadioMatrix
		'rows',
		'columns'
	);
	...
	function validate( $value, $alldata ) {
		...
		$validOptions = array();
		foreach ( $rows as $rowLabel => $rowInfos ) {
			$rowTag = isset( $rowInfos['name'] ) ? $rowInfos['name'] : $rowLabel;
			$validOptions[] = $rowTag;
		}
	...
	/**
	 * Build a table containing a matrix of radiobutton options.
	...
	function getInputHTML( $value ) {
		...
		$headerContents = Html::rawElement( 'th', array(), '&#160;' );
		foreach ( $columns as $columnLabel => $columnInfos ) {
			$columnTag = isset( $columnInfos['name'] ) ? $columnInfos['name'] : $columnLabel;
			$thAttribs = array();
			if ( isset( $columnInfos['cssclass'] ) ) {
				$thAttribs['class'] = $columnInfos['cssclass'];
			}
			$headerContents .= Html::rawElement( 'th', $thAttribs, $columnLabel );
		}
		...
		foreach ( $rows as $rowLabel => $rowInfos ) {
			...
			$rowOptions = isset( $rowInfos['options'] ) ? $rowInfos['options'] : array();
			$rowDefault = isset( $rowInfos['default'] ) ? $rowInfos['default'] : array();
			$rowTdAttribs = array();
			if ( isset( $rowInfos['cssclass'] ) ) {
				$rowTdAttribs['class'] = $rowInfos['cssclass'];
			}
			$rowContents = Html::rawElement( 'td', array(), $rowLabel );
			foreach ( $columns as $columnInfos ) {
				$columnTag = isset( $columnInfos['name'] ) ? $columnInfos['name'] : $columnLabel;
				$thisTag = "$rowTag-$columnTag";
				// Construct the radiobox
				$thisId = "{$this->mID}-$thisTag";
				$thisAttribs = array(
					'id' => $thisId,
				);
				$value = isset( $rowOptions[$columnTag] ) ? $rowOptions[$columnTag] : null;
				$checked = $value === $rowDefault;
				...
				$chkBox = Xml::radio( $rowTag, $value, $checked, $attribs + $thisAttribs );
	...
	function loadDataFromRequest( $request ) {
		if ( $this->mParent->getMethod() == 'post' ) {
			if ( $request->wasPosted() ) {
				// Radiobuttons are not added to the request arrays if non is checked,
