...
class CacheHeaderCdnController implements CdnController {
	 ...
	 * @param string[] $squidServers Addresses of servers to actively purge
	 */
	public function __construct( $maxAge, array $squidServers = [] ) {
		Assert::parameterType( 'int', $maxAge, '$maxAge' );
		$this->maxAge = $maxAge;

		Assert::parameterElementType( 'string', $squidServers, '$squidServers' );
		$this->squidServers = $squidServers;
	}
	...
	public function purgeURLs( array $resourceUris ) {
		// FIXME: pass $this->squidServers to CdnCacheUpdate
	 ...
	 */
	public function applyCacheControl( Title $requestTitle, WebRequest $request, OutputPage $output, array $dependsOn = [] ) {
