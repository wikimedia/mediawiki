...
class FSFileBackend extends FileBackendStore {
	...
	/**
	 * @see FileBackendStore::getDirListInternal()
	 * @return Array|null
	 */
	public function getSubdirectoryListInternal( $fullCont, $dirRel, array $params ) {
		list( $b, $shortCont, $r ) = FileBackend::splitStoragePath( $params['dir'] );
		$contRoot = $this->containerFSRoot( $shortCont, $fullCont ); // must be valid
		$dir = ( $dirRel != '' ) ? "{$contRoot}/{$dirRel}" : $contRoot;
		$exists = is_dir( $dir );
		...
		return new FSFileBackendDirList( $dir );
	}
...
abstract class FSFileBackendList implements Iterator {
	 ...
	 * @return Iterator
	 */
	abstract protected function initIterator( $dir );
...
class FSFileBackendDirList extends FSFileBackendList {
	protected function initIterator( $dir ) {
		try {
			# Get an iterator that will direct subdirectories
			return new DirectoryIterator( $dir );
		} catch ( UnexpectedValueException $e ) {
			return null; // bad permissions? deleted?
		}
	}
...
class FSFileBackendFileList extends FSFileBackendList {
	 ...
	 */
	public function current() {
		return $this->getRelPath( (string)$this->iter->current() );
	}
}
