...
class FileOpBatch {
	...
	const MAX_CONCURRENCY = 50; // integer
	...
	public static function attempt( array $performOps, array $opts, FileJournal $journal ) {
		...
		foreach ( $performOps as $index => $fileOp ) {
			...
			if ( $fileOp->dependsOn( $curBatchDeps )
				|| count( $curBatch ) >= self::MAX_CONCURRENCY
		...
		if ( !empty( $opts['parallelize'] ) && count( $pPerformOps ) < count( $performOps ) ) {
		...
		foreach ( $pPerformOps as $performOpsBatch ) {
			...
			foreach ( $performOpsBatch as $i => $fileOp ) {
				if ( !$fileOp->failed() ) { // failed => already has Status
					...
					if ( $subStatus->value instanceof FileOpHandle ) {
