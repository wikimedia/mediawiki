...
class FileBackendDBRepoWrapper extends FileBackend {
	 ...
	 *
	 * @param string $name File SHA-1 base36
	 * @param string $ext File extension
	 * @return string
	 */
	public function getPath( $sha1, $ext ) {
		if ( !strlen( $sha1 ) ) {
			throw new MWException( "Invalid file SHA-1." );
		}
		return $this->backend->getContainerStoragePath( "{$this->repoName}-original" ) .
			"/{$sha1[0]}/{$sha1[1]}/{$sha1[2]}/{$sha1}.$ext";
	}

	/**
	 * Translate a legacy "title" path to it's "sha1" counterpart
	 *
	 * E.g. mwstore://local-public/a/ab/<name>.jpg => mwstore://local-original/x/y/z/<sha1>.jpg
	 *
	 * @param string $path
	 * @param bool $latest
	 * @return string
