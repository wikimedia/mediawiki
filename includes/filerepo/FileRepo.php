...
class FileRepo {
	...
	public function __construct( array $info = null ) {
		...
		foreach ( array( 'public', 'thumb', 'transcode', 'temp', 'deleted' ) as $zone ) {
	...
	public function getZoneUrl( $zone, $ext = null ) {
		if ( in_array( $zone, array( 'public', 'temp', 'thumb', 'transcode' ) ) ) { // standard public zones
	...
	public function getZoneHandlerUrl( $zone ) {
		if ( isset( $this->zones[$zone]['handlerUrl'] )
			&& in_array( $zone, array( 'public', 'temp', 'thumb', 'transcode' ) ) )
		{
	...
	public function getTempRepo() {
		return new TempFileRepo( array(
			...
			'zones'     => array(
				...
				'transcode'  => array(
					'container' => $this->zones['transcode']['container'],
					'directory' => ( $this->zones['transcode']['directory'] == '' )
						? 'temp'
						: $this->zones['transcode']['directory'] . '/temp'
				)
			),
			'url'        => $this->getZoneUrl( 'temp' ),
			'thumbUrl'   => $this->getZoneUrl( 'thumb' ) . '/temp',
			'transcodeUrl'   => $this->getZoneUrl( 'transcode' ) . '/temp',
