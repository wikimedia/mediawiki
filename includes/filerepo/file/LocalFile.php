...
class LocalFile extends File {
	...
	function upload( $srcPath, $comment, $pageText, $flags = 0, $props = false,
		...
		if ( $status->successCount >= 2 ) {
			...
			if ( !$this->recordUpload2( $status->value, $comment, $pageText, $props, $timestamp, $user, $tags ) ) {
	...
	function recordUpload2(
		...
		$dbw->onTransactionIdle( function () use (
			...
			if ( $tags ) {
				ChangeTags::addTags( $tags, $rc->getAttribute( 'rc_id' ), $logEntry->getAssociatedRevId(), $logId );
			}
