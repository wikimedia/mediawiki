...
class LocalFile extends File {
	...
	function recordUpload2(
		...
		list( $actorFields, $actorCallback ) =
			$actorMigration->getInsertValuesWithTempTable( $dbw, 'img_user', $user );
		$dbw->insert( 'image',
			[
				'img_name' => $this->getName(),
				'img_size' => $this->size,
		...
		if ( $reupload ) {
			...
			if ( $wgActorTableSchemaMigrationStage !== MIGRATION_OLD &&
				...
				foreach ( $res as $row ) {
					list( , $callback ) = $actorMigration->getInsertValuesWithTempTable( $dbw, 'img_user',
						User::newFromAnyId( $row->img_user, $row->img_user_text, null )
					);
