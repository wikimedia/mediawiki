...
class LocalFile extends File {
	...
	function purgeCache( $options = array() ) {
		...
		DeferredUpdates::addUpdate(
			new CdnUpdate( array( $this->getUrl() ) ),
	...
	function purgeOldThumbnails( $archiveName ) {
		...
		DeferredUpdates::addUpdate( new CdnUpdate( $urls ), DeferredUpdates::PRESEND );
	}
	...
	function recordUpload2(
		...
		$dbw->onTransactionIdle( function () use (
			...
			if ( $reupload ) {
				...
				DeferredUpdates::addUpdate(
					new CdnUpdate( array( $that->getUrl() ) ),
					DeferredUpdates::PRESEND
				);
			} else {
	...
	function delete( $reason, $suppress = false, $user = null ) {
		...
		DeferredUpdates::addUpdate( new CdnUpdate( $purgeUrls ), DeferredUpdates::PRESEND );
	...
	function deleteOld( $archiveName, $reason, $suppress = false, $user = null ) {
		...
		DeferredUpdates::addUpdate(
			new CdnUpdate( array( $this->getArchiveUrl( $archiveName ) ) ),
			DeferredUpdates::PRESEND
		);
