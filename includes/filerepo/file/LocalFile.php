	...
	function recordUpload2(
		...
		if ( $reupload ) {
			...
			if ( $wgActorTableSchemaMigrationStage !== MIGRATION_OLD &&
				...
				foreach ( $res as $row ) {
					list( , $callback ) = $actorMigration->getInsertValuesWithTempTable( $dbw, User::newFromRow( [
	...
	protected function doDBInserts() {
		...
		if ( $deleteCurrent ) {
			...
			if ( $wgActorTableSchemaMigrationStage !== MIGRATION_OLD &&
				...
				foreach ( $res as $row ) {
					list( , $callback ) = $imgActorMigr->getInsertValuesWithTempTable( $dbw, User::newFromRow( [
						'user_id' => $row->img_user,
						'user_name' => $row->img_user_text,
					] ) );
					$callback( $row->img_name, (array)$row );
