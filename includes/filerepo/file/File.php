...
abstract class File {
	...
	/** @var array Cache of tmp filepaths pointing to generated thumbnails, keyed by width */
	protected $tmpThumbCache = array();
	...
	/**
	 * Return the nearest width thumbnail bucket, if any
	 *
	...
	public function getThumbnailBucket( $desiredWidth, $page = 1 ) {
		...
		if ( !is_array( $wgThumbnailBuckets ) || empty( $wgThumbnailBuckets ) ) {
			return false;
		}
	...
	function transform( $params, $flags = 0 ) {
		...
		do {
			...
			if ( !$tmpFile ) {
				...
				$thumb = $this->generateAndSaveThumb( $tmpFile, $thumbName, $params, $flags );
			}
		} while ( false );
	 ...
	 */
	public function generateAndSaveThumb( $tmpFile, $thumbName, $transformParams, $flags ) {
	 ...
	 * @param int $flags
	 */
	protected function generateBucketsIfNeeded( $params, $flags ) {
		$bucket = $this->getThumbnailBucket( $params[ 'physicalWidth' ] );

		if ( $this->repo && $bucket && $bucket != $params[ 'physicalWidth' ] ) {
			...
			if ( !$this->repo->fileExists( $bucketPath ) ) {
				...
				$thumb = $this->generateAndSaveThumb( $tmpFile, $bucketName, $params, $flags );

				if ( $thumb && !$thumb->isError() ) {
					$this->tmpThumbCache[ $bucket ] = $tmpFile->getPath();
					// For the caching to work, we need to make the tmp file survive as long as this object instance
					$tmpFile->bind( $this );
				}
			}
		}
	 ...
	 * @return string
	 */
	public function getSourcePath( $params ) {
		$bucket = $this->getThumbnailBucket( $params[ 'physicalWidth' ] );

		if ( $this->repo && $this->getHandler()->supportsBucketing() && $bucket ) {
			// Try to avoid reading from storage if the file was generated by this script
			if ( isset( $this->tmpThumbCache[ $bucket ] ) ) {
				$tmpPath = $this->tmpThumbCache[ $bucket ];
