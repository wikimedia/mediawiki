...
/**
 * Class that imports data into the facilitator from a WebRequest
 */
class WebEditFacilitator extends EditFacilitator {
	...
	final public function importFormData( WebRequest $request ) {
		...
		if ( $request->wasPosted() ) {
			...
			$this->dataWrapper->setTextbox1( $this->safeUnicodeInput( $request, 'wpTextbox1' ) );
			...
			$this->dataWrapper->setSummary( $request->getText( 'wpSummary' ) );
			$this->dataWrapper->setSectionTitle( $request->getText( 'wpSectionTitle' ) );

			$this->dataWrapper->setEditTime( $request->getVal( 'wpEdittime' ) );
			$this->dataWrapper->setEditRevId( $request->getIntOrNull( 'editRevId' ) );
			$this->dataWrapper->setStartTime( $request->getVal( 'wpStarttime' ) );
		...
		$this->dataWrapper->setContentModelAndFormat( $contentModel, $contentFormat );
