...
/**
 * Abstract class for an editor
 * An editor provides the ability to, at least, retrieve, edit and save page content.
 *
 * This class makes permission checks and handles the status returned by EditFacilitator::attemptSave.
 * The edit() method is actually implemented by subclasses.
 */
abstract class Editor {
	...
	/** @var string subclass of EditView */
	...
	/** @var string subclass of WebEditFacilitator */
	protected $facilitatorClass = 'WebEditFacilitator';
	...
	final public function __construct( Article $article, User $user, Title $contextTitle ) {
		$this->facilitator = new $this->facilitatorClass( $article->getPage(), $user );
		$this->edit = $this->facilitator->getEdit(); // shortcut
		...
		// edit view
		$this->view = new $this->viewClass( $this->context, $this->edit, $contextTitle,
			$this->action );
	}
	 ...
	 */
	protected function handleStatus( Status $status ) {
		$resultDetails = $status->details;
		/**
	...
	protected function initialiseForm( $revision ) {
		$this->edit->setLastRevisionTime( $this->page->getTimestamp() );
		$this->edit->setLastRevisionId( $this->page->getLatest() );
		...
		$undoMsg = '';
		// TODO: track content object?!
		$content = $this->facilitator->getContentObject( $revision, $undoMsg, false );

		if ( !$this->isSupportedContentModel( $content->getModel() ) ) {
			throw new MWException( 'This content model is not supported: ' . $content->getModel() );
		}

		if ( $this->edit->getSection() !== '' && !$this->supportsSectionEditing ) {
		...
		$watchThis = $this->facilitator->getWatchThis();
		...
		$minorEdit = $this->facilitator->getMinorEdit();
		if ( $this->user->getOption( 'minordefault' ) && $this->title->exists() &&
			$this->edit->getSection() !== 'new'
		) {
			$minorEdit = true;
		}
		$this->view->setMinorEdit( $minorEdit );

		if ( $this->edit->getText() === false ) {
			return false;
		}
		return true;
	}
	...
	final public function toEditText( $content ) {
		...
		return $content->serialize( $this->edit->getContentFormat() );
	}
