...
class StandardEditor extends BasicEditor {
	...
	public function appendToPreview( $text ) {
		$this->view->appendToPreviewTextAfterContent( $text );
	}
	...
	public function showDiff() {
		...
		if ( $this->title->getNamespace() == NS_MEDIAWIKI && !$this->title->exists() ) {
			...
			if ( $oldtext !== false ) {
				...
				$oldContent = $this->model->toEditContent( $oldtext );
			} else {
				$oldContent = null;
			}
		} else {
			$oldContent = $this->model->getCurrentContent();
		}

		$textboxContent = $this->model->toEditContent( $this->data->textbox1 );
	...
	public function getPreviewText() {
		...
		if ( $wgRawHtml && !$this->controller->isTokenOk() ) {
		...
		try {
			$content = $this->model->toEditContent( $this->data->textbox1 );
			...
			if ( $this->controller->triedSave() && !$this->controller->isTokenOk() ) {
				if ( $this->controller->isTokenOkExceptSuffix() ) {
			...
			} elseif ( $this->controller->isIncompleteForm() ) {
				$note = $this->context->msg( 'edit_form_incomplete' )->plain();
				if ( $this->controller->triedSave() ) {
	...
	protected function getPreviewParserOptions() {
		...
		$parserOptions->setIsSectionPreview( !is_null( $this->data->section ) && $this->data->section !== '' );
	...
	protected function showConflict( $isUnicodeCompliant ) {
		if ( Hooks::run( 'EditorBeforeConflictDiff', [ $this, $this->output ] ) ) {
			...
			$content1 = $this->model->toEditContent( $this->data->textbox1 );
			$content2 = $this->model->toEditContent( $this->data->textbox2 );
	...
	protected function onGetEditPermissionErrors( &$permErrors ) {
		...
		foreach ( $permErrors as $error ) {
			if ( ( $this->data->preview || $this->data->diff )
				&& (
					$error[0] == 'blockedtext' ||
	...
	protected function displayPermissionsError( array $permErrors, $revision ) {
		...
		$content = $this->getContentObject( $revision );
	...
	protected function getViewSourceText( Content $content ) {
		...
		if ( !$this->firstTime ) {
			...
			try {
				$text = $this->model->toEditText( $content );
			} catch ( MWException $e ) {
	...
	protected function disallowDifferentContentModel ( $revision ) {
		$this->view->displayViewSourcePage(
			$this->getViewSourceText( $this->getContentObject( $revision ) ),
			$this->context->msg(
