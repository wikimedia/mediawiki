...
class StandardEditor extends BasicEditor {
	...
	protected function onBeforeShowMainTextboxOnConflict() {
		...
		$this->conflictText = $this->edit->getText();
	}
	...
	protected function previewOnOpen( $request ) {
		...
		if ( $request->getVal( 'preview' ) === 'yes' ) {
			...
			// Explicit override from request
			return false;
		} elseif ( $this->edit->getSection() === 'new' ) {
	...
	public function showDiff() {
		...
		$textboxContent = $this->toEditContent( $this->edit->getText() );
		$lastRevId = $this->edit->getLastRevisionId();
		if ( $lastRevId !== null ) {
			$newContent = $this->page->replaceSectionAtRev(
				$this->edit->getSection(), $textboxContent, $this->edit->getSummary(), $lastRevId
			);
		} else {
			$lastRevTime = $this->edit->getLastRevisionTime();
			$newContent = $this->page->replaceSectionContent(
				$this->edit->getSection(), $textboxContent, $this->edit->getSummary(), $lastRevTime
			);
		}
	...
	public function getPreviewText() {
		...
		try {
			$content = $this->toEditContent( $this->edit->getText() );
			...
			if ( $this->edit->getSection() === "new" && $this->edit->getSummary() !== "" ) {
	...
	protected function getPreviewParserOptions() {
		...
		$parserOptions->setIsSectionPreview( $this->edit->getSection() !== '' );
	...
	protected function showConflict( $isUnicodeCompliant ) {
		if ( Hooks::run( 'EditorBeforeConflictDiff', [ $this, $this->output ] ) ) {
			...
			$content1 = $this->toEditContent( $this->edit->getText() );
			$content2 = $this->toEditContent( $this->conflictText );
			...
			$this->view->showConflictTextbox( $this->conflictText, $isUnicodeCompliant,
	...
	protected function displayPermissionsError( array $permErrors, $revision ) {
		...
		if ( $this->formType === 'initial' && ( !$content || $content->isEmpty() ) ) {
	...
	public function spamPageWithContent( $match = false ) {
		...
		$this->output->wrapWikiMsg( '<h2>$1</h2>', "yourtext" );
		$this->view->showSpamTextbox( $this->edit->getText(), $this->checkUnicodeCompliantBrowser(),
			$this->user->getOption( 'editfont' ) );
