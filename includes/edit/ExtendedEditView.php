<?php
/**
 * Extended user interface elements for page editing.
 *
...
class ExtendedEditView extends BasicEditView {
	...
	public function showConflictTextbox( $text, $isUnicodeCompliant, $font ) {
		$this->showTextbox( $text, 'wpTextbox2', [ 'tabindex' => 6, 'readonly' ],
			$isUnicodeCompliant, $font );
	}

	public function showSpamTextbox( $text, $isUnicodeCompliant, $font ) {
		$this->showTextbox( $text, 'wpTextbox2', [ 'tabindex' => 6, 'readonly' ],
			$isUnicodeCompliant, $font );
	}
	...
	public function getTemplates() {
		if ( $this->formType === 'preview' || $this->edit->getText() !== '' ) {
			$templates = [];
	...
	protected function getSummaryPreview( $isSubjectPreview, $summary = "" ) {
		...
		if ( !$summary || ( $this->formType !== 'preview' && $this->formType !== 'diff' ) ) {
			return "";
		}
	...
	/**
	 * Override parent to add a preview of the summary / section title
	 */
	protected function showSummaryInput( $isSubjectPreview, $summary = "", $noSummary = false ) {
		parent::showSummaryInput( $isSubjectPreview, $summary, $noSummary );
		$this->output->addHTML( $this->getSummaryPreview( $isSubjectPreview, $summary ) );
	}
}
