<?php
/**
 * Makes an edit attempt based on some edit form data
 *
...
class EditAttempt {
	...
	final public function __construct( WikiPage $page, User $user, Edit $edit ) {
		$this->page = $page;
	 ...
	 *  - autoSummHash (string): the md5 hash of the autosummary used
	 ...
	 *   and the following possible keys in $status->details:
	 ...
	 */
	final public function performSave( array $info, array $options ) {
		global $wgMaxArticleSize, $wgContentHandlerUseDB;

		$status = Status::newGood();
		$status->details = [];

		if ( !$this->runPreAttemptHooks( $info, $options, $status ) ) {
			return $status;
		}
		...
		try {
			...
			$textbox_content = ContentHandler::makeContent( $this->edit->getText(), $this->title,
				$this->edit->getContentModel(), $this->edit->getContentFormat() );
		} catch ( MWContentSerializationException $ex ) {
			$status->fatal(
				'content-failed-to-parse',
				$this->edit->getContentModel(),
				$this->edit->getContentFormat(),
				$ex->getMessage()
			);
		...
		# Check for spam
		$match = EditUtilities::matchSummarySpamRegex( $this->edit->getSummary() );
		if ( $match === false && $this->edit->getSection() === 'new' ) {
			# $wgSpamRegex is enforced on this new heading/summary because, unlike
			# regular summaries, it is added to the actual wikitext.
			if ( $this->edit->getSectionTitle() !== '' ) {
				# This branch is taken when the API is used with the 'sectiontitle' parameter.
				$match = EditUtilities::matchSpamRegex( $this->edit->getSectionTitle() );
			} else {
				# This branch is taken when the "Add Topic" user interface is used, or the API
				# is used with the 'summary' parameter.
				$match = EditUtilities::matchSpamRegex( $this->edit->getSummary() );
			}
		}
		if ( $match === false ) {
			$match = EditUtilities::matchSpamRegex( $this->edit->getText() );
		}
		if ( $match !== false ) {
			$status->details['spam'] = $match;
		...
		if ( !$this->runPreMergeHooks( $info, $options, $status ) ) {
			return $status;
		}
		...
		$contentLength = strlen( $this->edit->getText() );
		...
		if ( $this->edit->getContentModel() !== $this->title->getContentModel() ) {
			...
			$titleWithNewContentModel->setContentModel( $this->edit->getContentModel() );
		...
		# confirmation
		if ( EditUtilities::wasDeletedSince( $this->title, $this->edit->getStartTime() ) &&
			!$options['recreate']
		) {
		...
		if ( $new ) {
			...
			if ( !$options['allowBlankArticle'] && $this->edit->getText() === $defaultText ) {
				$status->fatal( 'blankarticle' );
				$status->setResult( false, self::AS_BLANK_ARTICLE );
				return $status;
			}

			if ( !$this->runPostMergeFilters( $textbox_content, $info, $options, $status ) ) {
				return $status;
			}

			$content = $textbox_content;

			$status->details['sectionanchor'] = '';
			if ( $this->edit->getSection() === 'new' ) {
				if ( $this->edit->getSectionTitle() !== '' ) {
					// Insert the section title above the content.
					$content = $content->addSectionHeader( $this->edit->getSectionTitle() );
				} elseif ( $this->edit->getSummary() !== '' ) {
					// Insert the section title above the content.
					$content = $content->addSectionHeader( $this->edit->getSummary() );
				}
				$this->edit->setSummary( $this->newSectionSummary( $status->details['sectionanchor'] ) );
			}
			...
			wfDebug( "timestamp: {$timestamp}, edittime: {$this->edit->getLastRevisionTime()}\n" );

			// Check editRevId if set, which handles same-second timestamp collisions
			if ( $timestamp !== $this->edit->getLastRevisionTime()
				|| ( $this->edit->getLastRevisionId() !== null &&
				$this->edit->getLastRevisionId() !== $latest )
			) {
				$isConflict = true;
				if ( $this->edit->getSection() === 'new' ) {
				...
				} elseif ( $this->edit->getSection() === ''
					&& Revision::userWasLastToEdit(
						DB_MASTER, $this->title->getArticleID(),
						$this->user->getId(), $this->edit->getLastRevisionTime()
					)
				) {
			...
			if ( $this->edit->getSectionTitle() !== '' ) {
				$sectionTitle = $this->edit->getSectionTitle();
			} else {
				$sectionTitle = $this->edit->getSummary();
			}
			...
			if ( $isConflict ) {
				wfDebug( __METHOD__
					. ": conflict! getting section '{$this->edit->getSection()}' ".
					" for time '{$this->edit->getLastRevisionTime()}'"
					. " (id '{$this->edit->getLastRevisionId()}') (article time '{$timestamp}')\n" );
				...
				if ( $this->edit->getLastRevisionId() !== null ) {
					$content = $this->page->replaceSectionAtRev(
						$this->edit->getSection(),
						$textbox_content,
						$sectionTitle,
						$this->edit->getLastRevisionId()
					);
				} else {
					$content = $this->page->replaceSectionContent(
						...
						$textbox_content,
						$sectionTitle,
						$this->edit->getLastRevisionTime()
					);
				}
			} else {
				wfDebug( __METHOD__ . ": getting section '{$this->edit->getSection()}'\n" );
				$content = $this->page->replaceSectionContent(
					$this->edit->getSection(),
					$textbox_content,
					$sectionTitle
			...
			if ( is_null( $content ) ) {
				...
				if ( $this->mergeChangesIntoContent( $content, $newParent ) ) {
					...
					$isConflict = false;
					$status->details['newParent'] = $newParent;
					wfDebug( __METHOD__ . ": Suppressing edit conflict, successful merge.\n" );
				} else {
					$this->edit->setSection( '' );
					$this->edit->setText( ContentHandler::getContentText( $content ) );
			...
			if ( !$this->runPostMergeFilters( $content, $info, $options, $status ) ) {
				return $status;
			}

			if ( $this->edit->getSection() === 'new' ) {
				// Handle the user preference to force summaries here
				if ( !$options['allowBlankSummary'] && trim( $this->edit->getSummary() ) === '' ) {
					// or 'missingcommentheader' if $this->edit->getSection() === 'new'. Blegh
				...
				&& !$content->isRedirect()
				&& md5( $this->edit->getSummary() ) === $info['autoSummHash']
			) {
				$status->fatal( 'missingsummary' );
				$status->value = self::AS_SUMMARY_NEEDED;
				return $status;
			...
			$sectionanchor = '';
			if ( $this->edit->getSection() === 'new' ) {
				$this->edit->setSummary( $this->newSectionSummary( $sectionanchor ) );
			} elseif ( $this->edit->getSection() !== '' ) {
			...
			$status->details['sectionanchor'] = $sectionanchor;

			// Save errors may fall down to the edit form, but we've now
			// merged the section into full text. Clear the section field
			// so that later submission of conflict forms won't try to
			// replace that into a duplicated mess.
			$this->edit->setText( $content->serialize( $this->edit->getContentFormat() ) );
			$this->edit->setSection( '' );
		...
		$contentLength = strlen( $content->serialize( $this->edit->getContentFormat() ) );
		if ( $contentLength > $wgMaxArticleSize * 1024 ) {
		...
		$minor = $options['minorEdit'] && !$new && $this->edit->getSection() !== 'new';
		...
		$status->details['nullEdit'] = $doEditStatus->hasMessage( 'edit-no-change' );
		if ( $status->details['nullEdit'] ) {
			// We don't know if it was a null edit until now, so increment here
			$this->user->pingLimiter( 'linkpurge' );
		}
		$status->details['redirect'] = $content->isRedirect();
	...
	private function newSectionSummary( &$sectionanchor = null ) {
		...
		$sectionTitle = $this->edit->getSectionTitle();
		$summary = $this->edit->getSummary();
		if ( $sectionTitle !== '' ) {
			$sectionanchor = $parser->guessLegacySectionNameFromWikiText( $sectionTitle );
			// If no edit summary was specified, create one automatically from the section
			// title and have it link to the new section. Otherwise, respect the summary as
			// passed.
			if ( $summary === '' ) {
				$cleanSectionTitle = $parser->stripSectionName( $sectionTitle );
				return wfMessage( 'newsectionsummary' )->title( $this->title )
					->rawParams( $cleanSectionTitle )->inContentLanguage()->text();
			}
		} elseif ( $summary !== '' ) {
			$sectionanchor = $parser->guessLegacySectionNameFromWikiText( $summary );
			# This is a new section, so create a link to the new section
			# in the revision summary.
			$cleanSummary = $parser->stripSectionName( $summary );
			return wfMessage( 'newsectionsummary' )->title( $this->title )
	...
	private function getStartingRevision() {
		$db = wfGetDB( DB_MASTER );
		return $this->edit->getLastRevisionId() !== null
			? Revision::newFromId( $this->edit->getLastRevisionId(), Revision::READ_LATEST )
			: Revision::loadFromTimestamp( $db, $this->title, $this->edit->getLastRevisionTime() );
	}

	private function runPreAttemptHooks( $info, $options, Status $status ) {
		$hookArgs = [ $this->page, $this->user, $this->edit, $info, $options, $status ];
	...
	private function runPreMergeHooks( $info, $options, Status $status ) {
		if ( !Hooks::run(
			'EditAttemptEditFilter',
			[ $this->page, $this->user, $this->edit, $info, $options, $status ] )
		) {
			if ( $status->isOK() ) {
	 ...
	 * @return bool
	 */
	private function runPostMergeFilters( Content $content, $info, $options, Status $status ) {
		if ( !Hooks::run( 'EditAttemptFilterMergedContent',
			[ $this->page, $this->user, $this->edit, $content, $info, $options, $status ] )
		) {
			// Error messages
			if ( $status->isGood() ) {
