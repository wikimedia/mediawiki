...
class EditAttempt {
	 ...
	 */
	final public function internalAttemptSave( array $info, array $options ) {
		...
		if ( $new ) {
			...
			if ( $this->edit->getSection() === 'new' ) {
				...
				&& !$content->isRedirect()
				&& md5( $this->edit->getSummary() ) === $info['autoSumm']
			) {
				$status->fatal( 'missingsummary' );
				$status->value = self::AS_SUMMARY_NEEDED;
				return $status;
	...
	private function runPreMergeHooks( Status $status ) {
		if ( !Hooks::run(
			...
			[ $this->page, $this->user, $this->edit, $status ] )
		) {
			if ( $status->isOK() ) {
	...
	private function runPostMergeFilters( Content $content, Status $status ) {
		if ( !Hooks::run( 'EditAttemptFilterMergedContent',
			[ $this->page, $this->user, $this->edit, $content, $status ] )
		) {
			// Error messages
			if ( $status->isGood() ) {
