...
class BasicEditor extends Editor {
	...
	/** @var string Requested form type (save, diff, preview, initial) */
	...
	final public function edit() {
		...
		if ( wfReadOnly() && $this->requestType === 'save' ) {
			$this->handleReadOnly();
		}
		...
		if ( $revision
			...
			$prevRev = null;
			$undidRevId = $this->controller->getUndidRevId();
			if ( $undidRevId ) {
		...
		$hookArgs = [ $this->page, $this->user, $this->context, $this->data ];
		...
		if ( $this->requestType === 'initial' ) {
			...
			$this->view->setMinorEdit( $this->controller->getMinorEdit() );
			$this->view->setWatchThis( $this->controller->getWatchThis() );
		}
		...
		if ( !$this->isConflict &&
			...
			// We use $this->data->section too much before this and getVal('wgSection') directly
			// in other places at this point we can't reset $this->data->section to '' to fallback
		...
		$lastDelete = null;
		$wasDeleted = $this->dataWrapper->wasDeletedWhileEditing( $lastDelete );
		...
		if ( $this->isConflict ) {
			...
			$content = $this->controller->getCurrentContent();
	...
	protected function importRequestData( $request ) {
		$this->requestType = $this->controller->getRequestType();
	...
	public function showHeader( $revision, $isUnicodeCompliant ) {
		...
		if ( $this->isConflict ) {
			...
			if ( $this->data->section !== '' && $this->data->section !== 'new' ) {
				if ( !$this->data->summary && $this->formType !== 'preview' &&
					!$this->formType !== 'diff'
				) {
					// FIXME: use Content object
					$sectionTitle = EditUtilities::extractSectionTitle( $this->data->textbox1 );
	...
	protected function addHiddenInputFields() {
		...
		if ( $this->missingSummary || ( $this->data->section === 'new' && $noSummary ) ) {
