<?php
/**
 * Basic user interface elements for page editing.
 *
 ...
 *
 * It provides a text box where the text can be edited, a summary inpu, various notices,
 * checkboxes and an edit button.
 */
class BasicEditView extends EditView {
	...
	/**
	 * To prevent showing some UI elements
	 */
	...
	public function setScrollTop( $val ) {
		$this->scrollTop = $val;
	}

	public function setMinorEdit( $val ) {
		$this->minorEdit = $val;
	}

	public function setWatchThis( $val ) {
		$this->watchThis = $val;
	}

	public function suppressEditIntro() {
		$this->suppressIntro = true;
	}

	public function hideSummaryField() {
		$this->showSummaryField = false;
	}
	...
	public function showBeforeEditForm( $isUnicodeCompliant, $noSummary ) {
		...
		if ( $this->edit->getSection() === 'new' ) {
			$this->showSummaryInput( true, $this->edit->getSummary(), $noSummary );
		}
	...
	public function setHeaders( $options ) {
		...
		if ( $this->isConflict() ) {
			$msg = 'editconflict';
		} elseif ( $this->contextTitle->exists() && $this->edit->getSection() !== '' ) {
			$msg = $this->edit->getSection() === 'new' ? 'editingcomment' : 'editingsection';
		} else {
	...
	protected function addEditNotices() {
		$editNotices = $this->title->getEditNotices( $this->oldid );
	...
	protected function showFormBeforeText( $isUnicodeCompliant ) {
		$section = htmlspecialchars( $this->edit->getSection() );
		$this->output->addHTML( <<<HTML
<input type='hidden' value="{$section}" name="wpSection"/>
<input type='hidden' value="{$this->edit->getStartTime()}" name="wpStarttime" />
<input type='hidden' value="{$this->edit->getLastRevisionTime()}" name="wpEdittime" />
<input type='hidden' value="{$this->edit->getLastRevisionId()}" name="editRevId" />
...
HTML
	 ...
	 */
	public function showMainTextbox( $displayNone, $isUnicodeCompliant, $isOldRev, $font,
		$customAttribs = null
	) {
		if ( $displayNone ) {
	...
	protected function showStandardInputs( &$tabindex = 2 ) {
		...
		if ( $this->edit->getSection() !== 'new' ) {
			$this->showSummaryInput( false, $this->edit->getSummary() );
		}
	...
	public function getCheckboxes( &$tabindex, $checked ) {
		...
		if ( $this->title->exists() && $this->edit->getSection() !== 'new' ) {
			$checkboxes['minor'] = '';
	...
	public function noSuchSectionPage() {
		...
		$res = $this->context->msg( 'nosuchsectiontext', $this->edit->getSection() )->parseAsBlock();
