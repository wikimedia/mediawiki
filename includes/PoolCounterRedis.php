 ...
 *   - poolcounter:z-lastevent-* : A sorted set of (slot ID, UNIX timestamp as score)
 *                                 used for tracking the last time a slot was created,
 *                                 locked, unlocked, or automatically released (expired)
 ...
 * re-initialized as needed. The "z-lastevent" key is used for detecting processes acquired
...
class PoolCounterRedis extends PoolCounter {
	...
	function release() {
		...
		static $script =
<<<LUA
		local kSlots,kSlotsLastEvent,kWakeup,kWaiting = unpack(KEYS)
		...
		-- Treat the list as expired if the "last event" time sorted-set is missing.
		if rSlot ~= 'w' and redis.call('exists',kSlotsLastEvent) == 1 then
			if 1*redis.call('zScore',kSlotsLastEvent,rSlot) ~= 1*rSlotTime then
			...
			elseif redis.call('lLen',kSlots) >= (1*rMaxWorkers - 1) then
				...
				redis.call('del',kSlots,kSlotsLastEvent)
			else
				-- Add slot back to pool and update the "last event" time
				redis.call('rPush',kSlots,rSlot)
				redis.call('zAdd',kSlotsLastEvent,rTime,rSlot)
				-- Always keep renewing the expiry on use
				redis.call('expireAt',kSlots,math.ceil(rTime + rExpiry))
				redis.call('expireAt',kSlotsLastEvent,math.ceil(rTime + rExpiry))
			end
		end
...
LUA;
	...
	protected function waitForSlotOrNotif( $doWakeup ) {
		...
		try {
			...
			if ( ctype_digit( $slot ) ) {
				...
				$this->slotTime = microtime( true ); // last microtime() was a few RTTs ago
				// Unregister this process as waiting and bump slot "last event" time
	...
	protected function initAndPopPoolSlotList( RedisConnRef $conn, $now ) {
		static $script =
<<<LUA
		local kSlots,kSlotsLastEvent,kSlotWaits = unpack(KEYS)
		local rMaxWorkers,rMaxQueue,rTimeout,rExpiry,rSess,rTime = unpack(ARGV)
		-- Initialize if the "last event" time sorted-set is empty. The slot key
		...
		if redis.call('exists',kSlotsLastEvent) == 0 then
			redis.call('del',kSlots)
			for i = 1,1*rMaxWorkers do
				redis.call('rPush',kSlots,i)
				redis.call('zAdd',kSlotsLastEvent,rTime,i)
			end
		-- Otherwise do maintenance to clean up after network partitions
			...
			local staleLocks = redis.call('zRangeByScore',kSlotsLastEvent,0,rTime - rExpiry)
			for k,slot in ipairs(staleLocks) do
				redis.call('lRem',kSlots,0,slot)
				redis.call('rPush',kSlots,slot)
				redis.call('zAdd',kSlotsLastEvent,rTime,slot)
			end
		...
		if redis.call('lLen',kSlots) > 0 then
			...
			-- Update the slot "last event" time
			redis.call('zAdd',kSlotsLastEvent,rTime,slot)
...
LUA;
	 ...
	 * @return string
	 */
	protected function getSlotLETimeSetKey() {
		return 'poolcounter:z-lastevent-' . sha1( $this->key ) . "-{$this->workers}";
	}
