 ...
 *                                 released. This is -1 for each created, released (expired),
 *                                 and unlocked slot and is set to a future expiry when locked.
...
<<<LUA
		...
		if rSlot ~= 'w' and redis.call('exists',kSlotsNextRelease) == 1 then
			...
			elseif redis.call('lLen',kSlots) >= (1*rMaxWorkers - 1) then
				...
				redis.call('zAdd',kSlotsNextRelease,-1,rSlot)
...
<<<LUA
		...
		-- Otherwise do maintenance to clean up after network partitions
			...
			for k,slot in ipairs(staleLocks) do
				...
				redis.call('zAdd',kSlotsNextRelease,-1,slot)
			end
...
LUA;
	...
	protected function getSlotRTimeSetKey() {
		return 'poolcounter:z-renewtime-' . sha1( $this->key ) . "-{$this->workers}";
	}
