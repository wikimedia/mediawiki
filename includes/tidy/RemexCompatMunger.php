...
class RemexCompatMunger implements TreeHandler {
	...
	private function getParentForInsert( $preposition, $refElement, $isBlank ) {
		if ( $preposition === TreeBuilder::ROOT ) {
			...
			if ( $refData->currentCloneElement ) {
				...
				&& ( !$isBlank || $refData->childPElement->userData->snData->nonblankNodeCount )
				// close disabled p-wrapper at next prospective sibling
				&& !$refData->childPElement->userData->snData->isDisabled
			) {
	...
	public function characters( $preposition, $refElement, $text, $start, $length,
		...
		if ( $preposition === TreeBuilder::UNDER && $parentData->isSplittable
			&& ( !$parentData->ancestorPNode
				|| (
					$parentData->ancestorPNode->snData->isDisabled
					&& !$parentData->isInBlock
				)
			)
		) {
	...
	public function insertElement( $preposition, $refElement, Element $element, $void,
		...
		if ( !$inline || $parentData->isInBlock ) {
			$elementData->isInBlock = true;
		}
	}
	...
	private function splitTagStack( SerializerNode $parentNode, $inline, $pos ) {
		...
		for ( $i = count( $nodes ) - 2; $i >= 0; $i-- ) {
			...
			$nodeParent->snData->hasText = true;
