...
class Balancer {
	...
	/**
	 * Create a new Balancer which accepts only those HTML tag names
	 * included in $htmlelements.
	 * @param array $htmlelements An associative array of acceptable HTML
	 *   tag names, where the keys of the array give the valid tag names.
	 *   If null, then no sanitization is done.
	 * @param bool $strict When true, enforces syntactic constraints on
	 *   input: all non-tag < must be escaped, all attributes must be
	 *   separated by a single space and double-quoted.  This is consistent
	 *   with the output of the Sanitizer.  Defaults to false.
	 */
	public function __construct( $strict = false, $htmlelements = null ) {
