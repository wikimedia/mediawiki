...
class InterwikiLookupAdapter implements InterwikiLookup {
	...
	/**
	 * @var Interwiki[] associative array mapping interwiki prefixes to Interwiki objects
	 */
	...
	public function invalidateCache( $prefix ) {
		...
		$interwikis = $this->siteInterwikis( $site );
		$this->interwikiMap = array_push( $this->interwikiMap, $interwikis[$prefix] );
	}
	...
	private function loadInterwikiMap() {
		...
		foreach ( $globals as $globalId ) {
			$site = $siteList->getSite( $globalId );
			$interwikis = $this->siteInterwikis( $site );
	...
	/**
	 * Get private interwikiMap private attribute, load if needed.
	 *
	 * @return Interwiki[]
	 */
	protected function getInterwikiMap() {
	 ...
	 * @return Interwiki[]
	 */
	private function siteInterwikis( MediawikiSite $site ) {
		$interwikis = [];
