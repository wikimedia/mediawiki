 ...
 *
 * This implements two levels of caching (in-process array and a BagOStuff from ObjectCache)
 * and two storage backends (SQL and CDB).
 *
...
class ClassicInterwikiLookup implements InterwikiLookup {
	...
	/**
	 * @var int 0 means unlimited, any other value is max number of entries.
	 */
	private $localCacheLimit = 100;
	...
	public function fetch( $prefix ) {
		...
		if ( isset( $this->localCache[$prefix] ) ) {
			return $this->localCache[$prefix];
		}
		...
		if ( $this->localCacheLimit && count( $this->localCache ) >= $this->localCacheLimit ) {
			reset( $this->localCache );
			unset( $this->localCache[key( $this->localCache )] );
		}

		$this->localCache[$prefix] = $iw;
	...
	private function getCacheValue( $key ) {
		if ( $this->cdb === null ) {
			$this->cdb = is_array( $this->cdbFile ) ? false : CdbReader::open( $this->cdbFile );
		}
		if ( $this->cdb ) {
			return $this->cdb->get( $key );
		} else {
			return isset( $this->localCache[$key] ) ? $this->localCache[$key] : false;
		}
	}
	...
	private function getAllPrefixesCached( $local ) {
		...
		try {
			if ( !$this->cdb ) {
				$this->cdb = CdbReader::open( $this->cdbFile );
			}
			/* Resolve site name */
			if ( $this->interwikiScopes >= 3 && !$this->thisSite ) {
				$site = $this->cdb->get( '__sites:' . wfWikiID() );
				if ( $site == '' ) {
					$this->thisSite = $this->fallbackSite;
				}
			}
			...
			foreach ( $sources as $source ) {
				$list = $this->cdb->get( "__list:{$source}" );
				foreach ( explode( ' ', $list ) as $iw_prefix ) {
					$row = $this->cdb->get( "{$source}:{$iw_prefix}" );
					if ( !$row ) {
						continue;
					}
