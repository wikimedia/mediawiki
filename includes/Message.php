...
class Message {
	 ...
	 * the content being evaluated.  Plaintext parameters are still valid
	...
	protected function formatPlaintext( $plaintext ) {
		...
		case 'parse':
		case 'block-parse':
			// Inject parameter before parsing so it can be used
			// as an argument to parser functions
			$when = 'before';

			// Escape any wikitext within the plaintext
			$escaped = wfEscapeWikiText( $plaintext );

			// replace $ in strings that look like Message arguments
			// parameter.
			$content = preg_replace( '/\$(\d+)/', '&dollar;\1', $escaped );
			break;

		case 'escaped':
			$content = htmlspecialchars( $plaintext, ENT_QUOTES, 'UTF-8', true );
			break;
