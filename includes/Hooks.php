...
class Hooks {
	 ...
	 * @param array $options [optional] (see self::run() for details)
	...
	private static function callHook( $event, $hook, array $args, $options = [],
		...
		// Handle back-compat of $options being a string
		$options = is_string( $options ) ? [ 'deprecated' => $options ] : $options;
		...
		if ( $checkArguments ) {
			...
			foreach ( $options['deprecatedPassByRef'] as $index => $version ) {
				...
				if ( $param->isPassedByReference() ) {
					wfDeprecated(
						"Passing argument #$index by reference (used in $fname) is deprecated",
						$version
					);
				}
			}
