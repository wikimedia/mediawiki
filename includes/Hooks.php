...
class Hooks {
	 ...
	 * @param int $flags [optional] Whether the hook is abortable.
	 *  Use Hooks::ABORTABLE or Hooks::NO_ABORT. Default: Hooks::ABORTABLE.
	...
	public static function run( $event, array $args = [], $deprecatedVersion = null, $flags = 0 ) {
		...
		foreach ( self::getHandlers( $event ) as $hook ) {
			...
			if ( $flags === self::NO_ABORT && $retval !== null && $retval !== true ) {
				wfLogWarning( "Invalid return from $func for unabortable hook $event." );
			} else {
