...
class Hooks {
	 ...
	 * @param string|null $deprecated [optional]
	 * @param string &$fname [optional] Readable name of hook [returned]
	 * @return null|string|bool
	 */
	private static function callHook( $event, $hook, array $args, $deprecated = null, &$fname = null ) {
		...
		if ( $deprecated !== null ) {
			wfDeprecated( "$event hook (used in $fname)", $deprecated );
		}
	 ...
	 * @throws Exception If a callback returns an abort value.
	 * @since 1.30
	 */
	public static function runWithoutAbort( $event, array $args = [], $deprecatedVersion = null ) {
		foreach ( self::getHandlers( $event ) as $hook ) {
			...
			if ( $retval !== null && $retval !== true ) {
				throw new Exception( "Invalid return from $fname for unabortable hook $event." );
			}
		}
		return true;
	}
