...
abstract class DataModel {
	...
	*/ );
	...
	public static function getList( $name, $shard = null, $offset = 0, $sort = 'ASC' ) {
		 ...
		 * Figure out in advance what server all shard-values live on, so that we can
		 * group the queries for items on the same servers.
		 */
		$partitionIndexes = array();
		foreach ( $missing as $shard => $ids ) {
			$index = $store->getPartitionIndex( $shard );

			if ( array_key_exists( $index, $partitionIndexes ) ) {
				$partitionIndexes[$index] = array_merge( $partitionIndexes[$index], $ids );
			} else {
				$partitionIndexes[$index] = $ids;
			}
		}
		 ...
		 */
		foreach ( $partitionIndexes as $index => $ids ) {
			$partition = $store->getPartitionByIndex( static::getTable(), static::getShardColumn(), $index );
	...
	protected function updateLists( DataModel $old = null ) {
		...
		foreach ( static::$lists as $list => $properties ) {
			...
			$partition = $store->getPartition( 'lists', 'list', $key );
	...
	protected static function getListFromDB( $name, $shard, $offset, $limit, $sort ) {
		...
		$storeGroup = RDBStoreGroup::singleton();
		$store = $storeGroup->getForTable( 'lists' );
		$partition = $store->getPartition( 'lists', 'list', $name );
