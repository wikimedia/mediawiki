...
class HookedLangLinkLoader implements LangLinkLoader {
	...
	public function getEffectiveLinksFor(
		...
		$originalLinkStrings = $linkStrings;

		wfRunHooks( $this->hook, array( $title, &$linkStrings ) );
		...
		// extra links are the effective links minus the original links
		$extraLinkStrings = array_diff( $linkStrings, $originalLinkStrings );

		$effectiveLinks = $this->stringsToRows( $pageId, $linkStrings );

		// get the languages of the extra links...
		$extraLanguages = $this->extractLanguages( $links );

		// ...and flag the links with these languages
		$effectiveLinks = $this->flagLinks( $effectiveLinks, 'extra', $extraLanguages );
