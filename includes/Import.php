...
class WikiImporter {
	 ...
	 */
	private function pageOutCallback( $title, $foreignTitle, $revCount, $sucCount, $pageInfo ) {
	...
	private function processTitle( $text, $ns = null ) {
		...
		$foreignTitle = ForeignTitle::newFromXmlData( $text, $ns, $this->foreignNamespaces );

		if ( !is_null( $this->mTargetRootPage ) ) {
			# We don't need to use ForeignTitle here, as the target root page will
			# give us the namespace component
			$title = Title::newFromText( $this->mTargetRootPage . '/' . $text );
		} else {
			$title = $foreignTitle->toLocalTitle( $this->mTargetNamespace );
		}
