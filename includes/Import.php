...
class WikiImporter {
	...
	private function processTitle( $text, $ns = null ) {
		...
		if ( !is_null( $this->mTargetRootPage ) ) {
			...
			$title = Title::newFromText( $this->mTargetRootPage . '/' . $workTitle );
		} else {
			$targetNs = $this->mTargetNamespace ?: $foreignTitle->getNamespaceId();
			if ( MWNamespace::exists( $targetNs ) ) {
				# We have a matching namespace ID locally, so put the page there
				$title = Title::makeTitleSafe( $targetNs, $foreignTitle->getPageName() );
			} else {
				...
				if ( $targetNs !== false ) {
					...
					$title = Title::makeTitleSafe( 0, $foreignTitle->getFullText() );
				}
			}
		}
