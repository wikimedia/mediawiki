...
class WikiImporter {
	...
	public function setTargetNamespace( $namespace ) {
		if ( is_null( $namespace ) ) {
			...
			// ...but don't overwrite the root page factory
			if ( $this->importTitleFactory instanceof NamespaceImportTitleFactory ) {
				$this->importTitleFactory = new NaiveImportTitleFactory();
			}
			return true;
		} elseif (
			...
			$this->importTitleFactory = new NamespaceImportTitleFactory( $namespace );
			return true;
		} else {
			return false;
		}
	...
	public function setTargetRootPage( $rootpage ) {
		...
		if ( is_null( $rootpage ) ) {
			...
			// ...but don't overwrite the namespace factory
			if ( $this->importTitleFactory instanceof SubpageImportTitleFactory ) {
				$this->importTitleFactory = new NaiveImportTitleFactory();
			}
		} elseif ( $rootpage !== '' ) {
			...
			if ( !$title || $title->isExternal() ) {
				...
				if ( !MWNamespace::hasSubpages( $title->getNamespace() ) ) {
					...
					$this->importTitleFactory = new SubpageImportTitleFactory( $title );
				}
			}
		}
		return $status;
	...
	private function processTitle( $text, $ns = null ) {
		...
		if ( is_null( $title ) ) {
			...
			$this->notice( 'import-error-invalid', $this->importTitleFactory->
				getTitleAsString( $foreignTitle ) );
			return false;
