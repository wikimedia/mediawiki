...
class WikiImporter {
	...
	private function processTitle( $text, $ns = null ) {
		$foreignTitleFactory = new XmlDumpForeignTitleFactory( $this->foreignNamespaces );
		$foreignTitle = $foreignTitleFactory->createForeignTitle( $text, $ns );

		if ( !is_null( $this->mTargetRootPage ) ) {
			# We don't need to use the ForeignTitle here, as the target root page
			# will give us the namespace component
			$titleText = $this->mTargetRootPage . '/' . $text;
			$title = Title::newFromText( $this->mTargetRootPage . '/' . $text );
		} else {
			$titleText = $text;
			$title = Title::newFromForeignTitle( $foreignTitle,
				$this->mTargetNamespace );
		}
