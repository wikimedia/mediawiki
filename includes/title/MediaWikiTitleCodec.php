...
class MediaWikiTitleCodec implements TitleFormatter, TitleParser {
	 ...
	 * @var string
	 */
	protected $localInterwiki;

	/**
	 * @param Language $language the language object to use for localizing namespace names.
	 * @param GenderCache $genderCache the gender cache for generating gendered namespace names
	 * @param bool $localInterwiki
	 */
	public function __construct( Language $language, GenderCache $genderCache, $localInterwiki = false ) {
	...
	public function parseTitle( $text, $defaultNamespace ) {
		// NOTE: this is an ugly clutch that allows this class to share the
	...
	public function splitTitleString( $text, $defaultNamespace = NS_MAIN ) {
		...
		do {
			...
			if ( preg_match( $prefixRegexp, $dbkey, $m ) ) {
				...
				if ( ( $ns = $this->language->getNsIndex( $p ) ) !== false ) {
					...
					if ( $this->localInterwiki !== false
						&& 0 == strcasecmp( $parts['interwiki'], $this->localInterwiki )
					) {
						if ( $dbkey == '' ) {
		...
		if ( $dbkey == '' && $parts['interwiki'] === '' ) {
			if ( $parts['namespace'] != NS_MAIN || $parts['fragment'] === '' ) {
				throw new MalformedTitleException( 'Empty title: ' . $text );
			}
		}
