...
/**
 * Represents a page (or page section) title within %MediaWiki.
 *
 ...
 *
 * @note TitleValue represents the title of a local page (or section or a page).
...
class TitleValue {
	...
	/**
	 * @todo should this be called "fraction" instead?
	 * @var string
	 */
	protected $section;
	 ...
	 *
	 * @note: No validation or normalization is applied. Use a TitleFormatter for that.
	 *        If the title is not known to be in a canonical form such as DBKEY_FORM
	 *        or TITLE_FORM, use UNKNOWN_FORM to indicate that.
	 *
	...
	public function __construct( $form, $namespace, $text, $section = '' ) {
		...
		if ( $text === '' && $section === '' ) {
			throw new InvalidArgumentException( '$text must not be empty if $section is empty' );
		}
	...
	/**
	 * @todo consider renaming this to getFraction().
	 * @return string
	 */
