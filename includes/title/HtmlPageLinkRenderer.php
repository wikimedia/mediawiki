...
class HtmlPageLinkRenderer implements PageLinkRenderer {
	...
	/**
	 * @note $formatter and $baseUrl are currently not used, since we still rely on
	 * the Linker class to generate the actual HTML. Once this is reversed so that
	 * Linker becomes a legacy interface to HtmlPageLinkRenderer, we will be using them,
	 * so it seems prudent to already declare the dependency and inject them.
	 *
	...
	public function renderLink( TitleValue $page, $text ) {
		...
		$link = Linker::linkKnown( Title::newFromTitleValue( $page ), htmlspecialchars( $text ) );
		return $link;
	}
}