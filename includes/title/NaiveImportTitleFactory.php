 ...
 * a matching namespace name. If that can't be foudn, we dump the page in the
...
class NaiveImportTitleFactory implements ImportTitleFactory {
	...
	/**
	 * Gets a string containing the page title that would be created by
	 * createTitleFromForeignTitle. This can be used for error messages in the
	 * case that createTitleFromForeignTitle returns null.
	 *
	 * @param ForeignTitle $foreignTitle The ForeignTitle to convert
	 * @return string
	 */
	public function getTitleAsString( ForeignTitle $foreignTitle ) {
		global $wgContLang;

		if ( $foreignTitle->isNamespaceIdKnown() ) {
			$foreignNs = $foreignTitle->getNamespaceId();
			if ( $foreignNs < 100 && MWNamespace::exists( $foreignNs ) ) {
				return $wgContLang->getNsText( $foreignNs ) . ':' .
					$foreignTitle->getText();
			}
		}

		return $foreignTitle->getFullText();
	}
}
