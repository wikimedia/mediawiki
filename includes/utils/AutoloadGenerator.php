...
class AutoloadGenerator {
	...
	/**
	 * @var array List of FQCN detected so far
	 */
	protected $classes = array();

	/**
	 * @var string sprintf format to use for each line of output.
	 */
	protected $variableNae = 'wgAutoloadClasses';
	 ...
	 * @param string $path Full path to the file containing this class
	 */
	public function forceClassPath( $fqcn, $path ) {
		$path = realpath( $path );
		if ( !$path ) {
			throw new \MWException( "Invalid path: $path" );
		}
		$len = strlen( $this->basepath );
		if ( substr( $path, 0, $len ) !== $this->basepath ) {
			throw new \MWException( "Path is not within basepath: $path" );
		}
	...
	/**
	 * @var string $path Path to a php file to find classes within
	 */
	 ...
	 *  for php files either either .php or .inc extensions
	 */
...
/**
 * Reads a php file and returns the FQCN of every class defined within it.
 */
class ClassCollector {
	...
	/**
	 * @var array List of tokens from token_get_all()
