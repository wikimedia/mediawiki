...
EOD;
	...
	public function initMediaWikiDefault() {
		...
		// Blobs generated by MediaWiki < 1.5 on PHP 4 were serialized with the class name
		// coerced to lowercase. For efficiency's sake, we recognize the lowercase form.
		foreach( [
			'ConcatenatedGzipHistoryBlob',
			'HistoryBlobCurStub',
			'HistoryBlobStub'
		] as $legacyBlobClass ) {
			$this->forceClassPath( strtolower( $legacyBlobClass ),
				$this->basepath . '/includes/HistoryBlob.php' );
		}
	}
}
