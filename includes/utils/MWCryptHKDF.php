...
class MWCryptHKDF {
	 ...
	 */
	public function __construct( $algorithm, $cache, $context ) {
		global $wgHKDFSecret, $wgSecretKey;
	...
	protected function setupPRK( $salt ) {
		global $wgHKDFSecret;
		if ( $this->prk === '' ) {
			$sourceKeyMaterial = hash( $this->algorithm, $wgHKDFSecret, true );
	...
	protected static function singleton( $context ) {
		...
		if ( is_null( self::$singleton ) ) {
			self::$singleton = new self( $wgHKDFAlgorithm, wfGetMainCache(), $context );
		}
