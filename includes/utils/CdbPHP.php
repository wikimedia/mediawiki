...
class CdbWriterPHP extends CdbWriter {
	...
	public function __construct( $fileName ) {
		...
		if ( fseek( $this->handle, $this->pos ) == -1 ) {
			throw new CdbException( 'fseek failed in file "' . $this->tmpFileName . '".' );
		}
	}
	...
	protected function write( $buf ) {
		...
		if ( $len !== strlen( $buf ) ) {
			throw new CdbException( 'Error writing to CDB file "' . $this->tmpFileName . '".' );
		}
	}
	...
	protected function addbegin( $keylen, $datalen ) {
		if ( $keylen > 0x7fffffff ) {
			throw new CdbException( 'Key length too long in file "' . $this->tmpFileName . '".' );
		}
		if ( $datalen > 0x7fffffff ) {
			throw new CdbException( 'Data length too long in file "' . $this->tmpFileName . '".' );
		}
	...
	protected function finish() {
		...
		if ( ftell( $this->handle ) != 0 ) {
			throw new CdbException( 'Error rewinding to start of file "' . $this->tmpFileName . '".' );
		}
		$this->write( $final );
	}
