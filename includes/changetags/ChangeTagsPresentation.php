...
class ChangeTagsPresentation {
	 ...
	 * The output here is not safe HTML.
	 * The output is (intended as) plain text that should be treated as any other plain text
	 * when being output in HTML.
	 * @since 1.29
	 */
	...
	public function buildTagFilterSelector( $selected = '', $formDescriptor = false ) {
		...
		$usedTags = $this->getUsedTags( $selected );
		if ( !$usedTags ) {
			return [];
		}
	...
	/**
	 * Tags in use on the wiki, checking the cache
	 * @param string $selected A tag to return even if not in cache
	 * @return array
	 */
	private function getUsedTags( $selected ) {
		if ( !$this->context->getConfig()->get( 'UseTagFilter' ) ) {
			return [];
		}
		$tagStats = ChangeTags::secondaryTagUsageStatistics();
		if ( !isset( $tagStats[$selected] ) ) {
			$tagStats[$selected] = 0;
		}
		return array_keys( $tagStats );
	}
