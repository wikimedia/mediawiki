...
class ChangeTagsUpdater {
	 ...
	 * @since 1.28
	 */
	public function canAddTagsAccompanyingChange( array $tagsToAdd ) {

		if ( !is_null( $this->performer ) ) {
			if ( !$this->performer->isAllowed( 'applychangetags' ) ) {
				return Status::newFatal( 'tags-apply-no-permission' );
			} elseif ( $this->performer->isBlocked() ) {
				return Status::newFatal( 'tags-apply-blocked', $this->performer->getName() );
			}
		}
	...
	public function canUpdateTags( array $tagsToAdd, array $tagsToRemove ) {
		...
		if ( !is_null( $this->performer ) ) {
			if ( !$this->performer->isAllowed( 'changetags' ) ) {
				...
				return Status::newFatal( 'tags-update-blocked', $this->performer->getName() );
			}
		}
	 ...
	 *
	 * Includes a call to self::canUpdateTags(), so your code doesn't need
	...
	public function updateTagsWithChecks( $tagsToAdd, $tagsToRemove,
		...
		list( $tagsAdded, $tagsRemoved, $initialTags ) = self::updateTags( $tagsToAdd,
			$tagsToRemove, $rc_id, $rev_id, $log_id, $params, null, $this->performer );
	...
	public static function updateTags( $tagsToAdd, $tagsToRemove, &$rc_id = null,
		&$rev_id = null, &$log_id = null, $params = nulll, RecentChange $rc = null,
		User $user = null
	) {
		...
		if ( count( $tagsToAdd ) ) {
			foreach ( $tagsToAdd as $tag ) {
				...
				if ( $dbw->affectedRows() && $wgUseChangeTagStatisticsTable > 0 ) {
					// increment tag hitcount
					$dbw->upsert(
						'change_tag_statistics',
						[ 'cts_tag' => $tag, 'cts_count' => 1 ],
						[ 'cts_tag' ],
	...
	public static function deleteTagEverywhere( $tag ) {
		...
		// purge all caches
		ChangeTagsCache::purgeTagCacheAll();
