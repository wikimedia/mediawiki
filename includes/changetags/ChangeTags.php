...
class ChangeTags {
	...
	/**
	* Represents a tag applied by an extension
	*/
	const UPDATE_EXTENSION = 1;

	/**
	* Represents a tag applied manually or in core along an action
	* by the user who performed it
	*/
	const UPDATE_USER_SELF = 2;

	/**
	* Represents a tag manually applied on a saved edit or logged action
	*/
	...
	public static function tagDescription( $tag ) {
		...
		if ( !$msg->exists() ) {
			...
			return self::tagAppearance( $tag, true );
		}
		if ( $msg->isDisabled() ) {
	 ...
	 * @since 1.27
	 */
	public static function tagAppearance( $tag, $notEmpty = false ) {
	...
	public static function addTags( $tags, $rc_id = null, $rev_id = null,
		$log_id = null, $params = null, User $user = null, RecentChange $rc = null,
		$flags = self::UPDATE_EXTENSION
	) {
	...
	public static function updateTags( $tagsToAdd, $tagsToRemove, &$rc_id = null,
		&$rev_id = null, &$log_id = null, $params = null, User $user = null,
		RecentChange $rc = null, $flags = self::UPDATE_EXTENSION ) {
		...
		Hooks::run( 'ChangeTagsAfterUpdateTags', array( $tagsToAdd, $tagsToRemove, $prevTags,
			$rc_id, $rev_id, $log_id, $params, $user, $rc, $flags ) );
	...
	public static function canAddTagsAccompanyingChange( array $tags,
		...
		$allowedTags = $changeTagsContext->getStored();
	...
	public static function addTagsAccompanyingChangeWithChecks(
		...
		// do it!
		self::addTags( $tags, $rc_id, $rev_id, $log_id, $params, $user, null,
			self::UPDATE_USER_SELF );
	...
	public static function canUpdateTags( array $tagsToAdd, array $tagsToRemove,
		User $user = null, ChangeTagsContext $changeTagsContext = null ) {
		...
		if ( $tagsToRemove ) {
			...
			$unremovableTags = array_merge( $changeTagsContext->getRegistered(),
	...
	public static function updateTagsWithChecks( $tagsToAdd, $tagsToRemove,
		...
		ChangeTagsContext $changeTagsContext ) {
		...
		// are we allowed to do this?
		$result = self::canUpdateTags( $tagsToAdd, $tagsToRemove, $user, $changeTagsContext );
		if ( !$result->isOK() ) {
			$result->value = null;
			return $result;
		}
		...
		list( $tagsAdded, $tagsRemoved, $initialTags ) = self::updateTags( $tagsToAdd,
			$tagsToRemove, $rc_id, $rev_id, $log_id, $params, $user, null,
			self::UPDATE_USER_POST );
	...
	public static function activateTagWithChecks( $tag, $reason, User $user,
		$ignoreWarnings = false, ChangeTagsContext $changeTagsContext = null ) {
	...
	/**
	 * @deprecated since 1.27, use ChangeTagsContext class instead
	 * @since 1.25
	 */
	public static function listExtensionActivatedTags() {
		...
		$changeTagsContext = new ChangeTagsContext();
		$tags = array();
		$registered = $changeTagsContext->getRegistered();
	...
	public static function listDefinedTags() {
		wfDeprecated( __METHOD__, '1.27' );
		$changeTagsContext = new ChangeTagsContext();
		return array_keys( $changeTagsContext->getDefined() );
	}
	...
	public static function listExtensionDefinedTags() {
		wfDeprecated( __METHOD__, '1.27' );
		$changeTagsContext = new ChangeTagsContext();
		return array_keys( $changeTagsContext->getRegistered() );
	}

	/**
	 * @deprecated since 1.27, use ChangeTagsContext::purgeTagCacheAll() instead
	 * @since 1.25
	 */
	...
	/**
	 * @deprecated since 1.27, use ChangeTagsContext::purgeTagUsageCache() instead
	 * @since 1.25
	 */
	...
	/**
	 * @deprecated since 1.27, use ChangeTagsContext::getStats() instead
	 */
