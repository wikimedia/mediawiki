...
class ChangeTagsContext {
	...
	public static function storedTags() {
		...
		$cacheDuration = $config->get( 'TagDefinitionCacheDuration' );
		...
		$callBack = function( $oldValue, &$ttl, array &$setOpts ) use ( $fname ) {
			...
			// They are not considered 'problem' by default, unless
			// listed in the 'tags-list-problem' message.
