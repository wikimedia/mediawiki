...
class ChangeTagsContext implements IChangeTagsContextSource {
	...
	public function getSoftwareTags() {
		...
		if ( $this->softwareTags === null ) {
			...
			$this->softwareTags = array_merge( $coreTags, self::fetchRegistered( $this ) );
		}
	...
	/**
	 * Gets cached tag usage statistics
	 * Hitcounts may not be up to date, however creation and deletions
	 * of tags invalidate the cache.
	 *
	 * @return array Array of tags mapped to their hitcount
	 * @since 1.28
	 */
	public function getCachedTagStats() {
		// If accurate tag stats were already retrieved, use them.
		if ( $this->tagStats !== null ) {
			return $this->tagStats;
		}
	 ...
	 * @since 1.27
	 */
	public static function cachedStats( Config $config ) {
