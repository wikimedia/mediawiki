...
class ChangeTagsContext {
	...
	private function fetchRegistered() {
		...
		// extensions might want to define only tags that have already been applied,
		// so fetch stats for callback, which can't use $this, but still have it
		// saved in $this
		$stats = $this->getStats();
		$callBack = function( $oldValue, &$ttl, array &$setOpts ) use ( $stats ) {
			...
			Hooks::run( 'ChangeTagsRegister', [ &$registeredTags, $stats ] );
	...
	public static function cachedStats( Config $config = null ) {
		$cacheDuration = $config->get( 'SecondaryTagUsageCacheDuration' );
	...
	public static function clearCachesAfterUpdate( $tagsToAdd, $tagsToRemove ) {
		...
		if ( $doFullPurge ) {
			...
			return [ 'tag-stats-reactive', 'tag-stats-stable', 'valid-tags-hook' ];
		}
