...
class ChangeTagsContext {
	...
	public static function tagStats() {
		...
		return ObjectCache::getMainWANInstance()->getWithSetCallback(
			...
			$callBack,
			array(
				'checkKeys' => array( $keyReactive, $keyStable ),
	...
	public static function clearCachesAfterUpdate( $tagsToAdd, $tagsToRemove ) {
		...
		// or one of the added tags doesn't appear in it, we purge it
		...
		// In other cases, we purge the reactive cache unless all of the added tags
		// have more hits than $wgTagMaxHitcountUpdate.
		$doBasicPurge = ( $wgTagMaxHitcountUpdate == 0 );
		if ( !$doBasicPurge ) {
			foreach ( $tagsToAdd as $tag ) {
					if ( $stats[$tag] < $wgTagMaxHitcountUpdate ) {
						$doBasicPurge = true;
						break;
					}
			}
