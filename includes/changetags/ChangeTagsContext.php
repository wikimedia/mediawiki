...
class ChangeTagsContext {
	...
	/**
	 * @var bool Indicates whether stored tags were retrieved
	 */
	public $storedTagsAreSet = false;
	...
	/**
	 * @var bool Indicates whether registered tags were retrieved
	 */
	public $registeredTagsAreSet = false;
	...
	/**
	 * @var bool Indicates whether tag stats were retrieved
	 */
	public $tagStatsAreSet = false;
	...
	public function getRegistered() {
		...
		$this->registeredTagsAreSet = true;
	...
	public static function registeredTags() {
		...
		if ( $registeredTags == false ) {
			...
			// When done, we'll be able to increase up to $wgTagUsageCacheDuration.
	...
	public static function tagStats( $upToDateHitcounts = true ) {
		...
		if ( $changeTags == false ) {
			...
			$duration = min( $wgTagUsageCacheDuration, 60*60*24*7 );
	 ...
	 * @since 1.26
	 */
	public static function appliedTags( $activeOnly = false,
		$definedTags = null ) {
		$appliedTags = self::tagStats( false );
		...
		if ( $definedTags == null ) {
			...
			$definedTags = $context->getDefinedTags();
		}

		// filtering out inactive tags (and preserve ordering)
		foreach ( array_keys( $appliedTags ) as $tag ) {
			if ( !isset( $definedTags[$tag]['active'] ) ||
	 ...
	 * Use case 1) alone : activating or deactivating tags
	 * Use case 2) in combination with initializeHitcountCache :
	 * creating tags
	 *
	 * @since 1.26
	 */
