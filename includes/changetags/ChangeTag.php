...
class ChangeTag {
	 ...
	 * @param ChangeTagsContext $context Provides context for the tag :
	 ...
	 * @since 1.26
	 */
	function __construct( $tag, $context = null ) {
		$this->name = $tag;

		// retrieve hitcount
		if ( $context->tagStats != null ) {
			$this->hitcount = isset( $context->tagStats[$this->name] ) ?
				$context->tagStats[$this->name] : 0;
		}

		// retrieve whether tag is stored in valid_tag
		if ( $context->storedTags != null ) {
			$this->isStored = isset( $context->storedTags[$this->name] );
		}

		// retrieve whether tag is registered by an extension,
		// and if it is, params provided by the extension
		if ( $context->registeredTags != null ) {
			$this->isRegistered = isset( $context->registeredTags[$this->name] );
			if ( $this->isRegistered ) {
				$this->extensionParams = $context->registeredTags[$this->name];
			}
		}
	...
	public function getHitcount() {
		if ( $this->hitcount == null ) {
			$stats = ChangeTagsContext::getStats( true );
	 ...
	 * @since 1.26
	 */
	public function extensionDefined() {
	...
	public function getExtensionName() {
		if ( $this->extensionDefined() && isset( $this->extensionParams['extName'] ) ) {
	...
	public function getExtensionDescriptionMessageParams() {
		if ( $this->extensionDefined() && isset( $this->extensionParams['descParams'] ) ) {
	...
	public function canDeactivate( User $user = null ) {
		...
		if ( !$this->userDefined() || ( $this->getHitcount() == 0 &&
			!$this->extensionDefined() ) ) {
	...
	public function canDelete( User $user = null ) {
		...
		if ( $this->extensionDefined() && ( !isset( $this->extensionParams['canDelete'] ) ||
	 ...
	 * @since 1.26
	 */
	public function undefineTag() {
		$dbw = wfGetDB( DB_MASTER );
		$dbw->delete( 'valid_tag', array( 'vt_tag' => $this->name ), __METHOD__ );
	 ...
	 * by user-facing code. See ChangeTags::deleteTagWithChecks() for functionality
	 * that can safely be exposed to users.
	 *
	 * @return Status The returned status will be good unless a hook changed it
	 * @since 1.26
	 */
	public function deleteTagEverywhere() {
		$tag = $this->name;
		$dbw = wfGetDB( DB_MASTER );
		...
		// Clearing tag caches
		ChangeTagsContext::purgeTagCacheAll( $tag );
