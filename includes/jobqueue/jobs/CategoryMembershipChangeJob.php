...
class CategoryMembershipChangeJob extends Job {
	...
	protected function notifyUpdatesForRevision( WikiPage $page, Revision $newRev ) {
		...
		$additionMap = $categoryChangesMap[CategoryMembershipChange::CATEGORY_ADDITION];
		$removalMap = $categoryChangesMap[CategoryMembershipChange::CATEGORY_REMOVAL];

		foreach ( $additionMap as $type => $categoryListMap ) {
		...
		foreach ( $removalMap as $type => $categoryListMap ) {
			foreach ( $categoryListMap as $categoryName ) {
				$categoryTitle = Title::makeTitle( NS_CATEGORY, $categoryName );
	...
	private function getExplicitCategoriesChanges(
		...
		$newCatsNotNormal = array_merge( $newCatsNoInclude, $newCatsIncludeOnly );
		$newCatsNormalText = array_values( array_diff( $newCats, $newCatsNotNormal ) );
		...
		$oldCatsNotNormal = array_merge( $oldCatsNoInclude, $oldCatsIncludeOnly );
		$oldCatsNormalText = array_values( array_diff( $oldCats, $oldCatsNotNormal ) );
