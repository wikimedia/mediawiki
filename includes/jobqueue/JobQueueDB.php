...
class JobQueueDB extends JobQueue {
	...
	protected function doBatchPush( array $jobs, $flags ) {
		...
		if ( !$dbw->getFlag( $dbw::DBO_TRX ) && !$dbw->trxLevel() ) {
			// DB connection is generally a separate CONN_TRX_AUTO handle.
			// No transaction is active or will be started by writes, so push the jobs
			// immediately so that any errors show up now as the interface expects. Any
			// errors that bubble up will rollback the main commit round.
			$this->doBatchPushInternal( $dbw, $jobs, $flags, __METHOD__ );
		} else {
			// DB connection is probably a regular round-aware handle (e.g. sqlite).
