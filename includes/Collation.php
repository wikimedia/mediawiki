...
class IcuCollation extends Collation {
	...
	function __construct( $locale ) {
		...
		$this->language = Language::factory( $locale === 'root' ? 'en' : $localeParts[0] );
	...
	function getFirstLetterData() {
		...
		$cacheKey = wfMemcKey( 'first-letters', $this->locale, $this->language->getCode() );
		...
		if ( isset( self::$tailoringFirstLetters[$this->locale] ) ) {
			...
			foreach ( $digits as $digit ) {
				$letters[] = $this->language->formatNum( $digit, true );
			}
		} else {
...
class CollationCkb extends IcuCollation {
	function __construct() {
		...
		// This will overwrite $language, which affects #getFirstLetterData()
		$this->language = Language::factory( 'ckb' );
	}
}
