...
abstract class ProfilerOutput {
	...
	public function log() {
		switch( get_class( $this->collector ) ) {
			case 'ProfilerStandard':
				$this->logStandardData( $this->collector->getCollatedData() );
				break;
			case 'ProfilerXhprof':
				$this->logXhprofData();
				break;
	...
	protected function logStandardData( array $collated ) {
		throw new MWException( get_class( $this ) . ' does not support the standard MediaWiki profiler.' )
	}

	/**
	 * Log XHProf-style profiling data.
	 * Up to classes to implement this.
	 */
	protected function logXhprofData() {
		throw new MWException( get_class( $this ) . ' does not support the XHProf profiler.' )
	}
}