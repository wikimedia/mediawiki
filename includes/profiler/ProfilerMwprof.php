...
class ProfilerMwprof extends Profiler {
	...
	public function profileIn( $inName ) {
		$this->mWorkStack[] = array( $inName, count( $this->mWorkStack ), $this->getTime(), $this->getTime( 'cpu' ) );
	}
	...
	public function logData() {
		...
		foreach ( $this->mCollated as $name => $entry ) {
			...
			// exceed the maximum UDP field size, send all pending data and
			// reset the buffer. Otherwise, continue accumulating entries into
			// the current buffer.
			if ( $length + $bufferLength > 65507 ) {
				socket_sendto( $sock, $buffer, $bufferLength, 0, $wgUDPProfilerHost, $wgUDPProfilerPort );
				$buffer = '';
				$bufferLength = 0;
			}
			$buffer .= $encoded;
		...
		socket_sendto( $sock, $buffer, $bufferLength, MSG_EOF, $wgUDPProfilerHost, $wgUDPProfilerPort );
	}
}
