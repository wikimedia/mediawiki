...
class User {
	...
	private function getBlockedStatus( $bFromSlave = true ) {
		...
		if ( !$block instanceof Block && $ip !== null && !$this->isAllowed( 'proxyunbannable' )
			&& !in_array( $ip, $wgProxyWhitelist ) ) {
			$block = $this->getBlockByXff( $ip, $bFromSlave );
	 ...
	 * @since 1.21
	 */
	public function getBlockByXff( $ip = false, $fromSlave = true ) {
		...
		if ( !$rows->numRows() ) {
			...
			wfProfileOut( __METHOD__ );
			return Block::newFromRow( $rows->fetchObject() );
		}
