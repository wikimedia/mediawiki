...
class User implements IDBAccessObject {
	...
	protected $queryFlags = self::READ_NORMAL;
	...
	public function saveToCache() {
		...
		// The cache needs good consistency due to its TTL so the user should
		// have been loaded from the master to begin with to lag amplification.
		if ( !( $this->queryFlags & self::READ_LATEST ) ) {
	...
	private function loadGroups() {
		if ( is_null( $this->mGroups ) ) {
			$dbr = ( $this->queryFlags & self::READ_LATEST )
				? wfGetDB( DB_MASTER )
				: wfGetDB( DB_SLAVE );
