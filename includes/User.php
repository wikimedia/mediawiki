...
class User {
	...
	public function resetPasswordExpiration( $load = true ) {
		...
		if ( $wgPasswordExpirationDays ) {
			#$expTS = wfTimestamp() + ( $wgPasswordExpirationDays * 24 * 3600 );
			#$newExpire = wfTimestamp( TS_MW, $expTS );
			$expInterval = new DateInterval( "P{$wgPasswordExpirationDays}D" );
			$expTs = new MWTimestamp;
			$expTs->timestamp->add( $expInterval );
			$newExpire = $expTS->getTimestamp( TS_MW );
		}
	...
	public function getPasswordExpired() {
		...
		if ( $expiration !== null && $expUnix < $now ) {
			$graceExpired = ( $expUnix + $wgPasswordExpireGrace < $now );
