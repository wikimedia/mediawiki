...
class User implements IDBAccessObject {
	...
	/**
	 * These fields were always marked "@private". With AuthManager, anything
	 * using them despite that will likely break since that takes over password
	 * handling, so make them actually private to fail more cleanly.
	 ...
	 * @deprecated Removed in 1.26. If a generic interface for expiring
	 *  passwords is needed, design and implement one in AuthManager.
	 * @since 1.23
	 * @param int $ts Optional timestamp to convert, default 0 for the current time
	 */
	public function expirePassword( $tz = 0 ) {
		throw new BadMethodCallException( __METHOD__ . ' has been removed in 1.26' );
	}
	 ...
	 * @todo Figure out a better place for this to exist, if it's needed.
	...
	public function isLocked() {
		...
		/// @todo: Provide a new way for CentralAuth to return this info
	...
	/**
	 * @deprecated Removed in 1.26. Users might not even have passwords
	 * anymore.
	 * @return Password
	 * @since 1.24
	 */
	 ...
	 *
	 * @deprecated since 1.26, use AuthManager instead
	...
	private function setPasswordInternal( $str ) {
		...
		// If the user doesn't exist yet, save the password for when we add it.
		...
		foreach ( $reqs as $req ) {
			$status = $manager->canChangeAuthenticationData( $req );
			if ( !$status->isOk() ) {
	 ...
	 * @deprecated since 1.26. Some way to do this via AuthManager (probably
	 ...
	 * @deprecated Removed in 1.26. See above.
	 * @return bool
	 */
	...
	public static function createNew( $name, $params = array() ) {
		foreach ( array( 'password', 'newpassword', 'newpass_time', 'password_expires' ) as $field ) {
			if ( isset( $params[$field] ) ) {
				wfDeprecated( __METHOD__ . " with param '$field'", '1.26' );
				unset( $params[$field] );
			}
		}
		...
		$noPass = $passwordFactory->newFromCiphertext( null )->toString();
	 ...
	 *
	 * @deprecated since 1.26
	 * @todo Define an appropriate replacement
