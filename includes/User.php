...
class User {
	 ...
	 * - 'registered-api' - as above, but not shown on Special:Preferences,
	 *                      intended to be set via API.
	...
	public function getOptionKinds( IContextSource $context, $options = null ) {
		...
		foreach ( $options as $key => $value ) {
			if ( isset( $prefs[$key] ) ) {
				if ( isset( $prefs[$key]['type'] ) && $prefs[$key]['type'] === 'api' ) {
					$mapping[$key] = 'registered-api';
				} else {
	 ...
	 *
	 * @param $resetKinds array|string which kinds of preferences to reset. Default is
	 *                                 array( 'registered', 'registered-api', 'registered-multiselect', 'unused' )
	 *                                 for backwards-compatibility.
	 * @param $context IContextSource|null context source necessary when $resetKinds contains
	 *                                     anything other than 'all', passed to getOptionKinds()
	 */
	public function resetOptions(
		$resetKinds = array( 'registered', 'registered-api', 'registered-multiselect', 'unused' ),
		$context = null
	) {
		$this->load();
		...
		if ( in_array( 'all', $resetKinds ) ) {
			...
			if ( !$context instanceof IContextSource ) {
				throw new MWException( 'Missing or invalid context for ' . __METHOD__ );
			}
