...
class LangLinkConverter {
	 ...
	 * The rows should also come from the same page, and each language should appear
	 ...
	 *
	 * @return string[] A map of language codes to links of the form "language:title"
	 */
	public function rowsToLinks( array $rows ) {
		$links = array();

		foreach ( $rows as $row ) {
			$links[$row->ll_lang] = $this->rowToLink( $row );
		};
