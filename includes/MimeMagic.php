...
class MimeMagic {
	...
	public function getPropsFromPath( $path, $ext ) {
		...
		if ( $info['fileExists'] ) {
			...
			$info['file-mime'] = $this->guessMimeType( $path, false );
			# Logical MIME type
			$ext = ( $ext === true ) ? FileBackend::extensionFromPath( $path ) : $ext;
			$info['mime'] = $this->improveTypeFromExtension( $info['file-mime'], $ext );

			list( $info['major_mime'], $info['minor_mime'] ) = File::splitMime( $info['mime'] );
			$info['media_type'] = $this->getMediaType( $path, $info['mime'] );
