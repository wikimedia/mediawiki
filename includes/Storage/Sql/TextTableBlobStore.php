...
class TextTableBlobStore implements BlobStore {
	...
	/**
	 * @var string[] groups for use with LoadBalancer::getConnection
	 */
	private $dbGroups = [];
	...
	public function setWiki( $wiki ) {
		Assert::parameterType( 'string|bool', $wiki, '$wiki' );
		$this->wiki = $wiki;
	}
	 ...
	 * @return string[]
	 */
	public function getDbGroups() {
		return $this->dbGroups;
	}

	/**
	 * @param string[] $dbGroups
	 */
	public function setDbGroups( $dbGroups ) {
		Assert::parameterElementType( 'string', $dbGroups, '$dbGroups' );
	...
	public function loadData( $address, $queryFlags = 0 ) {
		Assert::parameterType( 'string', $address, '$address' );
		Assert::parameter( preg_match( '/^\d+$/', $address ), '$address', 'must be a positive integer' );
		...
		try {
			$dbr = $this->dbLoadBalancer->getConnection( DB_SLAVE, $this->dbGroups, $this->wiki );
			...
			if ( !$row && ( $forUpdate || $this->dbLoadBalancer->getServerCount() > 1 ) ) {
				$dbw = $this->dbLoadBalancer->getConnection( DB_MASTER, $this->dbGroups, $this->wiki );
