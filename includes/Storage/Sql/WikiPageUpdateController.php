...
class WikiPageUpdateController implements PageUpdateController {
	...
	private function computeRevisionMetaData() {
		...
		foreach ( $this->newContent as $slotName => $content ) {
			...
			$hash = $hash ? ContentHandler::base36Sha1( $hash . $content->getHash() )
						: $content->getHash();

			unset( $extraSlots[$slotName] );
		}

		foreach ( $extraSlots as $slot ) {
			$size += $slot->getLogicalContentSize();
			$hash = $hash ? ContentHandler::base36Sha1( $hash . $slot->getHash() )
				: $slot->getHash();
		}
