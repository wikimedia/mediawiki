...
class NameTableStore {
	 ...
	 *
	 * Note: Calls to this method will result in a master select for non existing names.
	 *
	 * @param int $id
	 ...
	 */
	private function reapWANCache() {
		$purgeTimestamp = microtime( true );
		$cacheClear = function () use ( $purgeTimestamp ) {
			$this->cache->reap( $this->getCacheKey(), $purgeTimestamp );
		};
		...
		$this->getDBConnection( DB_MASTER )
			->onTransactionPreCommitOrIdle( $cacheClear, __METHOD__ );
	}
