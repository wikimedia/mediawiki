...
class PageStore implements IDBAccessObject {
	...
	public function loadLatestRevision( $page, $from = self::READ_NORMAL ) {
		// FIXME: decide: should this be extended to support "lastest approved", etc?
	...
	private function makeTitle( $page ) {
		if ( is_int( $page ) ) {
			return Title::newFromID( $page );
		} else {
			// FIXME: introduce Title::newFromPageIdentity, so we can pass in the ID.
			return Title::newFromLinkTarget( $page->getAsLinkTarget() );
		}
	}
	...
	public function getRedirectTarget( PageRecord $page ) {
		...
		if ( $row && !is_null( $row->rd_fragment ) && !is_null( $row->rd_interwiki ) ) {
			...
			// FIXME: we'll need a callback here to avoid binding against stuff we don't
			// want to bind against.
			$this->insertRedirectLater( $page, $target );
		}
	...
	private function getPageRedirectTarget( PageRecord $page ) {
		$revision = $this->loadLatestRevision( $page ); // FIXME: use $page->getLatestRevision()
	...
	public function getContributors( PageIdentity $page ) {
		...
		// FIXME: should $page have a cached latest revision?
	...
	*/
	...
	public function updatePageOn(
		...
		$page = new PageRecord( (object)$row );
