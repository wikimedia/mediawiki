...
class RevisionArchiveRecord extends RevisionRecord {
	...
	function __construct( Title $title, CommentStoreComment $comment, $row, RevisionSlots $slots, $wikiId = false ) {
		...
		$this->mPageId = isset( $row->ar_page_id ) ? intval( $row->ar_page_id ) : 0;
		$this->mParentId = isset( $row->ar_parent_id ) ? intval( $row->ar_parent_id ) : 0;
		...
		if ( isset( $row->page_id ) ) { // FIXME: not really needed, is it?
			$this->setTransientData( 'page_row', $row );
		}

		// NOTE: $this->mPageId may be different from $this->mTitle->getArticleID() in some cases,
