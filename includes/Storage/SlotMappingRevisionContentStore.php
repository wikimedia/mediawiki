...
class SlotMappingRevisionContentStore implements RevisionContentStore, RevisionContentLookup {
	...
	/**
	 * @var RevisionInfoLookup[] mapping of slot names to slot types
	 */
	private $auxilliaryInfoLookups = [];
	...
	/**
	 * @var BlobAddressResolver
	 */
	private $blobAddressResolver;

	/**
	 * @param array[] $slotDefinitions Slot definitions, mapping slot names to associative arrays
	 *        that contain the field "store" => store name.
	 *
	 * @todo: more info in the slot definition? Content model? Derived slots? Required slots?
	 *
	 * @param BlobStoreRegistry $blobStoreFactory
	 * @param BlobAddressResolver $blobAddressResolver
	 * @param RevisionSlotTable $slotTable
	 */
	public function __construct(
		array $slotDefinitions,
		BlobStoreRegistry $blobStoreFactory,
		BlobAddressResolver $blobAddressResolver,
		RevisionSlotTable $slotTable
	) {
		...
		$this->blobAddressResolver = $blobAddressResolver;
		$this->slotTable = $slotTable;

		// TODO: init $this->auxilliaryInfoLookups from constructor args
	}
	...
	public function storeRevisionContent( Title $title, $revisionId, $slots, $timestamp, $parentRevision ) {
		// FIXME: revisit! Use separate methods for aux and primary content! Don't mix them!
		$primarySlots = array_flip( $this->listContentSlots() );
		$hasPrimary = false;
		...
		// FIXME: we need to fall back to rev_text_id resp ar_text_id for old revisions!
		...
		$hints = [
			BlobStore::HINT_PAGE => $title->getArticleID(),
			BlobStore::HINT_REVISION => $revisionId,
			// TODO: BlobStore::HINT_PARENT => ...,
		];


		$this->upsertRevisionContent( $title, $revisionId, $slots, $existingAddresses, $timestamp, $hints );
	 ...
	 * @param array $hints
	 */
	private function upsertRevisionContent( Title $title, $revisionId, array $slots, array $existingAddresses, $timestamp, array $hints ) {
		...
		$contentRecords = [];
		foreach ( $slots as $slot => $content ) {
			$slotHints = array_merge( $hints, [
				BlobStore::HINT_REPLACE => isset( $existingAddresses[$slot] ) ? $existingAddresses[$slot] : null,
				// TODO: BlobStore::HINT_SIMILAR => address of this slot in the parent revision
			] );
			$contentRecords[$slot] = $this->storeContentBlob( $slot, $content, $timestamp, $slotHints );
		}
	...
	public function getRevisionContent( $revisionId, $slot ) {
		// TODO: use RevisionBlobInfo here?
		// TODO: re-design the reading interface to provide batching/pre-loading/lazy-loading for
		$records = $this->slotTable->getContentRecords( $revisionId );

		// FIXME: if !isset( $records[$slot] ) throw something meaningful
		$record = $records[$slot];

		// TODO: provide a batch interface
		list( $blob ) = $this->blobAddressResolver->getBlobs( [ $record['blob_address'] ] );
		$model = $record['content_model'];
		$format = $record['content_format'];
	 ...
	 */
	private function storeContentBlob( $slot, $content, $timestamp, $hints ) {
		...
		$blob = $handler->serializeContent( $content, $format );

		$hash = $content->getHash();

		$hints = array_merge( $hints, [
			BlobStore::HINT_MODEL => $model,
			BlobStore::HINT_FORMAT => $format,
			BlobStore::HINT_REVISION => $format,
			BlobStore::HINT_HASH => $hash,
		] );

		$address = $blobStore->storeData( $blob, $hints );
	...
	public function getRevisionInfo( $revisionId, $name ) {
		if ( isset( $this->auxilliaryInfoLookups[$name] ) ) {
			return $this->auxilliaryInfoLookups[$name]->getRevisionInfo( $revisionId, $name );
		}
