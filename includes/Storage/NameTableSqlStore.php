...
class NameTableSqlStore {
			...
			if ( $id === null ) {
				...
				if ( $searchResult === false ) {
					...
					throw new NameTableAccessException(
						"No insert possible but master didn't give us a record for: {$name} in {$this->table}"
					);
				}
	...
	public function getName( $id ) {
		...
		// TODO the accessor knows the id, so they probably know this exists, fall back to master
