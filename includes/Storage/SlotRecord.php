...
class SlotRecord {
	 ...
	 * to be used wit newForSaving(). They are incomplete, so some getters such as
	 ...
	 *
	 * @return SlotRecord An incomplete proto-slot object, to be used with newForSaving() later.
	 */
	public static function newUnsaved( $role, Content $content ) {
		...
		$row = [
			...
			'content_address' => null, // not yet known, will be set in newForSaving()
			'role_name' => $role,
	 ...
	 * particularly the revision ID and content address, which now need to be recorded for the slot.
	 *
	 * @param int $revisionId the revision the slot is to be associated with. If $protoSlot already
	 *        has a revision, it must be the same.
	 * @param string $contentAddress the slot's content address. If $protoSlot already
	 *        has an address, it must be the same.
	 ...
	 *
	 * @throws LogicException If the state of $protoSlot is inappropriate for saving a new revision.
	 * @return SlotRecord
	 */
	public static function newForSaving( $revisionId, $contentAddress, SlotRecord $protoSlot ) {
