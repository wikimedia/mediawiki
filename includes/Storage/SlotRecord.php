...
class SlotRecord {
	...
	public static function newWithSuppressedContent( SlotRecord $slot ) {
		...
		return new SlotRecord( $row, function() {
			throw new RevisionLookupException( 'Content suppressed!' );
		} );
	}

	/**
	 * Returns a SlotRecord for a new revision, inheriting the content of the given SlotRecord
	...
	/**
	 * Constructs a new Slot from a Content object. This is the preferred way to construct a slot
	 * for storing Content that resulted from a user edit.
	 *
	 * @param string $role
	...
	public function getModel() {
		try {
			// FIXME: will need mapping once it's an int!
			$model = $this->getStringField( 'cont_model' );
		} catch ( OutOfBoundsException $ex ) {
	...
	/**
	 * Returns the blob serialization format
	 *
	 * @deprecated There should be no need to know the blob format beyond Content::getDefaultFormat!
	 *
	 * @return string the content model of the content
	 */
	public function getBlobFormat() {
		try {
			// FIXME: will need mapping once it's an int!
			$model = $this->getStringField( 'cont_format' );
		} catch ( OutOfBoundsException $ex ) {
			$model = $this->getContent()->getDefaultFormat();
			$this->setField( 'cont_model', $model );
		}
	...
	// FIXME: getters for blob abd blob_flags!
