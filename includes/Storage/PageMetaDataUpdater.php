...
class PageMetaDataUpdater implements IDBAccessObject {
	 ...
	 *
	 * @return bool
	 * @throws LogicException if called before grabParentRevision
	 */
	...
	public function prepareEdit(
		...
		// FIXME: check that this works properly for a) null-edits and b) null-revisions!
		...
		$this->options['created'] = ( $this->oldPageState['oldRevision'] === null );
	...
	private function assertHasPageState( $method ) {
		if ( !$this->oldPageState ) {
			throw new LogicException(
				'Must call grabParentRevision() or prepareEdit() or prepareUpdate() before calling '
				. $method
			);
		}
	}
	...
	public function prepareUpdate( RevisionRecord $revision, array $options = [] ) {
		...
		// FIXME: make sure that this isn't falsely triggered if a revision uses {{subst:REVISIONID}}
		if ( $this->pstContentSlots
			&& !$this->pstContentSlots->hasSameContent( $revision->getSlots() )
		) {
			throw new LogicException(
	...
	public function getSlotParserOutput( $role, $generateHtml = true ) {
		...
		// FIXME: This makes sense when we are generating output for the parser cache.
		// However, we need the same code for private viewing by privileged users.
		if ( !$this->isContentPublic() ) {
			// empty output
