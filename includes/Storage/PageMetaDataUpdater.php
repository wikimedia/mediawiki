...
class PageMetaDataUpdater implements IDBAccessObject {
	...
	/**
	 * Checks whether this PageMetaDataUpdater can be re-used for the given
	 * user and RevisionRecord.
	 *
	 * @param User $user
	 ...
	 * @return bool
	 */
	public function canReuse( User $user, RevisionRecord $revision ) {
	...
	public function getRawSlot( $role ) {
		...
		return $this->pstContentSlots->getSlot( $role );
	}
	...
	public function wasRedirect() {
		$this->assertPrepared( __METHOD__ );
		return $this->options['oldIsRedirect'];
	}
	...
	public function wasCountable() {
		$this->assertPrepared( __METHOD__ );
		return $this->options['oldCountable'];
	}
		...
		if ( $this->oldPageState !== null ) {
			...
			$this->oldPageState = [
				...
				'oldIsRedirect' => isset( $options['oldredirect'] )
					? $options['oldredirect']
					: $this->isRevisionRedirect( $parentRevision ), // XXX: may not be relevant!
				'oldCountable' => isset( $options['oldcountable'] )
					? $options['oldcountable']
					: $this->isRevisionCountable( $parentRevision ),
			];
		}
