...
ion() was called.
	...
	public function prepareUpdate( RevisionRecord $revision, array $options = [] ) {
		...
		// $this->newContentSlots being set), then the given revision must have the acting user
		// as the revision author. Otherwise, user signatures generated by PST would mismatch
		// the use in the revision record.
		if ( $this->user !== null && $this->newContentSlots ) {
		...
		if ( !$this->oldPageState ) {
			...
			if ( isset( $this->options['oldrevision'] ) ) {
				$this->oldPageState['oldRevision'] = $this->options['oldrevision'];
			}
		}
