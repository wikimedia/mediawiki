...
class PageMetaDataUpdater implements IDBAccessObject {
	...
	/**
	 * @param RevisionSlots $a
	 * @param RevisionSlots $b
	 *
	 * @return bool
	 */
	private static function sameSlots( RevisionSlots $a, RevisionSlots $b ) {
		// FIXME
		return false;
	}
	 ...
	 * @return bool
	 */
	private static function sameRoles( RevisionSlots $a, RevisionSlots $b ) {
		// FIXME
		return false;
	}
	 ...
	 * @return bool
	 */
	private function revisionIsCountable( RevisionRecord $rev ) {
		// FIXME
		return false;
	}
	 ...
	 * @return bool
	 */
	public function canUseForRevision( UserIdentity $user, RevisionRecord $revision ) {
		if ( $user->getActorId() !== $this->user->getActorId() ) {
			return false;
		}

		if ( $this->revision && $this->revision->getId() !== $revision->getId() ) {
			return false;
		}
		...
		if ( $this->pstContentSlots
			&& !self::sameSlots( $this->pstContentSlots, $revision->getSlots() )
		) {
			return false;
		}
	...
	/**
	 * Checks whether this PageMetaDataUpdater can be re-used while performing an edit
	 * changing the given slots.
	 *
	 * @param UserIdentity $user
	 * @param RevisionSlots $newSlots
	 * @param array $stopSlots
	 * @param null|int $parentId
	 *
	 * @return bool
	 */
	public function canUseForContent(
		UserIdentity $user,
		...
		if ( $this->newContentSlots
			&& !self::sameSlots( $this->newContentSlots, $newSlots )
		) {
			return false;
		}
	 ...
	 */
	public function prepareEdit( RevisionSlots $newContentSlots, $stopSlots = [], $useStash = true ) {
		if ( $this->revision ) {
	...
	/**
	 * Whether the page was countable before the edit.
	 *
	 * @return array
	 */
	public function wasCountable() {
		$this->assertPrepared( __METHOD__ );

		if ( $this->oldPageState['oldCountable'] === null ) {
			/** @var RevisionRecord $rev */
			$rev = $this->oldPageState['oldRevision'];
			if ( $rev ) {
				$this->oldPageState['oldCountable'] = $this->revisionIsCountable( $rev );
			} else {
				$this->oldPageState['oldCountable'] = false;
			}
		}

		return $this->oldPageState['oldCountable'];
	}
	...
	public function prepareUpdate( RevisionRecord $revision, array $options = [] ) {
		...
		if ( $this->oldPageState !== null ) {
			...
			if ( isset( $options['oldrevision'] ) ) {
				$parentRevision = $options['oldrevision'];

				if ( $parentRevision instanceof Revision ) {
					$parentRevision = $parentRevision->getRevisionRecord();
				}

				$parentId = $parentRevision ? $parentRevision->getId() : 0;
			} else {
				$parentId = $revision->getParentId();
				$flags = $this->useMaster() ? RevisionStore::READ_LATEST : 0;
				$parentRevision = $parentId
					? $this->revisionStore->getRevisionById( $parentId, $flags )
					: null;
			}
			...
			$this->oldPageState = [
				...
				'oldId' => $parentId,
				'oldIsRedirect' => isset( $options['oldredirect'] )
						&& is_bool( $options['oldredirect'] )
					? $options['oldredirect']
					: null,
				'oldCountable' => isset( $options['oldcountable'] )
						&& is_bool( $options['oldcountable'] )
					? $options['oldcountable']
					: null,
			];
		}
		...
		// "created" and "changed" need to be derived here
		$this->options['created'] = $this->options['created']
			|| ( !$this->oldPageState['oldRevision'] );
	...
	public function getPreparedEdit() {
		...
		$preparedEdit->newContent = $preparedEdit->pstContent; // fake, unused
		$preparedEdit->oldContent = null; // unused
		$preparedEdit->revid = $this->revision ? $this->revision->getId() : 0;
		$preparedEdit->timestamp = $this->getTimestampNow();
