<?php
/**
 * A handle for managing updates for derived page meta-data on edit, import, purge, etc.
 *
 ...
 * controlled by the isReusableFor() method.
 *
 * When using a PageMetaDataUpdater, the following life cycle must be observed:
 * grabCurrentRevision (optional), prepareEdit (optional), prepareUpdate (required
...
class PageMetaDataUpdater implements IDBAccessObject {
	 ...
	 * This is determined by the first call to grabCurrentRevision, prepareEdit, or prepareUpdate.
	...
	public function isReusableFor(
		...
		// TODO: check "stop slots" if needed!
	...
	/**
	 * Whether prepareUpdate() or prepareEit() have been called on this instance.
	 *
	 * @return bool
	 */
	 ...
	 *
	 * @note: Calling this method more than once with the same $newContentSlots
	...
	private function outputVariesOnRevisionMetaData( ParserOutput $out, $method = __METHOD__ ) {
		if ( $out->getFlag( 'vary-revision' ) ) {
			...
			// This is only known after a null-edit has been detected.
			// XXX: Why would the existing rendering be using "Alice" in the first place?
			//      Parser::getRevisionObject() should already be doing the right thing.
			$this->saveParseLogger->info(
	...
	public function doUpdates() {
		...
		// TODO: if this happens post-send, investigate whether this may lead to
		// race conditions, cause by the client already requesting the page view after the edit
		// while parsing is still in progress. We may need to use the PoolCounter here.
		$output = $this->getCanonicalParserOutput();
