...
class BlobStore implements IDBAccessObject, BlobLookup {
	...
	public function expandBlob( $raw, $flags, $cacheKey = null ) {
		...
		if ( in_array( 'external', $flags ) ) {
			...
			return $this->decompressRevisionData( $raw, $flags );
		}
	}
	...
	public function decompressRevisionData( $blob, $blobFlags ) {
		...
		// Needed to support old revisions from the 1.4 / 1.5 migration using HistoryBlobCurStub
	 ...
	 * the old_id. However, the implementation may change to insert fows into the text
	 * table on the fly, in order to support at least the "ext" schema for ExternalStore
	 * addresses.
	 *
	...
	public function getTextIdFromAddress( $address ) {
		list( $schema, $id, ) = BlobStore::splitBlobAddress( $address );
