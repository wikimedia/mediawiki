 ...
 * grabCurrentRevision (optional), prepareRevisionCreation (optional), prepareUpdate (required
 * for doUpdates). getCaninicalParserOutput, getSlots, and getSecondaryDataUpdates
 * require prepareRevisionCreation or prepareUpdate to have been called first, to initialize the
...
class DerivedPageDataUpdater implements IDBAccessObject {
	 ...
	 * This is determined by the first call to grabCurrentRevision, prepareRevisionCreation,
	 * or prepareUpdate.
	...
	private function useMaster() {
		// TODO: can we just set a flag to true in prepareRevisionCreation()?
	 ...
	 */
	public function prepareRevisionCreation(
		User $user,
		...
		if ( $this->slotsUpdate ) {
			...
			if ( $this->user->getName() !== $user->getName() ) {
				throw new LogicException( 'Can\'t call prepareRevisionCreation() again for different user! '
			...
			if ( !$this->slotsUpdate->hasSameUpdates( $slotsUpdate ) ) {
				throw new LogicException(
					'Can\'t call prepareRevisionCreation() again with different slot content!'
				);
			}
		...
		if ( $this->revision ) {
			throw new LogicException(
				'Can\'t call prepareRevisionCreation() after prepareUpdate() has been called'
			);
		}
	...
	private function assertHasPageState( $method ) {
		if ( !$this->pageState ) {
			throw new LogicException(
				'Must call grabCurrentRevision() or prepareRevisionCreation() '
	...
	private function assertPrepared( $method ) {
		if ( !$this->pstContentSlots ) {
			throw new LogicException(
				'Must call prepareRevisionCreation() or prepareUpdate() before calling ' . $method
			);
		}
	}
	 ...
	 * record would return false after prepareRevisionCreation(), but true after prepareUpdate()!
	 ...
	 *
	 * @note: If grabCurrentRevision() (or prepareRevisionCreation()) has been called before
	...
	public function prepareUpdate( RevisionRecord $revision, array $options = [] ) {
		...
		// If prepareRevisionCreation() was used to generate the PST content (which is indicated by
		...
		// If $this->pageState was not yet initialized by grabCurrentRevision or prepareRevisionCreation,
	...
	public function getCanonicalParserOptions() {
		...
		//XXX: if $this->revision is not set but we already know that we pending update is a
		//     null-edit, we could perhaps use the page's current revision here. [dk 2018-04]
