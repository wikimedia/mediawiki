...
class DerivedPageDataUpdater implements IDBAccessObject {
	 ...
	 * This is determined by the first call to grabCurrentRevision, prepareRevisionCreation, or prepareUpdate.
	...
	public function prepareRevisionCreation(
		...
		if ( $this->revision ) {
			throw new LogicException( 'Can\'t call prepareRevisionCreation() after prepareUpdate() has been called' );
		}
	...
	private function assertHasPageState( $method ) {
		if ( !$this->pageState ) {
			throw new LogicException(
				'Must call grabCurrentRevision() or prepareRevisionCreation() or prepareUpdate() before calling '
				. $method
			);
		}
	}
	 ...
	 * record would return false after prepareRevisionCreation(), but true after prepareUpdate()! This should
	 * probably be fixed.
	 *
	 * @return array
	 */
	 ...
	 *
	 * @note: If grabCurrentRevision() (or prepareRevisionCreation()) has been called before calling this
