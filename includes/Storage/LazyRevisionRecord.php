...
class LazyRevisionRecord implements RevisionRecord {
	 ...
	 *
	 * @return int
	 * @deprecated since 1.25, use getUser( RevisionRecord::RAW )
	 */
	 ...
	 *
	 * @return string
	 * @deprecated since 1.25, use getUserText( RevisionRecord::RAW )
	 */
	 ...
	 *
	 * @return string
	 * @deprecated since 1.25, use getComment( RevisionRecord::RAW )
	 */
	...
	private function fetchText() {
		...
		if ( !$row ) {
			wfDebugLog( 'RevisionRecord', "No text row with ID '$textId' (revision {$this->getId()})." );
		}

		$text = self::getRevisionText( $row );
		if ( $row && $text === false ) {
			wfDebugLog( 'RevisionRecord', "No blob for text row '$textId' (revision {$this->getId()})." );
		}
	...
	public static function newNullRevision( $dbw, $pageId, $summary, $minor, $user = null ) {
		...
		if ( $current ) {
			...
			$revision = new RevisionRecord( $row );
		} else {
			$revision = null;
		}
	...
	public static function newKnownCurrent( IDatabase $db, $pageId, $revId ) {
		...
		return $cache->getWithSetCallback(
			...
			function ( $curValue, &$ttl, array &$setOpts ) use ( $db, $pageId, $revId ) {
				...
				$rev = RevisionRecord::loadFromPageId( $db, $pageId, $revId );
