 ...
 *
 * PageUpdater instances provide check-and-set (CAS) protection against concurrent updates
 * between the time grabParentRevision() is called and commitEdit() inserts a new revision.
...
class PageUpdater implements IDBAccessObject {
	...
	/**
	 * State of the page at the time time grabParentRevision() was called.
	 * Null before grabParentRevision() was called.
	 *
	 * @var array|null
	 */
	 ...
	 * time commitEdit() trying to insert the new revision.
	 *
	 ...
	 * hasEditConflict() was called and before commitEdit() could insert a new revision.
	 *
	 ...
	 * changes after grabParentRevision() was called and before commitEdit() can insert a new revision.
	 *
	...
	private function doModify( CommentStoreComment $summary, User $user, $flags, array $meta ) {
		...
		if ( $changed ) {
			...
			Hooks::run(
				...
				[ $wikiPage, $newLegacyRevision, $meta['baseRevId'], $user, &$tags ]
			);
	...
	private function getSecondaryDataUpdates( PreparedEdit $editInfo, $recursive = false ) {
		// FIXME: make public, replace direct calls to Content::getSecondaryLinksUpdates()
		...
		// FIXME: pass PreparedEDit instead of ParserOutput
		$updates = $content->getSecondaryDataUpdates(
			$this->getTitle(), null, $recursive, $output
		);

		// FIXME: call SecondaryDataUpdates Hook and create LinksUpdate here.
