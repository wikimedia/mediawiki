...
class PageUpdater {
	...
	public function replaceSectionContent(
		...
		if ( $edittime && $sectionId !== 'new' ) {
			...
			$rev = $this->revisionStore->loadFromTimestamp( $dbr, $this->getTitle(), $edittime );
			...
			if ( !$rev
				...
				$rev = $this->revisionStore->loadFromTimestamp( $dbw, $this->getTitle(), $edittime );
			}
			if ( $rev ) {
				$baseRevId = $rev->getId();
			}
	...
	/**
	 * @param string $slot the role name of the slot to modify
	 * @param string|int|null|bool $sectionId Section identifier as a number or string
	 * (e.g. 0, 1 or 'T-1'), null/false or an empty string for the whole page
	 * or 'new' for a new section.
	 * @param Content $sectionContent New content of the section.
	 ...
	 * @since 1.24
	 */
	public function replaceSectionAtRev( $slot, $sectionId, Content $sectionContent,
		...
		if ( strval( $sectionId ) === '' ) {
			...
			$contentHandler = $this->getContentHandler( $slot );
			...
			if ( is_null( $baseRevId ) || $sectionId === 'new' ) {
				...
				$rev = $this->revisionStore->newFromId( $baseRevId );
				if ( !$rev ) {
	...
	public function doEditContent(
		...
		// FIXME: RESTORE and MARK all places where loadPageData used to be called. We may actually want to (re)load.
		!!!!
		...
		$old_revision = $this->page->getCurrentRevision(); // current revision
		$old_content = $this->page->getContent( RevisionRecord::RAW ); // current revision's content
	...
	private function doModify(
		...
		if ( $changed ) {
			...
			$latestNow = $this->pageStore->getLatestForUpdate( $dbw, $this->page );
			if ( $latestNow != $oldid ) {
			...
			if ( !$this->pageStore->updatePageOn( $dbw, $revision, null, $meta['oldIsRedirect'] ) ) {
	 ...
	 * @param RevisionRecord|int|null $revision RevisionRecord object. For backwards compatibility, a
	...
	public function prepareContentForEdit(
		...
		$edit->oldContent = $this->page->getContent( RevisionRecord::RAW );
	...
	public function doEditUpdates( RevisionRecord $revision, User $user, array $options = [] ) {
		...
		if ( $this->getTitle()->getNamespace() == NS_MEDIAWIKI ) {
			MessageCache::singleton()->updateMessageOverride( $this->getTitle(), $content );
		}

		if ( $options['created'] ) {
			self::onArticleCreate( $this->getTitle() );
		} elseif ( $options['changed'] ) { // T52785
			self::onArticleEdit( $this->getTitle(), $revision );
		}
	...
	public function insertProtectNullRevision( $revCommentMsg, array $limit,
		...
		$nullRev = $this->revisionStore->newNullRevision( $dbw, $this->page, $editComment, true, $user );
		if ( $nullRev ) {
	...
	public function doDeleteArticleReal(
		...
		$lockedLatest = $this->pageStore->getLatestForUpdate( $dbw, $this->page );
		if ( $id == 0 || $this->page->getLatest() != $lockedLatest ) {
			$dbw->endAtomic( __METHOD__ );
			// Page not there or trx snapshot is stale
			$status->error( 'cannotdelete',
				wfEscapeWikiText( strval( $this->page->getPageIdentity() ) ) );
			return $status;
		}

		// Given the lock above, we can be confident in the title and page ID values
		$namespace = $this->page->getNamespace();
		$dbKey = $this->page->getTitleDBkey();
		...
		$this->eventEmitter->doDeleteUpdates( $this->page, $content, $revision, $user );
	...
	public function commitRollback( $fromP, $summary, $bot,
		...
		// Get the last editor
		$current = $this->page->getCurrentRevision();
		if ( is_null( $current ) ) {
	...
	private function asWikiPage() {
		if ( !$this->wikiPage ) {
			$this->wikiPage = WikiPage::newFromPageIdentity( $this->page );
		}
	...
	private function getTitle() {
		if ( !$this->title ) {
			$this->title = Title::newFromPageIdentity( $this->page );
		}
		return $this->title;
	}
	...
	private function getContentHandler( $role ) {
		...
		if ( $revision ) {
			// TODO: get rid of the need to have a Title here
