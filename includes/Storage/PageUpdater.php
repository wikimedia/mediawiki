 ...
 * between the time grabCurrentRevision() is called and doEdit() inserts a new revision.
...
class PageUpdater {
	 ...
	 * revision is determined by grabCurrentRevision().
	 *
	 * @var bool|int
	 */
	 ...
	 * caused by an edit being performed between the time grabCurrentRevision() was called and the
	 * time doEdit() trying to insert the new revision.
	 *
	 * @note This method calls grabCurrentRevision(), and thus causes the effective parent revision
	...
	public function hasEditConflict() {
		...
		$parent = $this->grabCurrentRevision();
	 ...
	 * was first called. This revision is the logical parent revision of the edit to be performed.
	 *
	 ...
	 * edits.
	 *
	 * @note The logical parent revision is not to be confused with the base revision.
	 ...
	 * the parent revision is determined by grabCurrentRevision().
	 *
	 ...
	 * and the revision returned by grabCurrentRevision().
	 *
	 ...
	 * changes after grabCurrentRevision() was called and before doEdit() can insert a new revision.
	 *
	...
	public function doEdit( CommentStoreComment $summary, $flags = 0 ) {
		...
		// NOTE: This garbs the parent revision as the CAS token, if grabCurrentRevision
		...
		/*
			...
			which calls PageMetaDataUpdater::prepareEdit() and PageUpdater::doCreate() without ever
			having to actually generate a ParserOutput. Thus, when PageMetaDataUpdater::doUpdates()
	...
	private function doModify( CommentStoreComment $summary, User $user, $flags, array $meta ) {
		...
		$oldRev = $this->grabCurrentRevision();
