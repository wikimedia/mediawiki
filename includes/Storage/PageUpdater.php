...
class PageUpdater implements IDBAccessObject {
	 ...
	 */
	private function getParentRevision( $from = self::READ_LATEST ) {
		// NOTE: eventually, we won't get a WikiPage passed into the constructor any more
		$this->loadPageData( $from ); // FIXME: just get the parent revision in the constructor!
	...
	private function loadPageData( $from = self::READ_LATEST ) {
		// FIXME: we really shouldn't need this!
		// NOTE: eventually, we won't get a WikiPage passed into the constructor any more
	...
	private function getRawSlot( $role ) {
		if ( $this->newContentSlots->hasSlot( $role ) ) { // FIXME: hasSlot
			return $this->newContentSlots->getSlot( $role );
		}

		$parent = $this->getParentRevision();

		if ( $parent && $parent->hasSlot( $role ) ) { // FIXME: hasSlot
	...
	public function setContent( $role, Content $content ) {
		$this->newContentSlots->setContent( 'main', $content );
	...
	public function commitUpdate( CommentStoreComment $summary, User $user, $flags = 0 ) {
		...
		if ( count( $this->newContentSlots->getSlotRoles() ) > 1 ) {
		...
		$old_revision = $this->getParentRevision();
	...
	private function addAllContentForEdit(
		...
		$editInfo = $this->prepareContentForEdit( $this->newContentSlots, null, $user, $useCache );
		$transformedSlots = $editInfo->getTransformedContentSlots();

		foreach ( $this->newContentSlots->getSlotRoles() as $role ) {
			$content = $transformedSlots->getContent( $role );
	...
	private function doModify( CommentStoreComment $summary, User $user, $flags, array $meta ) {
		...
		$newRevisionRecord = MutableRevisionRecord::newFromParentRevision(
			$this->getParentRevision(), // XXX: pass as parameter for clear information flow?
			$summary,
			$user,
			$now
		);
		...
		DeferredUpdates::addUpdate(
			new AtomicSectionUpdate(
				...
				function () use (
					...
					// Update links, etc.
					$this->doEditUpdates( $newRevisionRecord, $user, [ 'created' => true ] );
	 ...
	 * @param int $revid Revision ID, or 0 if not known or not relevant.
	...
	public function prepareContentForEdit(
		RevisionSlots $newContentSlots, $revid, User $user,
		$useCache = true
	) {
		$wikiPage = $this->getWikiPage(); // TODO: use only for legacy hooks!
		$title = $this->getTitle();

		$sig = PreparedEdit::makeSignature( $this->getTitle(), $newContentSlots, $user, $revid );
		...
		$content = $newContentSlots->getContent( 'main' );
		...
		if ( $cachedEdit ) {
			...
			$pstContent = $content->preSaveTransform( $title, $user, $userPopts );
		}

		// TODO: MCR: parser options for each slot, or combined for all slots? Including inherited?
		...
		if ( $cachedEdit ) {
			...
			$output = $pstContent
				? $pstContent->getParserOutput( $title, $revid, $canonPopts )
				: null;
		}
	...
	private function getSecondaryDataUpdates( PreparedEdit $editInfo, $recursive = false ) {
		...
		$output = $editInfo->output; // TODO: MCR: get the right part of the composite output
	...
	public function doEditUpdates( RevisionRecord $revision, User $user, array $options = [] ) {
		...
		if ( !isset( $this->preparedEdit ) ) {
			...
			$output = $this->preparedEdit->getParserOutput();
		...
		$this->parserCache->save(
			$editInfo->getParserOutput(), $wikiPage, $editInfo->getParserOptions(),
		...
		if ( $this->rcWatchCategoryMembership // FIXME: check if *any* changed slot supports categories!
		...
		if ( $title->getNamespace() == NS_MEDIAWIKI && $this->newContentSlots->hasSlot( 'main' ) ) {
			$mainContent = $this->newContentSlots->getContent( 'main' );
