...
class PageUpdater {
	 ...
	 * immediate parent revision (the current revision before the edit).
	 * The base revision is the last r evision known to the client, while the parent revision
	 ...
	 * @param DerivedPageDataUpdater $metaDataUpdater
	...
	public function __construct(
		User $user,
		WikiPage $wikiPage,
		DerivedPageDataUpdater $metaDataUpdater,
	 ...
	 * specified via setBaseRevisionId() is not the ID of the current revision before the edit.
	 * If setBaseRevisionId() was not called, this method always returns false.
	 *
	 ...
	 *
	 * @note This method calls grabParentRevision(), and thus causes the expected parent revision
	 ...
	 * was first called. This revision is the expected parent revision of the edit, and will be
	 * recorded as the new revision's parent revision, if a new revision is created by the edit.
	 *
	 * This method MUST not be called after doEdit() was called!
	 *
	 * The current revision determined by the first call to this methods effectively acts a
	 * compare-and-swap (CAS) token which is checked by doEdit(), which fails if any concurrent
	 * edits created a new current revision.
	 *
	 ...
	 * database. If base revision and parent revision are not the same, the edit is considered to
	 * require edit conflict resolution.
	 *
	 * @throws LogicException if called after doEdit().
	 * @return RevisionRecord|null the parent revision, or null of the page does not yet exist.
	 */
	public function grabParentRevision() {
		return $this->metaDataUpdater->grabCurrentRevision();
	}
	...
	public function doEdit( CommentStoreComment $summary, $flags = 0 ) {
		// FIXME: rename to doUpdate(), so we have a clear distinction between user-driven
		// edits and internal revision creation.
		...
		/*
			...
			parser output".
			The new version of WikiPage::doEditContent() makes a PageUpdater and calls its doEdit(),
			which calls DerivedPageDataUpdater::prepareEdit() and PageUpdater::doCreate() without ever
		 ...
		 */
		$this->metaDataUpdater->getCanonicalParserOutput();
		...
		// FIXME: don't check minoredit here, check where the bot right is checked!
		$meta = [
	...
	private function doCreate( CommentStoreComment $summary, User $user, $flags, array $meta ) {
		...
		if ( !$this->metaDataUpdater->getSlots()->hasSlot( 'main' )  ) {
