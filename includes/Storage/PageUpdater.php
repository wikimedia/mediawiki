...
class PageUpdater {
	...
	public function createRevision( CommentStoreComment $summary, $flags = 0 ) {
		...
		$meta = [
			'bot' => ( $flags & EDIT_FORCE_BOT ) > 0,
			'minor' => ( $flags & EDIT_MINOR ) > 0,
			'tags' => $tags
		];
	...
	private function addAllContentForEdit(
		...
		// FIXME: this should use modified slots, not touched slots. Otherwise we lose slots that
		// are inherited from older revisions, not the parent!
		// FIXME: this needs to handle removed slots!
	}
	...
	// FIXME: for all hook calls, remove pass-by-reference where not required by hooks.txt
