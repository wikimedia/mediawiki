...
class PageUpdater {
	...
	private function getContentHandler( $role ) {
		...
		$slot = $this->metaDataUpdater->getSlots()->getSlot( $role );
	 ...
	 */
	public function commitEdit( CommentStoreComment $summary, $flags = 0 ) {
		...
		$user = $this->metaDataUpdater->getUser();
		...
		if ( $flags & EDIT_UPDATE ) {
			$status = $this->doModify( $summary, $user, $flags, $meta );
		} else {
			$status = $this->doCreate( $summary, $user, $flags, $meta );
		}
