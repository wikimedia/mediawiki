...
class PageUpdater implements IDBAccessObject {
	...
	/**
	 * Returns the immediate parent revision of the edit. Not to be confused with the logical
	 * base revision. The base revision is defined by the client, the parent revision is
	 * determined by beginEdit().
	 *
	 ...
	 *  - slots: a RevisionSlots object containing the slots updated by the edit, before PST.
	 *    This is only used if there is need to call prepareContentForEdit() again.
	 */
	public function doEditUpdates( RevisionRecord $revision, User $user, array $options = [] ) {
		...
		$newSlots = isset( $options['slots'] )
			? $options['slots'] // FIXME: can we recycle PST versions from a previous PreparedEdit?
			: $revision->getTouchedSlots(); // FIXME: no PST for these! Create getTouchedSlots()!
		...
		if ( !$editInfo ) {
			...
			// Parse the text again if needed. Avoid using the edit stash.
		...
		if ( $title->getNamespace() == NS_MEDIAWIKI && $newSlots->hasSlot( 'main' ) ) {
			$mainContent = $newSlots->getContent( 'main' );
