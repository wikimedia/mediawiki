...
class PageUpdater implements IDBAccessObject {
	...
	public function doEditContent(
		...
		$old_content = $old_revision->getContent( $slotRole, RevisionRecord::RAW ); // current revision's content
	...
	private function doModify(
		...
		if ( $changed ) {
			// FIXME newMutableRevisionForEdit()!
			$newRevision = $this->revisionStore->newMutableRevisionForEdit(
				$this->getCurrentRevision(),
				[
					'page'       => $this->pageIdentity->getId(),
					'comment'    => $summary,
		...
		DeferredUpdates::addUpdate(
			new AtomicSectionUpdate(
				...
				__METHOD__,
				function () use (
					$newRevision, &$user, $content, $summary, &$flags,
					$changed, $meta, &$status
				) {
	...
	public function doEditUpdates( RevisionRecord $revision, User $user, array $options = [] ) {
		global $wgRCWatchCategoryMembership; // FIXME
		...
		$content = $revision->getContent(); // FIXME: for each slot
	...
	public function shouldCheckParserCache( ParserOptions $parserOptions, $oldId ) {
		...
		&& $this->getContentHandler()->isParserCacheSupported(); // FIXME: for all roles!
	}
	...
	public function doDeleteArticleReal(
		...
		global $wgUser, $wgContentHandlerUseDB, $wgCommentTableSchemaMigrationStage; // FIXME: inject
		...
		if ( $id == 0 || $this->pageIdentity->getLatest() != $lockedLatest ) {
			$dbw->endAtomic( __METHOD__ );
			// Page not there or trx snapshot is stale
			$status->error( 'cannotdelete',
				wfEscapeWikiText( strval( $this->pageIdentity->getPageIdentity() ) ) );
			return $status;
		}
	...
	public function commitRollback( $fromP, $summary, $bot,
		...
		// FIXME: for each slot? Main slot only?
		$targetContent = $target->getContent( $slotRole, RevisionRecord::RAW );
