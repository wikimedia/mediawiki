...
class PageUpdater implements IDBAccessObject {
	...
	public function commitUpdate(
		...
		if ( $flags & EDIT_UPDATE ) {
			$this->status = $this->doModify( $editInfo, $user, $summary, $flags, $meta );
		} else {
			$this->status = $this->doCreate( $editInfo, $user, $summary, $flags, $meta );
		}
	...
	private function doModify(
		...
		DeferredUpdates::addUpdate(
			new AtomicSectionUpdate(
				...
				__METHOD__,
				function () use (
					$wikiPage, $newRevisionRecord, $newLegacyRevision, &$user, $content,
					$comment, &$flags, $changed, $meta, &$status
				) {
