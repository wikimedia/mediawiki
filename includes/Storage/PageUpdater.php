...
class PageUpdater implements IDBAccessObject {
	 ...
	 * @var LoggerInterface
	 */
	private $saveParseLogger; // FIXME: init!

	/**
	 * @var JobQueueGroup
	 */
	private $jobQueueGroup;  // FIXME: init!

	/**
	 * @var MessageCache
	 */
	private $messageCache; // FIXME: init!
	 ...
	 * @var boolean,
	 */
	private $useRCPatrol = true; // TODO: setters, wiring!

	/**
	 * @var boolean
	 */
	private $useNPPatrol = true; // TODO: setters, wiring!

	/**
	 * @var boolean
	 */
	private $ajaxEditStash = true; // TODO: setters, wiring!

	/**
	 * @var boolean
	 */
	private $rcWatchCategoryMembership = false;  // FIXME: init!
	...
	/**
	 * @param boolean $useAutomaticEditSummaries
	 */
	...
	/**
	 * @param boolean $rcWatchCategoryMembership
	 */
	...
	private function getDBConnectionRef( $mode ) {
		$wikiId = false; // TODO: get from RevisionStore
		return $this->loadBalancer->getConnectionRef( $mode, [], $wikiId );
	}
	...
	private function getContentHandler( $role ) {
		// NOTE: eventually, we won't get a WikiPage passed into the constructor any more
		$this->wikiPage->getContentHandler( $role ); // FIXME: make WikiPage handle all slots here!
	}
	...
	public function doEditContent(
		...
		// TODO: MCR: old content and new content will have to be combined when parsing
		$editInfo = $this->prepareContentForEdit( 'main', $content, null, $user, $useCache );
		$meta = [
			...
			'oldCountable' => $wikiPage->isCountable( $editInfo ),
		...
		if ( $flags & EDIT_UPDATE ) {
			$status = $this->doModify( $editInfo, $user, $summary, $flags, $meta );
		} else {
			$status = $this->doCreate( $editInfo, $user, $summary, $flags, $meta );
		}
		...
		return $status; // FIXME: in WikiPage, update $this->mTimestamp!
	}
	...
	private function doModify(
		PreparedEdit $editInfo, User $user, CommentStoreComment $comment, $flags, array $meta
	) {
		$wikiPage = $this->getWikiPage(); // TODO: use for legacy hooks only!
		$content = $editInfo->pstContent; // TODO: MCR!
		...
		if ( $changed ) {
			...
			$newRevisionRecord->setContent( 'main', $content ); // TODO: MCR!

			//TODO: set content from $meta['serialized'] - or don't?!
			...
			if ( !( $flags & EDIT_SUPPRESS_RC ) ) {
				...
				RecentChange::notifyEdit(
					...
					$user,
					$comment->text, // TODO: pass object when that becomes possible
					$oldid,
		...
		if ( $changed ) {
			...
			$status->value['revision'] = $newRevisionRecord; // FIXME: convert this in WikiPage!
		} else {
		...
		DeferredUpdates::addUpdate(
			new AtomicSectionUpdate(
				...
				__METHOD__,
				function () use (
					$wikiPage, $newLegacyRevision, &$user, $content, $comment, &$flags,
					$changed, $meta, &$status
				) {
	...
	private function doCreate(
		PreparedEdit $editInfo, User $user, CommentStoreComment $summary, $flags, array $meta
	) {
		$wikiPage = $this->getWikiPage(); // TODO: use for legacy hooks only!
		$content = $editInfo->pstContent; // TODO: MCR!
		...
		$now = $this->getTimestampNow();
		$newsize = $content->getSize(); // FIXME: all slots, including inherited slots!

		$prepStatus = $content->prepareSave( $wikiPage, $flags, $meta['oldId'], $user );
		$status->merge( $prepStatus );
		if ( !$status->isOK() ) {
			return $status;
		}
		...
		$newRevisionRecord = $this->revisionStore->newMutableRevisionFromArray( [
			'page'       => $newid,
			'comment'    => $summary,
		...
		DeferredUpdates::addUpdate(
			new AtomicSectionUpdate(
				...
				__METHOD__,
				function () use (
					$wikiPage, $newLegacyRevision, &$user, $content, $summary, &$flags, $meta, &$status
				) {
					// Update links, etc.
	...
	public function prepareContentForEdit(
		...
		$edit->oldContent = $this->getBaseRevision()->getContent( 'main', RevisionRecord::RAW ); // TODO: MCR!
	...
	public function doEditUpdates( Revision $revision, User $user, array $options = [] ) {
		...
		} elseif ( $this->preparedEdit[$role]->output->getFlag( 'vary-user' ) && !$options['changed'] ) {
		...
		if ( !$editInfo ) {
			...
			$editInfo = $this->prepareContentForEdit( $role, $content, $revision, $user, null, false );
		}
		...
		ResourceLoaderWikiModule::invalidateModuleCache(
			$title, $options['oldrevision'], $revision, wfWikiID()
		);
	}
