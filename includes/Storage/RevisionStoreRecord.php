...
class RevisionStoreRecord extends RevisionRecord {
	...
	public static function newFromArray_1_29( Title $title, array $array, RevisionSlots $slots, $wikiId = false ) {
		...
		$row = (object)[];
		// FIXME... see MutableRevisionRecord::newFromArray_1_29

		return new RevisionStoreRecord( $title, $row, $slots, $wikiId );
	}
	...
	function __construct( Title $title, $row, RevisionSlots $slots, $wikiId = false ) {
		...
		// FIXME: require most fiedls!

		$this->mId = intval( $row->rev_id );
		$this->mPageId = intval( $row->rev_page );
		$this->mComment = $row->rev_comment;

		// TODO: don't do work in the constructor...
		$this->mComment = CommentStore::newKey( 'rev_comment' )
			->getCommentLegacy( wfGetDB( DB_REPLICA ), $row, true ); // FIXME: no wfGetDB!
