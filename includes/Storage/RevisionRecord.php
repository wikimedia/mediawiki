...
abstract class RevisionRecord {
	...
	/**
	 * Whether this RevisionRecord represents the same revision as $rec.
	 * If one has an ID set, the ID has to be the same. If they don't,
	 * then the revision's meta data and content is compared.
	 *
	 * @param RevisionRecord $rec
	 * @return bool
	 */
	public function equals( RevisionRecord $rec ) {
		if ( $rec === $this ) {
			return true;
		}

		if ( get_class( $rec ) !== get_class( $this ) ) {
			return false;
		}

		if ( $this->getId() !== $rec->getId() ) {
			return false; // different IDs
		} elseif ( $this->getId() !== null ) {
			return true; // same non-null ID, yay!
		}

		if ( $this->getUserId() !== $rec->getUserId() ) {
			return false; // different user IDs
		}

		if ( $this->getTimestamp() !== $rec->getTimestamp() ) {
			return false; // different user IDs
		}

		if ( $this->getParentId() !== $rec->getParentId() ) {
			return false; // different parent IDs
		}

		return $this->hasSameContent( $rec );
	}

	/**
	 * @param RevisionRecord $rec
	 * @return bool true if this RevisionRecord has the same content as $rec
	 */
	public function hasSameContent( RevisionRecord $rec ) {
		...
		if ( $this->getId() !== null && $rec->getId() !== null && $this->getId() === $rec->getId() ) {
			return true;
		}
