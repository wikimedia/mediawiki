...
class MutableRevisionRecord extends RevisionRecord {
	...
	public static function newFromArray_1_29( Title $title, array $array, RevisionSlots $slots, $wikiId = false ) {
		...
		// FIXME: kill $slots param, introduce a copySlots() method instead!
		foreach ( $slots->getSlotNames() as $role ) {
			$slot = $slots->getSlot( $role );
			$record->setSlot( $slot );
		}
		...
		if ( isset( $row['comment'] ) ) {
			Assert::parameterType( CommentStoreComment::class, $row['comment'], '$row[\'comment\']' );
