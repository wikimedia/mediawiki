...
class MutableRevisionRecord extends RevisionRecord {
	...
	public static function newFromArray_1_29( Title $title, array $array, RevisionSlots $slots, $wikiId = false ) {
		...
		if ( isset( $row['id'] ) ) {
			$record->setId( intval( $row['id'] ) );
		}
		if ( isset( $row['parent_id'] ) ) {
			$record->setParentId( intval( $row['parent_id'] ) );
		}

		if ( isset( $row['sha1'] ) ) {
			$record->setSha1( $row['sha1'] );
		}
		if ( isset( $row['size'] ) ) {
			$record->setSize( intval( $row['size'] ) );
		}

		if ( isset( $row['minor_edit'] ) ) {
			$record->setMinorEdit( intval( $row['minor_edit'] ) !== 0 );
		}
		if ( isset( $row['deleted'] ) ) {
			$record->setDeleted( intval( $row['deleted'] ) );
		}

		if ( isset( $row['comment'] ) ) {
			$commentData = isset( $row['comment_data'] ) ? $row['comment_data'] : null;

			if ( $row['comment'] instanceof CommentStoreComment ) {
				$comment = $row['comment'];
			} elseif ( $row['comment'] instanceof Message ) {
				$comment = CommentStoreComment::newFromMessage( $row['comment'], $commentData ); // FIXME: method does not exist
			} else {
				$commentText = trim( strval( $row['comment'] ) );
				$comment = CommentStoreComment::newUnsavedComment( $commentText, $commentData );
			}
