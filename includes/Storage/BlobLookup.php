...
/**
 * Service interface for retrieving binary data by address.
 *
 * @see BlobStore
 *
 * @license GPL 2+
...
interface BlobLookup {
	...
	/**
	 * Hint at the content model of the data.
	 * Value must be a content model ID as used by ContentHandler (or null).
	 * BlobStores may use this to optimize storage for well known content models.
	 */
	const HINT_MODEL = 'model';

	/**
	 * Hint at the serialization format of the data.
	 * Value must be a MIME type (or null).
	 * BlobStores may use this to optimize storage for well known data formats.
	 */
	const HINT_FORMAT = 'format';

	/**
	 * Hint at the hash of the data.
	 * Value must be encoded as a string (or null).
	 * BlobStores that need a content hash may use this instead of re-calculating the hash.
	 */
	const HINT_HASH = 'hash';

	/**
	 * Hint at the page the data belongs to.
	 * Value must be an int that corresponds to a page_id (or null).
	 * BlobStores may use this to group related data together, e.g. for prefetching.
	 */
	const HINT_PAGE = 'page';

	/**
	 * Hint at the revision the data belongs to.
	 * Value must be an int that corresponds to a rev_id (or null).
	 * BlobStores may use this to group related data together, e.g. for prefetching.
	 */
