 ...
 */
class RevisionStore implements IDBAccessObject { // FIXME: implement nice narrow interfaces
	 ...
	 */
	public function __construct( RevisionContentStore $blobStore, $wikiId = false ) {
	 ...
	 *
	 * @return object[] A list of emulated slot rows. Contains at least the main slot.
	 * @throws MWException
	 */
	private function getSlotsFromLegacySchema( $row ) {
		$main = new \stdClass(); // FIXME: make proper object wrapping the row, allow extra fields
		$main->slot_role = 'main';
		...
		if ( is_object( $row ) ) {
			...
			$main->cont_address = intval( $row->rev_text_id ); // FIXME: prefix?!
			...
			$main->cont_data = isset( $row->old_text ) ? strval( $row->old_text ) : null;
			$main->cont_data_flags = isset( $row->old_flags ) ? strval( $row->old_flags ) : 0;
		} elseif ( is_array( $row ) ) {
			$main->slot_page = isset( $row['page'] ) ? intval( $row['page'] ) : null;
			$main->slot_revision = isset( $row['id'] ) ? intval( $row['id'] ) : null;

			$main->cont_address = isset( $row['text_id'] ) ? intval( $row['text_id'] ) : null;
			...
			$main->cont_data = isset( $row['text'] ) ? rtrim( strval( $row['text'] ) ) : null;
			$main->cont_data_flags = '';
			...
			if ( !empty( $row['content'] ) ) {
				...
				$main->cont_object = $content;
				$main->cont_model =  $content->getModel();
				$main->cont_format = isset( $main->cont_format )
					? $main->cont_format
		...
		// FIXME: if model is not set, set a callback!

		if ( !isset( $main->cont_object ) ) {
			$main->cont_callback = function ( RevisionRecord $rev, &$slotRow ) {
				return $this->contentStore->getContentFromRow( $rev, $slotRow );
			};
		} else {
			$main->cont_object = null;
			unset( $main->cont_callback );
		}
	...
	public function newFromId( $id, $flags = 0 ) {
		return self::newRevisionFromConds( [ 'rev_id' => intval( $id ) ], $flags );
	}
	 ...
	 * @return RevisionRecord|null
	 */
	public function newRevisionFromTitle( LinkTarget $linkTarget, $id = 0, $flags = 0 ) {
		$conds = [
		...
		if ( $id ) {
			...
			$conds['rev_id'] = $id;
			return self::newRevisionFromConds( $conds, $flags );
		} else {
			// Use a join to get the latest revision
			$conds[] = 'rev_id=page_latest';
			$db = wfGetDB( ( $flags & self::READ_LATEST ) ? DB_MASTER : DB_REPLICA );
			return self::loadRevisionFromConds( $db, $conds, $flags );
		}
	}
	 ...
	 * @return RevisionRecord|null
	 */
	public function newRevisionFromPageId( $pageId, $revId = 0, $flags = 0 ) {
	...
	public function newRevisionFromArchiveRow( $row, $overrides = [] ) {
		...
		if ( isset( $row->ar_text ) && !$row->ar_text_id ) {
			// FIXME: map to old_text and old_flags instead!
			// Pre-1.5 ar_text row
			$attribs['text'] = $this->contentStore->getBlobFromRow( $row, 'ar_text', 'ar_flags' );
			if ( $attribs['text'] === false ) {
				throw new MWException( 'Unable to load text from archive row (possibly T24624)' );
			}
		}

		return $this->newRevisionFromLegacyRow( $attribs );
	}
	 ...
	 * @return RevisionRecord
	 */
	public function newRevisionFromLegacyRow( $row ) {
		return $this->newRevisionFromRow( $row, $this->getSlotsFromLegacySchema( $row ) );
	}
	 ...
	 * @return RevisionRecord|null
	 */
	public function loadRevisionFromTitle( $db, $title, $id = 0 ) {
		if ( $id ) {
			$matchId = intval( $id );
		} else {
			$matchId = 'page_latest';
		}
		return self::loadRevisionFromConds( $db,
		                                    [
				"rev_id=$matchId",
	 ...
	 * @return RevisionRecord|null
	 */
	public function loadRevisionFromTimestamp( $db, $title, $timestamp ) {
		return self::loadRevisionFromConds( $db,
		                                    [
	...
	private function newRevisionFromConds( $conditions, $flags = 0 ) {
		...
		$rev = self::loadRevisionFromConds( $db, $conditions, $flags );
	 ...
	 * @return RevisionRecord|null
	 */
	private function loadRevisionFromConds( $db, $conditions, $flags = 0 ) {
		$row = self::fetchRevisionRowFromConds( $db, $conditions, $flags );
		if ( $row ) {
			$rev = $this->newRevisionFromLegacyRow( $row, $db->getWikiID() );
	 ...
	 * @return RevisionRecord|null
	 */
	public function loadRevisionFromId( $db, $id ) {
		return self::loadRevisionFromConds( $db, [ 'rev_id' => intval( $id ) ] );
	}
	 ...
	 * @return RevisionRecord|null
	 */
	public function loadRevisionFromPageId( $db, $pageid, $id = 0 ) {
		...
		return self::loadRevisionFromConds( $db, $conds );
	}
	...
	public function fetchRevisionRows( LinkTarget $title ) {
		$row = self::fetchRevisionRowFromConds(
			wfGetDB( DB_REPLICA ),
			[
				'rev_id=page_latest',
				'page_namespace' => $title->getNamespace(),
	 ...
	 */
	private function fetchRevisionRowFromConds( $db, $conditions, $flags = 0 ) {
		$fields = array_merge(
			self::selectRevisionFields(),
	...
	public function getPreviousRevision( Title $title, $revId ) {
		...
		if ( $prev ) {
			return $this->newRevisionFromTitle( $title, $prev );
		}
		return null;
	}
	...
	public function getNextRevision( Title $title, $revId ) {
		...
		if ( $next ) {
			return self::newRevisionFromTitle( $title, $next );
		}
		return null;
	}
	 ...
	 * @return int
	 */
	public function countByPageId( $db, $id ) {
	...
	public function newKnownCurrent( IDatabase $db, $pageId, $revId ) {
		...
		throw new MWException( 'Not Implemented' ); // FIXME!
		...
		// Page/rev IDs passed in from DB to reflect history merges
			...
			function ( $curValue, &$ttl, array &$setOpts ) use ( $db, $pageId, $revId ) {
				...
				if ( $rev ) {
					...
					$rev->setRefreshCallback( function( LazyRevisionRecord $rev ) { // FIXME: raw only?
	...
	private function loadMutableFields( $revId ) {
		$dbr = wfGetLB( $this->wikiId )->getConnectionRef( DB_REPLICA, [], $this->wikiId );
		$row = $dbr->selectRow(
			[ 'revision', 'user' ],
	...
	public function getTitle( RevisionRecord $rev ) {
		$title = $this->getTransientData( $rev, 'title' ); // FIXME: consider moving everything that uses TransientData back into RevisionRecord!
	...
	public function insertOn( RevisionRecord $rev, $dbw, $flags = 0 ) {
		...
		// FIXME: move code for writing blob data to RevisionContentStore.
		...
		$textId = null; // FIXME: get this from the main slot's meta-data!
		if ( $textId === null ) {
		...
		$row = [
			...
			'rev_sha1'       => $rev->getSha1() === null
				? RevisionContentStore::base36Sha1( $data )
				: $rev->getSha1(),
		];
	 ...
	 * @return int
	 */
	private function getPreviousRevisionId( $db, RevisionRecord $rev ) {
		if ( $rev->getPage() === null ) {
			return 0;
		}
