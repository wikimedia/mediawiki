...
class RevisionStore
	...
	public function newRevisionFromArchiveRow(
		...
		// TODO for MCR this should also return with callbacks for MCR slots
	...
	public function newRevisionFromRow( $row, $queryFlags = 0, Title $title = null ) {
		//TODO for MCR this should return RevisionRecords with callbacks for slots.
	...
	/**
	 * @param IDatabase $dbr
	 * @param string $revIdJoinField
	 * @return string
	 */
	private function getSlotDataJoinString( IDatabase $dbr, $revIdJoinField ) {
		try {
			// Safeguard against possible rollbacks where multiple slots may exist and explicitly
			// request the main slot here.
			return $dbr->makeList(
				[
					$revIdJoinField . ' = a_slots.slot_revision_id',
					'a_slots.slot_role_id = ' . $this->slotRoleStore->getId( 'main' ),
				],
				IDatabase::LIST_AND
			);
		} catch ( NameTableAccessException $e ) {
			// This can only happen when just starting schema migration!
			// If the main slot is not known, this means there would be no chance of
			// multiple slots anyway, so don't include the slot in the join condition.
		}
	...
	public function getQueryInfo( $options = [] ) {
		...
		if ( $this->mcrMigrationStage === MIGRATION_OLD ) {
			...
			// TODO allow reading multiple slots.
			$ret['tables']['a_slot_data'] = [
				'a_slots' => 'slots',
				'a_content' => 'content',
				'a_content_models' => 'content_models'
			];
			$ret['joins']['a_content'] = [ 'JOIN', 'a_slots.slot_content_id = a_content.content_id' ];
			$ret['joins']['a_content_models'] =
			...
			if ( $this->mcrMigrationStage < MIGRATION_NEW ) {
				$ret['joins']['a_slot_data'] = [
					'LEFT JOIN',
					$this->getSlotDataJoinString( $dbr, 'rev_id' ),
				];
				// Note: rev_text_id is never null, but would be 0 when not written to
				$ret['fields']['rev_text_id'] = $dbr->conditional(
					'a_content.content_address IS NULL',
					'rev_text_id',
				...
				$ret['fields']['rev_text_id'] = $dbr->buildIntegerCast( $dbr->buildSubstring(
					'a_content.content_address',
					4
				) );
		...
		// TODO remove this block, text will have no place once multiple contents can be written and read
	...
	/**
	 * TODO new getQueryInfo for slots / slot rows
	 *  - optionally join the revision table?
	 *  - optionally join the content table?
	 *  - should only work with write new / both
	 */
	public function getSlotsQueryInfo() {
	...
	public function getArchiveQueryInfo() {
		...
		if ( $this->mcrMigrationStage === MIGRATION_OLD ) {
			...
			$ret['joins']['a_slot_data'] = [
				'JOIN',
				$this->getSlotDataJoinString( $dbr, 'ar_rev_id' ),
			];
			$ret['joins']['a_content'] = [ 'JOIN', 'a_slots.slot_content_id = a_content.content_id' ];
			$ret['joins']['a_content_models'] =
				[ 'JOIN', 'a_content.content_model = a_content_models.model_id' ];
			...
			if ( $this->mcrMigrationStage < MIGRATION_NEW ) {
				$ret['fields']['ar_text_id'] = $dbr->conditional(
					'a_content.content_address IS NULL',
					'ar_text_id',
					$dbr->buildIntegerCast(
						$dbr->buildSubstring(
				...
				// TODO kill the use of substring when the fields returned by this method change
				// TODO when switching to MCR mode we probably don't want to select the text_id
				$ret['fields']['ar_text_id'] = $dbr->buildIntegerCast( $dbr->buildSubstring(
					'a_content.content_address',
					4
				) );
				// content_format does not have a place in the new schema, so just select NULL
