	...
	private function emulateSlots_1_29( $row ) {
		...
		if ( is_object( $row ) ) {
			...
			if ( isset( $row->rev_text ) ) {
				// rev_text doesn't exist, this is an artifact of converting the ar_ prefix to rev_
				$main->blob = isset( $row->ar_text ) ? strval( $row->ar_text ) : null;
				$main->blob_flags = isset( $row->ar_flags ) ? strval( $row->ar_flags ) : 0;
			} else {
