...
class RevisionStore implements IDBAccessObject, RevisionFactory, RevisionLookup {
	...
	public function getTitle( RevisionRecord $rev ) {
		...
		$wikiId = $rev->getWikiId(); // TODO: MCR:  bail out if != $this->wikiId?!
		...
		if ( $rev->getId() !== null ) {
			$dbr = $this->getDbConnectionRef( $wikiId );
			$row = $dbr->selectRow(
				[ 'page', 'revision' ],
		...
		if ( $wikiId === false || $wikiId === wfWikiID() ) {
			...
			if ( !$title && $rev->getPageId() !== null && $rev->getPageId() > 0 ) {
	 ...
	 *
	 * @param RevisionRecord $rev // TODO: MCR: require a RevisionBuilder or some such here!
	...
	public function insertRevisionOn( RevisionRecord $rev, IDatabase $dbw, $flags = 0 ) {
		...
		$rev = $this->newRevisionFromArray( $row, new RevisionSlots( [ 'main' => $content ] ) );
	...
	private function getTitleForSlot( SlotRecord $slot ) {
		$pageId = $slot->getPage();
		$title = Title::newFromID( $pageId ); // TODO: MCR: make sure this is cached!
	...
	public function newRevisionFromArchiveRow( $row, $overrides = [] ) {
		...
		return $this->newRevisionFromArray_1_29( $attribs );
	}
