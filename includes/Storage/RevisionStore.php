...
class RevisionStore implements IDBAccessObject, RevisionFactory, RevisionLookup {
	 ...
	 *
	 * @param RevisionRecord $rev //FIXME: require a RevisionBuilder or some such here!
	...
	public function insertRevisionOn( RevisionRecord $rev, IDatabase $dbw, $flags = 0 ) {
		...
		// TODO: pass Content to the ContentStore, let it do the serialization and store the blob!
		...
		$slots = $rev->getSlots(); // FIXME: $rev should be a RevisionBuilder or some such
		if ( !isset( $slots['main'] ) ) {
		...
		// FIXME: update comment handling after to match Ic3a434c0 (T166732)
		...
		$row = [
			...
			'rev_sha1'       => $rev->getSha1() === null
				? SlotRecord::base36Sha1( $data )
				: $rev->getSha1(),
		];

		if ( $wgContentHandlerUseDB ) {
			// NOTE: rev_content_model and rev_content_format will go away with the new DB schema
	...
	public function newNullRevision( IDatabase $dbw, $pageId, $summary, $minor, User $user ) {
		...
		// FIXME: allow $summary to be a Message or CommentStoreComment, to accommodate Ic3a434c (T166732)
		...
		if ( $current ) {
			...
			$revision = new DefaultRevisionRecord( $row, $this->emulateSlots_1_29( $row ) );
		} else {
			$revision = null;
		}
	...
	private function emulateSlots_1_29( $row ) {
		...
		if ( is_object( $row ) ) {
			...
			// FIXME: model ans format as int or string?!
			...
			$main->cont_model = isset( $row['content_model'] )
				? strval( $row['content_model'] ) : null;  // FIXME: int or string?!
			// XXX: in the future, we'll probably always use the default format, and drop content_format
	...
	public function newRevisionFromRow( $row, RevisionSlots $slots ) {
		...
		return new DefaultRevisionRecord( $row, $slots, $this->wikiId );
	}
	...
	public function newRevisionFromArray( array $fields, RevisionSlots $slots ) {
		// XXX: create access mask here, so we don't have to do permission checks in RevisionRecord
		return new DefaultRevisionRecord( $fields, $slots, $this->wikiId );
	}
	 ...
	 *
	 * @todo // FIXME: make this return tables and joins as well, and use CommentStore::getJoin().
	 * @todo might also lose the userJoinCond, pageJoinCond, selectTextFields, selectPageFields,
	 *       and selectUserFields methods in favor of options to the one method.
	 * @return array
	 */
	...
	public function getKnownCurrentRevision( IDatabase $db, $pageId, $revId ) {
		...
		$cache = MediaWikiServices::getInstance()->getMainWANObjectCache(); // FIXME: inject!
		$row = $cache->getWithSetCallback(
			// Page/rev IDs passed in from DB to reflect history merges
			$cache->makeGlobalKey( 'revision-row-1.29', $db->getWikiID(), $pageId, $revId ),
			$cache::TTL_WEEK,
		...
		if ( $row ) {
			return $this->newRevisionFromRow_1_29( $row );
		} else {
			return false;
		}
	}
