...
class RevisionStore implements IDBAccessObject, RevisionFactory, RevisionLookup {
	...
	public function newNullRevision( IDatabase $dbw, Title $title, $summary, $minor, User $user ) {
		...
		if ( $current ) {
			...
			// TODO: MCR: use setters instead of newFromArray
	 ...
	 * @internal param object $slotRow The raw slot row, for context and additional information.
	 *
	 */
	...
	public function newRevisionFromArray_1_29( array $fields, Title $title = null ) {
		...
		$comment = CommentStore::newKey( 'comment' )
			// Legacy because $fields probably come from code that is not aware of CommentStore
			->getCommentLegacy( $this->getDBConnection( DB_REPLICA ), $fields, true );

		return RevisionStoreRecord::newFromArray_1_29( $title, $comment, $fields, $slots, $this->wikiId );
	}
	...
	private function checkDatabaseWikiId( IDatabase $db ) {
		if ( $db->getDomainID() !== $this->wikiId ) {
			$storeWiki = $this->wikiId === false ? 'local wiki' : $this->wikiId;
			$dbWiki = $db->getDomainID() === false ? 'local wiki' : $db->getDomainID();
