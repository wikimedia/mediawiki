...
class RevisionStore implements IDBAccessObject, RevisionFactory, RevisionLookup {
	...
	public function insertRevisionOn( RevisionRecord $rev, IDatabase $dbw ) {
		...
		if ( $blobAddress === null ) {
			...
			// FIXME: move this into BlobStore::storeBlob( $blob, $hints )
			...
			// FIXME: create and assign new SlotRecord to $main!!!
		} else {
			$model = $main->getModel();
	...
	public function newNullRevision( IDatabase $dbw, Title $title, $summary, $minor, User $user ) {
		...
		if ( $current ) {
			...
			// FIXME: MCR: MutableRevisionRecord::newFromParentRevision (and setters)
			$slots = $this->emulateSlots_1_29( $row );
			$revision = MutableRevisionRecord::newFromArray_1_29( $title, $row, $slots );
		} else {
			$revision = null;
		}
	 ...
	 * @throws MWException
	 */
	private function emulateSlots_1_29( $row, $queryFlags = 0, Title $title = null ) {
		...
		if ( !$content ) {
			// FIXME: pass query flags to callbacks
			$content = function ( SlotRecord $slot ) use ( $blobData, $blobFlags, $mainSlotRow ) {
	 ...
	 */
	private function loadSlotContent( SlotRecord $slot, $blobData = null, $blobFlags = '', $blobFormat = null ) {
		...
		if ( $blobData !== null ) {
			$cacheKey = $slot->getAddress();

			$data = $this->contentStore->expandBlob( $blobData, $blobFlags, $cacheKey );
		} else {
			$data = $this->contentStore->getBlob( $slot->getAddress() );
		}
	...
	public function newRevisionFromArchiveRow( $row, $queryFlags = 0, Title $title = null, $overrides = [] ) {
		...
		$slots = $this->emulateSlots_1_29( $row, $queryFlags, $title );
	...
	public function newRevisionFromArray_1_29( array $fields, $queryFlags = 0, Title $title = null ) {
		// FIXME: construct main slot content directly from $fields!
		$slots = $this->emulateSlots_1_29( $fields, $queryFlags, $title );
		...
		return MutableRevisionRecord::newFromArray_1_29( $title, $fields, $slots, $this->wikiId );
	}
