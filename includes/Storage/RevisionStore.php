...
class RevisionStore implements IDBAccessObject {
	...
	private function getSlotsFromLegacySchema( $row ) {
		...
		if ( is_object( $row ) ) {
			...
			$main->cont_data_flags = isset( $row->old_flags ) ? intval( $row->old_flags ) : 0;
		} elseif ( is_array( $row ) ) {
		...
		if ( !isset( $main->cont_object ) ) {
			$main->cont_callback = array( $this->contentStore, 'getSlotContent' );
		} else {
	...
	public function newRevisionFromArchiveRow( $row, $overrides = [] ) {
		...
		if ( isset( $row->ar_text ) && !$row->ar_text_id ) {
			// Pre-1.5 ar_text row
			$attribs['text'] = $this->contentStore->getRevisionText( $row, 'ar_' );
	 ...
	 * @return RevisionRecord
	 */
	public function newRevisionFromLegacyRow( $row, $wikiId = false ) {
		return $this->newRevisionFromRow( $row, $this->getSlotsFromLegacySchema( $row ), $wikiId );
	}
	 ...
	 * @return RevisionRecord
	 */
	public function newRevisionFromRow( $row, $slots, $wikiId = false ) {
		// TODO: audience wrapper!
		return new LazyRevisionRecord( $row, $slots, $wikiId );
	}
	...
	private function setTransientData( RevisionRecord $rev, $key, $value ) {
		if ( $rev instanceof TransientDataAccess ) {
			$rev->setTransientData( "RevisionStore.$key", $value );
		}
	}
	...
	private function getTransientData( RevisionRecord $rev, $key, $default = null ) {
		if ( $rev instanceof TransientDataAccess ) {
			return $rev->getTransientData( "RevisionStore.$key", $default );
		} else {
			return $default;
		}
	}
