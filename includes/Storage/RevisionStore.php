...
class RevisionStore
	...
	public function insertRevisionOn( RevisionRecord $rev, IDatabase $dbw ) {
		...
		// TODO: Loop over multiple slots ofr insertion.
		if ( $this->mcrMigrationStage >= MIGRATION_WRITE_BOTH && !$slot->hasAddress() ) {
			$this->insertSlotOn(
		...
		$newSlot = SlotRecord::newSaved( $revisionId, $blobAddress, $slot );
