...
class RevisionStore implements IDBAccessObject {
	...
	private function getSlotsFromLegacySchema( $row ) {
		...
		if ( !isset( $main->cont_object ) ) {
			$main->cont_callback = array( $this, 'getSlotContent' );
		} else {
	...
	public function getSlotContent( RevisionRecord $rev, $slotRow ) {
		!!!...from Revision::getContent()....
	}
	...
	public function newFromArchiveRow( $row, $overrides = [] ) {
		...
		return new LazyRevisionRecord( $attribs, $this->getSlotsFromLegacySchema( $attribs ) );
	}
	...
	public function newFromRow( $row ) {
		// TODO: audience wrapper!
		return new LazyRevisionRecord( $row, $this->getSlotsFromLegacySchema( $row ) );
	}
	...
	private function loadFromConds( $db, $conditions, $flags = 0 ) {
		...
		if ( $row ) {
			// TODO: audience wrapper!
			$rev = new LazyRevisionRecord(
				$row,
				$this->getSlotsFromLegacySchema( $row ),
				$db->getWikiID()
			);
	...
	public function getTitle( RevisionRecord $rev ) {
		...
		if ( $wikiId === false || $wikiId === wfWikiID() ) {
			...
			if ( !$title && $rev->getPage() !== null && $rev->getPage() > 0 ) {
				$this->mTitle!!! = Title::newFromID( $rev->getPage() );
			}
		}
