...
class RevisionStore implements IDBAccessObject, RevisionFactory, RevisionLookup {
	...
	public function insertRevisionOn( RevisionRecord $rev, IDatabase $dbw ) {
		...
		if ( $this->migrationStage >= MIGRATION_WRITE_BOTH ) {
			$roleId = $this->slotRoleStore->getId( $slot->getRole() );
			$modelId = $this->contentModelStore->getId( $slot->getModel() );

			$contentRow = [
				// TODO: Is it still okay to use the rev size and sha1 when only inserting 1 slot?
