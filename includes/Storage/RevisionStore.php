...
class RevisionStore implements IDBAccessObject, RevisionFactory, RevisionLookup {
	...
	public function newRevisionFromArchiveRow(
		...
		if ( !$page && isset( $overrides['title'] ) ) {
			...
			$page = $title->getAsImmutablePageIdentit();
		}
	 ...
	 * This method will try to find the specified revision on a replica first,
	 * and falls back to master if the revision is not found there.
	 *
	...
	private function newRevisionFromConds( $conditions, $flags = 0, PageIdentity $page = null ) {
		$db = $this->getDBConnection( ( $flags & self::READ_LATEST ) ? DB_MASTER : DB_REPLICA );
		$rev = $this->loadRevisionFromConds( $db, $conditions, $flags, $page );
		$this->releaseDBConnection( $db );
	...
	public function getTimestampFromId( PageIdentity $page, $id, $flags = 0 ) {
		...
		$conds = [ 'rev_id' => $id ];
		$conds['rev_page'] = $page->getArticleID();
