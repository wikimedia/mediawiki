...
class RevisionStore implements IDBAccessObject, RevisionFactory, RevisionLookup {
	...
	public function insertRevisionOn( RevisionRecord $rev, IDatabase $dbw ) {
		...
		if ( !$slot->hasAddress() ) {
			...
			$blobHints = [
				'designation' => 'page-content', // BlobStore may be used for other things too.
				'page_id' => $pageId,
				'role_name' => $slot->getRole(),
				'rev_parent_id' => $parentId,
				'cont_sha1' => $slot->getSha1(),
				'cont_model' => $model,
				'cont_format' => $format,
			];
		...
		$textId = SqlBlobStore::getTextIdFromAddress( $blobAddress );
		...
		$user = new UserIdentityValue( $row['rev_user'], $row['rev_user_text'] );
		...
		Assert::postcondition( $rev->getUser() !== null, 'revision must have a user identity' );
	...
	private function getUserIdentityFromRowObject( $row, $prefix = 'rev_' ) {
		...
		if ( isset( $row->user_name ) ) {
			...
			$userName = User::whoIs( $row->$idField );
		}
	...
	private function initializeMutableRevisionFromArray(
		...
		if ( isset( $fields['user'] ) && ( $fields['user'] instanceof User ) ) {
			$user = $fields['user'];
		} elseif ( isset( $fields['user'] ) && isset( $fields['user_text'] ) ) {
			$user = new UserIdentityValue( $fields['user'], $fields['user_text'] );
