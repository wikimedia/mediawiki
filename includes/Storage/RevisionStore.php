...
class RevisionStore implements IDBAccessObject, RevisionFactory, RevisionLookup {
	 ...
	 * @throws MWException
	 */
	private function emulateSlots_1_30( $row ) {
		$main = new \stdClass();
		$main->slot_role = 'main';
		...
		if ( is_object( $row ) ) {
			...
			$main->cont_address = intval( $row->rev_text_id );
	...
	public function newRevisionFromArchiveRow( $row, $overrides = [] ) {
		...
		return $this->newRevisionFromRow_1_30( $attribs );
	}

	/**
	 * Construct a RevisionRecord based on the MW1.30 database schema.
	 *
	 * @since 1.31
	...
	public function newRevisionFromRow_1_30( $row ) {
		return $this->newRevisionFromRow( $row, $this->emulateSlots_1_30( $row ) );
	}
	...
	private function loadRevisionFromConds( $db, $conditions, $flags = 0 ) {
		...
		if ( $row ) {
			$rev = $this->newRevisionFromRow_1_30( $row, $db->getWikiID() );
	...
	public function newNullRevision( $dbw, $pageId, $summary, $minor, $user = null ) {
		...
		if ( $current ) {
			...
			$revision = new LazyRevisionRecord( $row, $this->emulateSlots_1_30( $row ) );
		} else {
			$revision = null;
		}
