...
class RevisionStore implements IDBAccessObject, RevisionFactory, RevisionLookup {
	 ...
	 *
	 * @param RevisionRecord $rev // TODO: MCR: use a RevisionBuilder or some such here!
	...
	public function insertRevisionOn( RevisionRecord $rev, IDatabase $dbw, $flags = 0 ) {
		...
		if ( $wgContentHandlerUseDB ) {
			// TODO: rev_content_model and rev_content_format will go away with the new DB schema
		...
		$rev = $this->newRevisionFromArray( $row, $slots, $title );
	...
	private function emulateSlots_1_29( $row ) {
		...
		if ( is_object( $row ) ) {
			...
			// FIXME: model and format as int or string?!
	...
	public function newRevisionFromArchiveRow( $row, Title $title = null, $overrides = [] ) {
		...
		return $this->newRevisionFromArray_1_29( $attribs, $title );
	}
	...
	public function newRevisionFromRow_1_29( $row, Title $title = null ) {
		...
		return $this->newRevisionFromRow( $row, $this->emulateSlots_1_29( $row ), $title );
	}
	...
	public function newRevisionFromRow( $row, RevisionSlots $slots, Title $title = null ) {
		...
		return new DefaultRevisionRecord( $title, $row, $slots, $this->wikiId );
	}
	...
	public function newRevisionFromArray_1_29( array $fields, Title $title = null ) {
		return $this->newRevisionFromArray( $fields, $this->emulateSlots_1_29( $fields ), $title );
	}
	...
	public function newRevisionFromArray( array $fields, RevisionSlots $slots, Title $title = null ) {
		...
		// XXX: create access mask here, so we don't have to do permission checks in RevisionRecord
		return new DefaultRevisionRecord( $title, $fields, $slots, $this->wikiId );
	}
	...
	public function loadRevisionFromTimestamp( IDatabase $db, $title, $timestamp ) {
		//FIXME: provide getRevisionFromTimestamp as an alternative!
		return $this->loadRevisionFromConds( $db,
			[
