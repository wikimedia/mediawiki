...
class RevisionStore implements IDBAccessObject, RevisionFactory, RevisionLookup {
	...
	/** @var bool Is multi slot functionality enabled? */
	private $multiSlotEnabled;
	...
	public function __construct(
		...
		$this->multiSlotEnabled = $multiSlotEnabled;
		$this->wikiId = $wikiId;

		if ( $multiSlotEnabled ) {
			throw new InvalidArgumentException(
				'RevisionStore multi slot mode is not yet implemented.'
			);
		}

		if ( $multiSlotEnabled && $migrationStage < MIGRATION_NEW ) {
			throw new InvalidArgumentException(
				'RevisionStore can not be in multi slot mode without MIGRATION_NEW being set.'
			);
		}
	}
