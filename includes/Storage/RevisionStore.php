...
class RevisionStore implements IDBAccessObject, RevisionFactory, RevisionLookup {
	...
	public function insertRevisionOn( RevisionRecord $rev, IDatabase $dbw ) {
		...
		$pageId = $this->failOnNull( $rev->getPageId(), 'rev_page field' ); // check this early
		...
		$sha1 = $this->failOnNull( $rev->getSha1(), 'sha1 field' );
		...
		if ( !$slot->hasAddress() ) {
			...
			$blobHints = [
				...
				'slot' => $slot->getRole(),
				'page-id' => $pageId,
				// 'revision-id' => $rev->getId(), // XXX: would be nice, but we don't have that yet!
				'parent-id' => $parentId,
				'sha1' => $sha1,
				'model' => $model,
				'format' => $format,
		...
		// sanity checks
		Assert::postcondition( $rev->getId() > 0, 'Newly inserted revision must have an ID' );
		Assert::postcondition( $rev->getPageId() > 0, 'Newly inserted revision must have a page ID' );
		Assert::postcondition( $rev->getComment() !== null, 'Newly inserted revision must have a comment' );
		Assert::postcondition( $rev->getUserText() !== null, 'Newly inserted revision must have a user name' );

		Assert::postcondition( $newSlot !== null, 'Newly inserted revision must have a main slot' );
		Assert::postcondition( $newSlot->getAddress() !== null, 'Newly inserted main slot must have an addess' );
	 ...
	 */
	public function newNullRevision( IDatabase $dbw, Title $title, CommentStoreComment $comment, $minor, User $user ) {
		$this->checkDatabaseWikiId( $dbw );
	...
	private function emulateMainSlot_1_29( $row, $queryFlags, Title $title ) {
		...
		if ( is_object( $row ) ) {
			...
			$mainSlotRow->format_name = isset( $row->rev_content_format ) ? strval( $row->rev_content_format ) : null;
		} elseif ( is_array( $row ) ) {
	...
	private function loadSlotContent( SlotRecord $slot, $blobData = null, $blobFlags = '', $blobFormat = null, $queryFlags = 0 ) {
		if ( $blobData !== null ) {
			...
			if ( !$data ) {
				throw new RevisionLookupException( "Failed to expand blob data using flags $blobFlags (key: $cacheKey)" );
			}
		} else {
			...
			if ( !$data ) {
				throw new RevisionLookupException( "Failed to load data blob from $address" );
			}
		}
	...
	public function newRevisionFromArchiveRow( $row, $queryFlags = 0, Title $title = null, $overrides = [] ) {
		...
		if ( !isset( $title ) ) {
			if ( isset( $row->ar_namespace ) && isset( $row->ar_title ) ) {
				...
				throw new InvalidArgumentException( 'A Title or ar_namespace and ar_title must be given' );
			}
		}
	...
	public function newRevisionFromArray( array $fields, $queryFlags = 0, Title $title = null ) {
		...
		if ( !isset( $fields['user_text'] ) && isset( $fields['user'] ) ) {
			if ( $fields['user'] instanceof User ) {
				...
				if ( $name === false ) {
					throw new MWException( 'user_text not given, and unknown user ID ' . $fields['user'] );
				}
		...
		if ( isset( $fields['comment'] ) && !( $fields['comment'] instanceof CommentStoreComment ) ) {
			...
			if ( $fields['comment'] instanceof Message ) {
				$fields['comment'] = CommentStoreComment::newUnsavedComment( $fields['comment'], $commentData );
			} else {
