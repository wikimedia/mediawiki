...
class RevisionStore implements IDBAccessObject, RevisionFactory, RevisionLookup {
	...
	private function getSlotsFromLegacySchema( $row ) {
		...
		if ( is_object( $row ) ) {
			...
			$main->cont_address = intval( $row->rev_text_id ); // FIXME: prefix?! // FIXME: encode flags?!
			...
			$main->data = isset( $row->old_text ) ? strval( $row->old_text ) : null;
			$main->data_flags = isset( $row->old_flags ) ? strval( $row->old_flags ) : 0;
		} elseif ( is_array( $row ) ) {
			$main->slot_page = isset( $row['page'] ) ? intval( $row['page'] ) : null;
			$main->slot_revision = isset( $row['id'] ) ? intval( $row['id'] ) : null;

			$main->cont_address = isset( $row['text_id'] ) ? intval( $row['text_id'] ) : null; // FIXME: prefix?! // FIXME: encode flags?!
			...
			$main->data = isset( $row['text'] ) ? rtrim( strval( $row['text'] ) ) : null;
			$main->data_flags = isset( $row['flags'] ) ? trim( strval( $row['flags'] ) ) : null;;
		...
		// FIXME: if model is not set, set a callback! May need Title!
		// FIXME: if format is not set, set a callback!
		// FIXME: if content is not set, set a callback! May need model and format!
	 ...
	 */
	private function getContentModel( RevisionRecord $rev, &$row ) { // FIXME: callback from SlotRecord
		if ( !$row->cont_model ) {
			$title = $rev->getTitle(); // FIXME: use a callback for finding the default model
			if ( $title ) {
				$role = isset( $row->slot_role ) ? $row->slot_role : 'main';
				// TODO: consider role in getDefaultModelFor()
				$row->cont_model = ContentHandler::getDefaultModelFor( $title );
			} else {
				// TODO: getDefaultModelForRole()
	 ...
	 */
	private function getContentFormat( RevisionRecord $rev, &$row ) { // FIXME: callback from SlotRecord
		if ( !$row->cont_format ) {
	...
	public function getNextRevision( Title $title, $revId ) {
		...
		if ( $next ) {
			return self::getRevisionByTitle( $title, $next );
		}
		return null;
	}
