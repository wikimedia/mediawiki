...
class RevisionStore implements IDBAccessObject, RevisionFactory, RevisionLookup {
	...
	public function newNullRevision( IDatabase $dbw, Title $title, $summary, $minor, User $user ) {
		...
		if ( $current ) {
			...
			$revision = MutableRevisionRecord::newFromArray( $title, $row, $this->emulateSlots_1_29( $row ) ); // FIXME
		} else {
			$revision = null;
		}
	...
	private function emulateSlots_1_29( $row ) {
		...
		if ( is_object( $row ) ) {
			...
			if ( isset( $row->old_text ) ) {
				...
				$mainSlotRow->blob_data = isset( $row->old_text ) ? strval( $row->old_text ) : null;
				$mainSlotRow->blob_flags = isset( $row->old_flags ) ? strval( $row->old_flags ) : '';
			} elseif ( isset( $row->ar_text ) ) {
				$mainSlotRow->blob_data = isset( $row->ar_text ) ? strval( $row->ar_text ) : null;
				$mainSlotRow->blob_flags = isset( $row->ar_flags ) ? strval( $row->ar_flags ) : '';
				$mainSlotRow->cont_address = 'ar:' . $row->ar_id; // probably not needed.
			}
			...
			$mainSlotRow->cont_model = isset( $row->rev_content_model ) ? strval( $row->rev_content_model ) : null;
			...
			$mainSlotRow->cont_model = isset( $row['content_model'] )
				? strval( $row['content_model'] ) : null;  // XXX: must be a string!
			// XXX: in the future, we'll probably always use the default format, and drop content_format
			$mainSlotRow->cont_format = isset( $row['content_format'] )
				? strval( $row['content_format'] ) : null;
			$mainSlotRow->blob_data = isset( $row['text'] ) ? rtrim( strval( $row['text'] ) ) : null;
			$mainSlotRow->blob_flags = isset( $row['flags'] ) ? trim( strval( $row['flags'] ) ) : null;;
			...
			if ( !empty( $row['content'] ) ) {
				...
				$mainSlotRow->cont_model =  $content->getModel();
		...
		$mainSlotRow->slot_since = $mainSlotRow->slot_revision;

		if ( $mainSlotRow->cont_model === null ) {
			$mainSlotRow->cont_model = function ( SlotRecord $slot ) {
		...
		if ( !$content ) {
			$content = function ( SlotRecord $slot ) use ( $mainSlotRow ) {
				return $this->loadSlotContent( $slot, $mainSlotRow );
			};
		}

		return new RevisionSlots( [ 'main' => new SlotRecord( $mainSlotRow, $content ) ] );
	}
	 ...
	 * @param object $slotRow The raw slot row, for context and additional information.
	 *
	 * @throws MWException
	 * @throws MWUnknownContentModelException
	 * @return Content
	 */
	private function loadSlotContent( SlotRecord $slot, $slotRow ) {
		// Database result rows based on some legacy schemas may contain the blob data directly.
		if ( isset( $slotRow->blob_data ) ) {
			$flags = isset( $slotRow->blob_flags ) ? $slotRow->blob_flags : '';
			$cacheKey = isset( $slotRow->cont_address ) ? $slotRow->cont_address : null;

			$data = $this->contentStore->expandBlob( $slotRow->blob_data, $flags, $cacheKey );
		} elseif ( isset( $slotRow->cont_address ) ) {
			$data = $this->contentStore->getBlob( $slotRow->cont_address );
		} else {
			throw new IncompleteRevisionException( 'The revision slot has no content address set.' );
		}

		// Unserialize content
		$handler = ContentHandler::getForModelID( $slot->getModel() );

		// NOTE: cont_format is legacy support. In the future, we probably want to always use
		// the default format, or leave it to ContentHandler::deserialize to auto-detect.
		$format = isset( $slotRow->cont_format ) ? $slotRow->cont_format : $handler->getDefaultFormat();
