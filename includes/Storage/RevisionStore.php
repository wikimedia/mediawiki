...
class RevisionStore
	...
	public function getQueryInfo( $options = [] ) {
		...
		if ( $this->migrationStage === MIGRATION_OLD ) {
			...
			$ret['joins']['slots'] = [ $join, "slot_revision_id = rev_id" ];
			$ret['joins']['content'] = [ $join, "content_id = slot_content_id" ];
			$ret['joins']['content_models'] = [ $join, "content_model = model_id" ];
			$ret['tables'][] = 'slots';
			$ret['tables'][] = 'content';
			...
			if ( $this->migrationStage < MIGRATION_NEW ) {
				...
				$ret['fields']['rev_content_model'] =
					'COALESCE( model_name, rev_content_model )';
			} else {
				...
				$ret['fields']['rev_content_model'] = 'model_name';
			}
		}
	...
	public function getArchiveQueryInfo() {
		...
		if ( $this->migrationStage === MIGRATION_OLD ) {
			...
			$join = $this->migrationStage === MIGRATION_NEW ? 'JOIN' : 'LEFT JOIN';
			$ret['joins']['slots'] = [ $join, "slot_revision_id = ar_rev_id" ];
			$ret['joins']['content'] = [ $join, "content_id = slot_content_id" ];
			$ret['joins']['content_models'] = [ $join, "content_model = model_id" ];
			$ret['tables'][] = 'slots';
			$ret['tables'][] = 'content';
			...
			if ( $this->migrationStage < MIGRATION_NEW ) {
				...
				$ret['fields']['ar_content_model'] = 'COALESCE( model_name, ar_content_model )';
			} else {
				// content_address will be in the form tt:123, so strip the first 3 characters
				$ret['fields']['ar_text_id'] = 'SUBSTR(content_address, 4)';
				// content_format does not have a place in the new schema, so just select NULL
				$ret['fields']['ar_content_format'] = 'NULL';
				$ret['fields']['ar_content_model'] = 'model_name';
			}
		}
