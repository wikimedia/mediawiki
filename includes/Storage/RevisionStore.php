...
class RevisionStore implements IDBAccessObject, RevisionFactory, RevisionLookup {
	...
	public function insertRevisionOn( RevisionRecord $rev, IDatabase $dbw, $flags = 0 ) {
		...
		$rev = $this->newRevisionFromRow( (object)$row, $slots, $title );
	...
	public function newNullRevision( IDatabase $dbw, Title $title, $summary, $minor, User $user ) {
		...
		if ( $current ) {
			...
			$revision = new DefaultRevisionRecord( $title, $row, $this->emulateSlots_1_29( $row ) );
		} else {
			$revision = null;
		}
	 ...
	 *
	 * @deprecated since 1.30.
	 * @todo Introduce RevisionBuilder for programmatically constructing and inserting a revision row.
	 *
	 * @param array $fields
	 * @param Title|null $title
	 *
	 ...
	 * @return RevisionRecord
	 */
	private function newRevisionFromArray( array $fields, RevisionSlots $slots, Title $title = null ) {
		if ( !$title ) {
