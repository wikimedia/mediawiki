...
class RevisionStore
	 ...
	 * @return int|null
	 */
	private function getMainSlotRoleIdOrNull() {
		try {
			return $this->slotRoleStore->getId( 'main' );
	...
	private function getSlotDataJoinString( IDatabase $dbr, $revIdJoinField ) {
		...
		// Do this by fetching the main slot ID from the store.
		// If it is not set this means there would be no chance of multiple slots anyway, so
		// don't then explicitly request it
		$mainSlotRoleIdOrNull = $this->getMainSlotRoleIdOrNull();
		if ( $mainSlotRoleIdOrNull === null ) {
			return $revIdJoinField . ' = a_slots.slot_revision_id';
		} else {
			return $dbr->makeList(
				[
					$revIdJoinField . ' = a_slots.slot_revision_id',
					'a_slots.slot_role_id = ' . $mainSlotRoleIdOrNull,
				],
				IDatabase::LIST_AND
			);
		}
