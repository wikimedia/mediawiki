...
class InterwikiSearchResultWidget implements SearchResultWidget {
	...
	public function __construct( SpecialSearch $specialSearch, LinkRenderer $linkRenderer, $iwPrefixDisplayTypes ) {
	...
	public function render( SearchResult $result, $terms, $position ) {
		...
		$displayType = $this->iwPrefixDisplayTypes[ $iwPrefix ];
		...
		if ( $redirectTitle !== null ) {
			...
			if ( $redirectText ) {
				$redirectText = new HtmlArmor( $titleSnippet );
			} else {
				$redirectText = null;
			}
