...
class SearchFormWidget {
	 ...
	 * @param string $profile The active search profile
	 * @return string HTML
	 */
	public function render( $profile, $term, $numResults, $totalResults, $offset, $isPowerSearch, $profile ) {
	...
	protected function profileTabsHtml( $profile, $term ) {
		$baseterm = $this->startsWithImage( $term )
	 ...
	 *
	 * @params string $term The string to check
	 * @return bool
	 */
	protected function startsWithImage( $term ) {
		...
		return count( $parts ) > 1
			? $wgConfLang->getNsIndex( $parts[0] ) === NS_FILE
			: false;
	}

	/**
	 * Make a search link with some taret namespaces
	 *
	 * @param string $term
	 ...
	 */
	protected function makeSearchLink( $term, $namespace, $label, $tooltip, array $params = [] ) {
	...
	protected function optionsHtml( $term, $isPowerSearch, $profile ) {
		...
		if ( $isPowerSearch ) {
			...
			$form = '';
			Hooks::run( 'SpecialSearchProfileForm', [ $this->specialSearch, &$form, $profile, $term, $opts ] );
			$html .= $form;
		}
	...
	protected function powerSearchBox( $term, array $opts ) {
		...
		if ( $user->isLoggedIn() ) {
			$remember .= Xml::checkLabel(
				...
				false,
				// Teh token goes here rather than in a hidden field so it
