...
class DefaultOutputTransform {
	...
	public function transform( ParserOutput $po, array $options = [] ): ParserOutput {
		...
		if (
			...
			$text = $this->extractBodyAndFixLinks( $text );
		}
	...
	/**
	 * Return only the contents inside the <body> tag.
	 * @param string $text HTML as a string
	 * @return string The contents of the <body> tag
	 * @internal
	 */
	...
	/**
	 * Return only the contents inside the <body> tag, and fix
	 * relative links.
	 * @param string $text
	 * @return string
	 */
	private function extractBodyAndFixLinks( string $text ): string {
