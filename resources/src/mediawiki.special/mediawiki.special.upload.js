...
( function ( mw, $ ) {
	...
	$( function () {
		...
		if ( mw.config.get( 'wgAjaxPatentPreview' ) && $patent.length ) {
			...
			$patent.change( function ( e ) {
				// We might show a preview
				uploadTemplatePreview.getPreview( $( e.currentTarget ), $( '#mw-patent-preview' ) );
			} );
		}
	...
	$( function () {
		...
		// If there is only 1 (default = no patent) patent option, then don't show it
		if ( $( 'input[name=wpPatent]' ).length <= 1 ) {
			togglePatentSelector( false );
		}
	} );
