...
( function ( $, mw, OO ) {
	...
	WidgetMethods = {
		...
		tokenWidget: {
			...
			fetchToken: function () {
				this.pushPending();
				return Util.fetchToken( this.paramInfo.tokentype )
		...
		capsuleWidget: {
			...
			setApiValue: function ( v ) {
				this.setItemsFromData( v === undefined || v === '' ? [] : v.split( '|' ) );
			},
			apiCheckValid: function () {
				var ok = this.getApiValue() !== undefined;
				this.setIcon( ok ? null : 'alert' );
		...
		submoduleWidget: {
			...
			multi: function () {
				...
				return v === undefined || v === '' ? [] : $.map( v.split( '|' ), function ( v ) {
	...
	Util = {
		 ...
		 * @return {jQuery.Promise}
		 */
		fetchToken: function ( tokenType ) {
			var api, apiPromise, deferred;

			if ( !fetchedTokens.hasOwnProperty( tokenType ) ||
				fetchedTokens[ tokenType ].state() === 'rejected'
			) {
				deferred = $.Deferred();
				fetchedTokens[ tokenType ] = deferred.promise();
				...
				apiPromise = api.post( {
					...
					if ( data.query.tokens[ tokenType + 'token' ] ) {
						deferred.resolve( data.query.tokens[ tokenType + 'token' ] );
					} else if ( data.warnings && data.warnings.tokens ) {
						deferred.reject( '???', data.warnings.tokens[ '*' ] );
					} else {
						deferred.reject( '???', 'No token returned' );
					}
				} ).fail( function ( code, details ) {
					if ( code === 'http' ) {
						details = 'HTTP error: ' + details.exception;
			...
			return fetchedTokens[ tokenType ];
		},

		/**
		 * Forget all CSRF tokens
		 */
		 ...
		 * @return {OO.ui.Widget}
		 */
		createWidgetForParameter: function ( pi ) {
	...
	*/
	mw.special.ApiSandbox = ApiSandbox = {
		...
		sendRequest: function () {
			var page, subpages, i, ajaxOptions, token, query, $result,
			...
			$.when.apply( $, deferreds ).done( function () {
				...
				// Extract 'token' so we can put it at the end (in case of query truncation)
				token = params.token;
				delete params.token;

				// If possible, use FormData so file uploads work. If not, just
				// x-www-form-urlencoded it.
				if ( window.FormData ) {
					ajaxOptions.data = new FormData();
					for ( i in params ) {
						ajaxOptions.data.append( i, params[ i ] );
					}
					if ( token !== undefined ) {
						ajaxOptions.data.append( 'token', token );
					}
					ajaxOptions.processData = false;
					ajaxOptions.contentType = false;
				} else {
					// Encode periods for IE bugs
					ajaxOptions.data = $.param( params ).replace( /\./g, '%2E' );
					if ( token !== undefined ) {
				...
				$.ajax( ajaxOptions )
					.fail( function ( xhr, textStatus, exception ) {
						var details = 'HTTP error: ' + exception;
						$result.empty()
							.append(
	...
	ApiSandbox.PageLayout.prototype.setupOutlineItem = function () {
		...
		this.outlineItem.setIcon( this.apiIsValid ? null : 'alert' );
		this.outlineItem.setIconTitle(
			this.apiIsValid ? '' : mw.message( 'apisandbox-alert-page' ).plain()
		);
	};
			...
			.done( function ( pi ) {
				...
				items.push( new OO.ui.FieldLayout(
					new OO.ui.Widget( {} ).toggle( false ), {
						align: 'top',
						label: $( $.parseHTML( pi.description ) )
					}
				) );
				...
				if ( pi.parameters.length ) {
						...
						switch ( pi.parameters[ i ].type ) {
							...
							case 'limit':
								if ( pi.parameters[ i ].highmax !== undefined ) {
									dl.append( $( '<dd>', {
										addClass: 'info',
										html: mw.message( 'api-help-param-limit2',
												pi.parameters[ i ].max, pi.parameters[ i ].highmax
											).parse()
									} ) );
								} else {
									dl.append( $( '<dd>', {
										addClass: 'info',
										html: mw.message( 'api-help-param-limit',
												pi.parameters[ i ].max
											).parse()
									} ) );
								}
							...
							case 'integer':
								...
								if ( tmp !== '' ) {
									dl.append( $( '<dd>', {
										addClass: 'info',
										html: mw.message( 'api-help-param-integer-' + tmp,
					...
					items.push( new OO.ui.FieldLayout(
						new OO.ui.Widget( {} ).toggle( false ), {
							align: 'top',
							label: mw.message( 'apisandbox-no-parameters' ).parse()
						}
					) );
				}
		...
		if ( this.paramInfo === null ) {
			...
			return $.when.apply( $, $.map( this.widgets, function ( widget ) {
				...
				if ( that.getOutlineItem() ) {
					that.getOutlineItem().setIcon( that.apiIsValid ? null : 'alert' );
					that.getOutlineItem().setIconTitle(
						that.apiIsValid ? '' : mw.message( 'apisandbox-alert-page' ).plain()
					);
				}
