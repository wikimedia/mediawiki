...
( function ( $, mw, OO ) {
	...
	*/
	...
	ApiSandbox.PageLayout.prototype.loadParamInfo = function () {
		var undocFieldset, undocParamNameWidget,
			that = this,
			addUndocWidget = function () {
				var name, layout, widget, button;

				// Check name is filled in
				name = undocParamNameWidget.getValue().trim();
				if ( name === '' ) {
					undocParamNameWidget.focus();
					return;
				}
				...
				if ( that.widgets[ name ] !== undefined ) {
					windowManager.openWindow( 'errorAlert', {
						title: mw.message(
							'apisandbox-undocumented-error-exists', name
						).parse(),
						actions: [
							{
								action: 'accept',
				...
				button.on( 'click', removeUndocWidget, [ name, layout ] );
				that.widgets[ name ] = widget;
				undocFieldset.addItems( [ layout ], undocFieldset.getItems().length - 1 );
				widget.focus();

				undocParamNameWidget.setValue( '' );
			},
			removeUndocWidget = function ( name, layout ) {
				undocFieldset.removeItems( [ layout ] );
		...
		Util.fetchModuleInfo( this.apiModule )
			.done( function ( pi ) {
				...
				if ( !pi.parameters.length && !Util.apiBool( pi.undocumentedparameters ) ) {
					items.push( new OO.ui.FieldLayout(
						new OO.ui.Widget( {} ).toggle( false ), {
							align: 'top',
				...
				if ( Util.apiBool( pi.undocumentedparameters ) ) {
					undocFieldset = new OO.ui.FieldsetLayout();
					undocParamNameWidget = new OO.ui.TextInputWidget( {
						placeholder: mw.message( 'apisandbox-undocumented-parameters-add-placeholder' ).text()
					} ).on( 'enter', addUndocWidget );
					undocFieldset.addItems( [
						new OO.ui.FieldLayout(
							new OO.ui.Widget( {} ).toggle( false ), {
								align: 'top',
								label: Util.parseHTML( pi.undocumentedparameters )
							}
						),
						new OO.ui.ActionFieldLayout(
							undocParamNameWidget,
							new OO.ui.ButtonWidget( {
								icon: 'add',
								flags: 'constructive'
							} ).on( 'click', addUndocWidget ),
							{
								label: mw.message( 'apisandbox-undocumented-parameters-add-label' ).text(),
								align: 'left'
							}
						)
					] );
					$( '<fieldset>' )
						.append(
							$( '<legend>' ).text( mw.message( 'apisandbox-undocumented-parameters' ).text() ),
							undocFieldset.$element
						)
