...
( function ( mw, $ ) {
	...
	function showCIDRResults( size, cidr ) {
	...
	*/
	function updateCIDRResult() {
		var form, blocs, bloc, binBlock, cidr, prefix, iplist, text, ips, binPrefix, prefixCidr, i,
			ipV4, addy, ipV6, foundV4, foundV6, ipCidr, bin, z, start, end, ip, ipCount, abbrevs,
			colons, insert, needed, intBlock, full, x;
		form = document.getElementById( 'mw-cidrform' );
		if ( !form ) {
			return; // no JS form
		}
		form.style.display = 'inline'; // unhide form (JS active)
		iplist = document.getElementById( 'mw-iplist' );
		if ( !iplist ) {
			return; // no JS form
		}
		text = iplist.value;
		...
		for ( i = 0; i < ips.length; i++ ) {
			...
			addy = ips[ i ].replace( /^\s*|\s*$/, '' ); // trim
			// Match the first IP in each list (ignore other garbage)
			ipV4 = mw.util.isIPv4Address( addy, true );
			ipV6 = mw.util.isIPv6Address( addy, true );
			ipCidr = addy.match( /^(.*)(?:\/(\d+))?$/ );
			// Binary form
			bin = '';
			x = 0;
			...
			// Convert the IP to binary form: IPv6
				...
				if ( abbrevs && abbrevs.length > 0 ) {
					colons = ip.match( /:/g );
					needed = 7 - ( colons.length - 2 ); // 2 from "::"
					insert = '';
					while ( needed > 1 ) {
						insert += ':0';
						needed--;
				...
				for ( x = 0; x <= 7; x++ ) {
					...
					intBlock = hex2int( bloc ); // convert hex -> int
