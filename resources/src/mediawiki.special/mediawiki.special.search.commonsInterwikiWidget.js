( function ( mw, $ ) {
	...
	var api = new mw.Api( { /* because ForeignApi doesn't allow requests from local */
			ajax: {
	...
	function itemWrapperTemplate( pageQuery, itemTemplateOutput ) {
		...
		return '<li class="iw-result iw-result--image">' +
				...
				'<div class="iw-result__footer">' +
					'<a href="https://commons.wikimedia.org/w/index.php?title=Special:Search&search=' + encodeURIComponent( pageQuery ) + '&fulltext=1" class="extiw" title="wikt:Special:Search">more results</a></div>' +
			'</li>';
