	...
	function adjustFancyCaptcha( $content, buttonSubmit ) {
		...
		if ( $captchaStuff.length ) {
			...
			$el = $submit.closest( 'div' ).before(
				mw.template.get( 'mediawiki.special.userlogin.common.js', 'resources/src/mediawiki.special/templates/captcha.html' ).render() );
			$el.find( 'label' ).text( mw.message( 'createacct-captcha' ).escaped() );
			$el.find( '#wpCaptchaWord' ).attr( 'tabindex', tabIndex ).
				attr( 'placeholder', mw.message( 'createacct-imgcaptcha-ph' ).escaped() );
