	...
	function adjustFancyCaptcha( $content, buttonSubmit ) {
		...
		if ( $captchaStuff.length ) {
			...
			$el = $submit.closest( 'div' ).before(
				mw.template.get( 'resources/src/mediawiki.special/templates/captcha.html' ).render() );
			$el.find( 'label' ).text( mw.message( 'createacct-captcha' ).escaped() );
			$el.find( '#wpCaptchaWord' ).attr( 'placeholder', mw.message( 'createacct-imgcaptcha-ph' ).escaped() );
