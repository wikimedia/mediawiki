		...
		init: function () {
			...
			try {
				...
				mw.hook( 'wikipage.collapsibleContent' ).add( function( $content ) {
					var $title = $( '<span>' )
						.addClass( 'recentchanges-panel-collapsed-title' )
						.text( mw.message( 'recentchanges-panel-collapsed-title' ).text() );

					if ( mw.user.options.get( 'rcpanelcollapsed' ) === true ) {
						$title.addClass( 'recentchanges-panel-collapsed-title-shown' );
					}

					$content.prepend( $title );

					$content.on( 'beforeExpand.mw-collapsible', function () {
						$title.hide();
					} );
					$content.on( 'afterCollapse.mw-collapsible', function () {
						$title.show();
					} );
				} );
			} catch ( err ) {
