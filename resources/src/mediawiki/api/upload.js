...
( function ( mw, $ ) {
	...
	$.extend( mw.Api.prototype, {
		 ...
		 * @param {File} file
		 * @param {Object} data Other upload options, see action=upload API docs for more
		 * @param {int} [chunkSize] Size (in bytes) per chunk (default: 5MB)
		 * @param {int} [chunkRetries] Amount of times to retry a failed chunk (default: 1)
		...
		chunkedUpload: function ( file, data, chunkSize, chunkRetries ) {
			...
			for ( start = chunkSize; start < file.size; start += chunkSize ) {
				...
				// then, but then we'd hit an `Uncaught RangeError: Maximum call
				// stack size exceeded`, at as low as 1024 calls in Firefox 47.
				// This'll work around it, but comes with the drawback of having
				// to properly relay the results to the returned promise.
		 ...
		 * @param {int} [retries] Amount of times to retry request
		 * @return {jQuery.Promise}
		 */
