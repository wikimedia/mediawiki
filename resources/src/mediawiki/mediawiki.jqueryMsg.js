...
( function ( mw, $ ) {
	...
	mw.jqueryMsg.htmlEmitter.prototype = {
		...
		plural: function ( nodes ) {
			...
			for ( formIndex = 0; formIndex < forms.length; formIndex++ ) {
				...
				if ( form.jquery && form.hasClass( 'mediaWiki_htmlEmitter' ) ) {
					...
					if ( /\d+=/.test( firstChildText ) ) {
						explicitPluralFormNumber = firstChildText.substr( 0, firstChildText.length - 1 );
						explicitPluralFormNumber = parseInt( explicitPluralFormNumber, 10 );
						// Use the digit part as key and rest of first text node and
						// rest of child nodes as value. Concat those two arrays.
						explicitPluralForms[explicitPluralFormNumber] =
							firstChildText.split( /=/ ).splice( 1 ).concat( form.contents().splice( 1 ) );
						forms[ formIndex ] = undefined;
					}
				}
				// Simple explicit plural forms like 12=a dozen
				if ( /^\d+=/.test( form ) ) {
					explicitPluralForms[ parseInt( form.split( /=/ )[0], 10 ) ] =
						form.slice( form.indexOf( '=' ) + 1, form.length );
					forms[ formIndex ] = undefined;
				}
			}
			...
			return forms.length ? this.language.convertPlural( count, forms, explicitPluralForms ) : '';
		},
