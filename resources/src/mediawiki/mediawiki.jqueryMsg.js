...
( function ( mw, $ ) {
	...
	mw.jqueryMsg.htmlEmitter.prototype = {
		...
		plural: function ( nodes ) {
			 ...
			 * of strings. However, we may be dealing with $( '<span>' ) objects
			 * (from concat) here, which it won't know what to do with.
			 * Let's just quickly grab the HTML and stuff that in a temp array
			 * to feed to convertPlural().
			 */
			for ( i = 0; i < forms.length; i++ ) {
				temp[i] = forms[i];
				if ( temp[i] instanceof jQuery ) {
					temp[i] = temp[i].html();
				}
			}
			 ...
			 * the temp array. The original string or span object will then be
			 * in the forms array at the same index.
			 */
