		...
		this.uploadPromise.then( function () {
			layout.upload.finishStashUpload().then( function () {
				...
				if ( layout.upload.getState() === mw.Upload.State.WARNING ) {
					...
					// TODO: Club these together and have filename-* messages
					if ( stateDetails.thumb-name !== undefined ) {
						deferred.reject( new OO.ui.Error( 'Can\'t upload thumbnail', { // TODO: Message
							recoverable: false
						} ) );
					}

					if ( stateDetails.bad-prefix !== undefined ) {
n						deferred.reject( new OO.ui.Error( 'Bad prefix automatically added from camera', { // TODO: Message (re-use messages already available in langauges other than en)
							recoverable: false
						} ) );
					}

					// TODO: These could be clubbed too MAYBE
					// Change the name if the current name isn't acceptable
					if ( stateDetails.duplicate-archive !== undefined ) {
						deferred.reject( new OO.ui.Error( 'duplicate archive', { // TODO: Message (use api-error-bad-archive)
							recoverable: false
						} ) );
					}
