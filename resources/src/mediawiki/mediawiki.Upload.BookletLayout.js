			...
			layout.upload.finishStashUpload().then( function () {
				...
				if ( layout.upload.getState() === mw.Upload.State.WARNING ) {
					...
					if ( stateDetails[ 'thumb-name' ] !== undefined ) {
						// TODO: Message
						deferred.reject( new OO.ui.Error( 'Can\'t upload thumbnail', {
							recoverable: false
						} ) );
					}

					if ( stateDetails[ 'bad-prefix' ] !== undefined ) {
						// TODO: Message (re-use messages already available in langauges other than en)
						deferred.reject( new OO.ui.Error( 'Bad prefix automatically added from camera', {
							recoverable: false
						} ) );
					}
					...
					if ( stateDetails[ 'duplicate-archive' ] !== undefined ) {
						// TODO: Message (use api-error-duplicate-archive)
						deferred.reject( new OO.ui.Error( 'duplicate archive', {
							recoverable: false
						} ) );
					}
