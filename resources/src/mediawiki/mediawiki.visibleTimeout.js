( function ( mw, document ) {
	 ...
	 *  calling fn.
	 * @return {Integer} A positive integer value which identifies the timer. This
	 *  value can be passed to clearVisibleTimeout() to cancel the timeout.
	 */
	mw.setVisibleTimeout = function ( fn, delay ) {
	 ...
	 */
	mw.clearVisibleTimeout = function ( visibleTimeoutId ) {
		if ( activeTimeouts.hasOwnProperty( visibleTimeoutId ) ) {
			activeTimeouts[ visibleTimeoutId ]();
		}
	};

	// Test-only function to override the shadowed document 
	mw.setVisibleTimeout.debugOverrideDocument = init;
}( mediaWiki, document ) );
