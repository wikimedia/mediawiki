...
( function ( mw, $ ) {
	...
	sanitize = function ( s, filter ) {
		...
		for ( i = 0, ruleLength = rules.length; i < ruleLength; ++i ) {
			...
			for ( m = 0, filterLength = filter.length; m < filterLength; ++m ) {
				if ( rule[filter[m]] ) s = s.replace( rule.pattern, rule.replace );
			}
		}
		return s;
	},
	...
	trimToByteLength = function ( s, length ) {
		...
		while ( ( byteLength = $.byteLength( s ) ) > length ) {
			...
			// One character in UTF-8 is at most 4 bytes
			// Using bitwise shift operator to avoid float numbers as a result
			// One character must be removed in any case because the
			// string is too long
			chopOffChars = Math.max( 1, chopOffBytes >> 2 );
