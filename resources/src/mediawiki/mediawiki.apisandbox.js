...
( function ( $, mw, OO ) {
	...
	WidgetMethods = {
		...
		capsuleWidget: {
			...
			setApiValue: function ( v ) {
				this.setValues( v === undefined || v === '' ? [] : v.split( '|' ) );
			},
			apiCheckValid: function () {
				var ok = this.getApiValue() !== undefined;
				this.setIcon( ok ? null : 'alert' );
			...
			switch ( pi.type ) {
				...
				case 'user':
					if ( pi.tokentype ) {
						...
						widget = new OO.ui.CapsuleWidget( {
							allowArbitrary: true
						} );
						widget.paramInfo = pi;
				...
				case 'timestamp':
					widget = new OO.ui.DateTimeInputWidget( {
						formatter: {
				...
				func = function () {
					if ( !innerWidget.isDisabled() ) {
						innerWidget.apiCheckValid().done( function ( ok ) {
							if ( ok ) {
								widget.addValues( [ innerWidget.getApiValue() ] );
				...
				if ( pi.parameters.length ) {
					...
					for ( i = 0; i < pi.parameters.length; i++ ) {
						...
						if ( Util.apiBool( pi.parameters[i].multi ) ) {
							tmp = [];
							if ( flag && !( widget instanceof OO.ui.CapsuleWidget ) &&
								!( widget instanceof OptionalWidget && widget.widget instanceof OO.ui.CapsuleWidget )
							) {
