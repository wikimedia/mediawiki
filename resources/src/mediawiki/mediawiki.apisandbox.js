...
( function ( $, mw, OO ) {
	...
	var ApiSandbox, Util, WidgetMethods, Validators,
		$content, panel, booklet, ApiSandbox, oldhash, windowManager,
		parseMsg = mw.jqueryMsg.getMessageFunction( {
			messages: mw.messages,
			format: 'parse',
			allowedHtmlElements: [ 'b', 'i', 'kbd', 'var', 'samp', 'code' ]
		} ),
		bookletPages = [],
		fetchedTokens = {},
		availableFormats = {},
	...
	WidgetMethods = {
		genericWidget: {
			...
			saneIsValid: function () {
				// WhyTF does OO.ui.TextInputWidget's isValid return a *promise*?
				return !Util.apiBool( this.paramInfo.required ) || this.getApiValue() !== '';
			},
			apiCheckValid: function () {
				var ok = this.saneIsValid();
		...
		tokenWidget: {
			...
			fetchToken: function () {
				...
				return Util.fetchToken( this.paramInfo.tokentype )
					...
					.always( this.setDisabled.bind( this, false ) );
			},
			setApiValue: function ( v ) {
				WidgetMethods.genericWidget.setApiValue.call( this, v );
				if ( v === '123ABC' ) {
					this.fetchToken();
				}
			}
		...
		dropdownWidget: {
			...
			setApiValue: function ( v ) {
				...
				if ( v === undefined ) {
					...
					items = menu.getItemsFromData( String( v ) );
				}
				if ( items.length ) {
					menu.selectItem( items[0] );
				} else if ( menu.getSelectedItem() ) {
					// There's no way to directly select no item. But this
					// should do it.
					menu.getSelectedItem().setSelected( false );
					menu.emit( 'select' );
					this.setLabel( '' );
				}
			},
		...
		optionalWidget: {
			...
			apiCheckValid: function () {
				return this.isDisabled() || this.widget.apiCheckValid();
			}
		},
		...
		submoduleWidget: {
			...
			multi: function () {
				...
				return v === undefined ? [] : $.map( v.split( '|' ), function ( v ) {
	...
	Validators = {
		limit: function () {
			...
			v = this.getApiValue();
			if ( !Util.apiBool( this.paramInfo.required ) && v === '' ) {
				return true;
			}
			...
			return !isNaN( n ) &&
				n >= ( pi.min || 0 ) &&
				n <= ( mw.config.get( 'apihighlimits' ) ? pi.highmax : pi.max );
		},
		integer: function () {
			var v, n,
				pi = this.paramInfo;
			...
			if ( Util.apiBool( pi.enforcerange ) ) {
				return !isNaN( n ) &&
					( pi.min === undefined || n >= pi.min ) &&
					( pi.max === undefined || n <= pi.max );
			} else {
				return !isNaN( n );
			}
		}
	...
	Util = {
		...
		createWidgetForParameter: function ( pi ) {
			var widget, finalWidget;

			var dummyWidget = $.extend( new OO.ui.Widget(), {
				getApiValue: function () { return undefined; },
				setApiValue: function () {},
				apiCheckValid: function () { return true; },
				paramInfo: pi
			} );
			...
			switch ( pi.type ) {
				case 'boolean':
					...
					widget.paramInfo = pi;
					$.extend( widget, WidgetMethods.checkboxWidget );
				...
				case 'user':
					...
					widget.paramInfo = pi;
					$.extend( widget, WidgetMethods.genericWidget );
					if ( pi.tokentype ) {
				...
				case 'integer':
					...
					widget.paramInfo = pi;
					$.extend( widget, WidgetMethods.genericWidget );
					widget.saneIsValid = Validators.integer;
					if ( Util.apiBool( pi.enforcerange ) ) {
						widget.$element.find( 'input' ).attr( {
							min: pi.min,
							max: pi.max,
							step: 1
						} );
				...
				case 'limit':
					...
					widget.paramInfo = pi;
					$.extend( widget, WidgetMethods.genericWidget );
					widget.saneIsValid = Validators.limit;
					widget.$element.find( 'input' ).attr( {
						min: pi.min || 0,
						max: mw.config.get( 'apihighlimits' ) ? pi.highmax : pi.max,
						step: 1
					} );
					break;

				case 'timestamp':
					/** @todo Replace this with a real DateTimeInputWidget */
					widget = new OO.ui.TextInputWidget( {
						type: 'datetime-local',
						required: Util.apiBool( pi.required )
					} );
				...
				default:
					...
					if ( Util.apiBool( pi.multi ) ) {
						...
						if ( Util.apiBool( pi.submodules ) ) {
							...
							/** @todo Get the right event here */
							widget.on( '???', ApiSandbox.updateUI );
						}
					} else {
						widget = new OO.ui.DropdownWidget( {
							menu: {
			...
			/** @todo Handle multi in some way besides telling people to separate values with '|'? */
			...
			if ( Util.apiBool( pi.required ) ) {
				...
				if ( widget.getSubmodules ) {
					...
					finalWidget.on( 'disable', ApiSandbox.updateUI );
				}
	...
	mw.libs.ApiSandbox = ApiSandbox = {
		...
		init: function () {
			...
			$content
				.empty()
				.append( $( '<p>' ).append( parseMsg( 'apisandbox-intro' ) ) )
		 ...
		 *
		 * There isn't native support for "fit viewport" in OO.ui.PanelLayout.
		 */
		...
		sendRequest: function () {
			...
			if ( !ok ) {
				windowManager.openWindow( 'errorAlert', {
					title: parseMsg( 'apisandbox-submit-invalid-fields-title' ),
					message: parseMsg( 'apisandbox-submit-invalid-fields-message' ),
					actions: [
						{
							action: 'accept',
							label: OO.ui.msg( 'ooui-dialog-process-dismiss' ),
				...
				.append(
					new OO.ui.FieldLayout(
						new OO.ui.TextInputWidget( {
							...
							label: parseMsg( 'apisandbox-request-url-label' )
						}
					).$element,
					$result
				);
		...
		fixTokenAndResend: function () {
			var page, subpages, i, k,
				...
				checkPages = [ pages.main ],
				success = function ( k, token ) {
					delete tokenWait[k];
					if ( ok && $.isEmptyObject( tokenWait ) ) {
						ApiSandbox.sendRequest();
					}
				},
				failure = function ( k, code, error ) {
					delete tokenWait[k];
					ok = false;
				};
				...
				if ( pi.parameters.length ) {
					...
					for ( i = 0; i < pi.parameters.length; i++ ) {
						...
						switch ( pi.parameters[i].type ) {
							...
							case 'limit':
								if ( pi.parameters[i].highmax !== undefined ) {
									dl.append( $( '<dd>', {
										addClass: 'info',
										html: parseMsg( 'api-help-param-limit2',
									...
									dl.append( $( '<dd>', {
										addClass: 'info',
										html: parseMsg( 'api-help-param-limit',
												pi.parameters[i].max
											)
									} ) );
								}
							...
							case 'integer':
								...
								if ( tmp !== '' ) {
									dl.append( $( '<dd>', {
										addClass: 'info',
										html: parseMsg( 'api-help-param-integer-' + tmp,
						...
						if ( Util.apiBool( pi.parameters[i].multi ) ) {
							tmp = [];
							if ( flag ) {
								tmp.push( parseMsg( 'api-help-param-multi-separate' ) );
							}
							if ( count > pi.parameters[i].lowlimit ) {
								tmp.push(
									parseMsg( 'api-help-param-multi-max',
					...
					items.push( new OO.ui.FieldLayout(
						new OO.ui.Widget( {} ).toggle( false ), {
							align: 'top',
							label: parseMsg( 'apisandbox-no-parameters' )
						}
					) );
				}
	...
	ApiSandbox.PageLayout.prototype.apiCheckValid = function () {
		if ( this.paramInfo === null ) {
			...
			$.each( this.widgets, function ( name, widget ) {
				ok = widget.apiCheckValid() && ok;
			} );
			this.apiIsValid = ok;
			if ( this.getOutlineItem() ) {
				this.getOutlineItem().setIcon( ok ? null : 'alert' );
			}
			return ok;
		}
	};
	...
	ApiSandbox.PageLayout.prototype.getSubpages = function () {
		var prefix = this.apiModule !== 'main' ? this.apiModule + '+' : '',
			ret = [];
		$.each( this.widgets, function ( name, widget ) {
			var submodules, i;
	...
	OO.mixinClass( TextInputWithIndicatorWidget, OO.ui.TabIndexedElement );
	...
	OptionalWidget.prototype.onOverlayClick = function ( checked ) {
