...
( function ( $, mw ) {
	...
	mw.ForeignStructuredUpload.BookletLayout.prototype.renderUploadForm = function () {
		var fieldset,
			...
			ownWorkMessage = '<p>' + mw.message( 'foreign-structured-upload-form-label-own-work-message-' + target, targetName ).text() ).parse() + '</p>',
			notOwnWorkMessage = $( '<div>' ).append(
				$( '<p>' ).append( mw.message( 'foreign-structured-upload-form-label-not-own-work-message-' + target ).parse(), targetName ),
				$( '<p>' ).append( mw.message( 'foreign-structured-upload-form-label-not-own-work-local-' + target ).parse(), localName )
			),
			layout = this;
