...
( function ( $, mw ) {
	...
	mw.ForeignStructuredUpload.BookletLayout.prototype.renderUploadForm = function () {
		var fieldset,
			ownWorkMessage = '<p>' + mw.message( 'foreign-structured-upload-form-label-own-work-message' ).parse() + '</p>',
			notOwnWorkMessage = '<p>' + mw.message( 'foreign-structured-upload-form-label-not-own-work-message' ).parse() + '</p>',
			layout = this;
