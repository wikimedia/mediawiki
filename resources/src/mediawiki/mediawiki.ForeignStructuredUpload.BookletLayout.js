...
( function ( $, mw ) {
	...
	mw.ForeignStructuredUpload.BookletLayout.prototype.renderUploadForm = function () {
		var fieldset,
			...
			targetMsg = target === 'default' ? 'shared-repo' : 'shared-repo-name-' + target,
			targetName = mw.message( targetMsg ).text(),
			$ownWorkMessage = $( '<p>' ).html(
				mw.message( 'foreign-structured-upload-form-label-own-work-message-' + target, targetName ).parse()
			),
			$notOwnWorkMessage = $( '<div>' ).append(
				$( '<p>' ).html(
					mw.message( 'foreign-structured-upload-form-label-not-own-work-message-' + target, targetName ).parse()
				),
				$( '<p>' ).html(
					mw.message( 'foreign-structured-upload-form-label-not-own-work-local-' + target, localName ).parse()
				)
			),
			layout = this;
