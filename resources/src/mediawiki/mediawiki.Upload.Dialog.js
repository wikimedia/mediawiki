( function ( $, mw ) {
	 ...
	 * {@link mw.Upload.StackLayout#event-fileUploaded fileUploaded},
	 * and {@link mw.Upload.StackLayout#event-fileSaved fileSaved} events can
	...
	mw.Upload.Dialog.prototype.initialize = function () {
		...
		this.uploadStack = this.createUploadStack();
		this.uploadStack.connect( this, {
			panelNameSet: 'onPanelNameSet',
			uploadValid: 'onUploadValid',
			infoValid: 'onInfoValid'
		} );

		this.$body.append( this.uploadStack.$element );
	};

	/**
	 * Create an upload stack
	 *
	 * @return {mw.Upload.StackLayout} An upload stack
	 */
	mw.Upload.Dialog.prototype.createUploadStack = function () {
		return new mw.Upload.StackLayout();
	};
	 ...
	 */
	mw.Upload.Dialog.prototype.onPanelNameSet = function ( panelName ) {
	...
	mw.Upload.Dialog.prototype.getActionProcess = function ( action ) {
		...
		if ( action === 'upload' ) {
			return new OO.ui.Process( this.uploadStack.uploadFile() );
		}
		if ( action === 'save' ) {
			return new OO.ui.Process( this.uploadStack.saveFile() );
		}
		if ( action === 'insert' ) {
