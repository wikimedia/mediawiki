( function ( mw, $ ) {
	...
	UP.uploadToStash = function () {
		...
		return this.api.uploadToStash( this.file, {
			filename: this.filename
		} ).then( function ( cb ) {
			upload.state = Upload.State.STASHED;
			upload.stashUploadCompletion = cb;
		}, function () {
			upload.state = Upload.State.ERROR;
		} );
	};
	...
	UP.finishStashUpload = function () {
		...
		if ( !this.stashUploadCompletion || this.state !== Upload.State.STASHED ) {
			return $.Deferred().reject( 'This upload is not in the stash, please upload it to the stash first.' );
		}

		this.state = Upload.UPLOADING;

		return this.stashUploadCompletion( {
			watchlist: ( this.watchlist === true ) ? 1 : undefined,
			comment: this.comment,
			filename: this.filename,
			text: this.text
