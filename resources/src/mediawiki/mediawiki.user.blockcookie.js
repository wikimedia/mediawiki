( function ( mw ) {
	...
	mw.requestIdleCallback( function () {
		...
		// 1. there's no cookie and an expired localStorage
		// 2. there's no cookie and an unexpired localStorage; and
		// 3. there is a cookie and nothing in localStorage.

		cookieValue = mw.cookie.get( 'BlockID' );
		if ( !cookieValue && storedObject && storedObject.expiry < ( mw.now() / 1000 ) ) {
			// localStorage has expired, so remove it.
