	...
	mw.requestIdleCallback( function () {
		...
		if ( storedValue ) {
			...
			if ( storedObject.expiry === undefined ) {
				// If there's no 'expiry' property this must be an old-style localStorage item,
		...
		// 1. there is a cookie and an expired localStorage item;
		// 2. there's no cookie and there is something in localStorage; and
		// 3. there is a cookie and there's nothing in localStorage.

		cookieValue = mw.cookie.get( 'BlockID' );
		if ( cookieValue && storedObject && storedObject.expiry < ( mw.now() / 1000 ) ) {
			// localStorage has expired, so we delete it. It may be set again immediately (below).
		...
		if ( !cookieValue && storedObject ) {
			...
			// The storedObject variable at this point always has the 'data' and 'expiry' properties
			// (expiry is in seconds; mw.now() is in milliseconds).
