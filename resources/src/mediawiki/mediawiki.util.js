	...
	var util = {
		...
		getUrl: function ( str, params ) {
			...
			if ( typeof str === 'string' ) {
				...
				urlFragmentStart = url.indexOf( '%23' );
				if ( titleFragmentStart !== -1 && urlFragmentStart !== -1 ) {
					fragment = pageName.slice( titleFragmentStart + 3 );
					// Exclude the fragment from the URL
					url = url.slice( 0, urlFragmentStart ) + url.slice( urlFragmentStart + 3 + fragment.length );
				}
			}
