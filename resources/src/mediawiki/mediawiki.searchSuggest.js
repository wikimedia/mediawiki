...
( function ( mw, $ ) {
    ...
    mw.searchSuggest = {
		...
		install: function () {
			console.log( 'installing search suggester' );
			// Generic suggestions functionality for all search boxes
			var searchboxesSelectors = [
			...
			$( searchboxesSelectors.join( ', ' ) )
				.suggestions( mw.searchSuggest.config )
				...
				// is messed up in <body>.  (they use 2 elements to get a sane
				// font-height). So, instead of making exceptions for each skin
				// or adding more stylesheets, just copy it from the active
				// element so auto-fit.
				.each( function () {
					var $this = $( this );
					$this
	...
	function onBeforeUpdate() {
		...
		if ( searchText && searchText !== mw.searchSuggest.previousSearchText ) {
			mw.track( 'mediawiki.searchSuggest', {
				action: 'session-start'
			} );
		}
		mw.searchSuggest.previousSearchText = searchText;
	}
	...
	$( document ).ready( function () {
		...
		var $searchRegion = $( '#simpleSearch, #searchInput' ).first(),
		...
		mw.searchSuggest.previousSearchText = $searchInput.val();
		mw.searchSuggest.install();
