( function ( mw, $ ) {
	...
	mw.rcfilters.UriProcessor.prototype.updateModelBasedOnQuery = function ( uriQuery ) {
		var parameters = this.getNormalizedQueryParams( uriQuery || new mw.Uri().query );
	 ...
	 */
	mw.rcfilters.UriProcessor.prototype.getUpdatedUriQuery = function ( currentUriQuery ) {
		...
		if ( this.getVersion( currentUriQuery ) !== 2 ) {
			...
			// URLVersion 2 is always added after the first load, so
			// in essence, this is only for the first time we
			// load things, which is why it's safe to assume we
			// can in this case override the model state.
			// In general, we build the URL on initialization,
			// add urlversion=2 and then have the model re-read
			// it to get updated. In all later interactions, the
			// model is now synchronized to the URL so all changes
			// to the **model itself** update the URL, which is
			// when the paramsFromModel (without being merged with
			// defaults) override the state of the current query.
			$.extend(
				true,
				paramsFromModel,
				this._minimizeQuery( this.filtersModel.getDefaultParams() )
			);
		}
	 ...
	 */
	mw.rcfilters.UriProcessor.prototype.getNormalizedQueryParams = function ( uriQuery ) {
		...
		return this._minimizeQuery( $.extend( true, base, uriQuery, { urlversion: '2' } ) );
	};
