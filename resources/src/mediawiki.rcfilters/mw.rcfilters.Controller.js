( function ( mw, $ ) {
	...
	mw.rcfilters.Controller.prototype._cancelLiveUpdate = function () {
			...
			.then( function ( data ) {
				...
				if ( data.changes !== 'NO_RESULTS' ) {
					if ( this.changesListModel.getLiveUpdate() ) {
						return this.updateChangesList( null, true, true );
					} else {
	...
	mw.rcfilters.Controller.prototype.showNewChanges = function () {
		return this.updateChangesList( null, true, false );
	};
	 ...
	 * @param {boolean} [dontInvalidateCurrentChanges=false] Invalidate old changes by default (show spinner)
	 * @return {jQuery.Promise} Promise that is resolved when the update is complete
	 */
	mw.rcfilters.Controller.prototype.updateChangesList = function ( params, isLiveUpdate, dontInvalidateCurrentChanges ) {
		if ( params ) {
			this._updateURL( params );
		}
		if ( !dontInvalidateCurrentChanges ) {
