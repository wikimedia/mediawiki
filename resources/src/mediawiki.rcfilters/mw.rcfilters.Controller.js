( function ( mw, $ ) {
	...
	mw.rcfilters.Controller.prototype.initialize = function ( filterStructure, namespaceStructure ) {
		...
		if (
			...
			this._updateModelState(
				$.extend( true,
					// We're ignoring the highlight and nsinvert parameters,
					...
					// should retain their value when we update the model state
					{
						...
						nsinvert: String( Number( uri.query.nsinvert ) )
					},
	...
	mw.rcfilters.Controller.prototype._getUpdatedUri = function () {
		...
		Object.keys( highlightParams ).forEach( function ( paramName ) {
			...
			if ( highlightParams[ paramName ] !== baseParams[ paramName ] ) {
