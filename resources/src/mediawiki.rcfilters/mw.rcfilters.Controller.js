	...
	mw.rcfilters.Controller.prototype.initialize = function ( filterStructure, namespaceStructure, tagList, conditionalViews ) {
		...
		$.each( views, function ( viewName, viewData ) {
			...
			if ( viewData.async && viewData.getResultCallback ) {
				// mainGroupData = viewData.groups.filter( function ( groupData ) {
				// 	return groupData.name === viewData.mainGroupName;
				// } )[ 0 ] || {};
				// mainGroupData.isDynamic = true;
				// The search group should be on top
				viewData.groups.unshift( {
					name: 'searchresults_' + viewName,
	...
	mw.rcfilters.Controller.prototype.setSearch = function ( searchQuery ) {
		...
		if ( !this.searchInProgress && viewData.async && viewData.getResultCallback ) {
			...
			promise = viewData.getResultCallback( this.filtersModel.removeViewTriggers( searchQuery ) )
				.then( function ( items ) {
					controller.filtersModel.setSearchResultItems( viewData.name, items );
				} )
				.then( function () {
		...
		return promise.then( function () {
			...
			controller.filtersModel.setSearch( searchQuery );
		} );
	};
