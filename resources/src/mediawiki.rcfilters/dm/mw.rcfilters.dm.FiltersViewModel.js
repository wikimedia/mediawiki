( function ( mw, $ ) {
	...
	mw.rcfilters.dm.FiltersViewModel.prototype.reassessFilterInteractions = function ( item ) {
		var itemHasConflict,
			itemGroup = this.groups[ item.getGroup() ],
			updateData = {},
			conflictingGroups = [],
			model = this,
		...
		// Check for conflicts (conflicted filters)
		// Go over all items and check if they still have conflicts
		// This is the most straight-forward way to go; toggling 1 item
		// can affect the 'conflict' status of multiple others by changing
		// the state of the group it is in.
		// Instead of recursing into lists of conflicts and validating that
		// potential conflicts aren't already conflicting with other conflicts,
		// the most straight-forward method is to go over all items again and
		// revalidate their states.
		this.getItems().forEach( function ( filterItem ) {
			filterItem.toggleConflicted( model.doesItemHaveConflicts( filterItem ) );
		} );
	};

	/**
	 * Go over all groups and check if any of them is in conflict with this item
	 *
	 * @param {mw.rcfilters.dm.FilterItem} item Given item
	 * @return {boolean} Item is in conflict
	 */
	mw.rcfilters.dm.FiltersViewModel.prototype.doesItemHaveConflicts = function ( item ) {
		var hasConflict = false;

		if ( !item.getConflicts().length ) {
			return false;
		}

		$.each( this.groups, function ( groupName, groupModel ) {
			if ( !hasConflict ) {
				hasConflict = groupModel.areAllSelectedConflictWith( item );
				if ( hasConflict ) {
					console.log( item.getName(), 'is in conflict with', groupName, item.getConflicts() );
				}
			}
		} );
	...
	/**
	 * Get all the selected filters that are currently in conflict with the given item
	 *
	 * @param {[type]} item [description]
	 * @return {[type]} [description]
	 */
	mw.rcfilters.dm.FiltersViewModel.prototype.getCurrentActiveConflicts = function ( item, ignoreItem ) {
		var model = this;

		return item.getConflicts().filter( function ( conflictingFilterName ) {
			var conflictingFilter = model.getItemByName( conflictingFilterName );

			if ( ignoreItem && ignoreItem.getName() === conflictingFilterName ) {
				return false;
			}

			return (
				// If the conflicting item is selected
				conflictingFilter.isSelected() &&
				// If all selected items in the conflictingFilter's group are
				// also in conflict with item
				model.groups[ conflictingFilter.getGroup() ].areAllSelectedConflictWith( item )
			);
		} );
	};

	/**
	 * Check whether any of the given items are selected
	 *
	 * @param {mw.rcfilters.dm.FilterItem} item Requested items to check
	 * @return {boolean} Any of the items are selected
	 */
	mw.rcfilters.dm.FiltersViewModel.prototype.areAnySelected = function ( items ) {
		...
		$.each( filters, function ( group, data ) {
			...
			for ( i = 0; i < data.filters.length; i++ ) {
				...
				if ( data.filters[ i ].subset ) {
					data.filters[ i ].subset.forEach( function ( subsetFilterName ) {
				...
				if ( data.filters[ i ].conflicts ) {
					...
					data.filters[ i ].conflicts.forEach( function ( conflictingFilterName ) {
	 ...
	 *
	 * @param {string} groupName Group name
	 * @return {mw.rcfilters.dm.FilterGroup} Group model; null if no group was found
	 */
