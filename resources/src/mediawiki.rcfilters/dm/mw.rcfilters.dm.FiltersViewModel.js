( function ( mw, $ ) {
	...
	mw.rcfilters.dm.FiltersViewModel.prototype.reassessFilterInteractions = function ( item ) {
		var model = this,
			 ...
			 * This method also checks whether the itemInSubset has any other item
			 * that is active and affects it; if so, we are not changing its state.
			 * If there are no other affecting items, we change the included state
			 * of the itemInSubset to reflect the selection state of the affecting item.
			 *
			 * @method
			 * @param {mw.rcfilters.dm.FilterItem} itemAffecting Item potentially affecting the other
			 * @param {mw.rcfilters.dm.FilterItem} itemInSubset Item potentially in subset of itemAffecting
			 */
			changeIncludedIfNeeded = function( itemAffecting, itemInSubset ) {
				// itemInSubset is in itemAffecting's subsets. Check first if itemInSubset
		...
		iterationItems.forEach( function ( checkedItem ) {
			...
			checkedItem.getSubset().forEach( function ( filterItemName ) {
				var filterItem = model.getItemByName( filterItemName );
				// Check if the filterItem in the subset is affected by
				// anything else. If it does, we skip it, if it doesn't,
				// we change its state
				changeIncludedIfNeeded( checkedItem, filterItem );
			} );
		} );
	};
