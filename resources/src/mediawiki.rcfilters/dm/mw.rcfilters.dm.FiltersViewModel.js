( function ( mw, $ ) {
	...
	mw.rcfilters.dm.FiltersViewModel.prototype.onFilterItemUpdate = function ( item ) {
		...
		this.updateParameters( this.getParametersFromFilters() );
	...
	mw.rcfilters.dm.FiltersViewModel.prototype.reapplyActiveFilters = function ( item ) {
		var excludedFilters, exclusionMap,
			group = item.getGroup(),
			model = this,
		...
		if (
			...
			if ( item.hasExcludedFilters() ) {
				...
				item.getExcludedFilters().forEach( function ( filterName ) {
					...
					model.excludedByMap[ filterName ].forEach( function ( anotherExcludingFilterName ) {
						var anotherExcludingFilter;
						if (
							!excludedByAnotherSelectedFilter &&
							anotherExcludingFilter !== item.getName()
						) {
							anotherExcludingFilter = model.getItemByName( anotherExcludingFilterName );
							if ( anotherExcludingFilter.isSelected() ) {
								excludedByAnotherSelectedFilter = true;
							}
						}
					} );

					if ( !excludedByAnotherSelectedFilter ) {
			...
			for ( i = 0; i < data.filters.length; i++ ) {
				...
				if ( excludedFilters.length ) {
					...
					excludedFilters.forEach( function ( filterName ) {
						model.excludedByMap[ filterName ] = model.exclusionMap[ filterName ] || [];
	...
	mw.rcfilters.dm.FiltersViewModel.prototype.isParameterGroupValid = function ( groupName ) {
		...
		if ( this.groups[ groupName ] ) {
			...
			if ( this.groups[ groupName ].type === 'send_unselected_if_any' ) {
				...
				// the group is invalid (or, if any params in the group is 0, the group is valid)
				return this.groups[ groupName ].filters.some( function ( filterItem ) {
					// We have to cast to a number here, because both "1" and "0" are
					// strings, so both !"1" = true and !"0" = true
					return !Number( model.parameters[ filterItem.getName() ] );
				} );
				...
				// 1. Unique, and
				// 2. The word "all", or
				// 3. Every value is a valid filter name
				...
				// Unrecognized group type, by default, is always valid.
				return true;
			}
		}
	...
	mw.rcfilters.dm.FiltersViewModel.prototype.areAllParameterGroupsValid = function () {
		...
		return Object.keys( this.groups ).every( function ( groupName ) {
			return model.isParameterGroupValid( groupName );
		} );
	};
				...
				if ( data.type === 'send_unselected_if_any' ) {
					...
					data.filters.forEach( function ( filterItem ) {
						...
						if ( model.parameters[ filterItem.getName() ] ) {
							// Fix its value
							paramGroup[ filterItem.getName() ] = 0;
						}
					} );
