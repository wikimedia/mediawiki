( function ( mw, $ ) {
	 ...
	 */
	mw.rcfilters.dm.SavedQueriesModel = function MwRcfiltersDmSavedQueriesModel( config, filtersModel ) {
		config = config || {};
	...
	mw.rcfilters.dm.SavedQueriesModel.prototype.convertToParameters = function ( data ) {
		// Filters
		data.filters = this.filtersModel.getParametersFromFilters( data.filters );

		// Highlights
		$.each( data.highlights, function ( filterName, color ) {
			data.highlights[ filterName + '_color' ] = color;
		} );

		// Highlight toggle
		data.highlight = String( Number( data.highlights.highlight ) );
		// Invert toggle
		data.invert = String( Number( data.invert || 0 ) );
	...
	mw.rcfilters.dm.SavedQueriesModel.prototype.getMinimalParamList = function ( valuesObject ) {
		var result = { filters: {}, highlights: {}, invert: valuesObject.invert, highlight: valuesObject.highlight },
