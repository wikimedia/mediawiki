( function ( mw ) {
	...
	mw.rcfilters.ui.FilterTagMultiselectWidget = function MwRcfiltersUiFilterTagMultiselectWidget( controller, model, config ) {
		...
		// Parent
		mw.rcfilters.ui.FilterTagMultiselectWidget.parent.call( this, $.extend( {
	...
	mw.rcfilters.ui.FilterTagMultiselectWidget.prototype.onMenuChoose = function ( item ) {
		// Override this to do nothing. The operation of adding and removing
		// items through the menu isn't by the selection state but by the
		// model (items send their changed selection/highlight state to the
		// controller, that updates the model, causing an item change)
		return false;
	};
	...
	mw.rcfilters.ui.FilterTagMultiselectWidget.prototype.createMenuWidget = function ( menuConfig ) {
		...
		return new mw.rcfilters.ui.FilterFloatingMenuSelectWidget( $.extend( config, menuConfig ) );
	};
