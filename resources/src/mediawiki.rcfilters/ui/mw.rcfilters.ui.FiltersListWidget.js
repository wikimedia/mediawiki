( function ( mw, $ ) {
	 ...
	 * @cfg {string|jQuery} [label] Title for the list
	 */
	...
	/**
	 * Respond to model item update event
	 *
	 * @param {mw.rcfilters.dm.FilterItem} item Filter item
	 */
	mw.rcfilters.ui.FiltersListWidget.prototype.onModelItemUpdate = function ( item ) {
		var i, groupWidget,
			widgets = this.getItems();

		// Find the group
		for ( i = 0; i < widgets.length; i++ ) {
			if ( widgets[ i ].getName() === item.getGroup() ) {
				groupWidget = widgets[ i ];
				break;
			}
		}
		...
		if ( groupWidget ) {
			groupWidget.toggleValidState( this.model.isParameterGroupValid( item.getGroup() ) );
		}
	};
	 ...
	 *
	 * @param {[type]} showNoResults Show no results message
	 */
