( function ( mw ) {
	 ...
	 * @param {mw.rcfilters.dm.ItemModel} filterModel Item model
	...
	mw.rcfilters.ui.ItemMenuOptionWidget = function MwRcfiltersUiItemMenuOptionWidget(
		...
		mw.rcfilters.ui.ItemMenuOptionWidget.parent.call( this, $.extend( {
			...
			label: this.filterModel.getLabel()
		}, config ) );

		this.checkboxWidget = new mw.rcfilters.ui.CheckboxInputWidget( {
			value: this.filterModel.getName(),
			selected: this.filterModel.isSelected()
		} );
		...
		if ( this.filterModel.getDescription() ) {
			$label.append(
				$( '<div>' )
					.addClass( 'mw-rcfilters-ui-itemMenuOptionWidget-label-desc' )
					.text( this.filterModel.getDescription() )
			);
		}
		...
		this.excludeLabel.toggle( this.filterModel.isSelected() && this.invertModel.isSelected() );
		...
		this.$element
			...
			.addClass( 'mw-rcfilters-ui-itemMenuOptionWidget-view-' + this.filterModel.getGroupModel().getView() )
			.append(
				$( '<div>' )
		...
		if ( this.filterModel.getIdentifiers() ) {
			this.filterModel.getIdentifiers().forEach( function ( ident ) {
	...
	mw.rcfilters.ui.ItemMenuOptionWidget.prototype.updateUiBasedOnState = function () {
		this.checkboxWidget.setSelected( this.filterModel.isSelected() );
