...
( function ( $, mw ) {
	...
	mw.widgets.CalendarWidget.prototype.setValue = function ( value ) {
		this.moment = value ? moment( value, this.getFormat() ) : moment();
		this.setValueFromMoment();
	...
	mw.widgets.CalendarWidget.prototype.setValueFromMoment = function () {
		...
		if ( this.value !== newValue ) {
			this.value = newValue;
			this.emit( 'change', this.value, this /*TODO remove me*/ );
		}
	};
	...
	$( function () {
		function update( value, asd ) {
			asd.$element.next().text( value );
		}

		$( '#mw-content-text' ).empty().append(
			$( '<div>' ).append(
				$( '<p>' ).text( 'Select day:' ),
				new mw.widgets.CalendarWidget()
					.on( 'change', update )
					.$element,
				$( '<p>' )
			),
			$( '<div>' ).append(
				$( '<p>' ).text( 'Select month:' ),
				new mw.widgets.CalendarWidget( { precision: 'month' } )
					.on( 'change', update )
					.$element,
				$( '<p>' )
			)
