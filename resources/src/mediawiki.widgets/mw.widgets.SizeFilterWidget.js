...
( function ( $, mw ) {
	 ...
	 * @uses OO.ui.RadioSelectWidget
	 * @uses OO.ui.TextInputWidget
	 *
	 * @constructor
	 * @param {Object} [config] Configuration options
	 * @cgf {boolean} [selectMin=true] Whether to select 'min', false would select 'max'
	 */
	mw.widgets.SizeFilterWidget = function MwWidgetsSizeFilterWidget( config ) {
		...
		// Properties
		this.radioselect = new OO.ui.RadioSelectWidget( { items: [
			new OO.ui.RadioOptionWidget( { data: 'min', label: mw.msg( 'minimum-size' ) } ),
			new OO.ui.RadioOptionWidget( { data: 'max', label: mw.msg( 'maximum-size' ) } )
		] } );
		...
		// Initialization
		var selectData = config.selectMin ? 'min' : 'max';
		this.radioselect.selectItemByData( selectData );
		this.$element
	...
	mw.widgets.SizeFilterWidget.static.reusePreInfuseDOM = function ( node, config ) {
		...
		config.radioselect = OO.ui.RadioSelectWidget.static.reusePreInfuseDOM(
	...
	mw.widgets.SizeFilterWidget.static.gatherPreInfuseState = function ( node, config ) {
		...
		state.radioselect = OO.ui.RadioSelectWidget.static.gatherPreInfuseState(
			$( node ).find( '.oo-ui-radioSelectWidget' ),
			config.dropdowninput
		);
