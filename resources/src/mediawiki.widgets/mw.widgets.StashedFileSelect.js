...
( function ( $, mw, OO ) {
	...
	mw.widgets.StashedFileSelectWidget = function MWWStashedFileSelectWidget( config ) {
		...
		// Parent constructor
		mw.widgets.StashedFileSelectWidget.parent.call( this, config );
		...
		this.$thumbnail = $( '<div>' ).addClass( 'mediawiki-widgets-stashedFileSelectWidget-thumbnail' ).append( '&nbsp;' );
		this.setPendingElement( this.$thumbnail );

		this.$thumbContain = $( '<div>' )
			.addClass( 'mediawiki-widgets-stashedFileSelectWidget-thumbnail-container' )
	...
	OO.mixinClass( mw.widgets.StashedFileSelectWidget, OO.ui.mixin.PendingElement );
	...
	mw.widgets.StashedFileSelectWidget.prototype.updateUI = function () {
		...
		if ( this.filekey ) {
			...
			$label = $( [] );
			$filetype = $( '<span>' )
				.addClass( 'mediawiki-widgets-stashedFileSelectWidget-fileType' );
