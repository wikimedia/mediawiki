	...
	mw.widgets.TitleInputWidget.prototype.getLookupMenuOptionsFromData = function ( data ) {
		...
		for ( index in data.pages ) {
			...
			for ( i = 0, len = redirects.length; i < len; i++ ) {
				pageData[redirects[i]] = {
					...
					description: mw.msg( 'visualeditor-linkinspector-description-redirect', suggestionPage.title )
				};
		...
		if ( !pageExists ) {
			pageData[this.value] = {
				...
				description: mw.msg( 'visualeditor-linkinspector-description-new-page' )
			};
		}
		...
		for ( i = 0, len = titles.length; i < len; i++ ) {
			...
			items.push( new mw.widgets.TitleOptionWidget( {
				data: titles[i],
				imageUrl: this.showImages ? page.imageUrl : null,
				description: this.showDescriptions ? page.description : null,
