...
( function ( mw, $ ) {
	...
	$( function () {
		var editBox, scrollTop, $editForm,
			summaryCharLimit = mw.config.get( 'wgCommentCharacterLimit' ),
		...
		// (Showing 'chars' is still confusing with the character limit, since it's not obvious
		// that e.g. combining diacritics or zero-width punctuation count as characters.)
		if ( summaryCharLimit ) {
			mw.widgets.visibleCharLimit( wpSummary, summaryCharLimit );
