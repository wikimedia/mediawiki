...
( function ( mw, $ ) {
	...
	function doLivePreview( e ) {
		var isDiff, api, request, postData,
			$wikiPreview, $editform, copySelectors, $copyElements, $spinner;

		e.preventDefault();

		isDiff = ( e.target.id === 'wpDiff' );
		...
		if ( isDiff ) {
			...
			request = api.post( postData ).then( function ( response1 ) {
				...
				return api.post( postData ).then( function ( result2 ) {
					...
					try {
						var diff = result2.query.pages[result2.query.pageids[0]]
							.revisions[0].diff['*'];
			...
			$.extend( postData, {
				...
				disablepp: '',
				preview: '',
				prop: 'text|modules|categorieshtml|templates|langlinks',
				...
				if ( response.parse.templates ) {
					...
					$.each( response.parse.templates, function ( i, template ) {
						li = $( '<li>' )
							.append( $('<a>')
								.attr( {
									'href': mw.config.get( 'wgArticlePath' ).replace( '$1', template['*'] ),
