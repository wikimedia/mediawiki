...
( function ( mw, $ ) {
	...
	$( function () {
		...
		function stashEdit( priority, useTextHash ) {
			if ( pending ) {
				pending.abort();
			}
			...
			api.getToken( 'csrf' ).then( function ( token ) {
				...
				var hash = ( useTextHash && lastTextHash !== null ) ? lastTextHash : null;
...
alert(hash);
				var req = pending = api.post( {
					...
					if ( data.stashedit && data.stashedit.status === 'stashed' ) {
		...
		function onEditorIdle() {
			...
			stashEdit( priority, !textChanged );
		}
