...
( function ( $ ) {
	...
	CompletenessTest.fn = CompletenessTest.prototype = {
		...
		walkTheObject: function ( currName, currVar, masterVariable, parentPathArray, action ) {
			var key, value, curPathArray,
				type = util.type( currVar ),
				that = this;

			curPathArray = currName ? parentPathArray.concat( currName ) : parentPathArray.slice();

			// Hard ignores
			if ( this.ignoreFn( currVar, that, curPathArray ) ) {
				return null;
			}
			...
			if ( this.lazyCounter > this.lazyLimit ) {
				log( 'CompletenessTest.fn.walkTheObject> Limit reached: ' + this.lazyCounter, curPathArray );
				return null;
			}
			...
			if ( type === 'function' ) {
				...
				if ( !currVar.prototype || util.isEmptyObject( currVar.prototype ) ) {
					...
					if ( action === CompletenessTest.ACTION_INJECT ) {
						...
						that.injectionTracker[ curPathArray.join( '.' ) ] = true;
						that.injectCheck( masterVariable, curPathArray, function () {
							that.methodCallTracker[ curPathArray.join( '.' ) ] = true;
						} );
					}
					...
					if ( action === CompletenessTest.ACTION_INJECT ) {
						...
						for ( key in currVar.prototype ) {
							if ( hasOwn.call( currVar.prototype, key ) ) {
								...
								that.walkTheObject( key, value, masterVariable, curPathArray.concat( 'prototype' ), action );
							}
						}
			...
			if ( type === 'function' || type === 'object' ) {
				for ( key in currVar ) {
					if ( hasOwn.call( currVar, key ) ) {
						...
						that.walkTheObject( key, value, masterVariable, curPathArray, action );
					}
				}
			}
		},
