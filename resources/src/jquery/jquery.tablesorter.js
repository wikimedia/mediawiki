		...
		construct: function ( $tables, settings ) {
			return $tables.each( function ( i, table ) {
				...
				$headers.on( 'keypress click', function ( e ) {
					var header, $header, columns, newSortList, i,
						totalRows,
						j, s, o;
					...
					if ( totalRows > 0 ) {
						...
						$header.data( {
							order: $header.data( 'count' ) % 2,
							count: $header.data( 'count' ) + 1
						} );

						// Get current column index
						columns = config.headerToColumns[ $header.data( 'headerIndex' ) ];
						newSortList = $.map( columns, function ( c ) {
							// jQuery "helpfully" flattens the arrays...
							return [ [ c, $header.data( 'order' ) ] ];
						} );
