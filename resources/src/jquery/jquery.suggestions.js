 ...
 * @cfg {Object} options
 *
 * @cfg {Function} [options.fetch] Callback that should fetch suggestions and set the suggestions
 *  property. Called in context of the text box.
 * @cfg {string} options.fetch.query
 * @cfg {Function} options.fetch.response Callback to receive the suggestions with
 * @cfg {Array} options.fetch.response.suggestions
 * @cfg {number} options.fetch.maxRows
 *
 * @cfg {Function} [options.cancel] Callback function to call when any pending asynchronous
 *  suggestions fetches. Called in context of the text box.
 *
 * @cfg {Object} [options.special] Set of callbacks for rendering and selecting.
 *
 * @cfg {Function} options.special.render Called in context of the suggestions-special element.
 * @cfg {string} options.special.render.query
 * @cfg {Object} options.special.render.context
 *
 * @cfg {Function} options.special.select Called in context of the suggestions-result-current element.
 * @cfg {jQuery} options.special.select.$textbox
 *
 * @cfg {Object} [options.result] Set of callbacks for rendering and selecting
 *
 * @cfg {Function} options.result.render Called in context of the suggestions-result element.
 * @cfg {string} options.result.render.suggestion
 * @cfg {Object} options.result.render.context
 *
 * @cfg {Function} options.result.select Called in context of the suggestions-result-current element.
 * @cfg {jQuery} options.result.select.$textbox
 *
 * @cfg {Object} [options.update] Set of callbacks for listening to a change in the text input.
 *
 * @cfg {Function} options.update.before Called right after the user changes the textbox text.
 * @cfg {Function} options.update.after Called after results are updated either from the cache or
 * the API as a result of the user input.
 *
 * @cfg {jQuery} [options.$region=this] The element to place the suggestions below and match width of.
 *
 * @cfg {string[]} [options.suggestions] Array of suggestions to display.
 *
 * @cfg {number} [options.maxRows=10] Maximum number of suggestions to display at one time.
 *  Must be between 1 and 100.
 *
 * @cfg {number} [options.delay=120] Number of milliseconds to wait for the user to stop typing.
 *  Must be between 0 and 1200.
 *
 * @cfg {boolean} [options.cache=false] Whether to cache results from a fetch.
 *
 * @cfg {number} [options.cacheMaxAge=60000] Number of milliseconds to cache results from a fetch.
 *  Must be higher than 1. Defaults to 1 minute.
 *
 * @cfg {boolean} [options.submitOnClick=false] Whether to submit the form containing the textbox
 *  when a suggestion is clicked.
 *
 * @cfg {number} [options.maxExpandFactor=3] Maximum suggestions box width relative to the textbox
 ...
 *
 * @cfg {string} [options.expandFrom=auto] Which direction to offset the suggestion box from.
 *  Values 'start' and 'end' translate to left and right respectively depending on the directionality
 *   of the current document, according to `$( 'html' ).css( 'direction' )`.
 *   Valid values: "left", "right", "start", "end", and "auto".
 *
 * @cfg {boolean} [options.positionFromLeft] Sets `expandFrom=left`, for backwards
 *  compatibility.
 *
 * @cfg {boolean} [options.highlightInput=false] Whether to highlight matched portions of the
 *  input or not.
 */
( function ( $ ) {
	...
	$.suggestions = {
		 ...
		 *
		 * @param {jQuery.plugin.suggestions} context
		 */
