...
( function ( $ ) {
	 ...
	 */
	$.trimByteLength = function ( safeVal, newVal, byteLimit, fn, countFn ) {
	 ...
	 *
	 * @param {mixed} [limit] Limit to enforce, fallsback to maxLength-attribute,
	 *  called with fetched value as argument. If object, has two members, byte and codepoint
	 *  in order to set the limit in bytes, and in unicode codepoints. If number, then its
	 *  just the byte limit.
	...
	$.fn.byteLimit = function ( limit, fn ) {
		...
		return this.each( function ( i, el ) {
			...
			$el.on( eventKeys, function () {
				...
				var firstResultTrimmed = res.trimmed;

				if ( limit !== undefined && limit.codepoint !== undefined ) {
					res = $.trimByteLength(
						prevSafeVal,
						res.newVal,
						limit.codepoint,
