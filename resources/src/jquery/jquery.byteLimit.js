...
( function ( $ ) {
	 ...
	 */
	$.trimByteLength = function ( safeVal, newVal, byteLimit, filterFn ) {
		var lengthFn;
		if ( filterFn ) {
	...
	/**
	 * Utility function to trim down a string, based on charLimit
	 * and given a safe start position. It supports insertion anywhere
	 * in the string, so "foo" to "fobaro" if limit is 4 will result in
	 * "fobo", not "foba". Basically emulating the native maxlength by
	 * reconstructing where the insertion occurred.
	 ...
	 * function, if none, pass empty string.
	 * @param {string} newVal New value that may have to be trimmed down.
	 * @param {number} charLimit Number of characters the value may be in size.
	 * @param {Function} [filterFn] See jQuery#charLimit.
	 * @return {Object}
	 * @return {string} return.newVal
	 * @return {boolean} return.trimmed
	 */
	$.trimCharLength = function ( safeVal, newVal, charLimit, filterFn ) {
		var lengthFn;
		if ( filterFn ) {
			lengthFn = function ( val ) {
				return $.charLength( filterFn( val ) );
			};
		} else {
			lengthFn = $.charLength;
		}

		return trimLength( safeVal, newVal, charLimit, lengthFn );
	};
	...
	/**
	 * Enforces a character limit on an input field.
	 *
	 ...
	 * @chainable
	 */
	$.fn.charLimit = function ( limit, filterFn ) {
		return lengthLimit.call( this, $.trimCharLength, limit, filterFn );
	};
	...
	function lengthLimit( trimFn, limit, filterFn ) {
		...
		return this.each( function ( i, el ) {
			...
			if ( filterFn || !allowNativeMaxlength ) {
				...
				// faster by there being less left to interate over). This does not work for $.charLimit
