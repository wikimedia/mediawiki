...
mediaWiki.language.convertGrammar = function ( word, form ) {
	var grammarForms, aou, origWord, lastFiveCharacters, lastFourCharacters,
		matches, lastCharacterIsNumeric, newLastFiveCharacters;
	...
	switch ( form ) {
		...
		case 'allative':
			...
			// allative
			lastFiveCharacters = word.substr( -5 );
			lastFourCharacters = word.substr( -4 );
			matches = lastFiveCharacters.match( /[bcdfghjklmnpqrstvwxz]/ );
			...
			lastCharacterIsNumeric = jQuery.isNumeric( word.substr( -1, 1 ) );
			if ( matches ) {
				...
				newLastFiveCharacters = lastFiveCharacters.replace( ( matches[0] + matches[0] ), matches[0] );
