...
// Rough polyfill for Array.prototype.includes, used in MW 1.43 backport
const includes = ( arr, x, from ) => arr.indexOf( x, from ) !== -1;
...
const priv = {
	...
	isKnownZoneName( zoneName ) {
		...
		return Intl.supportedValuesOf && includes( Intl.supportedValuesOf( 'timeZone' ), zoneName );
	},
...
class DateFormatter {
	...
	formatInternal( style, type, date ) {
		...
		if ( pattern ) {
			...
			if ( includes( pattern, '{mwZoneOffset}' ) ) {
