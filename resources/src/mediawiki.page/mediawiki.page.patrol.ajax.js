...
( function ( mw, $ ) {
	...
	$( function () {
		...
		$patrolLinks.on( 'click', function ( e ) {
			var $spinner, href, rcid, apiRequest, result,
				i = 0,
				rcids = [];
			...
			.done( function ( data ) {
				...
				if ( data.patrol !== undefined ) {
					// Success
					// When called with a single rcid, action=patrol returns an object for B/C
					result = $.isArray( data.patrol ) ? data.patrol : [ data.patrol ];
					var title = new mw.Title( result[0].title );
					mw.notify( mw.msg(
						'markedaspatrollednotify', title.toText(), result.length
					) );
				} else {
