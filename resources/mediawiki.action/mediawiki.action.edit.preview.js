...
( function ( mw, $ ) {
	...
	function doLivePreview( e ) {
		var $wikiPreview, copySelectors, $copyElements, $spinner,
		...
		targetUrl = $( '#editform' ).attr( 'action' ) || mw.util.wikiScript();
	...
	$( document ).ready( function () {
		...
		if ( !document.getElementById( 'p-lang' ) && document.getElementById( 'p-tb' ) ) {
			$( '#p-tb' ).after( '<div id="p-lang"></div>' );
		}

		if ( !$( '.mw-summary-preview' ).length ) {
			$( '.editCheckboxes' ).before('<div class="mw-summary-preview"></div>' );
		}

		if ( !document.getElementById( 'wikiDiff' ) && document.getElementById( 'wikiPreview' ) ) {
			$( '#wikiPreview' ).after( '<div id="wikiDiff"></div>' );
		}
