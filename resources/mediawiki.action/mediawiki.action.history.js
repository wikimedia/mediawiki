...
( function ( mw, $ ) {
	$( document ).ready( function () {
		...
		function getDiffURI( oldid, diff ) {
			return mw.config.get( 'wgServer' ) + mw.config.get( 'wgScript' ) + '?title=' + mw.util.wikiUrlencode( mw.config.get( 'wgPageName' ) ) + '&diff=' + diff + '&oldid=' + oldid;
		}
		...
		function convertToLinks() {
			var $oldidRadio = $( '[name="oldid"]:checked', $historyCompareForm ),
			    $diffRadio = $( '[name="diff"]:checked', $historyCompareForm ),

			    $compareButtons = $( '.mw-history-compareselectedversions-button', $historyCompareForm ),
		...
		function updateDiffElements() {
			var diffLi = false, // the li where the diff radio is checked
			    ...
			    $checkedOldidRadio = null,
			    $checkedDiffRadio = null;
			...
			if ( $checkedOldidRadio !== null && $checkedDiffRadio !== null ) {
				$compareLinks.attr( 'href', getDiffURI( $checkedOldidRadio.val(), $checkedDiffRadio.val() ) );
			}
