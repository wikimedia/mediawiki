...
( function (mw, $) {
	...
	function loadPage( page ) {
		...
		$spinner.css( {
			height: $multipageimage.height(),
			width: $multipageimage.width()
		} );

		$multipageimage.html( $spinner ).load(
	...
	function ajaxifyPageNavigation() {
		...
		$( '.multipageimagenavbox' ).one( 'click', 'a', function() {
			loadPage( this.href );
			return false;
		} );

		// Prevent the submission of the page select form and instead call loadPage
		$( 'form[name="pageselector"]' ).one( 'change submit', function() {
