	...
	$( document ).ready( function () {
		...
		$patrolLinks.on( 'click', function ( event ) {
			...
			.done( function ( data ) {
				...
				$( '.mw-spinner-ajax-patrol' ).remove();
				$patrolLinks.closest( '.patrollink' ).remove();
				if ( data.patrol !== undefined ) {
					// Success
					var title = new mw.Title( data.patrol.title, data.patrol.ns ).toText(),
						msg = mw.message( 'markedaspatrollednotify', title );
					mw.notify( msg );
				} else {
