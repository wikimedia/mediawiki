		...
		loader: ( function () {
			...
			return {
				...
				store: {
					...
					// group). Browsers that don't implement all the prerequisite APIs (JSON, Navigation
					// Timing, or Web Storage) are ineligible. Eligible browsers have a 0.1% chance of
					// being included in the experiment, in which case they are equally likely to be
					// assigned to either the experimental or control group.
					experimentGroup: ( function () {
						var bucket, random;

						if ( !window.JSON || !window.performance || !window.performance.timing ) {
							return 0;
						}
					 ...
					 */
					init: function () {
						var raw, data, inExperimentalGroup;
						...
						inExperimentalGroup = mw.loader.store.experimentGroup === 2;
						if ( !( mw.config.get( 'wgResourceLoaderStorageEnabled' ) || inExperimentalGroup ) || mw.config.get( 'debug' ) ) {
