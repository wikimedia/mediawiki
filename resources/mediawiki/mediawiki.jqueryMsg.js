...
( function ( mw, $ ) {
	...
	function getFailableParserFn( options ) {
		...
		return function ( args ) {
			...
			try {
				...
				if ( parser.settings.emitter === 'text' ) {
	...
	mw.jqueryMsg.parser = function ( options ) {
		...
		if ( this.settings.emitter === 'text' ) {
			this.emitter = new mw.jqueryMsg.textEmitter( this.settings.language, this.settings.magic );
		} else if ( $.isFunction( this.settings.emitter ) ) {
			this.emitter = new this.settings.emitter( this.settings.language, this.settings.magic );
		} else if ( typeof this.settings.emitter === 'object' && this.settings.emitter !== null ) {
		...
		wikiTextToAst: function ( input ) {
			...
			function extlink() {
				...
				if ( parsedResult !== null ) {
					return [ 'LINK', parsedResult[1], parsedResult[3] ];
				}
				// One part extlink [a]
				var parsedResult = sequence( [
					openExtlink,
	...
	mw.jqueryMsg.Emitter.prototype = {
		...
		/**
		 * Return the next numeric extlink text (ie: "[1]")
		 * For use in LINK implementations.
		 * @return {String} in the form "[#]"
		 */
		nextExtlink: function () {
			this.extlinkCount++;
			return '[' + this.extlinkCount + ']';
		},
