...
( function ( mw, $ ) {
	...
	Notification.prototype.start = function () {
		...
		if ( options.tag && $tagMatches.length ) {
			...
			self.$replacementPlaceholder = $( '<div>' )
				...
				.insertAfter( $tagMatches.length ? $tagMatches[$tagMatches.length-1] : $notification )
	...
	// Wait for page.startup so that mw.util.init is ready,
	// since we need mw.util.$content.
	mw.loader.using( 'mediawiki.page.startup', function () {
		$( function () {
			var notif;
