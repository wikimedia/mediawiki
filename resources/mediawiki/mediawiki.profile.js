...
( function ( mw, $ ) {
	...
	mw.Debug.Profile = {
		...
		buildTimeline: function( group ) {
			...
			rect = this.createElement( 'line' );
			rect.setAttribute( 'class', 'mw-debug-profile-underline' );
			rect.setAttribute( 'x1', 0 );
			rect.setAttribute( 'y1', 28 );
			rect.setAttribute( 'x2', this.width );
			rect.setAttribute( 'y2', 28 );
		...
		buildPeriod: function( period ) {
			var start = this.data.timespan.start,
				timelineHeadPosition = ( period.start - start ) * this.ratio,
				timelineTailPosition = ( period.end - start ) * this.ratio,
			...
			if ( timelineHeadPosition + 16 > timelineTailPosition ) {
				...
				e.setAttribute( 'points', pointList( [
					[ timelineHeadPosition, 8 ],
					[ timelineHeadPosition, 19 ],
					[ timelineHeadPosition + 8, 8 ],
					[ timelineHeadPosition, 8]
				] ) );
				g.appendChild( e );
				...
				e = this.createElement( 'polygon' );
				e.setAttribute( 'points', pointList( [
					[ timelineTailPosition, 8 ],
					[ timelineTailPosition, 19 ],
					[ timelineTailPosition - 8, 8 ],
					[ timelineTailPosition, 8 ],
				] ) );
				g.appendChild( e );
		...
		attachFlyout: function() {
			...
			$( '.mw-debug-profile-period', this.e ) .on( 'mouseenter', function( e ) {
