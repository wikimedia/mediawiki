 ...
 * @mixins OO.EventEmitter
 *
 * Managed are mutually exclusive. If a window is opened while there is a current window already
 * opening or opened, the current window will be closed without data. Empty closing data should
 * always result in the window being closed without causing constructive or destructive action.
 *
 ...
 * @cfg {boolean} [modal=true] Prevent interaction outside the top-most dialog, ignored if
 *   `exclusive` is false
 */
...
OO.ui.WindowManager.prototype.removeWindows = function ( names ) {
	...
	for ( i = 0, len = names.length; i < len; i++ ) {
		...
		promises.push( this.closeWindow( name ).then( function () {
	...
	function proceed( step ) {
		return function () {
			...
			if ( typeof result === 'number' ) {
				...
				deferred = $.Deferred();
				setTimeout( deferred.resolve, Math.max( result, 0 ) );
				return deferred.promise();
			}
...
OO.ui.Process.prototype.createStep = function ( step, context ) {
	...
	if ( typeof step === 'number' ) {
		deferred = $.Deferred();
		setTimeout( deferred.resolve, Math.max( step, 0 ) );
		step = deferred;
	}

	if ( $.isFunction( step.promise ) ) {
		callback = function () {
			return step.promise();
		};
	} else if ( $.isFunction( step ) ) {
		callback = step;
	} else {
 ...
 * Get error message.
 *
 * @return {string|jQuery} Error message
 */
 ...
 * @cfg {number} [accessKey] Button's access key
 */
OO.ui.ButtonedElement = function OoUiButtonedElement( $button, config ) {
	...
	this.toggleFrame( config.framed !== undefined ? !!config.framed : true );
};
 ...
 * @chainable
 */
OO.ui.ButtonedElement.prototype.toggleFrame = function ( framed ) {
	framed = framed === undefined ? !this.framed : !!framed;
	if ( framed !== this.framed ) {
		this.framed = framed;
		this.$element.toggleClass( 'oo-ui-buttonedElement-frameless', !framed );
		this.$element.toggleClass( 'oo-ui-buttonedElement-framed', framed );
	}
 ...
 * Set tab index.
 *
 * @param {number} tabIndex Button's tab index, use -1 to prevent tab focusing
 * @chainable
 */
OO.ui.ButtonedElement.prototype.setTabIndex = function ( tabIndex ) {
	this.$element.prop( 'tabIndex', tabIndex );
	return this;
};
...
OO.ui.PopuppableElement = function OoUiPopuppableElement( config ) {
	...
	this.popup = new OO.ui.PopupWidget( $.extend(
		{ 'autoClose': true }, config.popup, { '$': this.$, '$autoCloseIgnore': this.$element }
	) );
};
...
/**
 * Dialog with actions in the foot.
 *
 * @abstract
 * @class
 ...
 * @param {Object} [config] Configuration options
 */
OO.ui.ActionDialog = function OoUiActionDialog( manager, config ) {
	// Parent constructor
	OO.ui.ActionDialog.super.call( this, manager, config );
	...
	// Initialization
	this.$element.addClass( 'oo-ui-actionDialog' );
};

/* Inheritance */

OO.inheritClass( OO.ui.ActionDialog, OO.ui.Dialog );
 ...
 * @property {Object[]}
 */
OO.ui.ActionDialog.static.actions = [];
 ...
 * @property {boolean}
 */
OO.ui.ActionDialog.static.escapable = true;
 ...
 * @param {jQuery.Event} e Key down event
 */
OO.ui.ActionDialog.prototype.onFrameDocumentKeyDown = function ( e ) {
 ...
 * @param {OO.ui.ActionWidget} action Action that was resized
 */
OO.ui.ActionDialog.prototype.onActionResize = function () {
 ...
 * @param {OO.ui.ActionWidget} action Action that was clicked
 */
OO.ui.ActionDialog.prototype.onActionClick = function ( action ) {
	if ( !this.isPending() ) {
 ...
 */
OO.ui.ActionDialog.prototype.onActionsChange = function () {
	this.detachActions();
	if ( !this.isClosing() ) {
		this.attachActions();
	}
 ...
 * @return {boolean}
 */
OO.ui.ActionDialog.prototype.isPending = function () {
	return !!this.pending;
};
 ...
 */
OO.ui.ActionDialog.prototype.getActions = function () {
	return this.actions;
};
 ...
 */
OO.ui.ActionDialog.prototype.getActionProcess = function ( action ) {
	return new OO.ui.Process()
		.next( function () {
			if ( !action ) {
				this.close();
 ...
 */
OO.ui.ActionDialog.prototype.getSetupProcess = function ( data ) {
	data = data || {};

	// Parent method
	return OO.ui.ActionDialog.super.prototype.getSetupProcess.call( this, data )
		.next( function () {
			var i, len,
				items = [],
 ...
 * @inheritdoc
 */
OO.ui.ActionDialog.prototype.getTeardownProcess = function ( data ) {
	// Parent method
	return OO.ui.ActionDialog.super.prototype.getTeardownProcess.call( this, data )
		.first( function () {
			this.actions.clear();
			this.currentAction = null;
		}, this );
 ...
 * @inheritdoc
 */
OO.ui.ActionDialog.prototype.initialize = function () {
	// Parent method
	OO.ui.ActionDialog.super.prototype.initialize.call( this );
	...
	// Initialization
	this.frame.$content.addClass( 'oo-ui-actionDialog-content' );
};

/**
 * Attach action actions.
 */
OO.ui.ActionDialog.prototype.attachActions = function () {
 ...
 * @chainable
 */
OO.ui.ActionDialog.prototype.detachActions = function () {
	var i, len;
 ...
 */
OO.ui.ActionDialog.prototype.executeAction = function ( action ) {
	this.pushPending();
 ...
 * @chainable
 */
OO.ui.ActionDialog.prototype.pushPending = function () {
	if ( this.pending === 0 ) {
 ...
 * @chainable
 */
OO.ui.ActionDialog.prototype.popPending = function () {
	if ( this.pending === 1 ) {
...
OO.inheritClass( OO.ui.MessageDialog, OO.ui.ActionDialog );
...
OO.ui.MessageDialog.prototype.attachActions = function () {
	...
	if ( special.safe ) {
		...
		special.safe.toggleFrame( false );
	}
	if ( others.length ) {
		for ( i = 0, len = others.length; i < len; i++ ) {
			other = others[i];
			this.$actions.append( other.$element );
			other.toggleFrame( false );
		}
	}
	if ( special.primary ) {
		this.$actions.append( special.primary.$element );
		special.primary.toggleFrame( false );
	}
...
OO.inheritClass( OO.ui.ProcessDialog, OO.ui.ActionDialog );
 ...
 * @inheritdoc
 */
OO.ui.ProcessDialog.prototype.setLabel = function () {
	// Mixin method
...
OO.ui.ProcessDialog.prototype.showErrors = function ( errors ) {
	var i, len, $errorItem, message,
		recoverable = true;

	this.$errorItems = this.$( [] );
	for ( i = 0, len = errors.length; i < len; i++ ) {
		if ( !errors[i].isRecoverable() ) {
			recoverable = false;
		}
		$errorItem = this.$( '<div>' ).addClass( 'oo-ui-processDialog-error' );
		...
		this.$errorItems = this.$errorItems.add( $errorItem );
	}
 ...
 *
 * @param {string} href Hyperlink location
 */
OO.ui.ButtonWidget.prototype.setHref = function ( href ) {
	href = typeof href === 'string' && href ? href : null;
 ...
 *
 * @param {string} target Hyperlink target
 */
