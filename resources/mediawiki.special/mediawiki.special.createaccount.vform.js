...
jQuery( function( $ ) {
	...
	if ( $captchaStuff.length ) {
		...
		$submit.closest( 'div' )
			.before( [
		'<div>',
			'<label for="wpCaptchaWord">Security check</label>',
			'<div class="mw-createacct-captcha-container">',
				'<img id="mw-createacct-captcha" alt="PLACEHOLDER">',
				// arguably mw.util.wikiGetLink() here...
				'<small class="mw-createacct-captcha-assisted">Can\'t see the image? <a href="/wiki/Wikipedia:Request_an_account" title="Wikipedia:Request an account" tabindex="-1">We can create an account for you!</a></small>',
				'<input id="wpCaptchaWord" name="wpCaptchaWord" type="text" placeholder="Enter the text you see above" name="captcha-text" tabindex="' + tabIndex + '" autocapitalize="off" autocorrect="off">',
			'</div>',
		'</div>'
				].join('') );
