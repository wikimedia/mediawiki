...
( function ( mw, $ ) {
	...
	$( function () {
		...
		function checkUsername( username ) {
			...
			var d, title, apiPromise;

			d = $.Deferred();

			// mw.Title does not do the exhaustive checks done by server-side MediaWiki,
			// so we can only use it to reject obviously invalid usernames.
			// We could attempt to use it for normalisation checks too (if title.getText() !== username),
			// but that would require reworking the interface that displays the errors and warnings
			// (in order for the warning not to override any possible errors).
			title = mw.Title.newFromText( 'User:' + username );
			if ( !title ) {
			...
			return d.promise( { abort: ( apiPromise ? apiPromise.abort : $.noop ) } );
		}
		...
		function updateUsernameStatus() {
			...
			currentRequest = thisRequest = checkUsername( username );
