...
jQuery( function ( $ ) {
	...
	function switchPrefTab( name, mode ) {
		...
		$preftoc.find( 'li' ).removeClass( 'selected' ).find( 'a' ).attr( 'tabIndex', -1 ).attr( 'aria-selected', 'false' );
		$tab = $( document.getElementById( 'preftab-' + name ) );
		if ( $tab.length ) {
			$tab.attr( 'tabIndex', 0 ).attr( 'aria-selected', 'false' ).focus().parent().addClass( 'selected' );
