	...
	$.fn.makeCollapsible = function ( options ) {
		...
		return this.each(function () {
			...
			defaultToggleHandler = function ( e, opts ) {
				var defaultOpts = { toggleClasses: true };
				opts = $.extend( defaultOpts, options, opts );
				togglingHandler( $(this), $collapsible, e, opts );
			};
			...
			if ( $customTogglers && $customTogglers.length ) {
				$customTogglers.on( 'click.mw-collapse', function ( e, opts ) {
					var defaultOpts = { linksPassthru: true };
					opts = $.extend( defaultOpts, options, opts );
					togglingHandler( $(this), $collapsible, e, opts );
				} );
				...
				if ( $collapsible.is( 'table' ) ) {
					...
					if ( !$toggle.length ) {
						...
						$toggleLink = $toggle.off( 'click.mw-collapse' ).on( 'click.mw-collapse', defaultToggleHandler );
					}
					...
					// If theres no toggle link, add it
					if ( !$toggle.length ) {
						$toggleLink = $collapsible.prepend( $defaultToggleLink );
					} else {
						$toggleLink = $toggle.off( 'click.mw-collapse' ).on( 'click.mw-collapse', defaultToggleHandler );
					}
				}
