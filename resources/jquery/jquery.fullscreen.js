...
( function ( $ ) {
	...
	function enterFullscreen () {
		...
		if ( element ) {
			...
			// Add the fullscreen class to `element`
			$element.addClass( fsClass );
			return this;
		} else {
			$element.data( 'isFullscreened', false );
			return this;
	 ...
	 * Check the .data( 'isFullscreened' ) of the return value to check
	 * success or failure, if you're into that sort of thing.
	 * Note: Will only return the first element, so watch out.
	 * @chainable
	 * @returns {jQuery}
	 */
	function exitFullscreen () {
		...
		if ( fullscreenElement && this.get(0) === fullscreenElement ) {
			...
			// We don't need need to remove the fullscreen class here,
	...
	setupFullscreen = function () {
		if ( document.fullscreenEnabled ||
			...
			$( document ).on( 'webkitfullscreenerror mozfullscreenerror msfullscreenerror', function () {
				$( document ).trigger( $.Event( 'jq-fullscreenerror' ) );
			} );
