			...
			case 'suggestions':
				...
				if ( context.data !== undefined ) {
					if ( context.data.$textbox.val().length === 0 ) {
						...
						// Compute parameters for links' hrefs
						var textbox_name = context.data.$textbox.attr( 'name' );
						var form = context.config.$region.closest( 'form' );
						var link_params = {};
						$.each( form.serializeArray(), function( idx, obj ) {
							link_params[obj.name] = obj.value;
						} );

						for ( var i = 0; i < context.config.suggestions.length; i++ ) {
							/*jshint loopfunc:true */
							var text = context.config.suggestions[i];
							link_params[textbox_name] = text; // link_params object is reused
							var href = form.attr( 'action' ) + '?' + $.param( link_params );
