...
return call_user_func( function () {
	...
	foreach ( $imageModules as $name ) {
		$modules[ "oojs-ui.styles.$name" ] = function () use ( $name, $rootPath, $themes ) {
			global $IP, $wgSkinOOUIThemes;

			$module = array();

			foreach ( $themes as $theme ) {
				$dataPath = $IP . '/' . $rootPath . '/' . $theme . '/' . $name . '.json';
				...
				foreach ( $data as $key => $value ) {
					switch ( $key ) {
						case 'images':
						case 'variants':
							$module[$key][$theme] = $data[$key];
							break;

						default:
							if ( !isset( $module[$key] ) ) {
								$module[$key] = $data[$key];
							} elseif ( $module[$key] === $data[$key] ) {
								// OK
							} else {
								throw new Exception( "Mismatched OOUI theme definitions are not supported" );
							}
							break;
					}
				}
			...
			$module['class'] = 'ResourceLoaderImageModule';
			if ( substr( $name, 0, 5 ) === 'icons' ) {
				$module['selectorWithoutVariant'] = '.oo-ui-icon-{name}, .mw-ui-icon-{name}:after, .mw-ui-icon-{name}:before';
				$module['selectorWithVariant'] = '.oo-ui-image-{variant} .oo-ui-icon-{name}, .oo-ui-image-{variant}.oo-ui-icon-{name}, .mw-ui-icon-{name}-{variant}:after, .mw-ui-icon-{name}-{variant}:before, .mw-ui-hovericon:hover .mw-ui-icon-{name}-{variant}-hover:before, .mw-ui-hovericon.mw-ui-icon-{name}-{variant}-hover:hover:before';
			}
