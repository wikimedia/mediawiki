...
var doneIEAlphaFix, doneIETransform, expandedURLs, fixalpha, hookit, isMSIE55,
	onafterprint, onbeforeprint, relativeforfloats, setrelative;

isMSIE55 = window.isMSIE55 = ( window.showModalDialog && window.clipboardData && window.createPopup );
doneIETransform = window.doneIETransform = false;
doneIEAlphaFix = window.doneIEAlphaFix = false;

hookit = window.hookit = function() {
...
expandedURLs = window.expandedURLs = false;

onbeforeprint = window.onbeforeprint = function() {
...
onafterprint = window.onafterprint = function() {
	var i;
	for ( i = 0; i < expandedURLs.length; i++ ) {
		if ( expandedURLs[i] ) {
