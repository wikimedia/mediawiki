...
wgUploadWarningObj = window.wgUploadWarningObj = {
	...
	timeout: function ( field, warningId, ackElt ) {
		var uploadWarningObj, filename, prefixedFilename;
		...
		// anonymous callback. this.nameToCheck is copied so that multiple overlapping
		...
		( new mw.Api() ).get( {
			...
			var resultOut = '';
			if ( result.query ) {
				var resultInfo = result.query.pages[result.query.pageids[0]];
