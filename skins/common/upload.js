...
wgUploadWarningObj = window.wgUploadWarningObj = {
	...
	keypress: function ( destFile, warningId, ackElt ) {
		...
		for ( cached in this.responseCache ) {
			if ( cached === filename ) {
				this.setWarning( this.responseCache[filename], warningId, ackElt );
				return;
			}
		}

		this.timeoutID = setTimeout( function () {
			wgUploadWarningObj.timeout( destFile, filename, warningId, ackElt );
		}, this.delay );
	},
	...
	checkNow: function ( field, warningId, ackElt ) {
		...
		this.timeout( field, filename, warningId, ackElt );
	},
	 ...
	 * @param {HTMLElement} ackElt hidden field element for warning acknowledgement
	 */
	timeout: function ( field, filename, warningId, ackElt ) {
		var uploadWarningObj, prefixedFilename;

		if ( !ajaxUploadDestCheck || filename === '' ) {
			return;
		}

		// Get this into context for anonymous callback. 
		uploadWarningObj = this;
		...
		( new mw.Api() ).get( {
			...
			uploadWarningObj.processResult( resultOut, filename, warningId, ackElt );
		} );
	},
