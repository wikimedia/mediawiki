...
class ApiStructureTest extends MediaWikiTestCase {
	...
	public function testParameterConsistency( $path ) {
		...
		/**
		 * Values are an array, where each array value is a permitted type.  A
		 * type can be a string, which is the name of an internal type or a
		 * class/interface.  Or it can be an array, in which case the value
		 * must be an array whose elements are the types given in the array
		 * (e.g., [ 'string', integer' ] means an array whose entries are
		...
		$paramTypes = [
			...
			ApiBase::PARAM_HELP_MSG => [ 'string', 'array', 'Message' ],
			ApiBase::PARAM_HELP_MSG_APPEND => [ [ 'string', 'array', 'Message' ] ],
			ApiBase::PARAM_HELP_MSG_INFO => [ [ 'array' ] ],
			ApiBase::PARAM_VALUE_LINKS => [ [ 'string' ] ],
			ApiBase::PARAM_HELP_MSG_PER_VALUE => [ [ 'string', 'array', 'Message' ] ],
			ApiBase::PARAM_SUBMODULE_MAP => [ [ 'string' ] ],
			ApiBase::PARAM_SUBMODULE_PARAM_PREFIX => [ 'string' ],
			ApiBase::PARAM_ALL => [ 'boolean', 'string' ],
			ApiBase::PARAM_EXTRA_NAMESPACES => [ [ 'int' ] ],
		...
		// param => other param(s) that must be present
		$paramRequirements = [
			ApiBase::PARAM_ALLOW_DUPLICATES => ApiBase::PARAM_ISMULTI,
			ApiBase::PARAM_ALL => ApiBase::PARAM_ISMULTI,
		...
		$constantNames = array_flip( array_filter(
			( new ReflectionClass( 'ApiBase' ) )->getConstants(),
			function ( $key ) {
				return substr( $key, 0, 6 ) === 'PARAM_';
			},
		...
		foreach ( [ $paramsPlain, $paramsForHelp ] as $params ) {
			foreach ( $params as $param => $config ) {
				...
				foreach ( $paramTypes as $key => $types ) {
					if ( !isset( $config[$key] ) ) {
						continue;
					}
					$keyName = $constantNames[$key];
					$this->validateType( $types, $config[$key], $param, $keyName );
				}

				foreach ( $paramRequirements as $key => $required ) {
					if ( !isset( $config[$key] ) ) {
						continue;
					}
					foreach ( (array)$required as $requiredKey ) {
						$this->assertArrayHasKey( $requiredKey, $config,
							"$param: When ${constantNames[$key]} is set, " .
							"$requiredKey must also be set" );
					}
				}

				foreach ( $paramAllowedTypes as $key => $allowedTypes ) {
					if ( !isset( $config[$key] ) ) {
						continue;
					}
					...
					$this->assertContains(
						$actualType,
						(array)$allowedTypes,
						"$param: ${constantNames[$key]} can only be used on " .
							implode( ', ', (array)$allowedTypes )
					);
				}

				foreach ( $paramProhibitedTypes as $key => $prohibitedTypes ) {
					if ( !isset( $config[$key] ) ) {
						continue;
					}
	 ...
	 *
	 * @param array $types From $paramTypes array
	...
	private function validateDefault( $param, $config ) {
		...
		if ( isset( $config[ApiBase::PARAM_ISMULTI] ) && $config[ApiBase::PARAM_ISMULTI] ) {
			if ( $default === '' ) {
			...
			foreach ( $defaults as $defaultValue ) {
				...
				$alteredConfig[ApiBase::PARAM_DFLT] = $defaultValue;
				$this->validateDefault( $param, $alteredConfig );
			}
			return;
		}
		switch ( $type ) {
			...
			case 'user':
				$this->assertTrue(
					User::isValidUserName( $default ) || IP::isValid( $default ),
