...
class ApiStructureTest extends MediaWikiTestCase {
	...
	/**
	 * @dataProvider provideCustomMultiValueLimits
	 * @param string $path
	 */
	public function testCustomMultiValueLimits( $path ) {
		$main = self::getMain();
		$module = TestingAccessWrapper::newFromObject( $main->getModuleFromPath( $path ) );
		$params = $module->getAllowedParams();

		// avoid warnings about empty tests when there is no parameter with custom limits
	 ...
	 */
	public static function provideCustomMultiValueLimits() {
		$main = self::getMain();
		$paths = self::getSubModulePaths( $main->getModuleManager() );
		array_unshift( $paths, $main->getModulePath() );
