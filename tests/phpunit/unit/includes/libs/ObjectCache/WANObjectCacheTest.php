...
class WANObjectCacheTest extends MediaWikiUnitTestCase {
	...
	public function testGetWithSetCallback_touchedCallback_versioned() {
		...
		$wanOpts = [
			...
			// 'logger' => new \Monolog\Logger( 'objectcache', [ new \Monolog\Handler\ErrorLogHandler() ] ),
		];
		$mockWallClock = 1549343530.0;
		$key = 'some-example-key';
		$wasSet = 0;
		...
		$this->assertSame( 1, $touchCallbackCalled, 'No value to check' );
		...
		$this->assertSame( 2, $touchCallbackCalled, 'No value to check' );
		...
		// are stored under an interim key.
		$mockWallClock += 3600;
		$cache = new WANObjectCache( $wanOpts );
		$cache->setMockTime( $mockWallClock );
		$cache->delete( $key );
		...
		$this->assertSame( 3, $touchCallbackCalled, 'No value to check' );
	...
	public function testPreemptiveRefresh() {
		...
		$wasSet = 0;
		$key = 'tiffany-breakfast:refresh-me-for-near-expiring';
		$opts = [ 'lowTTL' => 100, 'hotTTR' => 0 ];
		$v = $cache->getWithSetCallback( $key, 300, $func, $opts );
		$this->assertSame( $value, $v, "Value returned" );
		$this->assertSame( 1, $wasSet, "Value regenerated for miss" );
		$mockWallClock += 250;
		$v = $cache->getWithSetCallback( $key, 300, $func, $opts );
		$this->assertSame( 2, $wasSet, "Value regenerated by preemptive refresh" );

		$wasSet = 0;
		$key = 'tiffany-breakfast:too-early-for-refresh';
		$opts = [ 'lowTTL' => 100, 'hotTTR' => 0 ];
		$v = $cache->getWithSetCallback( $key, 300, $func, $opts );
		$this->assertSame( $value, $v, "Value returned" );
		$this->assertSame( 1, $wasSet, "Value regenerated for miss" );
		...
		$v = $cache->getWithSetCallback( $key, 300, $func, $opts );
		$this->assertSame( 1, $wasSet, "Value not regenerated" );
		$this->assertSame( $v, $value, "Cached value" );
		$mockWallClock += 250;
		$v = $cache->getWithSetCallback( $key, 300, $func, $opts );
		$this->assertSame( $value, $v, "Value returned" );
		$this->assertSame( 1, $wasSet, "Value not yet regenerated" );
		$this->assertCount( 1, $asycList, "Refresh is scheduled" );
		$value = 'NewCatsInTown'; // set next generated return value
		$asycList[0](); // run async refresh
		$asycList = [];
		$this->assertSame( 2, $wasSet, "Value regenerated by refresh" );
		$this->assertSame( [], $asycList, "No other refreshes scheduled" );
		$v = $cache->getWithSetCallback( $key, 300, $func, $opts );
		$this->assertSame( 2, $wasSet, "Value not regenerated" );
		...
		$wasSet = 0;
		$key = 'tiffany-breakfast:awaiting-popularity-refresh';
		$opts = [ 'hotTTR' => 100, 'lowTTL' => 0 ];
		$v = $cache->getWithSetCallback( $key, 300, $func, $opts );
		$this->assertSame( $value, $v, "Value returned" );
		$this->assertSame( 1, $wasSet, "Value regenerated for miss" );
		$mockWallClock += 50;
		...
		$mockWallClock = 1549343530.0;
		$wasSet = 0;
		$key = 'tiffany-breakfast:refresh-me-for-popularity';
		$opts = [ 'hotTTR' => 100, 'lowTTL' => 0 ];
		$v = $cache->getWithSetCallback( $key, 300, $func, $opts );
		$this->assertSame( $value, $v, "Value returned" );
		$this->assertSame( 1, $wasSet, "Value regenerated for miss" );
		...
		$v = $cache->getWithSetCallback( $key, 300, $func, $opts );
		$this->assertSame( $value, $v, "Value returned" );
		$this->assertSame( 2, $wasSet, "Value regenerated by popularity refresh" );

		// Exercise the real chance calculation. hotTTR=1 and ageNew=0 reduce
		// refreshWindowSec to 1, which makes the chance 1 and the refresh certain.
		...
		$wasSet = 0;
		$value = 'KatCafe';
		$key = 'tiffany-breakfast:certain-popularity-refresh';
		$opts = [ 'hotTTR' => 1, 'ageNew' => 0, 'lowTTL' => 0 ];
		$v = $cache->getWithSetCallback( $key, 300, $func, $opts );
		$this->assertSame( $value, $v, "Value returned" );
		$this->assertSame( 1, $wasSet, "Value regenerated for miss" );
		// Past the ramp-up window, so the chance is not scaled down
		$mockWallClock += 31;
		$v = $cache->getWithSetCallback( $key, 300, $func, $opts );
		$this->assertSame( $value, $v, "Value returned" );
		$this->assertSame( 2, $wasSet, "Value regenerated by popularity refresh" );
...
/**
 * Mock WANObjectCache::worthRefreshExpiring with a non-random chance:
 * - Never (0%) before the first 1s of the lowTTL window has passed.
 * - Always (100%) when we're past the first 1s of lowTTL.
 */
class NearExpiringWANObjectCache extends WANObjectCache {
	...
	protected function worthRefreshExpiring( $curTTL, $logicalTTL, $lowTTL ) {
		...
		if ( $effectiveLowTTL <= 0 || $curTTL <= 0 ) {
			return false;
		}
		$timeInLowTTL = $effectiveLowTTL - $curTTL;
		return $timeInLowTTL > self::CLOCK_SKEW;
	}
}

/**
 * Mock WANObjectache::worthRefreshPopular with non-random chance
 * - Never (0%) during the hotTTR window.
 * - Always (100%) exactly at the end of the hotTTR window.
 */
