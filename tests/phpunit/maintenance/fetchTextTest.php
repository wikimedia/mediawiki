...
class SemiMockedFetchText extends FetchText {
	 ...
	 */
	private $mockInvocations = array( 'getStdin' => 0, 'output' => 0 );
...
class FetchTextTest extends MediaWikiTestCase {
	...
	/**
	 * Checks output and mock status
	 *
	 * @param $expectedOutput String the expected output of FetchText
	 */
	private function assertFetchTextSane( $expectedOutput ) {
		$invocations = $this->fetchText->mockGetInvocations();
		$this->assertEquals( 1, $invocations['getStdin'],
			"getStdin invocation counter" );
		if ( $expectedOutput === null ) {
			$this->assertEquals( 0, $invocations['output'],
				"output invocation counter" );
		} else {
			$this->assertGreaterThanOrEqual( 1, $invocations['output'],
				"output invocation counter" );
		}
		$this->assertEquals( $expectedOutput,
			$this->fetchText->mockGetAccumulatedOutput(),
			"Text passed through output" );
	...
	private function assertFilter( $input, $expectedOutput ) {
		...
		$this->assertFetchTextSane( $expectedOutput );
	}
