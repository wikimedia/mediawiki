...
class BackupDumperPageTest extends DumpTestCase {
	...
	function testFullTextPlain() {
		...
		$dumper->loadWithArgv( array( '--full', '--output', 'file:' . $fname, '--quiet' ) );
	...
	function testFullStubPlain() {
		...
		$dumper = new DumpBackup();
		$dumper->loadWithArgv( array( '--full', '--output', 'file:' . $fname, '--stub', '--quiet' ) );
		$dumper->startId = $this->pageId1;
		$dumper->endId = $this->pageId4 + 1;
		$dumper->reporting = false;
		$dumper->setDb( $this->db );
	...
	function testCurrentStubPlain() {
		...
		$dumper = new DumpBackup( array( '--output', 'file:' . $fname, '--quiet' ) );
		$dumper->startId = $this->pageId1;
		$dumper->endId = $this->pageId4 + 1;
		$dumper->reporting = false;
		$dumper->setDb( $this->db );
	...
	function testCurrentStubGzip() {
		...
		$dumper = new DumpBackup( array( '--output', 'gzip:' . $fname, '--quiet' ) );
		$dumper->startId = $this->pageId1;
		$dumper->endId = $this->pageId4 + 1;
		$dumper->reporting = false;
		$dumper->setDb( $this->db );
