...
class UserOptionRenamerTest extends MediaWikiTestCase {
	...
	public function testExecute( $old, $new, $batchSize ) {
		...
		$this->assertEquals( 3, $this->countUsersWithOption( $old ), "3 users with old option $old before script" );
        $this->assertEquals( 1, $this->countUsersWithOption( $new ), "1 user with new option $new before script" );

		$renamer = new UserOptionRenamer();
		$renamer->loadParamsAndArgs();
		$renamer->execute();

		$this->assertEquals( 3, $this->countUsersWithOption( $new ), "3 users with new option $new" );
