...
class TextPassDumperTest extends DumpTestCase {
	...
	function testPrefetchPlain() {
		...
		$prefetchMap = array(
			array( intval($this->pageId1), intval($this->revId1_1), "Prefetch_________1Text1" ),
			array( $this->pageId2, $this->revId2_3, "Prefetch_________2Text3" )
		);

		// The mock itself
		$prefetchMock = $this->getMock( 'BaseDump', array( 'prefetch' ), array(), '', FALSE );
		$prefetchMock->expects( $this->atLeastOnce() )
			->method( 'prefetch' )
			->will( $this->returnValueMap( $prefetchMap ) );

		fwrite( STDOUT, "\n"); fflush(STDOUT);
		fwrite( STDOUT, gettype( $this->pageId1 ). "\n"); fflush(STDOUT);
		fwrite( STDOUT, gettype( $this->revId1_1 ). "\n"); fflush(STDOUT);
		fwrite( STDOUT, gettype( 5 ). "\n"); fflush(STDOUT);
		fwrite( STDOUT, gettype( 9 ). "\n"); fflush(STDOUT);
		fwrite( STDOUT, "this->pageId1 :".$this->pageId1."/\n"); fflush(STDOUT);
		fwrite( STDOUT, "this->revId1_1:".$this->revId1_1."/\n"); fflush(STDOUT);
		fwrite( STDOUT, "MockCheck/5/9/ :".$prefetchMock->prefetch(5,9)."/\n"); fflush(STDOUT);
		fwrite( STDOUT, "MockCheck/6/12/:".$prefetchMock->prefetch(6,12)."/\n"); fflush(STDOUT);
		fwrite( STDOUT, "MockCheck/5/9/ :".$prefetchMock->prefetch(5,9)."/\n"); fflush(STDOUT);
		fwrite( STDOUT, "MockCheck/6/12/:".$prefetchMock->prefetch(6,12)."/\n"); fflush(STDOUT);
		fwrite( STDOUT, "MockCheck/".$this->pageId1."/".$this->revId1_1."/ :".$prefetchMock->prefetch($this->pageId1,$this->revId1_1)."/\n"); fflush(STDOUT);
		fwrite( STDOUT, "MockCheck/6/12/:".$prefetchMock->prefetch(6,12)."/\n"); fflush(STDOUT);
		fwrite( STDOUT, "Used Mock:".get_class($prefetchMock)."/\n"); fflush(STDOUT);
		fwrite( STDOUT, "PHP:".phpversion()."/\n"); fflush(STDOUT);
		$rc = new ReflectionClass( new PHPUnit_Framework_MockObject_InvocationMocker() );
		$dc = $rc->getDocComment();
		fwrite( STDOUT, "PHPUnit_MockObject:". $dc . "\n" );fflush(STDOUT);
		//$this->assertEquals( "/".$this->pageId1."/".$this->revId1_1."/Prefetch_________1Text1", $prefetchMock->prefetch( $this->pageId1, $this->revId1_1 ), "Checking whether mock is ok");
		...
		fwrite( STDOUT, "Used Mock:".get_class($prefetchMock)."/\n"); fflush(STDOUT);
		fwrite( STDOUT, "Set  Mock:".get_class($dumper->prefetch)."/\n"); fflush(STDOUT);
		$dumper->reporting = false;
		$dumper->setDb( $this->db );
		fwrite( STDOUT, "Used Mock:".get_class($prefetchMock)."/\n"); fflush(STDOUT);
		fwrite( STDOUT, "Set  Mock:".get_class($dumper->prefetch)."/\n"); fflush(STDOUT);
	...
	private function checkpointHelper( $checkpointFormat = "file" ) {
		...
		$nameOutputDir = $this->getNewTempFile();

		// Get temporary /directory/ to store the checpoint files in
		unlink( $nameOutputDir );
		...
		while ( $lastDuration < $minDuration ) {
			...
			if ( $lastDuration < $minDuration ) {
				...
				if ( $old_iterations == $iterations ) {
					// Heuristics were not applied, so we just *4.
					$iterations *= 4;
				}
