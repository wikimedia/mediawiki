...
class CloakingLoadBalancer extends LoadBalancer {
	 ...
	 *
	 * @note only the first entry in $servers will be used!
	 *
	 * @param array $servers
	 *
	 * @throws MWException
	...
	public function __construct( array $servers ) {
		...
		$params['servers'] = array_slice( $servers, 0, 1 );
	 ...
	 * @throws MWException
	 */
	private function cloakConnection( Database $db ) {
		if ( isset( $db->_cloner ) ) {
	...
	private function cleanupConnection( IDatabase $db ) {
	...
	public function closeAll() {
		$this->forEachOpenConnection( function( Database $db ) {
	 ...
	 * @return array
	 */
	private function listTablesToClone( Database $db, $oldPrefix, $newPrefix ) {
	...
	/**
	 * @param string|bool $domain Domain ID for a selective domain
	 *                            false or "" equivalent DatabaseDomain object for current domain
	 * @return bool
	 */
	private function isForeignWiki( $domain ) {
		$result = $domain !== false && $domain != "" && $domain !== wfWikiID();
		return $result;
	}
	...
	public function getConnection( $i, $groups = [], $domain = false ) {
		if ( $this->isCloaked() && $this->isForeignWiki( $domain ) ) {
			throw new DBAccessError( 'Cannot access foreign databases when cloaked: ' . $domain );
		}

		return parent::getConnection( $i, $groups, $domain );
	}
