...
class MediaWikiTestEnvironment {
	 ...
	 * @var string
	 */
	private static $defaultDBprefix = null;
	...
	public function __construct( $useTemporaryTables = true, $reuseDB = false ) {
		...
		/**
		 * This could be set even earlier, for example in phpunit.php and this could be changed
		 * to a check that it has previously been set. This would avoid anybody possibly changing
		 * the global before this object is instantiated.
		 * If everything that used $wgDBprefix used a Config object from MediawikiService then this
		 * could be avoided all-together as we could override the config when we override the LB.
		 */
		if ( self::$defaultDBprefix === null ) {
			self::$defaultDBprefix = $wgDBprefix;
		}
	}
	...
	public static function teardownTestDB() {
		global $wgJobClasses, $wgDBprefix;
		...
		$wgDBprefix = self::$defaultDBprefix;
	}
