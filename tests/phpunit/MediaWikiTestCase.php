...
abstract class MediaWikiTestCase extends PHPUnit_Framework_TestCase {
	...
	protected static function setupExternalStoreTestDBs( $testPrefix ) {
		...
		foreach ( $connections as $dbw ) {
			...
			//
			// I'm not sure, but CloneDatabase::changeLBPrefix may also affect
			// other in the array (that haven't been cloned yet).
	...
	/**
	 * Resets test DBs set up previously
	 */
	private function resetAllTestDBs() {
		$this->resetDB( $this->db, $this->tablesUsed );

		if ( self::isUsingExternalStoreDB() ) {
			$externalStoreDB = ExternalStore::getStoreObject( 'DB' );
			$externalStoreDbws = self::getExternalStoreDatabaseConnections();
			foreach ( $externalStoreDbws as $dbw ) {
				$blobsTableName = $externalStoreDB->getTable( $dbw );
				$this->resetDB( $dbw, array( $blobsTableName ) );
			}
		}
	}
