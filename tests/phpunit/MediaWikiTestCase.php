...
abstract class MediaWikiTestCase extends PHPUnit_Framework_TestCase {
	...
	/**
	 * @var MediaWikiTestEnvironment
	 */
	protected $testEnvironment;

	/**
	 * Table name prefixes. Oracle likes it shorter.
	 * @deprecated use prefixes from MediaWikiTestEnvironment directly
	 */
	const DB_PREFIX = MediaWikiTestEnvironment::DB_PREFIX;
	const ORA_DB_PREFIX = MediaWikiTestEnvironment::ORA_DB_PREFIX;

	public function __construct( $name = null, array $data = [], $dataName = '' ) {
		parent::__construct( $name, $data, $dataName );

		$this->called['__construct'] = true;
		$this->backupGlobals = false;
		$this->backupStaticAttributes = false;
		$this->testEnvironment = new MediaWikiTestEnvironment(
	...
	public static function prepareServices( Config $bootstrapConfig ) {
		...
		if ( !$services ) {
			$services = MediaWikiTestEnvironment::resetGlobalServices( $bootstrapConfig );
		}
		return $services;
	}
	...
	public function run( PHPUnit_Framework_TestResult $result = null ) {
		...
		$this->testEnvironment->doLightweightServiceReset();

		$needsResetDB = false;

		if ( $this->needsDB() ) {
			// set up a DB connection for this test to use
			$this->testEnvironment->setupTestDB();
	...
	public function reusesDB() {
		return (bool)$this->getCliArg( 'reuse-db' );
	}
	 ...
	 * @coversNothing
	 */
	final public function testMediaWikiTestCaseParentConstructCalled() {
		$this->assertArrayHasKey( '__construct', $this->called,
			static::class . '::__construct() must call parent::__construct()'
		);
	}
	...
	protected function setService( $name, $object ) {
		...
		if ( MediaWikiServices::getInstance() === self::$serviceLocator ) {
			$this->testEnvironment->overrideMwServices();
		}
	...
	/**
	 * @since 1.27
	 * @see MediaWikiTestEnvironment::overrideMwServices
	 * @deprecated use MediaWikiTestEnvironment directly
	 */
	protected function overrideMwServices( Config $configOverrides = null, array $services = [] ) {
		return $this->testEnvironment->overrideMwServices( $configOverrides, $services );
	}
