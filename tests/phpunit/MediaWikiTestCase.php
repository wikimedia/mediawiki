...
abstract class MediaWikiTestCase extends PHPUnit_Framework_TestCase {
	...
	private function doLightweightServiceReset() {
		...
		$this->setService( 'MainWANObjectCache', new EmptyBagOStuff() ); // sanity
	...
	private function resetDB( $db, $tablesUsed ) {
		if ( $db ) {
			...
			foreach ( $tablesUsed as $tbl ) {
				...
				if ( $truncate ) {
					$ignore = true; // ignore "table does not exist"
					$db->query( 'TRUNCATE TABLE ' . $db->tableName( $tbl ), __METHOD__, $ignore );
				} else {
