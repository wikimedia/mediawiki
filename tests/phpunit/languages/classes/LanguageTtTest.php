...
Xaqan Fidan “Ğadälät häm qalqınu” partiäsennän xalıq
...
saylanğannan soñ Töp Qanunnıñ üzenä birgän wäqalätne qullanıp ministrlar şurasın
...
// https://tt.wikipedia.org/wiki/%C4%9E%C3%A4r%C3%A4p_%C3%A4lifbas%C4%B1
			'Bu mäqaläneñ kirill älifbasındağı igezäge bar.
...
ber urında asılınıp tora ala häm vertikal\' räweştä kütärelä häm utıra ala.'
		...
		array(
			'КамАЗ ЕРЭ Е А Ю АКШ ЮХИДИ',
			'KamAZ YeRE Ye A Yu AQŞ YuXİDİ'
		),
		array(
...
ğöref gorilla ğömum ğömumän ğömumi gol golsız golğa ğa gäp gramm ğarıq tağu
...
evolyutsiyağa evolyutsiägä ambitsiägä ambitsiyağa qawiä rawiä möxämmädiä avariya
...
уен йөрик яшик тарыйк тарыйкъ тарикъ бәлигъ
...
cämğiät cämğiäte cämğiätkä cämğese qiäfät pyala waqıyğa qıyar wiläyät zaruriät möxämmädiä
...
qaya aqaya sänäğät sänğät ua qua uıp quıp yuan yuın çuyın quyın muyın ürtäläw ädäbiät wayımsız
ürtä qıyldı qıldı çıyıldap möstäqil ixtilaf ixtıylaf qıyın cıyın bıyıl iqtisadıy tıydı tıy cinäyät
...
uyın yörik yäşik tarıyq tarıyq tariq bäliğ
...
әнвәр трамвай авыл штутгард вепс карт акт аяу Юнь китабындагы
...
төбәкара пауза мөгаен мөгаен мөлаем поезд якупов гыйлемхановка гайнанов тәэссорат
...
заказ коточкыч указ министрлыгында язарга йомгаклаячак Русия',
...
änwär tramvay awıl ştutgard veps qart akt ayaw Yün kitabındağı
variant versiä noyaberdän mäşäqät ğataullin yagfarovağa yağafarovağa yağfarovağa yagfar
yağafar yağfar ğäyetläre ğäyet qör\'än muzıka nihayät telefonğa web taswir
töbäkara pauza möğayen möğayen mölayem poyezd yaqupov ğilemxanovqa ğaynanov tä\'essorat
...
zakaz qotoçqıç ukaz ministrlığında yazarğa yomğaqlayaçaq Rusiä'
		),
		array(
// this was a test with tricky words for my converter http://aylandirow.tmf.org.ru/
// and also for other converters
			'лигасы васыять васыяте васыяви гомер
Әнкара гореф җәмгыять гошер голәма гобелен Гоген
...
Änqara ğöref cämğiät ğöşer ğöläma gobelen Gogen
...
куярга яратабыз. Бүген заман икенче, шуңа күрә иҗади эшләү ысуллары да башка төрле, дибез.' )
	...
	public function testConversionToLatin() {
		...
		// }
		$this->oneConversionToLatin($this->textPairs[15]);
		/*
