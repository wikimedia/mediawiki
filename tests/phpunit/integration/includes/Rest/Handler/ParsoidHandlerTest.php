...
class ParsoidHandlerTest extends MediaWikiIntegrationTestCase {
	...
	private function newHTMLTransformInput( $html, $methodOverrides = [] ): HTMLTransformInput {
		$input = $this->getMockBuilder( HTMLTransformInput::class )
			->onlyMethods( array_keys( $methodOverrides ) )
			->setConstructorArgs( [ $html ] )
			->getMock();

		foreach ( $methodOverrides as $method => $callback ) {
			$input->method( $method )->willReturnCallback( $callback );
		}
	...
	private function newParsoidHandler( $methodOverrides = [] ): ParsoidHandler {
		...
		$handler = new class (
			...
			protected function getHTMLTransformInput( array $attribs, string $html ) {
				if ( isset( $this->overrides['getHTMLTransformInput'] ) ) {
					return $this->overrides['getHTMLTransformInput']();
				}

				return parent::getHTMLTransformInput(
					$attribs,
					$html
				);
			}
	...
	public function testHtml2wtHandlesParseHtmlException() {
		...
		$handler = $this->newParsoidHandler( [
			...
			'getHTMLTransformInput' => static function () use ( $input ) {
				return $input;
			},
		] );
