...
class ApiResultTest extends MediaWikiTestCase {
	...
	private function getApiResult() {
		return new ApiResult( new DerivativeContext( RequestContext::getMain() ) );
	}
	...
	public function testInstanceDataMethods() {
		...
		$result->reset();
		$result->getContext()->setConfig( new HashConfig( array( 'APIMaxResultSize' => 10 ) ) );
	...
	public function testUtilityFunctions() {
		...
		$arr = ApiResult::removeMetadata( array(
			'foo' => array(
			...
			ApiResult::META_SUBELEMENTS => array( 'foo', 'bar' ),
			ApiResult::META_INDEXED_TAG_NAME => 'itn',
			ApiResult::META_PRESERVE_KEYS => array( 'foo', 'bar', '_dummy2' ),
			ApiResult::META_TYPE => 'array',
			'_dummy' => 'foobaz',
			'_dummy2' => 'foobaz!',
		) );
	...
	public function testDeprecatedFunctions() {
		...
		$result->reset();
		$result->getContext()->setConfig( new HashConfig( array( 'APIMaxResultSize' => 3 ) ) );
