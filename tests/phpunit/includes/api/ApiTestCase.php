...
abstract class ApiTestCase extends MediaWikiLangTestCase {
	 ...
	 * @param FauxRequest|null $lastReq Object of last request
	 *
	...
	protected function doApiRequest( array $params, array $session = null, $appendModule = false, User $user = null,
		...
		if ( $lastReq !== null ) {
			$request->applyResponse( $lastReq->response() );
		}
	 ...
	 * @return array [ ResultData, WebRequest, SessionData ]
	 */
	protected function doApiRequestInternal( FauxRequest $request, User $user = null, $appendModule = false ) {
	...
	/**
	 * @param TestUser $user User to get tokens for
	 * @param array|null $session Session data
	 * @param null &$request The request object from the API call will be stored here
	 *
	 * @return mixed
	 * @throws MWException
	 */
	protected function getTokenList( $user, $session = null, &$request = null ) {
		$data = $this->doApiRequest( array(
			'action' => 'tokens',
