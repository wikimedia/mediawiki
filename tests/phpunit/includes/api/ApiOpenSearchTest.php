...
class ApiOpenSearchTest extends PHPUnit_Framework_TestCase {
	...
	private function replaceSearchEngineConfig() {
		$config = $this->getMockBuilder( 'SearchEngineConfig' )
			...
			->getMock();
		$this->replaceService( 'SearchEngineConfig', $config );
	...
	private function replaceSearchEngine() {
		...
		$this->replaceService( 'SearchEngineFactory', $engineFactory );

		return $engine;
	}

	private function replaceService( $name, $replacement ) {
		MediaWikiServices::getInstance()->redefineService(
			$name,
			function ( MediaWikiServices $container ) use ( $replacement ) {
				return $replacement;
			}
		);
	}
