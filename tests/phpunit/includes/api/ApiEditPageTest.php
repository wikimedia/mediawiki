...
class ApiEditPageTest extends ApiTestCase {
	 ...
	 * undoafter doesn't exist in the database.
	 */
	public function testUndoAfterToInvalidRev() {
		...
		$revId3 = $titleObj->getLatestRevId( Title::GAID_FOR_UPDATE );
	...
	public function testUndoWithSwappedRevisions() {
		...
		// This is wrong!  It should be 2.  But let's test for our incorrect
		// behavior for now, so if someone fixes it they'll fix the test as
		// well to expect 2.  If we disabled the test, it might stay disabled
	...
	public function testAppendNewSectionWithSummary() {
		...
		$this->assertSame( "Initial content\n\n== Add new section ==\n\nMore content",
			$page->getContent()->getNativeData() );
		// XXX Is this really expected behavior?  Shouldn't summary set the
		// actual summary, not the section title?
...
//	// XXX The block deletion at the end doesn't effectively remove the block,
//	// so it interferes with subsequent tests.  What am I doing wrong?
...
// XXX This says invalid CSRF token, why?
