...
class ApiGeneratorTest extends MediaWikiTestCase {
	 ...
	 * Also tests whether all modules listed as a Generator in ApiQuery
	 * are actually a generator (subclass of ApiQueryGeneratorBase).
	 *
	...
	public function testIfGenerator() {
		...
		foreach ( $modules as $moduleName => $moduleClass ) {
			if ( is_subclass_of( $moduleClass, 'ApiQueryGeneratorBase' ) ) {
				if ( !isset( $generators[$moduleName] ) ) {
					$this->fail( "'{$moduleClass}' is a subclass of ApiQueryGeneratorBase, but isn't listed as being a generator" );
				}
			} elseif ( isset( $generators[$moduleName] ) ) {
				$this->fail( "'{$moduleClass}' is not a subclass of ApiQueryGeneratorBase, but is listed as being a generator" );
			}
		}
