...
class ApiMainTest extends ApiTestCase {
	...
	public function testNonWhitelistedCorsWithCookies() {
		...
		$this->mergeMwGlobalArrayValue( 'wgDebugLogGroups', [ 'cors' => $logFile ] );
	...
	public function testCheckConditionalRequestHeadersFailed() {
		...
		$mock->method( 'execute' )->will(
			$this->throwException( new MWException( 'Must not be executed' ) )
		);
		...
		$this->assertArrayNotHasKey( 'error', $api->getResult()->getResultData() );
	}
