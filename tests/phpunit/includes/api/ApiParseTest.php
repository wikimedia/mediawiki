...
class ApiParseTest extends ApiTestCase {
	...
	public function addDBDataOnce() {
		...
		$this->deleteRevision( self::$revIds['revdel'] );
		$this->deleteRevision(
	...
	public function testSectionNoContent() {
		...
		// @todo Any way to do this without a direct query?
		$dbw = wfGetDB( DB_MASTER );
		$dbw->delete( 'revision', [ 'rev_id' => $status->value['revision']->getId() ] );
	...
	public function testLimitReports() {
		...
		// @todo Should we test the actual values?
	...
	public function testParseTree() {
		...
		$this->assertSame(
			'<root>Some \'\'text\'\' is <template><title>nice</title>' .
				...
				'<part><name>i</name>=<value>think</value></part></template></root>',
	...
	public function testDisableTidy() {
		// @todo What's a case where this actually has an effect these days?
		$res = $this->doApiRequest( [
			'action' => 'parse',
