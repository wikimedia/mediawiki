...
class ApiQuerySiteinfoTest extends ApiTestCase {
	...
	public function testNamespaceAliases() {
		...
		$this->stashMwGlobals( 'wgNamespaceAliases' );
	...
	public function testInterwikiMap( $filter ) {
		...
		// @todo These don't seem to make the messages enabled
	...
	public function testUserGroups( $numInGroup ) {
		...
		foreach ( $data as $val ) {
			...
			if ( $val['name'] === 'viscount' ) {
				...
				// @todo User::getAllGroups returns an array with holes in the indices, should this
				// be fixed?  (Thus the need for array_values().)
				$this->assertSame( array_values( User::getAllGroups() ), $val['add'] );
			} elseif ( $val['name'] === 'bot' ) {
