...
class CSSJanusTest extends MediaWikiTestCase {
	 ...
	 */
	function testTransform( $code, $expectedOutput = null ) {
		$flipped = CSSJanus::transform( $code );

		if ( $expectedOutput ) {
			$this->assertEquals( $expectedOutput, $flipped, 'Test A-B flipping' );
			$this->assertEquals( $flipped, $expectedOutput, 'Test B-A flipping' );
		} else {
			$this->assertEquals( $flipped, $code, 'Should not be flipped' );
		}
	}
	...
	function provideTransformCases() {
		return array(
			...
			// Direction should not change, except on body
			...
			array(
				// inside selector (doesn't trigger it)
				'div, /* noflip */ .notme { float: left; }',
				'div, /* noflip */ .notme { float: right; }'
			),
			array(
				// after selector (doesn't trigger it)
				'div, .notme /* noflip */ { float: left; }',
				'div, .notme /* noflip */ { float: right; }'
			),
			array(
