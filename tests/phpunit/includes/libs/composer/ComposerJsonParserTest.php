...
class ComposerJsonParserTest extends MediaWikiTestCase {
	...
	public function testIsHashUpToDate( $file, $expected ) {
		$json = new ComposerJsonParser( $this->$file );
	...
	public function testGetRequiredDependencies() {
		$json = new ComposerJsonParser( $this->json );
	...
	public function testNormalizeVersion( $input, $expected ) {
		$this->assertEquals( $expected, ComposerJsonParser::normalizeVersion( $input ) );
	}
}
