...
class IPSetTest extends MediaWikiTestCase {
	...
	public function testIPSet() {
		...
		foreach ( $tests_1 as $ip => $expected ) {
			$result = $set_1->match( $ip );
			$this->assertEquals( $expected, $result, "Correct result in set_1 for $ip" );
		}

		# a recent propsed dataset for using subnets instead:
		$cfg_2 = array(
			'208.80.154.0/26',
			'2620:0:861:1::/64',
			'208.80.154.128/26',
			...
			'2620:0:862:102::/64',
			'10.128.0.0/24',
			'zzz/24',       # should get ignored with a warning
			'10.2.4.0/240', # should get ignored with a warning
			'2620:0:863:101::/64',
			'10.2.4.26',
		);

		$set_2 = new IPSet( $cfg_2 );
		$tests_2 = array(
			'0.0.0.0' => false,
			'255.255.255.255' => false,
			'10.2.4.25' => false,
			...
			'foobar' => false, # warning about invalid input
		);

		foreach ( $tests_2 as $ip => $expected ) {
			$result = $set_2->match( $ip );
			$this->assertEquals( $expected, $result, "Correct result in set_2 for $ip" );
		}
	}
}
