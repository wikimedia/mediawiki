...
class ObjectCacheManagerTest extends PHPUnit_Framework_TestCase {
	...
	public function testSetAnythingCandidates() {
		$cacheDefinitions = [
			CACHE_NONE => [ 'class' => 'EmptyBagOStuff' ],
			'test' => [ 'class' => 'EmptyBagOStuff' ],
			'foo' => [ 'class' => 'HashBagOStuff' ],
			'bar' => [ 'class' => 'EmptyBagOStuff' ],
		...
		$manager->setAnythingCandidates( [] );
		$this->assertInstanceOf( 'HashBagOStuff', $manager->getInstance( CACHE_ANYTHING ) );

		$manager->setAnythingCandidates( [ CACHE_ANYTHING, CACHE_NONE, 'foo', 'bar' ] );
		$this->assertInstanceOf( 'HashBagOStuff', $manager->getInstance( CACHE_ANYTHING ) );
	}
	...
	public function testNewAnything() {
		...
		$manager->setAnythingCandidates( [] );
		$this->assertInstanceOf( 'HashBagOStuff', $manager->newAnything( [] ) );

		$manager->setAnythingCandidates( [ CACHE_ANYTHING, CACHE_NONE, 'foo', 'bar' ] );
		$this->assertInstanceOf( 'HashBagOStuff', $manager->newAnything( [] ) );
	}

	public function testSetGetLocalClusterCache() {
		$cacheDefinitions = [
			'foo' => [ 'class' => 'HashBagOStuff' ],
			'bar' => [ 'class' => 'EmptyBagOStuff' ],
		];
		...
		$manager->setLocalClusterCache( 'foo' );
		$this->assertInstanceOf( 'HashBagOStuff', $manager->getLocalClusterInstance() );

		$manager->setLocalClusterCache( 'bar' );
		$this->assertInstanceOf( 'EmptyBagOStuff', $manager->getLocalClusterInstance() );
	}

	public function testSetGetMainWANCache() {
		$localCaches = [
			CACHE_NONE => [ 'class' => 'EmptyBagOStuff' ],
			'foo' => [ 'class' => 'HashBagOStuff' ],
			'bar' => [ 'class' => 'HashBagOStuff' ],
		];
		...
		$manager = $this->newObjectCacheManager( $localCaches, $wanCaches );

		$manager->setMainWANCache( 'FOO' );
		$this->assertInstanceOf( 'WANObjectCache', $manager->getMainWANInstance() );
	}

	public function testSetGetMainStash() {
		$cacheDefinitions = [
			'foo' => [ 'class' => 'HashBagOStuff' ],
			'bar' => [ 'class' => 'EmptyBagOStuff' ],
		];
		...
		$manager->setMainStash( 'foo' );
		$this->assertInstanceOf( 'HashBagOStuff', $manager->getMainStashInstance() );

		$manager->setMainStash( 'bar' );
		$this->assertInstanceOf( 'EmptyBagOStuff', $manager->getMainStashInstance() );
	}
