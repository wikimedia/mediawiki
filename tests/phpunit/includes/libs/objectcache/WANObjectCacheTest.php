...
class WANObjectCacheTest extends MediaWikiTestCase {
	...
	public function testLockTSE() {
		...
		$cache->delete( $key );
		$ret = $cache->getWithSetCallback( $key, 30, $func,
			[ 'lockTSE' => 5, 'checkKeys' => $checkKeys ] ); // should save interim value
	...
	public function testBusyValue() {
		...
		$ret = $cache->getWithSetCallback( $key, 30, $func,
			[ 'busyValue' => $busyValue, 'checkKeys' => $checkKeys ] );
		$this->assertEquals( $busyValue, $ret, 'Callback was not used; busy' );
		$this->assertEquals( 2, $calls, 'Callback was not used; busy' );

		$this->internalCache->unlock( $key );
		$ret = $cache->getWithSetCallback( $key, 30, $func,
			[ 'busyValue' => $busyValue, 'checkKeys' => $checkKeys ] ); // should save interim value
