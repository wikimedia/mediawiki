...
class HashRingTest extends PHPUnit\Framework\TestCase {
	 ...
	 * @covers HashRing
	 */
	public function testHashRingLong() {
	...
	public function testHashRingEjection() {
		...
		$liveRing = $ring->getLiveRing();

		$this->assertEquals(
			[ 's1' => 5, 's2' => 5, 's4' => 10, 's5' => 5 ],
			$liveRing->getLiveLocationWeights(),
		...
		for ( $i = 0; $i < 100; ++$i ) {
			...
			$this->assertNotEquals( 's3', $liveRing->getLocation( $key ), 'ejected' );
			$this->assertNotEquals( 's6', $liveRing->getLocation( $key ), 'ejected' );

			if ( !in_array( $ring->getLocation( $key ), [ 's3', 's6' ] ) ) {
				$this->assertEquals(
					$ring->getLocation( $key ),
					$liveRing->getLocation( $key ),
					'Live ring otherwise matches.'
				);
				$this->assertEquals(
					$ring->getLocations( $key, 1 ),
					$liveRing->getLocations( $key, 1 ),
					'Live ring otherwise matches.'
				);
			}
			...
			$this->assertEquals(
				...
				'Live ring wrapper matches.'
			);
			$this->assertEquals(
				$liveRing->getLocations( $key, 2 ),
				$ring->getLiveLocations( $key, 2 ),
				'Live ring wrapper matches.'
			);
		}
	}
