...
class DatabaseSQLTest extends PHPUnit\Framework\TestCase {
	...
	public function testAtomicSections() {
		...
		$otrCallback = function ( $trigger ) use ( $fname ) {
		...
		$this->database->startAtomic( __METHOD__ . '_outermost', IDatabase::ATOMIC_CANCELABLE );
		$this->database->startAtomic( __METHOD__ . '_outer', IDatabase::ATOMIC_CANCELABLE );
		$this->database->startAtomic( __METHOD__, IDatabase::ATOMIC_CANCELABLE );
		$this->database->onTransactionResolution( $otrCallback, __METHOD__ );
		$this->database->endAtomic( __METHOD__ );
		$this->database->onTransactionResolution( $otrCallback, __METHOD__ );
		$this->database->cancelAtomic( __METHOD__ . '_outer' );
		$this->database->onTransactionResolution( $otrCallback, __METHOD__ );
		$this->database->endAtomic( __METHOD__ . '_outermost' );
		// phpcs:ignore Generic.Files.LineLength
		$this->assertLastSql( 'BEGIN; SAVEPOINT wikimedia_rdbms_atomic1; SAVEPOINT wikimedia_rdbms_atomic2; RELEASE SAVEPOINT wikimedia_rdbms_atomic2; ROLLBACK TO SAVEPOINT wikimedia_rdbms_atomic1; COMMIT; SELECT \'reverted\'; SELECT \'reverted\'' );
		$this->database->begin( __METHOD__ );
		try {
			$this->database->doAtomicSection( __METHOD__, function () {
