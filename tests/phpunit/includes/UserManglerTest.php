...
class UserManglerTest extends MediaWikiTestCase {
	...
	public function testChangePassword() {
		...
		$userAnon = $this->getMockBuilder( 'User' )
			->getMock();
		$userAnon->expects( $this->any() )->method( 'getName' )->willReturn( '192.0.2.1' );
		$userAnon->expects( $this->any() )->method( 'isAnon' )->willReturn( true );

		$user = $this->getMockBuilder( 'User' )
			->getMock();
		$user->expects( $this->any() )->method( 'getName' )->willReturn( 'ResetPasswordUnitTestUser' );
		$user->expects( $this->any() )->method( 'isAnon' )->willReturn( false );
		$user->expects( $this->any() )->method( 'checkPassword' )->will( $this->returnCallback( function ( $p ) {
			return $p === 'old';
		} ) );
		...
		$user2 = $this->getMockBuilder( 'User' )
			->getMock();
		$user2->expects( $this->any() )->method( 'getName' )->willReturn( 'ResetPasswordUnitTestUser2' );
		$user2->expects( $this->any() )->method( 'isAnon' )->willReturn( false );
		$user2->expects( $this->any() )->method( 'checkPassword' )->will( $this->returnCallback( function ( $p ) {
			return $p === 'old';
		} ) );
	...
	public function testChangeEmail() {
		...
		$user->expects( $this->any() )->method( 'setEmailWithConfirmation' )->willReturn( Status::newGood() );
