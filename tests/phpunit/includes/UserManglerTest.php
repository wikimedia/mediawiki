...
class UserManglerTest extends MediaWikiTestCase {
	...
	public function testChangePassword() {
		...
		$user = $this->getMockBuilder( 'User' )
			->getMock();
		$user->method( 'getName' )->willReturn( 'ResetPasswordUnitTestUser' );
		$user->method( 'isAnon' )->willReturn( false );
		$user->method( 'checkPassword' )->will( $this->returnCallback( function ( $p ) {
			return $p === 'old';
		} ) );
		...
		$user2 = $this->getMockBuilder( 'User' )
			->getMock();
		$user2->method( 'getName' )->willReturn( 'ResetPasswordUnitTestUser2' );
		$user2->method( 'isAnon' )->willReturn( false );
		$user2->method( 'checkPassword' )->will( $this->returnCallback( function ( $p ) {
			return $p === 'old';
		} ) );
		...
		$user2->method( 'setPassword' )->will( $this->throwException( new PasswordError( 'error!' ) ) );
		$status = $mangler->changePassword( 'old', 'new', 'new' );
		$this->assertFalse( $status->isOk() );
	...
	public function testChangeEmail() {
		...
		$user->method( 'setEmailWithConfirmation' )->willReturn( Status::newGood() );
