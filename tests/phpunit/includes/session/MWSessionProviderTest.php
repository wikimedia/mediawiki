 ...
 * @group Session
 * @group Database
 * @covers MWSessionProvider
 */
class MWSessionProviderTest extends MediaWikiTestCase {

	/**
	 * @uses MWSessionManager
	 */
	public function testBasics() {
		$manager = new MWSessionManager();
		$logger = new Psr\Log\NullLogger();
		$config = new HashConfig();

		$provider = $this->getMockForAbstractClass( 'MWSessionProvider' );
	 ...
	 * @param bool $ok Whether a MWSessionInfo is provided
	 */
	public function testNewSessionInfo( $persistId, $persistUser, $ok ) {
		$manager = new MWSessionManager();

		$provider = $this->getMockBuilder( 'MWSessionProvider' )
			->setMethods( array( 'persistsUser', 'persistsSessionId' ) )
			->getMockForAbstractClass();
		$provider->expects( $this->any() )->method( 'persistsSessionId' )->willReturn( $persistId );
		$provider->expects( $this->any() )->method( 'persistsUser' )->willReturn( $persistUser );
		$provider->setManager( $manager );
	...
	public function testPersistsUser() {
		$provider = $this->getMockForAbstractClass( 'MWSessionProvider' );
		$provider->expects( $this->any() )->method( 'persistsSessionId' )->willReturn( true );
		$this->assertTrue( $provider->persistsUser() );

		$provider = $this->getMockForAbstractClass( 'MWSessionProvider' );
		$provider->expects( $this->any() )->method( 'persistsSessionId' )->willReturn( false );
	...
	public function testImmutableSessions() {
		$provider = $this->getMockBuilder( 'MWSessionProvider' )
			->setMethods( array( 'persistsUser', 'persistsSessionId' ) )
			->getMockForAbstractClass();
		$provider->expects( $this->any() )->method( 'persistsUser' )->willReturn( true );
		...
		$provider = $this->getMockBuilder( 'MWSessionProvider' )
			->setMethods( array( 'persistsUser', 'persistsSessionId' ) )
			->getMockForAbstractClass();
		$provider->expects( $this->any() )->method( 'persistsUser' )->willReturn( false );
		try {
	...
	public function testHashToSessionId() {
		...
		$provider = $this->getMockForAbstractClass( 'MWSessionProvider', array(), 'MockSessionProvider' );
		$provider->setConfig( $config );
		$priv = TestingAccessWrapper::newFromObject( $provider );

		$this->assertSame( '2refg4855cgh73pt6gooh32j9da825g4', $priv->hashToSessionId( 'foobar' ) );
		$this->assertSame( '1hjeglrlap79giv20llta52pj3u27dtj', $priv->hashToSessionId( 'foobar', 'secret' ) );
	}
