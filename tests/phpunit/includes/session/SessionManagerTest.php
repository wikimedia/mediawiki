...
class SessionManagerTest extends MediaWikiTestCase {
	...
	public function testAutoCreateUser() {
		...
		$this->assertSame( array(
			array( LogLevel::DEBUG, 'creating new user (UTSessionAutoCreate1) - from: XXX' ),
		), $logger->getBuffer() );
		$logger->clearBuffer();
		...
		$this->assertSame( array(
			array( LogLevel::DEBUG, 'creating new user (UTSessionAutoCreate2) - from: XXX' ),
		), $logger->getBuffer() );
		$logger->clearBuffer();
		...
		$this->assertSame( array(
			array( LogLevel::DEBUG, 'creating new user (UTSessionAutoCreate3) - from: XXX' ),
		), $logger->getBuffer() );
		$logger->clearBuffer();
		...
		$this->assertSame( array(
			array( LogLevel::DEBUG, 'creating new user (UTSessionAutoCreate4) - from: XXX' ),
		), $logger->getBuffer() );
		$logger->clearBuffer();
	}
	...
	public function testLoadSessionInfoFromStore() {
		...
		$logger = new \TestLogger( true, function ( $m ) {
			...
			$m = preg_replace( '/(\[Exception \w+\()[^)]+(\) [^]]+\])/', '\1\2', $m );
			return $m;
		} );
		$manager->setLogger( $logger );
		$request = new \FauxRequest();
		...
		$this->assertFalse( $loadSessionInfoFromStore( $info ) );
		$this->assertSame( array(
			array( LogLevel::ERROR, 'Session "X": [Exception InvalidArgumentException() Invalid ID]' ),
		), $logger->getBuffer() );
		$logger->clearBuffer();
		...
		$this->assertFalse( $loadSessionInfoFromStore( $info ) );
		$this->assertSame( array(
			array( LogLevel::ERROR, 'Session "X": [Exception InvalidArgumentException() Invalid user name]' ),
		), $logger->getBuffer() );
		$logger->clearBuffer();
		...
		$this->assertFalse( $loadSessionInfoFromStore( $info ) );
		$this->assertSame( array(
			array( LogLevel::WARNING, 'Session "X": Metadata merge failed: [Exception UnexpectedValueException() no merge!]' ),
		), $logger->getBuffer() );
		$logger->clearBuffer();
