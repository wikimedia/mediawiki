 ...
 * @group Session
 * @group Database
 * @covers MWSessionInfo
 * @uses MWSessionUserInfo
 */
class MWSessionInfoTest extends MediaWikiTestCase {
	...
	public function testBasics() {
		$anon = MWSessionUserInfo::newAnonymous();
		$user = MWSessionUserInfo::newFromName( 'UTSysop', true );
		$unauthUser = MWSessionUserInfo::newFromName( 'UTSysop', false );

		try {
			new MWSessionInfo( MWSessionInfo::MIN_PRIORITY - 1, array() );
		...
		try {
			new MWSessionInfo( MWSessionInfo::MAX_PRIORITY + 1, array() );
		...
		try {
			new MWSessionInfo( MWSessionInfo::MIN_PRIORITY, array( 'id' => 'ABC?' ) );
		...
		try {
			new MWSessionInfo( MWSessionInfo::MIN_PRIORITY, array( 'user' => new stdclass ) );
		...
		try {
			new MWSessionInfo( MWSessionInfo::MIN_PRIORITY, array() );
		...
		$info = new MWSessionInfo( MWSessionInfo::MIN_PRIORITY + 5, array(
			'provider' => $provider,
			'user' => $anon
		) );
		$this->assertSame( $provider, $info->getProvider() );
		$this->assertNotNull( $info->getId() );
		$this->assertSame( MWSessionInfo::MIN_PRIORITY + 5, $info->getPriority() );
		...
		$this->assertSame(
			'[' . MWSessionInfo::MIN_PRIORITY . "]null<null>$id",
			(string)$info,
			'toString'
		);
		...
		$this->assertSame(
			'[' . MWSessionInfo::MIN_PRIORITY . "]Mock<+:{$user->getId()}:UTSysop>$id",
			(string)$info,
			'toString'
		);
	...
	/**
	 * @uses MWSessionManager::validateSessionId
	 */
	public function testCompare() {
		$id = 'aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa';
		$info1 = new MWSessionInfo( MWSessionInfo::MIN_PRIORITY + 1, array( 'id' => $id ) );
		$info2 = new MWSessionInfo( MWSessionInfo::MIN_PRIORITY + 2, array( 'id' => $id ) );

		$this->assertTrue( MWSessionInfo::compare( $info1, $info2 ) < 0, '<' );
		$this->assertTrue( MWSessionInfo::compare( $info2, $info1 ) > 0, '>' );
		$this->assertTrue( MWSessionInfo::compare( $info1, $info1 ) === 0, '==' );
	}
	...
	public function testLoadFromStore() {
		...
		$this->assertTrue( $info->loadFromStore( $manager, $store, $logger, $request ) );
		$this->assertInstanceOf( 'MWSessionUserInfo', $info->getUser() );
		...
		$this->mergeMwGlobalArrayValue( 'wgHooks', array(
			'MWSessionCheckInfo' => array( function ( &$reason, $i, $r, $m, $d ) use ( $that, $info, $metadata, $data, $request, &$called ) {
