...
class SessionBackendTest extends MediaWikiTestCase {
	...
	protected function getBackend( User $user = null ) {
		...
		$backend = new SessionBackend( $info, $this->store, 10 );
	...
	public function testConstructor() {
		...
		try {
			new SessionBackend( $info, $this->store, 10 );
	...
	/**
	 * @covers MediaWiki\Session\SessionManager::changeBackendId
	 */
	public function testResetId() {
		...
		$ref = &$backend->getIdRef();
		$backend->resetId();
		$this->assertSame( self::SESSIONID, $backend->getId() );
		$this->assertSame( $backend->getId(), $ref );
	 ...
	 * @uses MediaWiki\Session\SessionManager
	 * @uses MediaWiki\Session\Session
	 * @uses MediaWiki\Session\PHPSessionHandler
	 *
	 * No idea why, but...
	 * @uses MediaWiki\Session\CookieSessionProvider
	 */
