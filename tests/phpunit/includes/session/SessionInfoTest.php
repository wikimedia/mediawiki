...
class SessionInfoTest extends MediaWikiTestCase {
	...
	public function testLoadFromStore() {
		...
		$store->set( $metaKey, array( 'userId' => 0, 'userName' => '<X>', 'userToken' => null ) + $metadata );
		$info = new SessionInfo( SessionInfo::MIN_PRIORITY, array(
			'provider' => $provider,
			'id' => $id,
		) );
		...
		$store->set( $metaKey, array( 'userId' => $user->getId() + 1, 'userToken' => null ) + $metadata );
		$info = new SessionInfo( SessionInfo::MIN_PRIORITY, array(
			'provider' => $provider,
			'id' => $id,
			'user' => $user
		...
		$store->set( $metaKey, array( 'userId' => 0, 'userName' => null, 'userToken' => null ) + $metadata );
		$info = new SessionInfo( SessionInfo::MIN_PRIORITY, array(
			'provider' => $provider,
			'id' => $id,
			'user' => $user
		...
		// Lookup user by name
		$store->set( $metaKey, array( 'userId' => 0, 'userName' => 'UTSysop', 'userToken' => null ) + $metadata );
		$info = new SessionInfo( SessionInfo::MIN_PRIORITY, array(
			'provider' => $provider,
			'id' => $id,
		) );
		...
		$this->mergeMwGlobalArrayValue( 'wgHooks', array(
			'SessionCheckInfo' => array( function ( &$reason, $i, $r, $m, $d ) use ( $that, $info, $metadata, $data, $request, &$called ) {
