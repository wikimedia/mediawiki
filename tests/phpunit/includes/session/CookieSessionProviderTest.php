...
class CookieSessionProviderTest extends MediaWikiTestCase {
	...
	public function testConstructor() {
		try {
			...
			$this->assertSame(
				'CookieSessionProvider::__construct: priority must be specified',
				$ex->getMessage()
			);
		}
	 ...
	 * @uses MWSessionManager::__construct
	 * @uses MWSessionManager::setLogger
	 * @uses MWSessionManager::validateSessionId
	 * @uses MWSessionManager::generateSessionId
	 */
	public function testProvideSessionInfo() {
		...
		$provider->setManager( new MWSessionManager() );
	...
	protected function getSentRequest() {
		...
		$sentResponse->expects( $this->any() )->method( 'headersSent' )->willReturn( true );
		...
		$sentRequest->expects( $this->any() )->method( 'response' )->willReturn( $sentResponse );
		return $sentRequest;
	}
