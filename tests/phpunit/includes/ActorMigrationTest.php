...
class ActorMigrationTest extends MediaWikiLangTestCase {
	...
	public function provideGetWhere() {
		...
		return [
			...
			'Image, old' => [
				MIGRATION_OLD, 'img_user', $genericUser, true, [
					'tables' => [],
					'orconds' => [ 'userid' => "img_user = '1'" ],
					'joins' => [],
				],
			],
			'Image, write-both' => [
				MIGRATION_WRITE_BOTH, 'img_user', $genericUser, true, [
					'tables' => [
						'temp_img_user' => 'image_actor_temp',
					],
					'orconds' => [
						...
						'userid' => "temp_img_user.imgactor_actor IS NULL AND img_user = '1'"
					],
					'joins' => [
						'temp_img_user' => [ 'LEFT JOIN', 'temp_img_user.imgactor_name = img_name' ],
					],
			...
			'Image, write-new' => [
				MIGRATION_WRITE_NEW, 'img_user', $genericUser, true, [
					'tables' => [
						'temp_img_user' => 'image_actor_temp',
					],
					'orconds' => [
			...
			'Image, new' => [
				MIGRATION_NEW, 'img_user', $genericUser, true, [
					'tables' => [
						'temp_img_user' => 'image_actor_temp',
					],
					'orconds' => [ 'actor' => "temp_img_user.imgactor_actor = '11'" ],
					'joins' => [
						'temp_img_user' => [ 'JOIN', 'temp_img_user.imgactor_name = img_name' ],
					],
				],
	...
	public function testInsertUserIdentity() {
		...
		$m = $this->makeMigration( MIGRATION_WRITE_BOTH, 'rev_user' );
		list( $fields, $callback ) = $m->getInsertValuesWithTempTable( $this->db, $userIdentity );
		$extraFields = [
			'rev_page' => 42,
			'rev_text_id' => 42,
			'rev_len' => 0,
		...
		$m = $this->makeMigration( MIGRATION_WRITE_BOTH, 'dummy_user' );
		$fields = $m->getInsertValues( $this->db, $userIdentity );
