...
class ActorMigrationTest extends MediaWikiLangTestCase {
	 ...
	 * @param array $expect
	 */
	public function testGetWhere( $stage, $key, $userRows, $expect ) {
		...
		$m = $this->makeMigration( $stage, $key );
		$result = $m->getWhere( $this->db, $users );
		$this->assertEquals( $expect, $result );
	}
	...
	public static function provideGetWhere() {
		...
		return [
			'Simple table, old' => [
				MIGRATION_OLD, 'rc_user', $genericRows, [
					'tables' => [],
					'orconds' => [ "rc_user = '1'" ],
					'joins' => [],
				],
			...
			'Simple table, write-both' => [
				MIGRATION_WRITE_BOTH, 'rc_user', $genericRows, [
					'tables' => [],
			...
			'Simple table, write-new' => [
				MIGRATION_WRITE_NEW, 'rc_user', $genericRows, [
					'tables' => [],
					'orconds' => [ "rc_actor = '11'", "rc_actor = '0' AND rc_user = '1'" ],
					'joins' => [],
				],
			...
			'Simple table, new' => [
				MIGRATION_NEW, 'rc_user', $genericRows, [
					'tables' => [],
					'orconds' => [ "rc_actor = '11'" ],
					'joins' => [],
				],
			...
			'ipblocks, old' => [
				MIGRATION_OLD, 'ipb_by', $genericRows, [
					'tables' => [],
					'orconds' => [ "ipb_by = '1'" ],
					'joins' => [],
				],
			...
			'ipblocks, write-both' => [
				MIGRATION_WRITE_BOTH, 'ipb_by', $genericRows, [
					'tables' => [],
			...
			'ipblocks, write-new' => [
				MIGRATION_WRITE_NEW, 'ipb_by', $genericRows, [
					'tables' => [],
					'orconds' => [ "ipb_by_actor = '11'", "ipb_by_actor = '0' AND ipb_by = '1'" ],
					'joins' => [],
				],
			...
			'ipblocks, new' => [
				MIGRATION_NEW, 'ipb_by', $genericRows, [
					'tables' => [],
			...
			'Revision, old' => [
				MIGRATION_OLD, 'rev_user', $genericRows, [
					'tables' => [],
					'orconds' => [ "rev_user = '1'" ],
					'joins' => [],
				],
			...
			'Revision, write-both' => [
				MIGRATION_WRITE_BOTH, 'rev_user', $genericRows, [
					'tables' => [
						'temp_rev_user' => 'revision_actor_temp',
					],
					'orconds' => [
			...
			'Revision, write-new' => [
				MIGRATION_WRITE_NEW, 'rev_user', $genericRows, [
					'tables' => [
						'temp_rev_user' => 'revision_actor_temp',
					],
					'orconds' => [
			...
			'Revision, new' => [
				MIGRATION_NEW, 'rev_user', $genericRows, [
					'tables' => [
						'temp_rev_user' => 'revision_actor_temp',
					],
			...
			'Image, old' => [
				MIGRATION_OLD, 'img_user', $genericRows, [
					'tables' => [],
					'orconds' => [ "img_user = '1'" ],
					'joins' => [],
				],
			...
			'Image, write-both' => [
				MIGRATION_WRITE_BOTH, 'img_user', $genericRows, [
					'tables' => [
						'temp_img_user' => 'image_actor_temp',
					],
					'orconds' => [
						"temp_img_user.imgactor_actor = '11'",
			...
			'Image, write-new' => [
				MIGRATION_WRITE_NEW, 'img_user', $genericRows, [
					'tables' => [
						'temp_img_user' => 'image_actor_temp',
					],
					'orconds' => [
			...
			'Image, new' => [
				MIGRATION_NEW, 'img_user', $genericRows, [
					'tables' => [
						'temp_img_user' => 'image_actor_temp',
					],
			...
			'Multiple users, old' => [
				MIGRATION_OLD, 'rc_user', $complicatedRows, [
					'tables' => [],
					'orconds' => [
						"rc_user IN ('1','2','3') ",
			...
			'Multiple users, write-both' => [
				MIGRATION_WRITE_BOTH, 'rc_user', $complicatedRows, [
					'tables' => [],
					'orconds' => [
			...
			'Multiple users, write-new' => [
				MIGRATION_WRITE_NEW, 'rc_user', $complicatedRows, [
					'tables' => [],
					'orconds' => [
						"rc_actor IN ('11','12','34') ",
						"rc_actor = '0' AND rc_user IN ('1','2','3') ",
			...
			'Multiple users, new' => [
				MIGRATION_NEW, 'rc_user', $complicatedRows, [
					'tables' => [],
	...
	public function testInsertRoundTrip( $table, $key, $pk, $extraFields ) {
		...
		foreach ( $stages as $writeStage => $readRange ) {
			...
			if ( $usesTemp ) {
				list( $fields, $callback ) = $w->getInsertValuesWithTempTable( $user );
			} else {
				$fields = $w->getInsertValues( $user );
			}
	...
	public function testInsertWrong( $stage ) {
		...
		$m->getInsertValues( $this->getTestUser()->getUser() );
	}
	...
	public function testInsertWithTempTableWrong( $stage ) {
		...
		$m->getInsertValuesWithTempTable( $this->getTestUser()->getUser() );
	}
	...
	public function testInsertWithTempTableDeprecated( $stage ) {
		...
		$m = $this->makeMigration( $stage, 'rc_user' );
		list( $fields, $callback ) = $m->getInsertValuesWithTempTable( $this->getTestUser()->getUser() );
