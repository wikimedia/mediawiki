...
class CategoryMembershipChangeJobTest extends MediaWikiTestCase {
	...
	public function testRun_normalCategoryAddedAndRemovedOnUsedPage() {
		...
		$this->assertEquals(
			'(recentchanges-page-added-to-category-bundled: ' . $this->titles['used'] . ', 1)',
			$this->getCategorizeRecentChangeForRevId( $addedRevId )->getAttribute( 'rc_comment' )
		);
		$this->assertEquals(
			'(recentchanges-page-removed-from-category-bundled: ' . $this->titles['used'] . ', 1)',
			$this->getCategorizeRecentChangeForRevId( $removedRevId )->getAttribute( 'rc_comment' )
		);
	}
	...
	public function testRun_includeOnlyCategoryAddedAndRemovedOnUsedPage() {
		...
		$this->assertEquals(
			'(recentchanges-page-added-to-category-transcluded: ' . $this->titles['used'] . ', 1)',
			$this->getCategorizeRecentChangeForRevId( $addedRevId )->getAttribute( 'rc_comment' )
		);
		$this->assertEquals(
			'(recentchanges-page-removed-from-category-transcluded: ' . $this->titles['used']. ', 1)',
			$this->getCategorizeRecentChangeForRevId( $removedRevId )->getAttribute( 'rc_comment' )
		);
	}
	...
	public function testRun_noIncludeCategoryAddedAndRemovedOnUsedPage() {
		...
		$this->assertEquals(
			'(recentchanges-page-added-to-category: ' . $this->titles['used'] . ', 1)',
			$this->getCategorizeRecentChangeForRevId( $addedRevId )->getAttribute( 'rc_comment' )
		);
		$this->assertEquals(
			'(recentchanges-page-removed-from-category: ' . $this->titles['used']. ', 1)',
			$this->getCategorizeRecentChangeForRevId( $removedRevId )->getAttribute( 'rc_comment' )
		);
	}
