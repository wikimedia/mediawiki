...
class EnhancedChangesListTest extends MediaWikiTestCase {
	...
	/**
	 * @covers EnhancedChangesList::recentChangesLine
	 */
	public function testFirstRecentChangesLine() {
		...
		$line = $changesList->recentChangesLine( $changes[0] );
	 ...
	 */
	public function testOutputChangesBlock( $position, $matchers, $patterns ) {
		$changes = $this->getRecentChanges();
		$changesList = new EnhancedChangesList( $this->getContext() );
		$changesList->beginRecentChangesList();
	...
	public function testEndRecentChangesList() {
		...
		foreach( $this->getEndLinkPatterns() as $pattern ) {
