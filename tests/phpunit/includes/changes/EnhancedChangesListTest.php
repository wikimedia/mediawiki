...
class EnhancedChangesListTest extends MediaWikiTestCase {
	...
	public function testBeginRecentChangesList() {
		if ( !self::$changesList ) {
			self::$changesList = new EnhancedChangesList( $this->getContext() );
		}

		$this->assertTrue( self::$changesList instanceof EnhancedChangesList );

		self::$changesList->beginRecentChangesList();

		$out = self::$changesList->getContext()->getOutput();
	...
	/**
	 * @depends testBeginRecentChangesList
	 * @dataProvider firstRecentChangesLineProvider
	 */
	public function testFirstRecentChangesLine( $change, $matchers ) {
		$line = self::$changesList->recentChangesLine( $change, false );
	...
	/**
	 * @depends testFirstRecentChangesLine
	 */
	public function testEmptyRecentChangesLines() {
		$changes = $this->getRecentChanges();

		foreach( range( 1, 3 ) as $i ) {
			$line = self::$changesList->recentChangesLine( $changes[$i], false );
			$this->assertEquals( '', $line, "change line $i is empty" );
		}
	}

	/**
	 * @depends testEmptyRecentChangesLines
	 */
	public function testFifthRecentChangesLine() {
		$changes = $this->getRecentChanges();

		$line = self::$changesList->recentChangesLine( $changes[4], false );
	...
	/**
	 * @depends testFifthRecentChangesLine
	 */
	public function testEndRecentChangesList() {
		...
		foreach( $changes as $change ) {
			self::$changesList->recentChangesLine( $change, false );
		}

		$end = self::$changesList->endRecentChangesList();
