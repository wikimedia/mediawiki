...
class WANObjectCacheTest extends MediaWikiTestCase {
	...
	protected function setUp() {
		...
		$this->cache = TestingAccessWrapper::newFromObject( $this->cache );
	}
	...
	public function testSetAndGet( $value, $ttl ) {
		...
		$curTTL = null;
		$this->assertEquals( $this->cache->get( $key, $curTTL ), $value );
	...
	/**
	 * @convers WANObjectCache::getWithSetCallback()
	 */
	...
	public function testGetMulti() {
		...
		$this->assertEquals(
			...
			$this->cache->getMulti( array( $key1, $key2, $key3 ), $curTTLs )
		);
		...
		$this->assertEquals(
			array(),
			$this->cache->getMulti( array( $key1, $key2, $key3 ), $curTTLs ),
			"Result array empty due to check keys"
		);
		$this->assertEquals( array(), $curTTLs, "Current TTLs array empty" );
		$t1 = $this->cache->getCheckKeyTime( $cKey1 );
		$this->assertGreaterThanOrEqual( $priorTime, $t1, 'Check key 1 generated on miss' );
		$t2 = $this->cache->getCheckKeyTime( $cKey2 );
		$this->assertGreaterThanOrEqual( $priorTime, $t2, 'Check key 2 generated on miss' );
	}

	/**
	 * @convers WANObjectCache::delete()
	 */
	...
	/**
	 * @convers WANObjectCache::touchCheckKey()
	...
	public function testTouchKeys() {
		...
		$this->assertGreaterThanOrEqual( $priorTime, $t1, 'Check key generated on miss' );
