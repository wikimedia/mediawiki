...
class LogFormatterTest extends MediaWikiLangTestCase {
	...
	public function testLogParamsTypeUserLinkTools() {
		$params = array( '4:user-link-tools:userLinkTools' => $this->user->getName() );
		$expected = Linker::userLink(
				$this->user->getId(),
				$this->user->getName()
			) .
	...
	public function testLogParamsTypeTitleLinkParams() {
		$params = array( '4:title-link-params:titleLinkParams' => $this->title->getText() . '|action=edit' );
		$expected = Linker::link( $this->title, null, array(), array( 'action' => 'edit' ) );
