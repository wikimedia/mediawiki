...
class EtcConfigTest extends PHPUnit_Framework_TestCase {
	 ...
	 *
	 * - [x] Cache miss with backend failure
	 *       Result: ConfigException
	 *       > cache miss | gets lock | backend fails (no retry)
	 *
	 ...
	 *
	 * - [x] Cache expired with backend timeout
	 *       Result: Cached value (stale)
	 *       > cache expired | gets lock | backend fail (timeout allows retry)
	 *
	 ...
	 *
	 * - [ ] Cache expired with backend failure (no retry)
	 *       Result: ConfigException
	 *       > cache expired | gets lock | backend fails
	 *       FIXME: Use stale value?
	 *
	 * - [ ] Backend retry
	 *       Result: Fetched value
	 *       > cache miss | gets lock | backend times out | backend retry
	 *
	 * - [ ] Wait loop timeout (between re-tries)
	 *       Result: ConfigException
	 *       > cache miss | gets lock | backend times out | no retry (loop timeout)
	 */
	 ...
	 * @covers EtcdConfig::load
	 */
	public function testLoadCacheMissBackendFail() {
		// Create cache mock
		$cache = $this->getMockBuilder( HashBagOStuff::class )
			->setMethods( [ 'get', 'lock' ] )
			->getMock();
	 ...
	 * @covers EtcdConfig::load
	 */
	public function testLoadCacheExpiredLockFetchTimeout() {
		// Create cache mock
		$cache = $this->getMockBuilder( HashBagOStuff::class )
			->setMethods( [ 'get', 'lock' ] )
			->getMock();
