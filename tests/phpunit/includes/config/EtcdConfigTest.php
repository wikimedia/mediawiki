...
class EtcConfigTest extends PHPUnit_Framework_TestCase {
	 ...
	 *       Result: ConfigException (FIXME: Use stale cache instead)
	 *       > cache expired | gets lock | backend fails (no retry)
	 *
	 ...
	 */
	public function testLoadCacheMissWithLock() {
		// Create cache mock
		$cache = $this->getMockBuilder( HashBagOStuff::class )
			->setMethods( [ 'get', 'lock' ] )
			->getMock();
	 ...
	 * @covers EtcdConfig::load
	 */
	public function testLoadCacheExpiredLockFetchFailed() {
		// Create cache mock
		$cache = $this->getMockBuilder( HashBagOStuff::class )
			->setMethods( [ 'get', 'lock' ] )
			->getMock();
