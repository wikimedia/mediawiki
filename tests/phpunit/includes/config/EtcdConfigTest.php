...
class EtcdConfigTest extends PHPUnit_Framework_TestCase {
	...
	private function createSimpleConfigMock( array $config, int $index=0 ) {
	...
	public function testLoadCacheMiss() {
		...
		$mock->expects( $this->once() )->method( 'fetchAllFromEtcd' )
			->willReturn( [ [ 'known' => 'from-fetch' ], null, false, 0 ] );
	...
	public function testLoadCacheMissBackendError() {
		...
		$mock->expects( $this->once() )->method( 'fetchAllFromEtcd' )
			->willReturn( [ null, 'Fake error', false, 0 ] );
	...
	public function testLoadCacheExpiredLockFetchFails() {
		...
		$mock->expects( $this->once() )->method( 'fetchAllFromEtcd' )
			->willReturn( [ null, 'Fake failure', true, 0 ] );
		...
		return [
			...
			'200 OK - Empty node list' => [
				...
				'expect' => [
					...
					null,
					false, // retry
					0 // Last modified index of a non-directory key
				],
			],
			'200 OK - Invalid JSON' => [
				'http' => [
