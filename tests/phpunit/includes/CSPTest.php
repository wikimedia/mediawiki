...
class CSPTest extends MediaWikiTestCase {
	 ...
	 */
	public function testMakeCSPDirectives( $policy, $expectedFull, $expectedReport, $expectedRestricted ) {
		$actualFull = $this->csp->makeCSPDirectives( $policy, CSP::FULL_MODE );
		$actualReport = $this->csp->makeCSPDirectives( $policy, CSP::REPORT_ONLY_MODE );
		$actualRestricted = $this->csp->makeCSPDirectives( $policy, CSP::FULL_MODE_RESTRICTED );
	...
	public function testCSPIsEnabled( $main, $reportOnly, $expected ) {
		...
		$res = CSP::isEnabled( RequestContext::getMain()->getConfig() );
