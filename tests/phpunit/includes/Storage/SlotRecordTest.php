...
class SlotRecordTest extends MediaWikiTestCase {
	...
	public function testNewInherited() {
		...
		// before saving slot meta-data.
		$saved = SlotRecord::newForSaving( 10, $inherited->getAddress(), $inherited );
	...
	public function testNewForSaving() {
		// This would happen while doing an edit, before saving revision meta-data.
		$unsaved = SlotRecord::newUnsaved( 'main', new WikitextContent( 'A' ) );

		// This would happen while doing an edit, after saving the revision meta-data,
		// before saving slot meta-data.
		$saved = SlotRecord::newForSaving( 10, 'theNewAddress', $unsaved );
	...
	public function provideNewForSaving_LogicException() {
		...
		yield 'mismatching address' => [ 1, 'address:BAD', $freshSlot ];
		yield 'mismatching revision' => [ 5, 'address:1', $freshSlot ];
		...
		yield 'inherited, but no address' => [ 1, 'address:2', $inheritedSlot ];
	}

	/**
	 * @dataProvider provideNewForSaving_LogicException
	 */
	public function testNewForSaving_LogicException(
		$revisionId,
		$contentAddress,
		SlotRecord $protoSlot
	) {
		$this->setExpectedException( LogicException::class );
		SlotRecord::newForSaving( $revisionId, $contentAddress, $protoSlot );
	}

	public function provideNewForSaving_InvalidArgumentException() {
		$unsaved = SlotRecord::newUnsaved( 'main', new WikitextContent( 'A' ) );

		yield 'bad revision id' => [ 'foo', 'address', $unsaved ];
		yield 'bad content address' => [ 7, 77, $unsaved ];
	}

	/**
	 * @dataProvider provideNewForSaving_InvalidArgumentException
	 */
	public function testNewForSaving_InvalidArgumentException(
		$revisionId,
		$contentAddress,
		SlotRecord $protoSlot
	) {
