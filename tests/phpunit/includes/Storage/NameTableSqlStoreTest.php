...
class NameTableSqlStoreTest extends MediaWikiTestCase {
	...
	private function getNameTableSqlStore( BagOStuff $cacheBag, $insertCalls, $selectCalls ) {
	...
	public function provideGetAndAcquireId() {
		return [
			'no wancache, empty table' =>
				[ new EmptyBagOStuff(), true, 2, [], 'foo', 1 ],
			'no wancache, one matching value' =>
				[ new EmptyBagOStuff(), false, 2, [ 'foo' ], 'foo', 1 ],
			'no wancache, one not matching value' =>
				[ new EmptyBagOStuff(), true, 2, [ 'bar' ], 'foo', 2 ],
			'no wancache, multiple, one matching value' =>
				[ new EmptyBagOStuff(), false, 2, [ 'foo', 'bar' ], 'bar', 2 ],
			'no wancache, multiple, no matching value' =>
				[ new EmptyBagOStuff(), true, 2, [ 'foo', 'bar' ], 'baz', 3 ],
	...
	public function testCacheRaceCondition() {
		...
		$store3 = $this->getNameTableSqlStore( $wanHashBag, 0, 0 );

		// Cache the current table in all instances (WAN and proc cache)
		$store1->getTable();
		$store2->getTable();
		$store3->getTable();

		// Store 2 separate names in each instance
		$fooId = $store1->acquireId( 'foo' );
		$barId = $store2->acquireId( 'bar' );

		// Try to get the IDs of both name added above
		$this->assertSame( $fooId, $store3->getId( 'foo' ) );
		$this->assertSame( $barId, $store3->getId( 'bar' ) );
	}
