...
class RevisionStoreDbTest extends MediaWikiTestCase {
	...
	private function assertRevisionRecordMatchesRevision( Revision $rev, RevisionRecord $record ) {
		...
		$this->assertSame( $rev->getParentId(), $record->getParentId() );
	...
	/**
	 * @covers RevisionStore::getParentLengths
	 */
	public function testGetParentLengths() {
		...
		$store = MediaWikiServices::getInstance()->getRevisionStore();
		// TODO if this ends up being int[] instead of string[] then switch to assertSame
		$this->assertEquals(
			[
				$revOne->getId() => strlen( __METHOD__ ),
			],
			$store->getParentLengths(
				wfGetDB( DB_MASTER ),
				[ $revOne->getId() ]
			)
		);
