			...
			[
				...
				'fields' => array_merge(
					...
					[
						...
						'rev_text_id' => 'COALESCE( SUBSTRING(a_content.content_address FROM 4), rev_text_id )',
				...
				'joins' => [
					...
					'user' => [ 'LEFT JOIN', [ 'rev_user != 0', 'user_id = rev_user' ] ],
					'text' => [ 'INNER JOIN', [ 'rev_text_id=old_id' ] ],
					'content_models' => [ 'LEFT JOIN', 'a_content.content_model = a_content_models.model_id' ],
					'slots' => [ 'LEFT JOIN', 'a_slots.slot_revision_id = rev_id' ],
					'content' => [ 'LEFT JOIN', 'a_content.content_id = a_slots.slot_content_id' ],
				],
			]
		];
		yield [
			...
			[
				...
				'fields' => array_merge(
					...
					[
						...
						'rev_text_id' => 'SUBSTRING(a_content.content_address FROM 4)',
						'content_id' => 'a_content.content_id',
					]
				),
				'joins' => [
					...
					'user' => [ 'LEFT JOIN', [ 'rev_user != 0', 'user_id = rev_user' ] ],
					'text' => [ 'INNER JOIN', [ 'rev_text_id=old_id' ] ],
					'content_models' => [ 'JOIN', 'a_content.content_model = a_content_models.model_id' ],
					'slots' => [ 'JOIN', 'a_slots.slot_revision_id = rev_id' ],
					'content' => [ 'JOIN', 'a_content.content_id = a_slots.slot_content_id' ],
				],
			]
		];
	}
			...
			[
				...
				'fields' => array_merge(
					...
					[
						...
						'ar_user' => 'ar_user',
						'ar_actor' => 'NULL',
						'ar_content_format' => 'ar_content_format',
						'ar_content_model' => 'COALESCE( a_content_models.model_name, ar_content_model )',
						'ar_text' => 'NULL',
						'ar_text_id' => 'COALESCE( SUBSTRING(a_content.content_address FROM 4), ar_text_id )',
					]
				),
				'joins' => [
					'content_models' => [ 'LEFT JOIN', 'a_content.content_model = a_content_models.model_id' ],
					'slots' => [ 'LEFT JOIN', 'a_slots.slot_revision_id = ar_rev_id' ],
					'content' => [ 'LEFT JOIN', 'a_content.content_id = a_slots.slot_content_id' ],
				],
			]
		];
			...
			[
				...
				'fields' => array_merge(
					...
					[
						...
						'ar_text' => 'NULL',
						'ar_text_id' => 'SUBSTRING(a_content.content_address FROM 4)',
					]
				),
				'joins' => [
					'content_models' => [ 'JOIN', 'a_content.content_model = a_content_models.model_id' ],
					'slots' => [ 'JOIN', 'a_slots.slot_revision_id = ar_rev_id' ],
					'content' => [ 'JOIN', 'a_content.content_id = a_slots.slot_content_id' ],
				],
			]
		];
