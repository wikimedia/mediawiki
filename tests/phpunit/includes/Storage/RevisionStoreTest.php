...
class RevisionStoreTest extends MediaWikiTestCase {
	...
	public function provideGetQueryInfo() {
		...
		yield [
			...
			[ 'page', 'user', 'text' ],
			[
				'tables' => [ 'revision', 'page', 'user', 'text', 'content_models', 'slots', 'content' ],
				'fields' => array_merge(
					...
					[
						...
						'rev_content_model' => 'COALESCE( model_name, rev_content_model )',
						'rev_text_id' => 'COALESCE( SUBSTR(content_address, 4), rev_text_id )',
					]
				),
				'joins' => [
					'page' => [ 'INNER JOIN', [ 'page_id = rev_page' ] ],
					...
					'slots' => [ 'LEFT JOIN', 'slot_revision_id = rev_id' ],
					'content' => [ 'LEFT JOIN', 'content_id = slot_content_id' ],
				],
			]
		];
		yield [
			...
			[
				...
				'fields' => array_merge(
					...
					[
						...
						'rev_text_id' => 'SUBSTR(content_address, 4)',
					]
				),
				'joins' => [
					'page' => [ 'INNER JOIN', [ 'page_id = rev_page' ] ],
					'user' => [ 'LEFT JOIN', [ 'rev_user != 0', 'user_id = rev_user' ] ],
					'text' => [ 'INNER JOIN', [ 'rev_text_id=old_id' ] ],
					'content_models' => [ 'JOIN', 'content_model = model_id' ],
	...
	public function testGetQueryInfo( $mcrMigration, $contentHandlerUseDb, $options, $expected ) {
		...
		$store = $this->getRevisionStore( null, null, null, $mcrMigration );
	...
	public function provideGetArchiveQueryInfo() {
		...
		yield [
			...
			true,
			[
				'tables' => [ 'archive', 'content_models', 'slots', 'content' ],
				'fields' => array_merge(
					...
					[
						...
						'ar_content_model' => 'COALESCE( model_name, ar_content_model )',
						'ar_text' => 'NULL',
						'ar_text_id' => 'COALESCE( SUBSTR(content_address, 4), ar_text_id )',
					]
				),
				'joins' => [
					'content_models' => [ 'LEFT JOIN', 'content_model = model_id' ],
					'slots' => [ 'LEFT JOIN', 'slot_revision_id = ar_rev_id' ],
					'content' => [ 'LEFT JOIN', 'content_id = slot_content_id' ],
				],
			]
		];
			...
			[
				...
				'fields' => array_merge(
					...
					[
						...
						'ar_content_model' => 'model_name',
						'ar_text' => 'NULL',
						'ar_text_id' => 'SUBSTR(content_address, 4)',
					]
				),
				'joins' => [
					'content_models' => [ 'JOIN', 'content_model = model_id' ],
					'slots' => [ 'JOIN', 'slot_revision_id = ar_rev_id' ],
					'content' => [ 'JOIN', 'content_id = slot_content_id' ],
				],
			]
		];
