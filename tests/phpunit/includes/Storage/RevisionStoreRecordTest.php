...
class RevisionStoreRecordTest extends MediaWikiTestCase {
	...
	public function testConstructorAndGetters(
		...
		$this->assertSame( (boolean)$row->rev_minor_edit, $rec->isMinor(), 'getIsMinor' );
		...
		if ( isset( $row->page_latest ) ) {
			$this->assertSame( (int)$row->rev_id === (int)$row->page_latest, $rec->isCurrent(), 'isCurrent' );
		} else {
			$this->assertSame( false, $rec->isCurrent(), 'isCurrent' );
		}

		if ( isset( $row->user_name ) ) {
			$this->assertSame( $row->user_name, $rec->getUserText( RevisionRecord::RAW ), 'getUserText' );
		} else {
			$this->assertSame( $row->rev_user_text, $rec->getUserText( RevisionRecord::RAW ), 'getUserText' );
		}
	}
