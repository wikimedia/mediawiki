...
class DerivedPageDataUpdaterTest extends MediaWikiTestCase {
	 ...
	 */
	public function testGetPreparedEditAfterPrepareRevisionCreation() {
		$user = $this->getTestUser()->getUser();
		$page = $this->getPage( __METHOD__ );
		...
		$this->assertSame( 0, $preparedEdit->revid );
	}
	...
	public function testGetSecondaryDataUpdatesAfterPrepareRevisionCreation() {
		$user = $this->getTestUser()->getUser();
		$page = $this->getPage( __METHOD__ );
	...
	private function makeRevision( Title $title, RevisionSlotsUpdate $update, User $user, $comment, $id, $parentId = 0 ) {
	...
	public function provideIsReusableFor() {
		...
		$update1 = new MutableRevisionSlots();
		$update1->setContent( 'main', $content1 );

		$update1b = new MutableRevisionSlots();
		$update1b->setContent( 'xyz', $content1 );

		$update2 = new MutableRevisionSlots();
		$update2->setContent( 'main', $content2 );
		...
		yield 'match prepareRevisionCreation' => [
			'$prepUser' => $user1,
			'$prepRevision' => null,
			'$prepUpdate' => $update1,
			'$forUser' => $user1,
		...
		yield 'mismatch prepareRevisionCreation update' => [
			'$prepUser' => $user1,
			'$prepRevision' => null,
			'$prepUpdate' => $update1,
			'$forUser' => $user1,
		...
		yield 'mismatch prepareRevisionCreation user' => [
			'$prepUser' => $user1,
			'$prepRevision' => null,
			'$prepUpdate' => $update1,
			'$forUser' => $user2,
		...
		yield 'mismatch prepareRevisionCreation parent' => [
			'$prepUser' => $user1,
			'$prepRevision' => null,
			'$prepUpdate' => $update1,
			'$forUser' => $user1,
		...
		yield 'mismatch prepareUpdate user' => [
			'$prepUser' => null,
			'$prepRevision' => $rev1,
			'$prepUpdate' => null,
			'$forUser' => $user2,
		...
		yield 'mismatch prepareUpdate parent' => [
			'$prepUser' => null,
			'$prepRevision' => $rev1,
			'$prepUpdate' => null,
			'$forUser' => null,
	...
	public function testIsReusableFor(
		...
		if ( $prepUpdate ) {
			$updater->prepareRevisionCreation( $prepUser, $prepUpdate, false );
		}
