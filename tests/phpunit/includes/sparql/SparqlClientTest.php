...
JSON;
		...
		$client = new SparqlClient( 'http://query.wikidata.org/', $this->getRequestFactory( $request ) );
	...
	public function optionsProvider() {
		return [
			'defaults' => [
				...
				[
					'method' => 'GET',
					'userAgent' => "MediaWiki/{$GLOBALS['wgVersion']} SparqlClient",
					'timeout' => 30
				]
			],
			'big query' => [
	...
	public function testOptions( $sparql, $options, $timeout, $expectedUrl, $expectedOptions ) {
		$requestFactory = $this->getMock( HttpRequestFactory::class );
		$client = new SparqlClient( 'http://query.wikidata.org/',  $requestFactory );
