...
class CoreVariablesTest extends MediaWikiLangTestCase {
	 ...
	 */
	public function testVariableHooks( $canonical, $variable ) {
		MagicWord::$mVariableIDsInitialised = false; // to run the hook
		$this->setMwGlobals( [
			// inject the magic word into the valid ones
			'wgHooks' => [
				'MagicWordwgVariableIDs' => [
					function ( &$aCustomVariableIds ) use ( $canonical ) {
						$aCustomVariableIds[] = $canonical;
					}
				]
			],
			// inject aliases for the magic words
			'wgExtensionMessagesFiles' => [
				'CoreVariablesTest' => __DIR__ . '/CoreVariablesTest.i18n.php'
			],
		] );
		...
		MagicWord::$mVariableIDsInitialised = false; // to run the hook again and clear test aliases
	}
