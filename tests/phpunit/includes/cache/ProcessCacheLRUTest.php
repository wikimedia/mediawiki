...
class ProcessCacheLRUTest extends MediaWikiTestCase {
	...
	/**
	 * @dataProvider provideMaxCacheSizes
	 */
	function testConstruction( $maxSize, $msg = '' ) {
		$cacheObject = new ProcessCacheLRUTestable( $maxSize );
		$this->assertAttributeEquals( $maxSize,
			'maxCacheEntries', $cacheObject,
			$msg
		);
	}

	function provideMaxCacheSizes() {
		return array(
			array( 1 ),
			array( 2 ),
			array( 999 ),
