...
class MessageCacheTest extends MediaWikiLangTestCase {
	...
	/**
	 *
	 * @dataProvider provideArgumentsForLinkParserFunction
	 */
	public function testLinkParserFunction( $message, $expected, $argument ) {
		$this->assertEquals(
			"<p>$expected\n</p>",
			MessageCache::singleton()->parse( "{{#link:$argument}}" )->getText(),
			$message
		);
	}

	function provideArgumentsForLinkParserFunction() {
		return array(
			array(
				'Allows link url to current domain',
				'<a href="/w/index.php?title=Main_Page&amp;action=edit">edit</a>',
				Title::newMainPage()->getLinkURL( 'action=edit' ) . '|edit',
			),
			...
			array(
				...
				'<a href="#">probably</a>',
				'//icanhazwiki.foo/bar|probably',
			),

			array(
				'disallows urls that change domain',
				'<a href="#">not</a>',
				'https://qwe.rty/qwerty|not',
			),
		);
	}
}
