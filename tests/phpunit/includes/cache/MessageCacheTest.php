...
class MessageCacheTest extends MediaWikiLangTestCase {
	 ...
	 */
	public function testLinkParserFunction( $message, $expected, $href, $content ) {
		$this->assertEquals(
			"<p>$expected\n</p>",
			MessageCache::singleton()->parse( "<link href='$href'>$content</link>" )->getText(),
			$message
		);
	}
	...
	function provideArgumentsForLinkParserFunction() {
		return array(
			array(
				...
				'<a href="' . Title::newMainPage()->getLinkURL( 'action=edit' ) . '">edit</a>',
