...
abstract class WikiPageDbTestBase extends MediaWikiLangTestCase {
	...
	public function testPrepareContentForEdit() {
		...
		$edit4 = $page->prepareContentForEdit( $content2, null, $user, null, false );
		$this->assertNotEquals( $edit3, $edit4 );

		$pstContent = $edit4->pstContent;
...
";
	...
	/**
	 * @covers WikiPage::getPageUpdater
	 * @covers WikiPage::getMetaDataUpdater
	 */
	public function testGetPageUpdater() {
		...
		$revision =  $updater->doEdit( CommentStoreComment::newUnsavedComment( 'test' ), EDIT_NEW );

		$this->assertSame( $revision->getId(), $page->getLatest() );

		$metaDataUpdater = $page->getMetaDataUpdater( $user, $revision );
		$this->assertTrue( $metaDataUpdater->isUpdatePrepared() );
		$this->assertSame( $content, $metaDataUpdater->getSlots()->getContent( 'main' ) );
		$this->assertNotNull( $metaDataUpdater->getCanonicalParserOutput() );
	}
	 ...
	 * @covers WikiPage::getMetaDataUpdater
	 */
	public function testGetMetaDataUpdaterUpdater() {
		$admin = $this->getTestSysop()->getUser();
		...
		$slots->setContent( 'main', new WikitextContent( 'Hello World' ) );

		// get a virgin updater
		$updater1 = $page->getMetaDataUpdater( $user );
