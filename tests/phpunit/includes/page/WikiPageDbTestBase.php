...
abstract class WikiPageDbTestBase extends MediaWikiLangTestCase {
	...
	public function testPrepareContentForEdit() {
		...
		$this->assertContains( '</a>', $edit->getCombinedParserOutput()->getText(), "getParserOutput()" );
		$this->assertContains(
			'consetetur sadipscing elitr',
			$edit->getCombinedParserOutput()->getText(),
			"getParserOutput()"
		);
		...
		$this->assertSame( $edit->getCombinedParserOutput(), $edit->output, "output field" );
	...
	public function testDoEditUpdates() {
		...
		// FIXME: test that PST is *not* applied to the content in $revision
		// TODO: test various options
		...
		$this->assertEquals( 1, $n, 'pagelinks should contain one link from the update' );
	}
