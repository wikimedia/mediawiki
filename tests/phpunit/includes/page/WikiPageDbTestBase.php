...
";
	...
	public function testGetPageUpdater() {
		...
		$revision = $updater->createRevision( CommentStoreComment::newUnsavedComment( 'test' ), EDIT_NEW );

		$this->assertSame( $revision->getId(), $page->getLatest() );

		$metaDataUpdater = $page->getDerivedDataUpdater( $user, $revision );
	 ...
	 * @covers WikiPage::getDerivedDataUpdater
	 */
	public function testGetDerivedDataUpdaterUpdater() {
		$admin = $this->getTestSysop()->getUser();
		...
		$this->assertSame( $updater1, $page->getDerivedDataUpdater( $user, null, $revisionSlots ) );

		// Don't re-use with different user
		$updater2 = $page->getDerivedDataUpdater( $admin, $revision );
		$updater2->prepareUpdate( $revision );
		$this->assertNotSame( $updater1, $updater2 );

		// Don't re-use with different content
		$updater3 = $page->getDerivedDataUpdater( $admin, null, $slots );
