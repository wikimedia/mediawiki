...
class ReverseChronologicalPagerTest extends MediaWikiLangTestCase {
	...
	public function testGetDateCond() {
		...
		// Test year and month set to false. mOffset should stay the same.
		$pager->getDateCond( 2006, 7, 23 );
		$pager->getDateCond( false, false );
		$this->assertEquals( $pager->mOffset, $db->timestamp( '20060724000000' ) );
		...
		if ( $currMonth < 11 ) {
			$pager->getDateCond( -1, 11 );
			$this->assertEquals( $pager->mOffset, $db->timestamp( $currYear - 1 . '1201000000' ) );
		}
		if ( $currMonth < 12 ) {
