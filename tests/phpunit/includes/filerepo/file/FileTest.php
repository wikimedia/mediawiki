...
class FileTest extends MediaWikiMediaTestCase {
	...
	/**
	 * @dataProvider getThumbnailSourcePathProvider
	 * @covers File::getThumbnailSourcePath
	 */
	public function testgetThumbnailSourcePath( $data ) {
		...
		$this->assertEquals( $data['expectedPath'], $fileMock->getThumbnailSourcePath(
			array( 'physicalWidth' => $data['physicalWidth'] ) ), $data['message'] );
	}

	public function getThumbnailSourcePathProvider() {
		return array(
			array( array(
	...
	public function testGenerateBucketsIfNeeded( $data ) {
		...
		$fileMock = $this->getMockBuilder( 'File' )
			->setConstructorArgs( array( 'fileMock', $repoMock ) )
			->setMethods( array( 'getWidth', 'getBucketThumbPath', 'makeTransformTmpFile', 'generateAndSaveThumb' ) )
			->getMockForAbstractClass();
