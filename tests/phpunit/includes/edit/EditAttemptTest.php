...
class EditAttemptTest extends MediaWikiLangTestCase {
	...
	protected function assertPerformSave( $title, $user, $edit, $info = [], $options = [],
		...
		if ( $baseText !== null ) {
			...
			$edit->setLastRevisionTime( $page->getTimestamp() );
		...
		$edit->setContentModelAndFormat( $title->getContentModel(),
			ContentHandler::getForModelID( $title->getContentModel() )->getDefaultFormat() );

		$editAttempt = new EditAttempt( $page, $user, $edit );
		$info += [
			'autoSumm' => md5( '' ),
			'changeTags' => null,
			'undidRevId' => 0,
	 ...
	 * @covers EditAttempt::performSave
	 */
	public function testCreatePage(
		...
		$page = $this->assertPerformSave( $pageTitle, $user, $edit, [], $opts, null,
	...
	public function testUpdatePage() {
		...
		$edit = new Edit();
		$edit->setText( $text );
		$edit->setLastRevisionTime( '20120101000000' );
		$edit->setSummary( 'second update' );

		$this->assertPerformSave(
...
';
	 ...
	 * @covers EditAttempt::performSave
	 */
	public function testAutoMerge( $baseUser, $text, $adamsEditData, $bertasEditData,
		...
		$baseEdit = new Edit();
		$baseEdit->setText( $text );

		$page = $this->assertPerformSave( 'EditAttemptTest_testAutoMerge',
			$baseUser, $baseEdit, [], [], null, null, null, __METHOD__ );
		...
		if ( !isset( $adamsEditData['wpStarttime'] ) ) {
			$adamsEditData['wpStarttime'] = 1;
		}
		if ( !isset( $bertasEditData['wpStarttime'] ) ) {
			$bertasEditData['wpStarttime'] = 2;
		}

		$starttime = wfTimestampNow();
		$adamsTime = wfTimestamp(
			TS_MW,
			(int)wfTimestamp( TS_UNIX, $starttime ) + (int)$adamsEditData['wpStarttime']
		);
		$bertasTime = wfTimestamp(
			TS_MW,
			(int)wfTimestamp( TS_UNIX, $starttime ) + (int)$bertasEditData['wpStarttime']
		);

		$adamsEdit->setStartTime( $adamsTime );
		$bertasEdit->setStartTime( $bertasTime );

		$adamsEdit->setSummary( 'Adam\'s edit' );
		$bertasEdit->setSummary( 'Bertas\'s edit' );

		$adamsEdit->setLastRevisionTime( $editTime );
		$bertasEdit->setLastRevisionTime( $editTime );

		$adamsEdit->setText( rtrim( $adamsEditData['wpTextbox1'] ) );
		$bertasEdit->setText( rtrim( $bertasEditData['wpTextbox1'] ) );

		if ( isset( $adamsEditData['wpSection'] ) ) {
			$adamsEdit->setSection( $adamsEditData['wpSection'] );
		}
		if ( isset( $bertasEditData['wpSection'] ) ) {
			$bertasEdit->setSection( $bertasEditData['wpSection'] );
		}

		// first edit
		$this->assertPerformSave( 'EditAttemptTest_testAutoMerge', 'Adam', $adamsEdit,
			[], [], null, EditAttempt::AS_SUCCESS_UPDATE, null, "expected successfull update" );

		// second edit
		$this->assertPerformSave( 'EditAttemptTest_testAutoMerge', 'Berta', $bertasEdit,
