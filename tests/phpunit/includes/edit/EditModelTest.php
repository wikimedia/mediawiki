 ...
 * @group Editing
 */
class EditModelTest extends MediaWikiLangTestCase {
	...
	protected function assertInternalAttemptSave( $title, $user, $data, $info = [], $options = [],
		...
		if ( $baseText !== null ) {
			...
			// As in EditController, user input is rtrim()-ed, so we do the same
			// to the output
		...
		$editModel = new EditModel( $page, $user, $data );
		$info += [
			'autoSumm' => md5( '' ),
		];
		$options += [
		...
		$result = [];
		$status = $editModel->internalAttemptSave( $info, $options, $result );
	...
	public static function provideCreatePages() {
		return [
			...
			[ 'expected not-registered MediaWiki: page not being created if empty',
				'MediaWiki:EditModelTest_testCreatePage',
				'UTSysop',
				'',
				EditAttempt::AS_BLANK_ARTICLE,
				null
	 ...
	 * @covers EditModel::internalAttemptSave
	 */
	...
	public function testUpdatePage() {
		...
		$page = $this->assertInternalAttemptSave(
			'EditModelTest_testUpdatePage', null, $edit, [], [], "zero",
		...
		$this->assertInternalAttemptSave(
			'EditModelTest_testUpdatePage', null, $edit, [], [], null,
			EditAttempt::AS_SUCCESS_UPDATE, $text,
			"expected successfull update with given text" );
	...
	public function testUpdatePageTrx() {
		...
		$page = $this->assertInternalAttemptSave(
			'EditModelTest_testTrxUpdatePage', null, $edit, [], [], "zero",
			EditAttempt::AS_SUCCESS_UPDATE, $text,
			"expected successfull update with given text" );
		...
		$this->assertInternalAttemptSave(
			'EditModelTest_testTrxUpdatePage', null, $edit, [], [], null,
			EditAttempt::AS_SUCCESS_UPDATE, $text,
			"expected successfull update with given text" );
		$this->forceRevisionDate( $page, '20120101000000' );
...
';
	...
	public function testAutoMerge( $baseUser, $text, $adamsEdit, $bertasEdit,
		...
		// create page
		$ns = $this->getDefaultWikitextNS();
		$title = Title::newFromText( 'EditModelTest_testAutoMerge', $ns );
		$page = WikiPage::factory( $title );
		...
		$this->assertInternalAttemptSave( 'EditModelTest_testAutoMerge',
		...
		// first edit
		$this->assertInternalAttemptSave( 'EditModelTest_testAutoMerge', 'Adam', $adamsEditData,
			[], [], null, EditAttempt::AS_SUCCESS_UPDATE, null, "expected successfull update" );

		// second edit
		$this->assertInternalAttemptSave( 'EditModelTest_testAutoMerge', 'Berta', $bertasEditData,
			[], [], null, $expectedCode, $expectedText, $message );
	}

	public function testCheckDirectEditingDisallowed_forNonTextContent() {
		$title = Title::newFromText( 'Dummy:NonTextPageForEditModel' );
		$page = WikiPage::factory( $title );
		$user = new User();

		$editData = new EditFormData();
		$editData->textbox1 = serialize( 'non-text content' );
		$editData->startTime = wfTimestampNow();

		$editModel = new EditModel( $page, $user, $editData );
		...
		$result = [];
		$editModel->internalAttemptSave( [], [], $result );
	}
}
