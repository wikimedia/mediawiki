...
class BlockTest extends MediaWikiLangTestCase {
	...
	function testBlocksOnXFF() {
		...
		foreach ( $xffHeaders as $test ) {
			...
			$this->assertEquals( $test['count'], count( $xffblocks ), 'Correct block type for XFF header' );
			$block = Block::chooseBlock( $xffblocks, $list );
			$this->assertEquals( $test['result'], $block->mReason, 'Correct block type for XFF header' );
		}
