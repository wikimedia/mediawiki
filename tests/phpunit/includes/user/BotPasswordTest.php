...
class BotPasswordTest extends MediaWikiTestCase {
	protected function setUp() {
		...
		$mock1->expects( $this->any() )->method( 'lookupUserNames' )
			->will( $this->returnValue( array( 'UTSysop' => 42, 'UTDummy' => 43 ) ) );
	...
	public function testLogin() {
		...
		$this->assertEquals( Status::newFatal( 'nosuchuser', 'UTDummy@BotPassword' ), $status );

		// No bot password
		$status = BotPassword::login( 'UTSysop@DoesNotExist', 'foobaz', new FauxRequest );
		$this->assertEquals( Status::newFatal( 'nosuchuser', 'UTSysop@DoesNotExist' ), $status );
