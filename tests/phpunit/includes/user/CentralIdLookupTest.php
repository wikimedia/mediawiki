...
class CentralIdLookupTest extends MediaWikiTestCase {
	...
	public function testCheckAudience() {
		...
		$user = $mock->checkAudience( CentralIdLookup::FOR_PUBLIC );
		$this->assertInstanceOf( 'User', $user );
		$this->assertSame( 0, $user->getId() );

		$this->assertNull( $mock->checkAudience( CentralIdLookup::RAW ) );
	...
	public function testNameFromCentralId() {
		...
		$mock->expects( $this->once() )->method( 'lookupCentralIds' )
			->with(
				...
				$this->equalTo( CentralIdLookup::RAW ),
		...
		$this->assertSame(
			'FooBar',
			$mock->nameFromCentralId( 15, CentralIdLookup::RAW, CentralIdLookup::READ_LATEST )
		);
	}
	...
	public function testLocalUserFromCentralId( $name, $succeeds ) {
		...
		$user = $mock->localUserFromCentralId( 42, CentralIdLookup::RAW, CentralIdLookup::READ_LATEST );
		if ( $succeeds ) {
			$this->assertInstanceOf( 'User', $user );
		...
		$mock->expects( $this->once() )->method( 'lookupCentralIds' )
			...
			->will( $this->returnValue( array( 42 => $name ) ) );
		$this->assertNull(
			$mock->localUserFromCentralId( 42, CentralIdLookup::RAW, CentralIdLookup::READ_LATEST )
		);
	}
	...
	public function testCentralIdFromName() {
		...
		$this->assertSame(
			23,
			$mock->centralIdFromName( 'FooBar', CentralIdLookup::RAW, CentralIdLookup::READ_LATEST )
		);
	}
	...
	public function testCentralIdFromLocalUser() {
		...
		$this->assertSame(
			...
			$mock->centralIdFromLocalUser(
				User::newFromName( 'FooBar' ), CentralIdLookup::RAW, CentralIdLookup::READ_LATEST
			)
		);
