...
class ReadOnlyModeTest extends MediaWikiTestCase {
	...
	private function createMode( $params, $makeLB ) {
		...
		$rom = new ReadOnlyMode( $config );

		if ( $makeLB ) {
			$lb = $this->createLB( $params );
			$rom->addLoadBalancer( $lb );
		}
		return $rom;
	}
	...
	private function createServices( $params, $makeLB ) {
		$services = MediaWikiServices::getInstance();
		$oldConfig = $services->getMainConfig();
		$myConfig = new MultiConfig( [
			new HashConfig( [
				'ReadOnly' => $params['confMessage'],
				'ReadOnlyFile' => $this->createFile( $params ),
			] ),
		...
		$bootstrap = new HashConfig( [
			'ConfigRegistry' => [
				'main' => function () use ( $myConfig ) {
					return $myConfig;
				},
			]
		] );
		$newServices = new MediaWikiServices( $bootstrap );
		$newServices->loadWiringFiles( $oldConfig->get( 'ServiceWiringFiles' ) );
		if ( $makeLB ) {
			$newServices->redefineService( 'DBLoadBalancer', function () use ( $params ) {
				return $this->createLB( $params );
			} );
		}
		return $newServices;
	}
	 ...
	 * @dataProvider provider
	 */
	public function testWiringWithLB( $params ) {
		$rom = $this->createServices( $params, true )->getReadOnlyMode();
		$this->assertSame( $params['expectedMessage'], $rom->getReason() );
		$this->assertSame( $params['expectedState'], $rom->isReadOnly() );
	}
	 ...
	 * @dataProvider provider
	 */
	public function testWiringWithoutLB( $params ) {
		$rom = $this->createServices( $params, false )->getConfiguredReadOnlyMode();
		$this->assertSame( $params['expectedConfMessage'], $rom->getReason() );
	}
