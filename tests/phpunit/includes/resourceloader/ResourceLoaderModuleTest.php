...
class ResourceLoaderModuleTest extends ResourceLoaderTestCase {
	...
	public function testPlaceholderize() {
		$this->setMwGlobals(array(
			...
			'wgExtensionDirectory' => '/srv/example/mediawiki/extensions',
			'wgStyleDirectory' => '/srv/example/mediawiki/skins'
		));
		$raw = array(
				'/srv/example/mediawiki/core/resources/foo.js',
				'/srv/example/mediawiki/extensions/Example/modules/bar.js',
		...
		$this->assertEquals(
			ResourceLoaderTestModule::placeholderize( $raw ),
			$canonical,
			'Insert placeholders'
		);
		$this->assertEquals(
			ResourceLoaderTestModule::unplaceholderize( $canonical ),
			$raw,
