...
class HTMLMultiSelectFieldTest extends MediaWikiTestCase {
	...
	function providerForUsecheckboxes() {
		...
		return array(
			...
			array( 5, true,  $CHECKS,
				"'usecheckboxes'=true force checkboxes" ),
			array( 5, null,  $CHECKS,
				"'usecheckboxes'=null force checkboxes" ),
			array( 5, false, $NO_CHECKS,
				"'usecheckboxes'=false disable checkboxes" ),

			# 'usecheckboxes' set to a numeric value
			array( 5, 5,     $CHECKS,
				"# of options equals 'usecheckboxes' value forces checkboxes",
			),

			array( 5, 4,     $NO_CHECKS,
				"# of options less than 'usecheckboxes' value has no checkboxes",
			),
			array( 5, 3,     $NO_CHECKS,
				"# of options less than 'usecheckboxes' value has no checkboxes",
			),
			...
			array( 1, 3,     $CHECKS,
				"# of options more than 'usecheckboxes' value has checkboxes",
			),
		);
	}
	...
	public function testUsecheckboxesOption( $nopts, $pval, $expected, $message = '' ) {
		# Forge an array of $nopts number of options. Resulting array
		# just contains letters up to $nopts (ie: A, B, C...)
		$options = array();
		for($i=0;$i<$nopts;$i++) {
			$options[] = chr( 65 + $i);
		}
