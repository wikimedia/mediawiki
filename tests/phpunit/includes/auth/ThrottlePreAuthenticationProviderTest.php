...
class ThrottlePreAuthenticationProviderTest extends \MediaWikiTestCase {
	...
	public function testTestForAccountCreation( $creatorname, $succeed, $hook ) {
		...
		$debug = $status->isGood() ? '' : ' ' . \Status::wrap( $status )->getMessage()->getKey() . ' ' . \Status::wrap( $status )->getMessage();
		$this->assertEquals(
	...
	public function testTestForAuthentication() {
		...
		for ( $i = 1; $i <= 3; $i++ ) {
			$status = $provider->testForAuthentication( [] );
		...
		$this->assertFalse( $provider->testForAuthentication( [] )->isGood(), 'after FAIL' );

		$provider->postAuthentication( \User::newFromName( 'SomeUser' ),
			AuthenticationResponse::newPass() );
		$this->assertTrue( $provider->testForAuthentication( [] )->isGood(), 'after PASS' );
