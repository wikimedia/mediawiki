...
class CheckBlocksSecondaryAuthenticationProviderTest extends \MediaWikiTestCase {
	...
	public function testBasics() {
		...
		$this->assertEquals(
			...
			$provider->beginSecondaryAccountCreation( \User::newFromName( 'UTSysop' ), [] )
		);
	}
	...
	public function testTestForAccountCreation() {
		...
		$this->assertEquals(
			\StatusValue::newGood(),
			$provider->testForAccountCreation( $user, $unblockedUser, [] )
		);

		$status = $provider->testForAccountCreation( $user, $blockedUser, [] );
	...
	public function testTestUserForCreation() {
		...
		$this->assertEquals(
			\StatusValue::newGood(),
			$provider->testUserForCreation( $unblockedUser, true )
		);
		$this->assertEquals(
			\StatusValue::newGood(),
		...
		$status = $provider->testUserForCreation( $blockedUser, true );
		$this->assertInstanceOf( 'StatusValue', $status );
		$this->assertFalse( $status->isOK() );
	...
	public function testRangeBlock() {
		...
		$status = $provider->testForAccountCreation( $newuser, $user, [] );
		$this->assertInstanceOf( 'StatusValue', $status );
		$this->assertFalse( $status->isOK() );

		$status = $provider->testUserForCreation( $newuser, true );
		$this->assertInstanceOf( 'StatusValue', $status );
		$this->assertFalse( $status->isOK() );
