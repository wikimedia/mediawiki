...
class AuthPluginPrimaryAuthenticationProviderTest extends MediaWikiTestCase {
	...
	public function testAuthentication() {
		...
		$this->assertEquals(
			...
			$provider->beginAuthentication( array() )
		);
		...
		$this->assertEquals(
			AuthenticationResponse::newAbstain(),
			$provider->beginAuthentication( $reqs )
		);
		...
		$plugin->expects( $this->never() )->method( 'setDomain' );
		$provider = new AuthPluginPrimaryAuthenticationProvider( $plugin );
		$ret = $provider->beginAuthentication( $reqs );
		...
		$req->password = 'bar';
		$req->domain = 'Domain1';
		$provider->beginAuthentication( array( $reqType => $req ) );
	}
	...
	public function testAllowPropertyChange() {
		...
		$this->assertTrue( $provider->allowPropertyChange( 'allow' ) );
		$this->assertFalse( $provider->allowPropertyChange( 'deny' ) );
	}

	/**
	 * @dataProvider provideAllowChangingAuthenticationType
	 * @param string $type
	 * @param bool|null $allow
	 * @param bool $expect
	 */
	public function testAllowChangingAuthenticationType( $type, $allow, $expect ) {
		$plugin = $this->getMock( 'AuthPlugin' );
		$plugin->expects( $allow === null ? $this->never() : $this->once() )
			->method( 'allowPasswordChange' )->willReturn( $allow );
		$provider = new AuthPluginPrimaryAuthenticationProvider( $plugin );

		$this->assertSame( $expect, $provider->allowChangingAuthenticationType( $type ) );
	}

	public static function provideAllowChangingAuthenticationType() {
		return array(
	...
	public function testCanChangeAuthenticationData( $type, $allow, $expect ) {
		...
		$this->assertEquals( $expect, $provider->canChangeAuthenticationData( $req ) );
	}
	...
	public function testChangeAuthenticationData() {
		...
		$provider = new AuthPluginPrimaryAuthenticationProvider( $plugin );
		$provider->changeAuthenticationData( $this->getMock( 'AuthenticationRequest' ) );
	...
	public function testAccountCreation() {
		...
		$provider = new AuthPluginPrimaryAuthenticationProvider( $plugin );
		try {
			$provider->beginAccountCreation( $user, array() );
		...
		$this->assertEquals(
			AuthenticationResponse::newAbstain(),
			$provider->beginAccountCreation( $user, $reqs )
		);
		...
		$plugin->expects( $this->never() )->method( 'setDomain' );
		$provider = new AuthPluginPrimaryAuthenticationProvider( $plugin );
		$ret = $provider->beginAccountCreation( $user, $reqs );
		$this->assertSame( AuthenticationResponse::FAIL, $ret->status );
		...
		$req->password = 'bar';
		$req->domain = 'Domain1';
		$provider->beginAccountCreation( $user, array( $reqType => $req ) );
	}
