...
class AuthPluginPrimaryAuthenticationProviderTest extends \MediaWikiTestCase {
	...
	public function testConstruction() {
		...
		$req = $this->getMock( 'MediaWiki\\Auth\\PasswordAuthenticationRequest' );
	...
	public static function provideGetAuthenticationRequests() {
		...
		return [
			...
			[ AuthManager::ACTION_CREATE, $arr2, true ],
			[ AuthManager::ACTION_CREATE, $arr2, false ],
			[ AuthManager::ACTION_LINK, [], true ],
			[ AuthManager::ACTION_LINK, [], false ],
			[ AuthManager::ACTION_CHANGE, $arr2, true ],
			[ AuthManager::ACTION_CHANGE, [], false ],
			[ AuthManager::ACTION_REMOVE, $arr1, true ],
	...
	public function testAuthentication() {
		...
		$plugin = $this->getMockBuilder( 'AuthPlugin' )
			->setMethods( [ 'authenticate', 'setDomain' ] )
			->getMock();
		...
		$plugin = $this->getMockBuilder( 'AuthPlugin' )
			->setMethods( [ 'userExists', 'authenticate', 'setDomain' ] )
			->getMock();
		...
		$pluginUser->expects( $this->once() )->method( 'isLocked' )
			->will( $this->returnValue( true ) );
		$plugin = $this->getMockBuilder( 'AuthPlugin' )
			->setMethods( [ 'userExists', 'getUserInstance', 'authenticate', 'setDomain' ] )
			->getMock();
		$plugin->expects( $this->once() )->method( 'userExists' )
			->will( $this->returnValue( true ) );
		...
		$plugin = $this->getMockBuilder( 'AuthPlugin' )
			->setMethods( [ 'userExists', 'authenticate', 'strict', 'setDomain' ] )
			->getMock();
		$plugin->expects( $this->once() )->method( 'userExists' )
			->will( $this->returnValue( true ) );
		$plugin->expects( $this->once() )->method( 'authenticate' )
		...
		$plugin = $this->getMockBuilder( 'AuthPlugin' )
			->setMethods( [ 'userExists', 'authenticate', 'strictUserAuth', 'setDomain' ] )
			->getMock();
		$plugin->expects( $this->once() )->method( 'userExists' )
			->will( $this->returnValue( true ) );
		$plugin->expects( $this->once() )->method( 'authenticate' )
	...
	public static function provideProviderAllowsAuthenticationDataChange() {
		return [
			[ 'MediaWiki\\Auth\\AuthenticationRequest', null, \StatusValue::newGood( 'ignored' ) ],
			[ 'MediaWiki\\Auth\\PasswordAuthenticationRequest', true, \StatusValue::newGood() ],
			[
				'MediaWiki\\Auth\\PasswordAuthenticationRequest',
				false,
				\StatusValue::newFatal( 'authmanager-authplugin-setpass-fail' )
			],
		];
	}
	...
	public function testProviderChangeAuthenticationData() {
		...
		$plugin->expects( $this->once() )->method( 'setPassword' )
			->with( $this->callback( function ( $u ) {
				return $u instanceof \User && $u->getName() === 'Foo@Domain3';
			} ), $this->equalTo( 'bar' ) )
			->will( $this->returnValue( true ) );
		$provider = new AuthPluginPrimaryAuthenticationProvider( $plugin );
		list( $req ) = $provider->getAuthenticationRequests( AuthManager::ACTION_CREATE, [] );
	...
	public function testAccountCreation() {
		...
		$req->username = 'foo@Domain2';
		$req->password = 'bar';
		$provider->beginPrimaryAccountCreation( $user, [ $req ] );
