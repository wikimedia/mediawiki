...
class AuthPluginPrimaryAuthenticationProviderTest extends \MediaWikiTestCase {
	...
	public function testConstruction() {
		...
		$provider = new AuthPluginPrimaryAuthenticationProvider( $plugin );
		$this->assertEquals(
			array( new PasswordAuthenticationRequest ),
	...
	public static function provideGetAuthenticationRequests() {
		...
		return array(
			...
			array( AuthManager::ACTION_REMOVE, $arr, true ),
			array( AuthManager::ACTION_REMOVE, array(), false ),
		);
	}
	...
	public function testAuthentication() {
		...
		$plugin->expects( $this->once() )->method( 'authenticate' )
			...
			->will( $this->returnValue( true ) );
		$provider = new AuthPluginPrimaryAuthenticationProvider( $plugin );
		list( $req ) = $provider->getAuthenticationRequests( AuthManager::ACTION_LOGIN, array() );
		$req->username = 'foo@Domain2';
		$req->password = 'bar';
		$provider->beginPrimaryAuthentication( array( $req ) );
	...
	public static function provideProviderAllowsAuthenticationDataChange() {
		return array(
			array( 'MediaWiki\\Auth\\AuthenticationRequest', null, \StatusValue::newGood( 'ignored' ) ),
			array( 'MediaWiki\\Auth\\PasswordAuthenticationRequest', true, \StatusValue::newGood() ),
			array(
	...
	public function testProviderChangeAuthenticationData() {
		...
		$plugin->expects( $this->once() )->method( 'setPassword' )
			...
			->will( $this->returnValue( true ) );
		$provider = new AuthPluginPrimaryAuthenticationProvider( $plugin );
		list( $req ) = $provider->getAuthenticationRequests( AuthManager::ACTION_CREATE, array() );
		$req->username = 'foo@Domain2';
		$req->password = 'bar';
		$provider->providerChangeAuthenticationData( $req );
	...
	public function testAccountCreation() {
		...
		$req->username = 'foo@Domain2';
		$req->password = 'bar';
		$provider->beginPrimaryAccountCreation( $user, array( $req ) );
