...
class TemporaryPasswordPrimaryAuthenticationProviderTest extends \MediaWikiTestCase {
	...
	public static function provideGetAuthenticationRequests() {
		$anon = array( 'username' => null );
		$loggedIn = array( 'username' => 'UTSysop' );

		return array(
			array( AuthManager::ACTION_LOGIN, $anon, array(
				new PasswordAuthenticationRequest
			) ),
			array( AuthManager::ACTION_LOGIN, $loggedIn, array(
				new PasswordAuthenticationRequest
			) ),
			array( AuthManager::ACTION_CREATE, $anon, array() ),
			array( AuthManager::ACTION_CREATE, $loggedIn, array(
				new TemporaryPasswordAuthenticationRequest( 'random' )
			) ),
			array( AuthManager::ACTION_LINK, $anon, array() ),
			array( AuthManager::ACTION_LINK, $loggedIn, array() ),
			array( AuthManager::ACTION_CHANGE, $anon, array(
				new TemporaryPasswordAuthenticationRequest( 'random' )
			) ),
			array( AuthManager::ACTION_CHANGE, $loggedIn, array(
				new TemporaryPasswordAuthenticationRequest( 'random' )
			) ),
			array( AuthManager::ACTION_REMOVE, $anon, array(
				new TemporaryPasswordAuthenticationRequest
			) ),
			array( AuthManager::ACTION_REMOVE, $loggedIn, array(
				new TemporaryPasswordAuthenticationRequest
			) ),
		);
	}
