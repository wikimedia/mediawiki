...
class TemporaryPasswordPrimaryAuthenticationProviderTest extends \MediaWikiTestCase {
	 ...
	 */
	protected function getProvider( $mockedMethods = [] ) {
		if ( !$this->config ) {
	...
	public static function provideProviderAllowsAuthenticationDataChange() {
		...
		return [
			[ 'MediaWiki\\Auth\\AuthenticationRequest', 'UTSysop',
				\Status::newGood( 'ignored' ) ],
			[ 'MediaWiki\\Auth\\PasswordAuthenticationRequest', 'UTSysop',
				\Status::newGood( 'ignored' ) ],
			[ 'MediaWiki\\Auth\\TemporaryPasswordAuthenticationRequest', 'UTSysop',
				\Status::newGood() ],
			[ 'MediaWiki\\Auth\\TemporaryPasswordAuthenticationRequest', 'UTSysop', $err ],
			[ 'MediaWiki\\Auth\\TemporaryPasswordAuthenticationRequest', 'UTSysop',
				\Status::newFatal( 'arbitrary-error' ) ],
			[ 'MediaWiki\\Auth\\TemporaryPasswordAuthenticationRequest', 'DoesNotExist',
				\Status::newGood( 'ignored' ) ],
		];
	}
	...
	public function testProviderChangeAuthenticationData( $type, $changed ) {
		...
		$cb = new \ScopedCallback( function () use ( $dbw, $user, $oldHash ) {
			$dbw->update( 'user', [ 'user_newpassword' => $oldHash ], [ 'user_name' => $user ] );
		} );
	...
	public static function provideProviderChangeAuthenticationData() {
		return [
			[ 'MediaWiki\\Auth\\AuthenticationRequest', false ],
			[ 'MediaWiki\\Auth\\PasswordAuthenticationRequest', false ],
			[ 'MediaWiki\\Auth\\TemporaryPasswordAuthenticationRequest', true ],
		];
	}
	...
	public function testTestForAccountCreation() {
		...
		$req->username = 'Foo';
		$req->password = 'Bar';
		$reqs = [ 'MediaWiki\\Auth\\TemporaryPasswordAuthenticationRequest' => $req ];
	...
	public function testAccountCreation() {
		...
		$authreqs = [ 'MediaWiki\\Auth\\PasswordAuthenticationRequest' => $authreq ];
		...
		$this->assertSame(
			'byemail',
			$provider->finishAccountCreation( $user, $user, $res2 )
		);
