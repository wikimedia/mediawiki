...
class TemporaryPasswordPrimaryAuthenticationProviderTest extends \MediaWikiTestCase {
	...
	public function testGetAuthenticationRequests( $action, $response ) {
		$this->assertEquals(
			$response,
			$this->getProvider()->getAuthenticationRequests( $action, array() )
		);
	}
	...
	public function testProviderAllowsAuthenticationDataChange( $type, $user, $validity ) {
		...
		// Always returns "good" for removes
		$req->action = AuthManager::ACTION_REMOVE;
		$req->password = null;
		$this->assertEquals(
			\StatusValue::newGood(), $provider->providerAllowsAuthenticationDataChange( $req, true )
		);
	}
