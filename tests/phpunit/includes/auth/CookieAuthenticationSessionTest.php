 ...
 * @group AuthManager
 * @group Database
 * @covers CookieAuthenticationSession
 * @uses AuthenticationSession
 */
class CookieAuthenticationSessionTest extends MediaWikiTestCase {
	 ...
	 * @return CookieAuthenticationSessionTest
	 */
	private function getSession( $loginMethod = null, $mockMethods = null ) {
		...
		if ( $loginMethod !== null ) {
			...
			if ( $loginMethod === 'session' || $loginMethod === 'both' ) {
				...
				$store->set( 'CookieAuthenticationSession:foobar:sess', array(
					'UserID' => $id,
					'UserName' => $name,
					'Token' => $token,
				) );
		...
		if ( $mockMethods ) {
			$ret = $this->getMockBuilder( 'CookieAuthenticationSession' )
				->disableOriginalConstructor()
				->setMethods( $mockMethods )
				->getMock();
			$sess = $store->get( 'CookieAuthenticationSession:foobar:sess' );
			if ( $sess ) {
			...
			return new CookieAuthenticationSession( $request, $store, $logger, $params );
		}
	}
	...
	public function testConstructor() {
		...
		// No data
		$session = new CookieAuthenticationSession( $emptyRequest, $emptyStore, $logger, $params );
		...
		// Data from cookies
		$session = new CookieAuthenticationSession( $fullRequest, $emptyStore, $logger, $params );
		$this->assertNull( $session->getSessionKey() );
		...
		$session = new CookieAuthenticationSession( $request, $emptyStore, $logger, $params );
		$this->assertNull( $session->getSessionKey() );
		$this->assertSame( $params['emptyPriority'], $session->getSessionPriority() );
		$this->assertSame( array( 0, null, null ), $session->getSessionUserInfo() );
		$this->assertSame( 'CookieAuthenticationSession<anon>', $session->__toString() );
		...
		$request = new FauxRequest();
		$request->setCookie( 'session', 'foobar', '' );
		$session = new CookieAuthenticationSession( $request, $fullStore, $logger, $params );
		...
		$session = new CookieAuthenticationSession( $fullRequest, $fullStore, $logger, $params );
		$this->assertSame( 'foobar', $session->getSessionKey() );
		$this->assertSame( $params['priority'], $session->getSessionPriority() );
		$this->assertSame( $userInfo, $session->getSessionUserInfo() );
		$this->assertSame( "CookieAuthenticationSession<$id=$name><from=session>", $session->__toString() );
		...
		$session = new CookieAuthenticationSession( $fullRequest, $fullStore, $logger,
	...
	public function testStoreSessionData() {
		...
		$priv = TestingAccessWrapper::newFromObject( $session );
		$priv->store->delete( 'CookieAuthenticationSession:foobar:sess' );
		$this->assertFalse( $priv->store->get( 'CookieAuthenticationSession:foobar:sess' ), 'sanity check' );
		$this->hook( 'UserSetCookies', $this->never() );
		$priv->storeSessionData( $session->getSessionKey() );
		$this->unhook( 'UserSetCookies' );
		$this->assertNotFalse( $priv->store->get( 'CookieAuthenticationSession:foobar:sess' ) );
		$response = $priv->request->response();
