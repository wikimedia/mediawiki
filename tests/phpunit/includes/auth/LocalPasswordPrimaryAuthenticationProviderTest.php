...
class LocalPasswordPrimaryAuthenticationProviderTest extends \MediaWikiTestCase {
	...
	public function testTestUserCanAuthenticate() {
		...
		$oldHash = $dbw->selectField( 'user', 'user_password', [ 'user_name' => $userName ] );
		$cb = new \ScopedCallback( function () use ( $dbw, $oldHash, $userName ) {
			$dbw->update( 'user', [ 'user_password' => $oldHash ], [ 'user_name' => $userName ] );
		} );
	 ...
	 * @param bool $changed
	 */
	public function testProviderChangeAuthenticationData( $usernameTransform, $type, $loginOnly, $changed ) {
		$testUser = $this->getMutableTestUser();
