...
class AuthManagerTest extends \MediaWikiTestCase {
	...
	public function testCreateFromLogin() {
		...
		$ret = $this->manager->beginAccountCreation(
			'UTDummy', $user, array( $createReq ), 'http://localhost/'
		);
		$this->logger->setCollect( false );
		$this->assertSame( AuthenticationResponse::UI, $ret->status );
		$this->assertCount( 3, $ret->neededRequests );
		$this->assertSame( $req1, $ret->neededRequests[0] );
		$this->assertInstanceOf(
			'MediaWiki\\Auth\\UserDataAuthenticationRequest', $ret->neededRequests[1]
		);
		$this->assertInstanceOf(
			'MediaWiki\\Auth\\CreateFromLoginAuthenticationRequest', $ret->neededRequests[2]
		);
		$this->assertSame( null, $ret->neededRequests[2]->createRequest );
		$this->assertSame( array(), $ret->neededRequests[2]->maybeLink );
		...
		$this->assertSame( $req2, $ret->neededRequests[2]->createRequest );
		$this->assertSame( array(), $ret->neededRequests[2]->maybeLink );
		$this->logger->setCollect( true );
	...
	public function testBeginAccountCreation() {
		...
		try {
			$this->manager->beginAccountCreation(
				self::usernameForCreation(), $creator, array(), 'http://localhost/'
			);
			$this->fail( 'Expected exception not thrown' );
		} catch ( \LogicException $ex ) {
		...
		$this->hook( 'LocalUserCreated', $this->never() );
		$ret = $this->manager->beginAccountCreation(
			$creator->getName(), $creator, array(), 'http://localhost/'
		);
		$this->unhook( 'LocalUserCreated' );
		$this->assertSame( AuthenticationResponse::FAIL, $ret->status );
		$this->assertSame( 'userexists', $ret->message->getKey() );
		...
		$this->manager->beginAccountCreation( $username, $creator, array( $req ), 'http://localhost/' );
	}
	...
	public function testAccountCreation(
		...
		foreach ( $managerResponses as $i => $response ) {
			...
			try {
				if ( $first ) {
					$ret = $this->manager->beginAccountCreation(
						$username, $creator, array( $req ), 'http://localhost/'
					);
				} else {
