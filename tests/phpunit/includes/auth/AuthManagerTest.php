...
class AuthManagerTest extends MediaWikiTestCase {
	...
	protected $manager;
	/** @var TestingAccessWrapper $managerPriv */
	...
	protected function setUp() {
		...
		$provider->method( 'provideAuthenticationSession' )
	 ...
	 * @param bool $settable
	 * @param array $methods
	 * @return AuthenticationSession mock
	 */
	...
	/**
	 * @uses NullAuthenticationSession
	 */
	public function testGetSession() {
		...
		$this->assertInstanceOf( 'AuthenticationSession', $session );
		...
		$this->assertInstanceOf( 'NullAuthenticationSession', $session );

		// A provider that returns an invalid session, then an empty one
		$provider = $this->getMockForAbstractClass( 'AbstractAuthenticationSessionProvider' );
		$provider->expects( $this->exactly( 2 ) )
			->method( 'provideAuthenticationSession' )
			->withConsecutive(
				array( $this->anything() ),
	...
	public function testGetVaryHeaders() {
		$builder = $this->getMockBuilder( 'AbstractAuthenticationSessionProvider' )
	 ...
	 * @param bool $primaryCan
	 * @param bool $sessionCan
	 * @param bool $expect
	 */
	public function testUserCanAuthenticate( $primaryCan, $sessionCan, $expect ) {
		$mock = $this->getMockForAbstractClass( 'PrimaryAuthenticationProvider' );
		$mock->method( 'getUniqueId' )->willReturn( 'primary' );
		$mock->method( 'testUserCanAuthenticate' )
			->with( $this->equalTo( 'UTSysop' ) )
			->willReturn( $primaryCan );
		$this->primaryauthMocks = array( $mock );

		$mock = $this->getMockBuilder( 'AbstractAuthenticationSessionProvider' )
			->setMethods( array( 'immutableSessionCouldExistForUser' ) )
			->getMockForAbstractClass();
		$mock->method( 'immutableSessionCouldExistForUser' )
			->with( $this->equalTo( 'UTSysop' ) )
			->willReturn( $sessionCan );
		$this->sessionMocks = array( $mock );
	...
	public function testRevokeAccessForUser() {
		...
		$mock = $this->getMockBuilder( 'AbstractAuthenticationSessionProvider' )
			->setMethods( array( 'preventImmutableSessionsForUser' ) )
			->getMockForAbstractClass();
		$mock->expects( $this->once() )->method( 'preventImmutableSessionsForUser' )
			->with( $this->equalTo( $name ) );
		$this->sessionMocks = array( $mock );
		...
		$this->assertSame( $token, $this->managerPriv->getUserToken( $id, $name ), 'sanity check' );
		$this->manager->revokeAccessForUser( $name );
		$this->assertNotSame( $token, $this->managerPriv->getUserToken( $id, $name ) );
	}
	...
	public function testAllowsAuthenticationDataChangeType() {
		...
		$mock1->method( 'providerAllowsAuthenticationDataChangeType' )->willReturn( true );
		$mock2 = $this->getMockForAbstractClass( 'PrimaryAuthenticationProvider' );
		$mock2->method( 'getUniqueId' )->willReturn( '2' );
		$mock2->method( 'providerAllowsAuthenticationDataChangeType' )->willReturn( false );
			...
			array(
				...
				array(
					'created' => new DomainException(
						'MockSecondaryAuthenticationProvider::beginSecondaryAccountCreation() returned FAIL. Secondary providers are not allowed to fail account creation, that should have been done via testForAccountCreation().'
					)
				)
			),
		);
	...
	public function testGetAuthenticationRequestTypes( $which, $expect, $state = array() ) {
		...
		foreach ( array( 'pre', 'primary', 'secondary' ) as $key ) {
			...
			$mocks[$key]->method( 'providerAllowsAuthenticationDataChangeType' )->willReturn( true );
		}
		...
		$mocks['primary2']->method( 'providerAllowsAuthenticationDataChangeType' )
