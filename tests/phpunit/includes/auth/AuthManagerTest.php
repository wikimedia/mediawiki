...
class AuthManagerTest extends MediaWikiTestCase {
	...
	public function testAuthentication(
		...
		$mocks['primary2']->expects( $this->atMost( 1 ) )->method( 'beginAuthentication' )->willReturn( $abstain );
		$mocks['primary2']->expects( $this->never() )->method( 'continueAuthentication' );
		...
		$this->secondaryauthMocks = array( $mocks['secondary'], $mocks['secondary2'] );
		$this->initializeManager( true );
	...
	public function testAllowChangingAuthenticationType() {
		...
		$mock1->method( 'allowChangingAuthenticationType' )->willReturn( true );
		$mock2 = $this->getMockForAbstractClass( 'PrimaryAuthenticationProvider' );
		$mock2->method( 'getUniqueId' )->willReturn( '2' );
		$mock2->method( 'allowChangingAuthenticationType' )->willReturn( false );
	...
	public function testChangeAuthenticationData() {
		...
		$mock1 = $this->getMockForAbstractClass( 'PrimaryAuthenticationProvider' );
		$mock1->method( 'getUniqueId' )->willReturn( '1' );
		$mock1->expects( $this->once() )->method( 'changeAuthenticationData' )
			->with( $this->equalTo( $req ) );
		$mock2 = $this->getMockForAbstractClass( 'PrimaryAuthenticationProvider' );
		$mock2->method( 'getUniqueId' )->willReturn( '2' );
		$mock2->expects( $this->once() )->method( 'changeAuthenticationData' )
			->with( $this->equalTo( $req ) );
	 ...
	 * @return string
	 */
	private static function usernameForCreation() {
		$i = 0;
		do {
			$username = "UTAuthManagerTestAccountCreation" . ++$i;
	...
	public function testBeginAccountCreation() {
		...
		$mock->method( 'userStatus' )->willReturn( AuthnUserStatus::newExistsStatus() );
		$this->primaryauthMocks = array( $mock );
		$this->initializeManager( true );
		...
		$mock->method( 'getUniqueId' )->willReturn( 'X' );
		$mock->method( 'accountCreationType' )->willReturn( PrimaryAuthenticationProvider::TYPE_CREATE );
		$mock->method( 'userStatus' )->willReturn( AuthnUserStatus::newDefaultStatus() );
		$this->primaryauthMocks = array( $mock );
		$this->initializeManager( true );
	...
	public function testContinueAccountCreation() {
		...
		$mock->method( 'accountCreationType' )->willReturn( PrimaryAuthenticationProvider::TYPE_CREATE );
		$mock->method( 'userStatus' )->willReturn( AuthnUserStatus::newDefaultStatus() );
		$mock->method( 'beginAccountCreation' )->willReturn(
	...
	public function testAccountCreation(
		...
		$creator = User::newFromName( 'UTSysop' );
		$username = $this->usernameForCreation();
		...
		$mocks['primary']->method( 'userStatus' )->willReturn( AuthnUserStatus::newDefaultStatus() );
		$ct = count( $req->primary );
		...
		$mocks['primary2']->method( 'userStatus' )->willReturn( AuthnUserStatus::newDefaultStatus() );
		$mocks['primary2']->expects( $this->atMost( 1 ) )->method( 'beginAccountCreation' )->willReturn( $abstain );
		$mocks['primary2']->expects( $this->never() )->method( 'continueAccountCreation' );
		$mocks['primary3']->method( 'accountCreationType' )->willReturn( PrimaryAuthenticationProvider::TYPE_NONE );
		$mocks['primary3']->method( 'userStatus' )->willReturn( AuthnUserStatus::newDefaultStatus() );
		$mocks['primary3']->expects( $this->never() )->method( 'beginAccountCreation' );
		$mocks['primary3']->expects( $this->never() )->method( 'continueAccountCreation' );
	...
	public function provideAccountCreation() {
		...
		return array(
			...
			array(
				...
				array(
					$tmp,
					new DomainException( 'MockPrimaryAuthenticationProvider::continueAccountCreation() returned ABSTAIN' )
				)
			),
			array(
	...
	public function testAutoAccountCreation() {
		...
		$mocks['primary']->method( 'accountCreationType' )->willReturn( PrimaryAuthenticationProvider::TYPE_CREATE );
		$mocks['primary']->method( 'userStatus' )->willReturn( AuthnUserStatus::newExistsStatus() );
	...
	public function testGetAuthenticationRequestTypes( $which, $expect, $state = array() ) {
		...
		foreach ( array( 'pre', 'primary', 'secondary' ) as $key ) {
			...
			$mocks[$key]->method( 'allowChangingAuthenticationType' )->willReturn( true );
		}
		...
		$mocks['primary2']->method( 'allowChangingAuthenticationType' )
	...
	public function testAllowPropertyChange() {
		...
		foreach ( array( 'primary', 'secondary' ) as $key ) {
			...
			$mocks[$key] = $this->getMockForAbstractClass( $class, array(), "Mock$class" );
			$mocks[$key]->method( 'getUniqueId' )->willReturn( $key );
			$mocks[$key]->method( 'allowPropertyChange' )
	...
	public function testAutoCreateOnLogin() {
		...
		$mock->method( 'beginAuthentication' )->willReturn(
