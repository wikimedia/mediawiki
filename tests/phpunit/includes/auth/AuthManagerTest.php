...
class AuthManagerTest extends \MediaWikiTestCase {
	...
	public function testContinueAccountCreation() {
		...
		$this->request->setSessionData( 'AuthManager::accountCreationState', $session );
		$this->hook( 'LocalUserCreated', $this->never() );
		...
		$this->assertSame(
			$session, $this->request->getSessionData( 'AuthManager::accountCreationState' )
		);
	...
	public function testAutoAccountCreation() {
		...
		$mocks['pre']->expects( $this->exactly( 11 ) )->method( 'testUserForCreation' )
		...
		$mocks['primary']->expects( $this->exactly( 8 ) )->method( 'testUserForCreation' )
			->with( $callback, $this->identicalTo( true ) )
			->will( $this->onConsecutiveCalls(
		...
		$mocks['secondary']->expects( $this->exactly( 7 ) )->method( 'testUserForCreation' )
			->with( $callback, $this->identicalTo( true ) )
			->will( $this->onConsecutiveCalls(
		...
		$user->setName( $username );
		try {
			$this->manager->autoCreateUser( $user, true );
			$this->fail( 'Expected exception not thrown' );
		} catch ( \Exception $ex ) {
