...
class AuthManagerTest extends \MediaWikiTestCase {
	...
	public function testTimeSinceAuthentication() {
		...
		$session->set( 'AuthManager:lastAuthTimestamp', time() - 5 );
		$this->assertEquals( 5, $this->manager->timeSinceAuthentication(),
			'session-stored time for valid session', 1 );
		...
		list( $provider, $reset ) = $this->getMockSessionProvider( false );
		$this->assertSame( -1, $this->manager->timeSinceAuthentication(),
			'-1 for immutable session' );
		\ScopedCallback::consume( $reset );
	}
	...
	public function provideAuthentication() {
		...
		$req = $this->getMockForAbstractClass( 'MediaWiki\\Auth\\AuthenticationRequest' );
		$req->foobar = 'baz';
		$failWithReq = AuthenticationResponse::newFail(
			$this->messageSpecifier( 'authmanager-authn-no-local-user' )
		);
		$failWithReq->createRequest = $req;
		$failWithReq2 = AuthenticationResponse::newFail(
	...
	public static function provideGetAuthenticationRequestTypes() {
		return array(
			array(
				...
				array( 'pre-login', 'primary-login', 'secondary-login', 'generic' ),
			),
			array(
			...
			array(
				...
				array( 'pre-all', 'primary-all', 'secondary-all', 'generic' ),
			),
			array(
