...
class AuthManagerTest extends \MediaWikiTestCase {
	...
	public function testAuthentication(
		...
		foreach ( $mocks as $mock ) {
			$mock->expects( $this->any() )->method( 'getAuthenticationRequests' )->will(
				$this->returnValue( array( $this->getMockRequest( 'X', AuthManager::ACTION_LOGIN ) ) ) );
		}
	...
	public function provideAuthentication() {
		...
		$restartResponse->neededRequests = array( $this->getMockRequest( 'X',
		...
		$restartResponse2->neededRequests = array( $this->getMockRequest( 'X',
			AuthManager::ACTION_LOGIN ) );
		$restartResponse2->linkRequest = $req;
		...
		return array(
			...
			array(
				'Primary UI, then pass with no local user (link type, no link request)',
				StatusValue::newGood(),
				array(
					$tmp = AuthenticationResponse::newUI( array(), $this->messageSpecifier( '...' ) ),
					AuthenticationResponse::newPass( null, $req ),
			...
			array(
				'Primary UI, then pass with no local user (link type with link request)',
				StatusValue::newGood(),
				array(
					$tmp = AuthenticationResponse::newUI( array(), $this->messageSpecifier( '...' ) ),
					AuthenticationResponse::newPass( null, null, $req ),
				),
				array(),
				array(
					$tmp,
	 ...
	 * @param string $action
	 * @param AuthenticationRequest[] $expectedMockRequests getMockRequest keys for mock requests
	 * @param AuthenticationRequest[] $expectedRealRequests
	 * @param array $state
	 */
	public function testGetAuthenticationRequests(
		$action, $expected, $state = array()
	) {
		$expected = array_map( function ( $req ) use ( $action ) {
			return is_string( $req ) ? $this->getMockRequest( $req, $action ) : $req;
		}, $expected );
		...
		foreach ( array(
			...
			$mocks["primary-$type"]->expects( $this->any() )->method( 'getAuthenticationRequests' )
				->will( $this->returnCallback( function ( $action ) use ( $type ) {
					return $this->getMockRequest( array( "primary-$type-$action", 'generic' ), $action );
				} ) );
		...
		$genericType = get_class( $this->getMockRequest( 'generic', AuthManager::ACTION_CHANGE ) );
	...
	protected function getMockRequest( $name, $action ) {
		if ( $name === array() ) {
			return array();
		} elseif ( is_array( $name ) ) {
			return array_map( array( $this, 'getMockRequest' ), $name,
		...
		if ( !isset( self::$requestCache[$key] ) ) {
			...
			self::$requestCache[$key] = $this->getMockForAbstractClass(
		...
		return array(
			...
			array(
				...
				array( 'pre-create', 'primary-none-create', 'primary-create-create',
					'primary-link-create', 'secondary-create', 'generic', $userDataReq ),
			),
			array(
			...
			array(
				AuthManager::ACTION_LOGIN_CONTINUE,
				array( 'primary-none-login', 'primary-create-login', 'primary-link-login',
					   'secondary-login', 'generic' ),
				array(
					'primary' => null,
					'primaryResponse' => null,
					'secondary' => array(),
