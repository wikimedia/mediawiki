...
class AuthManagerTest extends \MediaWikiTestCase {
	...
	protected function initializeConfig() {
		...
		foreach ( [ 'preauth', 'primaryauth', 'secondaryauth' ] as $type ) {
			...
			foreach ( $this->$key as $mock ) {
				$config[$type][] = [ 'factory' => function () use ( $mock ) {
					return $mock;
				} ];
			}
		}
	...
	public function testRevokeAccessForUser() {
		...
		$this->assertSame( [
			[ LogLevel::INFO, 'Revoking access for UTSysop' ],
	...
	public function testBeginAuthentication() {
		...
		$this->assertSame( [
			[ LogLevel::INFO, 'Logging in UTSysop after account creation' ],
		], $this->logger->getBuffer() );
	}
	...
	public function testCreateFromLogin() {
		...
		$this->assertSame( [ $req2->getUniqueId() => $req2 ], $ret->createRequest->maybeLink );
		...
		$this->assertSame( [], $ret->createRequest->maybeLink );
		...
		$this->assertSame( [], $ret->neededRequests[3]->maybeLink );
	...
	public function testAuthentication(
		...
		foreach ( $managerResponses as $i => $response ) {
			...
			if ( $link && $response->status === AuthenticationResponse::RESTART ) {
				...
				$this->assertSame( [], $maybeLink, "Response $i, maybeLink" );
			}
		}
	...
	public function testChangeAuthenticationData() {
		...
		$this->assertSame( [
			[ LogLevel::INFO, 'Changing authentication data for UTSysop' ],
		], $this->logger->getBuffer() );
	}
	...
	public function testAccountCreation(
		...
		foreach ( $managerResponses as $i => $response ) {
			...
			if ( $i === 'created' ) {
				...
				$expectLog[] = [ LogLevel::INFO, "Creating user $username during account creation" ];
			} else {
				$this->hook( 'LocalUserCreated', $this->never() );
			}
	...
	public function testAutoAccountCreation() {
		...
		$this->assertSame( [
			[ LogLevel::DEBUG, 'already exists locally' ],
		], $logger->getBuffer() );
		$logger->clearBuffer();
		...
		$this->assertEquals( 0, $session->getUser()->getId() );
		$this->assertSame( [
			[ LogLevel::DEBUG, 'denied by wfReadOnly(): Because' ],
		], $logger->getBuffer() );
		$logger->clearBuffer();
		...
		$this->assertEquals( 0, $session->getUser()->getId() );
		$this->assertSame( [
			[ LogLevel::DEBUG, 'blacklisted in session' ],
		], $logger->getBuffer() );
		$logger->clearBuffer();
		...
		$this->assertEquals( 0, $session->getUser()->getId() );
		$this->assertSame( [
			[ LogLevel::DEBUG, 'name is not creatable' ],
		], $logger->getBuffer() );
		$logger->clearBuffer();
		...
		$this->assertEquals( 0, $session->getUser()->getId() );
		$this->assertSame( [
			[ LogLevel::DEBUG, 'Provider denied creation: <fail-in-pre>' ],
		], $logger->getBuffer() );
		$logger->clearBuffer();
		$this->assertEquals(
		...
		$this->assertEquals( 0, $session->getUser()->getId() );
		$this->assertSame( [
			[ LogLevel::DEBUG, 'Provider denied creation: <fail-in-primary>' ],
		], $logger->getBuffer() );
		$logger->clearBuffer();
		$this->assertEquals(
		...
		$this->assertEquals( 0, $session->getUser()->getId() );
		$this->assertSame( [
			[ LogLevel::DEBUG, 'Provider denied creation: <fail-in-secondary>' ],
		], $logger->getBuffer() );
		$logger->clearBuffer();
		$this->assertEquals(
		...
		$this->assertEquals( $user->getId(), $session->getUser()->getId() );
		$this->assertSame( [
			[ LogLevel::INFO, "creating new user ($username) - from: XXX" ],
		], $logger->getBuffer() );
		$logger->clearBuffer();
	...
	public function testGetAuthenticationRequests( $action, $expect, $state = [] ) {
		...
		$mocks['primary2'] = $this->getMockForAbstractClass(
			'MediaWiki\\Auth\\PrimaryAuthenticationProvider', [], "MockPrimaryAuthenticationProvider"
		);
