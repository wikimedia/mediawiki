...
class AuthManagerTest extends \MediaWikiTestCase {
	 ...
	 * @return Message
	 */
	protected function message( $key, $params = array() ) {
		if ( $key === null ) {
			return null;
		}
	...
	public function testBeginAuthentication() {
		...
		$this->request->setSessionData( 'AuthManager::authnState', 'test' );
		try {
			$this->manager->beginAuthentication( array(), 'http://localhost/' );
			$this->fail( 'Expected exception not thrown' );
	...
	public function testCreateFromLogin() {
		...
		$this->primaryauthMocks = array( $primary );
		$this->initializeManager( true );
		...
		$createReq = new CreateFromLoginAuthenticationRequest(
			null, array( $req2->getUniqueId() => $req2 )
		);
		$this->logger->setCollect( true );
		$ret = $this->manager->beginAuthentication( array( $createReq ), 'http://localhost/' );
		$this->logger->setCollect( false );
		$this->assertSame( AuthenticationResponse::FAIL, $ret->status );
		$this->assertInstanceOf(
			'MediaWiki\\Auth\\CreateFromLoginAuthenticationRequest', $ret->createRequest
		);
		$this->assertSame( $req1, $ret->createRequest->createRequest );
		$this->assertSame( array( $req2->getUniqueId() => $req2 ), $ret->createRequest->maybeLink );

		// UI, then FAIL in beginAuthentication()
		$primary = $this->getMockBuilder( 'MediaWiki\\Auth\\AbstractPrimaryAuthenticationProvider' )
			->setMethods( array( 'continuePrimaryAuthentication' ) )
			->getMockForAbstractClass();
		$this->primaryauthMocks = array( $primary );
		$this->initializeManager( true );
		$primary->expects( $this->any() )->method( 'beginPrimaryAuthentication' )
			->will( $this->returnValue(
				AuthenticationResponse::newUI( array( $req1 ), wfMessage( 'foo' ) )
			) );
		$res = AuthenticationResponse::newFail( wfMessage( 'foo' ) );
		...
		$primary->expects( $this->any() )->method( 'continuePrimaryAuthentication' )
			->will( $this->returnValue( $res ) );
		$this->logger->setCollect( true );
		$ret = $this->manager->beginAuthentication( array(), 'http://localhost/' );
		...
		$this->assertSame( array(), $ret->createRequest->maybeLink );

		// Pass into beginAccountCreation(), no createRequest, primary needs reqs
		$primary = $this->getMockBuilder( 'MediaWiki\\Auth\\AbstractPrimaryAuthenticationProvider' )
			->setMethods( array( 'testForAccountCreation' ) )
			->getMockForAbstractClass();
		$this->primaryauthMocks = array( $primary );
		$this->initializeManager( true );
		...
		$primary->expects( $this->any() )->method( 'getAuthenticationRequests' )
			->will( $this->returnValue( array( $req1 ) ) );
		...
		$ret = $this->manager->beginAccountCreation(
			$user, array( $userReq, $createReq ), 'http://localhost/'
		);
		$this->logger->setCollect( false );
		...
		$this->assertInstanceOf(
			'MediaWiki\\Auth\\UsernameAuthenticationRequest', $ret->neededRequests[1]
		);
		$this->assertInstanceOf(
			'MediaWiki\\Auth\\UserDataAuthenticationRequest', $ret->neededRequests[2]
		);
		$this->assertInstanceOf(
			'MediaWiki\\Auth\\CreateFromLoginAuthenticationRequest', $ret->neededRequests[3]
		);
		$this->assertSame( null, $ret->neededRequests[3]->createRequest );
		$this->assertSame( array(), $ret->neededRequests[3]->maybeLink );

		// Pass into beginAccountCreation(), with createRequest, primary needs reqs
		$createReq = new CreateFromLoginAuthenticationRequest( $req2, array() );
		$this->logger->setCollect( true );
		$ret = $this->manager->beginAccountCreation(
			$user, array( $userReq, $createReq ), 'http://localhost/'
		);
		...
		$secondary = $this->getMockBuilder( 'MediaWiki\\Auth\\AbstractSecondaryAuthenticationProvider' )
			->getMockForAbstractClass();
		$this->secondaryauthMocks = array( $secondary );
		$this->initializeManager( true );
		$secondary->expects( $this->any() )->method( 'getAuthenticationRequests' )
			->will( $this->returnValue( array( $req3 ) ) );
		$createReq = new CreateFromLoginAuthenticationRequest( $req2, array() );
		$this->logger->setCollect( true );
		$ret = $this->manager->beginAccountCreation(
			$user, array( $userReq, $createReq ), 'http://localhost/'
	...
	public function testAuthentication(
		...
		foreach ( $managerResponses as $i => $response ) {
			...
			try {
				if ( !$i ) {
					$ret = $this->manager->beginAuthentication( array( $req ), 'http://localhost/' );
				} else {
			...
			$maybeLink = isset( $state['maybeLink'] ) ? $state['maybeLink'] : array();
			if ( $link && $response->status === AuthenticationResponse::RESTART ) {
				...
				$this->assertSame( array(), $maybeLink, "Response $i, maybeLink" );
			}
		}
	...
	public function provideAuthentication() {
		...
		$restartResponse->neededRequests = array( $rememberReq );
		...
		$restartResponse2->createRequest = new CreateFromLoginAuthenticationRequest(
			null, array( $req->getUniqueId() => $req )
		);
		$restartResponse2->neededRequests = array( $rememberReq, $restartResponse2->createRequest );

		return array(
			'Failure in pre-auth' => array(
			...
			'Primary UI, then redirect, then fail' => array(
				StatusValue::newGood(),
				$tmp = array(
					AuthenticationResponse::newUI( array( $req ), $this->message( '...' ) ),
					AuthenticationResponse::newRedirect( array( $req ), '/foo.html', array( 'foo' => 'bar' ) ),
					AuthenticationResponse::newFail( $this->message( 'fail-in-primary-continue' ) ),
				),
				array(),
				$tmp
			),
			'Primary redirect, then abstain' => array(
				StatusValue::newGood(),
				array(
					$tmp = AuthenticationResponse::newRedirect(
						array( $req ), '/foo.html', array( 'foo' => 'bar' )
					),
					AuthenticationResponse::newAbstain(),
				),
				array(),
			...
			'Primary UI, then pass with no local user' => array(
				StatusValue::newGood(),
				array(
					$tmp = AuthenticationResponse::newUI( array( $req ), $this->message( '...' ) ),
			...
			'Primary UI, then pass with no local user (link type)' => array(
				StatusValue::newGood(),
				array(
					$tmp = AuthenticationResponse::newUI( array( $req ), $this->message( '...' ) ),
			...
			'Primary pass with invalid username' => array(
				StatusValue::newGood(),
				array(
			...
			'Secondary UI, then abstain' => array(
				StatusValue::newGood(),
				array(
					AuthenticationResponse::newPass( $name ),
				),
	...
	public function testBeginAccountCreation() {
		...
		$this->hook( 'LocalUserCreated', $this->never() );
		$ret = $this->manager->beginAccountCreation( $creator, array(), 'http://localhost/' );
		$this->unhook( 'LocalUserCreated' );
		$this->assertSame( AuthenticationResponse::FAIL, $ret->status );
		...
		$ret = $this->manager->beginAccountCreation(
			$creator, array( $userReq, $userReq2 ), 'http://localhost/'
		);
		$this->unhook( 'LocalUserCreated' );
		$this->assertSame( AuthenticationResponse::FAIL, $ret->status );
		$this->assertSame( 'noname', $ret->message->getKey() );
		...
		$this->hook( 'LocalUserCreated', $this->never() );
		$userReq->username = self::usernameForCreation();
		$ret = $this->manager->beginAccountCreation( $creator, array( $userReq ), 'http://localhost/' );
		$this->unhook( 'LocalUserCreated' );
		$this->assertSame( AuthenticationResponse::FAIL, $ret->status );
		...
		$ret = $this->manager->beginAccountCreation( $creator, array( $userReq, $req ), 'http://localhost/' );
		$this->assertSame( AuthenticationResponse::FAIL, $ret->status );
		$this->assertSame( 'fail', $ret->message->getKey() );

		$this->manager->beginAccountCreation(
			\User::newFromName( $userReq->username ), array( $userReq, $req ), 'http://localhost/'
		);
		$this->assertSame( AuthenticationResponse::FAIL, $ret->status );
		$this->assertSame( 'fail', $ret->message->getKey() );
	}
			...
			$mocks[$key]->expects( $this->any() )->method( 'testForAccountCreation' )
				->will( $this->returnCallback(
						...
						use ( $that, $username, $creator, $req, $key )
					{
						...
						$that->assertSame( $creator->getId(), $creatorIn->getId() );
						$that->assertSame( $creator->getName(), $creatorIn->getName() );
						foreach ( $reqs as $req ) {
		...
		foreach ( $managerResponses as $i => $response ) {
			...
			try {
				if ( $first ) {
					...
					$ret = $this->manager->beginAccountCreation(
						$creator, array( $userReq, $req ), 'http://localhost/'
					);
				} else {
	...
	public function provideAccountCreation() {
		...
		return array(
			'Pre-creation test fail in pre' => array(
			...
			'Pre-creation test fail in primary' => array(
			...
			'Pre-creation test fail in secondary' => array(
			...
			'Primary UI, then pass; secondary abstain' => array(
				$good, $good, $good,
				array(
					$tmp1 = AuthenticationResponse::newUI( array( $req ), $this->message( '...' ) ),
					AuthenticationResponse::newPass(),
				),
				array(
					AuthenticationResponse::newAbstain(),
			...
			'Primary pass; secondary UI then pass' => array(
				$good, $good, $good,
				array(
					AuthenticationResponse::newPass( '' ),
				),
			...
			'Primary pass; secondary fail' => array(
				$good, $good, $good,
				array(
					AuthenticationResponse::newPass(),
				),
	...
	public function testAutoAccountCreation() {
		...
		$mocks['pre']->expects( $this->exactly( 8 ) )->method( 'testUserForCreation' )
			->with( $callback, $this->identicalTo( true ) )
		...
		$mocks['primary']->expects( $this->exactly( 5 ) )->method( 'testUserForCreation' )
			->with( $callback, $this->identicalTo( true ) )
			->will( $this->onConsecutiveCalls(
		...
		$mocks['secondary']->expects( $this->exactly( 4 ) )->method( 'testUserForCreation' )
			->with( $callback, $this->identicalTo( true ) )
			->will( $this->onConsecutiveCalls(
	...
	public function testGetAuthenticationRequests( $action, $expect, $state = array() ) {
		$that = $this;
		$makeReq = function ( $key ) use ( $action, $that ) {
			$req = $that->getMock( 'MediaWiki\\Auth\\AuthenticationRequest' );
			$req->expects( $that->any() )->method( 'getUniqueId' )
				->will( $that->returnValue( $key ) );
		...
		foreach ( array( 'pre', 'primary', 'secondary' ) as $key ) {
			...
			$mocks[$key]->expects( $this->any() )->method( 'getAuthenticationRequests' )
				->will( $this->returnCallback( function ( $action ) use ( $key, $makeReq ) {
					return array( $makeReq( "$key-$action" ), $makeReq( 'generic' ) );
				} ) );
		...
		foreach ( array(
			...
			$mocks["primary-$type"]->expects( $this->any() )->method( 'getAuthenticationRequests' )
				->will( $this->returnCallback( function ( $action ) use ( $type, $makeReq ) {
					return array( $makeReq( "primary-$type-$action" ), $makeReq( 'generic' ) );
				} ) );
	...
	public static function provideGetAuthenticationRequests() {
		return array(
			...
			array(
				...
				array( 'primary-none-remove', 'primary-create-remove', 'primary-link-remove',
					'secondary-remove' ),
			),
			array(
				AuthManager::ACTION_UNLINK,
				array( 'primary-link-remove' ),
			),
			array(
			...
			array(
				AuthManager::ACTION_LOGIN_CONTINUE,
				$reqs = array( 'continue-login', 'foo', 'bar' ),
				array(
			...
			array(
				AuthManager::ACTION_CREATE_CONTINUE,
				$reqs = array( 'continue-create', 'foo', 'bar' ),
				array(
					'continueRequests' => $reqs,
				),
			),
			...
			array(
				AuthManager::ACTION_LINK_CONTINUE,
				$reqs = array( 'continue-link', 'foo', 'bar' ),
				array(
					'continueRequests' => $reqs,
				),
			),
	...
	public function testGetAuthenticationRequestsRequired() {
		$that = $this;
		$makeReq = function ( $key, $required ) use ( $that ) {
			$req = $that->getMock( 'MediaWiki\\Auth\\AuthenticationRequest' );
			$req->expects( $that->any() )->method( 'getUniqueId' )
				->will( $that->returnValue( $key ) );
		...
		$primary1 = $this->getMockForAbstractClass( 'MediaWiki\\Auth\\PrimaryAuthenticationProvider' );
		...
		$primary2 = $this->getMockForAbstractClass( 'MediaWiki\\Auth\\PrimaryAuthenticationProvider' );
		...
		$secondary = $this->getMockForAbstractClass( 'MediaWiki\\Auth\\SecondaryAuthenticationProvider' );
		...
		$this->primaryauthMocks = array( $primary1, $primary2 );
		$this->secondaryauthMocks = array( $secondary );
		$this->initializeManager( true );
		...
		$this->primaryauthMocks = array( $primary1 );
		$this->secondaryauthMocks = array( $secondary );
		$this->initializeManager( true );
	...
	public function testAutoCreateOnLogin() {
		...
		$req = $this->getMock( 'MediaWiki\\Auth\\AuthenticationRequest' );
	...
	public function testBeginAccountLink() {
		...
		try {
			$this->manager->beginAccountLink( $user, array(), 'http://localhost/' );
			$this->fail( 'Expected exception not thrown' );
		} catch ( \LogicException $ex ) {
		...
		$ret = $this->manager->beginAccountLink( new \User, array(), 'http://localhost/' );
		$this->assertSame( AuthenticationResponse::FAIL, $ret->status );
		$this->assertSame( 'noname', $ret->message->getKey() );

		$ret = $this->manager->beginAccountLink(
			\User::newFromName( 'UTDoesNotExist' ), array(), 'http://localhost/'
		);
		$this->assertSame( AuthenticationResponse::FAIL, $ret->status );
	...
	public function testAccountLink(
		...
		foreach ( $managerResponses as $i => $response ) {
			...
			try {
				if ( $first ) {
					$ret = $this->manager->beginAccountLink( $user, array( $req ), 'http://localhost/' );
				} else {
	...
	public function provideAccountLink() {
		...
		return array(
			'Pre-link test fail in pre' => array(
				StatusValue::newFatal( 'fail-from-pre' ),
				array(),
				array(
					AuthenticationResponse::newFail( $this->message( 'fail-from-pre' ) ),
