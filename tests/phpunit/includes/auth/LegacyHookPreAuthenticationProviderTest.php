...
class LegacyHookPreAuthenticationProviderTest extends \MediaWikiTestCase {
	...
	public function testTestUserForCreation( $error, $failMsg ) {
		...
		$this->hook( 'AbortNewAccount', $this->once() )
			->will( $this->returnCallback( function ( $user, &$abortError, &$abortStatus ) use ( $that, $error ) {
				$that->assertInstanceOf( 'User', $user );
				$that->assertSame( 'UTSysop', $user->getName() );
				$abortError = $error;
				return $error === null;
		...
		if ( $failMsg === null ) {
			...
			$this->assertFalse( $status->isOk(), 'should fail (ok)' );
			$errors = $status->getErrors();
			$this->assertEquals( 'createaccount-hook-aborted', $errors[0]['message'], 'should fail (message)' );
		}
		...
		if ( $error !== false ) {
			...
			$this->hook( 'AbortNewAccount', $this->once() )
				->will( $this->returnCallback(
					function ( $user, &$abortError, &$abortStatus ) use ( $that, $error ) {
						$that->assertInstanceOf( 'User', $user );
						$that->assertSame( 'UTSysop', $user->getName() );
