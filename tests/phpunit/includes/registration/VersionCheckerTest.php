...
class VersionCheckerTest extends PHPUnit_Framework_TestCase {
	...
	/**
	 * Check, if a non-parsable version constraint adds an error message.
	 */
	public function testInvaliConstraint() {
		$checker = new VersionChecker();
		$checker
			->setCoreVersion( '1.0.0' )
			->setLoaded( array(
		...
		$this->assertEquals( false, !(bool)$checker->checkArray( array(
				'FakeExtension' => array(
					'Extension:FakeDependency' => '1.24.3',
				)
			) )
		);
	}
