...
class AutopromoteTest extends MediaWikiLangTestCase {
	...
	public function testEditCountLookupIsSkippedIfRequirementIsZero( $editCount, $requirement ) {
		...
		if ( $editCount >= $requirement ) {
			$this->assertContains( 'autoconfirmed', $result, 'User must be promoted if they meet edit count requirement' );
		} else {
			$this->assertNotContains( 'autoconfirmed', $result, 'User must not be promoted if they fail edit count requirement' );
		}
	}
