...
var_dump($GLOBALS['wgDBtype']);exit;
		$primaryKey = $GLOBALS['wgDBtype'] === 'postgres' ? 'PRIMARY KEY AUTOINCREMENT' : 'auto_increment PRIMARY KEY';

		$dbw->safeQuery(
			'CREATE TABLE IF NOT EXISTS ' . $dbw->tableName( 'orm_test' ) . '(
				test_id                    INT unsigned        NOT NULL ' . $primaryKey . ',
