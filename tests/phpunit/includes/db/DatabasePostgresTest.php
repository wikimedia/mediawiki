...
class DatabasePostgresTest extends MediaWikiTestCase {
	...
	private function doTestInsertIgnore() {
		...
		$oldIgnore = $w->ignoreErrors( false );
		$reset = new ScopedCallback( function () use ( $w, $oldIgnore ) {
		...
		// INSERT IGNORE while ignoring errors
		$this->db->begin( __METHOD__ );
		$w->ignoreErrors( true );
		$res = $this->db->insert(
			'foo', [ [ 'i' => 3 ], [ 'i' => 2 ], [ 'i' => 5 ] ], __METHOD__, [ 'IGNORE' ]
		...
		// And the same for the NOT NULL violation
		$this->db->begin( __METHOD__ );
		$w->ignoreErrors( true );
		$res = $this->db->insert(
			'foo', [ [ 'i' => 7 ], [ 'i' => null ] ], __METHOD__, [ 'IGNORE' ]
