 ...
 * @group Database
 *
 * @covers TitleSecurity::getUserPermissionsErrors
 * @covers TitleSecurity::getUserPermissionsErrorsInternal
 * @covers TitleSecurity::quickUserCan
 * @covers TitleSecurity::userCan
 */
class TitleSecurityTest extends MediaWikiLangTestCase {
	...
	protected function setTitle( $ns, $title = "Main_Page" ) {
		...
		$this->title = $title;
		$this->titleSecurity = TestingAccessWrapper::newFromObject( TitleSecurity::instance() );
	}
	...
	public function testQuickPermissions() {
		...
		$res = $this->titleSecurity->getUserPermissionsErrors( $this->title, 'create', $this->user );
		...
		$this->setUserPerm( "" );
		$res = $this->titleSecurity->getUserPermissionsErrors( $this->title, 'move', $this->user );
		...
		if ( $this->isWikitextNS( NS_MAIN ) ) {
			...
			$this->setUser( 'anon' );
			$this->setUserPerm( 'move' );
			$res = $this->titleSecurity->getUserPermissionsErrors( $this->title, 'move-target',
				$this->user );
			$this->assertEquals( array(), $res );
		...
		$res = $this->titleSecurity->getUserPermissionsErrors( $this->title, 'move-target', $this->user );
		$this->assertEquals( array(), $res );
		...
		foreach ( array( "edit", "protect", "" ) as $action ) {
			...
			$this->assertEquals( $check[$action][0],
				$this->titleSecurity->getUserPermissionsErrors( $this->title, $action, $this->user, true ) );
			$this->assertEquals( $check[$action][0],
				$this->titleSecurity->getUserPermissionsErrors( $this->title, $action, $this->user, 'full' ) );
			$this->assertEquals( $check[$action][0],
				$this->titleSecurity->getUserPermissionsErrors( $this->title, $action, $this->user,
					'secure' ) );
			...
			$this->assertEquals( $check[$action][3],
				$this->titleSecurity->userCan( $this->title, $action, $this->user, true ) );
			$this->assertEquals( $check[$action][3],
				$this->titleSecurity->quickUserCan( $this->title, $action, $this->user ) );
	...
	protected function runGroupPermissions( $action, $result, $result2 = null ) {
		...
		$res = $this->titleSecurity->getUserPermissionsErrors( $this->title, $action, $this->user );
	...
	public function testSpecialsAndNSPermissions() {
		...
		$this->assertEquals( array( array( 'badaccess-group0' ), array( 'ns-specialprotected' ) ),
			$this->titleSecurity->getUserPermissionsErrors( $this->title, 'bogus', $this->user ) );
		...
		$this->assertEquals( true,
			$this->titleSecurity->userCan( $this->title, 'bogus', $this->user ) );
	...
	protected function runCSSandJSPermissions( $result0, $result1, $result2, $result3, $result4 ) {
		...
		$this->assertEquals( $result0,
			$this->titleSecurity->getUserPermissionsErrors( $this->title, 'bogus',
				$this->user ) );
	...
	public function testPageRestrictions() {
		...
		$this->titleSecurity->mRestrictions = array(
			'bogus' => array( 'bogus', 'sysop', 'protect', '' )
		);

		$this->assertEquals( array(),
			$this->titleSecurity->getUserPermissionsErrors( $this->title, 'edit',
				$this->user ) );

		$this->assertEquals( true,
			$this->titleSecurity->quickUserCan( $this->title, 'edit', $this->user ) );
		$this->titleSecurity->mRestrictions = array(
			'edit' => array( 'bogus', 'sysop', 'protect', '' ),
			'bogus' => array( 'bogus', 'sysop', 'protect', '' )
		);
		...
		$this->setUserPerm( "edit" );
		$this->assertEquals( false,
			$this->titleSecurity->quickUserCan( $this->title, 'bogus', $this->user ) );
		$this->assertEquals( false,
			$this->titleSecurity->quickUserCan( $this->title, 'edit', $this->user ) );
		$this->assertEquals( array( array( 'badaccess-group0' ),
				array( 'protectedpagetext', 'bogus', 'bogus' ),
	...
	public function testCascadingSourcesRestrictions() {
		...
		$this->assertEquals( true,
			$this->titleSecurity->userCan( $this->title, 'edit', $this->user ) );
		$this->assertEquals( array(),
			$this->titleSecurity->getUserPermissionsErrors( $this->title, 'edit', $this->user ) );
	}
	...
	public function testActionPermissions() {
		...
		// because we're relying on the TestingAccessWrapper's magic __get method,
		// which does not return a reference to the underlying object's array member.
		...
		$this->assertEquals( array( array( 'titleprotected', 'Useruser', 'test' ) ),
			$this->titleSecurity->getUserPermissionsErrors( $this->title, 'create', $this->user ) );
		$this->assertEquals( false,
			$this->titleSecurity->userCan( $this->title, 'create', $this->user ) );
		...
		$this->assertEquals( false,
			$this->titleSecurity->userCan( $this->title, 'move', $this->user ) );
		$this->assertEquals( array( array( 'immobile-source-namespace', 'Media' ) ),
			$this->titleSecurity->getUserPermissionsErrors( $this->title, 'move', $this->user ) );
		...
		$this->setTitle( NS_MEDIA, "test page" );
		$this->assertEquals( false,
			$this->titleSecurity->userCan( $this->title, 'move-target', $this->user ) );
		$this->assertEquals( array( array( 'immobile-target-namespace', 'Media' ) ),
			$this->titleSecurity->getUserPermissionsErrors( $this->title, 'move-target', $this->user ) );
	...
	public function testUserBlock() {
		...
		$this->assertEquals( true, $this->titleSecurity->userCan( $this->title, 'move-target',
			$this->user ) );

		# $wgEmailConfirmToEdit && !$user->isEmailConfirmed() && $action != 'createaccount'
		$this->assertEquals( array(),
			$this->titleSecurity->getUserPermissionsErrors( $this->title, 'move-target',
				$this->user ) );
		...
		$this->assertEquals( false, $this->titleSecurity->userCan( $this->title, 'move-target',
			$this->user ) );
		// quickUserCan should ignore user blocks
		$this->assertEquals( true, $this->titleSecurity->quickUserCan( $this->title, 'move-target',
			$this->user ) );
