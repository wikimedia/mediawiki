...
class UserTest extends MediaWikiTestCase {
	...
	public static function setCookieDataProvider() {
		...
		// If exp is zero and $wgExtendedLoginCookieExpiration is
		// non-zero, then the expiry passed to setcookie is time() +
		...
		// regardless of the value of $wgExtendedLoginCookieExpiration.
		$data[] = array(
			123456789,
			1234,
			123456789,
		...
		// If exp and $wgExtendedLoginCookieExpiration are zero, then
		// the expiry passed to setcookie is zero.
		$data[] = array(
			0,
			0,
			0,
	 ...
	 */
	public function testSetCookie($expiry, $actualWgExtendedLoginCookieExpiration, $expectedExpiry) {
		...
		$backupWgExtendedLoginCookieExpiration = $wgExtendedLoginCookieExpiration;
		$wgExtendedLoginCookieExpiration = $actualWgExtendedLoginCookieExpiration;

		$user->setExtendedLoginCookie( 'name', 'value', $expiry );

		$wgExtendedLoginCookieExpiration = $backupWgExtendedLoginCookieExpiration;
...
class UserProxy extends User {
	...
	public function setExtendedLoginCookie( $name, $value, $exp = 0, $secure = null, $params = array() ) {
		$this->user->setExtendedLoginCookie( $name, $value, $exp, $secure, $params );
	}
}
