...
class CommentStoreTest extends MediaWikiLangTestCase {
	...
	/**
	 * @dataProvider provideNoDB
	 * @param string $method
	 * @param array $args
	 * @param bool $exception
	 */
	public function testNoDB( $method, $args, $exception ) {
		if ( $exception ) {
			$this->setExpectedException(
				BadMethodCallException::class,
				"Cannot call CommentStore::$method when no database is set"
			);
		}

		$store = CommentStore::newNull();
		call_user_func_array( [ $store, $method ], $args );
		$this->assertTrue( true );
	}

	public static function provideNoDB() {
		return [
			[ 'getFields', [ 'ipb_reason' ], false ],
			[ 'getJoin', [ 'ipb_reason' ], false ],
			[ 'getComment', [ 'ipb_reason', [] ], true ],
			[ 'insert', [ 'ipb_reason', 'foo' ], true ],
			[ 'insertWithTempTable', [ 'ipb_reason', 'foo' ], true ],
		];
	}

	public function testConstructors() {
		$this->assertInstanceOf( CommentStore::class, CommentStore::newNull() );
		$this->assertInstanceOf( CommentStore::class, CommentStore::newReplica() );
	}
