...
class LinkerTest extends MediaWikiLangTestCase {
	...
	public function testFormatComment( $expected, $comment, $title = false, $local = false, $wikiId = null ) {
		...
		if ( $title && !$title->exists() ) {
			$testPage = WikiPage::factory( $title );
			$testPage->doEditContent( $testPage->getContentHandler()->makeEmptyContent(), 'testing', EDIT_NEW );
		}
	...
	public function provideCasesForFormatComment() {
		...
		$testTitle = Title::makeTitle( $this->getDefaultWikitextNS(), 'FormatCommentTest' );
		...
		return array(
			...
			array(
				'<a href="/wiki/' . wfUrlencode( $testTitleText ) . '#autocomment" title="' . $testTitleText . '">→</a>‎<span dir="auto"><span class="autocomment">autocomment</span></span>',
				"/* autocomment */",
				$testTitle, false
			),
			array(
				'<a class="external" rel="nofollow" href="//en.example.org/w/' . wfUrlencode( $testTitleText ) . '#autocomment">→</a>‎<span dir="auto"><span class="autocomment">autocomment</span></span>',
				"/* autocomment */",
