...
/**
 * @covers MediaWiki\ExternalStore\DiffMultiContentBlob
 */
class DiffMultiContentBlobTest extends \MediaWikiTestCase {
	...
	public function testXdiffAdler32( $input, $expect ) {
		$dhb = TestingAccessWrapper::newFromObject( new DiffMultiContentBlob );
	...
	public function testEncodeDecode( $diffs, $diffMap, $expect, $expectSize, $expectCount ) {
		...
		try {
			...
			$this->assertSame(
				DiffMultiContentBlob::class . '::addItem: Cannot add more items after sleep/wakeup',
				$ex->getMessage()
			);
		}
	}
	...
	public function testDecodeBadMetadata() {
		...
		$blob1 = "\x7FMCB\xFF\xFE\x04diff\x00\x0B{\"map\":\"0\"}\x01\x0D\x00\x00\x00\x00\x00\x00\x00\x00\x01\x03foo37\xE5\xAC";
		$blob2 = "\x7FMCB\xFF\xFE\x04diff\x00\x0B{\"Map\":\"0\"}\x01\x0D\x00\x00\x00\x00\x00\x00\x00\x00\x01\x03foo\xCD\xE1\x1B\xD6";
		// @codingStandardsIgnoreEnd

		$obj = MultiContentBlob::decode( $blob1 );
		$this->assertInstanceOf( DiffMultiContentBlob::class, $obj, 'sanity check' );
	...
	public function testNewFromDiffHistoryBlob(
		...
		$newObj = DiffMultiContentBlob::newFromDiffHistoryBlob( $obj );
