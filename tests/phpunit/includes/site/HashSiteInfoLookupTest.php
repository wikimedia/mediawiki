...
class HashSiteInfoLookupTest extends PHPUnit_Framework_TestCase {
	...
	public function testResolveLocalId() {
		...
		$this->assertSame( 'acme', $lookup->resolveLocalId( SiteInfoLookup::INTERWIKI_ID, 'acme' ) );
		$this->assertSame( 'enwiki', $lookup->resolveLocalId( SiteInfoLookup::INTERWIKI_ID, 'en' ) );
		$this->assertSame( 'enwiki', $lookup->resolveLocalId( SiteInfoLookup::NAVIGATION_ID, 'en' ) );
		$this->assertSame( 'enwiki', $lookup->resolveLocalId( SiteInfoLookup::ALIAS_ID, 'wiki' ) );

		$this->assertSame(
			'commonswiki',
			$lookup->resolveLocalId( SiteInfoLookup::INTERWIKI_ID, 'commons' )
		);

		$this->assertNull(
			$lookup->resolveLocalId( SiteInfoLookup::INTERWIKI_ID, 'wiki' ),
			'id unknown in interwiki scope'
		);

		$this->assertNull(
			$lookup->resolveLocalId( SiteInfoLookup::INTERWIKI_ID, 'enwiki' ),
			'site id is not a local id'
		);

		$this->assertNull(
			$lookup->resolveLocalId( 'Kittens', 'en' ),
			'unknown scope'
		);
	}
	...
	public function testGetAssociatedLocalIds() {
		...
		$enwikiIds = [
			...
			SiteInfoLookup::INTERWIKI_ID => [ 'en', 'e' ],
