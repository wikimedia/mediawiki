	...
	public function testTimestampedUID( $method, $digitlen, $bits, $tbits, $hostbits ) {
		...
		foreach ( $ids as $id ) {
			...
			$this->assertGreaterThanOrEqual(
				$last_timestamp_bin,
				$timestamp_bin,
				"timestamp ($timestamp_bin) of current id ($id_bin) >= timestamp ($last_timestamp_bin), of prior one ($lastId_bin)" );
			...
			if ( $hostbits ) {
				$this->assertEquals(
					$hostbits_bin,
					$last_hostbits_bin,
					"Host ID ($hostbits_bin) of current id ($id_bin) is same as host ID ($last_hostbits_bin) of prior one ($lastId_bin)." );
			}
