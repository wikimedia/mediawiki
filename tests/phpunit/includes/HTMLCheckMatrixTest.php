...
class HtmlCheckMatrixTest extends MediaWikiTestCase {
	...
	public function testValidateRequiresOnlyKnownTags() {
		$field = new HTMLCheckMatrix( self::$defaultOptions );
	...
	/**
	 * This form object actually has no visibility into what happens later on, but basically
	 * if the data submitted by the user passes validate the following is run:
	 * foreach ($data as $k => $v) { $user->setOption($k, $v); }
	 */
