...
class WatchedItemStoreUnitTest extends PHPUnit_Framework_TestCase {
	...
	public function provideGetWatchedItemsForUser_fromUntilNoSort() {
		return [
			[ [ 'from' => new TitleValue( 0, 'SomeDbKey' ), ] ],
			[ [ 'until' => new TitleValue( 0, 'SomeDbKey' ), ] ],
		];
	...
	/**
	 * @dataProvider provideGetWatchedItemsForUser_fromUntilNoSort
	 */
	public function testGetWatchedItemsForUser_noSortThrowsException( array $options ) {
		$store = new WatchedItemStore(
			$this->getMockLoadBalancer( $this->getMockDb() ),
			$this->getMockCache()
		);
