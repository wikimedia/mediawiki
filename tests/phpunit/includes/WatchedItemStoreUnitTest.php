...
class WatchedItemStoreUnitTest extends PHPUnit_Framework_TestCase {
	 ...
	 */
	private function getMockCacheItem( $isHit ) {
	...
	/**
	 * @param string[] $methodsToIgnore Method names that we want to expect
	 *
	 * @return PHPUnit_Framework_MockObject_MockObject|CacheItemPoolInterface
	 */
	private function getMockCacheWithNoExpectedCalls( array $methodsToIgnore = [] ) {
		$methods =
			[
				'getItem',
				'getItems',
		...
		foreach ( array_diff( $methods, $methodsToIgnore ) as $method ) {
	...
	public function testGetWatchedItem_cachedItem() {
		...
		$mockCacheItem = $this->getMockCacheItem( true );
		$mockCacheItem->expects( $this->once() )
			->method( 'get' )
	...
	public function testResetNotificationTimestamp_oldidSpecifiedNotLatestRevisionForced() {
		...
		$mockCache = $this->getMockCacheWithNoExpectedCalls( [ 'getItem', 'save', 'deleteItem' ] );
		$mockCache->expects( $this->once() )
			->method( 'getItem' )
			->with( '0|SomeDbKey|1' )
			->will( $this->returnValue( $mockCacheItem ) );
