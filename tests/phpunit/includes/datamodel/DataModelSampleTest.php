...
class DataModelSampleTest extends MediaWikiTestCase {
	...
	public function testInsert() {
		...
		$this->assertEquals( get_object_vars( $this->sample ), get_object_vars( $row->getObject() ) );
	}
	...
	public function testGetList() {
		for ( $i = 0; $i < 10; $i++ ) {
			$this->sample->title = 'Title #'. ( $i + 1 );
			$this->sample->visible = $i % 2;
			$this->sample->insert();
		}

		$list = DataModelSample::getList( 'hidden', null, 0, 'DESC' );
		$this->assertEquals( count( $list ), 5 );
		$this->assertEquals( $list[0]->title, 'Title #8' );
	}
}
