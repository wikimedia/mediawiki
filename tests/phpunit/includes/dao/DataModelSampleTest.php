...
class DataModelSampleTest extends MediaWikiTestCase {
	...
	public function tearDown() {
		...
		// clear db, lists
		$lists = array_keys( DataModelSample::$lists );
		foreach ( $lists as &$list ) {
			$list = wfMemcKey( 'DataModelSample', $list );
		}
		$store = $storeGroup->getForTable( 'datamodel_lists' );
		$partitions = $store->getMultiplePartitions( 'datamodel_lists', 'shard', $lists );
		foreach( $partitions as $partition ) {
			$partition['partition']->delete(
				array( 'list' => $partition['values'] ),
				__METHOD__
			);
		}
	}
