...
class ParserEditTests extends Maintenance {
	...
	public function execute() {
		...
		$this->runner = new ParserTestRunner( $this->recorder );
		$this->setupFileData();
	...
	protected function dwdiff( $diff ) {
	...
	protected function alternatingAligned( $expectedStr, $actualStr ) {
		...
		for ( $i = 0; $i < $maxLines; $i++ ) {
			...
			$maxChunks = min( count( $expectedChunks ), count( $actualChunks ) );
