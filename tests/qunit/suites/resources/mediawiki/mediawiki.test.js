...
( function ( mw, $ ) {
	...
	QUnit.test( 'mw.Map', 32, function ( assert ) {
		...
		assert.equal( anotherGlobalMapChecker, 'Hello', 'get value from global Map via global window object' );

		// Change value via global window object
		anotherGlobalMapChecker = 'World';
		assert.equal( globalConf.get( 'anotherGlobalMapChecker' ), 'World', 'get changed value from global Map via get()' );
		assert.equal( anotherGlobalMapChecker, 'World', 'get changed value from global Map via global window object' );
