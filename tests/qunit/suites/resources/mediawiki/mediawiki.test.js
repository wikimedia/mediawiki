...
QUnit.test( 'mw.Map', 40, function ( assert ) {
	var arry, conf, funky, globalConf, nummy, pCloned, pCount, pOrig, safeConf, safeConf2, safeConf3, someValues;
	...
	assert.strictEqual( safeConf3.unset( 'lorem' ), true, '.unset() returns true for a single key/value pair' );
	assert.strictEqual( safeConf3.get( 'lorem', 'deleted' ), 'deleted', '.unset() can remove a single key/value pair' );

	assert.strictEqual( safeConf3.unset( ['ipsum', 'dolor'] ), true, '.unset() returns true for multiple key/value pairs' );
	assert.deepEqual( safeConf3.get( ['ipsum', 'dolor'], 'deleted' ), {
		ipsum: 'deleted',
		dolor: 'deleted'
	}, '.unset() can remove multiple key/value pairs' );
