...
// @import (bug 31676)
	...
	QUnit.test( 'mw.hook', 10, function ( assert ) {
		...
		mw.hook( 'test.hook.variadic' )
			.add( callback, callback, function ( chr ) { chars.push( chr ); }, callback )
			.fire( 'x' )
			.remove( function () { 'not-added'; }, callback )
			.fire( 'y' )
			.remove( callback )
			.fire( 'z' );
		assert.deepEqual( chars, ['x', 'x', 'x', 'x', 'y', 'z'], '"add" and "remove" support variadic arguments. "add" does not filter unique. remove removes all equal by reference, remove is silent if the function is not found' );
	} );
