...
( function ( mw ) {
	...
	QUnit.test( 'mw.Map', function ( assert ) {
		if ( window.Map ) {
			assert.equal( new mw.Map().internalMap.constructor.name, 'Map', 'Uses native Map internally' );
		}
	...
	if ( window.Map ) {
		QUnit.test( 'mw.Map (StringMap fallback)', function ( assert ) {
			...
			assert.equal( new mw.Map().internalMap.constructor.name, 'StringMap', 'Uses StringMap internally' );
			testMwMap.call( this, assert );
