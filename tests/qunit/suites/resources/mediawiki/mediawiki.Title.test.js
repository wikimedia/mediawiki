( function ( mw, $ ) {
	...
	QUnit.test( 'newFromURL', function ( assert ) {
		var i, thisCase, title, prefix,
			...
			cases = [
				...
				{
					url: '/wiki/Some_Page#somesection',
					nameText: 'Some Page',
					fragment: 'somesection',
					namespace: 0
				},
				{
		...
		for ( i = 0; i < cases.length; i++ ) {
			...
			if ( thisCase.nameText !== undefined ) {
				...
				assert.equal( title.getFragment(),
					thisCase.fragment === undefined ? null : thisCase.fragment,
					prefix + 'Fragment matches original' );
			} else {
	...
	QUnit.test( 'newFromLink', function ( assert ) {
