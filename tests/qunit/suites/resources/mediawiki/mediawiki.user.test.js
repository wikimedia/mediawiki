( function ( mw, $ ) {
	...
	QUnit.test( 'user status', 11, function ( assert ) {
		...
		assert.strictEqual( mw.user.getId(), 0, 'user.getId() returns null when not logged-in (wgUserId is not set)' );
		...
		assert.strictEqual( mw.user.getId(), 123, 'user.getId() returns correct ID for logged in user' );
