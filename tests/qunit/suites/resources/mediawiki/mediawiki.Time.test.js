...
test( 'Create valid mw.Time object with Date object, and make instances of the ago message', 5, function ( assert ) {
...
test( 'Create valid mw.Time object with Date object, and make an MW-style timestamp', 1, function ( assert ) {
	var realDate = new Date( 2012, 03, 01, 10, 12, 14 );
	var mwTime = new mw.Time( realDate );
	assert.strictEqual( mwTime.getMwTimestamp(), '20120401101214' );
} );

test( 'Create a new mw.Time object with no arguments, make sure it is the current time', 1, function ( assert ) {
	var realDate = new Date();
	var mwTime = new mw.Time();
	assert.deepEqual( mwTime.dateObj, realDate );
} );

test( 'Intentionally cause an error', 1, function ( assert ) {
