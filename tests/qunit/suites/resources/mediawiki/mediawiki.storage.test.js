( function ( mw ) {
	QUnit.module( 'mw.storage', {
		tearDown: function () {
			mw.storage.localStorage = window.localStorage;
		}
	} );

	QUnit.test( 'set/get with localStorage', 2, function ( assert ) {
		...
		assert.strictEqual( mw.storage.get( 'foo' ), 'test' );
	} );
