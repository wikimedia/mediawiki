( function ( mw, $ ) {
	var mwLanguageCache = {}, oldGetOuterHtml, formatText, formatParse,
...
// HTML in wikitext
QUnit.test( 'HTML', 11, function ( assert ) {
	...
	assert.htmlEqual(
		...
		'&lt;script&gt;<a title="Foo" href="' + mw.util.wikiGetlink( 'Foo' ) + '">bar</a>&lt;/script&gt;',
	...
	mw.messages.set( 'jquerymsg-script-and-external-link', '<script>alert( "Test" );</script> [http://example.com <i>Foo</i> bar]' );
	assert.htmlEqual(
		formatParse( 'jquerymsg-script-and-external-link' ),
		'&lt;script&gt;alert( "Test" );&lt;/script&gt; <a href="http://example.com"><i>Foo</i> bar</a>',
		'HTML tags in external links not interfering with escaping of other tags'
	);

	mw.messages.set( 'jquerymsg-link-script', '[http://example.com <script>alert( "Test" );</script>]' );
	assert.htmlEqual(
		...
		'<a href="http://example.com"><span class="mediaWiki_htmlEmitter">&lt;script&gt;alert( "Test" );&lt;/script&gt;</span></a>',
