( function ( mw ) {
	...
	QUnit.module( 'mediawiki.template', {
		setup: function () {
			...
			this.templates = mw.templates.get( 'test.mediawiki.template' );

			// Register some templates
			mw.templates.set( 'test.mediawiki.template', {
					'test_templates_foo.xyz': 'goodbye',
					'test_templates_foo.abc': 'thankyou'
			} );
		},
		teardown: function() {
			// restore old value of templates
			mw.templates.set( 'test.mediawiki.template', this.templates );
		}
	} );
