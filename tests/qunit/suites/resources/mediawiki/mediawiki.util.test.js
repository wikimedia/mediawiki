( function ( mw, $ ) {
	...
	QUnit.test( 'isIPv6Address', 40, function ( assert ) {
		...
		for ( i = 0; i < IPV6_CASES.length; i++ ) {
			ipCase = IPV6_CASES[i];

			assert.strictEqual( mw.util.isIPv6Address( ipCase[1] ), ipCase[0], 'isIPv6Address: ' + ipCase[2] );
		}
	...
	QUnit.test( 'isIPv4Address', 11, function ( assert ) {
		...
		for ( i = 0; i < IPV4_CASES.length; i++ ) {
			ipCase = IPV4_CASES[i];

			assert.strictEqual( mw.util.isIPv4Address( ipCase[1] ), ipCase[0], 'isIPv4Address: ' + ipCase[2] );
		}
	} );

	QUnit.test( 'isIPAddress', 51, function ( assert ) {
		var i, ipCase, PREFIX = 'isIPAddress: ';

		for ( i = 0; i < IPV4_CASES.length; i++ ) {
			ipCase = IPV4_CASES[i];

			assert.strictEqual( mw.util.isIPv4Address( ipCase[1] ), ipCase[0], PREFIX + ipCase[2] );
		}

		for ( i = 0; i < IPV6_CASES.length; i++ ) {
			ipCase = IPV6_CASES[i];

			assert.strictEqual( mw.util.isIPv6Address( ipCase[1] ), ipCase[0], PREFIX + ipCase[2] );
		}
	} );
}( mediaWiki, jQuery ) );
