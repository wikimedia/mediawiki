( function ( mw, $ ) {
	...
	QUnit.test( 'addPortletLink', 11, function ( assert ) {
		...
		assert.equal( $tbMW.next()[0], tbRL, 'Link is in the correct position (by passing nextnode)' );
		...
		addedAfter = mw.util.addPortletLink( 'p-test-custom', '#', 'After foo', 'post-foo', 'After foo', null, $( tbRL ) );
		assert.strictEqual( $( addedAfter ).next()[0], tbRL, 'Link is in the correct position (by passing a jQuery object as ntextnode)' );
	} );
