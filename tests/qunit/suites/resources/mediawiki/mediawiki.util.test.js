( function ( mw, $ ) {
	...
	QUnit.test( 'getUrl', 12, function ( assert ) {
		...
		href = util.getUrl( 'Foo:Sandbox#', { action: 'edit' } );
	...
	QUnit.test( 'getParamValue', 5, function ( assert ) {
		...
		assert.strictEqual( util.getParamValue( 'foo', url ), null, 'Ignore hash if param is not in querystring but in hash (bug 27427)' );

		url = 'example.org?' + $.param( { TEST: 'a b+c' } );
		assert.strictEqual( util.getParamValue( 'TEST', url ), 'a b+c', 'Bug 30441: getParamValue must understand "+" encoding of space' );

		url = 'example.org?' + $.param( { TEST: 'a b+c d' } ); // check for sloppy code from r95332 :)
		assert.strictEqual( util.getParamValue( 'TEST', url ), 'a b+c d', 'Bug 30441: getParamValue must understand "+" encoding of space (multiple spaces)' );
	} );

	QUnit.test( 'tooltipAccessKey', 4, function ( assert ) {
		this.suppressWarnings();

		assert.equal( typeof util.tooltipAccessKeyPrefix, 'string', 'tooltipAccessKeyPrefix must be a string' );
		assert.equal( $.type( util.tooltipAccessKeyRegexp ), 'regexp', 'tooltipAccessKeyRegexp is a regexp' );
		assert.ok( util.updateTooltipAccessKeys, 'updateTooltipAccessKeys is non-empty' );

		'Example [a]'.replace( util.tooltipAccessKeyRegexp, function ( sub, m1, m2, m3, m4, m5, m6 ) {
	...
	QUnit.test( '$content', 2, function ( assert ) {
		assert.ok( util.$content instanceof jQuery, 'util.$content instance of jQuery' );
		assert.strictEqual( util.$content.length, 1, 'util.$content must have length of 1' );
	} );
