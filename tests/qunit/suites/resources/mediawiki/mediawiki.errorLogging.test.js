( function ( $, mw ) {
	...
	QUnit.module( 'mediawiki.errorLogging', QUnit.newMwEnvironment( config: { wgJavascriptErrorLoggingSamplingRate: true } ) );
	...
	QUnit.test( 'wgJavascriptErrorLoggingSamplingRate', 3, function ( assert ) {
		var w = getWindowMock( this.sandbox ),
			jq = getJqueryMock( this.sandbox );
		...
		// jscs:disable disallowDanglingUnderscores
		this.sandbox.stub( mw.config, 'get' ).withArgs( 'wgJavascriptErrorLoggingSamplingRate' ).returns( false );
		mw.errorLogging.register( w, jq );
