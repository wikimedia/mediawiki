( function (  mw, $ ) {
	var now = 9012; // milliseconds

	QUnit.module( 'mediawiki.cookie', QUnit.newMwEnvironment( {
		setup: function () {
			this.sandbox.spy( $, 'cookie' );

			this.clock = this.sandbox.useFakeTimers( now );
		},
		config: {
			...
			wgCookiePath: 'path',
			wgCookieSecure: true,
			wgCookieExpiration: 5678 // seconds
		}
	} ) );
	...
	QUnit.test( 'should return the value of the cookie', 1, function ( assert ) {
		...
		$.cookie.restore();
		this.sandbox.stub( $, 'cookie' ).returns( 'bar' );
	...
	QUnit.test( 'should return the default value when the cookie isn\'t set', 1, function ( assert ) {
		var value = mw.cookie.get( 'foo', null, 'bar' );
		assert.strictEqual( value, 'bar' );
	} );
