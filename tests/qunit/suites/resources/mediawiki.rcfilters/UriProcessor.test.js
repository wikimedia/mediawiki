...
( function ( mw, $ ) {
	...
	QUnit.test( 'getUpdatedUriQueryFromModel', function ( assert ) {
		var uriProcessor,
			filtersModel = new mw.rcfilters.dm.FiltersViewModel();
		...
		assert.deepEqual(
			uriProcessor.getUpdatedUriQueryFromModel( {} ),
		...
		assert.deepEqual(
			uriProcessor.getUpdatedUriQueryFromModel( { urlversion: '2' } ),
			{ urlversion: '2' },
			'Initially empty URI query with urlversion=2 does not merge defaults'
		);

		assert.deepEqual(
			uriProcessor.getUpdatedUriQueryFromModel( { filter1: '0' } ),
			{
				filter1: '1',
				filter4: '1',
				urlversion: '2'
			},
			// Filter state always overrides URL when the model is built.
			// The model gets populated on initialization from the URL params
			// and then, as it is synchronized, it is considered the only true
			// source of truth for the state. If the user manually changes the
			// parameters, it will trigger a reload that will also repopulate
			// the model again, re-sync it, and again consider it the source
			// of truth.
			// URL should never have a different state than the full model
			// while the system is working. It is only allowed to have it while
			// we initialize, on first build.
			'When getting an updated URL (with merging defaults) we consider only filter state and not query params.'
		);

		assert.deepEqual(
			uriProcessor.getUpdatedUriQueryFromModel( { filter1: '1', urlversion: '2' } ),
			{
				urlversion: '2'
			},
		...
		assert.deepEqual(
			uriProcessor.getUpdatedUriQueryFromModel( { foo: 'bar', urlversion: '2' } ),
			{
				foo: 'bar',
				urlversion: '2'
			},
		...
		assert.deepEqual(
			uriProcessor.getUpdatedUriQueryFromModel( { filter3: '1', urlversion: '2' } ),
			{
				...
				// model. If we ever manually change parameters in the URL, the
		...
		var uriProcessor,
			...
			cases = [
				...
				},
				{
					...
					// (See also testsgetUpdatedUriQueryFromModel for preservation of unrecognized params)
