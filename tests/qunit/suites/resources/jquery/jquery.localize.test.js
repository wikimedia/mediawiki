...
QUnit.test( 'Handle data text', 2, function ( assert ) {
	...
	assert.strictEqual( $( $lc[ 0 ] ).text(), mw.msg( 'option-one' ), 'data-msg-text becomes text of options' );
	assert.strictEqual( $( $lc[ 1 ] ).text(), mw.msg( 'option-two' ), 'data-msg-text becomes text of options' );
} );
