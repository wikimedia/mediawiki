( function ( mw, $ ) {
	...
	function listTest( listType, assert ) {
		...
		assert.equal( $contentItem.attr( 'value' ), '1', 'numeral order is forcefully preserved' );
	...
	QUnit.asyncTest( 'basic operation (<ul>)', 8, function ( assert ) {
		listTest( 'ul', assert );
	} );

	QUnit.asyncTest( 'basic operation (<ol>)', 8, function ( assert ) {
		listTest( 'ol', assert );
	} );
