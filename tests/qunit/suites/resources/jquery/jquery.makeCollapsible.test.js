( function ( mw, $ ) {
	...
	QUnit.test( 'predefined toggle using the mediawiki message toggle text (mw-collapsible-toggle/mw-collapsible-text)', function ( assert ) {
		var $collapsible = prepareCollapsible(
				'<div class="mw-collapsible">' +
					'<div class="mw-collapsible-toggle" style="text-align: center;>' +
						'<span>(</span><span class="mw-collapsible-text" style="float: none; text-align: center;">{{int:show}}</span><span>)</span>' +
					'</div>' +
		...
		assert.equal( $toggleText.text(), 'Hide', 'predefined toggle: mw msg is respected' );

		$collapsible.on( 'afterCollapse.mw-collapsible', function () {
			assert.equal( $toggleText.text(), 'Show', 'predefined toggle: text is toggled' );
		} );
