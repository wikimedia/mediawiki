( function ( mw, $ ) {
	...
	QUnit.test( 'addSuffix', data.length, function ( assert ) {
		$.each( data, function ( i, row ) {
			var expected = row[0],
				...
				frontSuffix = row[3],
				labialSuffix = row[4];
			assert.strictEqual( expected, mediawiki.language.addSuffix( word, backSuffix, frontSuffix, labialSuffix ) );
		} );
	} );
	...
	QUnit.test( 'article', data.length, function ( assert ) {
		$.each( data, function ( i, row ) {
			var expected = row[0],
				word = row[1];
			assert.strictEqual( expected, mediawiki.language.getArticle( word ) );
		} );
	} );
	...
	QUnit.test( 'convertGrammar', data.length, function ( assert ) {
		$.each( data, function ( i, row ) {
			var expected = row[0],
				...
				param3 = row[4],
				param4 = row[5];
			assert.strictEqual( expected, mediawiki.language.convertGrammar(
