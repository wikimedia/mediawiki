( function ( mw ) {
	...
	QUnit.test( '#saveOption calls #saveOptions with appropriate parameters', function ( assert ) {
		QUnit.expect( 2 );
	...
	QUnit.test( '#saveOptions', function ( assert ) {
		...
		this.server.respond(
			...
			[ 200, { 'Content-Type': 'application/json' },
				JSON.stringify( { "tokens": { "optionstoken": "+\\" } } ) ]
		);
			...
			switch ( request.requestBody ) {
				...
				case 'action=options&format=json&optionname=foo%7Cbar%3Dquux&token=%2B%5C':
					...
					request.respond( 200, { 'Content-Type': 'application/json' },
						JSON.stringify( { options: 'success' } ) );
					break;
				default:
					assert.ok( false, 'Unexpected request sent' );
			}
		} );
	} );
}( mediaWiki ) );
