/*global isMobileDevice: true, isCompatibleUserAgent: true */
( function ( $ ) {
	var mobileBrowsers =
			[
		...
		desktopBrowsers =
			[
				...
				// While not true desktop browsers, text browsers and crawlers are something that needs no special treatment
	...
	QUnit.test( 'isMobileDevice', mobileBrowsers.length + desktopBrowsers.length, function ( assert ) {
		$.each( mobileBrowsers, function( index, device ) {
				assert.equal( isMobileDevice( device[1] ), true, '"' + device[1] + '" is a mobile device' );
			}
		);
		$.each( desktopBrowsers, function( index, device ) {
				assert.equal( isMobileDevice( device[1] ), false, '"' + device[1] + '" is a desktop device' );
			}
		);
	} );

	QUnit.test( 'isCompatibleUserAgent', mobileBrowsers.length + desktopBrowsers.length, function ( assert ) {
		$.each( mobileBrowsers.concat( desktopBrowsers ), function( index, device ) {
				assert.equal( isCompatibleUserAgent( device[1] ), device[0],
