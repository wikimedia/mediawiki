...
( function ( $, mw, QUnit, undefined ) {
	...
	QUnit.test( 'htmlEqual', 6, function ( assert ) {
		assert.htmlEqual(
		...
		assert.htmlEqual(
			'&lt;script&gt;<a href="http://example.com">example</a>&lt;/script&gt;',
			'&lt;script&gt;<a href=\'http://example.com\'>example</a>&lt;/script&gt;',
			'Escaped text at outer level is handled correctly when equal'
		);

		assert.notHtmlEqual(
			'&lt;script&gt;<a href="http://example.com">example</a>&lt;/script&gt;',
			'&lt;script&gt;<a href=\'http://example.com\'>example</a>&lt;/scripts&gt;',
			'Escaped text at outer level is handled correctly when not equal'
		);
