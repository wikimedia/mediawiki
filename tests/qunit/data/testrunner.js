...
( function ( $, mw, QUnit ) {
	...
	/**
	 * Utility for supporting QUnit 1.x 'setup' and 'teardown' hooks
	 * (deprecated in QUnit 1.16, removed in QUnit 2).
-	 */
	function normLocalEnv( localEnv ) {
		if ( !localEnv ) {
			return {};
		}
		if ( localEnv.setup ) {
			localEnv.beforeEach = localEnv.beforeEach || localEnv.setup;
			delete localEnv.setup;
		}
		if ( localEnv.teardown ) {
			localEnv.afterEach = localEnv.afterEach || localEnv.teardown;
	...
	( function () {
		...
		QUnit.module = function ( name, localEnv, executeNow ) {
			...
			localEnv = normLocalEnv( localEnv );
