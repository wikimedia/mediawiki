...
class SuppressText extends Supressor {
	/**
	 * @param string $input
	 * @return bool do errors remain
	 */
...
class SuppressCodeClimate extends Supressor {
	/**
	 * @param string $input
...
class NoopSupressor extends Supressor {
	private $mode;
...
switch( $mode ) {
case 'text':
	$suppressor = new TextSupressor();
	break;
case 'codeclimate':
	$suppressor = new CodeClimateSupressor();
	break;
default:
	$suppressor = new NoopSupressor( $mode );
}
